...
class Open3rd extends BaseObject
{
    ...
    protected function curl($api, $data){
        try {
            return (new WechatHttpClient())->post($api, json_encode($data));
        }catch (WechatException $e){
            if($e->getRaw()['errcode'] == '40001' && strpos($e->getMessage(), "access_token is invalid or not latest") !== false){
                // componentAccessToken 过期重新更新并刷新浏览器
                ...
                \Yii::$app->response->redirect(\Yii::$app->request->absoluteUrl)->send();
            }
            throw $e;
        }
    }
