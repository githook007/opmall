...
class CommonCoupon extends Model
{
     ...
     * @throws Exception
     */
    public function getDetail($with = [])
    {
    ...
    public function receive(Coupon $coupon, UserCouponData $class, $content, $send_num = 1)
    {
        ...
        for ($i = 0; $i < $send_num; $i++) {
            ...
            $userCoupon->mch_id = $coupon->mch_id; // @czs
    ...
    {
        return UserCoupon::find()->where(['id' => $userCouponId, 'user_id' => $this->user->id, 'mall_id' => $this->mall->id, 'is_delete' => 0])
            ->with('coupon')->one();
    }
    ...
    // 检查会员权限
    public function checkMemberPower($couponMember){
        if(empty($couponMember) || count($couponMember) == 0 || !$this->user){
            return;
        }
        foreach ($couponMember as $item){
            if($item['member_level'] && $item['is_delete'] == 0){
                if($this->user->identity->member_level == $item['member_level']){
                    return;
                }
            }
        }
        throw new \Exception('会员专享优惠券，无权限');
    }
}
