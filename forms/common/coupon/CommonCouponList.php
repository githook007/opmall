...
class CommonCouponList extends BaseObject
{
    ...
    public function getList()
    {
        ...
        $receiveQuery = UserCoupon::find()->alias('uc')
            ...
            ->keyword($this->mall_mch_id >= 0, ['uc.mch_id' => $this->mall_mch_id]) // @czs
        ...
        $couponCenter = CouponCenter::find()->where([
            ...
            'is_delete' => 0
        ])->keyword($this->mall_mch_id >= 0, ['mch_id' => $this->mall_mch_id])->select('coupon_id'); // @czs
        ...
        $list = Coupon::find()->alias('c')
            ->where([
                ...
                'c.is_delete' => 0,
                'c.id' => $couponCenter
            ])->keyword($this->mall_mch_id >= 0, ['c.mch_id' => $this->mall_mch_id]) // @czs
            ->andWhere([
                'or',
                [
                    'and',
    ...
    public function getGoodsCoupons(Goods $goods)
    {
        ...
        $this->mall_mch_id = $goods->mch_id; // @czs
        $allList = $this->getList();
        $goodsCatRelations = $goods->goodsWarehouse->goodsCatRelation;
