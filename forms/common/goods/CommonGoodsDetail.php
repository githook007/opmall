...
class CommonGoodsDetail extends Model
{
    ...
    {
        $goods = Goods::find()->with(['attr.memberPrice', 'goodsWarehouse.cats', 'goodsCardRelation.goodsCards', 'goodsCouponRelation.goodsCoupons'])
            ->where([
                'is_delete' => 0,
                'id' => $id,
...
//            if (version_compare(\Yii::$app->getAppVersion(), '2.3.44') <= 0) {
//                $services[] = $item->name;
//            } else {
...
/* @var GoodsCardRelation $item */
    ...
    public function getAll($params = [], $ignore = [])
    {
        ...
        $result['video_channel'] = \yii\helpers\BaseJson::decode($this->goods->video_channel) ?: new \stdClass();
    ...
    public function getTemplateMessageList()
    {
        ...
        if (in_array(\Yii::$app->appPlatform, ['webapp', 'app'])) {
            return [];
        }
    ...
    {
        return [
            'attr', 'goods_num', 'goods_no', 'goods_weight', 'services',
            ...
            'guarantee_title', 'guarantee_pic', 'template_message_list'
        ];
    }
