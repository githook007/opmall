...
class AttachmentUpload extends Model
{
    ...
    {
        if ($this->mall_id > 0) {
            $extend = $this->mall->extend ?: $this->mall->extendObj();
            $usedMemory = $extend->used_memory + round($this->file->size / 1024 / 1024, 8);
            if (!$extend->memory || ($extend->memory > 0 && $extend->memory < $usedMemory)) {
                throw new \Exception('您的存储空间不足，请联系管理员');
            }
        }
        $dateFolder = date('Ymd');
        ...
        $attachment = $this->attachmentSave();
        if ($this->mall_id > 0 && isset($usedMemory)){
            $this->mall->extendObj(['used_memory' => $usedMemory]);
        }
        return $attachment;
    }
    ...
    public function attachmentSave()
    {
        ...
        $attachment->user_id = \Yii::$app->user->id ?? 0;
    ...
    private function saveToLocal()
    {
        ...
        if (\Yii::$app instanceof \yii\web\Application) {
            ...
            $baseWebUrl = \Yii::$app->hostInfo . \Yii::$app->baseUrl;
        }
