...
class OrderRefundForm extends Model
{
    ...
    public function triggerEvent($orderRefund)
    {
    ...
    private function refuse($orderRefund)
    {
        ...
        try {
            ...
            $this->triggerEvent($orderRefund); // @czs
    ...
    private function agree($orderRefund, $remark)
    {
        ...
        try {
            ...
            // 货到付款方式售后处理
            (new OrderRefund())->updatePayStatus($orderRefund);
    ...
    private function refund($orderRefund)
    {
        ...
        try {
            ...
            if ($orderRefund->is_refund == 2) {
                $paymentOrder = $orderRefund->order->paymentOrder;
            ...
            if ($goodsAttr['use_integral']) {
                ...
                \Yii::$app->currency->setUser($orderRefund->order->user)->integral->refund(
            ...
            if ($orderRefund->order->pay_type == 2) {
                ...
                if ($orderRefund->reality_refund_price > 0) {
                    ...
                    if ($orderRefund->order->sign == 'advance') { //预售订单售后退款，退款金额包含定金，判断中需要扣除
                        if (\Yii::$app->plugin->getInstalledPlugin('advance') && $advanceFun()) {
            ...
            $advance_refund = price_format(max($advance_refund, 0));
            $orderRefund->reality_refund_price = $price + $advance_refund;
            if (!$orderRefund->save()) {
                throw new \Exception($this->getErrorMsg($orderRefund));
            }
    ...
    private function autoConfirm($orderRefund)
    {
        ...
        if ($orderRefund->order->is_confirm == 0) {
            ...
            if ($num == 0) {
                if($orderRefund->order->is_send == 0){ // @czs
                    $orderRefund->order->is_send = 1;
