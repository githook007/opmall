...
class CommonPageTwo extends Model
{
    ...
    {
        $query = DiyPage::find()->select('id,title,show_navs,is_home_page,access_limit')
            ->where([
                'mall_id' => $this->mall->id,
                'is_disable' => 0,
                'is_delete' => 0,
...
//            $query->findInSet(\Yii::$app->appPlatform, 'platform');
            $query->andWhere([
                'or',
                'find_in_set(\'' . \Yii::$app->appPlatform . '\', `platform`)',
                ['is_home_page' => 1]
            ])->orderBy(['platform' => SORT_DESC, 'is_home_page' => SORT_DESC]);
        } else {
            throw new \Exception('页面不存在');
        }
        ...
        foreach ($model['navs'] as &$nav) {
            ...
            ////////权限判断
            $access_limit = \yii\helpers\BaseJson::decode($this->model['access_limit']);
            if ($access_limit && is_array($access_limit) && intval($access_limit['is_all']) !== 1) {
                $levels = array_column($access_limit['member'], 'level');
                if (!\Yii::$app->user->identity || !in_array(\Yii::$app->user->identity->identity->member_level, $levels)) {
                    $nav['template']['data'] = [[
                        'id' => 'app-nav-bar',
                        'permission_key' => '',
                        'data' => (new CommonFormat())->appNavBar()
                    ], [
                        'id' => 'access_limit',
                        'permission_key' => '',
                        'data' => $access_limit
    ...
    public function getPage($pageId = null, $isIndex = false, $oldData = null)
    {
                ...
                if ($item['id'] === 'module') {
                    if (isset($item['data']['list']) && is_array($item['data']['list'])) {
                        foreach ($item['data']['list'] as $key1 => $module) {
                            ...
                            $module['data'] = array_merge($module['data']);
        ...
        //区分
        $data = $this->HFxhb($pageId == 0, $data);
        // 将搜索组件放到首位 @czs
        $temp = [];
        foreach ($data as $key => $datum) {
            if ($datum['id'] == 'search' && $datum['data']['typeStyle'] == '1') { // 判断是沉浸模式
                $temp = $datum;
                unset($data[$key]);
                break;
            }
        ...
        !in_array('app-nav-bar', array_column($data, 'id')) && array_push($data, [
            'id' => 'app-nav-bar',
            'permission_key' => '',
            'data' => $formatModel->appNavBar()
        ]);
        //日志
    ...
    protected function sRubik(bool $is_show, array $arr)
    {
        if ($is_show) {
            foreach ($arr['data']['list'] as $rIndex => $rItem) {
                try {
                    if ($arr['data']['style'] == 0 && isset($arr['data']['a_height'])) {
                        $arr['data']['list'][$rIndex]['height'] = 'calc(100% + 0px)';
                        $arr['data']['height'] = $arr['data']['a_height'];
                    }
                ...
                if(empty($rItem['link'])){ // @czs app兼容
                    $arr['data']['list'][$rIndex]['link'] = new \stdClass();
                }
            }
    ...
    {
        if ($is_show) {
            $diyVideoForm = new DiyVideoForm();
            $arr['data'] = $diyVideoForm->getNewData($arr['data']);
            return $arr;
        } else {
            $selectIds = array_column(array_column(array_filter($arr, function ($item) {
                return $item['data']['addType'] === 'auto' && $item['data']['video_id'];
            }), 'data'), 'video_id');
            empty($selectIds) || (new DiyVideoForm())->getVideoList($selectIds);
        }
    }
    ...
    protected function sUserOrder(bool $is_show, array $arr)
    {
        if ($is_show) {
            ...
            foreach ($arr['data']['navs'] as $key => $value) {
                ...
                if ((int)$key + 1 === 5) {
                    $arr['data']['navs'][$key]['url'] = '/pages/order/refund/index';
                }
    ...
    //步数宝
    protected function sStep(bool $is_show, array $arr)
    {
        if ($is_show) {
            $diyStepForm = new DiyStepForm();
            $arr['data'] = $diyStepForm->getNewGoods($arr['data'], $this->stepGoods);
            return $arr;
        } else {
            $diyStepForm = new DiyStepForm();
            $stepGoodsIds = [];
            foreach ($arr as $goods) {
                $stepGoodsIds = array_merge(
                    $diyStepForm->getGoodsIds($goods['data']),
                    $stepGoodsIds
                );
            }
            $this->stepGoods = $diyStepForm->getGoodsById($stepGoodsIds);
        }
    }

    //标题
    protected function sLink(bool $is_show, array $arr)
    {
        if ($is_show) {
            if(empty($arr['data']['link'])){
                $arr['data']['link'] = new \stdClass();
            }
            return $arr;
        }
    }
