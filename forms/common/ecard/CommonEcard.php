        ...
        try {
            ...
            foreach ($orderDetailList as $orderDetail) {
                try {
                    ...
                    \Yii::warning('不是虚拟商品');
                    $exception->getCode() == 10 ? \Yii::warning($exception->getMessage()) : \Yii::warning($exception);
                    continue;
                }
            }
            if ($count != count($orderDetailList)) {
                throw new \Exception('不全是虚拟商品', 10);
            }
            if ($order->status == 1) {
                $this->autoSend($order);
            }
    ...
    protected function setEcard(OrderDetail $detail, $order)
    {
        ...
        if ($ecardOrder) {
            ...
            if (count($list) != $detail->num) {
                throw new \Exception('卡密数据库存不足', 10);
            }
            $data = [];
        ...
        try {
            ...
            foreach ($orderDetailList as $orderDetail) {
                try {
                    ...
                    \Yii::warning('不是虚拟商品');
                    \Yii::warning($exception->getMessage() . '= Line : ' . $exception->getLine() . " = File: " . $exception->getFile());
                    continue;
                }
            }
            $transaction->commit();
