...
class MpTemplateMsgData extends Model
{
    ...
    public function newOrderTpl($config, $params = array())
    {
        ...
        if (!isset($params['is_pay'])) {
            throw new \Exception('数组缺少参数is_pay');
        }
        ...
        if(is_string($params['is_pay'])){
            $status = $params['is_pay'];
        }
        $remark = $params['remark'] ?? ($select[$params['sign']] ?? '商城订单') . "，购买人：{$params['user']}";
        return [
            ...
            'data' => [
                ...
                'keyword3' => [
                    'value' => $params['total_pay_price'] ?? 0,
                    'color' => '#000000',
                ],
                'keyword4' => [
                    'value' => $status ?? ($params['is_pay'] == 0 ? '待付款' : '已付款'),
                    'color' => '#000000',
                ],
                'keyword5' => [
                    'value' => $params['time'],
