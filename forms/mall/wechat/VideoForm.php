...
class VideoForm extends Model
{
    ...
    public function save()
    {
        ...
        try {
            $this->video_number_member_list = is_array($this->video_number_member_list) ? $this->video_number_member_list : [];
            $this->video_number_template_list = is_array($this->video_number_template_list) ? $this->video_number_template_list : [];
            if ($this->is_video_number) {
                ...
                if ($this->is_video_number_member && !count($this->video_number_member_list)) {
                    throw new \Exception('请选择视频号分销等级');
                }
