...
class BasicForm extends Model
{
    ...
    public function save()
    {
        ...
        try {
            ...
            $where = [];
            if($this->share_goods_status == 2 && $this->share_goods_warehouse_id){
                $where[] = ['not in', 'id', $this->share_goods_warehouse_id];
            }elseif($this->share_goods_status == 3 && $this->cat_list){
                $goodsId = GoodsCatRelation::find()->where(['cat_id' => $this->cat_list, 'is_delete' => 0])
                    ->select('goods_warehouse_id');
                $where[] = ['not in', 'id', $goodsId];
            }
            if($where){
                array_unshift($where, 'and');
                $where[] = ["mall_id" => \Yii::$app->mall->id];
                ShareLevelGoods::updateAll(["is_delete" => 1], $where);
            }
            return [
                'code' => 0,
                'msg' => '保存成功',
    ...
    public function search()
    {
        ...
        $list['cat_list'] = []; // @czs 修复页面加载会vue警告
