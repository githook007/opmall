...
class CouponForm extends Model
{
    ...
    public function rules()
    {
        return [
            [['couponMember'], 'trim'],
            [['id', 'mall_id', 'type', 'total_count', 'is_join', 'sort', 'expire_type', 'expire_day',
    ...
    public function getList()
    {
        ...
        $query = Coupon::find()->alias('c')->where([
            ...
            'c.is_delete' => 0,
            "c.mch_id" => \Yii::$app->user->identity->mch_id // @czs
        ]);
        if ($this->is_expired) {
            $query->andWhere([
                'or',
    ...
    public function destroy()
    {
        ...
        $model = Coupon::findOne([
            ...
            "mch_id" => \Yii::$app->user->identity->mch_id, // @czs
            'is_delete' => 0
        ]);
        if (!$model) {
            return [
    ...
    public function editCenter()
    {
        ...
        if ($this->is_join) {
            if (!$couponCenter) {
                ...
                $form->mch_id = \Yii::$app->user->identity->mch_id; // @czs
    ...
    public function save()
    {
        ...
        $model->mch_id = \Yii::$app->user->identity->mch_id; // @czs
