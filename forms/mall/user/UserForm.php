...
class UserForm extends Model
{
    ...
    public function getList()
    {
        ...
        $orderSum = Order::find()->where(['mall_id' => \Yii::$app->mall->id, 'is_delete' => 0, 'is_pay' => 1])
            ->andWhere('user_id = u.id')->select(['COALESCE(SUM(`total_pay_price`),0)']);
        //未发货 成功取消的订单金额
        ...
        $orderSumRefund = Order::find()->alias('o')->where(['o.mall_id' => \Yii::$app->mall->id, 'o.is_delete' => 0, 'o.is_pay' => 1])
            ...
            ->andWhere(['re.status' => 2, 're.is_refund' => 1])
        ...
        switch ($this->sort) {
            ...
            case 'order_count_asc':
                ...
                break;
            case 'coupon_count_desc':
                $query->orderBy('coupon_count DESC');
                break;
            case 'coupon_count_asc':
                $query->orderBy('coupon_count ASC');
                break;
        ...
        foreach ($list as &$v) {
            if (isset($memberList[$v['member_level']])) {
                ...
                $v['member_name'] = !empty($userCenterConfig['general_user_text']) ? $userCenterConfig['general_user_text'] : '普通用户';
            }
            ...
            $length = strlen($v['order_sum']);
            if ($length > 11) { //亿单位
                $v['order_sum'] = substr_replace(substr($v['order_sum'], 0, -10), '.', -1, 0) . "亿";
            } elseif ($length > 7) { //万单位
                $v['order_sum'] = substr_replace(substr($v['order_sum'], 0, -6), '.', -1, 0) . "w";
            }
        }
        unset($v);
        return [
            ...
            'data' => [
                ...
                'general_user_text' => !empty($userCenterConfig['general_user_text']) ? $userCenterConfig['general_user_text'] : '普通用户',
            ],
        ];
    }
