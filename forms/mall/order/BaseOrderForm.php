...
abstract class BaseOrderForm extends Model
{
    ...
    public function search()
    {
        ...
        foreach ($list as &$item) {
            ...
            if ($item['sign'] == '' && $item['mch_id'] == 0) {
                ...
                $item['plugin_name'] = $item['mch']['store']['name'] ?? '多商户';
            } else {
                try {
            ...
            foreach ($item['detail'] as $key => &$detail) {
                ...
                if (in_array($item['sign'], $orderSign)) {
                    ...
                    $detail['plugin_name'] = $detail['goods']['mch']['store']['name'] ?? '多商户';
                } else {
                    try {
            ...
            // 对接第三方聚合配送加的判断 @czs
            if(isset($item['detailExpress'][0]) && $item['detailExpress'][0]['express_type'] == '聚合配送'){
                $item['is_update_send'] = 0;
            }
    ...
    protected function where()
    {
        ...
        $query->keyword($this->status == -1, ['AND', ['o.is_recycle' => 0]])
