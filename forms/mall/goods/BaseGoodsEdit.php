    ...
    public function rules()
    {
        return [
            ...
            [['use_attr', 'goods_num', 'virtual_sales', 'goods_weight', 'individual_share',
                'share_type', 'attr_setting_type', 'sort', 'is_level', 'id',
                'confine_count', 'give_integral', 'give_integral_type', 'forehead_integral_type',
                'accumulative', 'freight_id', 'shipping_id', 'is_level_alone', 'is_default_services',
            ...
            [['cats', 'mchCats', 'services', 'cards', 'attr', 'attrGroups', 'member_price', 'video_channel',
                'select_attr_groups', 'shareLevelList', 'plugin_data', 'coupons', 'send_type', 'sell_time'], 'safe'],
            [['virtual_sales', 'freight_id', 'is_level', 'is_level_alone', 'forehead_integral',
                'give_integral', 'individual_share', 'is_level_alone', 'share_type', 'accumulative',
            ...
            [['price', 'give_integral', 'give_balance', 'forehead_integral', 'confine_count',
...
//        $goods->forehead = $this->forehead;
        ...
        $goods->video_channel = \Yii::$app->serializer->encode($this->video_channel);
    ...
    protected function setGoodsShare($goodsAttrId, $shareLevelList)
    {
        ...
        if (!$this->isNewRecord) {
            $res = GoodsShare::find()
                ->where(['goods_id' => $this->goods->id, 'goods_attr_id' => $goodsAttrId])
                ->all();
            /* @var GoodsShare[] $res */
            foreach ($res as $item) {
                $item->is_delete = 1;
