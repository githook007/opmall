...
class ShareForm extends Model
{
    ...
    public function search()
    {
        ...
        if (!$user->identity->is_distributor) {
            return [
        ...
        $first_count = $setting > 0 ? count($team->info($user->id, 1)) : 0;
        $second_count = $setting > 1 ? count($team->info($user->id, 2)) : 0;
        $third_count = $setting > 2 ? count($team->info($user->id, 3)) : 0;
        ...
        $shareLevel = CommonShareLevel::getInstance()->getShareLevelByLevel($user->share->level);
