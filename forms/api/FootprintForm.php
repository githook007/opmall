    ...
    public function footprint()
    {
        ...
        try {
            $query = FootprintGoodsLog::find()->alias('fg')
                ...
                ->andWhere([
                    'fg.mall_id' => \Yii::$app->mall->id,
                    'fg.user_id' => \Yii::$app->user->id,
                    'fg.is_delete' => 0,
                    'g.status' => 1
                ])
            ...
            switch ($this->time_type){
                case 1: // 今日
                    ...
                    break;
                case 2: // 昨日
                    $this->start_time = date("Y-m-d 00:00:00", mktime(0, 0, 0, date('m'), date('d') - 1, date('Y')));
                    $this->end_time = date("Y-m-d 23:59:59", mktime(0, 0, 0, date('m'), date('d'), date('Y')) - 1);
                    break;
                case 3: // 7天
                    $this->start_time = date("Y-m-d 00:00:00", mktime(0, 0, 0, date('m'), date('d') - 7, date('Y')));
                    $this->end_time = date("Y-m-d 23:59:59");
                    break;
                case 4: // 30天
                    $this->start_time = date("Y-m-d 00:00:00", mktime(0, 0, 0, date('m'), date('d') - 30, date('Y')));
                    $this->end_time = date("Y-m-d 23:59:59");
                    break;
            }
            if ($this->start_time) {
