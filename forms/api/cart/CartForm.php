...
class CartForm extends Model
{
    ...
    public function search()
    {
        ...
        foreach ($list as $item) {
            ...
            $this->handleAttr($newItem, $item, $userIdentity);
        ...
        // 加入插件商品
        $list = $this->getPluginCarts($newList);

        // 将数据按商城 商户区分
        $newList = $this->getResultData($list);
    ...
    public function getResultData($list){
        $newDataList = [];
        foreach ($list as $item) {
    ...
    public function getPluginCarts($list){
    ...
    public static function nums()
    {
        ...
        while (true) {
            /* @var Cart[] $list */
            $list = Cart::find()->with(['goods.mallGoods'])->where([
                'user_id' => \Yii::$app->user->id, 'is_delete' => 0, 'mall_id' => \Yii::$app->mall->id
            ...
            foreach ($list as $item) {
                ...
                if ($item->sign == '' && $item->goods->mallGoods && $item->goods->mallGoods->is_negotiable == 1) {
                    continue;
                }
                $sum += $item->num;
            }
