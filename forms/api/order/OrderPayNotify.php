    ...
    public function notify($paymentOrder)
    {
        ...
        switch ($paymentOrder->payType) {
            ...
            case PaymentOrder::PAY_TYPE_POS:
                $order->pay_type = 5;
                break;
            case PaymentOrder::PAY_TYPE_WECHAT_PAY_FOR_ANOTHER:
                $order->pay_type = 6;
                break;
            default:
                break;
        ...
        if($paymentOrder->openid && $paymentOrder->openid != $order->user->username){
            // 代付
            $user = User::find()->where(['username' => $paymentOrder->openid, "mall_id" => $order->mall_id])->one();
            $order->replace_user_id = $user->id ?? 0;
        }
        $order->save();
