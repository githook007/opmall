...
class OrderSubmitJob extends BaseJob implements JobInterface
{
    ...
    public function execute($queue)
    {
        ...
        // @czs 等待上次请求的商品生成订单，在多队列情况下才生效使用
        $redisKey = "goods_submit_order_";
        do {
            $wait = false;
            foreach ($this->form_data['list'] as $form_datum) {
                foreach ($form_datum['goods_list'] as $item) {
                    if (\Yii::$app->redis->get($redisKey . $item['id'] . $item['goods_attr_id'])) {
                        $wait = true;
                        break;
                    }
                }
                if($wait){
                    usleep(rand(200, 800) . '000'); // 休息200 - 800毫秒
                    break;
                }
            }
        } while ($wait);
        foreach ($this->form_data['list'] as $form_datum) {
            foreach ($form_datum['goods_list'] as $item) {
                \Yii::$app->redis->setex($redisKey . $item['id'] . $item['goods_attr_id'], 60, $item['id']);
            }
        }
        ...
        try {
            ...
            // 多地址处理 @czs
            $data = $form->handleMultipleAddress($data);
            ...
            foreach ($data['mch_list'] as $mchItem) {
                ...
                $order->distance = $mchItem['form_data']['distance'] ?? 0;//同城距离
                $order->words = '';
                $order->is_pay = 0;
                $order->pay_type = 0;
                $order->is_send = 0;
                ...
                if (!empty($mchItem['vip_card_detail'])) {
                    ...
                    $vipCardDetailModel[$vipCardDetail->id] = $vipCardDetail;

                    $tempMchItem = (new \app\plugins\vip_card\Plugin())->vipDiscount($mchItem, true, $form);
                    if(!isset($orderDetailVipCardInfoData[$vipCardDetail->id])){
                        $orderDetailVipCardInfoData[$vipCardDetail->id] = [];
                    }
            ...
            foreach ($vipCardDetailModel as $vipCardDetail) {
                $vipCardJob = new \app\plugins\vip_card\jobs\OrderSubmitJob([
                    'mall' => $this->mall,
                    'user' => $this->user,
                    'id' => $vipCardDetail->id,
                    'token' => $this->token,
                    'orderDetailVipCardInfoData' => $orderDetailVipCardInfoData[$vipCardDetail->id],
                ]);
    ...
    public function delete($redisKey){
        // @czs 等待上次请求的商品生成订单，在多队列情况下才生效使用
        foreach ($this->form_data['list'] as $form_datum) {
            foreach ($form_datum['goods_list'] as $item) {
                \Yii::$app->redis->del($redisKey . $item['id'] . $item['goods_attr_id']);
            }
        }
    }
}
