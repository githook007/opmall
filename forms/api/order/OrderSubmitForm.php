...
class OrderSubmitForm extends Model
{
    ...
    public function getXAddress($addressId = null, $type = 0)
    {
        if($addressId === null){
            $addressId = $this->form_data['address_id'] ?? 0;
        }
        $key = $type . $addressId;
        return $this->xAddress[$key];
    }

    public function setXAddress($val, $addressId = null, $type = 0)
    {
        if($addressId === null){
            $addressId = $this->form_data['address_id'] ?? 0;
        }
        $key = $type . $addressId;
        $this->xAddress[$key] = $val;
        return $this;
    }
    ...
    // 多地址处理 @czs
    public function handleMultipleAddress($data){
        $newMchList = [];
        foreach ($data['mch_list'] as $mchItem) {
            if ($mchItem['is_goods_address']) {
                $addressList = [];
                foreach ($mchItem['goods_list'] as $goodsItem) {
                    if(!empty($goodsItem['address'])){
                        $address = $goodsItem['address'];
                    }else{
                        $address = $mchItem['address'];
                    }
                    if(isset($addressList[$address['id']])){
                        $addressList[$address['id']]['goods_list'][] = $goodsItem;
                    }else {
                        $newItem = $mchItem;
                        $newItem['address'] = $address;
                        $newItem['goods_list'] = [$goodsItem];

                        $newItem['total_price'] = 0;
                        $newItem['total_goods_original_price'] = 0;
                        $newItem['member_discount'] = 0;
                        $newItem['coupon']['coupon_discount'] = 0;
                        $newItem['integral']['use_num'] = 0;
                        $newItem['integral']['deduction_price'] = 0;
                        $newItem['full_reduce_discount'] = 0;
                        if (empty($newItem['vip_card_detail']) && isset($mchItem['vip_discount'])) {
                            $newItem['vip_discount'] = 0;
                        }

                        $addressList[$address['id']] = $newItem;
                    }
                }

                if(count($addressList) > 1){
                    $original_express_price = $mchItem['express_price'];
                    $express_average = price_format($mchItem['express_price'] / $mchItem['total_price']);

                    $index = 0;
                    foreach ($addressList as &$item){
                        foreach ($item['goods_list'] as $goodsItem){
                            $item['total_price'] += $goodsItem['total_price'];
                            $item['member_discount'] += $goodsItem['member_discount'];
                            $item['total_goods_original_price'] += $goodsItem['total_original_price'];
                            $item['full_reduce_discount'] += $goodsItem['full_reduce_discount'] ?? 0;
                            $item['coupon']['coupon_discount'] += $goodsItem['coupon_discount'] ?? 0;
                            $item['integral']['use_num'] += $goodsItem['goods_attr']['use_integral'] ?: 0;
                            $item['integral']['deduction_price'] += $goodsItem['goods_attr']['integral_price'] ?: 0;
                            if (empty($item['vip_card_detail']) && isset($mchItem['vip_discount'])) {
                                $item['vip_discount'] += $goodsItem['vip_discount'] ?? 0;
                            }
                        }

                        $express_price = price_format($item['total_price'] * $express_average);
                        if ($original_express_price < $express_price || ($index == count($addressList) - 1 && $original_express_price > 0)) {
                            $express_price = $original_express_price;
                        }
                        $item['express_price'] = $express_price;
                        $original_express_price -= $express_price;

                        $item['total_goods_price'] = $item['total_price'];
                        $item['total_price'] = price_format($item['total_goods_price'] + $item['express_price']);
                        $item['total_discounts_price'] = price_format($item['total_goods_original_price'] - $item['total_price']);
                        if($item['coupon']['coupon_discount'] == 0) {
                            $item['coupon'] = [
                                'enabled' => true,
                                'use' => false,
                        ...
                        // 超级会员卡开卡标识，用户选择开通会员卡
                        if (!empty($item['vip_card_detail'])) {
                            $mchItemWithTempVip = (new \app\plugins\vip_card\Plugin())->vipDiscount($item, true, $this);
                            if (isset($mchItemWithTempVip['vip_discount'])) {
                                $item['temp_vip_discount'] = $mchItemWithTempVip['vip_discount'];
                            }
                        }
                ...
                $newMchList = array_merge($newMchList, array_values($addressList));
            }else{
                $newMchList[] = $mchItem;
            }
        }
        $data['mch_list'] = $newMchList;
        return $data;
    }
    ...
    public function submit()
    {
        ...
        $dataArr = [
            'mall' => \Yii::$app->mall,
            'user' => $this->getUser(),
            'form_data' => (array)$this->form_data,
            'token' => $token,
            'sign' => $this->sign,
        ...
        if(isset(\Yii::$app->kafka)){
            $queueId = \Yii::$app->kafka->delay(0)->push($class);
        }else {
            $queueId = \Yii::$app->queue2->delay(0)->push($class);
        }
     ...
     * @throws \yii\db\Exception
     * @throws \app\bootstrap\exceptions\ClassNotFoundException
     */
    public function getAllData()
    {
        ...
        foreach ($listData as &$mchItem) {
            ...
            $mchItem['remark'] = $mchItem['form_data']['remark'] ?? null;
            $mchItem['order_form_data'] = $mchItem['form_data']['order_form'] ?? null;

            $totalGoodsPrice = 0;
            $totalGoodsOriginalPrice = 0;
            foreach ($mchItem['goods_list'] as $goodsItem) {
                $totalGoodsPrice += $goodsItem['total_price']; // 商品金额
                $totalGoodsOriginalPrice += $goodsItem['total_original_price'];
            }
            $mchItem['total_goods_price'] = price_format($totalGoodsPrice); // 订单里总商品金额，优惠金额会处理 total_price 和 total_goods_price
            $mchItem['total_goods_original_price'] = price_format($totalGoodsOriginalPrice); // 订单里总商品的原件金额
            ...
            $total_price += $mchItem['total_price'];
            // 总折扣
            $mchItem['total_discounts_price'] = price_format($mchItem['total_goods_original_price'] - $mchItem['total_price']);

            if (!$this->getAddressEnable($mchItem)) {
                $addressEnable = false;
            }
        }
        unset($mchItem);
        ...
        if (!$allZiti) {
            foreach ($listData as &$mchItem) {
                ...
                // 拼装地址，多地址 - 前端不想做
                foreach ($mchItem['goods_list'] as $i => $goodsList){
                    if(!isset($goodsList['address'])){
                        continue;
                    }
                    $item = $mchItem['goods_list'][$i]['address'];
                    $mchItem['goods_list'][$i]['address'] = ArrayHelper::toArray($item);
                    $mchItem['goods_list'][$i]['address']['address'] = $item['province'] . $item['city'] . $item['district'] . $item['detail'];
                    unset($item);
                }
            }
        } else {
            ...
            // 下单预览记住用户更改的自提信息
            if (!empty($this->form_data['address']['name']) && !empty($this->form_data['address']['mobile'])) {
    ...
    {
        $listData = [];
        foreach ($formMchList as $formMchItem) {
            $mchItem = [
    ...
    protected function getGoodsListData($goodsList)
    {
        ...
        foreach ($goodsList as $item) {
            ...
            $result['form_data'] = $item['form_data'] ?? null;
    ...
    protected function getGoodsItemData($item)
    {
        ...
        try {
            ...
            if (!$attrList) {
                throw new OrderException('规格有变更，请重新下单');
            }
    ...
    protected function setMemberDiscountData($mchItem)
    {
        ...
        foreach ($mchItem['goods_list'] as &$goodsItem) {
            ...
            if ($memberUnitPrice && is_numeric($memberUnitPrice) && $memberUnitPrice >= 0) {
                ...
                if ($memberSubPrice != 0) {
                    // 减去会员优惠金额
                    $memberSubPrice = price_format(min($goodsItem['total_price'], $memberSubPrice));
                    ...
                    $goodsItem['member_discount'] = $memberSubPrice;
                }
            }
        }
        if ($totalSubPrice) {
    ...
    public function setFullReduceDiscountData($mchItem)
    {
        ...
        $subPrice = price_format(min($totalGoodsPrice, $sub, $mchItem['total_goods_price']));
        $resetPrice = $subPrice;
        if ($subPrice) {
            $mchItem['full_reduce_discount'] = $subPrice;
        }
        foreach ($mchItem['goods_list'] as $index => &$goods) {
            if (in_array($goods['goods_warehouse_id'], $fullReduceGoodsIdList)) {
                ...
                $subFullReducePrice = price_format(min($goodsPrice, $goods['total_price']));
                $goods['total_price'] -= $subFullReducePrice;

                $goods['discounts'][] = [
                    'name' => '满减优惠',
                    'value' => '-' . $subFullReducePrice
                ];
                $mchItem['total_goods_price'] = price_format($mchItem['total_goods_price'] - $subFullReducePrice);
                $goods['full_reduce_discount'] = $subFullReducePrice;
            }
        }
    ...
    protected function setCouponDiscountData($mchItem, $formMchItem)
    {
        ...
        if ($subPrice > 0) {
            ...
            $mchItem['coupon']['coupon_discount'] = $subPrice;
    ...
    protected function setIntegralDiscountData($mchItem, $formMchItem)
    {
        ...
        return $this->setGoodsListIntegralSub($mchItem);
    }
    ...
    private function setGoodsListIntegralSub($mchItem)
    {
        ...
        foreach ($mchItem['goods_list'] as &$goodsItem) {
            /* @var OrderGoodsAttr $orderGoodsAttr */
            $orderGoodsAttr = $goodsItem['goods_attr'];
            $goodsItem['total_price'] = price_format($goodsItem['total_price'] - $orderGoodsAttr->integral_price);
        }
    ...
    protected function getSendType($mchItem)
    {
        ...
        return \Yii::$app->mall->getMallSettingOne('send_type');
    }
    ...
    protected function setDeliveryData($mchItem, $formMchItem)
    {
        ...
        if (!in_array($formMchItem['send_type'], $sendType)) {
            if (\Yii::$app->appPlatform == 'ttapp' && $formMchItem['send_type'] == 'city') {
     ...
     * @param integer $type  类型：0快递 1同城
     * @return Address|\yii\db\ActiveRecord|null
     */
    protected function getAddress($addressId = null, $type = 0)
    {
        /** @var User $user */
        $user = $this->getUser();
        if($addressId === null){
        ...
        $key = $type . $addressId;
        if (isset($this->xAddress[$key])) {
            return $this->xAddress[$key];
        }
        $this->xAddress[$key] = Address::find()
            ->where([
                'user_id' => $user->id,
                'is_delete' => 0,
                'type' => $type,
            ])->keyword($addressId, ['id' => $addressId])
            ->keyword(!$addressId, ['is_default' => 1])
            ->one();
        return $this->xAddress[$key];
    }
    ...
    protected function setExpressData($mchItem)
    {
        ...
        if ($mchItem['delivery']['send_type'] == 'city') { // 同城不支持多地址 @czs
            $address = $this->getAddress($mchItem['form_data']['address_id'] ?? 0, 1);
        } else {
            $address = $this->getAddress();
            $goodsAddress = [];
            foreach ($mchItem['form_data']['goods_list'] as $formData) {
                if(!empty($formData['address_id'])){
                    $goodsAddress[$formData['goods_attr_id']] = $this->getAddress($formData['address_id']);
                }
            }
            if($goodsAddress){
                foreach ($mchItem['goods_list'] as &$item){
                   if(isset($goodsAddress[$item['goods_attr']['id']])){
                       $item['address'] = $goodsAddress[$item['goods_attr']['id']];
                   }
                }
                $mchItem['is_goods_address'] = 1; // 多地址
                unset($item);
                if(!$address){
                    $address = array_values($goodsAddress)[0];
        ...
        if ($mchItem['delivery']['send_type'] == 'city') { // 同城配送
            try {
                ...
                if (!($address->longitude && $address->latitude)) {
                    ...
                    $this->setXAddress(null, $mchItem['form_data']['address_id'], 1);
                    // 没有设置定位
                    $mchItem['city']['error'] = '未选择收货地址';
                    return $mchItem;
                }
        ...
        foreach ($mchItem['goods_list'] as $goodsItem) {
            ...
            $condition = $this->getCondition($freeDelivery, $goodsItem['address'] ?? $address);
            if ($condition == -1) {
                $needPostageGoodsList[] = $goodsItem;
                continue;
            }
        ...
        foreach ($needPostageGoodsList as $goodsItem) {
            $postageRule = $this->getPostageRules($goodsItem['freight_id'], $mchItem['mch']['id']);
            if (!$postageRule) {
                continue;
            }

            $addressTemp = $goodsItem['address'] ?? $address;
            ...
            foreach ($ruleDetails as $ruleDetail) {
                foreach ($ruleDetail['list'] as $district) {
                    if ($district['id'] == $addressTemp->province_id) {
                        $rule = $ruleDetail;
                        break;
                    } elseif ($district['id'] == $addressTemp->city_id) {
                        $rule = $ruleDetail;
                        break;
                    } elseif ($district['id'] == $addressTemp->district_id) {
                        $rule = $ruleDetail;
                        break;
                    }
                }
        ...
        unset($group);
        // 续费价格，首件价最低的规则的续费量是所有
        $priceFun = function($rule, $is_max_first_price, $totalNum, $secondPrice) {
            if ($rule['second'] > 0) {
                if ($is_max_first_price) {
        ...
        foreach ($postageRuleGroups as $group) {
            ...
            $firstPrice = $rule['firstPrice']; // 首件价格
            $secondPrice = 0;
            if ($postageRule->type == 1) { // 按重量计费
                ...
                $secondPrice = $priceFun($rule, $group['is_max_first_price'], $totalWeight, $secondPrice);
            } elseif ($postageRule->type == 2) { // 按件数计费
                $totalNum = 0;
                foreach ($goodsList as $goods) {
                    $totalNum += $goods['num'];
                }
                $secondPrice = $priceFun($rule, $group['is_max_first_price'], $totalNum, $secondPrice);
            }
            if ($secondPrice < 0) {
                $secondPrice = 0;
            }
    ...
    protected function setGoodsForm($mchItem)
    {
        ...
        $mchItem['diff_goods_form_count'] = count($existsFormIds);
    ...
    public function getUsableCouponList($mchData)
    {
        ...
        foreach ($mch['goods_list'] as &$goodsItem) {
            /** @var Goods $goods */
            $goods = $goodsItem['goods_attr']->goods;
    ...
    public function extraGoodsDetail($order, $goodsItem)
    {
        ...
        $orderDetail->back_price = $goodsItem['back_price'] ?? 0;
     ...
     * @return bool
     */
    protected function getAddressEnable(array &$mchItem)
    {
        ...
        foreach ($mchItem['goods_list'] as &$item) {
            ...
            if ($isGoodsEnableAreaLimit) {
                ...
                if (!$is_address_in_area($goodsAreaLimit, $item['address'] ?? $address)) {
                    $item['address_disabled'] = true;
                    $isHasGoodsDisabled = true;
                    continue;
                }
            ...
            if (!$is_address_in_area($model['detail'], $item['address'] ?? $address)) {
                $item['address_disabled'] = true;
                $isHasGoodsDisabled = true;
            }
        }
        return !$isHasGoodsDisabled;
    }
    ...
    protected function getPriceEnable($totalPrice, $address, $mchItem, &$minPrice)
    {
        ...
        // 新加-多地址
        $globalGoods = $singleGoods = [];
        foreach ($mchItem['goods_list'] as $item) {
            if(isset($item['address']) && $item['address']['id'] != $address->id){
                $singleGoods[] = $item;
                $totalPrice -= $item['total_original_price'];
            }else{
                $globalGoods[] = $item;
            }
        }

        $minPrice = $model['is_total_price'] == 1 ? price_format($model['total_price']) : 0;
        if (is_array($model['detail'])) {
            ...
            if($singleGoods) {
                foreach ($model['detail'] as $group) {
                    ...
                    foreach ($singleGoods as $goods) {
                        ...
                        if ($singleInArr) {
                            if ($goods['total_original_price'] < price_format($group['total_price'])) {
                                throw new OrderException('商品"' . $goods['name'] . "\"的快递地址满{$group['total_price']}元起送");
                            }
                        }
                    }
                }
            ...
            foreach ($singleGoods as $goods){
                if($goods['total_original_price'] < $minPrice){
                    throw new OrderException('商品"' . $goods['name'] . "\"的快递地址满{$minPrice}元起送");
                }
            }
        }
        return !$globalGoods || $totalPrice >= $minPrice;
    }
     ...
     * @param $item
     * @return bool
     * @throws OrderException|\app\bootstrap\exceptions\ClassNotFoundException
     * 商品信息的其他判断
     */
    ...
    private function setCoupon($mchItem, $totalGoodsPrice, $subPrice, $couponGoodsIdList = null)
    {
        ...
        foreach ($mchItem['goods_list'] as $index => &$goods) {
            if (!empty($couponGoodsIdList) && !in_array($goods['goods_warehouse_id'], $couponGoodsIdList)) {
                continue;
            }
            /* @var float $goodsPrice 商品可优惠的金额 */
            $goodsPrice = price_format($goods['total_price'] * $subPrice / $totalGoodsPrice);
            ...
            $goods['coupon_discount'] = min($goodsPrice, $goods['total_price']); // @czs 商品平均减多少
            $resetPrice -= $goods['coupon_discount'];
            $goods['total_price'] -= $goods['coupon_discount'];
        }
    ...
    protected function getNewSendType($sendType)
    {
        ...
        foreach ($sendType as $item) {
            // 字节跳动小程序不支持同城配送
            if (\Yii::$app->appPlatform == 'ttapp' && $item == 'city') {
                continue;
            }
            $list[] = $item;
        }
    ...
    protected function getTemplateMessage()
    {
        try {
            ...
            return TemplateList::getInstance()->getTemplate(\Yii::$app->appPlatform, $arr);
        }catch(Exception $exception) {
            \Yii::error($exception);
        }
    }
    ...
    public function setDiscountDataAfterMember($mchItem, $formMchItem)
    {
        ...
        foreach ($mchItem['goods_list'] as &$goodsItem) {
            ...
            if (isset($goodsItem['sign']) && $goodsItem['sign'] == 'wholesale') {
    ...
    public function isGoodsEnableMemberPrice($goodsItem)
    {
        ...
        return (bool)$this->enableMemberPrice;
    }
    ...
    public function isGoodsEnableFullReduce($goodsItem)
    {
        ...
        return (bool)$this->enableFullReduce;
    }
    ...
    public function isGoodsEnableVipPrice($goodsItem)
    {
        ...
        return (bool)$this->enableVipPrice;
    }
    ...
    public function isGoodsEnableIntegral($goodsItem)
    {
        ...
        return (bool)$this->enableIntegral;
    }
    ...
    public function isGoodsEnableAddressLimit($goodsItem)
    {
        ...
        return (bool)$this->enableAddressEnable;
    }
