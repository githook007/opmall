...
class OrderGoodsAttr extends Model
{
    ...
    public function rules()
    {
        return [
            ...
            [['price', 'original_price', 'member_price', 'integral_price', 'cost_price'], 'number', 'min' => 0],
    ...
    public function setGoodsAttr($goodsAttr)
    {
        ...
        $this->cost_price = $this->goods->goodsWarehouse->cost_price;
        $this->discount = [];
    ...
    public function setGoodsAttrById($goodsAttrId)
    {
        ...
        $goodsAttr = GoodsAttr::find()->where(['id' => $goodsAttrId, 'goods_id' => $this->goods->id])->one();
        if (!$goodsAttr) {
