...
class OrderRefundSubmitForm extends Model
{
    ...
    public function submit()
    {
        ...
        try {
            $orderRefund = $this->create();

            \Yii::$app->trigger(OrderRefund::EVENT_CREATE_REFUND, new OrderRefundEvent([
                'order_refund' => $orderRefund
            ])); // @czs

            $t->commit();
            $this->notice($orderRefund);
            return [
    ...
    public function notice($orderRefund){
        $this->sendRefundSms($this->orderDetail->order);
        $this->sendMail($orderRefund->type, $this->orderDetail->order);
        $this->sendMpTplMsg($this->orderDetail->order);
    }
    ...
    private function checkIsRefund()
    {
        ...
        if ((!empty($orderRefundList[0]) && $orderRefundList[0]->status !=3) || count($orderRefundList) >= 2) {
