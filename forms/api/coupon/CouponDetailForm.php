...
class CouponDetailForm extends Model
{
    ...
    public function receive()
    {
        ...
        try {
            ...
            $coupon = $common->getDetail(['couponMember' => function ($query) {
            ...
            if($coupon->couponMember){
                $common->checkMemberPower($coupon->couponMember);
            }
    ...
    public function getDetail()
    {
        ...
        $common->user = \Yii::$app->user->identity;
        $res = $common->getDetail(['couponMember' => function ($query) {
            $query->where(['is_delete' => 0]);
        }]);
        if ($res['is_delete'] == 1) {
        ...
        if($res['couponMember']){
            $common->checkMemberPower($res['couponMember']);
        }
