...
class ConfigForm extends Model
{
    public function search()
    {
        ...
        foreach ($navbar['navs'] as $key => $nav) {
            if($navbar['is_eject'] && $navbar['eject_num'] == $key + 1){
                $navbar['navs'][$key]['text'] = '';
                break;
            }
        }
        $navbar = Transform::getInstance()->transformNavbar($navbar);

        $copyrightType = in_array('copyright', \Yii::$app->mall->role->permission) ? false : true;
        ...
        return [
            ...
            'data' => [
                ...
                'plugin' => $plugin,
                'copyright' => CommonAppConfig::getCoryRight($copyrightType),
                ...
                'invoice' => $plugin['invoice'] ?? 0,  // 发票开关0:关闭; 1:开启
            ],
        ];
    }
