...
class User extends ModelActiveRecord implements IdentityInterface
{
    ...
    {
        return $this->hasOne(Mch::className(), ['user_id' => 'id'])->andWhere(['is_delete' => 0]);
    }
    ...
    const LOGIN_STAFF = 'staff'; // 员工
    const LOGIN_ADMIN = 'admin'; // 管理员和超管
    const LOGIN_MCH = 'mch'; // 商户
    const LOGIN_CASHIER = 'cashier'; // 收银台

    public function setLoginData($loginType){
