 ...
 * @property int $pay_type 支付方式：1.在线支付 2.货到付款 3.余额支付 4.现金 5.pos  6.代付
...
class Order extends ModelActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['mall_id', 'user_id', 'mch_id', 'use_user_coupon_id', 'use_integral_num', 'is_pay', 'pay_type',
                ...
                'clerk_id', 'store_id', 'is_comment', 'sale_status', 'status', 'distance', 'replace_user_id'], 'integer'],
            ...
            [['order_no', 'mobile', 'address', 'remark', 'words', 'seller_remark', 'offline_qrcode',
                'sign', 'location'], 'string', 'max' => 255],
            [['name'], 'string', 'max' => 65],
    ...
    public function getReplaceUser()
    {
        return $this->hasOne(User::className(), ['id' => 'replace_user_id']);
    }
    ...
    public function getExtraData(){
        if($this->is_pay){
            try{
                $data['is_trade_managed'] = $this->paymentOrder->orderTradeManage->order_state == 2 ? 1 : 0;
            }catch (\Exception $e){
                $data['is_trade_managed'] = 0;
            }
            if($data['is_trade_managed']) {
                if ($this->paymentOrder->orderTradeManage->transaction_id) {
                    $data['transaction_id'] = $this->paymentOrder->orderTradeManage->transaction_id;
                } else {
                    $data['merchant_id'] = $this->paymentOrder->orderTradeManage->merchant_id;
                    if (!empty($this->paymentOrder->orderTradeManage->sub_merchant_id)) {
                        $data['sub_merchant_id'] = $this->paymentOrder->orderTradeManage->sub_merchant_id;
                    }
                    $data['merchant_trade_no'] = $this->paymentOrder->orderTradeManage->merchant_trade_no;
                }
            }
        }
    ...
    public function checkClerkPermission($storeId, $userId, $mchId = 0){
        if(!$storeId){
            return false;
        }
        $clerkUserIds = ClerkUserStoreRelation::find()->where(['store_id' => $storeId])->select('clerk_user_id');
