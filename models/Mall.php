 ...
 * @property User $user
 * @property MallExtend $extend
 * @property MallSetting[] $option
 * @property BaseRole $role
 * @property WlhulianData $wlHulian   聚合配送 @czs
 */
...
//    public $options;
    ...
    {
        return $this->hasOne(MallExtend::className(), ['mall_id' => 'id'])->andWhere(['is_delete' => 0]);
    }

    public function extendObj($data = null){
        $extend = $this->extend;
        if(!$extend){
            $extend = new MallExtend();
            $extend->mall_id = $this->id;
        }
        if(is_array($data)){
            $extend->attributes = $data;
        }
        if(!$extend->save()){
            throw new \Exception((new Model())->getErrorMsg($extend));
        }
        return $extend;
    }

    public function getWlHulian()
    {
        return $this->hasOne(WlhulianData::className(), ['mall_id' => 'id'])->andWhere(['is_delete' => 0]);
    }

    public function getOption()
    {
        return $this->hasMany(MallSetting::className(), ['mall_id' => 'id'])->andWhere(['is_delete' => 0]);
    }

    /**
     * api端去掉一些第三方配置 @czs
     */
    public function apiDislodge(){
        $detail = $this->getMallSetting();
        $keyList = [
            ...
            'over_time', // 未支付订单超时时间（分钟）
            'delivery_time', // 收货时间（天）
            'after_sale_time', // 售后时间（天）
        ];
        foreach ($detail['option'] as $key => $value) {
            if (in_array($value['key'], $keyList)) {
                unset($detail['option'][$key]);
            }
        }
        $detail['option'] = array_values($detail['option']);
        foreach ($detail['setting'] as $key => $value){
            if (in_array($key, $keyList)) {
                unset($detail['setting'][$key]);
            }
        }
        return $detail;
    }
    ...
    public function getMallSetting($columns = [])
    {
        if ($this->mallAllOptions) {
            ...
            // TODO 兼容
            if (isset($newOption['video_number_member_list']) && $newOption['video_number_member_list'] == 'null') {
                $newOption['video_number_member_list'] = [];
            }

            if (isset($newOption['video_number_template_list']) && $newOption['video_number_template_list'] == 'null') {
                $newOption['video_number_template_list'] = [];
            }
            ...
            foreach ($defaultArr as $k => $item) {
                ...
                try {
                    if (is_integer($item)) {
                        $newOption[$k] = (int)$newOption[$k];
                    }

                    if (is_array($item)) {
                        $newOption[$k] = Json::decode($newOption[$k]);
                    }

                    if ($k == 'web_service_url') {
                        $newOption[$k] = urldecode($newOption[$k]);
                    }

                    if ($k == 'send_type') {
                        $newOption[$k] = Compatible::getInstance()->sendType($newOption[$k]);
                    }
    ...
    public function getDefault()
    {
        ...
        return [
             ...
             * balance 余额支付
             * huodao 货到付款
             * wechat_pay_for_another 微信代付  @czs
             */
            'payment_type' => [
                'online_pay',
            ],
            ...
            'send_type_desc' => [
                [
                    'key' => 'express',
                    'origin' => \Yii::t('common', '快递配送'),
                    'modify' => '',
                ],
                [
                    'key' => 'offline',
                    'origin' => \Yii::t('common', '到店自提'),
                    'modify' => '',
                ],
                [
                    'key' => 'city',
                    'origin' => \Yii::t('common', '同城配送'),
                    'modify' => '',
                ],
            ],
            /////////////////////////////////////////////////////////////////////////////////
...
//            'mobile_verify' => '1', // 商城手机号是否验证 0.关闭 1.开启  @czs 没找到使用
            'is_small_app' => '0', //跳转小程序开关
            'profit_sharing' => 0, //微信分账功能 @czs
            ...
            'web_service_type' => 1, // 客服外链类型 1：其它客服，2：企业微信客服
            ...
            'theme_color' => 'a', // 商城风格
            'is_required_position' => 1, // 定位地址是否必填
            ...
            'member_grade' => 1, // 1:会员等级金额叠加，0：单个等级金额

            'goods_coupon_usage_scenario' => 1, // 商品优惠券下发使用场景
        ];
    }
