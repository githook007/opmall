...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header" flex="dir:left" style="justify-content:space-between;">
            <span><?= \Yii::t('mall/goods', '商品热搜');?></span>
            <el-button @click="$navigate({r:'mall/goods-hot-search/edit'})" style="margin: -5px 0" type="primary"
                       size="small"><?= \Yii::t('mall/goods', '新建热搜商品');?>
            </el-button>
        </div>
        <div class="table-body">
            <el-table v-loading="listLoading" :data="list" border>
                <el-table-column prop="goods_id" label="<?= \Yii::t('mall/goods', '商品ID');?>" width="100"></el-table-column>
                <el-table-column prop="goods_name" label="<?= \Yii::t('mall/goods', '商品名称');?>" min-width="300" width="400">
                ...
                <el-table-column prop="title" label="<?= \Yii::t('mall/goods', '热搜词');?>">
                    <template slot-scope="scope">
                        <div flex="cross:center">
                            <div class="t-omit-two">
                                {{scope.row.title}}
                ...
                <el-table-column prop="type" label="<?= \Yii::t('mall/goods', '获取方式');?>" width="250">
                    <template slot-scope="scope">
                        <el-tag size="medium" v-if="scope.row.type === 'goods'"><?= \Yii::t('mall/goods', '自动获取');?></el-tag>
                        <el-tag size="medium" v-if="scope.row.type === 'hot-search'" type="success"><?= \Yii::t('mall/goods', '手动添加');?></el-tag>
                    </template>
                </el-table-column>
                <el-table-column prop="sort" label="<?= \Yii::t('mall/goods', '热搜名次');?>" width="250">
                    <template slot-scope="scope">
                ...
                <el-table-column label="<?= \Yii::t('mall/goods', '操作');?>" width="120" fixed="right">
                    <template slot-scope="scope">
                        <el-button type="text" @click="edit(scope.row)"
                                   size="small" circle>
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/goods', '编辑');?>" placement="top">
                                <img src="statics/img/mall/edit.png" alt="">
                            </el-tooltip>
                        </el-button>
                        <el-button class="set-el-button" size="mini" type="text" circle @click="destroy(scope.row,scope.$index)">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/goods', '删除');?>" placement="top">
    ...
    const app = new Vue({
        ...
        methods: {
            ...
            destroy(params, index) {
                this.$confirm('<?= \Yii::t('mall/goods', '删除该条数据');?>?', '<?= \Yii::t('mall/goods', '提示');?>', {
                    ...
                    this.$message({type: 'info', message: '<?= \Yii::t('mall/goods', '已取消删除');?>'});
                });
            },
        }
    });
