    ...
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        ...
        <div class="table-body">
            <el-tabs v-model="activeName" @tab-click="handleClick">
                <el-tab-pane label="<?= \Yii::t('mall/goods', '商品导入');?>" name="first"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('mall/goods', '分类导入');?>" name="second"></el-tab-pane>
            </el-tabs>

            <el-steps style="margin-left: 20px; margin-top: 20px;" :space="300" :active="active">
                <el-step title="<?= \Yii::t('mall/goods', '选择文件');?>">
                    <template slot="icon">
                        <img v-if="active > 0" src="statics/img/mall/goods/finish-2.png">
                    </template>
                </el-step>
                <el-step title="<?= \Yii::t('mall/goods', '导入数据');?>">
                    <template slot="icon">
                        <img v-if="active > 1" src="statics/img/mall/goods/finish-2.png">
                    </template>
                </el-step>
                <el-step title="<?= \Yii::t('mall/goods', '导入完成');?>">
            ...
            <template v-if="active <= 2">
                ...
                <el-form v-show="activeName == 'first'" :model="ruleForm" :rules="rules" ref="ruleForm"
                    ...
                    <el-form-item></el-form-item>
                    <el-form-item label="<?= \Yii::t('mall/goods', '状态');?>" prop="goods_status">
                        <el-radio v-model="ruleForm.goods_status" :label="0"><?= \Yii::t('mall/goods', '暂不上架');?></el-radio>
                        <el-radio v-model="ruleForm.goods_status" :label="1"><?= \Yii::t('mall/goods', '立即上架');?></el-radio>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('mall/goods', '上传数据');?>" prop="file">
                        <template slot="label">
                            <span><?= \Yii::t('mall/goods', '上传数据');?></span>
                            <el-tooltip effect="dark" content="<?= \Yii::t('mall/goods', '仅支持上传csv格式');?>"
                                        placement="top">
                        ...
                        <el-upload
                            ...
                            <span style="color: #409EFF;"><?= \Yii::t('mall/goods', '选择本地文件');?></span>
                        </el-upload>
                    </el-form-item>
                    <el-form-item v-if="mch_id > 0" label="<?= \Yii::t('mall/goods', '系统分类');?>" prop="system_cat_ids">
                        ...
                        <el-button type="primary" @click="$refs.systemCats.openDialog()" size="small"><?= \Yii::t('mall/goods', '选择分类');?>
                        </el-button>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('mall/goods', '商品分类');?>" prop="cat_ids">
                        ...
                        <el-button type="primary" @click="$refs.cats.openDialog()" size="small"><?= \Yii::t('mall/goods', '选择分类');?></el-button>
                    </el-form-item>

                    <el-form-item>
                        <el-button :loading="btnLoading" type="primary" @click="submit('ruleForm')"
                                   size="small"><?= \Yii::t('mall/goods', '一键导入');?>
                        </el-button>
                    </el-form-item>
                </el-form>
                <!-- 分类导入 -->
                <el-form v-show="activeName == 'second'" :model="ruleCatForm" :rules="catRules" ref="ruleCatForm"
                    ...
                    <el-form-item label="<?= \Yii::t('mall/goods', '是否启用');?>" prop="status">
                        <el-switch
                    ...
                    <el-form-item label="<?= \Yii::t('mall/goods', '是否显示');?>" prop="is_show">
                        <el-switch
            ...
            <template v-else>
                <div style="width: 600px;height: 200px" flex="dir:top cross:center main:center">
                    <div flex="dir:left cross:center">
                        ...
                        <span style="margin-left: 10px"><?= \Yii::t('mall/goods', '成功导入');?>{{successCount}}<?= \Yii::t('mall/goods', '条数据');?></span>
                    </div>
                    <div v-if="errorCount > 0" style="margin-top: 20px;">
                        <span><?= \Yii::t('mall/goods', '未导入');?><span style="color: red;">{{errorCount}}</span><?= \Yii::t('mall/goods', '条数据');?></span>
                        <span style="color: #409EFF;margin-left: 10px;"></span>
                        <app-new-export-dialog-2
                                text='<?= \Yii::t('mall/goods', '下载未导入数据');?>'
                                :params="searchData"
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                rules: {
                    goods_status: [
                        {required: true, message: '<?= \Yii::t('mall/goods', '请选择商品状态');?>', trigger: 'change'},
                    ],
                    file: [
                        {required: true, message: '<?= \Yii::t('mall/goods', '请上传文件');?>', trigger: 'change'},
                    ],
                    cat_ids: [
                        {required: true, message: '<?= \Yii::t('mall/goods', '请选择商品分类');?>', trigger: 'change'},
                    ],
                    system_cat_ids: [
                        {required: true, message: '<?= \Yii::t('mall/goods', '请选择系统分类');?>', trigger: 'change'},
                    ],
                },
                btnLoading: false,
                errorCount: 0,
        ...
        methods: {
            ...
            submit(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        this.$confirm('<?= \Yii::t('mall/goods', '是否确认导入');?>', '<?= \Yii::t('mall/goods', '提示');?>', {
                            confirmButtonText: '<?= \Yii::t('mall/goods', '确定');?>',
                            cancelButtonText: '<?= \Yii::t('mall/goods', '取消');?>',
                            type: 'warning'
                        }).then(() => {
            ...
            beforeClose() {
                if (this.importParams.percentage < 100) {
                    this.$confirm('<?= \Yii::t('mall/goods', '导入尚未完成');?>?', '<?= \Yii::t('mall/goods', '警告');?>', {
                        confirmButtonText: '<?= \Yii::t('mall/goods', '确定');?>',
                        cancelButtonText: '<?= \Yii::t('mall/goods', '取消');?>',
                        type: 'warning'
                    }).then(() => {
