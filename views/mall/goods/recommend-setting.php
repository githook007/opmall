...
<div id="app" v-cloak>
    <el-card v-loading="loading" v-if="is_show" style="border:0"
        ...
        <div slot="header">
            <?= \Yii::t('mall/goods', '推荐设置');?>
        </div>
        <el-form @submit.native.prevent ref="form" :model="form" label-width="150px" size="small">
            <el-card style="margin-top: 10px;" shadow="never" v-for="(item, index) in list" :key="index">
                <div slot="header">
                    {{item.name}}
                </div>
                <el-row>
                    <el-col :span="12">
                        <el-form-item label="<?= \Yii::t('mall/goods', '推荐商品状态');?>">
                            <el-switch :active-value="1" :inactive-value="0"
                                       v-model="form[item.key].is_recommend_status"></el-switch>
                        </el-form-item>
                        <template v-if="form[item.key].is_recommend_status">
                            <el-form-item v-if="item.type == 2" label="<?= \Yii::t('mall/goods', '自定义推荐商品');?>">
                                <el-switch :active-value="1" :inactive-value="0"
                                           v-model="form[item.key].is_custom"></el-switch>
                                <span>{{form[item.key].is_custom ? "<?= \Yii::t('mall/goods', '最多添加20件商品');?>" : "<?= \Yii::t('mall/goods', '按商品列表排序显示前10件商品');?>"}}</span>
                                <div v-if="form[item.key].is_custom">
                                    <draggable v-model="form[item.key].goods_list" flex class="goods-list">
                                        <div class="goods-item"
                                             v-for="(goods,goodsIndex) in form[item.key].goods_list">
                                            <el-tooltip effect="dark" content="<?= \Yii::t('mall/goods', '移除商品');?>" placement="top">
                                                <el-button @click="deleteGoods(goodsIndex, item.key)" circle
                                    ...
                                    <div v-if="form[item.key].goods_list.length < goodsDialog.max"
                                         class="goods-add">
                                        <el-tooltip effect="dark" content="<?= \Yii::t('mall/goods', '添加商品');?>" placement="top">
                                            <el-button @click="showGoodsDialog(item.key)"
                                                       icon="el-icon-plus"></el-button>
                                        </el-tooltip>
                                    </div>
                                </div>
                            </el-form-item>
                            <el-form-item label="<?= \Yii::t('mall/goods', '推荐商品显示数量');?>" v-else>
                                <el-input v-model="form[item.key].goods_num" type="number"
            ...
            <el-card style="margin-top: 10px;" shadow="never">
                <el-button :loading="btnLoading" type="primary" size="small" @click="store"><?= \Yii::t('mall/goods', '保存');?></el-button>
            </el-card>
        </el-form>
    </el-card>

    <el-dialog @open="getGoods" title="<?= \Yii::t('mall/goods', '选择商品');?>" :visible.sync="goodsDialog.visible" :close-on-click-modal="false">
            ...
            <el-form-item>
                <div class="input-item">
                    <el-input @clear="toSearch" clearable @keyup.enter.native="toSearch" size="small"
                              placeholder="<?= \Yii::t('mall/goods', '请输入商品ID');?>"
        ...
        <el-table :data="goodsDialog.list" v-loading="goodsDialog.loading" @selection-change="goodsSelectionChange">
            <el-table-column label="<?= \Yii::t('mall/goods', '选择');?>" type="selection"></el-table-column>
            <el-table-column label="ID" prop="id" width="100px"></el-table-column>
            <el-table-column label="<?= \Yii::t('mall/goods', '名称');?>" prop="name"></el-table-column>
        </el-table>
        ...
        <div slot="footer">
            <el-button @click="goodsDialog.visible = false"><?= \Yii::t('mall/goods', '取消');?></el-button>
            <el-button type="primary" @click="addGoods"><?= \Yii::t('mall/goods', '确定');?></el-button>
        </div>
        ...
        data() {
            return {
                ...
                list: [
                    {
                        key: 'goods',
                        name: '<?= \Yii::t('mall/goods', '商品详情页推荐设置');?>',
                        type: 1
                    },
                    {
                        key: 'order_pay',
                        name: '<?= \Yii::t('mall/goods', '订单完成后推荐设置');?>',
                        type: 2
                    },
                    {
                        key: 'order_comment',
                        name: '<?= \Yii::t('mall/goods', '评论后推荐设置');?>',
                        type: 2
                    },
                    {
                        key: 'cart',
