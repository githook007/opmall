...
<div id="app" v-cloak>
    <app-goods-list
        ...
        <template slot="column-col">
            <el-table-column v-if="!is_mch" label="<?= \Yii::t('mall/goods', '是否加入快速购买');?>">
                <template slot-scope="scope">
                    <el-tooltip class="item" effect="dark"
                                content="<?= \Yii::t('mall/goods', '虚拟商品不能加入快速购买');?>"
        ...
        <template slot="batch" slot-scope="item">
            <div v-if="item.item === 'quick'">
                <el-form-item label="<?= \Yii::t('mall/goods', '是否加入快速购买');?>">
                    <el-switch
                            @change="batch('quick')"
            ...
            <div v-if="item.item === 'negotiable'">
                <el-form-item label="<?= \Yii::t('mall/goods', '是否加入商品面议');?>">
                    <el-tooltip effect="dark" content="<?= \Yii::t('mall/goods', '如果开启面议，则商品无法在线支付');?>" placement="top">
    ...
    const app = new Vue({
        ...
        data() {
            ...
            return {
                ...
                batchList: [
                    {
                        name: '<?= \Yii::t('mall/goods', '快速购买');?>',
                        key: 'quick',
                        url: 'mall/goods/batch-update-quick',
                        content: '<?= \Yii::t('mall/goods', '批量移除快速购买,是否继续');?>',
                        params: {
                            status: 0
                        }
                    },
                    {
                        name: '<?= \Yii::t('mall/goods', '面议');?>',
                        key: 'negotiable',
                        url: 'mall/goods/batch-update-negotiable',
                        content: '<?= \Yii::t('mall/goods', '批量移除商品面议,是否继续');?>',
                        params: {
                            status: 0
                        }
                    },
        ...
        methods: {
            ...
            batch(key) {
                ...
                if (!this.is_mch) {
                    if (key === 'quick') {
                        this.batchList[0].content = isAllChecked ? '<?= \Yii::t('mall/goods', '警告: 批量设置所有商品');?>' + (this.batchList[0].params.status ? '<?= \Yii::t('mall/goods', '加入');?>' : '<?= \Yii::t('mall/goods', '移除');?>') + '<?= \Yii::t('mall/goods', '快速购买,是否继续');?>' : '<?= \Yii::t('mall/goods', '批量');?>' + (this.batchList[0].params.status ? '<?= \Yii::t('mall/goods', '加入');?>' : '<?= \Yii::t('mall/goods', '移除');?>') + '<?= \Yii::t('mall/goods', '快速购买,是否继续');?>';
                    } else if (key === 'negotiable') {
                        this.batchList[1].content = isAllChecked ? '<?= \Yii::t('mall/goods', '警告: 批量设置所有商品');?>' + (this.batchList[1].params.status ? '<?= \Yii::t('mall/goods', '加入');?>' : '<?= \Yii::t('mall/goods', '移除');?>') + '<?= \Yii::t('mall/goods', '商品面议,是否继续');?>' : '<?= \Yii::t('mall/goods', '批量');?>' + (this.batchList[1].params.status ? '<?= \Yii::t('mall/goods', '加入');?>' : '<?= \Yii::t('mall/goods', '移除');?>') + '<?= \Yii::t('mall/goods', '商品面议,是否继续');?>';
                    }
                }
            },
