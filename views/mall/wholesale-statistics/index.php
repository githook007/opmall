...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <app-header :url="url" :new-search="JSON.stringify(search)"><?= \Yii::t('mall/wholesale_statistics', '商品批发');?></app-header>
        </div>
        <div class="table-body">
            <app-search
                    ...
                    placeholder="<?= \Yii::t('mall/wholesale_statistics', '请输入商品名称搜索');?>"
        ...
        <div class="table-body" style="padding: 20px">
            <el-table @sort-change="changeSort" v-loading="loading" :header-cell-style="{background:'#F3F5F6','color':'#303133',padding: '6px 0',fontWeight: '400'}" :data="list">
                <el-table-column width="600" prop="goods" label="<?= \Yii::t('mall/wholesale_statistics', '商品名称');?>">
                ...
                <el-table-column prop="user_num" label="<?= \Yii::t('mall/wholesale_statistics', '支付人数');?>" :label-class-name="prop == 'user_num' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                <el-table-column prop="goods_num" label="<?= \Yii::t('mall/wholesale_statistics', '支付件数');?>" :label-class-name="prop == 'goods_num' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                <el-table-column prop="total_price" label="<?= \Yii::t('mall/wholesale_statistics', '支付金额');?>" :label-class-name="prop == 'total_price' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
            </el-table>
            <div style="margin-top: 10px;" flex="box:last cross:center">
                <div style="visibility: hidden">
                    <el-button plain type="primary" size="small"><?= \Yii::t('mall/wholesale_statistics', '批量操作');?>1</el-button>
                    <el-button plain type="primary" size="small"><?= \Yii::t('mall/wholesale_statistics', '批量操作');?>2</el-button>
                </div>
                <div>
                    <el-pagination
                            v-if="pagination"
...
<script>
    const app = new Vue({
        ...
        data() {
            return {
                url: 'mall/wholesale-statistics/index',
                loading: false,
                // 今天
                today: '',
                // 七天前
        ...
        methods: {
            ...
            getList() {
                ...
                request({
                    params: {
                        r: 'mall/wholesale-statistics/index',
