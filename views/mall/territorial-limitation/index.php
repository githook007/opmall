...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;" v-loading="loading">
        <div slot="header">
            <span><?= \Yii::t('mall/territorial_limitation', '区域允许购买');?></span>
        </div>
        <el-form :model="ruleForm" ref="ruleForm" :rules="rules" label-width="150px">
            <div class="form-body">
                <el-form-item label="<?= \Yii::t('mall/territorial_limitation', '是否开启');?>" prop="is_enable" required>
                    <el-switch
                ...
                <el-form-item label="<?= \Yii::t('mall/territorial_limitation', '允许购买区域');?>" v-if="ruleForm.is_enable" prop="detail" required>
                    <el-button style="margin-left: 20px;" size="small" v-if="ruleForm.detail[0].list.length == 0" type="text" @click="openDistrict"><i class="el-icon-plus"><?= \Yii::t('mall/territorial_limitation', '添加地区');?></i>
                    </el-button>
                        ...
                        <div flex="dir:left box:last">
                            <div>
                                <div flex="dir:left" style="flex-wrap: wrap;width: 70%">
                                    <div><?= \Yii::t('mall/territorial_limitation', '区域');?></div>
                            ...
                            <div class="doit">
                                <el-button size="small" type="text" circle @click="openDistrict(index)">
                                    <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/territorial_limitation', '编辑');?>" placement="top">
                                        <img src="statics/img/mall/edit.png" alt="">
                                    </el-tooltip>
                                </el-button>
                                <el-button size="small" circle type="text" @click="deleteDistrict(index)">
                                    <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/territorial_limitation', '删除');?>" placement="top">
            ...
            <el-form-item class="form-button">
                <el-button sizi="mini" class="button-item" :loading="submitLoading" type="primary" @click="onSubmit('ruleForm')">
                    <?= \Yii::t('mall/territorial_limitation', '保存');?>
                </el-button>
            </el-form-item>
        </el-form>
        <el-dialog title="<?= \Yii::t('mall/territorial_limitation', '选择地区');?>" :visible.sync="dialogVisible" width="50%">
            <div style="margin-bottom: 1rem;">
                ...
                <div style="text-align: right;margin-top: 1rem;">
                    <el-button type="primary" @click="districtConfirm">
                        <?= \Yii::t('mall/territorial_limitation', '确定选择');?>
                    </el-button>
                </div>
            </div>
        </el-dialog>
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                rules: {
                    is_enable: [
                        {type: 'integer', trigger: 'blur', required: true, message: '<?= \Yii::t('mall/territorial_limitation', '请选择是否开启');?>'}
                    ],
                    detail: [
                        {trigger: 'blur', required: true, message: '<?= \Yii::t('mall/territorial_limitation', '请选择地区');?>'}
                    ]
                },
                detail: {
                    list: []
