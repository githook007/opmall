        ...
        <div class="table-body">
            <el-form style="margin-bottom: 25px;" @submit.native.prevent size="small" inline :model="search">
                <el-form-item>
                    <div class="input-item">
                        <el-input size="small" placeholder="<?= \Yii::t('mall/we7', '请输入搜索内容');?>" clearable @clear="clearInput"
                ...
                <el-form-item v-if="multipleSelection.length > 0">
                    <el-button type="primary" size="small" @click="batch"><?= \Yii::t('mall/we7', '批量设置权限');?></el-button>
                </el-form-item>
                <el-form-item v-if="status && multipleSelection.length < 1">
                    <template slot='label'>
                        <span><?= \Yii::t('mall/we7', '子账户默认权限');?></span>
                        <el-tooltip effect="dark" content="<?= \Yii::t('mall/we7', '仅适用于新添加的账户从未设置过权限的账户');?>"
                                    placement="top">
                            <i class="el-icon-info"></i>
                        </el-tooltip>
                    </template>
                    <div>
                        <el-radio v-model="status" @change="updateStatus" label="0"><?= \Yii::t('mall/we7', '默认无权限');?></el-radio>
                        <el-radio v-model="status" @change="updateStatus" label="1"><?= \Yii::t('mall/we7', '默认有所有权限');?></el-radio>
                        <div style="display: inline-block;position: relative;">
                            <el-radio v-model="status" label="2"><?= \Yii::t('mall/we7', '默认自定义权限');?></el-radio>
            ...
            <div>
                <el-table
                    ...
                    <el-table-column
                            prop="username"
                            label="<?= \Yii::t('mall/we7', '用户名');?>"
                            width="120">
                    </el-table-column>
                    <el-table-column
                            prop="created_at"
                            label="<?= \Yii::t('mall/we7', '注册时间');?>">
                    </el-table-column>
                    <el-table-column
                            label="<?= \Yii::t('mall/we7', '操作');?>"
                            width="280">
                ...
                <div flex="box:last cross:center" style="margin-top: 20px;">
                    <div>
                        <el-button style="visibility: hidden" plain type="primary" size="small" @click="batch"><?= \Yii::t('mall/we7', '批量设置权限');?>
                        </el-button>
                    </div>
                    <div>
                        <el-pagination
    ...
    <el-dialog
        ...
        <div class="window">
            ...
            <el-checkbox v-if="is_custom_edit" class="check-title" v-model="is_default" :true-label="1" :false-label="0">
                <?= \Yii::t('mall/we7', '默认权限');?>
                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/we7', '使用默认权限');?>" placement="top">
            ...
            <template v-if="is_default == 0">
                <el-checkbox class="check-title" :indeterminate="mallIndeterminate" v-model="checkMall"
                             @change="handleCheckMallChange"><?= \Yii::t('mall/we7', '基础权限');?>
                </el-checkbox>
                ...
                <el-checkbox class="check-title" :indeterminate="pluginsIndeterminate" v-model="checkPlugins"
                             @change="handleCheckPluginsChange"><?= \Yii::t('mall/we7', '插件权限');?>
                </el-checkbox>
                ...
                <template v-if="storageShow()">
                    <el-checkbox class="check-title" :indeterminate="storageIndeterminate()"
                                 ...
                                 @change="storageCheckAll"><?= \Yii::t('mall/we7', '上传权限');?>
                    </el-checkbox>
                ...
                <template v-if="templateShow()">
                    <div class="check-title" style="height: 50px;line-height: 50px"><?= \Yii::t('mall/we7', '模板权限');?></div>
                    <div style="padding: 10px 20px;">
                        <div style="margin-right: 10px;"><?= \Yii::t('mall/we7', '显示权限');?></div>
                        <div flex>
                            <el-button size="mini" type="text" style="margin-right: 10px;" @click="show = true"
                                       v-if="secondary_permissions.template.is_all == 0"><?= \Yii::t('mall/we7', '添加模板');?></el-button>
                            <el-checkbox v-model="secondary_permissions.template.is_all"
                                         true-label="1" false-label="0"><?= \Yii::t('mall/we7', '全部选择');?></el-checkbox>
                        </div>
                        ...
                        <div style="margin-right: 10px;margin-top: 10px;"><?= \Yii::t('mall/we7', '使用权限');?></div>
                        <div flex>
                            <el-button size="mini" type="text" style="margin-right: 10px;" @click="show_use = true"
                                       v-if="secondary_permissions.template.use_all == 0"><?= \Yii::t('mall/we7', '添加模板');?></el-button>
        ...
        <span slot="footer" class="dialog-footer">
            <el-button size="small" @click="dialogVisible = false"><?= \Yii::t('mall/we7', '取消');?></el-button>
            <el-button size="small" :loading="btnLoading" type="primary" @click="updatePermission"><?= \Yii::t('mall/we7', '保存');?></el-button>
        </span>
    </el-dialog>
</div>
<script>
        ...
        methods: {
            ...
            batch() {
                ...
                if (self.multipleSelection.length < 1) {
                    self.$message.warning('<?= \Yii::t('mall/we7', '请先选择用户');?>');
                    return;
                }
                self.user = {};
            ...
            permissionText(user) {
                ...
                return `<?= \Yii::t('mall/we7', '权限管理');?>(` + own + `/` + total + `)`;
            },
            customPermissionText() {
                let total = this.permissions.mall.length + this.permissions.plugins.length;
