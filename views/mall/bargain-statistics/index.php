...
<div id="app" v-cloak>
    <el-card v-loading="loading" shadow="never" style="border:0"
        ...
        <div slot="header">
            <app-header :url="url" :new-search="JSON.stringify(search)"><?= \Yii::t('mall/bargain_statistics', '砍价');?></app-header>
        </div>
        <div class="table-body">
            <app-search
                    ...
                    placeholder="<?= \Yii::t('mall/bargain_statistics', '请输入商品名称搜索');?>"
        ...
        <div class="table-body" style="padding: 20px">
            <el-table @sort-change="changeSort"
                      ...
                      :data="list">
                <el-table-column prop="name" width="330" label="<?= \Yii::t('mall/bargain_statistics', '商品名称');?>">
                ...
                <el-table-column prop="min_price" label="<?= \Yii::t('mall/bargain_statistics', '活动价格');?>">
                </el-table-column>
                <el-table-column width="100" prop="initiator" label="<?= \Yii::t('mall/bargain_statistics', '发起人数');?>"
                                 :label-class-name="prop == 'initiator' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                <el-table-column width="100" prop="participant" label="<?= \Yii::t('mall/bargain_statistics', '参与人数');?>"
                                 :label-class-name="prop == 'participant' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                <el-table-column prop="min_price_goods" label="<?= \Yii::t('mall/bargain_statistics', '砍到最低商品数');?>">
                </el-table-column>
                <el-table-column prop="underway" label="<?= \Yii::t('mall/bargain_statistics', '进行中活动数');?>">
                </el-table-column>
                <el-table-column prop="success" label="<?= \Yii::t('mall/bargain_statistics', '成功活动数');?>">
                </el-table-column>
                <el-table-column prop="fail" label="<?= \Yii::t('mall/bargain_statistics', '失败活动数');?>">
                </el-table-column>
                <el-table-column width="100" prop="payment_people" label="<?= \Yii::t('mall/bargain_statistics', '支付人数');?>"
                                 :label-class-name="prop == 'payment_people' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                <el-table-column width="100" prop="payment_num" label="<?= \Yii::t('mall/bargain_statistics', '支付件数');?>"
                                 :label-class-name="prop == 'payment_num' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                <el-table-column width="100" prop="payment_amount" label="<?= \Yii::t('mall/bargain_statistics', '支付金额');?>"
                                 :label-class-name="prop == 'payment_amount' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                <el-table-column prop="status" label="<?= \Yii::t('mall/bargain_statistics', '状态');?>">
                    <template slot-scope="scope">
                        <span v-if="scope.row.status == '<?= \Yii::t('mall/bargain_statistics', '未开始');?>'" style="color: #FFA360">{{scope.row.status}}</span>
                        <span v-if="scope.row.status == '<?= \Yii::t('mall/bargain_statistics', '进行中');?>'" style="color: #4BC282">{{scope.row.status}}</span>
                        <span v-if="scope.row.status == '<?= \Yii::t('mall/bargain_statistics', '已结束');?>'" style="color: #FF8585">{{scope.row.status}}</span>
                    </template>
                </el-table-column>
            </el-table>
            <div style="margin-top: 10px;" flex="box:last cross:center">
                <div style="visibility: hidden">
                    <el-button plain type="primary" size="small"><?= \Yii::t('mall/bargain_statistics', '批量操作');?>1</el-button>
                    <el-button plain type="primary" size="small"><?= \Yii::t('mall/bargain_statistics', '批量操作');?>2</el-button>
                </div>
                <div>
                    <el-pagination
                            v-if="pagination"
...
<script>
    const app = new Vue({
        ...
        data() {
            return {
                url: 'mall/bargain-statistics/index',
                loading: false,
                // 今天
                today: '',
                // 七天前
