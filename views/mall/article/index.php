...
<div id="app" v-cloak>
    <el-card class="box-card" shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <span><?= \Yii::t('mall/article', '文章列表');?></span>
            <div style="float: right;margin-top: -5px">
                <el-button type="primary" @click="$navigate({r: 'mall/article/edit', article_cat_id:2})"
                           size="small"><?= \Yii::t('mall/article', '添加文章');?>
                </el-button>
            </div>
        </div>
        <div class="table-body">
            <div class="input-item">
                <el-input @keyup.enter.native="search" size="small" placeholder="<?= \Yii::t('mall/article', '请输入搜索内容');?>" v-model="keyword" clearable @clear="search">
                    <el-button slot="append" icon="el-icon-search" @click="search"></el-button>
                </el-input>
            </div>
            <el-table :data="list" highlight-current-row v-loading="listLoading" style="width: 100%;" border>
                <el-table-column prop="id" label="ID" width="150"></el-table-column>
                <el-table-column prop="title" label="<?= \Yii::t('mall/article', '标题');?>"></el-table-column>
                <el-table-column prop="sort" label="<?= \Yii::t('mall/article', '排序');?>" width="250">
                    <template slot-scope="scope">
                        <div v-if="id != scope.row.id">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/article', '排序');?>" placement="top">
                ...
                <el-table-column prop="status" label="<?= \Yii::t('mall/article', '状态');?>" width="150">
                    <template slot-scope="scope">
                        <el-switch
                                active-value="1"
                                inactive-value="0"
                ...
                <el-table-column label="<?= \Yii::t('mall/article', '操作');?>" width="200" fixed="right">
                    <template slot-scope="scope">
                        <el-button class="set-el-button" size="mini" type="text" circle @click="$navigate({r: 'mall/article/edit', article_cat_id:article_cat_id, id:scope.row.id})">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/article', '编辑');?>" placement="top">
                                <img src="statics/img/mall/edit.png" alt="">
                            </el-tooltip>
                        </el-button>
                        <el-button class="set-el-button" size="mini" type="text" circle @click="destroy(scope.row)">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/article', '删除');?>" placement="top">
        ...
        methods: {
            ...
            switchStatus(row) {
                ...
                request({
                    ...
                    if (e.data.code === 0) {
                        if(row.status == 1) {
                            self.$message.success('<?= \Yii::t('mall/article', '文章已显示');?>');
                        }else if(row.status == 0) {
                            self.$message.success('<?= \Yii::t('mall/article', '文章已隐藏');?>');
                        }
                    } else {
                        self.$message.error(e.data.msg);
                    }
            ...
            destroy: function (column) {
                this.$confirm('<?= \Yii::t('mall/article', '确认删除该记录吗');?>?', '<?= \Yii::t('mall/article', '提示');?>', {
                    type: 'warning'
                }).then(() => {
