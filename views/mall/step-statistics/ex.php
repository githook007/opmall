...
<div id="app" v-cloak>
    <el-card v-loading="loading" shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <app-header :url="url" :new-search="JSON.stringify(search)"><?= \Yii::t('mall/step_statistic', '步数兑换');?></app-header>
        </div>
        ...
        <div class="table-area">
            <el-card shadow="never">
                <div slot="header">
                    <span><?= \Yii::t('mall/step_statistic', '总成交');?></span>
                </div>
                <div class="num-info">
                    <div class="num-info-item">
                        <div>{{all.step_num}}</div>
                        <div class="info-item-name"><?= \Yii::t('mall/step_statistic', '步数兑换总数');?></div>
                    </div>
                    <div class="num-info-item">
                        <div>{{all.user_num}}</div>
                        <div class="info-item-name"><?= \Yii::t('mall/step_statistic', '兑换人数');?></div>
                    </div>
                    <div class="num-info-item">
                        <div>{{all.goods_num}}</div>
                        <div class="info-item-name"><?= \Yii::t('mall/step_statistic', '兑商品数');?></div>
                    </div>
                    <div class="num-info-item">
                        <div>{{all.goods_pay}}</div>
                        <div class="info-item-name"><?= \Yii::t('mall/step_statistic', '兑换商品支出');?></div>
                    </div>
                </div>
            </el-card>
        </div>
        <div class="table-body" style="padding: 20px">
            <el-table :header-cell-style="{background:'#F3F5F6','color':'#303133',padding: '6px 0',fontWeight: '400'}" :data="list">
                <el-table-column prop="c_date" label="<?= \Yii::t('mall/step_statistic', '日期');?>">
                </el-table-column>
                <el-table-column prop="step_num" label="<?= \Yii::t('mall/step_statistic', '步数兑换总数');?>">
                </el-table-column>
                <el-table-column prop="user_num" label="<?= \Yii::t('mall/step_statistic', '兑换人数');?>">
                </el-table-column>
                <el-table-column prop="goods_num" label="<?= \Yii::t('mall/step_statistic', '兑商品数');?>">
                </el-table-column>
                <el-table-column prop="goods_pay" label="<?= \Yii::t('mall/step_statistic', '兑换商品支出');?>">
                </el-table-column>
            </el-table>
            <div style="margin-top: 10px;" flex="box:last cross:center">
                <div style="visibility: hidden">
                    <el-button plain type="primary" size="small"><?= \Yii::t('mall/step_statistic', '批量操作');?>1</el-button>
                    <el-button plain type="primary" size="small"><?= \Yii::t('mall/step_statistic', '批量操作');?>2</el-button>
                </div>
                <div>
                    <el-pagination
                            v-if="pagination"
