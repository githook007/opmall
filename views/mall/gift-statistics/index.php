...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <span><?= \Yii::t('mall/gift_statistics', '社交送礼');?></span>
        </div>
            ...
            <el-form class="search" size="small" :inline="true" style="padding: 20px" :model="search">
                ...
                <el-form-item>
                    <el-date-picker
                            ...
                            range-separator="<?= \Yii::t('mall/gift_statistics', '至');?>"
                            start-placeholder="<?= \Yii::t('mall/gift_statistics', '开始日期');?>"
                            end-placeholder="<?= \Yii::t('mall/gift_statistics', '结束日期');?>">
                    </el-date-picker>
                </el-form-item>
                <el-form-item>
                    <el-tabs v-model="activeName" @tab-click="tab_total">
                        <el-tab-pane label="7<?= \Yii::t('mall/gift_statistics', '日');?>" name="week"></el-tab-pane>
                        <el-tab-pane label="30<?= \Yii::t('mall/gift_statistics', '日');?>" name="month"></el-tab-pane>
                    </el-tabs>
                </el-form-item>
                <el-form-item>
                    <div class="input-item">
                        <el-input @keyup.enter.native="searchKeyword" size="small" placeholder="<?= \Yii::t('mall/gift_statistics', '输入商品名称搜索');?>" v-model="search.mch">
                            <el-button slot="append" icon="el-icon-search" @click="searchKeyword"></el-button>
                        </el-input>
                    </div>
                </el-form-item>
                <el-form-item>
                    <div class="clean" @click="clean"><?= \Yii::t('mall/gift_statistics', '清空筛选');?></div>
                </el-form-item>
            </el-form>
            <app-new-export-dialog-2
                    text='<?= \Yii::t('mall/gift_statistics', '导出报表');?>'
                    :params="search"
                    :directly=true
                    @selected='exportBefore()'
                    :action_url="url">
            </app-new-export-dialog-2>
        </div>
        <div class="table-body" style="padding: 20px">
            <el-table @sort-change="changeSort" v-loading="loading" :header-cell-style="{background:'#F3F5F6','color':'#303133',padding: '6px 0',fontWeight: '400'}" :data="list">
                <el-table-column width="600" prop="goods" label="<?= \Yii::t('mall/gift_statistics', '商品名称');?>">
                ...
                <el-table-column prop="goods_num" label="<?= \Yii::t('mall/gift_statistics', '支付件数');?>" :label-class-name="prop == 'goods_num' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                <el-table-column prop="total_price" label="<?= \Yii::t('mall/gift_statistics', '支付金额');?>" :label-class-name="prop == 'total_price' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                <el-table-column prop="user_num" label="<?= \Yii::t('mall/gift_statistics', '支付人数');?>" :label-class-name="prop == 'user_num' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                <el-table-column prop="convert_num" label="<?= \Yii::t('mall/gift_statistics', '领取件数');?>" :label-class-name="prop == 'convert_num' ? 'sort-active': ''" >
                </el-table-column>
            </el-table>
            <div style="margin-top: 10px;" flex="box:last cross:center">
                <div style="visibility: hidden">
                    <el-button plain type="primary" size="small"><?= \Yii::t('mall/gift_statistics', '批量操作');?>1</el-button>
                    <el-button plain type="primary" size="small"><?= \Yii::t('mall/gift_statistics', '批量操作');?>2</el-button>
                </div>
                <div>
                    <el-pagination
                            v-if="pagination"
...
<script>
    const app = new Vue({
        ...
        data() {
            return {
                url: 'mall/gift-statistics/index',
                loading: false,
                // 今天
                today: '',
                // 七天前
        ...
        methods: {
            ...
            exportBefore() {
                this.search.date_start = this.date_start;
                this.search.date_end = this.date_end;
                this.search.order = this.order;
                this.search.name = this.search.mch;
            },
