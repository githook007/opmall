            ...
            <div slot="header">
                <el-breadcrumb separator="/">
                    <el-breadcrumb-item><span style="color: #409EFF;cursor: pointer"
                                              @click="$navigate({r:'mall/order/refund'})"><?= \Yii::t('mall/order', '售后订单');?></span>
                    </el-breadcrumb-item>
                    <el-breadcrumb-item><?= \Yii::t('mall/order', '售后订单详情');?></el-breadcrumb-item>
                </el-breadcrumb>
            </div>
            <!-- 订单进度 -->
            <div class="table-body" v-loading="loading">
                ...
                <el-row :gutter="12">
                    <el-col :span="order.type == 3 ? 12 : 8">
                        <div flex="dir:top" class="card-box">
                            <h3><?= \Yii::t('mall/order', '售后信息');?></h3>
                            <div class="item-box" flex="dir:left cross:center">
                                <span class="label"><?= \Yii::t('mall/order', '售后类型');?>:</span>
                            ...
                            <div v-if="order.refund_data && order.refund_data.goods_status" class="item-box" flex="dir:left cross:center">
                                <span class="label"><?= \Yii::t('mall/order', '货物状态');?>:</span>
                            ...
                            <div v-if="order.remark" class="item-box" flex="dir:left cross:center">
                                <span class="label"><?= \Yii::t('mall/order', '备注信息');?>:</span>
                                <div>{{ order.remark}}</div>
                            </div>
                            <div v-if="order.mobile" class="item-box" flex="dir:left cross:center">
                                <span class="label"><?= \Yii::t('mall/order', '联系方式');?>:</span>
                                <div>{{ order.mobile}}</div>
                            </div>
                            <div class="item-box" flex="dir:left cross:center">
                                <span class="label" flex-box="0"><?= \Yii::t('mall/order', '订单号');?>:</span>
                                <div flex-box="1">{{ order.order_no }}</div>
                            </div>
                            <div class="item-box" flex="dir:left cross:center">
                                <span class="label" flex-box="0"><?= \Yii::t('mall/order', '实付金额');?>:</span>
                                <div flex-box="1">
                                    {{ order.order.total_pay_price }}
                                    <span v-if="order.order.express_price > 0" style="color: #999999;">(<?= \Yii::t('mall/order', '含运费');?>￥{{order.order.express_price}})</span>
                                </div>
                            </div>
                            <template v-if="order.type == 1 || order.type == 3">
                                <div class="item-box" flex="dir:left cross:center">
                                    <span class="label" flex-box="0"><?= \Yii::t('mall/order', '申请退款');?>:</span>
                                    <div flex-box="1">￥{{ order.refund_price }}</div>
                                </div>
                                <div v-if="order.is_refund == 1" class="item-box" flex="dir:left cross:center">
                                    <span class="label" flex-box="0"><?= \Yii::t('mall/order', '实际退款');?>:</span>
                                    <div flex-box="1">￥{{ order.reality_refund_price }}</div>
                                </div>
                            </template>
                            <div class="item-box" flex="dir:left cross:center">
                                <span class="label" flex-box="0"><?= \Yii::t('mall/order', '买家');?>:</span>
                    ...
                    <el-col v-show="order.type" v-if="order.type != 3" :span="8">
                        <div flex="dir:top" class="card-box">
                            <h3><?= \Yii::t('mall/order', '售后物流');?></h3>
                            <template v-if='order.order.send_type == 1'>
                                <div><?= \Yii::t('mall/order', '到店自提');?></div>
                            </template>
                            <template v-else-if='order.order.send_type == 2'>
                                <div><?= \Yii::t('mall/order', '同城配送');?></div>
                            </template>
                            <template v-else-if="(order.type == 1 || order.type == 2) && order.status == 2">
                                <div v-if="order.merchant_express && order.merchant_express_no">
                                    <div class="item-box" flex="dir:left cross:center">
                                        <span class="label"><?= \Yii::t('mall/order', '收货人');?>:</span>
                                        <div>{{order.user ? order.user.nickname : '<?= \Yii::t('mall/order', '未知用户');?>'}}</div>
                                    </div>
                                    <div class="item-box" flex="dir:left cross:center">
                                        <span class="label"><?= \Yii::t('mall/order', '快递信息');?>:</span>
                                        <div>
                                            ...
                                            <a :href="'https://www.baidu.com/s?wd='+ order.merchant_express + order.merchant_express_no"
                                               target="_blank" title='<?= \Yii::t('mall/order', '点击搜索运单号');?>'>{{order.merchant_express_no}}</a>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="item-box" flex="dir:left cross:center">
                                        ...
                                        <div>
                                            ...
                                            <a :href="'https://www.baidu.com/s?wd='+ order.express + order.express_no"
                                               target="_blank" title='<?= \Yii::t('mall/order', '点击搜索运单号');?>'>{{order.express_no}}</a>
                                        </div>
                                    </div>
                                </div>
                            </template>
                ...
                <el-card shadow="never" style="margin-top: 15px;">
                    <el-table stripe border :data="order.detail" style="width: 100%;margin-bottom: 15px;">
                        <el-table-column prop="goods" label="<?= \Yii::t('mall/order', '商品标题');?>">
                        ...
                        <el-table-column align="center" prop="attr" label="<?= \Yii::t('mall/order', '规格');?>" width="220">
                            <template slot-scope="scope">
                        ...
                        <el-table-column align="center" prop="unit_price" label="<?= \Yii::t('mall/order', '小计');?>" width="120">
                            <template slot-scope="scope">
                                ￥{{scope.row.goods_info.goods_attr.price}}
                            </template>
                        </el-table-column>
                        <el-table-column align="center" prop="num" label="<?= \Yii::t('mall/order', '数量');?>" width="80"></el-table-column>
                        <el-table-column align="center" prop="total_original_price" label="<?= \Yii::t('mall/order', '原价');?>" width="120">
                            <template slot-scope="scope">
                                ￥{{scope.row.goods_info.goods_attr.original_price}}
                            </template>
                        </el-table-column>
                        <el-table-column align="center" prop="total_price" label="<?= \Yii::t('mall/order', '折扣后');?>" width="120">
                            <template slot-scope="scope">
                    ...
                    <el-form label-width="100px" :model="order" class="app-order-count-price">
                        <el-form-item label="<?= \Yii::t('mall/order', '商品小计');?>">
                            <span>￥{{ order.order.total_goods_original_price }}</span>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('mall/order', '运费');?>">
                            <span>￥{{ order.order.express_original_price }}</span>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('mall/order', '实付款');?>">
                ...
                <div class="action-box" flex="dir:right">
                    <div>
                        <el-button v-if="order.action_status.is_show_cancel_refund" size="small" type="primary"
                                   @click="openDialog(order, cancelVisible = true)"><?= \Yii::t('mall/order', '取消售后');?>
                        </el-button>
                        <el-button v-if="order.action_status.is_show_apply == 1" size="small" type="primary"
                                   @click="openDialog(order, agreeRefundVisible = true)"><?= \Yii::t('mall/order', '同意');?>
                        </el-button>
                        <el-button v-if="order.action_status.is_show_apply == 1" size="small" type="danger"
                                   @click="openDialog(order, remarkVisible = true)"><?= \Yii::t('mall/order', '拒绝');?>
                        </el-button>
                        <el-button v-if="order.action_status.is_show_confirm == 1" size="small"
                                   type="primary" @click="shouHuo(order)"><?= \Yii::t('mall/order', '确认收货');?>
                        </el-button>
                        <el-button
                                v-if="order.action_status.is_show_refund == 1"
                                size="small" type="primary" @click="openDialog(order, refundConfirmVisible = true)"><?= \Yii::t('mall/order', '打款');?>
                        </el-button>
                        <el-button size="small" type="primary" @click="toDetail(order.order_id)"><?= \Yii::t('mall/order', '订单详情');?></el-button>
                    </div>
                </div>
            </div>
        </el-card>
            ...
            setStepActive() {
                ...
                if (this.order.status != 1) {
                    ...
                    if (this.order.status != 1 && this.order.status == 2) {
                        this.steps[1].title = '<?= \Yii::t('mall/order', '卖家同意售后');?>';
                        this.steps[1].icon = 'statics/img/mall/order/refund_status/status_2_1_active.png';
                    } else {
                        this.steps[1].title = '<?= \Yii::t('mall/order', '卖家拒绝售后');?>';
                ...
                if (this.order.is_confirm == 1 && this.order.status == 2) {
                    if ((this.order.type == 1 || this.order.type == 3) && (this.order.is_refund == 1 || this.order.is_refund == 2)) {
                        this.stepActive = 3;
                        this.steps[2].title = '<?= \Yii::t('mall/order', '已完成售后');?>';
