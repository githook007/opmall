    ...
    <el-card shadow="never" style="border:0" class="box-card" body-style="background-color: #f3f3f3;padding: 10px 0 0;" class="box-card" v-loading="cardLoading">
        <div slot="header">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><span style="color: #409EFF;cursor: pointer" @click="$navigate({r:'mall/service/index'})"><?= \Yii::t('mall/service', '服务');?></span></el-breadcrumb-item>
                <el-breadcrumb-item v-if="edit"><?= \Yii::t('mall/service', '服务编辑');?></el-breadcrumb-item>
                <el-breadcrumb-item v-else><?= \Yii::t('mall/service', '添加服务');?></el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="form-body">
            <el-form :model="ruleForm" :rules="rules" size="small" ref="ruleForm" label-width="120px">
                <el-form-item prop="name">
                    <template slot='label'>
                        <span><?= \Yii::t('mall/service', '服务名称');?></span>
                        <el-tooltip effect="dark" content="<?= \Yii::t('mall/service', '例如');?>"
                                    placement="top">
                ...
                <el-form-item  label="<?= \Yii::t('mall/service', '商品服务标识');?>">
                    <el-row>
                        <el-col :span="4">
                            <app-attachment v-model="ruleForm.pic" :multiple="false" :max="1">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/service', '建议尺寸');?>:28 * 28"
                                            placement="top">
                                    <el-button size="mini"><?= \Yii::t('mall/service', '选择图片');?></el-button>
                                </el-tooltip>
                            </app-attachment>
                        </el-col>
                        <el-col :span="4">
                            <el-button size="mini" type="primary" @click="setPic"><?= \Yii::t('mall/service', '恢复默认');?></el-button>
                        </el-col>
                    </el-row>
                    ...
                    <el-button @click="app_share.dialog = true;app_share.type = 'pic_bg'"
                               type="text"><?= \Yii::t('mall/service', '查看图例');?>
                    </el-button>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('mall/service', '排序');?>" prop="sort">
                    <el-input type="number" v-model="ruleForm.sort"></el-input>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('mall/service', '备注');?>" prop="remark">
                    <el-input :autosize="{ minRows: 6, maxRows: 6 }" type="textarea" v-model="ruleForm.remark"></el-input>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('mall/service', '是否默认');?>" prop="is_default">
                    <el-switch v-model="ruleForm.is_default" active-value="1" inactive-value="0"></el-switch>
                </el-form-item>
            </el-form>
        </div>
        <el-button class="button-item" :loading="btnLoading" type="primary" @click="store('ruleForm')" size="small"><?= \Yii::t('mall/service', '保存');?></el-button>
    </el-card>
    <!-- 自定义 -->
    <el-dialog title="<?= \Yii::t('mall/service', '查看商品服务标识图例');?>"
        ...
        <div slot="footer" class="dialog-footer">
            <el-button @click="app_share.dialog = false" type="primary"><?= \Yii::t('mall/service', '我知道了');?></el-button>
        </div>
    </el-dialog>
</div>
<script>
    const app = new Vue({
        ...
        data() {
            return {
                ...
                rules: {
                    name: [
                        {required: true, message: '<?= \Yii::t('mall/service', '请输入服务名称');?>', trigger: 'change'},
                    ],
                    sort: [
                        {required: true, message: '<?= \Yii::t('mall/service', '请输入排序');?>', trigger: 'change'},
                    ],
                },
                btnLoading: false,
                cardLoading: false,
