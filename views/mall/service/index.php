...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <div>
                <span><?= \Yii::t('mall/service', '服务列表');?></span>
                <div style="float: right;margin-top: -5px">
                    <el-button type="primary" @click="edit" size="small"><?= \Yii::t('mall/service', '添加服务');?></el-button>
                </div>
            </div>
        </div>
        <div class="table-body">
            <div class="input-item">
                <el-input @keyup.enter.native="search" size="small" placeholder="<?= \Yii::t('mall/service', '请输入搜索内容');?>" v-model="keyword"clearable @clear="getList">
            ...
            <el-table
                ...
                <el-table-column
                        label="<?= \Yii::t('mall/service', '服务名称');?>" width="350px">
                ...
                <el-table-column
                        label="<?= \Yii::t('mall/service', '服务标识');?>">
                    <template slot-scope="scope">
                        <img :src="scope.row.pic" alt="" style="width: 50px;height: 50px;">
                    </template>
                </el-table-column>
                <el-table-column
                        label="<?= \Yii::t('mall/service', '是否默认');?>"
                        width="100">
                    <template slot-scope="scope">
                        <el-switch
                ...
                <el-table-column prop="sort" label="<?= \Yii::t('mall/service', '排序');?>" width="150">
                    <template slot-scope="scope">
                        <div v-if="id != scope.row.id">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/service', '排序');?>" placement="top">
                ...
                <el-table-column
                        prop="created_at"
                        width="220"
                        label="<?= \Yii::t('mall/service', '添加日期');?>">
                </el-table-column>
                <el-table-column
                        label="<?= \Yii::t('mall/service', '操作');?>"
                        fixed="right"
                        width="220">
                    <template slot-scope="scope">
                        <el-button @click="edit(scope.row.id)" type="text" circle size="mini">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/service', '编辑');?>" placement="top">
                                <img src="statics/img/mall/edit.png" alt="">
                            </el-tooltip>
                        </el-button>
                        <el-button @click="destroy(scope.row.id, scope.$index)" circle type="text" size="mini">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/service', '删除');?>" placement="top">
...
<script>
    const app = new Vue({
        ...
        methods: {
            ...
            search: function() {
                ...
                request({
                    ...
                    if (e.data.code === 0) {
                        ...
                        this.$alert(e.data.msg, '<?= \Yii::t('mall/service', '提示');?>', {
                            confirmButtonText: '<?= \Yii::t('mall/service', '确定');?>'
                        })
                    }
                }).catch(e => {
                    this.listLoading = false;
            ...
            destroy(id, index) {
                let self = this;
                self.$confirm('<?= \Yii::t('mall/service', '删除该条数据');?>', '<?= \Yii::t('mall/service', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('mall/service', '确定');?>',
                    cancelButtonText: '<?= \Yii::t('mall/service', '取消');?>',
                    type: 'warning'
                }).then(() => {
                    self.listLoading = true;
                    request({
                    ...
                    self.$message.info('<?= \Yii::t('mall/service', '已取消删除');?>')
                });
            },
            switchChange(e) {
                let self = this;
