            ...
            <el-form :model="form" :rules="rules" ref="form" label-width="200px" size="small">
                <el-card shadow="never" style="border:0;">
                    <el-row class="table-body">
                        <div class="tab-box" flex="dir:left">
                            <div :class="{'active': activeName == 'one' ? true : false}" class="btn" @click="handleClick('one')"><?= \Yii::t('mall/cat', '一级分类样式');?></div>
                            <div :class="{'active': activeName == 'two' ? true : false}" class="btn" @click="handleClick('two')"><?= \Yii::t('mall/cat', '二级分类样式');?></div>
                            <div :class="{'active': activeName == 'three' ? true : false}" class="btn" @click="handleClick('three')"><?= \Yii::t('mall/cat', '三级分类样式');?></div>
                        </div>
                ...
                <el-card shadow="never" style="border:0;">
                    <template v-if="!is_mch">
                        <div slot="header">
                            <span><?= \Yii::t('mall/cat', '显示设置');?></span>
                        </div>
                        <el-row class="new-table-body">
                            <el-form-item label="" prop="cat_goods_count">
                                <template slot='label'>
                                    <span><?= \Yii::t('mall/cat', '每个分类商品显示总数');?></span>
                                    <el-tooltip effect="dark" content="<?= \Yii::t('mall/cat', '每个分类板块显示的商品最大数');?>(0~100)"
                                                placement="top">
                                        <i class="el-icon-info"></i>
                                    </el-tooltip>
                                </template>
                                <el-input v-model="form.cat_goods_count" style="width:30%" type="number">
                                    <template slot="append"><?= \Yii::t('mall/cat', '个');?></template>
                                </el-input>
                            </el-form-item>
                            <el-form-item prop="cat_goods_cols">
                                <template slot='label'>
                                    <span><?= \Yii::t('mall/cat', '商品每行显示数量');?></span>
                                    <el-tooltip effect="dark" content="<?= \Yii::t('mall/cat', '默认首页显示');?>"
                                                placement="top">
                                        <i class="el-icon-info"></i>
                                    </el-tooltip>
                                </template>
                                <el-select v-model="form.cat_goods_cols" placeholder="<?= \Yii::t('mall/cat', '请选择');?>">
    ...
    Vue.component('app-style', {
        ...
        data() {
            return {
                ...
                rules: {
                    cat_goods_count: [
                        {required: true, message: '<?= \Yii::t('mall/cat', '每个分类商品显示总数不能为空');?>', trigger: 'change'},
                    ],
                    cat_goods_cols: [
                        {required: true, message: '<?= \Yii::t('mall/cat', '商品每行显示数量不能为空');?>', trigger: 'change'},
                    ]
                },
                is_mch: <?= $mchId > 0 ? 1 : 0 ?>,
            }
