    ...
    <div class="app-cat-list">
        <el-form size="small" :inline="true" :model="search" @submit.native.prevent>
                ...
                <el-form-item>
                    <div class="input-item">
                        <el-input @keyup.enter.native="searchCat" clearable @clear="searchCat" size="small"
                                  placeholder="<?= \Yii::t('mall/cat', '请输入搜索内容');?>"
            ...
            <el-form-item v-if="!isEditSort">
                <el-button @click="isEditSort=true" style="margin-left: 10px" type="primary"><?= \Yii::t('mall/cat', '编辑排序');?></el-button>
            </el-form-item>
            <el-form-item v-if="isEditSort">
                <el-button :loading="submitLoading" @click="storeSort" style="margin-left: 10px" type="primary"><?= \Yii::t('mall/cat', '保存排序');?>
                </el-button>
                <el-button @click="isEditSort=false" style="margin-left: 10px"><?= \Yii::t('mall/cat', '取消编辑');?>
                </el-button>
                <span style="margin-left: 10px;"><?= \Yii::t('mall/cat', '拖动分类名称排序');?></span>
            </el-form-item>
        </el-form>
                                    ...
                                    <el-form v-if="!isEditSort" flex="cross:center" :inline="true"
                                        ...
                                        <el-form-item>
                                            <el-button style="display: block;" type="text"
                                                ...
                                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/cat', '编辑');?>" placement="top">
                                        ...
                                        <el-form-item>
                                            <el-button style="display: block;" type="text"
                                                ...
                                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/cat', '删除');?>" placement="top">
        ...
        <el-dialog :visible.sync="searchFinish">
            <el-table border :data="searchList" @row-click="rowClick">
                <el-table-column align="center" property="status_text" label="<?= \Yii::t('mall/cat', '分类等级');?>" width="200">
                    <template slot-scope="scope">
                        <span v-if="!scope.row.status_text"><?= \Yii::t('mall/cat', '一级分类');?></span>
                        <span v-else>{{scope.row.status_text}}</span>
                    </template>
                </el-table-column>
                <el-table-column align="center" property="name" label="<?= \Yii::t('mall/cat', '分类名称');?>" width="300">
                    <template slot-scope="scope">
                ...
                <el-table-column align="center" label="<?= \Yii::t('mall/cat', '操作');?>">
                    <template slot-scope="scope">
...
<script>
    Vue.component('app-cat-list', {
        ...
        methods: {
            ...
            destroy(row) {
                let self = this;
                self.$confirm('<?= \Yii::t('mall/cat', '删除该条数据');?>?', '<?= \Yii::t('mall/cat', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('mall/cat', '确定');?>',
                    cancelButtonText: '<?= \Yii::t('mall/cat', '取消');?>',
                    type: 'warning'
                }).then(() => {
                    self.listLoading = true;
                    request({
                    ...
                    self.$message.info('<?= \Yii::t('mall/cat', '已取消删除');?>')
                });
            },
            switchStatus(id) {
                let self = this;
