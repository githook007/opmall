...
<div id="app" v-cloak>
        ...
        <div slot="header">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><span style="color: #409EFF;cursor: pointer"
                                          @click="$navigate({r:'mall/cat/index'})"><?= \Yii::t('mall/cat', '分类');?></span></el-breadcrumb-item>
                <el-breadcrumb-item><?= \Yii::t('mall/cat', '分类编辑');?></el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="form-body">
            <el-form :model="ruleForm" :rules="rules" size="small" ref="ruleForm" label-width="120px">
                <el-form-item label="选择分类级别">
                    <div flex="dir:left" class="radio-box">
                        <div @click="changeParent(1)">
                            <el-radio :disabled="ruleForm.id ? true : false" v-model="currentRadioKey" :label="1"><?= \Yii::t('mall/cat', '一级分类');?></el-radio>
                        </div>
                        <div @click="changeParent(2)">
                            <el-radio :disabled="ruleForm.id ? true : false"  v-model="currentRadioKey" :label="2"><?= \Yii::t('mall/cat', '二级分类');?></el-radio>
                        </div>
                        <div @click="changeParent(3)">
                            <el-radio :disabled="ruleForm.id ? true : false" v-model="currentRadioKey" :label="3"><?= \Yii::t('mall/cat', '三级分类');?></el-radio>
                        </div>
                    </div>
                </el-form-item>
                <el-form-item v-if="currentCatLevelShow == 2 && currentCatShow.value" label="<?= \Yii::t('mall/cat', '一级分类');?>">
                    <div flex="dir:left">
                        <div>{{currentCatShow.label}}</div>
                        <div v-if="!ruleForm.id" @click="changeParent(currentCatLevelShow)" class="cat-edit-text"><?= \Yii::t('mall/cat', '修改');?></div>
                    </div>
                </el-form-item>
                <template v-if="currentCatLevelShow == 3 && currentCatShow.value">
                    <el-form-item label="<?= \Yii::t('mall/cat', '一级分类');?>">
                        <div flex="dir:left">
                            <div>{{currentCatShow.label}}</div>
                            <div v-if="!currentCatShow.oneChildren && !ruleForm.id" @click="changeParent(currentCatLevelShow)"
                                 class="cat-edit-text"><?= \Yii::t('mall/cat', '修改');?>
                            </div>
                        </div>
                    </el-form-item>
                    <el-form-item v-if="currentCatShow.oneChildren" label="<?= \Yii::t('mall/cat', '二级分类');?>">
                        <div flex="dir:left">
                ...
                <el-form-item label="<?= \Yii::t('mall/cat', '分类名称');?>" prop="name">
                    <el-input v-model="ruleForm.name"></el-input>
                </el-form-item>
                <el-form-item prop="sort">
                    <template slot='label'>
                        <span><?= \Yii::t('mall/cat', '排序');?></span>
                        <el-tooltip effect="dark" content="<?= \Yii::t('mall/cat', '排序值越小排序越靠前');?>"
                                    placement="top">
                ...
                <el-form-item label="<?= \Yii::t('mall/cat', '分类图标');?>" prop="pic_url">
                    <app-attachment :multiple="false" :max="1" v-model="ruleForm.pic_url">
                        <el-tooltip effect="dark" content="<?= \Yii::t('mall/cat', '建议尺寸200');?>" placement="top">
                            <el-button style="margin-bottom: 10px;" size="mini"><?= \Yii::t('mall/cat', '选择文件');?></el-button>
                        </el-tooltip>
                    </app-attachment>
                ...
                <el-form-item prop="pic_url">
                    <template slot='label'>
                        <span><?= \Yii::t('mall/cat', '分类大图');?></span>
                        <el-tooltip effect="dark" content="<?= \Yii::t('mall/cat', '显示在分类页面');?>"
                                    placement="top">
                            <i class="el-icon-info"></i>
                        </el-tooltip>
                    </template>
                    <app-attachment :multiple="false" :max="1" v-model="ruleForm.big_pic_url">
                        <el-tooltip effect="dark" content="<?= \Yii::t('mall/cat', '建议尺寸702');?>" placement="top">
                            <el-button style="margin-bottom: 10px;" size="mini"><?= \Yii::t('mall/cat', '选择文件');?>
                            </el-button>
                        </el-tooltip>
                    </app-attachment>
                ...
                <template v-if="!is_mch">
                    <el-form-item prop="advert_pic">
                        <template slot='label'>
                            <span><?= \Yii::t('mall/cat', '分类广告图');?></span>
                            <el-tooltip effect="dark" content="<?= \Yii::t('mall/cat', '小图标模式下显示');?>"
                                        placement="top">
                                <i class="el-icon-info"></i>
                            </el-tooltip>
                        </template>
                        <app-attachment :multiple="false" :max="1" v-model="ruleForm.advert_pic">
                            <el-tooltip effect="dark" content="<?= \Yii::t('mall/cat', '建议尺寸500');?>" placement="top">
                                <el-button style="margin-bottom: 10px;" size="mini"><?= \Yii::t('mall/cat', '选择文件');?></el-button>
                            </el-tooltip>
                        </app-attachment>
                    ...
                    <el-form-item label="<?= \Yii::t('mall/cat', '分类广告链接');?>" prop="advert_url">
                        <div flex="box:last">
                            <el-input disabled v-model="ruleForm.advert_url"></el-input>
                            <app-pick-link @selected="selectAdvertUrl">
                                <el-button><?= \Yii::t('mall/cat', '选择链接');?></el-button>
                            </app-pick-link>
                        </div>
                    </el-form-item>
                </template>
                <el-form-item label="<?= \Yii::t('mall/cat', '是否启用');?>" prop="status">
                    <el-switch v-model="ruleForm.status" :active-value="1" :inactive-value="0"></el-switch>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('mall/cat', '是否显示');?>" prop="is_show">
                    <el-switch v-model="ruleForm.is_show" :active-value="1" :inactive-value="0"></el-switch>
                </el-form-item>
            </el-form>
        </div>
        <el-button class="button-item" :loading="btnLoading" type="primary" @click="store('ruleForm')" size="small"><?= \Yii::t('mall/cat', '保存');?>
        </el-button>
    </el-card>
    <div class="el-dialog-1">
        <el-dialog
                title="<?= \Yii::t('mall/cat', '选择归属的一级分类');?>"
                :visible.sync="dialogVisible1"
                width="20%">
            <el-form v-loading="dialogLoading" size="small" :inline="true" :model="search" @submit.native.prevent>
                <el-form-item>
                    <div class="input-item">
                        <el-input @keyup.enter.native="searchCat" size="small" placeholder="<?= \Yii::t('mall/cat', '请输入分类名称搜索');?>"
            ...
            <el-button size="small" @click="dialogVisible1 = false"><?= \Yii::t('mall/cat', '取消');?></el-button>
            <el-button size="small" type="primary" @click="selectCatSubmit"><?= \Yii::t('mall/cat', '确定');?></el-button>
        </span>
        </el-dialog>
    </div>
    <div class="el-dialog-2">
        <el-dialog
                title="<?= \Yii::t('mall/cat', '选择归属的二级分类');?>"
            ...
            <span slot="footer" class="dialog-footer">
            <el-button size="small" @click="dialogVisible2 = false"><?= \Yii::t('mall/cat', '取消');?></el-button>
            <el-button size="small" type="primary" @click="selectCatSubmit"><?= \Yii::t('mall/cat', '确定');?></el-button>
        </span>
        </el-dialog>
    </div>
    <el-dialog
        ...
        <div slot="footer">
            <el-button type="primary" size="small" @click="imageDialogVisible = false"><?= \Yii::t('mall/cat', '我知道了');?></el-button>
        </div>
    </el-dialog>
</div>
<script>
    const app = new Vue({
        ...
        data() {
            return {
                ...
                rules: {
                    name: [
                        {required: true, message: '<?= \Yii::t('mall/cat', '请输入分类名称');?>', trigger: 'change'},
                        {max: 16, message: '<?= \Yii::t('mall/cat', '最多输入16个字符');?>', trigger: 'change'},
                    ],
                    sort: [
                        {required: true, message: '<?= \Yii::t('mall/cat', '请输入排序');?>', trigger: 'change'},
                    ],
                },
                btnLoading: false,
                cardLoading: false,
        ...
        methods: {
            ...
            imageShow(type) {
                ...
                if (type === 1) {
                    this.imageShowData.title = '<?= \Yii::t('mall/cat', '分类大图图例');?>';
                    this.imageShowData.image_url = 'statics/img/mall/cat/example-1.png';
                }
                if (type === 2) {
                    this.imageShowData.title = '<?= \Yii::t('mall/cat', '分类广告图图例');?>';
