    ...
    <el-card shadow="never" v-loading="loading">
        <div flex style="margin-bottom: 20px">
            <app-attachment-edit @save="loadData()" submit-url="mall/attachment/attachment-create-storage"
                ...
                <el-button size="small"><?= \Yii::t('mall/attachment', '添加存储位置');?></el-button>
            </app-attachment-edit>
            <el-button size="small" style="margin-left: 20px" @click="storageShow" :loading="dialog.loading"><?= \Yii::t('mall/attachment', '选取账户存储');?>
            </el-button>
        </div>
        <div>
            <div><?= \Yii::t('mall/attachment', '商城创建者');?>{{nickname}}</div>
            <div><?= \Yii::t('mall/attachment', '商城当前上传设置');?>{{storage}}</div>
        </div>
        <el-table border :data="list" style="width: 100%">
            <el-table-column label="<?= \Yii::t('mall/attachment', '存储位置');?>">
                <template slot-scope="scope">
                    {{storageTypes[scope.row.type]}}
                </template>
            </el-table-column>
            <el-table-column label="<?= \Yii::t('mall/attachment', '使用中');?>">
                <template slot-scope="scope">
            ...
            <el-table-column label="<?= \Yii::t('mall/attachment', '操作');?>">
                <template slot-scope="scope">
                    <app-attachment-edit @save="loadData()" :item="scope.row" :storage-types="storageTypes"
                        ...
                        <el-button size="mini" type="text" circle>
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/attachment', '编辑');?>" placement="top">
        ...
        <el-dialog :visible.sync="dialog.dialogVisible" title="<?= \Yii::t('mall/attachment', '存储方式选择');?>" :close-on-click-modal="false">
            <el-form ref="form" :model="dialog.form" :rules="dialog.rules" label-position="left"
                     v-if="dialog.list.length > 0">
                <el-form-item label="<?= \Yii::t('mall/attachment', '存储位置');?>" prop="type">
                ...
                <el-form-item>
                    <el-button type="primary" size="small" @click="handleSaveStorage('form')"><?= \Yii::t('mall/attachment', '保存');?></el-button>
                </el-form-item>
            </el-form>
            <div v-else><?= \Yii::t('mall/attachment', '账户尚未设置存储方式');?></div>
        </el-dialog>
    </el-card>
</div>
<script>
    new Vue({
        ...
        data() {
            return {
                ...
                dialog: {
                    ...
                    rules: {
                        id: [
                            {required: true, message: '<?= \Yii::t('mall/attachment', '请选择存储位置');?>',},
                        ]
                    }
                },
                nickname: '',
        ...
        methods: {
            ...
            handleEnable(item) {
                this.$confirm('<?= \Yii::t('mall/attachment', '确认切换存储位置');?>', '<?= \Yii::t('mall/attachment', '提示');?>').then(e => {
                    this.$request({
                        params: {
                ...
                this.$refs[formName].validate(valid => {
                    if (valid) {
                        this.$request({
                            ...
                            if (e.data.code === 0) {
                                this.$message.success('<?= \Yii::t('mall/attachment', '保存成功');?>');
                        ...
                        this.$message.error('<?= \Yii::t('mall/attachment', '提交内容有误');?>');
                    }
                });
            }
        },
