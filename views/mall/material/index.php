...
<div id="app" v-cloak class="material material-dialog">
    <el-card shadow="never" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
                ...
                <el-breadcrumb separator="/" style="display: inline-block">
                    <el-breadcrumb-item>
                        <span v-if="is_recycle" style="color: #409EFF;cursor: pointer"
                              @click="$navigate({r:'mall/material/index'})"><?= \Yii::t('mall/material', '素材管理');?></span>
                        <span v-else><?= \Yii::t('mall/material', '素材管理');?></span>
                    </el-breadcrumb-item>
                    <el-breadcrumb-item v-if="is_recycle"><?= \Yii::t('mall/material', '回收站');?></el-breadcrumb-item>
                </el-breadcrumb>
                <div style="float: right; margin: -5px 0" v-if="!is_recycle">
                    <el-button type="primary" @click="recoverClick" size="small"><?= \Yii::t('mall/material', '回收站');?></el-button>
                </div>
            </div>
        </div>
        ...
        <div class="table-body">
            <div flex="main:center cross:center" v-if="space.memory">
            ...
            <el-tabs v-model="tabs" @tab-click="dialogOpened">
                <el-tab-pane v-for="(v,index) in tab_list" :key="index" :label="v.label" :name="v.name">
                    <div flex="cross:center" style="margin-bottom: 12px;">
                        ...
                        <el-button size="small" v-else type="primary" @click="showAddGroup(-1)"><?= \Yii::t('mall/material', '添加分组');?></el-button>
                        <div style="margin-left:200px"><?= \Yii::t('mall/material', '全部');?></div>
                        <div flex="cross:center" style="margin-left:auto">
                            <div class="search" style="margin-right: 12px">
                                <el-input placeholder="<?= \Yii::t('mall/material', '请输入名称搜索');?>" v-model="keyword"
                            ...
                            <el-button v-if="is_recycle"
                                       ...
                                       @click="deleteItems(2)"
                                       size="small"
                                       style="margin-right: 12px"><?= \Yii::t('mall/material', '还原');?>
                            </el-button>
                            <el-button :loading="deleteLoading"
                                       @click="deleteItems(is_recycle?3:1)"
                                       size="small"
                                       style="margin-right: 12px"><?= \Yii::t('mall/material', '删除');?>
                            </el-button>
                            <el-dropdown v-if="!is_recycle"
                                         ...
                                         size="small"
                                         @command="moveItems">
                                <span><?= \Yii::t('mall/material', '移动至');?></span>
                            ...
                            <el-menu class="group-menu"
                                ...
                                <el-scrollbar style="height:635px;width:100%">
                                    <el-menu-item index="all" @click="switchGroup(-1)">
                                        ...
                                        <span><?= \Yii::t('mall/material', '全部');?></span>
                                    </el-menu-item>
                                        ...
                                        <el-menu-item :index="'' + index" @click="switchGroup(index)">
                                            <div flex="dir:left box:last">
                                                ...
                                                <div v-if="is_recycle" flex="dir:left">
                                                    <el-button @click.stop="deleteGroup(index,2)" type="text"><?= \Yii::t('mall/material', '还原');?></el-button>
                                                    <div style="color:#409EFF;margin:0 2px">|</div>
                                                    <el-button type="text" @click.stop="deleteGroup(index,3)"><?= \Yii::t('mall/material', '删除');?></el-button>
                                                </div>
                                                <div v-else flex="dir:left">
                                                    <el-button type="text" @click.stop="showAddGroup(index)"><?= \Yii::t('mall/material', '编辑');?>
                                                    </el-button>
                                                    <div style="color:#e3e3e3;margin:0 2px">|</div>
                                                    <el-button type="text" @click.stop="deleteGroup(index,1)"><?= \Yii::t('mall/material', '删除');?></el-button>
                                                </div>
                                            </div>
                                        </el-menu-item>
                                    </template>
                                            ...
                                            <div flex="dir:left" style="margin-top:5px">
                                                ...
                                                <div>
                                                    <el-button @click.stop="showPicModel(index)" type="text"
                                                               style="padding:0"><?= \Yii::t('mall/material', '编辑');?>
                                                    </el-button>
                                                </div>
                                            </div>
                                        </div>
                ...
                <!-- 分类 -->
                <el-dialog append-to-body title="<?= \Yii::t('mall/material', '分组管理');?>" :visible.sync="addGroupVisible" :close-on-click-modal="false"
                           width="30%">
                    <el-form @submit.native.prevent label-width="90px" ref="groupForm" :model="groupForm"
                             :rules="groupFormRule">
                        <el-form-item label="<?= \Yii::t('mall/material', '分组名称');?>" prop="name" style="margin-bottom: 22px;">
                        ...
                        <el-form-item style="text-align: right">
                            <el-button type="primary" @click="groupFormSubmit('groupForm')" :loading="groupFormLoading"
                                       size="medium"><?= \Yii::t('mall/material', '保存');?>
                            </el-button>
                        </el-form-item>
                    </el-form>
                </el-dialog>
                <!-- 名称修改 -->
                <el-dialog append-to-body :title="tab_list[tabs=='image' ? 0: 1]['label'] + '<?= \Yii::t('mall/material', '名称修改');?>'"
                    ...
                    <el-form @submit.native.prevent label-width="90px" ref="picForm" :model="picForm"
                             :rules="picFormRule">
                        <el-form-item :label="tab_list[tabs=='image' ? 0: 1]['label'] + '<?= \Yii::t('mall/material', '名称');?>'" prop="name"
...
<script>
    const app = new Vue({
        ...
        data() {
            return {
                tab_list: [
                    {label: '<?= \Yii::t('mall/material', '图片');?>', name: 'image'},
                    {label: '<?= \Yii::t('mall/material', '视频');?>', name: 'video'}
                ],
                tabs: 'image',
                is_recycle: 0,
                keyword: '',
                ...
                groupFormRule: {
                    name: [
                        {required: true, message: '<?= \Yii::t('mall/material', '请填写分组名称');?>',}
                    ],
                },
                ...
                picFormRule: {
                    name: [
                        {required: true, message: '<?= \Yii::t('mall/material', '请填写名称');?>',}
                    ],
                },
        ...
        methods: {
            ...
            deleteItems(type) {
                ...
                if (!itemIds.length) {
                    this.$message.warning('<?= \Yii::t('mall/material', '请先选择需要处理的图片');?>');
                    return;
                }
                ...
                switch (type) {
                    case 1:
                        title = '<?= \Yii::t('mall/material', '是否确认将选中素材放入回收站中');?>';
                        break;
                    case 2:
                        title = '<?= \Yii::t('mall/material', '确认还原选择素材');?>';
                        break;
                    case 3:
                        title = '<?= \Yii::t('mall/material', '素材删除后将无法恢复');?>';
                        break;
                    default:
                        title = '';
                        break;
                }
                this.$confirm(title, '<?= \Yii::t('mall/material', '提示');?>', {
                    type: 'warning'
                }).then(() => {
            ...
            moveItems(index) {
                ...
                if (!itemIds.length) {
                    this.$message.warning('<?= \Yii::t('mall/material', '请先选择需要移动的图片');?>');
                    return;
                }
                this.$confirm('<?= \Yii::t('mall/material', '确认移动所选的');?>' + itemIds.length + '<?= \Yii::t('mall/material', '张图片');?>', '<?= \Yii::t('mall/material', '提示');?>', {
                    type: 'warning'
                }).then(() => {
            ...
            deleteGroup(index, type) {
                ...
                switch (type) {
                    case 1:
                        title = '<?= \Yii::t('mall/material', '是否确认将分组放入回收站中');?>';
                        break;
                    case 2:
                        title = '<?= \Yii::t('mall/material', '确认还原选择分组');?>';
                        break;
                    case 3:
                        title = '<?= \Yii::t('mall/material', '分组删除后将无法恢复');?>';
                        break;
                    default:
                        title = '';
                        break;
                ...
                this.$confirm(title, '<?= \Yii::t('mall/material', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('mall/material', '确定');?>',
                    cancelButtonText: '<?= \Yii::t('mall/material', '取消');?>',
                    type: 'warning'
                }).then(() => {
                    this.$request({
                    ...
                    if (this.attachments[i].type == 2) {
                        ...
                        const timer = setInterval(() => {
                            ...
                            try {
                                ...
                                console.warn('<?= \Yii::t('mall/material', '获取视频封面异常');?>: ', e);
                            }
