...
<div id="app" v-cloak>
    <el-card v-loading="loading" shadow="never" style="border:0"
        ...
        <div slot="header">
            <span><?= \Yii::t('mall/data_statistics', '数据概况');?></span>
        </div>
        <!-- 公告 -->
        ...
        <div class="table-area" style="margin-bottom: 10px">
            <el-card shadow="never">
                <div slot="header">
                    <form>
                        <span style="float: left;height: 32px;line-height: 32px"><?= \Yii::t('mall/data_statistics', '商品购买力TOP排行');?></span>
                        <div flex="dir:right" style="">
                            <app-new-export-dialog-2
                                    text='<?= \Yii::t('mall/data_statistics', '导出TOP100');?>'
                                    :params="search"
                                    :directly=true
                ...
                <el-table v-loading="goods_loading" :row-style="{height:'63px'}" @sort-change="changeGoods"
                    ...
                    <el-table-column align="center" label="<?= \Yii::t('mall/data_statistics', '排名');?>">
                    ...
                    <el-table-column prop="name" width="400" label="<?= \Yii::t('mall/data_statistics', '商品');?>">
                        <template slot-scope="scope">
                            <div class="text-omit">{{scope.row.name}}</div>
                        </template>
                    </el-table-column>
                    <el-table-column prop="total_price" label="<?= \Yii::t('mall/data_statistics', '销售额');?>" width="150" sortable='custom'
                                     :label-class-name="goodsProp == 'total_price' ? 'sort-active': ''">
                    </el-table-column>
                    <el-table-column prop="num" label="<?= \Yii::t('mall/data_statistics', '销量');?>" width="150" sortable='custom'
            ...
            <el-card shadow="never">
                <div slot="header">
                    <form>
                        <span style="float: left;height: 32px;line-height: 32px"><?= \Yii::t('mall/data_statistics', '用户购买力TOP排行');?></span>
                        <div flex="dir:right" style="">
                            <app-new-export-dialog-2
                                    text='<?= \Yii::t('mall/data_statistics', '导出TOP100');?>'
                                    :params="search"
                ...
                <el-table v-loading="user_loading" :row-style="{height:'63px'}" @sort-change="changeUser"
                    ...
                    <el-table-column prop="name" label="<?= \Yii::t('mall/data_statistics', '用户');?>" width="400">
                        <template slot-scope="scope">
                    ...
                    <el-table-column prop="total_price" label="<?= \Yii::t('mall/data_statistics', '支付金额');?>" width="150"
                                     :label-class-name="userProp == 'total_price' ? 'sort-active': ''"
                                     sortable='custom'>
                    </el-table-column>
                    <el-table-column prop="num" label="<?= \Yii::t('mall/data_statistics', '支付件数');?>" width="150"
                                     :label-class-name="userProp == 'num' ? 'sort-active': ''" sortable='custom'>
                    </el-table-column>
                </el-table>

                <el-dialog :title="noticeTitle" :visible.sync="dialogNotice" width="35%">
                    <div v-html="noticeContent">
                        {{ noticeContent }}
                    </div>
                    <div slot="footer" class="dialog-footer">
                        <el-button @click="dialogNotice = false">取消</el-button>
                        <el-button :loading="loading" type="primary" @click="iDo(noticeId)">不再提醒</el-button>
                    </div>
                </el-dialog>
            </el-card>
        </div>
    ...
    const app = new Vue({
        ...
        methods: {
            ...
            noticeEject(){
                ...
                request({
                    params: {
                        r: 'mall/data-statistics/notice-eject',
                    },
                    data: {},
                }).then(e => {
                    this.loading = false;
                    if (e.data.code === 0) {
                        this.noticeTitle = e.data.data.type_text;
                        this.noticeContent = e.data.data.content;
                        this.noticeId = e.data.data.id;
            ...
            iDo(id){
                ...
                request({
                    params: {
                        r: 'mall/data-statistics/notice-i-do',
                    },
                    data: {
                        id: id
                    },
