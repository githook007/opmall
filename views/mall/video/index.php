...
<div id="app" v-cloak>
    <el-card class="box-card" v-loading="listLoading" shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <div>
                <span><?= \Yii::t('mall/video', '视频');?></span>
                <div style="float: right;margin-top: -5px">
                    <el-button type="primary" @click="$navigate({r: 'mall/video/edit'})" size="small"><?= \Yii::t('mall/video', '添加视频');?></el-button>
                </div>
            </div>
        </div>
                ...
                <el-card :body-style="{ padding: '0px'}" :class="item.id == edit_id ? 'active':''" shadow="never">
                    <div style="position:relative">
                        ...
                        <div v-if="item.id == edit_id" style="position:absolute;bottom:20px;right:20px">
                            <el-button type="text" class="set-el-button" size="mini" circle @click="$navigate({r: 'mall/video/edit', id:item.id})">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/video', '编辑');?>" placement="top">
                                    <img src="statics/img/mall/edit.png" alt="">
                                </el-tooltip>
                            </el-button>
                            <el-button type="text" class="set-el-button" size="mini" circle @click="destroy(item)">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/video', '删除');?>" placement="top">
                    ...
                    <div class="end">
                        ...
                        <div flex="dir:left box:first cross:center">
                            ...
                            <div v-if="id != item.id">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/video', '排序');?>" placement="top">
                                    <span>{{item.sort}}</span>
                                </el-tooltip>
    ...
    const app = new Vue({
        ...
        methods: {
            ...
            destroy: function(column) {
                this.$confirm('<?= \Yii::t('mall/video', '确认删除该记录吗');?>?', '<?= \Yii::t('mall/video', '提示');?>', {
                    type: 'warning'
                }).then(() => {
