...
<div id="app" v-cloak>
    <el-card v-loading="loading" shadow="never" style="border:0"
        ...
        <div slot="header">
            <app-header :url="url" :new-search="JSON.stringify(search)"><?= \Yii::t('mall/order_statistics', '数据概况');?></app-header>
        </div>
        <div class="table-body">
            <app-search
                ...
                <template slot="select">
                    <div>
                        <el-select size="small" popper-class="select-item" @change="getList" style="width: 160px" filterable v-model="search.sign" placeholder="<?= \Yii::t('mall/order_statistics', '请输入搜索内容');?>">
                            <el-option label="<?= \Yii::t('mall/order_statistics', '全部订单');?>" value="0"></el-option>
        ...
        <div class="table-area">
            <el-card shadow="never">
                <div slot="header">
                    <span><?= \Yii::t('mall/order_statistics', '总成交');?></span>
                </div>
                <div class="num-info">
                    <div class="num-info-item">
                        <div>{{all.order_num}}</div>
                        <div class="info-item-name"><?= \Yii::t('mall/order_statistics', '付款订单数');?></div>
                    </div>
                    <div class="num-info-item">
                        <div>{{all.goods_num}}</div>
                        <div class="info-item-name"><?= \Yii::t('mall/order_statistics', '付款件数');?></div>
                    </div>
                    <div class="num-info-item">
                        <div>{{all.user_num}}</div>
                        <div class="info-item-name"><?= \Yii::t('mall/order_statistics', '付款人数');?></div>
                    </div>
                    <div class="num-info-item">
                        <div>{{all.total_pay_price}}</div>
                        <div class="info-item-name"><?= \Yii::t('mall/order_statistics', '付款金额');?></div>
                    </div>
                </div>
            </el-card>
            <el-card shadow="never">
        ...
        <div class="table-body" style="padding: 20px">
            <el-tabs v-model="search.status" @tab-click="tab_order">
                <el-tab-pane label="<?= \Yii::t('mall/order_statistics', '全部');?>" name="-1"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('mall/order_statistics', '未完成订单');?>" name="0"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('mall/order_statistics', '已完成订单');?>" name="1"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('mall/order_statistics', '已取消订单');?>" name="2"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('mall/order_statistics', '售后中订单');?>" name="3"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('mall/order_statistics', '已完成售后订单');?>" name="4"></el-tab-pane>
            </el-tabs>
            <el-table v-loading="list_loading"
                      :header-cell-style="{background:'#F3F5F6','color':'#303133',padding: '6px 0',fontWeight: '400'}"
                      :data="list">
                <el-table-column prop="time" label="<?= \Yii::t('mall/order_statistics', '日期');?>">
                </el-table-column>
                <el-table-column prop="user_num" label="<?= \Yii::t('mall/order_statistics', '付款人数');?>">
                </el-table-column>
                <el-table-column prop="order_num" label="<?= \Yii::t('mall/order_statistics', '付款订单数');?>">
                </el-table-column>
                <el-table-column prop="total_pay_price" label="<?= \Yii::t('mall/order_statistics', '付款金额');?>">
                </el-table-column>
                <el-table-column prop="goods_num" label="<?= \Yii::t('mall/order_statistics', '付款件数');?>">
                </el-table-column>
            </el-table>
            <div style="margin-top: 10px;" flex="box:last cross:center">
                <div style="visibility: hidden">
                    <el-button plain type="primary" size="small"><?= \Yii::t('mall/order_statistics', '批量操作');?>1</el-button>
                    <el-button plain type="primary" size="small"><?= \Yii::t('mall/order_statistics', '批量操作');?>2</el-button>
                </div>
                <div>
                    <el-pagination
                            v-if="pagination"
