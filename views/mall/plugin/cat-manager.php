...
<div id="app" v-cloak>
            ...
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <span style="color: #409EFF;cursor: pointer"
                          @click="$navigate({r:'mall/plugin/index'})"><?= \Yii::t('mall/plugin', '全部应用');?></span></el-breadcrumb-item>
                <el-breadcrumb-item><?= \Yii::t('mall/plugin', '编辑应用');?></el-breadcrumb-item>
            </el-breadcrumb>
        </div>
    </el-card>

    <el-card shadow="never" style="margin-bottom: 20px;">
        <div slot="header"><?= \Yii::t('mall/plugin', '分类设置');?></div>
        <div flex="box:first" class="cat-list">
            <div class="label-name"><?= \Yii::t('mall/plugin', '分类设置');?></div>
            <div>
                <div flex="box:last cross:center" class="cat-item">
                    <div></div>
                    <el-button @click="handleAddCat" size="mini"><?= \Yii::t('mall/plugin', '添加');?></el-button>
                </div>
                        ...
                        <div>
                            <div v-if="cat.edit" style="padding: 4px 0;">
                                <el-input v-model="cat.edit.display_name"
                                          ...
                                          placeholder="<?= \Yii::t('mall/plugin', '请填写分类名称');?>"
                                          size="small"></el-input>
                            ...
                            <div flex="cross:center">
                                <div style="margin-right: 20px"><?= \Yii::t('mall/plugin', '图标背景色');?></div>
    ...
    <el-card shadow="never" style="padding-bottom: 100px">
        <div slot="header"><?= \Yii::t('mall/plugin', '插件分组设置');?></div>
        <template v-if="cats && cats.length">
            <div v-if="other_plugins && other_plugins.length" flex="box:first cross:center" class="cat-group">
                <div class="label-name"><?= \Yii::t('mall/plugin', '未分组');?></div>
    ...
    <div class="submit-bar">
        <el-button type="primary" @click="confirm" :loading="confirming"><?= \Yii::t('mall/plugin', '保存');?></el-button>
    </div>
</div>
    ...
    new Vue({
        ...
        methods: {
            ...
            handleDeleteCat(index) {
                this.$confirm('<?= \Yii::t('mall/plugin', '被删除分类下的原有插件');?>', '<?= \Yii::t('mall/plugin', '警告');?>', {
                    type: 'warning'
                }).then(e => {
            ...
            handleEditCatConfirm(index) {
                ...
                if (typeof displayName === 'undefined' || displayName === null || displayName === '' || displayName === false) {
                    this.$message.error('<?= \Yii::t('mall/plugin', '分类名称不能为空');?>');
                    return;
                }
                if (displayName.length > 6) {
                    this.$message.error('<?= \Yii::t('mall/plugin', '分类名称不能超过6个字');?>');
                    return;
                }
            ...
            confirm() {
                ...
                for (let i in this.cats) {
                    ...
                    if (cat.display_name === '') {
                        ...
                        this.$message.error('<?= \Yii::t('mall/plugin', '插件分类名称未填写');?>');
                        return;
                    }
                    let plugins = [];
                ...
                this.$request({
                    ...
                    if (e.data.code === 0) {
                        this.$message.success('<?= \Yii::t('mall/plugin', '保存成功');?>');
                        setTimeout(() => {
