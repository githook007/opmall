...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <app-header :url="url" :new-search="JSON.stringify(search)"><?= \Yii::t('mall/composition_statistics', '套餐组合');?></app-header>
        </div>
        ...
        <div class="table-body" style="padding: 20px">
            <el-table @sort-change="changeSort" v-loading="loading" :header-cell-style="{background:'#F3F5F6','color':'#303133',padding: '6px 0',fontWeight: '400'}" :data="list">
                <el-table-column width="600" prop="name" label="<?= \Yii::t('mall/composition_statistics', '商品名称');?>">
                ...
                <el-table-column prop="payment_people" label="<?= \Yii::t('mall/composition_statistics', '支付人数');?>" :label-class-name="prop == 'payment_people' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                <el-table-column prop="payment_num" label="<?= \Yii::t('mall/composition_statistics', '支付件数');?>" :label-class-name="prop == 'payment_num' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                <el-table-column prop="payment_amount" label="<?= \Yii::t('mall/composition_statistics', '支付金额');?>" :label-class-name="prop == 'payment_amount' ? 'sort-active': ''" sortable='custom'>
            </el-table>
...
<script>
    const app = new Vue({
        ...
        data() {
            return {
                url: 'mall/composition-statistics/index',
                loading: false,
                // 今天
                today: '',
                // 七天前
