...
<div id="app" v-cloak>
    <el-card shadow="never" body-style="background-color: #f3f3f3;padding: 10px 0 0;" v-loading="loading">
        <div slot="header">
            <div>
                <span><?= \Yii::t('mall/theme_color', '商城风格');?></span>
            </div>
        </div>
        <div style="background-color: #fff;padding: 20px 0;">
            <div flex="dir:left" class="style-tab">
                <div class="style-label"><?= \Yii::t('mall/theme_color', '商城风格');?></div>
                <div>
                    <el-radio @change="changeStyle" v-model="mallStyle" :label="1"><?= \Yii::t('mall/theme_color', '系统模版');?>
                    </el-radio>
                    <el-radio @change="changeStyle" v-model="mallStyle" :label="2"><?= \Yii::t('mall/theme_color', '自定义');?>
                    </el-radio>
                </div>
            </div>
            ...
            <div v-if="mallStyle == 2" class="style-tab">
                <div flex="dir:left cross:center">
                    <div flex="dir:left cross:center">
                        <div class="style-label"><?= \Yii::t('mall/theme_color', '主色');?></div>
                        <el-color-picker v-model="diy_color.main"></el-color-picker>
                    </div>
                    <div flex="dir:left cross:center">
                        <div class="style-label style-label-2"><?= \Yii::t('mall/theme_color', '主色按钮文字颜色');?></div>
                        <el-color-picker v-model="diy_color.main_text"></el-color-picker>
                    </div>
                </div>
                <div flex="dir:left cross:center" style="margin-top: 30px;">
                    <div flex="dir:left cross:center">
                        <div class="style-label"><?= \Yii::t('mall/theme_color', '辅色');?></div>
                        <el-color-picker v-model="diy_color.secondary"></el-color-picker>
                    </div>
                    <div flex="dir:left cross:center">
                        <div class="style-label style-label-2"><?= \Yii::t('mall/theme_color', '辅色按钮文字颜色');?></div>
            ...
            <div class="theme-show" flex="dir:left main:justify">
                ...
                <div v-if="mallStyle == 2" class="theme-item" v-for="(item,index) in pic_list" :style="{backgroundImage: `url(${item})`,backgroundColor: `${diy_color.main}`}">
                    <div v-if="index == 0" class="diy-show" flex="main:center cross:center">
                        <div class="diy-show-left" :style="{backgroundColor: `${diy_color.secondary}`,color: `${diy_color.secondary_text}`}"><?= \Yii::t('mall/theme_color', '加入购物车');?></div>
                        <div class="diy-show-right" :style="{backgroundColor: `${diy_color.main}`,color: `${diy_color.main_text}`}"><?= \Yii::t('mall/theme_color', '立即购买');?></div>
                    </div>
                </div>
            </div>
        </div>
        <el-button class="button-item" type="primary"  @click="onSubmit"><?= \Yii::t('mall/theme_color', '保存');?></el-button>
    </el-card>
</div>
