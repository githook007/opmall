    ...
    <el-card class="box-card" shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;"
        ...
        <div slot="header">
            ...
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item><?= \Yii::t('mall/live', '编辑直播商品');?></el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="form-body">
            <el-form :model="ruleForm" :rules="rules" size="small" ref="ruleForm" label-width="130px">
                <el-form-item label="<?= \Yii::t('mall/live', '添加直播商品');?>" prop="goods_list">
                    <div><el-button @click="openDiloag" size="small" plain><?= \Yii::t('mall/live', '选择商品');?></el-button></div>
                    <div v-if="!list.length" class="add-button" @click="$navigate({r: 'mall/live/goods-edit'})"><?= \Yii::t('mall/live', '直播商品还未添加商品');?></div>
                    <template v-if="ruleForm.goods_list.length">
                        <div flex="dir:left" class="table-box">
                            <el-table
                                ...
                                <el-table-column prop="name" label="<?= \Yii::t('mall/live', '名称');?>" >
                        ...
                        <div flex="dir:left box:last" style="margin-top: 10px;">
                            <div><el-button type="primary" size="small" @click="batchDelete()"><?= \Yii::t('mall/live', '批量删除');?></el-button></div>
                            <div v-if="multipleSelection2.length"><?= \Yii::t('mall/live', '已选');?> {{multipleSelection2.length}}</div>
                        </div>
                    </template>
                </el-form-item>
            </el-form>
        ...
        <el-dialog title="<?= \Yii::t('mall/live', '选择商品');?>" :visible.sync="dialogTableVisible">
            <el-table
                    v-loading="listLoading"
                    :data="list"
                    ref="multipleTable"
                ...
                <el-table-column property="name" label="<?= \Yii::t('mall/live', '名称');?>" >
                    <template slot-scope="scope">
                        <div flex="dir:left cross:center">
                            <img :src="scope.row.coverImgUrl" class="goods-images">
            ...
            <div slot="footer" class="dialog-footer">
                <div flex="dir:left box:last">
                    ...
                    <el-button type="primary" @click="selectGoods()" size="small"><?= \Yii::t('mall/live', '选择');?></el-button>
                </div>
            </div>
        </el-dialog>
    </el-card>
        ...
        methods: {
                ...
                this.$refs[formName].validate((valid) => {
                    ...
                    if (valid) {
                        if (!self.ruleForm.goods_list.length) {
                            self.$message.warning('<?= \Yii::t('mall/live', '请先选择商品');?>');
                            return false;
                        }
            ...
            batchDelete() {
                ...
                if (self.multipleSelection2.length == 0) {
                    self.$message.warning('<?= \Yii::t('mall/live', '请先选择要删除的商品');?>');
                    return false;
                }
