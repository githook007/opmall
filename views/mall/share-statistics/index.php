...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <app-header :url="url" :new-search="JSON.stringify(search)"><?= \Yii::t('mall/share_statistics', '分销排行');?></app-header>
        </div>
        <div style="margin-bottom: 10px;background: #ffffff">
            <app-search
                    ...
                    @search="searchList"
                    :new-search="search"
                    placeholder="<?= \Yii::t('mall/share_statistics', '请输入ID');?>"
        ...
        <div class="table-body">
            <el-table v-loading="loading" @sort-change="changeSort"
                ...
                <el-table-column prop="name" label="<?= \Yii::t('mall/share_statistics', '分销商名称');?>" width="180">
                ...
                <el-table-column prop="first_children" width="200" label="<?= \Yii::t('mall/share_statistics', '直接下级数量');?>" sortable='custom'
                                 :label-class-name="chooseProp == 'first_children' ? 'sort-active': ''">
                </el-table-column>
                <el-table-column prop="all_children" width="200" label="<?= \Yii::t('mall/share_statistics', '总下级数');?>" sortable='custom'
                                 :label-class-name="chooseProp == 'all_children' ? 'sort-active': ''">
                </el-table-column>
                <el-table-column prop="all_order" width="200" label="<?= \Yii::t('mall/share_statistics', '分销订单数量');?>" sortable='custom'
                                 :label-class-name="chooseProp == 'all_order' ? 'sort-active': ''">
                </el-table-column>
                <el-table-column prop="all_money" width="200" label="<?= \Yii::t('mall/share_statistics', '总佣金');?>" sortable='custom'
                                 :label-class-name="chooseProp == 'all_money' ? 'sort-active': ''">
                    <template slot="header" slot-scope="scope">
                        <span><?= \Yii::t('mall/share_statistics', '总佣金');?></span>
                        <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/share_statistics', '包括已完成订单和未完成订单');?>" placement="bottom">
                            <i style="color: #92959B" class="el-icon-question"></i>
                        </el-tooltip>
                    </template>
                </el-table-column>
                <el-table-column prop="total_money" width="200" label="<?= \Yii::t('mall/share_statistics', '累计佣金');?>" sortable='custom'
                                 :label-class-name="chooseProp == 'total_money' ? 'sort-active': ''">
                    <template slot="header" slot-scope="scope">
                        <span><?= \Yii::t('mall/share_statistics', '累计佣金');?></span>
                        <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/share_statistics', '包括已提现佣金和未提现佣金');?>" placement="bottom">
                            <i style="color: #92959B" class="el-icon-question"></i>
                        </el-tooltip>
                    </template>
                </el-table-column>
                <el-table-column prop="price" width="200" label="<?= \Yii::t('mall/share_statistics', '已提现佣金');?>" sortable='custom'
                                 :label-class-name="chooseProp == 'price' ? 'sort-active': ''">
                </el-table-column>
            </el-table>
            <div style="margin-top: 10px;" flex="box:last cross:center">
                <div style="visibility: hidden">
                    <el-button plain type="primary" size="small"><?= \Yii::t('mall/share_statistics', '批量操作');?>1</el-button>
                    <el-button plain type="primary" size="small"><?= \Yii::t('mall/share_statistics', '批量操作');?>2</el-button>
                </div>
                <div>
                    <el-pagination
                            v-if="pagination"
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                pagination: [],
                page: 1,
                url: 'mall/share-statistics/index',
                search: {
                    platform: '',
                    keyword: '',
                    order: null,
