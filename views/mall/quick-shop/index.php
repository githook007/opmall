...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <div>
                <span><?= \Yii::t('mall/qyick_shop', '快速购买商品分类');?></span>
                <div style="float: right;margin-top: -5px">
                    <el-button type="primary" @click="editAdd" size="small"><?= \Yii::t('mall/qyick_shop', '添加分类');?></el-button>
                </div>
            </div>
        </div>
        <div class="table-body">
            <div style="display: flex">
                <div class="input-item">
                    <el-input @keyup.enter.native="search" size="small" placeholder="<?= \Yii::t('mall/qyick_shop', '请输入搜索内容');?>" v-model="keyword" clearable @clear='search'>
                        <el-button slot="append" icon="el-icon-search" @click="search"></el-button>
                    </el-input>
                </div>
                <el-alert type="info" style="height: 32px;padding: 0 16px;margin-left: 10px;width: auto" :closable="false">
                    <template>
                        <?= \Yii::t('mall/qyick_shop', '温馨提示');?><el-button type="text" @click="$navigate({r:'mall/cat/index'}, true)"><?= \Yii::t('mall/qyick_shop', '商品管理');?></el-button><?= \Yii::t('mall/qyick_shop', '修改');?>
                    </template>
                </el-alert>
            </div>
            <el-table v-loading="listLoading" :data="form" border height="620" style="width: 100%">
                <el-table-column prop="id" label="ID" width="80"></el-table-column>
                <el-table-column label="<?= \Yii::t('mall/qyick_shop', '分类名称');?>">
                    <template slot-scope="scope">{{scope.row.cats.name}}</template>
                </el-table-column>
                <el-table-column label="<?= \Yii::t('mall/qyick_shop', '图标');?>">
                    <template slot-scope="scope">
                        <app-image mode="aspectFill" :src="scope.row.cats.pic_url"></app-image>
                    </template>
                </el-table-column>
                <el-table-column label="<?= \Yii::t('mall/qyick_shop', '排序');?>">
                    <template slot-scope="scope">
                        <div v-if="id != scope.row.id">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/qyick_shop', '排序');?>" placement="top">
                ...
                <el-table-column prop="created_at" label="<?= \Yii::t('mall/qyick_shop', '添加日期');?>" width="220"></el-table-column>
                <el-table-column label="<?= \Yii::t('mall/qyick_shop', '操作');?>" width="150" fixed="right">
                    <template slot-scope="scope">
                        <el-button @click="destroy(scope.row)" type="text" circle size="mini">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/qyick_shop', '删除');?>" placement="top">
    ...
    <!-- 更新分类 -->
    <el-dialog title="<?= \Yii::t('mall/qyick_shop', '添加分类');?>" :visible.sync="dialogSortAdd" width="50%">
        <el-form :model="sortAddForm" label-width="60px" :rules="sortAddFormRules" ref="sortAddForm">
            <el-form-item label="<?= \Yii::t('mall/qyick_shop', '分类');?>" prop="cat_list" size="small">
            ...
            <el-form-item label="<?= \Yii::t('mall/qyick_shop', '排序');?>" prop="sort" size="small">
                <el-input style="width: 50%" v-model="sortAddForm.sort"></el-input>
            </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
            <el-button size="small" @click="dialogSortAdd = false"><?= \Yii::t('mall/qyick_shop', '取消');?></el-button>
            <el-button size="small" :loading="btnLoading" type="primary" @click="sortAddSubmit"><?= \Yii::t('mall/qyick_shop', '提交');?></el-button>
        </div>
    </el-dialog>
</div>
<script>
    const app = new Vue({
        ...
        data() {
            return {
                ...
                sortAddFormRules: {
                    cat_list: [
                        { required: true, message: '<?= \Yii::t('mall/qyick_shop', '分类ID不能为空');?>', trigger: 'blur' },
                    ],
                },
                defaultProps: {
                    children: 'child',
        ...
        methods: {
            ...
            destroy: function(column) {
                this.$confirm('<?= \Yii::t('mall/qyick_shop', '确认删除该记录吗');?>?', '<?= \Yii::t('mall/qyick_shop', '提示');?>', {
                    type: 'warning'
                }).then(() => {
