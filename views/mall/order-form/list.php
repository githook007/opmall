...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <span><?= \Yii::t('mall/order_form', '表单列表');?></span>
            <el-button style="float: right; margin: -5px 0" type="primary" size="small"
                       @click="$navigate({r:'mall/order-form/setting'})"><?= \Yii::t('mall/order_form', '添加表单');?>
            </el-button>
        </div>
        <div class="table-body">
            <div class="input-item">
                <el-input @keyup.enter.native="search" size="small" placeholder="<?= \Yii::t('mall/order_form', '请输入表单名称搜索');?>" v-model="keyword"
                          clearable @clear="search">
                    <el-button slot="append" icon="el-icon-search" @click="search"></el-button>
                </el-input>
            </div>
            <el-table v-loading="loading" border :data="list" style="width: 100%;margin-bottom: 15px">
                <el-table-column prop="name" label="<?= \Yii::t('mall/order_form', '表单名称');?>"></el-table-column>
                <el-table-column width='200' prop="sub_price" label="<?= \Yii::t('mall/order_form', '是否默认');?>">
                    <template slot-scope="scope">
                        <el-radio v-model="scope.row.is_default"
                                  :label="1" @change="changeDefault(scope.row,scope.$index)"><?= \Yii::t('mall/order_form', '默认');?></el-radio>
                    </template>
                </el-table-column>
                <el-table-column width='200' prop="sub_price" label="<?= \Yii::t('mall/order_form', '状态');?>">
                    <template slot-scope="scope">
                        <el-switch v-model="scope.row.status" :inactive-value="0" :active-value="1"
                                   @change="changeStatus(scope.row,scope.$index)"></el-switch>
                    </template>
                </el-table-column>
                <el-table-column label="<?= \Yii::t('mall/order_form', '操作');?>" width="220">
                    <template slot-scope="scope">
                        <el-button circle size="mini" type="text"
                                   @click="$navigate({r:'mall/order-form/setting',id:scope.row.id})">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/order_form', '编辑');?>" placement="top">
                                <img src="statics/img/mall/edit.png" alt="">
                            </el-tooltip>
                        </el-button>
                        <el-button :loading="scope.row.loading" circle size="mini" type="text"
                                   @click="deleteItem(scope.row,scope.$index)">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/order_form', '删除');?>" placement="top">
    ...
    const app = new Vue({
        ...
        methods: {
            ...
            deleteItem(item, index) {
                this.$confirm('<?= \Yii::t('mall/order_form', '确认删除');?>', '<?= \Yii::t('mall/order_form', '提示');?>', {
                    type: 'warning',
                    center: true
                }).then(() => {
                    item.is_delete = 1;
