...
<div id="app" v-cloak>
    ...
    <el-dialog :visible.sync="goodsVisible" width="60%" class="activity-visible">
        <div slot="title">
            <span class="goods-visible-title" style="font-size: 23px;"><?= \Yii::t('mall/full_reduce', '选择商品');?></span>
        </div>
        <div style="margin-bottom: 27px;">
            <el-input v-model="goodsSearch.keyword"  @change="selectGood" @clear="selectGood" placeholder="<?= \Yii::t('mall/full_reduce', '根据名称搜索');?>" clearable autocomplete="off">
                <template slot="append">
                    <el-button @click="selectGood"><?= \Yii::t('mall/full_reduce', '搜索');?></el-button>
                </template>
            </el-input>
        </div>
        <el-table
            ...
            <el-table-column
                    prop="name"
                    label="<?= \Yii::t('mall/full_reduce', '名称');?>">
        ...
        <div slot="footer" class="dialog-footer" flex="main:justify">
            ...
            <el-button type="primary" size="small" @click="addGoods()"><?= \Yii::t('mall/full_reduce', '确定');?></el-button>
        </div>
    </el-dialog>
        ...
        <div slot="header" class="header">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><span style="color: #409EFF;cursor: pointer"
                                          @click="$navigate({r:'mall/full-reduce/index'})"><?= \Yii::t('mall/full_reduce', '满减列表');?></span></el-breadcrumb-item>
                <el-breadcrumb-item>{{form.id ? '<?= \Yii::t('mall/full_reduce', '编辑满减');?>' : '<?= \Yii::t('mall/full_reduce', '添加满减');?>'}}</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <el-form size="small"  label-width="100px" :model="form" :rules="rules" >
            <div class="table-body">
                <div class="table-item-header">
                    <?= \Yii::t('mall/full_reduce', '活动设置');?>
                </div>
                <div class="table-item" >
                    <el-form-item prop="name" class="name">
                        <span slot="label" style="color:#606266"><?= \Yii::t('mall/full_reduce', '活动名称');?></span>
                        <el-input v-model="form.name" :disabled="editBool">
                        </el-input>
                    </el-form-item>
                    <el-form-item prop="start_at" class="name">
                        <span slot="label" style="color:#606266"><?= \Yii::t('mall/full_reduce', '开始时间');?></span>
                        <el-date-picker
                                v-model="form.start_at"
                                type="datetime"
                                :disabled="editBool"
                                value-format="yyyy-MM-dd HH:mm:ss"
                                placeholder="<?= \Yii::t('mall/full_reduce', '选择日期时间');?>">
                        </el-date-picker>
                    </el-form-item>
                    <el-form-item prop="end_at" class="name">
                        <span slot="label" style="color:#606266"><?= \Yii::t('mall/full_reduce', '结束时间');?></span>
                        <el-date-picker
                                v-model="form.end_at"
                                type="datetime"
                                :disabled="editBool"
            ...
            <div class="table-body">
                <div class="table-item-header">
                    <?= \Yii::t('mall/full_reduce', '商品设置');?>
                </div>
                <div class="table-item">
                    <el-form-item prop="name" class="goods">
                        <span slot="label" style="color:#606266"><?= \Yii::t('mall/full_reduce', '满减商品');?></span>
                        <div class="item">
                            <el-radio :disabled="editBool" v-model="form.appoint_type" :label="1"><?= \Yii::t('mall/full_reduce', '全部商品');?></el-radio>
                        </div>
                        <div class="item">
                            <el-radio :disabled="editBool" v-model="form.appoint_type" :label="2"><?= \Yii::t('mall/full_reduce', '全部自营商品');?></el-radio>
                        </div>
                        <div class="item">
                            <el-radio :disabled="editBool" v-model="form.appoint_type" :label="3"><?= \Yii::t('mall/full_reduce', '指定商品参与');?>
                                <el-button :disabled="form.appoint_type !== 3" @click="chooseGoods"><?= \Yii::t('mall/full_reduce', '选择商品');?></el-button>
                            </el-radio>
                            <div class="goods-table" v-if="form.appoint_type === 3">
                                ...
                                >
                                    ...
                                    <el-table-column
                                            prop="date"
                                            label="<?= \Yii::t('mall/full_reduce', '商品');?>"
                                            width="500"
                                    >
                                        <template slot-scope="scope">
                                            <div flex="">
                                                ...
                                                <div class="content">
                                                    ...
                                                    <div>
                                                        ...
                                                        <span style="margin-left: 80px;"><?= \Yii::t('mall/full_reduce', '库存');?>{{scope.row.goods_stock}}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </template>
                                    </el-table-column>
                                    <el-table-column prop="shop_name" label="<?= \Yii::t('mall/full_reduce', '店铺名称');?>" width="180"></el-table-column>
                                    <el-table-column
                                            prop="name"
                                            label="<?= \Yii::t('mall/full_reduce', '操作');?>"
                                    >
                                        <template slot-scope="scope">
                                            <el-button class="set-el-button" size="mini" type="text" circle
                                                       @click="handleDel(scope.row)">
                                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/full_reduce', '删除');?>" placement="top">
                            ...
                            <div v-if="form.appoint_type === 3" style="width: 830px;margin-top: 20px;" flex="main:justify">
                                <el-button type="primary" size="small" @click="deleteNoappoint"><?= \Yii::t('mall/full_reduce', '批量删除');?></el-button>
                                <div flex="">
                                    <span style="color: #616161;margin-right: 20px;"><?= \Yii::t('mall/full_reduce', '已选');?>{{form.appoint_goods.length}}</span>
                                    <el-pagination
                        ...
                        <div class="item">
                            <el-radio :disabled="editBool" v-model="form.appoint_type" :label="4"><?= \Yii::t('mall/full_reduce', '指定商品不参与');?>
                                <el-button :disabled="form.appoint_type !== 4" @click="chooseGoods"><?= \Yii::t('mall/full_reduce', '选择商品');?></el-button>
                            </el-radio>
                            ...
                            <div v-if="form.appoint_type === 4" style="width: 830px;margin-top: 20px;" flex="main:justify">
                                <el-button type="primary" size="small" @click="deleteNoappoint"><?= \Yii::t('mall/full_reduce', '批量删除');?></el-button>
                                <div flex="">
                                    <span style="color: #616161;margin-right: 20px;"><?= \Yii::t('mall/full_reduce', '已选');?>{{form.noappoint_goods.length}}</span>
                                    <el-pagination
            ...
            <div class="table-body">
                <div class="table-item-header">
                    <?= \Yii::t('mall/full_reduce', '满减规则');?>
                </div>
                <div class="table-item">
                    <el-form-item prop="name" class="name">
                        <span slot="label" style="color:#606266"><?= \Yii::t('mall/full_reduce', '规则设置');?></span>
                        <span  style="font-size: 13px;color: #999999;"><?= \Yii::t('mall/full_reduce', '阶梯优惠中满足二级优惠');?></span>
                    </el-form-item>
                    <div >
                        <el-form-item>
                            <span slot="label" style="color:#606266"><?= \Yii::t('mall/full_reduce', '满减方式');?></span>
                            <el-radio :disabled="editBool" v-model="form.rule_type" :label="1"><?= \Yii::t('mall/full_reduce', '阶梯满减');?></el-radio>
                            <el-radio :disabled="editBool" v-model="form.rule_type" :label="2"><?= \Yii::t('mall/full_reduce', '循环满减');?></el-radio>
                            <div v-if="form.rule_type === 1">
                                <el-radio
                                        ...
                                        label="2"
                                        :disabled="editBool"
                                ><?= \Yii::t('mall/full_reduce', '打折');?></el-radio>
                                <el-radio
                                        style="margin: 20px 0 20px 20px;"
                                        v-model="discount_type"
                                        label="1"
                                        :disabled="editBool"
                                ><?= \Yii::t('mall/full_reduce', '减钱');?></el-radio>
                            </div>
                            <div class="flight-item" v-if="form.rule_type === 1" v-for="(item, index) in form.discount_rule">
                                <div class="item-header">
                                    {{index === 0 ? '<?= \Yii::t('mall/full_reduce', '一');?>' : index === 1 ? '<?= \Yii::t('mall/full_reduce', '二');?>' : index === 2 ? '<?= \Yii::t('mall/full_reduce', '三');?>' : index === 3 ? '<?= \Yii::t('mall/full_reduce', '四');?>' : '<?= \Yii::t('mall/full_reduce', '五');?>'}}<?= \Yii::t('mall/full_reduce', '级优惠');?>
                                </div>
                                <div class="item-body">
                                    <el-form-item label="<?= \Yii::t('mall/full_reduce', '优惠门槛');?>" required>
                                        <div flex="main:left" style="width: 100%">
                                            <span style="margin-right: 10px;" :style="{color: editBool ? '#C0C4CC' : 'rgb(96, 98, 102)'}"><?= \Yii::t('mall/full_reduce', '满');?></span>
                                            <div style="width: 100%" >
                                                <el-input :disabled="editBool" v-model="item.min_money" type="number" oninput="this.value = this.value.replace(/^(\-)*(\d+)\.(\d\d).*$/,'$1$2.$3');">
                                                </el-input>
                                            </div>
                                            <span style="margin-left: 10px;" :style="{color: editBool ? '#C0C4CC' : 'rgb(96, 98, 102)'}"><?= \Yii::t('mall/full_reduce', '元');?></span>
                                        </div>
                                    </el-form-item>
                                    <el-form-item label="<?= \Yii::t('mall/full_reduce', '优惠内容');?>" required>
                                        <div class="less">
                                            <div style="width: 100%" >
                                                <template v-if="discount_type == 1">
                                                    <span style="margin-right: 10px;"><?= \Yii::t('mall/full_reduce', '减');?></span>
                                                    <el-input  :disabled="editBool " type="number" oninput="this.value = this.value.replace(/^(\-)*(\d+)\.(\d\d).*$/,'$1$2.$3');" v-model="item.cut">
                                                    </el-input>
                                                    <span style="margin-left: 10px;"><?= \Yii::t('mall/full_reduce', '元');?></span>
                                                </template>

                                                <template v-if="discount_type == 2">
                                                    <span style="margin-right: 10px;"><?= \Yii::t('mall/full_reduce', '打');?></span>
                                                    <el-input :disabled="editBool" type="number" oninput="this.value = this.value.replace(/^(\-)*(\d+)\.(\d).*$/,'$1$2.$3');" v-model="item.discount">
                                                    </el-input>
                                                    <span style="margin-left: 10px;"><?= \Yii::t('mall/full_reduce', '折');?></span>
                                                </template>
                                            </div>
                                        </div>
                                    </el-form-item>
                            ...
                            <div style="padding-left: 20px" v-if="form.rule_type === 1">
                                <div style="font-size: 13px;color: #999999;"><?= \Yii::t('mall/full_reduce', '最多添加5级阶梯');?></div>
                                <el-button class="rule-btn" :disabled="editBool" @click="addDiscount" v-if="form.discount_rule.length < 5">
                                    +<?= \Yii::t('mall/full_reduce', '添加');?>{{form.discount_rule.length === 0 ? '<?= \Yii::t('mall/full_reduce', '一');?>' : form.discount_rule.length === 1 ? '<?= \Yii::t('mall/full_reduce', '二');?>' :
                                    form.discount_rule.length === 2 ? '<?= \Yii::t('mall/full_reduce', '三');?>' : form.discount_rule.length === 3 ? '<?= \Yii::t('mall/full_reduce', '四');?>':'<?= \Yii::t('mall/full_reduce', '五');?>'
                                    }}<?= \Yii::t('mall/full_reduce', '级优惠');?>
                                </el-button>
                            </div>
                        </el-form-item>
                    </div>
                    <div style="width: 35%;margin-left: 70px;">
                        <div style="margin-top: 10px;" v-if="form.rule_type === 2">
                            <el-form-item label="<?= \Yii::t('mall/full_reduce', '优惠门槛');?>" required>
                                <div flex="main:left" style="width: 100%">
                                    <span style="margin-right: 10px;width: 40px;"><?= \Yii::t('mall/full_reduce', '每满');?></span>
                                    <div style="width: 100%" >
                                        <el-input
                                                :disabled="editBool"
                                                type="number"
                            ...
                            <el-form-item label="<?= \Yii::t('mall/full_reduce', '优惠内容');?>" required>
                                <div flex="main:left" style="width: 100%">
                                    <span style="margin-right: 10px;width: 40px;"><?= \Yii::t('mall/full_reduce', '减');?></span>
                                    <div style="width: 100%" >
                                        <el-input
                                                :disabled="editBool"
                                                type="number"
                    ...
                    <el-form-item label="<?= \Yii::t('mall/full_reduce', '活动规则');?>">
    ...
    <el-button  type="primary" style="margin-bottom: 65px;" @click="submit"><?= \Yii::t('mall/full_reduce', '保存');?></el-button>
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                rules: {
                    name: [
                        { required: true, message: '<?= \Yii::t('mall/full_reduce', '请输入活动名称');?>', trigger: 'blur' },
                        { min: 1, max: 15, message: '<?= \Yii::t('mall/full_reduce', '长度在');?>', trigger: 'blur' }
                    ],
                    start_at: [
                        { required: true, message: '<?= \Yii::t('mall/full_reduce', '请输入开始时间');?>', trigger: 'change'},
                    ],
                    end_at: [
                        { required: true, message: '<?= \Yii::t('mall/full_reduce', '请输入结束时间');?>', trigger: 'change'},
                    ]
                },
                editBool: false,
                discount_type: '1'
