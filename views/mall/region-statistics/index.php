...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <app-header :url="url" :new-search="JSON.stringify(search)"><?= \Yii::t('mall/region_statistics', '区域代理');?></app-header>
        </div>
            ...
            <app-search
                ...
                <template slot="other">
                    <div style="margin-left: 20px;" flex="dir:left cross:center">
                        <div style="margin-right: 10px;"><?= \Yii::t('mall/region_statistics', '选择省份');?></div>
        ...
        <div class="table-body" style="padding: 20px">
            <el-table v-loading="loading" :header-cell-style="{background:'#F3F5F6','color':'#303133',padding: '6px 0',fontWeight: '400'}" :data="list">
                <el-table-column width="600" prop="level" label="<?= \Yii::t('mall/region_statistics', '代理级别');?>">
                </el-table-column>
                <el-table-column prop="rate" label="<?= \Yii::t('mall/region_statistics', '分红比例');?>">
                </el-table-column>
                <el-table-column prop="price" label="<?= \Yii::t('mall/region_statistics', '分红总金额');?>">
                </el-table-column>
                <el-table-column prop="num" label="<?= \Yii::t('mall/region_statistics', '分红订单总数');?>">
                </el-table-column>
            </el-table>
        </div>
    </el-card>
...
<script>
    const app = new Vue({
        ...
        data() {
            return {
                url: 'mall/region-statistics/index',
                loading: false,
                // 今天
                today: '',
                // 七天前
                ...
                request({
                    ...
                    if (e.data.code === 0) {
                        if(e.data.data.model != null) {
                            this.list = [
                                {level:'<?= \Yii::t('mall/region_statistics', '省代理');?>',rate: '', num: '', price:''},
                                {level:'<?= \Yii::t('mall/region_statistics', '市代理');?>',rate: '', num: '', price:''},
                                {level:'<?= \Yii::t('mall/region_statistics', '区');?>',rate: '', num: '', price:''}
                            ]
