...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" v-loading="loading"
        ...
        <div slot="header">
            <span><?= \Yii::t('mall/offer_price', '起送规则');?></span>
        </div>
        <el-form ref="ruleForm" label-width="150px" :model="ruleForm" :rules="rules"
                 class="demo-ruleForm" position-label="right">
            <div class="form-body">
                <el-form-item label="<?= \Yii::t('mall/offer_price', '是否开启');?>" prop="is_enable" required>
                    <el-switch
                ...
                <template v-if="ruleForm.is_enable == 1">
                    <el-form-item label="<?= \Yii::t('mall/offer_price', '全地区起送金额设置');?>" prop="is_total_price" required>
                        <el-switch
                    ...
                    <el-form-item v-if="ruleForm.is_total_price" label="<?= \Yii::t('mall/offer_price', '全地区起送金额');?>" prop="total_price" required>
                        <el-input size="small" v-model.number="ruleForm.total_price" type="number">
                            <template slot="append"><?= \Yii::t('mall/offer_price', '元');?></template>
                        </el-input>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('mall/offer_price', '起送规则');?>" prop="detail">
                        <el-card shadow="never" style="margin-bottom: 12px;" v-if="ruleForm.detail"
                            ...
                            <div flex="dir:left box:last">
                                <div><?= \Yii::t('mall/offer_price', '起送金额');?>{{item.total_price}}<?= \Yii::t('mall/offer_price', '元');?>
                                </div>
                                <div style="text-align: right">
                                    <el-button size="small" type="text" circle @click="openDistrict(index)">
                                        <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/offer_price', '编辑');?>" placement="top">
                                            <img src="statics/img/mall/edit.png" alt="">
                                        </el-tooltip>
                                    </el-button>
                                    <el-button style="margin-left: -5px" size="small" type="text" circle
                                               @click="deleteDistrict(index)">
                                        <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/offer_price', '删除');?>" placement="top">
                            ...
                            <div flex="dir:left" style="flex-wrap: wrap">
                                <div><?= \Yii::t('mall/offer_price', '区域');?></div>
                        ...
                        <el-button size="small" type="text" @click="openDistrict()"><i class="el-icon-plus"><?= \Yii::t('mall/offer_price', '新增规则');?></i>
                        </el-button>
                    </el-form-item>
                </template>
            </div>
            <el-form-item class="form-button">
                <el-button class="button-item" :loading="submitLoading" type="primary" @click="onSubmit('ruleForm')">
                    <?= \Yii::t('mall/offer_price', '保存');?>
                </el-button>
            </el-form-item>
        </el-form>
        <el-dialog title="<?= \Yii::t('mall/offer_price', '新增规则');?>" :visible.sync="dialogVisible" width="50%">
            <el-form ref="detail" @submit.native.prevent :model="detail" label-width="150px" :rules="dialogRules">
                <el-form-item label="<?= \Yii::t('mall/offer_price', '起送金额');?>" prop="total_price">
                    <el-input v-model="detail.total_price" type="number">
                        <template slot="append"><?= \Yii::t('mall/offer_price', '元');?></template>
                    </el-input>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('mall/offer_price', '地区选择');?>" prop="list">
    ...
    const app = new Vue({
        ...
        data() {
            let min_number = (rule, value, callback) => {
                if (value < 0) {
                    callback(new Error('<?= \Yii::t('mall/offer_price', '输入值必须大于等于0');?>'));
                } else {
                    callback();
                }
            };
            return {
                ...
                rules: {
                    is_enable: [
                        {type: 'required', message: '<?= \Yii::t('mall/offer_price', '请选择是否开启');?>'}
                    ],
                    total_price: [
                        {validator: min_number, trigger: 'change'},
                        {required: true, type: 'number', trigger: 'change', message: '<?= \Yii::t('mall/offer_price', '请输入起送金额');?>'},
                    ],
                },
                dialogRules: {
                    total_price: [
                        {required: true, message: '<?= \Yii::t('mall/offer_price', '请输入起送金额');?>', trigger: 'change'},
                        {validator: min_number, trigger: 'change'},
                    ],
                    list: [
                        {required: true, message: '<?= \Yii::t('mall/offer_price', '请选择起送规则');?>'}
                    ]
                },
                detail: {
                    total_price: 0,
        ...
        methods: {
            load() {
                ...
                request({
                    ...
                    this.loading = false;
                    this.$message.error('<?= \Yii::t('mall/offer_price', '获取数据出错');?>');
                });
            },
            openDistrict(index) {
