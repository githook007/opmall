...
<div id="app" v-cloak>
    <el-card v-loading="loading" shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <span><?= \Yii::t('mall/statistic', '核销员业绩');?></span>
        </div>
        <div class="table-body">
            <el-form class="search" size="small" :inline="true" style="padding: 20px" :model="search">
                <!-- 搜索框 -->
                <el-form-item>
                    <el-select @change="tabMch" style="width: 160px" filterable v-model="search.mch" placeholder="<?= \Yii::t('mall/statistic', '请输入搜索内容');?>">
                        <el-option label="<?= \Yii::t('mall/statistic', '全部');?>" value="0"></el-option>
                ...
                <el-form-item>
                    <el-date-picker
                            ...
                            range-separator="<?= \Yii::t('mall/statistic', '至');?>"
                            start-placeholder="<?= \Yii::t('mall/statistic', '开始日期');?>"
                            end-placeholder="<?= \Yii::t('mall/statistic', '结束日期');?>">
                    </el-date-picker>
                </el-form-item>
                <el-form-item>
                    <el-tabs v-model="activeName" @tab-click="tab_total">
                        <el-tab-pane label="7<?= \Yii::t('mall/statistic', '日');?>" name="week"></el-tab-pane>
                        <el-tab-pane label="30<?= \Yii::t('mall/statistic', '日');?>" name="month"></el-tab-pane>
                    </el-tabs>
                </el-form-item>
                <el-form-item>
                    <div class="clean" @click="clean"><?= \Yii::t('mall/statistic', '清空筛选');?></div>
                </el-form-item>
                <el-form-item style="float: right">
                    <el-button @click="exportForm" type="primary"><?= \Yii::t('mall/statistic', '导出报表');?></el-button>
                </el-form-item>
            </el-form>
        </div>
        <div class="table-area">
            <el-card shadow="never">
                <div slot="header">
                    <span><?= \Yii::t('mall/statistic', '总业绩');?></span>
                </div>
                <div class="num-info">
                    <div class="num-info-item">
                        <div>{{all.goods_num}}</div>
                        <div class="info-item-name"><?= \Yii::t('mall/statistic', '核销订单数');?></div>
                    </div>
                    <div class="num-info-item">
                        <div>{{all.user_num}}</div>
                        <div class="info-item-name"><?= \Yii::t('mall/statistic', '核销件数');?></div>
                    </div>
                    <div class="num-info-item">
                        <div>{{all.total_pay_price}}</div>
                        <div class="info-item-name"><?= \Yii::t('mall/statistic', '核销金额');?></div>
                    </div>
                </div>
            </el-card>
            <el-card shadow="never">
                <div slot="header">
                    <span><?= \Yii::t('mall/statistic', '今日实时业绩');?></span>
                </div>
                <div class="num-info">
                    <div class="num-info-item">
        ...
        <div class="table-body" style="padding: 20px">
            <el-tabs v-model="orderSataus" @tab-click="tab_order">
                <el-tab-pane label="<?= \Yii::t('mall/statistic', '全部');?>" name="-1"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('mall/statistic', '未完成订单');?>" name="0"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('mall/statistic', '已完成订单');?>" name="1"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('mall/statistic', '已取消订单');?>" name="2"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('mall/statistic', '售后中订单');?>" name="3"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('mall/statistic', '已完成售后订单');?>" name="4"></el-tab-pane>
            </el-tabs>
            <el-table v-loading="list_loading" :header-cell-style="{background:'#F3F5F6','color':'#303133',padding: '2px 0',fontWeight: '400'}" :data="list">
                ...
                <el-table-column prop="user_num" label="<?= \Yii::t('mall/statistic', '核销人昵称');?>">
                ...
                <el-table-column prop="order_num" label="<?= \Yii::t('mall/statistic', '联系方式');?>">
                </el-table-column>
                <el-table-column prop="total_pay_price" label="<?= \Yii::t('mall/statistic', '核销订单数量');?>" sortable>
                </el-table-column>
                <el-table-column prop="goods_num" label="<?= \Yii::t('mall/statistic', '核销卡券数量');?>" sortable>
                </el-table-column>
                <el-table-column prop="goods_num" label="<?= \Yii::t('mall/statistic', '累计金额');?>" sortable>
                </el-table-column>
            </el-table>
            <div style="margin-top: 10px;" flex="box:last cross:center">
                <div style="visibility: hidden">
                    <el-button plain type="primary" size="small"><?= \Yii::t('mall/statistic', '批量操作');?>1</el-button>
                    <el-button plain type="primary" size="small"><?= \Yii::t('mall/statistic', '批量操作');?>2</el-button>
                </div>
                <div>
                    <el-pagination
                            v-if="pagination"
