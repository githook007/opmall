...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;"
        ...
        <div slot="header">
            <div>
                <span><?= \Yii::t('mall/wechat', '菜单设置');?></span>
            </div>
        </div>
                ...
                <div class="screen">
                    ...
                    <div class="foot" flex="dir:left cross:center">
                        ...
                        <div class="menus-list" flex="dir:left">
                            <el-button v-if="list.length == 0" @click="addMenu" class="menu-item" type="text" icon="el-icon-plus"><?= \Yii::t('mall/wechat', '添加菜单');?></el-button>
                            <div @click="showSubMenu(item,index)" class="menu-item toggle-menu" :class="index == activeIndex ? 'active' : ''" v-for="(item,index) in list" :key="index">
                                {{item.name ? item.name : '<?= \Yii::t('mall/wechat', '菜单名称');?>'}}
                                ...
                                <div v-if="item.sub_button && index == activeIndex" class="sub-menu">
                                    <div @click.stop="showSubMenuItem(sub,idx)" v-for="(sub,idx) in item.sub_button" class="sub-menu-item" :key="idx"  :class="idx == activeSubIndex ? 'active' : ''">
                                        {{sub.name ? sub.name : '<?= \Yii::t('mall/wechat', '子菜单名称');?>'}}
            ...
            <div style="width: 100%;">
                <el-form @submit.native.prevent class="form-body" :model="form" :rules="rules" size="small" ref="form" label-width="120px">
                    <div class="tips" flex="dir:left cross:center">
                        ...
                        <div>
                            <div><?= \Yii::t('mall/wechat', 'a38');?></div>
                            <div><?= \Yii::t('mall/wechat', 'a39');?></div>
                        </div>
                    </div>
                    <div v-if="form">
                        <div v-if="form.sub_button && form.sub_button.length > 0" style="padding: 12px 0 24px 40px">
                            <?= \Yii::t('mall/wechat', '已为');?>“{{form.name ? form.name : '<?= \Yii::t('mall/wechat', '菜单名称');?>'}}”<?= \Yii::t('mall/wechat', 'a42');?>
                        </div>
                        <el-form-item label="<?= \Yii::t('mall/wechat', '菜单名称');?>" prop="name">
                            <el-input @input="checkName" v-model.trim="form.name" placeholder="<?= \Yii::t('mall/wechat', 'a43');?>"></el-input>
                        </el-form-item>
                        <div v-if="!form.sub_button || form.sub_button.length == 0">
                            <el-form-item required label="<?= \Yii::t('mall/wechat', '菜单消息');?>" prop="type">
                                <el-radio-group v-model="form.type">
                                    <el-radio label="click"><?= \Yii::t('mall/wechat', '发送消息');?></el-radio>
                                    <el-radio label="view"><?= \Yii::t('mall/wechat', '跳转网页');?></el-radio>
                                    <el-radio label="miniprogram"><?= \Yii::t('mall/wechat', '跳转小程序');?></el-radio>
                                </el-radio-group>
                            </el-form-item>
                            <el-form-item required label="<?= \Yii::t('mall/wechat', '消息内容');?>" v-if="form.type == 'click'" prop="reply_type">
                                <el-radio-group v-model="form.reply_type">
                                    <el-radio :label="0"><?= \Yii::t('mall/wechat', '文字');?></el-radio>
                                    <el-radio :label="1"><?= \Yii::t('mall/wechat', '图片');?></el-radio>
                                </el-radio-group>
                            </el-form-item>
                            <el-form-item class="content-input" v-if="form.type == 'click'" prop="content">
                                ...
                                <div v-else>
                                    <app-attachment v-model="form.picurl" :multiple="false" :max="1">
                                        <el-button size="mini"><?= \Yii::t('mall/wechat', '选择图片');?></el-button>
                                    </app-attachment>
                            ...
                            <el-form-item label="<?= \Yii::t('mall/wechat', '跳转网页');?>" v-if="form.type == 'view'" prop="url">
                                <el-input placeholder="<?= \Yii::t('mall/wechat', 'a52');?>" size="small" v-model="form.url">
                                </el-input>
                            </el-form-item>
                            <el-form-item label="APPID" v-if="form.type == 'miniprogram'" prop="appid">
                                <el-input placeholder="<?= \Yii::t('mall/wechat', 'a53');?>" size="small" v-model="form.appid">
                                </el-input>
                            </el-form-item>
                            <el-form-item label="<?= \Yii::t('mall/wechat', '页面地址');?>" v-if="form.type == 'miniprogram'" prop="page">
                                <el-input placeholder="<?= \Yii::t('mall/wechat', 'a55');?>" size="small" v-model="form.page">
                                </el-input>
                                <app-pick-link @selected="selectPageUrl">
                                    <el-button type="text" size="mini"><?= \Yii::t('mall/wechat', '选择链接');?></el-button>
                                </app-pick-link>
                            </el-form-item>
                            <el-form-item label="<?= \Yii::t('mall/wechat', '备用网址');?>" v-if="form.type == 'miniprogram'" prop="url">
                                <el-input placeholder="<?= \Yii::t('mall/wechat', '请填写无法打开小程序时跳转的页面');?>" size="small" v-model="form.url">
                                </el-input>
                            </el-form-item>
                        </div>
                    </div>
                ...
                <el-button v-if="form" class="button-item" :loading="btnLoading" type="primary" @click="store('form')"
                           size="small"><?= \Yii::t('mall/wechat', '提交发布');?>
                </el-button>
            </div>
        </div>
    </el-card>
        ...
        data() {
            var checkUrl = (rule, value, callback) => {
                if (this.form.type === 'view') {
                    if(!this.form.url) {
                        callback(new Error('<?= \Yii::t('mall/wechat', '请填写跳转网页');?>'));
                    }else if(this.form.url.indexOf('http://') == -1 && this.form.url.indexOf('https://') == -1) {
                        callback(new Error('<?= \Yii::t('mall/wechat', 'a52');?>'));
                    } else {
                        callback();
                    }
                } else if (this.form.type === 'miniprogram' && !this.form.url) {
                    callback(new Error('<?= \Yii::t('mall/wechat', '请填写无法打开小程序时跳转的页面');?>'));
                } else {
                    callback();
                }
            };
            return {
                ...
                rules: {
                    name: [
                        {required: true, message: '<?= \Yii::t('mall/wechat', '请填写');?><?= \Yii::t('mall/wechat', '菜单名称');?>', trigger: 'blur'},
                    ],
                    url: [
                        {required: true, validator: checkUrl, trigger: 'blur'},
                    ],
                    appid: [
                        {required: true, message: '<?= \Yii::t('mall/wechat', '请填写');?>APPID', trigger: 'blur'},
                    ],
                    page: [
                        {required: true, message: '<?= \Yii::t('mall/wechat', '请填写');?><?= \Yii::t('mall/wechat', '跳转页面的小程序访问路径');?>', trigger: 'blur'},
                    ]
                },
                btnLoading: false,
                cardLoading: false,
                ...
                request({
                    ...
                    if (e.data.code == 0) {
                        ...
                        if(e.data.msg == '微信公众平台信息尚未配置。') {
                            this.$alert(e.data.msg, '<?= \Yii::t('mall/wechat', '提示');?>', {
                                confirmButtonText: '<?= \Yii::t('mall/wechat', '确定');?>',
                                callback: action => {
                                    navigateTo({
