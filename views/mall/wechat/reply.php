...
<div id="app" v-cloak>
    <el-card shadow="never" v-loading="cardLoading" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <el-tabs v-model="activeName" @tab-click="tabClick">
            <el-tab-pane label="<?= \Yii::t('mall/wechat', '关注回复');?>" name="first">
                <el-card shadow="never" class="form_box">
                    <app-wechat-reply v-if="form" :form="form" @update="update"></app-wechat-reply>
                </el-card>
            </el-tab-pane>
            <el-tab-pane label="<?= \Yii::t('mall/wechat', '关键词回复');?>" name="two">
                <el-card shadow="never">
                    <el-button type="primary" size="small"
                               @click="$navigate({r:'mall/wechat/keyword-rule'})"><?= \Yii::t('mall/wechat', '新增关键词回复');?>
                    </el-button>
                    <el-table v-loading="listLoading" border :data="list" style="width: 100%;margin-top: 15px">
                        <el-table-column prop="name" label="<?= \Yii::t('mall/wechat', '规则名称');?>"></el-table-column>
                        <el-table-column prop="keyword" label="<?= \Yii::t('mall/wechat', '关键词');?>"></el-table-column>
                        <el-table-column prop="reply" label="<?= \Yii::t('mall/wechat', '内容');?>"></el-table-column>
                        <el-table-column label="<?= \Yii::t('mall/wechat', '操作');?>" width="280px">
        ...
        <el-button v-if="activeName == 'first'" class='button-item' :loading="btnLoading" @click="submit" type="primary" size="small"><?= \Yii::t('mall/wechat', '保存');?></el-button>
    </el-card>
</div>
<script>
    const app = new Vue({
        ...
        methods: {
            destroy(id) {
                this.$confirm('<?= \Yii::t('mall/wechat', '确认删除该条关键词回复规则');?>?', '<?= \Yii::t('mall/wechat', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('mall/wechat', '确定');?>',
                    cancelButtonText: '<?= \Yii::t('mall/wechat', '取消');?>',
                    type: 'warning'
                }).then(() => {
                    request({
                        params: {
            ...
            submit() {
                if(this.valid) {
                    this.$confirm('<?= \Yii::t('mall/wechat', '是否保存回复内容');?>?', '<?= \Yii::t('mall/wechat', '提示');?>', {
                        confirmButtonText: '<?= \Yii::t('mall/wechat', '确定');?>',
                        cancelButtonText: '<?= \Yii::t('mall/wechat', '取消');?>',
                        type: 'warning'
                    }).then(() => {
