...
<div id="app" v-cloak>
    <el-card shadow="never" class="el-card-app" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;"
        ...
        <div slot="header">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    ...
                    ><?= \Yii::t('mall/wechat', '关键词回复');?></span>
                </el-breadcrumb-item>
                <el-breadcrumb-item>{{edit ? '<?= \Yii::t('mall/wechat', '编辑规则');?>':'<?= \Yii::t('mall/wechat', '新建规则');?>'}}</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="form_box">
            <el-form :model="form" ref="form" :rules="rules" size="small" label-width="150px">
                <el-form-item class="form-item" label="<?= \Yii::t('mall/wechat', '规则名称');?>" prop="name" >
                    <el-input size='small' v-model="form.name" placeholder="<?= \Yii::t('mall/wechat', '请输入规则名称');?>" show-word-limit maxlength="15"></el-input>
                </el-form-item>
                <el-form-item class="form-item" label="<?= \Yii::t('mall/wechat', '关键词');?>" prop="keyword" >
                    <div v-for="(item,index) in form.keyword_list" :key="index" class="select-input">
                        <el-input placeholder="<?= \Yii::t('mall/wechat', '请输入关键词');?>" v-model="item.name" show-word-limit maxlength="15">
                            <el-select v-model="item.status" slot="prepend" placeholder="<?= \Yii::t('mall/wechat', '请选择');?>">
                                <el-option label="<?= \Yii::t('mall/wechat', '全匹配');?>" :value="0"></el-option>
                                <el-option label="<?= \Yii::t('mall/wechat', '模糊匹配');?>" :value="1"></el-option>
                            </el-select>
                        </el-input>
                        <el-button v-if="index > 0" class="del-keyword-btn" size="small" type="text" @click="destroy(index)" circle>
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/wechat', '删除');?>" placement="top">
                                <img src="statics/img/mall/del.png" alt="">
                            </el-tooltip>
                        </el-button>
                    </div>
                    <div class="add-btn">
                        <el-button plain @click="addKeyword" size="small">+<?= \Yii::t('mall/wechat', '添加关键词');?></el-button>
                    </div>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('mall/wechat', '回复方式');?>" prop="type">
                    <el-radio-group v-model="form.status">
                        <el-radio :label="0"><?= \Yii::t('mall/wechat', '回复全部');?></el-radio>
                        <el-radio :label="1"><?= \Yii::t('mall/wechat', '随机回复');?></el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('mall/wechat', '回复内容');?>" prop="show_reply_list">
                    <el-table border :data="form.show_reply_list" ref="reply_table" max-height="760" style="width: 100%;margin-bottom: 20px">
                        <el-table-column prop="index" label="<?= \Yii::t('mall/wechat', '序号');?>"></el-table-column>
                        <el-table-column prop="type" label="<?= \Yii::t('mall/wechat', '类型');?>" width="260">
                            <template slot-scope="scope">
                                {{scope.row.type == 4 ? '<?= \Yii::t('mall/wechat', '图文');?>' : scope.row.type == 3 ? '<?= \Yii::t('mall/wechat', '视频');?>' : scope.row.type == 2 ? '<?= \Yii::t('mall/wechat', '语言');?>' : scope.row.type == 1 ? '<?= \Yii::t('mall/wechat', '图片');?>' : '<?= \Yii::t('mall/wechat', '文字');?>'}}
                            </template>
                        </el-table-column>
                        <el-table-column prop="content" width="320" label="<?= \Yii::t('mall/wechat', '内容');?>">
                            <template slot-scope="scope">
                        ...
                        <el-table-column label="<?= \Yii::t('mall/wechat', '操作');?>" width="180px">
                            <template slot-scope="scope">
                                <el-button size="small" type="text" @click="editRules(scope.row)" circle>
                                    <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/wechat', '编辑');?>" placement="top">
                    ...
                    <el-button plain @click="addContent" size="small">+<?= \Yii::t('mall/wechat', '添加回复');?></el-button>
                    <div class="add-btn">
                    </div>
                </el-form-item>
            </el-form>
        ...
        <el-dialog title="<?= \Yii::t('mall/wechat', '添加回复');?>" :visible.sync="dialogVisible" width="720px">
            <app-wechat-reply v-if="replyForm" :form="replyForm" @update="update"></app-wechat-reply>
            <span slot="footer" class="dialog-footer">
                <el-button size="small" @click="dialogVisible = false"><?= \Yii::t('mall/wechat', '取消');?></el-button>
                <el-button size="small" :loading="dialogBtnLoading" type="primary" @click="addRlue"><?= \Yii::t('mall/wechat', '确定');?></el-button>
            </span>
        </el-dialog>
    </el-card>
</div>
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                rules: {
                    name: [
                        {message: '<?= \Yii::t('mall/wechat', '请填写规则名称');?>', trigger: 'blur', required: true},
                        { min: 1, max: 15, message: '<?= \Yii::t('mall/wechat', 'a32');?>', trigger: 'blur' }
                    ],
                },
            };
        },
