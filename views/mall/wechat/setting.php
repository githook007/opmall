...
<div id="app" v-cloak>
    <el-card shadow="never" v-loading="cardLoading" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <span><?= \Yii::t('mall/wechat', '基础设置');?></span>
        </div>
        <div class="form_box">
            <el-row>
                <el-form :model="ruleForm" :rules="rules" size="small" ref="ruleForm" label-width="150px">
                    <el-col :span="12">
                        <el-form-item v-if="has_third_permission" label="<?= \Yii::t('mall/wechat', '是否使用第三方授权');?>" prop="is_third">
                            <el-radio-group :disabled="third && third.id > 0" v-model="is_third">
                                <el-radio :label="0"><?= \Yii::t('mall/wechat', '否');?></el-radio>
                                <el-radio :label="1"><?= \Yii::t('mall/wechat', '是');?></el-radio>
                            </el-radio-group>
                        </el-form-item>
                        <el-form-item v-if="is_third == 0" label="<?= \Yii::t('mall/wechat', '公众号');?>AppId" prop="appid">
                            <el-input v-model.trim="ruleForm.appid"></el-input>
                        </el-form-item>
                        <el-form-item v-if="is_third == 0" label="<?= \Yii::t('mall/wechat', '公众号');?>appSecret" prop="appsecret">
                            <el-input @focus="hidden.appsecret = false"
                                      v-if="hidden.appsecret"
                                      readonly
                                      placeholder="<?= \Yii::t('mall/wechat', '已隐藏内容');?>">
                            </el-input>
                        ...
                        <div v-if="is_third == 1 && !third" class="choose" flex="dir:left cross:center">
                            ...
                            <div><?= \Yii::t('mall/wechat', '选择已有公众号');?></div>
                            <el-button class='button-item' type="primary" @click="auth" size="small"><?= \Yii::t('mall/wechat', '去绑定');?></el-button>
                        </div>
                        <div v-if="is_third == 1 && third">
                            <div v-if="!qrcode" class="choose" flex="dir:left cross:center">
                                ...
                                <div><?= \Yii::t('mall/wechat', '公众号绑定成功');?></div>
                            </div>
                        </div>
                    </el-col>
                </el-form>
        ...
        <el-button v-if="activeName != 'first' || is_third == 0" class='button-item' :loading="btnLoading" type="primary" @click="store('ruleForm')" size="small"><?= \Yii::t('mall/wechat', '保存');?></el-button>
    </el-card>
</div>
<script>
    const app = new Vue({
        ...
        data() {
            return {
                ...
                rules: {
                    appid: [
                        {required: true, message: '<?= \Yii::t('mall/wechat', '请输入');?>appid', trigger: 'change'},
                    ],
                    appsecret: [
                        {required: true, message: '<?= \Yii::t('mall/wechat', '请输入');?>appsecret', trigger: 'change'},
                    ]
                },
                btnLoading: false,
                cardLoading: false,
        ...
        methods: {
            ...
            auth() {
                ...
                this.$confirm('<?= \Yii::t('mall/wechat', '请在新窗口中完成微信公众号授权');?>', '<?= \Yii::t('mall/wechat', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('mall/wechat', '已成功授权');?>',
                    cancelButtonText: '<?= \Yii::t('mall/wechat', '授权失败');?>',
                    type: 'warning'
                }).then(() => {
                    this.$message({
                        type: 'success',
                        message: '<?= \Yii::t('mall/wechat', '已完成授权');?>!'
                    });
                    this.getDetail();
                }).catch(() => {
                });
