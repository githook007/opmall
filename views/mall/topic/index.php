...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <span><?= \Yii::t('mall/topic', '专题');?></span>
            <el-button style="float: right; margin: -5px 0" type="primary" size="small"
                       @click="$navigate({r:'mall/topic/edit'})"><?= \Yii::t('mall/topic', '新增');?>
            </el-button>
        </div>
        <div class="table-body">
            <el-form size="small" :inline="true" :model="search">
                <el-form-item>
                    <el-select @change="onSubmit" style="width: 150px" v-model="value">
                        <el-option value="" label="<?= \Yii::t('mall/topic', '全部专题');?>"></el-option>
                        <el-option
                ...
                <el-form-item>
                    <div class="input-item">
                        <el-input @keyup.enter.native="onSubmit" size="small" placeholder="<?= \Yii::t('mall/topic', '请输入搜索内容');?>" v-model="keyword" clearable @clear="onSubmit">
            ...
            <el-table v-loading="loading" border :data="list" style="width: 100%;margin-bottom: 15px">
                ...
                <el-table-column width='120' prop="topicType.name" label="<?= \Yii::t('mall/topic', '分类');?>"></el-table-column>
                <el-table-column prop="title" label="<?= \Yii::t('mall/topic', '专题');?>">
                ...
                <el-table-column width='120' prop="layout" label="<?= \Yii::t('mall/topic', '布局方式');?>">
                    <template slot-scope="scope">
                        <span v-if="scope.row.layout == 0"><?= \Yii::t('mall/topic', '小图模式');?></span>
                        <span v-else-if="scope.row.layout == 1"><?= \Yii::t('mall/topic', '大图模式');?></span>
                        <span v-else-if="scope.row.layout == 2"><?= \Yii::t('mall/topic', '多图模式');?></span>
                    </template>
                </el-table-column>
                <el-table-column prop="is_chosen" label="<?= \Yii::t('mall/topic', '是否精选');?>" width="100">
                    <template slot-scope="scope">
                        <el-switch v-model="scope.row.is_chosen" :active-value="1" :inactive-value="0" @change="toChosen(scope.row)">
                        </el-switch>
                    </template>
                </el-table-column>
                <el-table-column prop="sort" label="<?= \Yii::t('mall/topic', '排序');?>" width="200">
                    <template slot-scope="scope">
                        <div v-if="id != scope.row.id">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/topic', '排序');?>" placement="top">
                ...
                <el-table-column label="<?= \Yii::t('mall/topic', '操作');?>" width="150" fixed="right">
                    <template slot-scope="scope">
                        <el-button type="text" class="set-el-button" size="mini" circle @click="handleEdit(scope.$index, scope.row,list.id)">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/topic', '编辑');?>" placement="top">
                                <img src="statics/img/mall/edit.png" alt="">
                            </el-tooltip>
                        </el-button>
                        <el-button type="text" class="set-el-button" size="mini" circle @click="handleDel(scope.$index, scope.row)">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/topic', '删除');?>" placement="top">
        ...
        methods: {
            ...
            toChosen: function(val) {
                ...
                request({
                    ...
                    if(e.data.code == 0){
                        if(val.is_chosen == 1) {
                            this.$message.success('<?= \Yii::t('mall/topic', '已设置为精选');?>');
                        }else if(val.is_chosen == 0) {
                            this.$message.success('<?= \Yii::t('mall/topic', '已取消精选');?>');
                        }
                    }else{
                        this.$alert(e.data.msg, '<?= \Yii::t('mall/topic', '提示');?>', {
                            confirmButtonText: '<?= \Yii::t('mall/topic', '确定');?>'
            ...
            handleDel: function(index, row) {
                let _this = this;
                this.$confirm('<?= \Yii::t('mall/topic', '确认删除该记录吗');?>?', '<?= \Yii::t('mall/topic', '提示');?>', {
                    type: 'warning'
                }).then(() => {
                    let para = { id: row.id};
                    request({
