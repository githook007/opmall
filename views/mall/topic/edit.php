...
<section id="app" v-cloak>
    <el-card class="box-card" shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header" class="clearfix">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><span style="color: #409EFF;cursor: pointer"
                                          @click="$navigate({r:'mall/topic/index'})"><?= \Yii::t('mall/topic', '专题');?></span></el-breadcrumb-item>
                <el-breadcrumb-item><?= \Yii::t('mall/topic', '专题编辑');?></el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <el-tabs v-model="activeName" @tab-click="handleClick" style="background-color: #FFFFFF;padding: 20px;">
            <el-tab-pane label="<?= \Yii::t('mall/topic', '基础设置');?>" name="first">
                <div class="form-body">
                    <el-form :model="form" label-width="10rem" v-loading="loading" :rules="FormRules" ref="form">
                        <el-form-item label="<?= \Yii::t('mall/topic', '标题');?>" prop="title" size="small">
                            <el-input v-model="form.title" auto-complete="off"></el-input>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('mall/topic', '专题列表布局方式');?>" prop="layout" size="small">
                            <el-radio v-model="form.layout" :label="0"><?= \Yii::t('mall/topic', '小图模式');?></el-radio>
                            <el-radio v-model="form.layout" :label="1"><?= \Yii::t('mall/topic', '大图模式');?></el-radio>
                            <el-radio v-model="form.layout" :label="2"><?= \Yii::t('mall/topic', '多图模式');?></el-radio>
                            <div style="color: #636C72;font-size: 12px;margin-top: -0.5rem;">
                                <?= \Yii::t('mall/topic', '小图模式建议封面图片大小');?>
                            </div>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('mall/topic', '封面图');?>" prop="cover_pic" size="small">
                            <app-attachment @selected="coverUrl" :multiple="form.layout == 2">
                                <el-tooltip class="item" :max="3"
                                            effect="dark"
                                            :content="form.layout == 1 ? '<?= \Yii::t('mall/topic', '建议尺寸');?>: 702 * 350' : '<?= \Yii::t('mall/topic', '建议尺寸');?>: 268 * 202'"
                                            placement="top">
                                    <el-button size="mini"><?= \Yii::t('mall/topic', '选择图片');?></el-button>
                                </el-tooltip>
                            </app-attachment>
                        ...
                        <el-form-item prop="abstract" size="small">
                            <template slot='label'>
                                <span><?= \Yii::t('mall/topic', '摘要');?></span>
                                <el-tooltip effect="dark" content="<?= \Yii::t('mall/topic', '专题内容的简介');?>"
                                            placement="top">
                        ...
                        <el-form-item label="<?= \Yii::t('mall/topic', '自定义分享标题');?>" prop="app_share_title" size="small">
                            <el-input v-model="form.app_share_title" auto-complete="off"></el-input>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('mall/topic', '自定义分享图片');?>">
                            <app-attachment v-model="form.qrcode_pic" :multiple="false" :max="1">
                                <el-tooltip class="item"
                                            effect="dark"
                                            content="<?= \Yii::t('mall/topic', '建议尺寸');?>:420 * 336"
                                            placement="top">
                                    <el-button size="mini"><?= \Yii::t('mall/topic', '选择图片');?></el-button>
                                </el-tooltip>
                            </app-attachment>
                        ...
                        <el-form-item label="<?= \Yii::t('mall/topic', '是否精选');?>" prop="is_chosen" size="small">
                            <el-switch v-model="form.is_chosen" :active-value="1" :inactive-value="0"></el-switch>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('mall/topic', '分类');?>" prop="type" size="small">
                        ...
                        <el-form-item prop="virtual_read_count" size="small">
                            <template slot='label'>
                                <span><?= \Yii::t('mall/topic', '虚拟阅读量');?></span>
                                <el-tooltip effect="dark" content="<?= \Yii::t('mall/topic', '手机端显示的阅读量');?>"
                                            placement="top">
                                    <i class="el-icon-info"></i>
                                </el-tooltip>
                            </template>
                        ...
                        <el-form-item prop="virtual_favorite_count" size="small" hidden>
                            <template slot='label'>
                                <span><?= \Yii::t('mall/topic', '虚拟收藏量');?></span>
                                <el-tooltip effect="dark" content="<?= \Yii::t('mall/topic', '手机端显示的收藏量');?>"
                                            placement="top">
                                    <i class="el-icon-info"></i>
                                </el-tooltip>
                            </template>
                        ...
                        <el-form-item label="<?= \Yii::t('mall/topic', '排序');?>" prop="sort" size="small">
                            <el-input v-model="form.sort" auto-complete="off"></el-input>
                        </el-form-item>
                    </el-form>
                </div>
            </el-tab-pane>
            <el-tab-pane label="<?= \Yii::t('mall/topic', '详情设置');?>" name="second">
        ...
        <div class="button">
            <el-button class="button-item" type="primary" @click="onSubmit" :loading=btnLoading size="small"><?= \Yii::t('mall/topic', '提交');?>
            </el-button>
        </div>
    </el-card>
</section>
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                FormRules: {
                    title: [
                        {required: true, message: '<?= \Yii::t('mall/topic', '标题不能为空');?>', trigger: 'blur'},
                        {min: 1, max: 30, message: "<?= \Yii::t('mall/topic', '标题长度在1');?>"},
                    ],
                    cover_pic: [
                        {required: true, message: '<?= \Yii::t('mall/topic', '封面图片不能为空');?>', trigger: 'blur'},
                    ],
                    qrcode_pic: [
                        {required: true, message: '<?= \Yii::t('mall/topic', '海报分享图不能为空');?>', trigger: 'blur'},
                    ],
                    type: [
                        {required: true, message: '<?= \Yii::t('mall/topic', '请选择专题分类');?>', trigger: 'blur'},
                    ],
                },
                activeName: 'first',
            };
                ...
                this.$refs.form.validate((valid) => {
                    if (valid) {
                        ...
                        request({
                            ...
                            this.btnLoading = false;
                            this.$alert(e.data.msg, '<?= \Yii::t('mall/topic', '提示');?>', {
                                confirmButtonText: '<?= \Yii::t('mall/topic', '确定');?>'
                            })
                        });
                    }
                });
                ...
                if (e.length) {
                    ...
                    if (this.form.layout == 2) {
                        if (this.form.pic_list.length >= 3) {
                            this.$message.error('<?= \Yii::t('mall/topic', '图片最多上传三张');?>');
                        } else {
                            for (let i in e) {
                                if (this.form.pic_list.length < 3) {
