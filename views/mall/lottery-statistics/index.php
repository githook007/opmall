...
<div id="app" v-cloak>
    <el-card v-loading="loading" shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <app-header :url="url" :new-search="JSON.stringify(search)"><?= \Yii::t('mall/lottery_statistics', '幸运抽奖');?></app-header>
        </div>
        <div class="table-body" style="display:flex;justify-content: space-between;align-items: center;padding-right: 20px">
            <app-search
                    ...
                    placeholder="<?= \Yii::t('mall/lottery_statistics', '请输入商品名称搜索');?>"
        ...
        <div class="table-body" style="padding: 20px">
            <el-table @sort-change="changeSort" :header-cell-style="{background:'#F3F5F6','color':'#303133',padding: '10px 0',fontWeight: '400'}" :data="list">
                <el-table-column prop="name" width="500" label="<?= \Yii::t('mall/lottery_statistics', '商品名称');?>">
                ...
                <el-table-column prop="miaosha_time" width="180" label="<?= \Yii::t('mall/lottery_statistics', '活动时间');?>">
                    <template slot-scope="scope">
                        {{scope.row.start_at}}<?= \Yii::t('mall/lottery_statistics', '至');?>{{scope.row.end_at}}
                    </template>
                </el-table-column>
                <el-table-column prop="invitee" label="<?= \Yii::t('mall/lottery_statistics', '被邀请人数');?>" :label-class-name="prop == 'invitee' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                <el-table-column prop="participant" label="<?= \Yii::t('mall/lottery_statistics', '参与人数');?>" :label-class-name="prop == 'participant' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                <el-table-column prop="code_num" label="<?= \Yii::t('mall/lottery_statistics', '抽奖券码数量');?>" :label-class-name="prop == 'code_num' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                <el-table-column prop="status" label="<?= \Yii::t('mall/lottery_statistics', '状态');?>">
                    <template slot-scope="scope">
                        <span v-if="scope.row.status == '<?= \Yii::t('mall/lottery_statistics', '未开始');?>'" style="color: #FFA360">{{scope.row.status}}</span>
                        <span v-if="scope.row.status == '<?= \Yii::t('mall/lottery_statistics', '进行中');?>'" style="color: #4BC282">{{scope.row.status}}</span>
                        <span v-if="scope.row.status == '<?= \Yii::t('mall/lottery_statistics', '已结束');?>'" style="color: #FF8585">{{scope.row.status}}</span>
                    </template>
                </el-table-column>
            </el-table>
            <div style="margin-top: 10px;" flex="box:last cross:center">
                <div style="visibility: hidden">
                    <el-button plain type="primary" size="small"><?= \Yii::t('mall/lottery_statistics', '批量操作');?>1</el-button>
                    <el-button plain type="primary" size="small"><?= \Yii::t('mall/lottery_statistics', '批量操作');?>2</el-button>
                </div>
                <div>
                    <el-pagination
                            v-if="pagination"
...
<script>
    const app = new Vue({
        ...
        data() {
            return {
                url: 'mall/lottery-statistics/index',
                loading: false,
                // 今天
                today: '',
                // 七天前
