...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <div>
                <span><?= \Yii::t('mall/user', '用户卡券');?></span>
            </div>
        </div>
        <div class="table-body">
            <div>
                <el-select size="small" v-model="status" @change="handleCommand" class="select">
                    <el-option key="0" label="<?= \Yii::t('mall/user', '全部');?>" :value="0"></el-option>
                    <el-option key="1" label="<?= \Yii::t('mall/user', '未使用');?>" :value="1"></el-option>
                    <el-option key="2" label="<?= \Yii::t('mall/user', '已使用');?>" :value="2"></el-option>
                </el-select>
                <div class="input-item">
                    <el-input size="small" @keyup.enter.native="search" clearable @clear="search" placeholder="<?= \Yii::t('mall/user', '请输入搜索内容');?>"
                              v-model="key_name">
                        <el-select style="width: 100px" slot="prepend" v-model="key_code">
                            <el-option label="<?= \Yii::t('mall/user', '门店');?>" value="0"></el-option>
                            <el-option label="<?= \Yii::t('mall/user', '昵称');?>" value="2"></el-option>
                            <el-option label="<?= \Yii::t('mall/user', '卡券ID');?>" value="3"></el-option>
                            <el-option label="<?= \Yii::t('mall/user', '卡券名称');?>" value="1"></el-option>
                        </el-select>
                        <el-button slot="append" icon="el-icon-search" @click="search"></el-button>
                    </el-input>
                </div>
                <el-button type="primary" @click="batchDestroy" size="small"><?= \Yii::t('mall/user', '批量删除');?></el-button>
            </div>
            <div style="display: flex;margin: 10px 0;">
                <div style="margin:5px 10px 5px 0;">
                    <el-date-picker
                            ...
                            start-placeholder="<?= \Yii::t('mall/user', '发放开始日期');?>"
                            end-placeholder="<?= \Yii::t('mall/user', '发放结束日期');?>"
                    ></el-date-picker>
                    <br>
                </div>
                <div style="margin:5px 0;">
                    <el-date-picker
                            ...
                            value-format="yyyy-MM-dd HH:mm:ss"
                            range-separator="<?= \Yii::t('mall/user', '至');?>"
                            start-placeholder="<?= \Yii::t('mall/user', '核销开始日期');?>"
                            end-placeholder="<?= \Yii::t('mall/user', '核销结束日期');?>"
                    ></el-date-picker>
                </div>
            </div>
            <el-table border :data="form" style="width: 100%" @selection-change="selsChange" v-loading="listLoading">
                ...
                <el-table-column prop="nickname" label="<?= \Yii::t('mall/user', '昵称');?>" width="150"></el-table-column>
                <el-table-column label="<?= \Yii::t('mall/user', '卡券名称');?>" width="150">
                ...
                <el-table-column label="<?= \Yii::t('mall/user', '卡券信息');?>" width="340">
                    <template slot-scope="scope">
                        <div>
                            ...
                            <div flex="dir:left box:first cross:top" style="margin-top: 8px;">
                                <el-tag size="small"><?= \Yii::t('mall/user', '来源于');?></el-tag>
                                ...
                                <div>
                                    ...
                                    <el-tag size="mini" v-if="scope.row.from.id" :disable-transitions="true"><?= \Yii::t('mall/user', '原卡券id');?>{{scope.row.from.id}}</el-tag>
                                </div>
                            </div>
                            <div flex="dir:left box:first" v-if="scope.row.receive.name" style="margin-top: 8px;">
                                <el-tag size="small" type="warning"><?= \Yii::t('mall/user', '已转赠');?></el-tag>
                ...
                <el-table-column prop="created_at" label="<?= \Yii::t('mall/user', '发放时间');?>" width="180"></el-table-column>
                <el-table-column width="250" label="<?= \Yii::t('mall/user', '状态剩余次数');?>">
                    <template slot-scope="scope">
                        {{scope.row.is_use == 1 ? '<?= \Yii::t('mall/user', '已使用');?>' : '<?= \Yii::t('mall/user', '未使用');?>'}}({{scope.row.number -
                        scope.row.use_number}}/{{scope.row.use_number}}/{{scope.row.number}})
                    </template>
                </el-table-column>
                <el-table-column prop="store_name" label="<?= \Yii::t('mall/user', '门店');?>"></el-table-column>
                <el-table-column label="<?= \Yii::t('mall/user', '核销日期');?>" width="180">
                    <template slot-scope="scope">
                        <div flex="dir:top">
                            ...
                            <span @click="openDialog(scope.row)" v-if="scope.row.is_show_history == 1"
                                  class="history-clerk"><?= \Yii::t('mall/user', '查看历史核销');?></span>
                        </div>
                    </template>
                </el-table-column>
    ...
    <el-dialog title="<?= \Yii::t('mall/user', '核销记录');?>" :visible.sync="historyForm.dialogVisible">
        <el-table v-loading="historyForm.listLoading" :data="historyForm.list" border>
            <el-table-column property="clerked_at" label="<?= \Yii::t('mall/user', '核销时间');?>" width="180"></el-table-column>
            <el-table-column property="store_name" label="<?= \Yii::t('mall/user', '门店');?>" width="150"></el-table-column>
            <el-table-column property="clerk_user" label="<?= \Yii::t('mall/user', '核销员');?>" width="150"></el-table-column>
            <el-table-column property="use_number" label="<?= \Yii::t('mall/user', '核销次数');?>"></el-table-column>
            <el-table-column property="surplus_number" label="<?= \Yii::t('mall/user', '剩余次数');?>"></el-table-column>
        </el-table>
        <div style="margin: 20px 0;" flex="dir:left box:last">
            ...
            <el-button type="primary" size="small" @click="historyForm.dialogVisible = false"><?= \Yii::t('mall/user', '确定');?></el-button>
        </div>
    </el-dialog>
</div>
<script>
        ...
        methods: {
            ...
            batchDestroy: function () {
                if (this.ids.length == 0) {
                    this.$message.error('<?= \Yii::t('mall/user', '请先勾选商品');?>');
                    return;
                }
                let ids = [];
                this.ids.forEach(v => {
                ...
                this.$confirm('<?= \Yii::t('mall/user', '确认删除选中记录吗');?>?', '<?= \Yii::t('mall/user', '提示');?>', {
                    type: 'warning'
                }).then(() => {
