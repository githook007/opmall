...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <app-header :url="url" :new-search="JSON.stringify(search)"><?= \Yii::t('mall/miaosha_statistics', '整点秒杀');?></app-header>
        </div>
        <div class="table-body">
            <app-search
                    ...
                    @search="searchList"
                    :new-search="search"
                    placeholder="<?= \Yii::t('mall/miaosha_statistics', '输入商品名称搜索');?>"
        ...
        <div class="table-body" style="padding: 20px">
            <el-table v-loading="loading" @sort-change="changeSort" :header-cell-style="{background:'#F3F5F6','color':'#303133',padding: '6px 0',fontWeight: '400'}" :data="list">
                <el-table-column prop="miaosha_time" width="250" label="<?= \Yii::t('mall/miaosha_statistics', '秒杀时间');?>">
                </el-table-column>
                <el-table-column prop="name" width="600" label="<?= \Yii::t('mall/miaosha_statistics', '商品名称');?>">
                ...
                <el-table-column prop="user_num" label="<?= \Yii::t('mall/miaosha_statistics', '支付人数');?>" :label-class-name="prop == 'user_num' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                <el-table-column prop="goods_num" label="<?= \Yii::t('mall/miaosha_statistics', '支付件数');?>" :label-class-name="prop == 'goods_num' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                <el-table-column prop="pay_price" label="<?= \Yii::t('mall/miaosha_statistics', '支付金额');?>" :label-class-name="prop == 'pay_price' ? 'sort-active': ''" sortable='custom'>
                </el-table-column>
                <el-table-column prop="total_pay_price" label="<?= \Yii::t('mall/miaosha_statistics', '状态');?>">
                    <template slot-scope="scope">
                        <span v-if="scope.row.status == '<?= \Yii::t('mall/miaosha_statistics', '未开始');?>'" style="color: #FFA360">{{scope.row.status}}</span>
                        <span v-if="scope.row.status == '<?= \Yii::t('mall/miaosha_statistics', '进行中');?>'" style="color: #4BC282">{{scope.row.status}}</span>
                        <span v-if="scope.row.status == '<?= \Yii::t('mall/miaosha_statistics', '已结束');?>'" style="color: #FF8585">{{scope.row.status}}</span>
                    </template>
                </el-table-column>
            </el-table>
            <div style="margin-top: 10px;" flex="box:last cross:center">
                <div style="visibility: hidden">
                    <el-button plain type="primary" size="small"><?= \Yii::t('mall/miaosha_statistics', '批量操作');?>1</el-button>
                    <el-button plain type="primary" size="small"><?= \Yii::t('mall/miaosha_statistics', '批量操作');?>2</el-button>
                </div>
                <div>
                    <el-pagination
                            v-if="pagination"
...
<script>
    const app = new Vue({
        ...
        data() {
            return {
                url: 'mall/miaosha-statistics/index',
                loading: false,
                // 今天
                today: '',
                // 七天前
