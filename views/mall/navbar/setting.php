...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;"
        ...
        <div slot="header">
            <div>
                <span><?= \Yii::t('mall/navbar', '标签栏设置');?></span>
            </div>
        </div>
        <div style="display: flex;">
            <div class="mobile">
                <div class="screen">
                    <div class="head"
                         :style="{backgroundColor: ruleForm.top_background_color, color: ruleForm.top_text_color}"><?= \Yii::t('mall/navbar', '这里是标题');?>
                    </div>
                    <div class="foot"
            ...
            <div style="width: 100%;">
                <el-form :model="ruleForm" :rules="rules" size="small" ref="ruleForm" label-width="120px">
                    <div class="title"><?= \Yii::t('mall/navbar', '顶部标题栏');?></div>
                    <el-row class='form-body'>
                        <el-col :span="6">
                            <el-form-item label="<?= \Yii::t('mall/navbar', '文字颜色');?>" prop="top_text_color">
                                <el-radio v-model="ruleForm.top_text_color" label="#ffffff"><?= \Yii::t('mall/navbar', '白色');?></el-radio>
                                <el-radio v-model="ruleForm.top_text_color" label="#000000"><?= \Yii::t('mall/navbar', '黑色');?></el-radio>
                            </el-form-item>
                            <el-form-item label="<?= \Yii::t('mall/navbar', '背景颜色');?>" prop="top_background_color">
                                <el-color-picker
                    ...
                    <div class="title"><?= \Yii::t('mall/navbar', '底部标签栏');?></div>
                    <div class='form-body'>
                        <el-form-item label="<?= \Yii::t('mall/navbar', '背景颜色');?>" prop="bottom_background_color">
                            <el-color-picker
                                    v-model="ruleForm.bottom_background_color">
                            </el-color-picker>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('mall/navbar', '导航阴影效果');?>">
                            <el-switch
                                    v-model="ruleForm.shadow"
                                    active-color="#3399ff">
                            </el-switch>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('mall/navbar', '是否弹出效果');?>">
                            <el-switch
                                    v-model="ruleForm.is_eject"
                                    active-color="#3399ff">
                            </el-switch>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('mall/navbar', '第几个导航弹出');?>">
                            <el-input size="small" style="width: 200px;" type="number" v-model="ruleForm.eject_num"></el-input>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('mall/navbar', '底部导航图标');?>">
                            <div flex="main:left">
                    ...
                    <el-button class="button-item" :loading="btnLoading" type="primary" @click="store('ruleForm')"
                               size="small"><?= \Yii::t('mall/navbar', '保存');?>
                    </el-button>
                    <el-button class="button-item" :loading="btnLoading" size="small" plain @click="restoreDefault">
                        <?= \Yii::t('mall/navbar', '恢复默认');?>
                    </el-button>
                </el-form>
            </div>
        </div>
        <el-dialog @close="dialogColse" title="<?= \Yii::t('mall/navbar', '导航菜单编辑');?>" :visible.sync="dialogFormVisible">
            <el-form :model="dialogRuleForm" :rules="dialogRules" size="small" ref="dialogRuleForm" label-width="120px">
                <el-row>
                    <el-col :span="18">
                        <el-form-item label="<?= \Yii::t('mall/navbar', '图标');?>" prop="icon">
                            <app-attachment :multiple="false" :max="1" @selected="iconUrl" tag="navbar">
                                <el-tooltip effect="dark" content="<?= \Yii::t('mall/navbar', '建议尺寸');?>64*64" placement="top">
                                    <el-button size="mini"><?= \Yii::t('mall/navbar', '选择文件');?></el-button>
                                </el-tooltip>
                            </app-attachment>
                            <app-image mode="aspectFill" width="80px" height="80px"
                                       :src="dialogRuleForm.icon"></app-image>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('mall/navbar', '选择状态图标');?>" prop="active_icon">
                            <app-attachment :multiple="false" :max="1" @selected="activeIconUrl" tag="navbar">
                                <el-tooltip effect="dark" content="<?= \Yii::t('mall/navbar', '建议尺寸');?>64*64" placement="top">
                                    <el-button size="mini"><?= \Yii::t('mall/navbar', '选择文件');?></el-button>
                                </el-tooltip>
                            </app-attachment>
                        ...
                        <el-form-item label="<?= \Yii::t('mall/navbar', '名称');?>" prop="text">
                            <el-input v-model.trim="dialogRuleForm.text"></el-input>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('mall/navbar', '文字颜色');?>" prop="color">
                            <el-color-picker
                                    v-model="dialogRuleForm.color">
                            </el-color-picker>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('mall/navbar', '选中文字颜色');?>" prop="active_color">
                            <el-color-picker
                                    v-model="dialogRuleForm.active_color">
                            </el-color-picker>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('mall/navbar', '导航链接');?>" prop="url">
                            <div flex="box:last">
                                <el-input disabled v-model="dialogRuleForm.url"></el-input>
                                <div>
                                    <app-pick-link @selected="selectNavUrl" ignore="navigate">
                                        <el-button><?= \Yii::t('mall/navbar', '选择链接');?></el-button>
                                    </app-pick-link>
                                </div>
                            </div>
                        </el-form-item>
            ...
            <div slot="footer">
                <el-button size="small" @click="dialogFormVisible = false"><?= \Yii::t('mall/navbar', '取消');?></el-button>
                <el-button size="small" type="primary" @click="dialogFormSubmit('dialogRuleForm')"><?= \Yii::t('mall/navbar', '提交');?></el-button>
            </div>
        </el-dialog>
    </el-card>
</div>
...
<script>
    const app = new Vue({
            ...
            return {
                ...
                rules: {
                    bottom_background_color: [
                        {required: true, message: '<?= \Yii::t('mall/navbar', '请选择底栏背景颜色');?>', trigger: 'change'},
                    ],
                },
                btnLoading: false,
                cardLoading: false,
                ...
                dialogRules: {
                    icon: [
                        {required: true, message: '<?= \Yii::t('mall/navbar', '请选择图标');?>', trigger: 'change'},
                    ],
                    active_icon: [
                        {required: true, message: '<?= \Yii::t('mall/navbar', '请选择选中状态图标');?>', trigger: 'change'},
                    ],
                    color: [
                        {required: true, message: '<?= \Yii::t('mall/navbar', '请选择文字颜色');?>', trigger: 'change'},
                    ],
                    active_color: [
                        {required: true, message: '<?= \Yii::t('mall/navbar', '请选择文字选中状态颜色');?>', trigger: 'change'},
                    ],
                    text: [
                        {required: true, message: '<?= \Yii::t('mall/navbar', '请输入导航名称');?>', trigger: 'change'},
                    ],
                    url: [
                        {required: true, message: '<?= \Yii::t('mall/navbar', '请选择导航链接');?>', trigger: 'change'},
                    ],
                },
                navCurrent: 0,
            };
        ...
        methods: {
            ...
            restoreDefault() {
                let self = this;
                self.$confirm('<?= \Yii::t('mall/navbar', '恢复默认是否继续');?>', '<?= \Yii::t('mall/navbar', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('mall/navbar', '确定');?>',
                    cancelButtonText: '<?= \Yii::t('mall/navbar', '取消');?>',
                    type: 'warning'
                }).then(() => {
                    self.btnLoading = true;
                    request({
                    ...
                    self.$message.info('<?= \Yii::t('mall/navbar', '已取消');?>')
                });
            },
            navClick(index) {
