...
<div id="app" v-cloak>
    <div slot="header" class="header-box">
        <el-breadcrumb separator="/">
            ...
            <el-breadcrumb-item><?= \Yii::t('mall/coupon', '优惠券');?>{{id > 0 ? '<?= \Yii::t('mall/coupon', '编辑');?>' : '<?= \Yii::t('mall/coupon', '新建');?>'}}</el-breadcrumb-item>
        </el-breadcrumb>
    </div>
    <el-card v-loading="loading" style="border:0" shadow="never" body-style="background-color: #f3f3f3;padding: 0 0;">
        <el-form :model="form" label-width="171px" :rules="FormRules" ref="form">
            <el-tabs v-model="activeName">
                <el-tab-pane label="<?= \Yii::t('mall/coupon', '基础设置');?>" name="first">
                    <div class="form-body">
                        <el-form-item label="<?= \Yii::t('mall/coupon', '优惠券名称');?>" prop="name">
                            <el-input size="small" v-model="form.name" autocomplete="off"></el-input>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('mall/coupon', '优惠券类型');?>" prop="type">
                            <el-radio v-model="form.type" label="2" @change="typeChange"><?= \Yii::t('mall/coupon', '满减券');?></el-radio>
                            <el-radio v-model="form.type" label="1" @change="typeChange"><?= \Yii::t('mall/coupon', '打折券');?></el-radio>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('mall/coupon', '指定商品类别或商品');?>" prop="appoint_type">
                            <el-radio v-model="form.appoint_type" @click.native="catsStatus" label="1"><?= \Yii::t('mall/coupon', '指定商品类别');?></el-radio>
                            <el-radio v-model="form.appoint_type" @click.native="goodsListVisible = true" label="2">
                                <?= \Yii::t('mall/coupon', '指定商品');?>
                            </el-radio>
                            <el-radio v-model="form.appoint_type" label="3"><?= \Yii::t('mall/coupon', '全场通用');?></el-radio>
                            <el-radio v-if="isScanCodePayShow" v-model="form.appoint_type" label="4"><?= \Yii::t('mall/coupon', '当面付');?></el-radio>
                            <el-radio v-if="isExchange" v-model="form.appoint_type" label="5"><?= \Yii::t('mall/coupon', '礼品卡');?></el-radio>
                        ...
                        <el-form-item prop="min_price">
                            <template slot='label'>
                                <span><?= \Yii::t('mall/coupon', '最低消费金额');?></span>
                                <el-tooltip effect="dark" content="<?= \Yii::t('mall/coupon', '购物金额');?>"
                                            placement="top">
                        ...
                        <template v-if="form.type == 2">
                            <el-form-item prop="sub_price">
                                <template slot='label'>
                                    <span><?= \Yii::t('mall/coupon', '优惠金额');?></span>
                                    <el-tooltip effect="dark" content="<?= \Yii::t('mall/coupon', '优惠券只能抵消商品金额');?>"
                                                placement="top">
                                        <i class="el-icon-info"></i>
                                    </el-tooltip>
                                </template>
                        ...
                        <block v-show="form.type == 1">
                            <el-form-item prop="discount">
                                <template slot='label'>
                                    <span><?= \Yii::t('mall/coupon', '优惠折扣');?></span>
                                </template>
                                <el-input size="small" type="number" v-model="form.discount" autocomplete="off">
                                    <template slot="append"><?= \Yii::t('mall/coupon', '折');?></template>
                                </el-input>
                            </el-form-item>
                            <el-form-item prop="discount_limit">
                                <template slot='label'>
                                    <span><?= \Yii::t('mall/coupon', '优惠上限');?></span>
                                </template>
                                <el-input size="small" type="number" v-model="form.discount_limit" autocomplete="off">
                                    <template slot="prepend"><?= \Yii::t('mall/coupon', '最多优惠');?></template>
                                    <template slot="append"><?= \Yii::t('mall/coupon', '元');?></template>
                                </el-input>
                            </el-form-item>
                        </block>

                        <el-form-item label="<?= \Yii::t('mall/coupon', '优惠券有效期');?>" prop="expire_type">
                            <el-radio v-model="form.expire_type" @change="toExpirce" label="1"><?= \Yii::t('mall/coupon', '领取后N天内有效');?></el-radio>
                            <el-radio v-model="form.expire_type" @change="toExpirce" label="2"><?= \Yii::t('mall/coupon', '时间段');?></el-radio>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('mall/coupon', '有效天数');?>" v-if="form.expire_type == 1" prop="expire_day">
                            <el-input size="small" v-model="form.expire_day" autocomplete="off"></el-input>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('mall/coupon', '有效期范围');?>" v-if="form.expire_type == 2" prop="time">
                            <el-date-picker size='small' v-model="time" value-format="yyyy-MM-dd HH:mm:ss" type="datetimerange"
                                            range-separator="<?= \Yii::t('mall/coupon', '至');?>" start-placeholder="<?= \Yii::t('mall/coupon', '开始日期');?>" end-placeholder="<?= \Yii::t('mall/coupon', '结束日期');?>">
                            </el-date-picker>
                        </el-form-item>
                        <el-form-item prop="total_count">
                            <template slot='label'>
                                <span><?= \Yii::t('mall/coupon', '优惠券库存');?></span>
                                <el-tooltip effect="dark" content="<?= \Yii::t('mall/coupon', '优惠券库存为0则无法领取或发放');?>"
                                            placement="top">
                                    <i class="el-icon-info"></i>
                                </el-tooltip>
                            </template>
                            ...
                            <el-checkbox v-model="form.total_count" :true-label="-1" :false-label="0"><?= \Yii::t('mall/coupon', '无限制');?></el-checkbox>
                        </el-form-item>

                        <el-form-item label="<?= \Yii::t('mall/coupon', '赠送规则');?>" prop="use_obtain">
                            <el-radio v-model="form.use_obtain" label="1"><?= \Yii::t('mall/coupon', '使用后赠送一张相同优惠券');?></el-radio>
                            <el-radio v-model="form.use_obtain" label="0"><?= \Yii::t('mall/coupon', '使用不赠送优惠券');?></el-radio>
                        </el-form-item>

                        <el-form-item label="<?= \Yii::t('mall/coupon', '每人限领次数');?>" prop="can_receive_count" class="can_receive_count">
                            <el-radio-group v-model="receive_count_type" class="special">
                                <div>
                                    <el-radio :label="0"><?= \Yii::t('mall/coupon', '不限次数');?></el-radio>
                                </div>
                                <div style="margin-top: 10px;" @click="receive_count_type = 1">
                                    <el-radio :label="1">
                                        <el-input size="small" type="number" v-model.number="form.can_receive_count"
                                                  :min="-1" :disabled="receive_count_type == 0" placeholder="<?= \Yii::t('mall/coupon', '请输入次数');?>"
                                                  oninput="this.value = this.value.replace(/[^0-9]/g, '')"
                                                  autocomplete="off" @focus="receive_count_type = 1">
                                            <template slot="prepend"><?= \Yii::t('mall/coupon', '限');?></template>
                                            <template slot="append"><?= \Yii::t('mall/coupon', '次');?></template>
                                        </el-input>
                                    </el-radio>
                                </div>
                            </el-radio-group>
                        ...
                        <el-form-item prop="app_share_title">
                            <label slot="label">
                                <span><?= \Yii::t('mall/coupon', '自定义分享标题');?></span>
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/coupon', '分享给好友时作为商品名称');?>"
                                            placement="top">
                                    <i class="el-icon-info"></i>
                                </el-tooltip>
                            </label>
                            <el-input placeholder="<?= \Yii::t('mall/coupon', '请输入分享标题');?>" size="small"
                                      v-model="form.app_share_title"></el-input>
                            <el-button @click="app_share.dialog = true;app_share.type = 'name_bg'"
                                       type="text"><?= \Yii::t('mall/coupon', '查看图例');?>
                            </el-button>
                        </el-form-item>

                        <el-form-item  prop="app_share_pic">
                            <label slot="label">
                                <span><?= \Yii::t('mall/coupon', '自定义分享图片');?></span>
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/coupon', '分享给好友时');?>"
                                            placement="top">
                                    <i class="el-icon-info"></i>
                                </el-tooltip>
                            </label>
                            <app-attachment v-model="form.app_share_pic" :multiple="false" :max="1">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/coupon', '建议尺寸');?>:420 * 336"
                                            placement="top">
                                    <el-button size="mini"><?= \Yii::t('mall/coupon', '选择图片');?></el-button>
                                </el-tooltip>
                            </app-attachment>
                        ...
                        <el-dialog :title="app_share['type'] == 'pic_bg' ? `<?= \Yii::t('mall/coupon', '查看自定义分享图片图例');?>`:`<?= \Yii::t('mall/coupon', '查看自定义分享标题图例');?>`"
                            ...
                            <div slot="footer" class="dialog-footer">
                                <el-button @click="app_share.dialog = false" type="primary"><?= \Yii::t('mall/coupon', '我知道了');?></el-button>
                            </div>
                        </el-dialog>
                        <el-form-item label="" prop="sort">
                            <template slot='label'>
                                <span><?= \Yii::t('mall/coupon', '排序');?></span>
                                <el-tooltip effect="dark" content="<?= \Yii::t('mall/coupon', '排序按升序排列');?>"
                                            placement="top">
                                    <i class="el-icon-info"></i>
                                </el-tooltip>
                            </template>
                        ...
                        <el-form-item label="<?= \Yii::t('mall/coupon', '使用说明');?>" prop="rule">
                            <el-input type="textarea" v-model="form.rule"></el-input>
                        </el-form-item>
                    </div>
                </el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('mall/coupon', '领取方式设置');?>" name="second">
                    <div class="form-body">
                        <el-form-item label="<?= \Yii::t('mall/coupon', '领取方式');?>">
                            <el-checkbox :true-label="1" :false-label="0" v-model="form.is_member"><?= \Yii::t('mall/coupon', '会员专享');?></el-checkbox>
                            <el-checkbox :true-label="1" :false-label="0" v-model="form.is_join"><?= \Yii::t('mall/coupon', '领券中心');?></el-checkbox>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('mall/coupon', '会员专享');?>" prop="couponMember" v-if="form.is_member == 1">
                            <template v-if="members.length > 0">
                                <el-checkbox :indeterminate="isIndeterminate" v-model="checkAll"
                                             @change="handleCheckAllChange"><?= \Yii::t('mall/coupon', '全选');?>
                                </el-checkbox>
                            ...
                            <template v-else>
                                <div flex>
                                    <div style="color: #ff4544;font-size: 12px;margin-left: 24px;"><?= \Yii::t('mall/coupon', '必须在');?>
                                        <el-button type="text"
                                                   @click="$navigate({r:'mall/mall-member/index'}, true)">
                                            <?= \Yii::t('mall/coupon', '用户管理会员等级');?>
                                        </el-button>
                                        <?= \Yii::t('mall/coupon', '中设置了会员');?>
                                    </div>
                                </div>
                            </template>
                        </el-form-item>
            ...
            <el-button class="button-item" type="primary" size="small" :loading=submitLoading @click="onSubmit"><?= \Yii::t('mall/coupon', '保存');?>
            </el-button>
        </el-form>
        <!--指定商品-->
        <el-dialog title="<?= \Yii::t('mall/coupon', '已指定商品');?>" :visible.sync="goodsListVisible">
            <el-dialog title="<?= \Yii::t('mall/coupon', '添加商品');?>" :visible.sync="goodsAddVisible" append-to-body>
                <el-form @submit.native.prevent>
                    <el-form-item>
                        <el-input size="small" v-model="keyword" autocomplete="off" placeholder="<?= \Yii::t('mall/coupon', '商品名称');?>"
                                  style="width: 40%"></el-input>
                        <el-button size="small" :loading="btnLoading"
                                   @click="search('mall/coupon/search-goods','goods')"><?= \Yii::t('mall/coupon', '查找商品');?>
                        </el-button>
                        <el-button size="small" style="float:right" @click="goodsBatchAdd"><?= \Yii::t('mall/coupon', '批量添加');?></el-button>
                    </el-form-item>
                </el-form>
                <el-table v-loading="btnLoading" :data="goods_list" @selection-change="selsChange" style="width: 100%"  height="500">
                    ...
                    <el-table-column prop="cover_pic" label="<?= \Yii::t('mall/coupon', '商品图');?>" width="180">
                        <template slot-scope="scope">
                            <app-image mode="aspectFill" :src="scope.row.cover_pic"></app-image>
                        </template>
                    </el-table-column>
                    <el-table-column prop="name" label="<?= \Yii::t('mall/coupon', '商品名称');?>">
                        <template slot-scope="scope">
                            <app-ellipsis :line="2">{{scope.row.name}}</app-ellipsis>
                        </template>
                    </el-table-column>
                    <el-table-column label="<?= \Yii::t('mall/coupon', '操作');?>">
                        <template slot-scope="scope">
                            <el-button v-if="form.goods_id_list.indexOf(scope.row.id) == -1" plain size="mini"
                                       type="primary" @click="goodsAdd(scope.$index, scope.row.id)"><?= \Yii::t('mall/coupon', '添加');?>
                            </el-button>
                            <el-button v-else plain size="mini" type="primary" disabled><?= \Yii::t('mall/coupon', '已添加');?></el-button>
                        </template>
                    </el-table-column>
                </el-table>
                <div slot="footer" class="dialog-footer">
            ...
            <el-button style="float: right" type="primary" size="small" @click="goodsAddVisible = true"><?= \Yii::t('mall/coupon', '新增');?></el-button>
            <el-table :data="form.goodsWarehouse" style="width: 100%" del-max-height="99999px">
                <el-table-column prop="id" label="ID" width="80"></el-table-column>
                <el-table-column prop="cover_pic" label="<?= \Yii::t('mall/coupon', '商品图');?>" width="80">
                    <template slot-scope="scope">
                        <app-image mode="aspectFill" :src="scope.row.cover_pic"></app-image>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="<?= \Yii::t('mall/coupon', '操作');?>" width="220">
                    <template slot-scope="scope">
                        <el-button plain size="mini" type="danger" @click="goodsDestroy(scope.$index)"><?= \Yii::t('mall/coupon', '删除');?></el-button>
                    </template>
                </el-table-column>
            </el-table>
        </el-dialog>
        <!--指定商品分类-->
        <el-dialog title="<?= \Yii::t('mall/coupon', '已指定商品类别');?>" :visible.sync="catsListVisible" width="35%">
            <el-tree
            ...
            ></el-tree>
            <div slot="footer" class="dialog-footer">
                <el-button plain type="submit" @click="catsListVisible = false"><?= \Yii::t('mall/coupon', '取消');?></el-button>
                <el-button plain type="primary" @click="catsAdd"><?= \Yii::t('mall/coupon', '确定');?></el-button>
            </div>
        </el-dialog>
    </el-card>
</div>
    ...
    const app = new Vue({
        ...
        data() {
            var validDate = (rule, value, callback) => {
                if (this.form.expire_type == 2 && (!this.time || !this.time[0] || !this.time[1])) {
                    callback(new Error("<?= \Yii::t('mall/coupon', '优惠券有效期不能为空');?>"))
                } else {
                    callback()
                }
            };
            var checkAge = (rule, value, callback) => {
                if (!value) {
                    return callback(new Error('<?= \Yii::t('mall/coupon', '优惠金额不能为空');?>'));
                }
                setTimeout(() => {
                    if (value < 0.01) {
                        callback(new Error('<?= \Yii::t('mall/coupon', '优惠金额需大于零');?>'));
                    } else {
                        callback();
                    }
                }, 0);
            ...
            let check_can_receive_count = (rule, value, callback) => {
                if (this.receive_count_type === 1 && value === '') {
                    callback(new Error('<?= \Yii::t('mall/coupon', '每人限领次数不能为空');?>'));
                } else {
                    callback();
                }
            };
            let discountData = (rule, value, callback) => {
                ...
                if (this.form.type == 1 && value === '') {
                    callback(new Error('<?= \Yii::t('mall/coupon', '优惠券折扣不能为空');?>'));
                } else {
                    callback();
                }
            }
            ...
            return {
                ...
                FormRules: {
                    name: [
                        {required: true, message: '<?= \Yii::t('mall/coupon', '优惠券名称不能为空');?>', trigger: 'blur'},
                        {min: 1, max: 30, message: "<?= \Yii::t('mall/coupon', '优惠券名称长度在1');?>"},
                    ],
                    type: [
                        {required: true, message: '<?= \Yii::t('mall/coupon', '优惠券类型不能为空');?>', trigger: 'blur'}
                    ],
                    min_price: [
                        {required: true, message: '<?= \Yii::t('mall/coupon', '最低消费金额不能为空');?>', trigger: 'blur'}
                    ],
                    sub_price: [
                        {validator: checkAge, trigger: 'blur'},
                    ],
                    expire_day: [
                        {required: true, message: '<?= \Yii::t('mall/coupon', '优惠券有效天数不能为空');?>', trigger: 'blur'}
                    ],
                    time: [
                    ...
                    total_count: [
                        {required: true, message: '<?= \Yii::t('mall/coupon', '优惠券库存不能为空');?>', trigger: 'change'}
                    ],
                    can_receive_count: [
                        {type: 'number', required: true, validator: check_can_receive_count, trigger: 'change'}
                    ],
                    sort: [
                        {required: true, pattern: /^[0-9]\d{0,8}$/, message: '<?= \Yii::t('mall/coupon', '排序必须在9位整数内');?>'}
                    ],
                    is_member: [
                        {required: true, message: '<?= \Yii::t('mall/coupon', '指定会员领取不能为空');?>', trigger: 'blur'}
                    ],
                    discount: [
                        {validator: discountData, required: true, message: '<?= \Yii::t('mall/coupon', '优惠折扣不能为空');?>', trigger: 'change'},
                    ]
                },
                receive: [],
        ...
        methods: {
            ...
            catsAdd() {
                ...
                this.form.cat = array;
                this.$message({
                    message: '<?= \Yii::t('mall/coupon', '添加成功');?>',
                    type: 'success'
                });
            ...
            onSubmit() {
                if (this.form.appoint_type == 1) {
                    if (this.form.cat_id_list.length == 0) {
                        this.loading = false;
                        this.$alert('<?= \Yii::t('mall/coupon', '请添加指定商品类型');?>', '<?= \Yii::t('mall/coupon', '提示');?>', {
                    ...
                    if (this.form.goods_id_list.length == 0) {
                        this.loading = false;
                        this.$alert('<?= \Yii::t('mall/coupon', '请添加指定商品');?>', '<?= \Yii::t('mall/coupon', '提示');?>', {
                            confirmButtonText: '<?= \Yii::t('mall/coupon', '确定');?>'
                        })
                        return false;
                    }
