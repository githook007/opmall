...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header" class="clearfix">
            <el-breadcrumb separator="/" style="height: 28px;line-height: 28px;display: inline-block;">
                ...
                <el-breadcrumb-item><?= \Yii::t('mall/printer', '打印设置列表');?></el-breadcrumb-item>
            </el-breadcrumb>
            <el-button style="float: right;" type="primary" size="small"
                       @click="handleEdit(0,0)"><?= \Yii::t('mall/printer', '添加打印设置');?>
            </el-button>
        </div>
        <div class="table-body">
            <el-table v-loading="loading" border :data="list" style="width: 100%;margin-bottom: 15px">
                <el-table-column width='100' prop="id" label="ID"></el-table-column>
                <el-table-column width="150" prop="printer.name" label="<?= \Yii::t('mall/printer', '打印机名称');?>"></el-table-column>
                <el-table-column label="<?= \Yii::t('mall/printer', '打印方式');?>">
                    <template slot-scope="scope">
                        <el-tooltip style="margin-right: 10px" effect="dark" v-if="scope.row.type.order == '1'"
                                    content="<?= \Yii::t('mall/printer', '下单打印');?>" placement="top">
                            <img src="statics/img/mall/order_print.png" alt="">
                        </el-tooltip>
                        <el-tooltip style="margin-right: 10px" effect="dark" v-if="scope.row.type.pay == '1'"
                                    content="<?= \Yii::t('mall/printer', '付款打印');?>" placement="top">
                            <img src="statics/img/mall/pay_print.png" alt="">
                        </el-tooltip>
                        <el-tooltip style="margin-right: 10px" effect="dark" v-if="scope.row.type.confirm == '1'"
                                    content="<?= \Yii::t('mall/printer', '确认收货打印');?>" placement="top">
                            <img src="statics/img/mall/confirm_print.png" alt="">
                        </el-tooltip>
                    </template>
                </el-table-column>
                <el-table-column prop="store_name" label="<?= \Yii::t('mall/printer', '所属门店');?>"></el-table-column>
                <el-table-column label="<?= \Yii::t('mall/printer', '是否使用规格');?>" width="120" v-if="false">
                    <template slot-scope="scope">
                        <el-switch
                                :active-value="1"
                                :inactive-value="0"
                ...
                <el-table-column label="<?= \Yii::t('mall/printer', '是否启用');?>" width="100">
                    <template slot-scope="scope">
                        <el-switch
                                :active-value="1"
                                :inactive-value="0"
                ...
                <el-table-column label="<?= \Yii::t('mall/printer', '操作');?>" width="220">
                    <template slot-scope="scope">
                        <el-button type="text" @click="handleEdit(scope.$index, scope.row,list.id)" size="small" circle>
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/printer', '编辑');?>" placement="top">
                                <img src="statics/img/mall/edit.png" alt="">
                            </el-tooltip>
                        </el-button>
                        <el-button type="text" @click="handleDel(scope.$index, scope.row,list.id)" size="small" circle>
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('mall/printer', '删除');?>" placement="top">
            ...
            <div flex="box:last cross:center">
                <div style="visibility: hidden">
                    <el-button plain type="primary" size="small"><?= \Yii::t('mall/printer', '批量操作');?>1</el-button>
                    <el-button plain type="primary" size="small"><?= \Yii::t('mall/printer', '批量操作');?>2</el-button>
                </div>
                <div>
                    <el-pagination
                            v-if="pagination"
    ...
    <el-dialog title="<?= \Yii::t('mall/printer', '打印设置');?>" :visible.sync="dialog" width="35%">
        <el-form v-loading="load" :model="form" label-width="10rem" label-position="right">
            <el-form-item label="<?= \Yii::t('mall/printer', '选择打印机');?>" prop="select">
            ...
            <el-form-item hidden label="<?= \Yii::t('mall/printer', '是否打印规格');?>" prop="is_attr">
                <el-switch v-if="!load"
                           v-if="dialog"
            ...
            <el-form-item label="<?= \Yii::t('mall/printer', '打印参数');?>" prop="show_type">
                <el-checkbox-group v-if="!load" v-model="show_type">
                    <el-checkbox label="attr"><?= \Yii::t('mall/printer', '规格显示');?></el-checkbox>
                    <el-checkbox label="goods_no"><?= \Yii::t('mall/printer', '货号显示');?></el-checkbox>
                    <el-checkbox label="form_data"><?= \Yii::t('mall/printer', '下单表单显示');?></el-checkbox>
                </el-checkbox-group>
            </el-form-item>
            <el-form-item label="<?= \Yii::t('mall/printer', '订单发货方式');?>" prop="order_send_type">
                <el-checkbox-group v-if="!load" v-model="order_send_type">
                    <el-checkbox label="express"><?= \Yii::t('mall/printer', '快递配送');?></el-checkbox>
                    <el-checkbox label="offline"><?= \Yii::t('mall/printer', '到店自提');?></el-checkbox>
                    <el-checkbox label="city"><?= \Yii::t('mall/printer', '同城配送');?></el-checkbox>
                    <el-checkbox label="auto"><?= \Yii::t('mall/printer', '自动发货');?></el-checkbox>
                </el-checkbox-group>
            </el-form-item>
            <el-form-item prop="select" v-if="order_send_type.includes('offline')">
                <template slot="label">
                    <span><?= \Yii::t('mall/printer', '选择门店');?></span>
                    <el-tooltip effect="dark" placement="top"
                                content="<?= \Yii::t('mall/printer', '选择了门店表示该打印设置只打印该门店的到店自提订单');?>">
            ...
            <el-form-item label="<?= \Yii::t('mall/printer', '订单打印方式');?>" prop="type">
                <el-checkbox-group v-if="!load" v-model="type">
                    <el-checkbox label="order"><?= \Yii::t('mall/printer', '下单打印');?></el-checkbox>
                    <el-checkbox label="pay"><?= \Yii::t('mall/printer', '付款打印');?></el-checkbox>
                    <el-checkbox label="confirm"><?= \Yii::t('mall/printer', '确认收货打印');?></el-checkbox>
                </el-checkbox-group>
            </el-form-item>
            <el-form-item prop="big">
                <template slot="label">
                    <span><?= \Yii::t('mall/printer', '显示大小');?></span>
                    <el-tooltip effect="dark" placement="top" content="<?= \Yii::t('mall/printer', '收货信息或门店');?>">
                        <i class="el-icon-info"></i>
                    </el-tooltip>
                </template>
                <el-radio-group v-model="form.big">
                    <el-radio :label="0"><?= \Yii::t('mall/printer', '一倍');?></el-radio>
                    <el-radio :label="1"><?= \Yii::t('mall/printer', '两倍');?></el-radio>
                    <el-radio :label="2"><?= \Yii::t('mall/printer', '三倍');?></el-radio>
                </el-radio-group>
            </el-form-item>
            <el-form-item label="<?= \Yii::t('mall/printer', '是否启用');?>" prop="status">
                <el-switch v-if="!load"
                           v-if="dialog"
                           v-model="form.status"
                           :active-value="1"
            ...
            <el-form-item style="margin-bottom: 0">
                <el-button type="primary" size="small" style="float: right;width: 80px;margin-right: 20px;"
                           :loading="submitLoading" @click="onSubmit"><?= \Yii::t('mall/printer', '提交');?>
                </el-button>
            </el-form-item>
        </el-form>
    </el-dialog>
        ...
        methods: {
                ...
                request({
                    ...
                    if (e.data.code == 0) {
                        ...
                        if (this.form.order_send_type === null) {
                            this.order_send_type = ['express', 'city', 'offline', 'auto'];
                        } else {
            ...
            onSubmit() {
                ...
                this.form.order_send_type = {
                    ...
                    auto: this.order_send_type.indexOf('auto') !== -1 ? 1 : 0,
                }
            ...
            handleDel: function(row, column) {
                this.$confirm('<?= \Yii::t('mall/printer', '确认删除该记录吗');?>?', '<?= \Yii::t('mall/printer', '提示');?>', {
                    type: 'warning'
                }).then(() => {
                    ...
                    request({
                        ...
                        if (e.data.code === 0) {
                            ...
                            this.$message({
                                message: '<?= \Yii::t('mall/printer', '删除成功');?>',
                                type: 'success'
                            });
                            setTimeout(function(){
                                location.reload();
                    ...
                    this.$message.info('<?= \Yii::t('mall/printer', '已取消删除');?>')
                });
            }
        },
        created() {
            ...
            loadList('mall/printer/setting').then(e => {
                this.$message({
                    message: '<?= \Yii::t('mall/printer', '请求成功');?>',
                    type: 'success'
                });
                this.loading = false;
                this.list = e.list;
