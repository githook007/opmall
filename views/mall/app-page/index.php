...
<div id="app" v-cloak>
                ...
                <div style="display: flex;flex-wrap: wrap">
                    <div v-for="(item,itemIndex) in list" class="item">
                        ...
                        <el-form @submit.native.prevent label-position="left" label-width="90px">
                            <el-form-item v-if="item.params" v-for="(param, index) in item.params" :key="index" :label="`<?= \Yii::t('mall/app_page', '参数');?>` + (index + 1)">
                                <el-input size="small" v-model="param.value" :placeholder="param.desc"></el-input>
                            </el-form-item>
                            <el-form-item label="<?= \Yii::t('mall/app_page', '小程序路径');?>">
                                ...
                                <div class="app-icon">
                                    <el-tooltip effect="dark" content="<?= \Yii::t('mall/app_page', '复制链接');?>" placement="top">
                                        <img class="copy-btn" src="statics/img/mall/copy.png" alt=""
                                             data-clipboard-action="copy" @click="copyMethod(itemIndex, 0)">
                                    </el-tooltip>
                                    <el-tooltip effect="dark" content="<?= \Yii::t('mall/app_page', '二维码');?>" placement="top">
                                        <img src="statics/img/mall/qr.png" @click="qrcode(item, 'all')" alt="">
                                    </el-tooltip>
                                </div>
                            </el-form-item>
                            <el-form-item v-if="wechat" label="<?= \Yii::t('mall/app_page', '公众号路径');?>">
                                <div style="width: 75%;word-break: break-all;display: none;">
                            ...
                            <el-form-item v-if="mobile" label="<?= \Yii::t('mall/app_page', 'H5路径');?>">
                                <div style="width: 75%;word-break: break-all;display: none;">
                            ...
                            <el-form-item v-if="wechat" label="<?= \Yii::t('mall/app_page', '公众号跳转小程序路径');?>" label-width="180px">
                                <div style="width: 75%;word-break: break-all;display: none;">
    ...
    <el-dialog class="showqr" :visible.sync="showqr" width="20%" center>
        ...
        <span slot="footer" class="dialog-footer">
            <el-button type="primary" style="margin-bottom: 10px;" size="small" @click="down"><?= \Yii::t('mall/app_page', '保存二维码图片');?></el-button>
        </span>
    </el-dialog>
</div>
        ...
        methods: {
            copyMethod(i,type) {
                ...
                clipboard.on('success', function (e) {
                    self.$message({
                        message: '<?= \Yii::t('mall/app_page', '复制成功');?>',
                        type: 'success'
                    });
                    e.clearSelection();
                    clipboard.destroy()
                ...
                clipboard.on('error', function (e) {
                    self.$message.error('<?= \Yii::t('mall/app_page', '复制失败');?>')
                    clipboard.destroy()
                });
            },
            down() {
            ...
            getTabList() {
                ...
                tabList.forEach((item) => {
                    if (item === 'base') {
                        newTabList.push({
                            name: '<?= \Yii::t('mall/app_page', '基础页面');?>',
                            value: item
                        })
                    }
                    if (item === 'marketing') {
                        newTabList.push({
                            name: '<?= \Yii::t('mall/app_page', '营销页面');?>',
                            value: item
                        })
                    }
                    if (item === 'order') {
                        newTabList.push({
                            name: '<?= \Yii::t('mall/app_page', '订单页面');?>',
                            value: item
                        })
                    }
                    if (item === 'diy') {
                        newTabList.push({
                            name: '<?= \Yii::t('mall/app_page', 'diy页面');?>',
                            value: item
                        })
                    }
                    if (item === 'plugin') {
                        newTabList.push({
                            name: '<?= \Yii::t('mall/app_page', '插件页面');?>',
                            value: item
                        })
                    }
                })
