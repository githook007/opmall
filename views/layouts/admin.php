...
if ($indSetting && !empty($indSetting['name'])) {
    ...
    $siteName = \Yii::t('layouts/layouts', '商城管理');
}
?>
<?php $this->beginPage(); ?>
<!DOCTYPE html>
...
<el-container>
    <div id="_header">
                ...
                <div>
                    <el-dropdown @command="handleCommand" style="margin-right: 20px;">
                        <span class="el-dropdown-link">
                            <span style="font-size: 16px;" id="language"><?= \Yii::t('common', '语言');?></span>
                            <i class="el-icon-arrow-down el-icon--right"></i></span>
                        <el-dropdown-menu slot="dropdown">
                            <el-dropdown-item command="cn" style="text-align: center"><?= \Yii::t('common', '中文');?></el-dropdown-item>
                            <el-dropdown-item command="en" style="text-align: center"><?= \Yii::t('common', '英语');?></el-dropdown-item>
                        </el-dropdown-menu>
                    </el-dropdown>
                    <el-dropdown @command="handleCommand">
                        ...
                        <el-dropdown-menu slot="dropdown">
                            <el-dropdown-item
                                ...
                                <?= \Yii::t('admin/common', '修改密码');?>
                            </el-dropdown-item>
                            <el-dropdown-item command="logout" style="text-align: center"><?= \Yii::t('admin/common', '注销');?></el-dropdown-item>
                        </el-dropdown-menu>
                    </el-dropdown>
                </div>
            </div>
            <div class="main-container" style="font-size: 20px;margin-top: 35px;padding-left: 10px;"><?= \Yii::t('admin/common', '管理中心');?></div>
        ...
        <!--    超级管理员 修复密码-->
        <el-dialog v-cloak title="<?= \Yii::t('admin/common', '修改密码');?>" :close-on-click-modal="false" :visible.sync="dialogFormVisible" width="35%">
            <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm">
                <el-form-item label="<?= \Yii::t('admin/common', '密码');?>" prop="pass">
                    <el-input type="password" v-model="ruleForm.pass" autocomplete="off"></el-input>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('admin/common', '确认密码');?>" prop="checkPass">
                    <el-input type="password" v-model="ruleForm.checkPass" autocomplete="off"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer">
                <el-button @click="dialogFormVisible = false"><?= \Yii::t('admin/common', '取消');?></el-button>
                <el-button :loading="btnLoading" type="primary" @click="updatePasswordSubmit('ruleForm')"><?= \Yii::t('admin/common', '确定');?>
                </el-button>
            </div>
        </el-dialog>
        ...
        <div class="expired-box" v-cloak>
            <el-dialog
                    title="<?= \Yii::t('admin/common', '即将到期提醒');?>"
                ...
                <div>
                    ...
                    <div class="message-hint">
                        <div><?= \Yii::t('admin/common', '您的账户将在');?> {{expiredData.expired_day}} <?= \Yii::t('admin/common', '天后到期');?></div>
                        <div><?= \Yii::t('admin/common', '到期时间');?>{{expiredData.expired_at}}</div>
                    </div>
                </div>
                <span slot="footer" class="dialog-footer">
                <el-button type="primary" size="small" @click="handleCloseExpired"><?= \Yii::t('admin/common', '我知道了');?></el-button>
              </span>
            </el-dialog>
        </div>
    </div>
    ...
    <footer class="el-footer">
        ...
        <?php else : ?>
            &copy;2021 <a href="https://www.opmall.com" target="_blank"><?= \Yii::t('admin/common', 'hook007');?></a>
        <?php endif; ?>
    </footer>
</el-container>
<script>
    ...
    new Vue({
        ...
        data() {
            var validatePass = (rule, value, callback) => {
                if (value === '') {
                    callback(new Error("<?= \Yii::t('admin/common', '请输入密码');?>"));
                } else {
            ...
            var validatePass2 = (rule, value, callback) => {
                if (value === '') {
                    callback(new Error("<?= \Yii::t('admin/common', '请再次输入密码');?>"));
                } else if (value !== this.ruleForm.pass) {
                    callback(new Error("<?= \Yii::t('admin/common', '两次输入密码不一致');?>"));
                } else {
                    callback();
                }
            };
