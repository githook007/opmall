    ...
    <el-card class="box-card" shadow="never">
        <div>
            <div class="input-item">
                <el-input @keyup.enter.native="getList" size="small" placeholder="<?= \Yii::t('admin/user', '请输入账号');?>" v-model="keyword" clearable @clear='getList'>
            ...
            <el-table
                ...
                <el-table-column label="<?= \Yii::t('admin/user', '申请帐号');?>" prop="username" width="150">
                </el-table-column>
                <el-table-column label="<?= \Yii::t('admin/user', '申请人信息');?>" prop="name" width="220">
                    <template slot-scope="scope">
                        <div>{{scope.row.name}}</div>
                        <div>{{scope.row.mobile}}</div>
                        <div><?= \Yii::t('admin/user', '微信号');?>: {{scope.row.wechat_id}}</div>
                    </template>
                </el-table-column>
                <el-table-column
                        label="<?= \Yii::t('admin/user', '申请时间');?>"
                        prop="created_at" width="90">
                </el-table-column>
                <el-table-column label="<?= \Yii::t('admin/user', '申请理由');?>" prop="remark">
                ...
                <el-table-column label="<?= \Yii::t('admin/user', '证件信息');?>" prop="remark" width="95">
                    <template slot-scope="props">
                        <el-button type="text" size="mini" @click="showCardDialog(props.row)"><?= \Yii::t('admin/user', '点击查看');?></el-button>
                    </template>
                </el-table-column>
                <el-table-column label="<?= \Yii::t('admin/user', '操作');?>" width="150">
                    <template slot-scope="scope">
                        <template v-if="scope.row.status == 0">
                            <el-button @click="audit(scope.row.id, 1)" plain size="mini" type="info"><?= \Yii::t('admin/user', '通过');?></el-button>
                            <el-button @click="audit(scope.row.id, 2)" plain size="mini" type="info"><?= \Yii::t('admin/user', '拒绝');?></el-button>
                        </template>
                        <el-button disabled v-if="scope.row.status == 1" plain type="info"
                                   size="mini"><?= \Yii::t('admin/user', '已通过');?>
                        </el-button>
                        <el-button disabled v-if="scope.row.status == 2" plain type="info"
                                   size="mini"><?= \Yii::t('admin/user', '已拒绝');?>
                        </el-button>
                        <el-button v-if="scope.row.status != 0" @click="destroy(scope.row.id)" plain type="info"
                                   size="mini"><?= \Yii::t('admin/user', '删除');?>
                        </el-button>
                    </template>
                </el-table-column>
            </el-table>
...
<script>
    const app = new Vue({
        ...
        methods: {
            ...
            audit(id, status) {
                let self = this;
                let text = status == 1 ? "<?= \Yii::t('admin/user', '审核通过');?>" : "<?= \Yii::t('admin/user', '审核拒绝');?>";
                self.$confirm(text + ", <?= \Yii::t('admin/user', '是否继续');?> ?", "<?= \Yii::t('admin/user', '提示');?>", {
            ...
            destroy(id) {
                let self = this;
                self.$confirm("<?= \Yii::t('admin/user', '删除该条记录');?>", "<?= \Yii::t('admin/user', '提示');?>", {
                    confirmButtonText: "<?= \Yii::t('admin/user', '确定');?>",
                    cancelButtonText: "<?= \Yii::t('admin/user', '取消');?>",
                    type: 'warning'
                }).then(() => {
