...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 0;">
        <div class="table-body">
            <div>
                <el-select @change="search" style="width: 120px;" size="small" v-model="type" placeholder="<?= \Yii::t('admin/user', '请选择');?>">
                    <el-option
                ...
                <div class="input-item">
                    <el-input @keyup.enter.native="search" size="small" placeholder="<?= \Yii::t('admin/user', '请输入用户名');?>" clearable v-model="keyword" clearable @clear='getList'>
            ...
            <el-table
                ...
                <el-table-column
                        prop=""
                        label="<?= \Yii::t('admin/user', '已创建小程序数量');?>">
                    <template slot-scope="scope">
                        <span v-if="scope.row.adminInfo.app_max_count == -1"><?= \Yii::t('admin/user', '无限制');?></span>
                ...
                <el-table-column
                        width="240"
                        label="<?= \Yii::t('admin/user', '有效期');?>">
                    <template slot-scope="scope">
                        <el-tag v-if="scope.row.expired_type == '未到期'" type="success"><?= \Yii::t('admin/user', '未到期');?></el-tag>
                        <el-tag v-if="scope.row.expired_type == '已到期'" type="warning"><?= \Yii::t('admin/user', '已到期');?></el-tag>
                        <span>{{scope.row.adminInfo.expired_at == '0000-00-00 00:00:00' ? "<?= \Yii::t('admin/user', '永久');?>" : scope.row.adminInfo.expired_at}}</span>
                    </template>
                </el-table-column>
                <el-table-column
                        prop="created_at"
                        width="180"
                        label="<?= \Yii::t('admin/user', '创建日期');?>">
                </el-table-column>
                <el-table-column
                        fixed="right"
                        label="<?= \Yii::t('admin/user', '操作');?>"
                        width="220">
                    <template slot-scope="scope">
                        <div class="action-box">
                            <el-button @click="edit(scope.row.id)" type="text" circle size="mini">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('admin/user', '编辑');?>" placement="top">
                                    <img src="statics/img/mall/edit.png" alt="">
                                </el-tooltip>
                            </el-button>
                            <el-button @click="editPassword(scope.row.id)" type="text" circle size="mini">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('admin/user', '修改密码');?>" placement="top">
                                    <img src="statics/img/mall/change.png" alt="">
                                </el-tooltip>
                            </el-button>
                            <el-button @click="destroy(scope.row.id)" type="text" circle size="mini">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('admin/user', '删除');?>" placement="top">
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                pageCount: 0,
                keyword: '',
                type: "<?= \Yii::t('admin/user', '全部');?>",

                multipleSelection: [],

                options: [{
                    value: "<?= \Yii::t('admin/user', '全部');?>",
                    label: "<?= \Yii::t('admin/user', '全部');?>"
                }, {
                    value: "未到期",
                    label: "<?= \Yii::t('admin/user', '未到期');?>"
                },{
                    value: "已到期",
                    label: "<?= \Yii::t('admin/user', '已到期');?>"
                }],
                isPageShow: false
            };
        },
        methods: {
            ...
            destroy(id) {
                let self = this;
                self.$confirm("<?= \Yii::t('admin/user', '删除该用户');?>", "<?= \Yii::t('admin/user', '提示');?>", {
                    ...
                    self.$message.info("<?= \Yii::t('admin/user', '已取消删除');?>")
                });
            },
            editPassword(id) {
                let self = this;
                self.$prompt("<?= \Yii::t('admin/user', '请输入新密码');?>", "<?= \Yii::t('admin/user', '提示');?>", {
                    confirmButtonText: "<?= \Yii::t('admin/user', '确定');?>",
                    cancelButtonText: "<?= \Yii::t('admin/user', '取消');?>",
                    inputPattern: /\S+/,
                    inputErrorMessage: "<?= \Yii::t('admin/user', '请输入新密码');?>",
                    inputType: 'password',
                }).then(({value}) => {
                    request({
                        params: {
