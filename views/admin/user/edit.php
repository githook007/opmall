...
<div id="app" v-cloak>
    <el-card v-loading="cardLoading" class="box-card">
        <div slot="header" class="clearfix">
            <span v-if="isDisabled"><?= \Yii::t('admin/user', '编辑子账户信息');?></span>
            <span v-else><?= \Yii::t('admin/user', '新增子账户信息');?></span>
            <span style="float: right;">
                <?= \Yii::t('admin/user', '当前子账户数量');?>：{{cloudAccount.user_num}},
                <?= \Yii::t('admin/user', '最大子账户数量');?>：{{cloudAccount.account_num == -1 ? "<?= \Yii::t('admin/user', '无限制');?>" : cloudAccount.account_num}}
            </span>
        </div>
        <div class="form">
            <el-form ref="form" label-position="left" :model="form" :rules="rules" label-width="120px" size="small">
                <el-form-item label="<?= \Yii::t('admin/user', '用户名');?>" prop="username">
                    <el-input :disabled="isDisabled" class="common-width" v-model="form.username"></el-input>
                </el-form-item>
                <el-form-item v-if="isShow" label="<?= \Yii::t('admin/user', '登录密码');?>" prop="password">
                ...
                <el-form-item label="<?= \Yii::t('admin/user', '手机号');?>" prop="mobile">
                    <el-input class="common-width" v-model="form.mobile"></el-input>
                </el-form-item>
                <el-form-item label="头像图片">
                    <el-input class="currency-width isAppend" v-model="form.adminInfo.avatar">
                        <template slot="append">
                            <app-attachment v-model="form.adminInfo.avatar">
                                <el-button>上传图片</el-button>
                            </app-attachment>
                        </template>
                    </el-input>
                    <img class="my-img" style="background-color: #100a46;border-color: #100a46; height: 36px;"
                         v-if="form.adminInfo.avatar" :src="form.adminInfo.avatar">
                    <div v-else class="preview">建议尺寸100*100</div>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('admin/user', '备注');?>">
                    <el-input type="text" class="common-width" v-model="form.adminInfo.remark"></el-input>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('admin/user', '小程序数量');?>" prop="adminInfo.app_max_count">
                    <el-input :disabled="isAppMaxCount" type="number" class="common-width"
                              v-model="form.adminInfo.app_max_count">
                    </el-input>
                    <el-checkbox v-model="isAppMaxCount" @change="appMaxCount"><?= \Yii::t('admin/user', '无限制');?></el-checkbox>
                    <div class="common-width" style="color:#BBBBBB;font-size: 11px;line-height: 1;margin-top: 5px">
                        <?= \Yii::t('admin/user', '此用户可以创建的小程序的数量');?>
                    </div>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('admin/user', '账户有效期');?>" prop="adminInfo.expired_at" ref="expired_at">
                    <el-date-picker :disabled='isExpiredDisabled' class="common-width"
                                    type="datetime"
                                    value-format="yyyy-MM-dd HH:mm:ss"
                                    placeholder="<?= \Yii::t('admin/user', '选择日期');?>"
                                    v-model="form.adminInfo.expired_at">
                    </el-date-picker>
                    <el-checkbox v-model="isCheckExpired" @change="checkExpiredAt"><?= \Yii::t('admin/user', '永久');?></el-checkbox>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('admin/user', '权限设置');?>">
                    <div class="permissions-item" @click="clickAll" :class="{active:allIsCheck}"><?= \Yii::t('admin/user', '全选');?></div>
                ...
                <el-form-item label="<?= \Yii::t('admin/user', '上传权限');?>" v-if="storageShow()">
                    <div class="permissions-list">
                ...
                <el-form-item label="<?= \Yii::t('admin/user', '模板权限');?>" v-if="templateShow()">
                    <el-form-item label="<?= \Yii::t('admin/user', '显示权限');?>">
                        <div flex>
                            <el-button size="mini" type="text" style="margin-right: 10px;" @click="show = true"
                                       v-if="form.adminInfo.secondary_permissions.template.is_all == 0"><?= \Yii::t('admin/user', '添加模板');?></el-button>
                            <el-checkbox v-model="form.adminInfo.secondary_permissions.template.is_all"
                                         true-label="1" false-label="0"><?= \Yii::t('admin/user', '全选');?></el-checkbox>
                        </div>
                    ...
                    <el-form-item label="<?= \Yii::t('admin/user', '使用权限');?>">
                        <div flex>
                            <el-button size="mini" type="text" style="margin-right: 10px;" @click="show_use = true"
                                       v-if="form.adminInfo.secondary_permissions.template.use_all == 0"><?= \Yii::t('admin/user', '添加模板');?></el-button>
                ...
                <el-form-item>
                    <el-button :loading="btnLoading" class="submit-btn" type="primary" @click="store('form')"><?= \Yii::t('admin/user', '保存');?>
                    </el-button>
                </el-form-item>
            </el-form>
        </div>
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                rules: {
                    username: [
                        {required: true, message: "<?= \Yii::t('admin/user', '请输入用户名');?>", trigger: 'change'},
                        {min: 4, max: 20, message: "<?= \Yii::t('admin/user', '长度在');?>", trigger: 'change'}
                    ],
                    password: [
                        {required: true, message: "<?= \Yii::t('admin/user', '请输入密码');?>", trigger: 'change'},
                    ],
                    mobile: [
                        {required: true, message: "<?= \Yii::t('admin/user', '请输入手机号');?>", trigger: 'change'},
                    ],
                    'adminInfo.expired_at': [
                        {required: true, message: "<?= \Yii::t('admin/user', '请选择账户有效期');?>", trigger: 'change'},
                    ],
                    'adminInfo.app_max_count': [
                        {required: true, message: '<?= \Yii::t('admin/user', '请填写可创建小程序数量');?>', trigger: 'change'},
                    ]
                },
                allIsCheck: false,
                show: false,
