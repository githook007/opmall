...
<div id="app" v-cloak>
    <el-card shadow="never" style="margin-bottom: 20px;">
        <div style="padding: 20px 0;"><?= \Yii::t('admin/app_manage', '分类设置');?></div>
        <div flex="box:first" class="cat-list">
            <div class="label-name"><?= \Yii::t('admin/app_manage', '分类设置');?></div>
            <div>
                <div flex="box:last cross:center" class="cat-item">
                    <div></div>
                    <el-button @click="handleAddCat" size="mini"><?= \Yii::t('admin/app_manage', '添加');?></el-button>
                </div>
                        ...
                        <div>
                            <div v-if="cat.edit" style="padding: 4px 0;">
                                <el-input v-model="cat.edit.display_name"
                                          ...
                                          placeholder="<?= \Yii::t('admin/app_manage', '请填写分类名称');?>"
                                          size="small"></el-input>
                            ...
                            <div flex="cross:center">
                                <div style="margin-right: 20px"><?= \Yii::t('admin/app_manage', '图标背景色');?></div>
        ...
        <div style="padding: 20px 0;"><?= \Yii::t('admin/app_manage', '插件分组设置');?></div>
        <template v-if="cats && cats.length">
            <div v-if="other_plugins && other_plugins.length" flex="box:first cross:center" class="cat-group">
                <div class="label-name"><?= \Yii::t('admin/app_manage', '未分组');?></div>
        ...
        <el-button size="small" type="primary" @click="confirm" :loading="confirming"><?= \Yii::t('admin/app_manage', '保存');?></el-button>
    </el-card>
</div>
    ...
    new Vue({
        ...
        methods: {
            loadData() {
                ...
                this.$request({
                    params: {
                        r: 'admin/app-manage/cat-manager'
                    },
                }).then(e => {
                    this.loading = false;
                    if (e.data.code === 0) {
            ...
            handleDeleteCat(index) {
                this.$confirm('<?= \Yii::t('admin/app_manage', '被删除分类下的原有插件');?>', '<?= \Yii::t('admin/app_manage', '警告');?>', {
                    type: 'warning'
                }).then(e => {
            ...
            handleEditCatConfirm(index) {
                ...
                if (typeof displayName === 'undefined' || displayName === null || displayName === '' || displayName === false) {
                    this.$message.error('<?= \Yii::t('admin/app_manage', '分类名称不能为空');?>');
                    return;
                }
                if (displayName.length > 6) {
                    this.$message.error('<?= \Yii::t('admin/app_manage', '分类名称不能超过6个字');?>');
                    return;
                }
            ...
            confirm() {
                ...
                for (let i in this.cats) {
                    ...
                    if (cat.display_name === '') {
                        ...
                        this.$message.error('<?= \Yii::t('admin/app_manage', '插件分类名称未填写');?>');
                        return;
                    }
                    let plugins = [];
                ...
                this.$request({
                    ...
                    this.confirming = false;
                    if (e.data.code === 0) {
                        this.$message.success('<?= \Yii::t('admin/app_manage', '保存成功');?>');
                    } else {
