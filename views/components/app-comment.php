...
<template id="app-comment">
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <span><?= \Yii::t('components/other', '评价管理');?></span>
            <el-button style="float: right; margin: -5px 0" type="primary" size="small" @click="$navigate({r: edit_url})"><?= \Yii::t('components/other', '添加客户评价');?></el-button>
        </div>
        <div class="app-comment-body">
            <el-select size="small" v-model="comment_type" class="select">
                <el-option key="1" label="<?= \Yii::t('components/other', '全部评价');?>" value="0"></el-option>
                <el-option key="2" label="<?= \Yii::t('components/other', '好评');?>" value="1"></el-option>
                <el-option key="3" label="<?= \Yii::t('components/other', '中评');?>" value="2"></el-option>
                <el-option key="4" label="<?= \Yii::t('components/other', '差评');?>" value="3"></el-option>
            </el-select>
            <div class="input-item">
                <el-input size="small" placeholder="<?= \Yii::t('components/other', '请输入搜索内容');?>" v-model="keyword">
                    <el-button slot="append" icon="el-icon-search" @click="search"></el-button>
                </el-input>
            </div>
            <el-table :data="form" border style="width: 100%" v-loading="listLoading">
                <el-table-column prop="id" label="ID" width="80"></el-table-column>
                <el-table-column prop="nickname" label="<?= \Yii::t('components/other', '用户');?>" width="180"></el-table-column>
                <el-table-column prop="platform" align="center" label="<?= \Yii::t('components/other', '平台');?>" width="80">
                    <template slot-scope="scope">
                        <el-tooltip class="item" effect="dark" v-if="scope.row.platform == 'wxapp'" content="<?= \Yii::t('components/other', '微信');?>" placement="top">
                            <img src="statics/img/mall/wx.png" alt="">
                        </el-tooltip>
                        <el-tooltip class="item" effect="dark" v-else-if="scope.row.platform == 'aliapp'" content="<?= \Yii::t('components/other', '支付宝');?>" placement="top">
                            <img src="statics/img/mall/ali.png" alt="">
                        </el-tooltip>
                        <el-tooltip class="item" effect="dark" v-else content="<?= \Yii::t('components/other', '后台');?>" placement="top">
                            <img src="statics/img/mall/site.png" alt="">
                        </el-tooltip>
                    </template>
                </el-table-column>
                <el-table-column prop="name" label="<?= \Yii::t('components/other', '商品名称');?>" width="300">
                    <template slot-scope="scope">
                ...
                <el-table-column align="center" prop="score" label="<?= \Yii::t('components/other', '评分');?>" width="80">
                    <template slot-scope="scope">
                        <el-tooltip class="item" effect="dark" v-if="scope.row.score==3" content="<?= \Yii::t('components/other', '好评');?>" placement="top">
                            <img src="statics/img/mall/good.png" alt="">
                        </el-tooltip>
                        <el-tooltip class="item" effect="dark" v-else-if="scope.row.score==2" content="<?= \Yii::t('components/other', '中评');?>" placement="top">
                            <img src="statics/img/mall/normal.png" alt="">
                        </el-tooltip>
                        <el-tooltip class="item" effect="dark" v-else-if="scope.row.score==1" content="<?= \Yii::t('components/other', '差评');?>" placement="top">
                            <img src="statics/img/mall/bad.png" alt="">
                        </el-tooltip>
                    </template>
                </el-table-column>
                <el-table-column prop="content" label="<?= \Yii::t('components/other', '详情');?>" width='300'>
                    <template slot-scope="scope">
                ...
                <el-table-column prop="reply_content" label="<?= \Yii::t('components/other', '评价回复');?>"></el-table-column>
                <el-table-column prop="is_show" label="<?= \Yii::t('components/other', '状态');?>" width="80">
                    <template slot-scope="scope">
                        <el-switch active-value="1" inactive-value="0" @change="switchShow(scope.row)" v-model="scope.row.is_show"></el-switch>
                    </template>
                </el-table-column>
                <el-table-column label="<?= \Yii::t('components/other', '操作');?>" width="180">
                    <template slot-scope="scope">
                        <el-button size="small" type="text" circle @click="reply(scope.row.id)">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('components/other', '评价回复');?>" placement="top">
    ...
    Vue.component('app-comment', {
        ...
        methods: {
            ...
            destroy: function(column) {
                this.$confirm("<?= \Yii::t('components/other', '确认删除该记录吗');?>", "<?= \Yii::t('components/other', '提示');?>", {
                    type: 'warning'
                }).then(() => {
