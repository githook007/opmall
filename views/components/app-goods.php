...
Yii::$app->loadViewComponent('diy/diy-video-channel');
?>
<style>
    .mt-24 {
        margin-bottom: 24px;
...
<template id="app-goods">
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;" class="app-goods"
        ...
        <div class='form-body'>
            ...
            <el-form :model="cForm" :rules="cRule" ref="ruleForm" label-width="180px" size="small"
                ...
                <el-tabs v-model="activeName" @tab-click="handleClick" :before-leave="leaveTab">
                    <el-tab-pane label="<?= \Yii::t('components/goods', '商品设置');?>" name="first" v-if="is_basic == 1">
                        <el-card v-if="is_ecard == 1 && is_e_card == 1 && is_mch == 0" shadow="never" class="mt-24">
                            <div slot="header">
                                <span><?= \Yii::t('components/goods', '选择商品类型');?></span>
                            </div>
                            <el-row>
                                <el-col :xl="12" :lg="16">
                                    <div flex="">
                        ...
                        <el-card v-if="ruleForm.type == 'ecard' && is_e_card == 1 && is_mch == 0 && is_ecard == 1" shadow="never" class="mt-24">
                            <div slot="header">
                                <span><?= \Yii::t('components/goods', '选择卡密');?></span>
                            </div>
                            <el-row>
                                <el-col :xl="12" :lg="16">
                                    <el-form-item label="<?= \Yii::t('components/goods', '卡密名称');?>" required>
                                        <div flex="dir:left">
                                            <div flex="dir:top" v-if="ruleForm.plugin_data.ecard.name">
                                                ...
                                                <span style="color: #ff4544;font-size: 14px;" v-if="ruleForm.plugin_data.ecard.stock < 50"><?= \Yii::t('components/goods', '库存少于50');?></span>
                                            </div>
                                            <div>
                                                <el-button :disabled="is_edit == 1" type="primary" @click="$refs.ecard.openDialog()"><?= \Yii::t('components/goods', '选择卡密');?></el-button>
                                            </div>
                                            <div style="margin-left: 10px;">
                                                <el-button type="text" @click="$navigate({r:'plugin/ecard/mall/index/edit'}, true)"><?= \Yii::t('components/goods', '添加卡密');?></el-button>
                                            </div>
                                            <app-add-ecard ref="ecard" :ecard_url="ecard_url" @select="selectCard"></app-add-ecard>
                                        </div>
                                    </el-form-item>
                                </el-col>
                            </el-row>
                        ...
                        <el-card v-if="is_cats === 1" shadow="never" class="mt-24">
                            <div slot="header">
                                <span><?= \Yii::t('components/goods', '选择分类');?></span>
                            </div>
                            <el-row>
                                <el-col :xl="12" :lg="16">
                                    <el-form-item label="<?= \Yii::t('components/goods', '商品分类');?>" prop="cats">
                                        ...
                                        <el-button type="primary" @click="$refs.cats.openDialog()"><?= \Yii::t('components/goods', '选择分类');?></el-button>
                                        <el-button type="text" @click="$navigate({r:'mall/cat/edit'}, true)"><?= \Yii::t('components/goods', '添加分类');?></el-button>
                                        <app-add-cat ref="cats" :new-cats="ruleForm.cats" @select="selectCat"></app-add-cat>
                                    </el-form-item>
                                    <!-- mch -->
                                    <el-form-item v-if="is_mch" label="<?= \Yii::t('components/goods', '多商户分类');?>" prop="mchCats">
                                        ...
                                        <el-button type="primary" @click="$refs.mchCats.openDialog()"><?= \Yii::t('components/goods', '选择分类');?></el-button>
                                        <el-button type="text" @click="$navigate({r:'mall/cat/edit'}, true)"><?= \Yii::t('components/goods', '添加分类');?></el-button>
                                        <app-add-cat ref="mchCats" :new-cats="ruleForm.mchCats" :mch_id="mch_id" @select="selectMchCat"></app-add-cat>
                                    </el-form-item>
                                </el-col>
                            </el-row>
                        </el-card>
                        ...
                        <el-card shadow="never" class="mt-24" v-if="is_product_setting === 1">
                            <div slot="header">
                                <span><?= \Yii::t('components/goods', '商品设置');?></span>
                            </div>
                            <el-row>
                                <el-col :xl="12" :lg="16">
                                    <template v-if="is_info == 1 || sign === 'exchange'">
                                        ...
                                        <el-form-item prop="number" v-if="is_copy_id === 1 && is_edit === 0">
                                            <template slot="label">
                                                <span><?= \Yii::t('components/goods', '商城商品编码');?></span>
                                                <el-tooltip effect="dark" placement="top"
                                                            content="<?= \Yii::t('components/goods', '只能从商城中获取商品信息，且基本信息与商城商品保持一致');?>">
                                                    <i class="el-icon-info"></i>
                                                </el-tooltip>
                                            </template>
                                            <el-input v-model="copyId" type="number" min="0"
                                                      oninput="this.value = this.value.replace(/[^0-9]/g, '')"
                                                      placeholder="<?= \Yii::t('components/goods', '请输入商城商品id');?>">
                                                <template slot="append">
                                                    <el-button @click="getDetail(copyId)" :loading="copyLoading"><?= \Yii::t('components/goods', '获取');?></el-button>
                                                </template>
                                            </el-input>
                                        </el-form-item>

                                        <!-- 商品名称 -->
                                        <el-form-item label="<?= \Yii::t('components/goods', '商品名称');?>" v-if="is_name === 1" prop="name" class="title-input">
                                            <el-input v-model="ruleForm.name" maxlength="100" show-word-limit></el-input>
                                        </el-form-item>

                                        <el-form-item label="<?= \Yii::t('components/goods', '商品名称');?>" class="title-input" v-else>
                                            <el-input v-model="ruleForm.name" maxlength="100" disabled show-word-limit></el-input>
                                        </el-form-item>

                                        <!-- 副标题 -->
                                        <el-form-item label="<?= \Yii::t('components/goods', '副标题');?>" v-if="is_name === 1" class="title-input">
                                            <el-input v-model="ruleForm.subtitle" maxlength="90" type="textarea" :rows="4" show-word-limit></el-input>
                                        </el-form-item>

                                        <el-form-item label="<?= \Yii::t('components/goods', '副标题');?>" class="title-input" v-else>
                                            <el-input v-model="ruleForm.subtitle" maxlength="90" type="textarea" :rows="4" disabled show-word-limit></el-input>
                                        </el-form-item>
                                        <!-- 原价 -->
                                        <el-form-item v-if="is_original_price === 1" >
                                            <template slot="label">
                                                <span><?= \Yii::t('components/goods', '原价');?></span>
                                                <el-tooltip effect="dark" content="<?= \Yii::t('components/goods', '以划线形式显示');?>" placement="top">
                                                    <i class="el-icon-info"></i>
                                                </el-tooltip>
                                            </template>

                                            <el-input type="number" min="0" disabled v-model="ruleForm.original_price">
                                                <template slot="append"><?= \Yii::t('components/goods', '元');?></template>
                                            </el-input>
                                        </el-form-item>
                                        ...
                                        <el-form-item prop="pic_url" v-if="is_pic_url === 1">
                                            <template slot="label">
                                                <span><?= \Yii::t('components/goods', '商品轮播图(多张)');?></span>
                                                <el-tooltip effect="dark" placement="top" content="<?= \Yii::t('components/goods', '第一张图片为封面图');?>">
                                                    <i class="el-icon-info"></i>
                                                </el-tooltip>
                                            </template>
                                            <div class="pic-url-remark">
                                                <?= \Yii::t('components/goods', '第一张图片为缩略图,其它图片为轮播图,建议像素750*750,可拖拽使其改变顺序，最多支持上传9张');?></div>
                                            <div flex="dir:left">
                                                    ...
                                                    <draggable v-model="ruleForm.pic_url" flex="dif:left">
                                                        <div v-for="(item,index) in ruleForm.pic_url" :key="index"
                                                            ...
                                                            <app-attachment @selected="updatePicUrl" :params="{'currentIndex': index}">
                                                                <app-image mode="aspectFill" width="100px" height='100px' :src="item.pic_url"></app-image>
                                                            </app-attachment>
                                                ...
                                                <template v-if="ruleForm.pic_url.length < 9">
                                                    <app-attachment style="margin-bottom: 10px;" :multiple="true"
                                                        ...
                                                        <el-tooltip class="item" effect="dark" content="<?= \Yii::t('components/goods', '建议尺寸');?>:750 * 750"
                                                                    placement="top">
                                                            <div flex="main:center cross:center" class="add-image-btn">
                                                                + <?= \Yii::t('components/goods', '添加图片');?></div>
                                                        </el-tooltip>
                                                    </app-attachment>
                                                </template>
                                            </div>
                                        ...
                                        <!-- 商品视频 -->
                                        <el-form-item label="<?= \Yii::t('components/goods', '商品视频');?>" v-if="is_video_url === 1" prop="video_url">
                                            <el-input v-model="ruleForm.video_url" placeholder="<?= \Yii::t('components/goods', '请输入视频原地址或选择上传视频');?>">
                                                <template slot="append">
                                                    <app-attachment :multiple="false" :max="1" @selected="videoUrl" type="video">
                                                        <el-tooltip class="item"
                                                                    effect="dark"
                                                                    content="<?= \Yii::t('components/goods', '支持格式mp4;支持编码H.264;视频大小不能超过50 MB');?>"
                                                                    placement="top">
                                                            <el-button size="mini"><?= \Yii::t('components/goods', '添加视频');?></el-button>
                                                        </el-tooltip>
                                                    </app-attachment>
                                                </template>
                                            </el-input>
                                            <el-link class="box-grow-0" type="primary" style="font-size:12px"
                                                     ...
                                                     :href="ruleForm.video_url"><?= \Yii::t('components/goods', '视频链接');?></el-link>
                                        </el-form-item>
                                    ...
                                    <template v-else>
                                        ...
                                        <!-- 商品信息获取 -->
                                        <el-form-item label="<?= \Yii::t('components/goods', '商品信息获取');?>" width="120" v-if="is_product_info === 1">
                                            <label slot="label">
                                                <?= \Yii::t('components/goods', '商品信息获取');?>
                                                <el-tooltip class="item" effect="dark"
                                                            content="<?= \Yii::t('components/goods', '只能从商城中获取商品信息，且基本信息与商城商品保持一致');?>" placement="top">
                                                    <i class="el-icon-info"></i>
                                                </el-tooltip>
                                            </label>
                                            <div>
                                                <el-row type="flex">
                                                    ...
                                                    <app-select-goods :multiple="false" submit_text="<?= \Yii::t('components/goods', '选择');?>" :sign="sign" v-if="is_edit == 0"
                                                                      @selected="selectGoodsWarehouse">
                                                        <el-button><?= \Yii::t('components/goods', '选择商品');?></el-button>
                                                    </app-select-goods>
                                                </el-row>
                                                <el-button v-if="is_edit == 0" type="text" @click="$navigate({r:'mall/goods/edit'}, true)">
                                                    <?= \Yii::t('components/goods', '商城还未添加商品？点击前往');?></el-button>
                                            </div>
                                        </el-form-item>

                                        <el-form-item label="<?= \Yii::t('components/goods', '商品名称');?>">
                                            <el-input :value="goods_warehouse.name" :disabled="true"></el-input>
                                        </el-form-item>

                                        <el-form-item label="<?= \Yii::t('components/goods', '副标题');?>" class="title-input">
                                    ...
                                    <!-- 上架状态 -->
                                    <el-form-item v-if="is_shelves_status === 1" label="<?= \Yii::t('components/goods', '上架状态');?>" prop="status">
                                    ...
                                    <el-form-item prop="sort">
                                        <template slot="label">
                                            <span><?= \Yii::t('components/goods', '排序');?></span>
                                            <el-tooltip effect="dark" content="<?= \Yii::t('components/goods', '排序值越小排序越靠前');?>" placement="top">
                                                <i class="el-icon-info"></i>
                                            </el-tooltip>
                                        </template>
                                        <el-input type="number" placeholder="<?= \Yii::t('components/goods', '请输入排序');?>" min="0"
                                                  oninput="this.value = this.value.replace(/[^0-9]/g, '');" v-model.number="ruleForm.sort">
                                        </el-input>
                                    </el-form-item>
                        ...
                        <el-card id="price" shadow="never" class="mt-24" v-if="is_price_inventory === 1">
                            <div slot="header">
                                <span><?= \Yii::t('components/goods', '价格库存');?></span>
                            </div>
                            <el-row>
                                <el-col :xl="12" :lg="16">
                                    ...
                                    <!-- 售价 -->
                                    <el-form-item :label="price_label" prop="price" v-if="no_price === 1">
                                    ...
                                    <el-form-item v-if="sign === 'exchange'" label="<?= \Yii::t('components/goods', '库存');?>" prop="goods_num">
                                        <el-input type="number"
                                                  @input="change($event)"
                                    ...
                                    <el-form-item v-if="sign !== 'exchange'" prop="original_price"  :required="sign ? false : true" >
                                        <template slot="label">
                                            <span>{{original_price_label}}</span>
                                            <el-tooltip effect="dark" content="<?= \Yii::t('components/goods', '以划线形式显示');?>" placement="top">
                                                <i class="el-icon-info"></i>
                                            </el-tooltip>
                                        </template>

                                        <el-input type="number" min="0" :disabled="is_info == 0" v-model="ruleForm.original_price">
                                            <template slot="append"><?= \Yii::t('components/goods', '元');?></template>
                                        </el-input>
                                    </el-form-item>
                                    ...
                                    <template v-if="is_info == 1">
                                        ...
                                        <!-- 单位 -->
                                        <el-form-item label="<?= \Yii::t('components/goods', '单位');?>" prop="unit">
                                            <el-input v-model="ruleForm.unit" maxlength="4" show-word-limit></el-input>
                                        </el-form-item>

                                        <!-- 成本价 -->
                                        <el-form-item :label="cost_price_label" prop="cost_price">
                                            <el-input type="number"
                                    ...
                                    <template v-if="is_attr == 1">
                                        <el-form-item label="<?= \Yii::t('components/goods', '商品总库存');?>" prop="goods_num">
                                            <el-input type="number" min="0"
                                                      placeholder="<?= \Yii::t('components/goods', '请输入商品总库存');?>"
                                                      oninput="this.value = this.value.replace(/[^0-9]/g, '');"
                                                      :disabled="ruleForm.use_attr == 1 ? true : false"
                                                      v-model="ruleForm.goods_num">
                                            </el-input>
                                        </el-form-item>
                                        <el-form-item  v-if="ruleForm.type == 'goods'" label="<?= \Yii::t('components/goods', '默认规格名');?>" prop="attr_default_name">
                                            <el-input :disabled="ruleForm.use_attr == 1"
                                                      placeholder="<?= \Yii::t('components/goods', '请输入默认规格名');?>"
                                                      v-model="ruleForm.attr_default_name">
                                            </el-input>
                                        </el-form-item>
                                        <el-form-item v-if="ruleForm.type == 'goods'">
                                            <label slot="label">
                                                <span><?= \Yii::t('components/goods', '商品规格');?></span>
                                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('components/goods', '如有颜色、尺码等多种规格，请添加商品规格');?>"
                                                            placement="top">
                                                    <i class="el-icon-info"></i>
                                                </el-tooltip>
                                            </label>
                                            <div style="width:150%">
                                                <app-attr-group @select="makeAttrGroup" v-model="attrGroups"></app-attr-group>
                                            </div>
                                        ...
                                        <el-form-item v-if="is_show == 1 && !is_mch && ruleForm.type == 'goods'" prop="is_negotiable">
                                            <template slot='label'>
                                                <span><?= \Yii::t('components/goods', '商品面议');?></span>
                                                <el-tooltip effect="dark" content="<?= \Yii::t('components/goods', '如果开启面议，则商品无法在线支付');?>" placement="top">
                                                    <i class="el-icon-info"></i>
                                                </el-tooltip>
                                            </template>
                                            <el-switch :active-value="1" :inactive-value="0" v-model="ruleForm.is_negotiable"></el-switch>
                                        </el-form-item>

                                        <!-- 商品货号 -->
                                        <el-form-item label="<?= \Yii::t('components/goods', '商品货号');?>">
                                            <el-input :disabled="ruleForm.use_attr == 1 || (sign !== 'mall') ? true : false"
                                                      placeholder="<?= \Yii::t('components/goods', '请输入商品货号');?>"
                                                      v-model="ruleForm.goods_no">
                                            </el-input>
                                        </el-form-item>

                                        <!-- 商品货号 -->
                                        <el-form-item label="<?= \Yii::t('components/goods', '条形码');?>">
                                            <el-input :disabled="ruleForm.use_attr == 1 || (sign !== 'mall') ? true : false"
                                                      placeholder="<?= \Yii::t('components/goods', '请输入条形码');?>"
                                        ...
                                        <!-- 商品重量 -->
                                        <el-form-item label="<?= \Yii::t('components/goods', '商品重量');?>" v-if="ruleForm.type == 'goods'">
                                            <el-input oninput="this.value = this.value.replace(/[^0-9]/g, '');"
                                                      placeholder="<?= \Yii::t('components/goods', '请输入商品重量');?>"
                                                      :disabled="ruleForm.use_attr == 1 || ( sign !== 'mall')  ? true : false"
                                                      v-model="ruleForm.goods_weight">
                                                <template slot="append"><?= \Yii::t('components/goods', '克');?></template>
                                            </el-input>
                                        </el-form-item>
                                    ...
                                    <!-- 规格 -->
                                    <el-form-item label="<?= \Yii::t('components/goods', '选择规格');?>" v-if="is_attr == 0 && sign !== 'exchange'" required>
                                        <el-row type="flex">
                                            <el-select v-model="select_attr_groups" @change="selectAttrGroups" placeholder="<?= \Yii::t('components/goods', '请选择');?>" v-if="is_edit == 0">
                                                <el-option
                        ...
                        <el-card shadow="never" class="mt-24" v-if="is_purchase_settings === 1">
                            <div slot="header">
                                <span><?= \Yii::t('components/goods', '购买设置');?></span>
                            </div>
                            <el-row>
                                <el-col :xl="12" :lg="16">
                                    <!-- 单商品设置发货方式 -->
                                    <el-form-item label="<?= \Yii::t('components/goods', '发货方式');?>" prop="is_setting_send_type" v-if="is_setting_send_type && ruleForm.type == 'goods'">
                                        <label slot="label">
                                            <?= \Yii::t('components/goods', '发货方式');?>
                                            <el-tooltip
                                                    class="item"
                                                    effect="dark"
                                                    content="<?= \Yii::t('components/goods', '自提需要设置门店，如果您还未设置门店请保存本页后设置门店');?>"
                                                    placement="top">
                                                <i class="el-icon-info"></i>
                                            </el-tooltip>
                                        </label>
                                        <div>
                                            <el-switch v-model="ruleForm.is_setting_send_type" :active-value="1" :inactive-value="0"></el-switch>
                                    ...
                                    <!-- 销售时间 -->
                                    <el-form-item label="<?= \Yii::t('components/goods', '销售时间');?>" prop="is_time" v-if="is_time">
                                        <div>
                                            <el-switch v-model="ruleForm.is_time" :active-value="1" :inactive-value="0"></el-switch>
                                            <div v-if="ruleForm.is_time == 1">
                                                ...
                                                <div style="color: #CCCCCC">
                                                    <?= \Yii::t('components/goods', '开启开售提醒，商品开售前5分钟会发消息提醒买家。');?><el-button v-if="is_mch == 0" type="text" @click="$navigate({r:'mall/index/goods'}, true)"><?= \Yii::t('components/goods', '去设置');?></el-button>
                                                </div>
                                            </div>
                                        </div>
                                    </el-form-item>

                                    <el-form-item v-if="cForm.buy_extra" v-for="(item, key, index) in cForm.buy_extra" :key="item.id">
                                        <label slot="label">{{item}}</label>
                                        <el-input v-model="ruleForm[key]"></el-input>
                                    </el-form-item>
                                    ...
                                    <el-form-item prop="virtual_sales" v-if="is_virtual_sales === 1">
                                        <template slot='label'>
                                            <span><?= \Yii::t('components/goods', '已出售量');?></span>
                                            <el-tooltip effect="dark" content="<?= \Yii::t('components/goods', '前端展示的销量=实际销量+已出售量');?>" placement="top">
                                                <i class="el-icon-info"></i>
                                            </el-tooltip>
                                        </template>
                                    ...
                                    <el-form-item label="<?= \Yii::t('components/goods', '浏览和购买权限');?>" prop="is_setting_show_and_buy_auth" v-if="is_show_and_buy_auth">
                                        <div class="limit-buy">
                                            <el-switch
                                            ...
                                            <el-form-item label="<?= \Yii::t('components/goods', '会员等级浏览权限');?>" prop="show_goods_auth" label-width="140px"
                                                          v-if="ruleForm.is_setting_show_and_buy_auth == 1">
                                                <app-member-auth v-model="ruleForm.show_goods_auth" :members="memberLevel"></app-member-auth>
                                            </el-form-item>
                                            <el-form-item label="<?= \Yii::t('components/goods', '会员等级购买权限');?>" prop="buy_goods_auth" label-width="140px"
                                                          v-if="ruleForm.is_setting_show_and_buy_auth == 1">
                                                <app-member-auth v-model="ruleForm.buy_goods_auth" :members="memberLevel"></app-member-auth>
                                            </el-form-item>
                                        </div>
                                    </el-form-item>
                                    ...
                                    <el-form-item label="<?= \Yii::t('components/goods', '起售');?>" prop="min_number" v-if="is_min_number">
                                        <div flex="dir:left">
                                            <span class="box-grow-0" style="color:#606266"><?= \Yii::t('components/goods', '商品至少');?></span>
                                            <div style="width: 100%;margin:0 10px">
                                                <el-input oninput="this.value = this.value.replace(/[^0-9]/g, '');"
                                                          :min="1"
                                                          :max="9999"
                                                          placeholder="<?= \Yii::t('components/goods', '请输入1～9999之间的整数');?>"
                                                          v-model="ruleForm.min_number">
                                                    <template slot="append"><?= \Yii::t('components/goods', '件');?></template>
                                                </el-input>
                                            </div>
                                            <span class="box-grow-0" style="color:#606266"><?= \Yii::t('components/goods', '起售');?></span>
                                        </div>
                                    </el-form-item>

                                    <el-form-item label="<?= \Yii::t('components/goods', '限购');?>" prop="limit_buy_status" v-if="is_limit_buy">
                                        <div class="limit-buy">
                                            <el-switch
                                            ...
                                            <el-form-item :label="sign == 'miaosha' ? '<?= \Yii::t('components/goods', '限购');?>' : '<?= \Yii::t('components/goods', '终身限购');?>'" prop="confine_count" label-width="90px"
                                                ...
                                                <div flex="dir:left">
                                                    ...
                                                    <el-checkbox style="margin-left: 5px;" @change="itemChecked"
                                                                 v-model="ruleForm.confine_count <= -1"><?= \Yii::t('components/goods', '无限制');?></el-checkbox>
                                                </div>
                                            </el-form-item>
                                            <el-form-item label="<?= \Yii::t('components/goods', '按周期限购');?>" prop="limit_buy_value" label-width="90px"
                                                ...
                                                <div flex="dir:left">
                                                            ...
                                                            <el-input oninput="this.value = this.value.replace(/[^0-9]/g, '');"
                                                                ...
                                                                <template slot="prepend">
                                                                    <el-select size="small" style="width: 120px" v-model="ruleForm.limit_buy_type" class="select">
                                                                        <el-option key="day" label="<?= \Yii::t('components/goods', '每天');?>" value="day"></el-option>
                                                                        <el-option key="week" label="<?= \Yii::t('components/goods', '每周');?>" value="week"></el-option>
                                                                        <el-option key="month" label="<?= \Yii::t('components/goods', '每月');?>" value="month"></el-option>
                                                                    </el-select>
                                                                </template>
                                                                <template slot="append"><?= \Yii::t('components/goods', '件');?></template>
                                                            </el-input>
                                                    ...
                                                    <el-checkbox style="margin-left: 5px;" @change="limitBuyValueChecked"
                                                                 v-model="ruleForm.limit_buy_value <= -1"><?= \Yii::t('components/goods', '无限制');?></el-checkbox>
                                                </div>
                                            </el-form-item>
                                        </div>
                                    </el-form-item>
                                    ...
                                    <el-form-item v-if="sign !== 'exchange'" label="<?= \Yii::t('components/goods', '商品服务标题');?>">
                                        <el-input v-model="ruleForm.guarantee_title" placeholder="<?= \Yii::t('components/goods', '请输入商品服务标题');?>">
                                        </el-input>
                                        <el-button  type="text" @click="guarantee.dialog = true;guarantee.type = 'goods_title_example'">
                                            <?= \Yii::t('components/goods', '查看图例');?>
                                        </el-button>
                                    </el-form-item>

                                    <el-form-item v-if="sign !== 'exchange'" label="<?= \Yii::t('components/goods', '商品服务标识');?>">
                                        <el-row>
                                            <el-col :span="4">
                                                <app-attachment v-model="ruleForm.guarantee_pic" :multiple="false" :max="1">
                                                    <el-tooltip class="item" effect="dark" content="<?= \Yii::t('components/goods', '建议尺寸');?>:28 * 28"
                                                                placement="top">
                                                        <el-button size="mini"><?= \Yii::t('components/goods', '选择图片');?></el-button>
                                                    </el-tooltip>
                                                </app-attachment>
                                            </el-col>
                                            <el-col :span="4">
                                                <el-button size="mini" type="primary" @click="ruleForm.guarantee_pic = guarantee.goods_pic"><?= \Yii::t('components/goods', '恢复默认');?></el-button>
                                            </el-col>
                                        </el-row>
                                        ...
                                        <el-button @click="guarantee.dialog = true;guarantee.type = 'goods_pic_example'" type="text">
                                            <?= \Yii::t('components/goods', '查看图例');?>
                                        </el-button>
                                    </el-form-item>

                                    <!-- 商品服务 -->
                                    <el-form-item v-if="sign !== 'exchange'" label="<?= \Yii::t('components/goods', '商品服务内容');?>">
                                        <template v-if="!defaultServiceChecked">
                                            ...
                                            <el-button type="button" size="mini" @click="serviceOpen"><?= \Yii::t('components/goods', '选择服务');?></el-button>
                                            <el-dialog title="<?= \Yii::t('components/goods', '选择商品服务');?>" :visible.sync="service.dialog" width="30%">
                                                ...
                                                <div slot="footer" class="dialog-footer">
                                                    <el-button @click="serviceCancel"><?= \Yii::t('components/goods', '取消');?></el-button>
                                                    <el-button type="primary" @click="serviceConfirm"><?= \Yii::t('components/goods', '确定');?></el-button>
                                                </div>
                                            </el-dialog>
                                        </template>
                                        <el-checkbox v-model="defaultServiceChecked" @change="defaultService()"><?= \Yii::t('components/goods', '默认服务');?></el-checkbox>
                                    </el-form-item>

                                    <!-- 运费设置 -->
                                    <el-form-item prop="freight_id"  v-if="ruleForm.type == 'goods' && is_shipping">
                                        <template slot='label'>
                                            <span><?= \Yii::t('components/goods', '运费设置');?></span>
                                            <el-tooltip effect="dark" content="<?= \Yii::t('components/goods', '选择第一项（默认运费）将会根据运费管理的（默认运费）变化而变化');?>"
                                                        placement="top">
                                                <i class="el-icon-info"></i>
                                            </el-tooltip>
                                        </template>
                                        ...
                                        <el-button type="button" size="mini" @click="freightOpen">
                                            <?= \Yii::t('components/goods', '选择运费');?>
                                        </el-button>
                                        <el-dialog title="<?= \Yii::t('components/goods', '选择运费');?>" :visible.sync="freight.dialog" width="30%">
                                            <div class="input-item">
                                                <el-input size="small" placeholder="<?= \Yii::t('components/goods', '请输入运费名称');?>" v-model="search.ssYF" clearable>
                                            ...
                                            <el-card shadow="never" flex="dir:left" style="flex-wrap: wrap"
                                                ...
                                                <div v-else style="width: 520px;color:#999999" flex="cross:center main:center"><?= \Yii::t('components/goods', '暂无搜索结果');?></div>
                                            </el-card>
                                            <div slot="footer" class="dialog-footer">
                                                <el-button @click="freightCancel"><?= \Yii::t('components/goods', '取消');?></el-button>
                                                <el-button type="primary" @click="freightConfirm"><?= \Yii::t('components/goods', '确定');?></el-button>
                                            </div>
                                        </el-dialog>
                                    </el-form-item>
                                    ...
                                    <el-form-item prop="freight_id" v-if="ruleForm.type == 'goods' && is_shipping_rules　">
                                        <template slot='label'>
                                            <span><?= \Yii::t('components/goods', '包邮规则');?></span>
                                        </template>
                                    ...
                                    <el-form-item prop="freight_id" v-if="is_form == 1">
                                        <template slot='label'>
                                            <span><?= \Yii::t('components/goods', '自定义表单');?></span>
                                            <el-tooltip effect="dark" content="<?= \Yii::t('components/goods', '选择第一项（默认表单）将会根据表单列表的（默认表单）变化而变化');?>"
                                                        placement="top">
                                                <i class="el-icon-info"></i>
                                            </el-tooltip>
                                        </template>
                                        <app-goods-form v-model="ruleForm.form" @selected="selectForm"
                                                        title="<?= \Yii::t('components/goods', '选择表单');?>"
                                    ...
                                    <!-- 区域购买 -->
                                    <el-form-item label="<?= \Yii::t('components/goods', '区域购买');?>" prop="is_area_limit" v-if="is_area_limit == 1 && ruleForm.type == 'goods' && sign !='community'">
                                        <el-switch
                                    ...
                                    <el-form-item v-if="is_area_limit == 1 && ruleForm.is_area_limit" label="<?= \Yii::t('components/goods', '允许购买区域');?>" prop="area_limit">
                        ...
                        <el-card shadow="never" class="mt-24" v-if="is_display_setting === 1">
                            <div slot="header">
                                <span><?= \Yii::t('components/goods', '显示设置');?></span>
                            </div>
                            <el-row>
                                <el-col :xl="12" :lg="16">
                                    <el-form-item label="<?= \Yii::t('components/goods', '添加到快速购买');?>" prop="is_quick_shop">
                        ...
                        <el-card shadow="never" class="mt-24" v-if="is_marketing === 1 && mch_id == 0">
                            <div slot="header">
                                <span><?= \Yii::t('components/goods', '营销设置');?></span>
                            </div>
                            <el-row>
                                <el-col :xl="12" :lg="16">
                                    <el-form-item>
                                        <template slot='label'>
                                            <span><?= \Yii::t('components/goods', '积分赠送');?></span>
                                            <el-tooltip effect="dark" placement="top">
                                                <div slot="content"><?= \Yii::t('components/goods', 'a1');?>"<br/>
                                                    <?= \Yii::t('components/goods', 'a2');?> </br>
                                                    <?= \Yii::t('components/goods', 'a3');?>
                                                </div>
                                                <i class="el-icon-info"></i>
                                            </el-tooltip>
                                        </template>
                                        <el-input type="number" min="0"
                                                  ...
                                                  placeholder="<?= \Yii::t('components/goods', '请输入赠送积分数量');?>" v-model="ruleForm.give_integral">
                                            <template slot="append">
                                                {{ruleForm.give_integral_type === 1 ? "<?= \Yii::t('components/goods', '元');?>" : '%'}}
                                                <el-radio v-model="ruleForm.give_integral_type" :label="1"><?= \Yii::t('components/goods', '固定值');?></el-radio>
                                                <el-radio v-model="ruleForm.give_integral_type" :label="2"><?= \Yii::t('components/goods', '百分比');?></el-radio>
                                            </template>
                                        </el-input>
                                    </el-form-item>
                                    ...
                                    <el-form-item v-if="is_forehead_integral === 1">
                                        <template slot='label'>
                                            <span><?= \Yii::t('components/goods', '积分抵扣');?></span>
                                            <el-tooltip effect="dark" content="<?= \Yii::t('components/goods', '如果设置0，则不支持积分抵扣 如果带%则为按成交价格的比例计算抵扣多少元');?>"
                                                        placement="top">
                                                <i class="el-icon-info"></i>
                                            </el-tooltip>
                                        </template>
                                        <el-input type="number" min="0"
                                                  oninput="this.value = this.value.replace(/^(\-)*(\d+)\.(\d\d).*$/,'$1$2.$3');"
                                                  placeholder="<?= \Yii::t('components/goods', '请输最高抵扣金额');?>"
                                                  v-model="ruleForm.forehead_integral">
                                            <template slot="prepend">
                                                <?= \Yii::t('components/goods', '最多抵扣');?>
                                            </template>
                                            <template slot="append">
                                                {{ruleForm.forehead_integral_type === 1 ? "<?= \Yii::t('components/goods', '元');?>" : '%'}}
                                                <el-radio v-model="ruleForm.forehead_integral_type" :label="1"><?= \Yii::t('components/goods', '固定值');?>
                                                </el-radio>
                                                <el-radio v-model="ruleForm.forehead_integral_type" :label="2"><?= \Yii::t('components/goods', '百分比');?>
                                                </el-radio>
                                            </template>
                                        </el-input>
                                        <el-checkbox :true-label="1" :false-label="0"
                                                     v-model="ruleForm.accumulative">
                                            <?= \Yii::t('components/goods', '允许多件累计抵扣');?>
                                        </el-checkbox>
                                    </el-form-item>

                                    <el-form-item>
                                        <template slot='label'>
                                            <span><?= \Yii::t('components/goods', '余额赠送');?></span>
                                        </template>
                                        <el-input type="number" min="0"
                                                  oninput="this.value = this.value.replace(/^(\-)*(\d+)\.(\d\d).*$/,'$1$2.$3')"
                                                  placeholder="<?= \Yii::t('components/goods', '请输入赠送余额');?>" v-model="ruleForm.give_balance">

                                            <template slot="append">
                                                {{ruleForm.give_balance_type === 1 ? "<?= \Yii::t('components/goods', '元');?>" : '%'}}
                                                <el-radio v-model="ruleForm.give_balance_type" :label="1"><?= \Yii::t('components/goods', '固定值');?></el-radio>
                                                <el-radio v-model="ruleForm.give_balance_type" :label="2"><?= \Yii::t('components/goods', '百分比');?></el-radio>
                                            </template>
                                        </el-input>
                                    </el-form-item>

                                    <el-form-item label="<?= \Yii::t('components/goods', '卡券发放');?>" v-if="sign != 'mch' && sign != 'exchange'">
                                        <el-tag v-for="(card, index) in ruleForm.cards" @close="cardDelete(index)"
                                            ...
                                            {{card.num}}<?= \Yii::t('components/goods', '张');?> | {{card.name}}
                                        </el-tag>
                                        <el-button type="button" size="mini" @click="cardDialogVisible = true"><?= \Yii::t('components/goods', '新增卡券');?>
                                        </el-button>
                                    </el-form-item>

                                    <el-form-item label="<?= \Yii::t('components/goods', '优惠券发放');?>" v-if="is_mch !== 1">

                                        <el-tag v-for="(coupon, index) in ruleForm.coupons" @close="couponDelete(index)"
                                                :key="index" :disable-transitions="true"
                                                style="margin: 0 10px 10px 0;" closable>
                                            {{coupon.send_num}}<?= \Yii::t('components/goods', '张');?> | {{coupon.name}}
                                        </el-tag>
                                        <div style="display: inline-block">
                                            <app-select-coupon v-model="ruleForm.coupons" @select="couponSubmit">
                                                <el-button type="button" size="mini"><?= \Yii::t('components/goods', '选择优惠券');?>
                                                </el-button>
                                            </app-select-coupon>
                                        </div>
                        ...
                        <el-card shadow="never" class="mt-24" v-if="is_sharing_setting === 1">
                            <div slot="header">
                                <span><?= \Yii::t('components/goods', '分享设置');?></span>
                            </div>
                            <el-row>
                                <el-col :xl="12" :lg="16">
                                    <el-form-item prop="app_share_title">
                                        <label slot="label">
                                            <span><?= \Yii::t('components/goods', '自定义分享标题');?></span>
                                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('components/goods', '分享给好友时，作为商品名称');?>"
                                                        placement="top">
                                                <i class="el-icon-info"></i>
                                            </el-tooltip>
                                        </label>
                                        <el-input placeholder="<?= \Yii::t('components/goods', '请输入分享标题');?>"
                                                  v-model="ruleForm.app_share_title"></el-input>
                                        <el-button @click="app_share.dialog = true;app_share.type = 'name_bg'"
                                                   type="text"><?= \Yii::t('components/goods', '查看图例');?>
                                        </el-button>
                                    </el-form-item>

                                    <el-form-item  prop="app_share_pic">
                                        <label slot="label">
                                            <span><?= \Yii::t('components/goods', '自定义分享图片');?></span>
                                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('components/goods', '分享给好友时，作为分享图片');?>"
                                                        placement="top">
                                                <i class="el-icon-info"></i>
                                            </el-tooltip>
                                        </label>
                                        <app-attachment v-model="ruleForm.app_share_pic" :multiple="false" :max="1">
                                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('components/goods', '建议尺寸');?>:420 * 336"
                                                        placement="top">
                                                <el-button size="mini"><?= \Yii::t('components/goods', '选择图片');?></el-button>
                                            </el-tooltip>
                                        </app-attachment>
                        ...
                        <el-card shadow="never" class="mt-24" v-if="is_detail === 1">
                            <div slot="header">
                                <span><?= \Yii::t('components/goods', '商品详情');?></span>
                            </div>
                            <el-row>
                                <el-col :xl="12" :lg="16">
                    ...
                    <el-tab-pane label="<?= \Yii::t('components/goods', '分销价设置');?>" name="there" v-if="is_show_share">
                        <el-form-item :label="is_mch ? '<?= \Yii::t('components/goods', '是否开启分销佣金');?>' : '<?= \Yii::t('components/goods', '是否开启单独分销');?>'" prop="individual_share">
                            <el-switch :active-value="1" :inactive-value="0" v-model="ruleForm.individual_share">
                            </el-switch>
                        </el-form-item>
                        <template v-if="ruleForm.individual_share == 1">
                            <el-form-item label="<?= \Yii::t('components/goods', '分销类型');?>" prop="attr_setting_type" v-if="cForm.use_attr == 1 && ruleForm.type == 'goods'">
                                <el-radio v-model="ruleForm.attr_setting_type" :label="0"><?= \Yii::t('components/goods', '普通设置');?></el-radio>
                                <el-radio v-model="ruleForm.attr_setting_type" :label="1"><?= \Yii::t('components/goods', '详细设置');?></el-radio>
                            </el-form-item>
                            <el-form-item label="<?= \Yii::t('components/goods', '分销佣金类型');?>" prop="share_type">
                                <el-radio v-model="ruleForm.share_type" :label="0"><?= \Yii::t('components/goods', '固定金额');?></el-radio>
                                <el-radio v-model="ruleForm.share_type" :label="1"><?= \Yii::t('components/goods', '百分比');?></el-radio>
                            </el-form-item>
                            ...
                            <template v-if="sign !== 'pintuan'">
                                <app-goods-share v-model="ruleForm" :is_mch="is_mch" :attr-groups="attrGroups"
                                                 ...
                                                 :sign="sign"
                                                 pintuan_sign="<?= \Yii::t('components/goods', '单独购买');?>"
                                ></app-goods-share>
                            </template>
                            <template v-if="sign === 'pintuan'">
                                    ...
                                    <div class="pintuan-share">
                                        <el-card v-for="(item, index) in ruleForm.group_list" :key="index">
                                            <app-goods-share v-model="ruleForm.group_list[index]" :attr-groups="attrGroups"
                                                             ...
                                                             :use_attr="ruleForm.use_attr"
                                                             :sign="sign"
                                                             :pintuan_sign="item.people_num + '<?= \Yii::t('components/goods', '人团');?>'"
                                            ></app-goods-share>
                                        </el-card>
                                    </div>
                                </template>
                                <template v-else>
                                    <el-form-item>
                                        <el-tag type="danger"><?= \Yii::t('components/goods', '请先添加阶梯团');?></el-tag>
                                    </el-form-item>
                                </template>
                            </template>
                        </template>
                    ...
                    <el-tab-pane label="<?= \Yii::t('components/goods', '会员价设置');?>" name="third" v-if="is_member === 1">

                        <el-form-item v-if="is_svip" label="<?= \Yii::t('components/goods', '是否享受超级会员功能');?>" prop="is_vip_card_goods">
                            <el-switch v-model="is_vip_card_goods" :active-value="1" :inactive-value="0"></el-switch>
                        </el-form-item>

                        <el-form-item label="<?= \Yii::t('components/goods', '是否享受会员功能');?>" prop="is_level">
                        ...
                        <template v-if="ruleForm.is_level == 1">
                            <el-form-item label="<?= \Yii::t('components/goods', '是否单独设置会员价');?>" prop="is_level_alone">
                            ...
                            <template v-if="ruleForm.is_level_alone == 1">
                                ...
                                <template v-if="ruleForm.use_attr == 1 && memberLevel.length > 0">
                                    <!--多规格会员价设置-->
                                    <template v-if="sign !== 'pintuan'">
                                        <el-form-item label="<?= \Yii::t('components/goods', '会员价设置');?>">
                                            <app-attr v-model="ruleForm.attr" :attr-groups="attrGroups"
                                ...
                                <template v-if="sign !== 'pintuan'">
                                    ...
                                    <template v-if="ruleForm.use_attr == 0 && memberLevel.length > 0">
                                        <el-form-item label="<?= \Yii::t('components/goods', '默认规格会员价设置');?>">
                                            <el-col :xl="12" :lg="16">
                                                <el-input v-for="item in defaultMemberPrice" :key="item.id" type="number"
                                                    ...
                                                    <span slot="append"><?= \Yii::t('components/goods', '元');?></span>
                                                </el-input>
                                            </el-col>
                                        </el-form-item>
                                        <el-form-item>
                                            <el-tag type="danger"><?= \Yii::t('components/goods', '如需设置多规格会员价,请先添加商品规格');?></el-tag>
                                        </el-form-item>
                                    </template>
                                </template>
                                    ...
                                    <template v-if="pintuan_list.group_list.length > 0">
                                        <div  class="pintuan-share">
                                            <el-card v-for="(item, index) in pintuan_list.group_list">
                                                ...
                                                <template v-if="ruleForm.use_attr == 1 && memberLevel.length > 0">
                                                    <div flex="dir:top">
                                                        <div style="margin-left: 180px; padding: 16px; border-left: 1px solid #EBEEF5;border-right: 1px solid #EBEEF5;border-top: 1px solid #EBEEF5;">
                                                            <el-tag style="float: left;margin-right: 50px" type="danger" >
                                                                {{item.people_num}}<?= \Yii::t('components/goods', '人团');?>
                                                            </el-tag>
                                                        </div>
                                                        <!--多规格会员价设置-->
                                                        <el-form-item>
                                                ...
                                                <el-form-item v-if="memberLevel.length == 0" label="<?= \Yii::t('components/goods', '会员价设置');?>">
                                                    <el-button type="danger"
                                                               @click="$navigate({r: 'mall/mall-member/edit'})">
                                                        <?= \Yii::t('components/goods', '如需设置,请先添加会员');?>
                                                    </el-button>
                                                </el-form-item>
                                            </el-card>
                                        </div>
                    ...
                    <el-tab-pane label="视频号设置" name="four_video" v-if="ruleForm.type == 'goods' && is_video_channel === 1">
                        <diy-video-channel v-model="ruleForm.video_channel" :is-diy-area="false"></diy-video-channel>
                    </el-tab-pane>
            ...
            <div class="bottom-div" flex="cross:center" v-if="is_save_btn == 1">
                <el-button class="button-item" :loading="btnLoading" type="primary" size="small"
                           @click="store('ruleForm')"><?= \Yii::t('components/goods', '保存');?>
                </el-button>
                <el-button class="button-item" size="small" @click="showPreview"><?= \Yii::t('components/goods', '预览');?></el-button>
            </div>
        </div>
        ...
        <!-- 自定义 -->
        <el-dialog :title="app_share['type'] == 'pic_bg' ? `<?= \Yii::t('components/goods', '查看自定义分享图片图例');?>`:`<?= \Yii::t('components/goods', '查看自定义分享标题图例');?>`"
            ...
            <div slot="footer" class="dialog-footer">
                <el-button @click="app_share.dialog = false" type="primary"><?= \Yii::t('components/goods', '我知道了');?></el-button>
            </div>
        </el-dialog>
        <!-- 自定义 -->
        <el-dialog :title="guarantee['type'] == 'goods_pic_example' ? `<?= \Yii::t('components/goods', '查看商品服务标识图例');?>`:`<?= \Yii::t('components/goods', '查看商品服务标题图例');?>`"
            ...
            <div slot="footer" class="dialog-footer">
                <el-button @click="guarantee.dialog = false" type="primary"><?= \Yii::t('components/goods', '我知道了');?></el-button>
            </div>
        </el-dialog>
    </el-card>
</template>
...
<script>
    Vue.component('app-goods', {
        ...
        props: {
            ...
            price_label:{
                type: String,
                default: '<?= \Yii::t('components/goods', '售价');?>'
            },
            original_price_label:{
                type: String,
                default: '<?= \Yii::t('components/goods', '原价');?>'
            },
            cost_price_label:{
                type: String,
                default: '<?= \Yii::t('components/goods', '成本价');?>'
            },
            sign_cn: '',
        ...
        data() {
            let ruleForm = {
                ...
                guarantee_title: '<?= \Yii::t('components/goods', '保障');?>',
                param_name: '',
                param_content: [],
                min_number: 1,
                limit_buy_status: 1,
            ...
            let rules = {
                cats: [
                    {
                        required: true, type: 'array', validator: (rule, value, callback) => {
                            ...
                            callback('<?= \Yii::t('components/goods', '请选择分类');?>');
                        }
                    }
                ],
                mchCats: [
                    {
                        required: true, type: 'array', validator: (rule, value, callback) => {
                            ...
                            callback('<?= \Yii::t('components/goods', '请选择系统分类');?>');
                        }
                    }
                ],
                name: [
                    {required: true, message: '<?= \Yii::t('components/goods', '请输入商品名称');?>', trigger: 'change'},
                ],
                price: [
                    {required: true, message: '<?= \Yii::t('components/goods', '请输入商品价格');?>', trigger: 'change'}
                ],
                original_price: [
                    {message: '<?= \Yii::t('components/goods', '请输入商品原价');?>', trigger: 'change'}
                ],
                cost_price: [
                    {required: false, message: '<?= \Yii::t('components/goods', '请输入商品成本价');?>', trigger: 'change'}
                ],
                unit: [
                    {required: true, message: '<?= \Yii::t('components/goods', '请输入商品单位');?>', trigger: 'change'},
                    {max: 5, message: '<?= \Yii::t('components/goods', '最大为5个字符');?>', trigger: 'change'}
                ],
                goods_num: [
                    // {required: true, message: '请输入商品总库存', trigger: ['change', 'blur']}
                ],
                is_area_limit: [
                    {required: false, type: 'integer', message: '<?= \Yii::t('components/goods', '请选择是否开启');?>', trigger: 'blur'}
                ],
                area_limit: [
                    {
                        required: true, type: 'array', validator: (rule, value, callback) => {
                            if (value instanceof Array && value[0]['list'].length === 0) {
                                callback('<?= \Yii::t('components/goods', '允许购买区域不能为空');?>');
                            }
                            callback();
                        }
                    }
                ...
                pic_url: [
                    {required: true, message: '<?= \Yii::t('components/goods', '请上传商品轮播图');?>', trigger: 'change'}
                ],
                min_number: [
                    {
                        type: 'integer', validator: (rule, value, callback) => {
                            if (value < 1) {
                                callback('<?= \Yii::t('components/goods', '最少起售数量为1');?>');
                            }
                            if (value > max) {
                                callback('<?= \Yii::t('components/goods', '最大起售数量为');?>' + max);
                            }
                            callback();
                        }, trigger: 'change'
                    }
                ...
                confine_count: [
                    {
                        type: 'integer', validator: (rule, value, callback) => {
                            if (value > -1 && value < 1) {
                                callback('<?= \Yii::t('components/goods', '终身限购数量最少为1');?>');
                            }
                            if (value > max) {
                                callback('<?= \Yii::t('components/goods', '终身限购数量最大为');?>' + max);
                            }
                            callback();
                        }, trigger: 'change'
                    }
                ...
                limit_buy_value: [
                    {
                        type: 'integer', validator: (rule, value, callback) => {
                            if (value > -1 && value < 1) {
                                callback('<?= \Yii::t('components/goods', '按周期限购数量最少为1');?>');
                            }
                            if (value > max) {
                                callback('<?= \Yii::t('components/goods', '按周期限购数量最大为');?>' + max);
                            }
                            callback();
                        }, trigger: 'change'
                    }
        ...
        methods: {
            leaveTab(activeName) {
                ...
                if(activeName == 'four' && self.sign == 'wholesale') {
                    ...
                    if(!pass) {
                        self.$message.error('<?= \Yii::t('components/goods', '请先设置价格');?>');
                    }
                    return pass
                }
            },
            ...
            store(formName) {
                ...
                try {
                    if(self.sign == 'wholesale') {
                        ...
                        throw new Error('<?= \Yii::t('components/goods', '请选择兑换码库');?>');
                    }
                    ...
                    if(self.cForm.param_content && self.cForm.param_content.length){
                        ...
                        if(s){
                            throw new Error('<?= \Yii::t('components/goods', '参数名或参数值不能为空');?>');
                        }
                    }
                    ...
                    self.cForm.attr.map(item => {
                        if (item.price < 0 || item.price === '') {
                            if(self.sign == 'community') {
                                throw new Error('<?= \Yii::t('components/goods', '买家购买价不能为空');?>');
                            }else {
                                throw new Error('<?= \Yii::t('components/goods', '规格价格不能为空');?>');
                            }
                        }
                        if (item.stock < 0 || item.stock === '') {
                            throw new Error('<?= \Yii::t('components/goods', '库存不能为空');?>');
                        }
                        if(self.sign == 'community') {
                            if(item.supply_price < 0 || item.supply_price === '') {
                                throw new Error('<?= \Yii::t('components/goods', '团长供货价不能为空');?>');
                            }
                            if(+item.supply_price > +item.price) {
                                throw new Error('<?= \Yii::t('components/goods', '团长供货价不能大于买家购买价');?>');
                            }
                    ...
                    if (self.sign === 'pintuan' && self.cForm.group_list.length === 0) {
                        this.$confirm('<?= \Yii::t('components/goods', '该商品未添加阶梯团, 将无法在小程序端展示,是否继续');?>', '<?= \Yii::t('components/goods', '提示');?>', {
                            confirmButtonText: '<?= \Yii::t('components/goods', '保存');?>',
                            cancelButtonText: '<?= \Yii::t('components/goods', '继续编辑');?>',
                            type: 'warning'
                        }).then(() => {
                ...
                self.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        request({
                            ...
                            if (e.data.code === 0) {
                                if(isZero) {
                                    self.$message.success('<?= \Yii::t('components/goods', '保存成功，温馨提示："买家购买价"或"团长供货价"为0，建议重新设置');?>');
                                    time = 2000;
                                }else {
                        ...
                        self.$message.error('<?= \Yii::t('components/goods', '请填写必填参数');?>');
                        return false;
                    }
                });
            },
            ...
            statusChange(res) {
                if (res && this.status_change_text) {
                    this.$alert(this.status_change_text, '<?= \Yii::t('components/goods', '提示');?>', {
