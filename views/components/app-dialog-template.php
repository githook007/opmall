...
<template id="app-dialog-template">
    <div class="app-dialog-template">
        <el-dialog append-to-body title="<?= \Yii::t('components/other', '选择模板');?>" :visible.sync="show" :close-on-click-modal="false"
            ...
            <el-form size="small" :inline="true" v-if="status == 1">
                <el-form-item>
                    <el-select @change="onSubmit" style="width: 150px" v-model="is_buy">
                        <el-option value="" label="<?= \Yii::t('components/other', '全部模板');?>"></el-option>
                        <el-option :value="1" label="<?= \Yii::t('components/other', '已购买');?>"></el-option>
                        <el-option :value="0" label="<?= \Yii::t('components/other', '未购买');?>"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item style="display: none">
                    <div class="input-item">
                        <el-input @keyup.enter.native="onSubmit" size="small" placeholder="<?= \Yii::t('components/other', '请输入模板名称');?>"
                ...
                <el-form-item>
                    <el-checkbox v-model="checkAll" :indeterminate="isIndeterminate" @change="handleCheckAllChange"><?= \Yii::t('components/other', '批量选择');?></el-checkbox>
                </el-form-item>
            </el-form>
            <div v-loading="listLoading">
                ...
                <div style="text-align: center;margin-top: 20px;color: #999999;">
                    <template v-if="no_more">
                        <span><?= \Yii::t('components/other', '没有更多数据');?></span>
                    </template>
                    <el-button type="text" :loading="loading" @click="more" v-else><?= \Yii::t('components/other', '加载更多');?></el-button>
                </div>
            </div>
            <div style="margin-top: 24px;" slot="footer">
                <el-button size="small" @click="cancel"><?= \Yii::t('components/other', '取消');?></el-button>
                <el-button type="primary" size="small" @click="confirm"><?= \Yii::t('components/other', '确定');?></el-button>
            </div>
        </el-dialog>
    </div>
</template>
