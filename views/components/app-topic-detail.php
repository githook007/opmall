    ...
    <div class="app-topic-detail">
        <div flex="box:first" style="margin-bottom: 10px;min-width: 1280px;height: 725px;">
            ...
            <div style="padding-left: 2px;position: relative;overflow-y: auto">
                <div style="overflow-y: auto;overflow-x: hidden;padding: 0 25px;width: 435px;height: 705px;">
                        ...
                        <div class="mobile-framework-body">
                            ...
                            <div v-else flex="main:center cross:center"
                                ...
                                <div>
                                    ...
                                    <div><?= \Yii::t('components/other', '空空如也');?></div>
                                    <div><?= \Yii::t('components/other', '请从上方组件库添加组件');?></div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                allComponents: [
                    {
                        list: [
                            {
                                id: 'rubik',
                                name: '<?= \Yii::t('components/other', '图片广告');?>',
                                icon: '<?=$baseUrl?>/statics/img/mall/diy/rubik.png',
                            },
                            {
                                id: 'video',
                                name: '<?= \Yii::t('components/other', '视频');?>',
                                icon: '<?=$baseUrl?>/statics/img/mall/diy/video.png',
                            },
                            {
                                id: 'image-text',
                                name: '<?= \Yii::t('components/other', '图文详情');?>',
                                icon: '<?=$baseUrl?>/statics/img/mall/diy/image-text.png',
                            },
                            {
                                id: 'video-channel',
                                name: '视频号',
                                icon: '<?=$baseUrl?>/statics/img/mall/diy/video_channel.png',
                            },
                        ]
                    }
                ],
                ...
                if (e.single) {
                    for (let i in this.components) {
                        if (this.components[i].id === e.id) {
                            this.$message.error('<?= \Yii::t('components/other', '该组件只允许添加一个');?>');
                            return;
                        }
                    }
                }
