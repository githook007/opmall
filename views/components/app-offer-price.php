...
<div id="app-offer-price" v-cloak>
    <el-card shadow="never" style="border:0" v-loading="loading"
        ...
        <el-form ref="ruleForm" label-width="150px" :model="ruleForm" :rules="rules"
            ...
            <div class="form-body">
                <el-form-item label="<?= \Yii::t('components/other', '是否开启');?>" prop="is_enable" required>
                    <el-switch
                ...
                <template v-if="ruleForm.is_enable == 1">
                    <el-form-item label="<?= \Yii::t('components/other', '全地区起送金额设置');?>" prop="is_total_price" required>
                        <el-switch
                    ...
                    <el-form-item v-if="ruleForm.is_total_price" label="<?= \Yii::t('components/other', '全地区起送金额');?>" prop="total_price" required>
                        <el-input size="small" v-model.number="ruleForm.total_price" type="number">
                            <template slot="append"><?= \Yii::t('components/other', '元');?></template>
                        </el-input>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('components/other', '起送规则');?>" prop="detail">
                        <el-card shadow="never" style="margin-bottom: 12px;" v-if="ruleForm.detail"
                            ...
                            <div flex="dir:left box:last">
                                <div><?= \Yii::t('components/other', '起送金额');?>：{{item.total_price}}<?= \Yii::t('components/other', '元');?>
                                </div>
                            ...
                            <div flex="dir:left" style="flex-wrap: wrap">
                                <div><?= \Yii::t('components/other', '区域');?>：</div>
                        ...
                        <el-button size="small" type="text" @click="openDistrict()"><i class="el-icon-plus"><?= \Yii::t('components/other', '新增规则');?></i>
                        </el-button>
                    </el-form-item>
                </template>
            </div>
        ...
        <el-dialog title="<?= \Yii::t('components/other', '新增规则');?>" :visible.sync="dialogVisible" width="50%">
            <el-form ref="detail" @submit.native.prevent :model="detail" label-width="150px" :rules="dialogRules">
                <el-form-item label="<?= \Yii::t('components/other', '起送金额');?>" prop="total_price">
                    <el-input v-model="detail.total_price" type="number">
                        <template slot="append"><?= \Yii::t('components/other', '元');?></template>
                    </el-input>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('components/other', '地区选择');?>" prop="list">
    ...
    Vue.component('app-offer-price', {
        ...
        data() {
            let min_number = (rule, value, callback) => {
                if (value < 0) {
                    callback(new Error("<?= \Yii::t('components/other', '输入值必须大于等于0');?>"));
                } else {
                    callback();
                }
            };
            return {
                ...
                rules: {
                    is_enable: [
                        {type: 'required', message: "<?= \Yii::t('components/other', '请选择是否开启');?>"}
                    ],
                    total_price: [
                        {validator: min_number, trigger: 'change'},
                        {required: true, type: 'number', trigger: 'change', message: "<?= \Yii::t('components/other', '请输入起送金额');?>
                            "},
                    ],
                },
                dialogRules: {
                    total_price: [
                        {required: true, message: "<?= \Yii::t('components/other', '请输入起送金额');?>", trigger: 'change'},
                        {validator: min_number, trigger: 'change'},
                    ],
                    list: [
                        {required: true, message: "<?= \Yii::t('components/other', '请选择起送规则');?>"}
                    ]
                },
                detail: {
                    total_price: 0,
            ...
            methods: {
                load() {
                    ...
                    request({
                        ...
                        this.loading = false;
                        this.$message.error("<?= \Yii::t('components/other', '获取数据出错');?>");
                    });
                },
                openDistrict(index) {
