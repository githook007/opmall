    ...
    <div class="app-edit-address">
        <el-dialog @close="dialogClose" :title="title" :visible.sync="openDialog" width="30%">
            <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="80px">
                <el-form-item prop="name" label="<?= \Yii::t('components/order', '收件人');?>">
                    <el-input type="text" size="small" v-model="ruleForm.name" autocomplete="off"></el-input>
                </el-form-item>
                <el-form-item prop="mobile" label="<?= \Yii::t('components/order', '电话');?>">
                    <el-input size="small" v-model="ruleForm.mobile" autocomplete="off"></el-input>
                </el-form-item>
                <el-form-item prop="provinceName" label="<?= \Yii::t('components/order', '所在区域');?>">
                    <el-select v-model="ruleForm.provinceName" size="small" placeholder="<?= \Yii::t('components/order', '请选择');?>" @change="getCity"
                               style="width:30%;">
                        <el-option
                    ...
                    <el-select v-model="cityName" size="small" placeholder="<?= \Yii::t('components/order', '请选择');?>" @change="getDistrict"
                               style="width:30%;">
                        <el-option
                                v-for="item in city"
                                :key="item.id"
                    ...
                    <el-select v-model="districtName" size="small" placeholder="<?= \Yii::t('components/order', '请选择');?>" style="width:30%;">
                        <el-option
                ...
                <el-form-item prop="address_detail" label="<?= \Yii::t('components/order', '详细地址');?>">
                    <el-input type="text" size="small" v-model="ruleForm.address_detail" autocomplete="off"></el-input>
                </el-form-item>
                <el-form-item style="text-align: right">
                    <el-button size="small" @click="openDialog = false"><?= \Yii::t('components/order', '取消');?></el-button>
                    <el-button :loading="btnLoading" size="small" type="primary" @click="changAddress('ruleForm')"><?= \Yii::t('components/order', '确定');?>
                    </el-button>
                </el-form-item>
            </el-form>
        </el-dialog>
    ...
    Vue.component('app-edit-address', {
            ...
            isShow: function (newVal) {
                if (newVal) {
                    if (this.order.send_type == 1 && this.order.is_send == 0) {
                        this.title = "<?= \Yii::t('components/order', '添加收货地址');?>"
                    }
                    this.openAddress();
                }
            }
        ...
        data() {
            return {
                ...
                district: [],
                btnLoading: false,
                title: "<?= \Yii::t('components/order', '修改收货信息');?>",
                rules: {
                    name: [
                        { required: true, message: "<?= \Yii::t('components/order', '请填写收件人');?>", trigger: 'change' }
                    ],
                    mobile: [
                        { required: true, message: "<?= \Yii::t('components/order', '请填写电话');?>", trigger: 'change' }
                    ],
                    provinceName: [
                        { required: true, message: "<?= \Yii::t('components/order', '请填写所在区域');?>", trigger: 'change' }
                    ],
                    address_detail: [
                        { required: true, message: "<?= \Yii::t('components/order', '请填写详细地址');?>", trigger: 'change' }
                    ],
                },
                ruleForm: {
                    name: '',
