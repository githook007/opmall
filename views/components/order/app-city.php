...
<template id="app-city">
    <div class="app-city">
        <!-- 配送员选择 -->
        <el-dialog title="<?= \Yii::t('components/order', '选择配送员');?>" :visible.sync="city.dialog" @close="closeDialog" width="35%">
            <el-form v-loading="dialogLoading" label-width="130px" class="sendForm" @submit="prevD">
                <el-form-item label="<?= \Yii::t('components/order', '配送员');?>" prop="city_name" hidden>
                    <el-input></el-input>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('components/order', '配送员');?>" prop="city_name">
                    <el-autocomplete @keyup.enter.native="prevD" v-if="city.list.length > 0"
                                     size="small"
                                     v-model="city.man"
                                     :fetch-suggestions="searchCity"
                                     placeholder="<?= \Yii::t('components/order', '选择配送员');?>"
                    ></el-autocomplete>
                    <template v-else>
                ...
                <el-form-item style="text-align: right">
                    <el-button size="small" @click="city.dialog = false"><?= \Yii::t('components/order', '取消');?></el-button>
                    <el-button size="small" type="primary" :loading="submitLoading"
                               @click="citySend"><?= \Yii::t('components/order', '确定');?>
                    </el-button>
                </el-form-item>
            </el-form>
        </el-dialog>
