        ...
        <el-dialog :title="title" :visible.sync="dialogVisible" width="30%" @close="closeDialog">
            <el-form>
                ...
                <div v-if="order.cancel_data.image_list && order.cancel_data.image_list.length" flex="dir:left" class="div-item">
                    <div flex-box="0"><?= \Yii::t('components/order', '图片凭证');?>：</div>
                    <div flex-box="1">
                ...
                <el-form-item style="text-align: right">
                    ...
                    <el-button size="small" type="primary" :loading="submitLoading" @click="toSumbit"><?= \Yii::t('components/order', '确定');?>
                    </el-button>
                </el-form-item>
            </el-form>
        </el-dialog>
        ...
        methods: {
            openDialog() {
                ...
                if (this.cancelType == 1) {
                    this.status = 1;
                    this.title = "<?= \Yii::t('components/order', '同意取消');?>";
                    this.content = "<?= \Yii::t('components/order', '填写同意理由');?>:";
                } else if (this.cancelType == 0) {
                    this.status = 2;
                    this.title = "<?= \Yii::t('components/order', '拒绝取消');?>";
                    this.content = "<?= \Yii::t('components/order', '填写拒绝理由');?>:";
                } else if (this.cancelType == 2) {
                    this.status = 1;
                    this.title = "<?= \Yii::t('components/order', '订单取消');?>";
                    this.content = "<?= \Yii::t('components/order', '填写取消理由');?>:";
                }
            },
            closeDialog() {
                this.$emit('close')
