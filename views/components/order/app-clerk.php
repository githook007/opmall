...
<template id="app-clerk">
    <div class="app-clerk">
        <!-- 核销员选择 -->
        <el-dialog title="<?= \Yii::t('components/order', '选择核销员');?>" :visible.sync="dialogVisible" width="30%" @close="closeDialog">
            <el-form v-loading="dialogLoading" label-position="left" :model="clerk_user" ref="clerk_user"
                     :rules="clerkRules" label-width="90px">
                <el-form-item label="<?= \Yii::t('components/order', '核销员');?>" prop="user.nickname">
                    <el-autocomplete
                            size="small"
                            v-model="clerk_user.user.nickname"
                            :fetch-suggestions="clerkSearch"
                            placeholder="<?= \Yii::t('components/order', '选择核销员');?>"
                            @select="dialogFormChooseClerk"
                    ></el-autocomplete>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('components/order', '所属门店');?>" v-if="clerk_user.store.length > 0">
                    <div v-for="store in clerk_user.store" :key="store.id">{{ store.name }}</div>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('components/order', '核销备注');?>" prop="clerk_remark">
                    <el-input type="textarea" v-model="clerk_remark" autocomplete="off"></el-input>
                </el-form-item>
                <el-form-item style="text-align: right;margin-bottom: 0">
                    <el-button size="small" type="primary" @click="dialogVisible = false"><?= \Yii::t('components/order', '取消');?></el-button>
        ...
        data() {
            return {
                ...
                clerkRules: {
                    user: {
                        nickname: [
                            {required: true, message: "<?= \Yii::t('components/order', '选择核销员');?>", trigger: 'change'},
                        ]
                    },
                },
                keyword: '',
        ...
        methods: {
            ...
            openDialog() {
                if (this.order.is_pay == 1) {
                    ...
                    this.$confirm("<?= \Yii::t('components/order', '是否确认该订单已线下收款');?>", "<?= \Yii::t('components/order', '提示');?>", {
