 ...
 * author: opmall
 */
Yii::$app->loadViewComponent('app-send-second', __DIR__);
?>
...
<template id="app-send">
    <div class="app-send">
        <!-- 发货 -->
        <el-dialog title="<?= \Yii::t('components/order', '发货');?>" :visible.sync="dialogVisible" width="35%" @close="closeDialog">
            <el-tabs v-model="activeName" @tab-click="handleClick">
                <el-tab-pane label="<?= \Yii::t('mall/index', '基础配送');?>" name="first">
                    <div class="title-box">
                        <span class="text"><?= \Yii::t('components/order', '选择发货商品');?></span>
                        <span>(<?= \Yii::t('components/order', '默认全选');?>)</span></div>
                    <el-table
                            ref="multipleTable"
                            :data="orderDetail"
                            tooltip-effect="dark"
                        ...
                        <el-table-column
                                label="<?= \Yii::t('components/order', '图片');?>"
                                width="60">
                        ...
                        <el-table-column
                                label="<?= \Yii::t('components/order', '名称');?>"
                                show-overflow-tooltip>
                            <template slot-scope="scope">
                                <el-tag v-if="scope.row.expressRelation" type="success" size="mini"><?= \Yii::t('components/order', '已发货');?></el-tag>
                                <span>{{scope.row.goods_info.goods_attr.name}}</span>
                            </template>
                        </el-table-column>
                        <el-table-column
                                prop="goods_info.goods_attr.number"
                                label="<?= \Yii::t('components/order', '数量');?>"
                                width="80"
                                show-overflow-tooltip>
                        </el-table-column>
                        <el-table-column
                                label="<?= \Yii::t('components/order', '规格');?>"
                                width="120"
                                show-overflow-tooltip>
                            <template slot-scope="scope">
                    ...
                    <div class="title-box">
                        <span class="text"><?= \Yii::t('components/order', '物流信息');?></span>
                    </div>
                    <el-form label-width="130px"
                        ...
                        <template v-if="order.send_type == 0 || order.send_type == 1">
                            <el-form-item label="<?= \Yii::t('components/order', '物流选择');?>">
                                <el-radio @change="resetForm('sendForm')" v-model="express.is_express" label="1"><?= \Yii::t('components/order', '快递');?></el-radio>
                                <el-radio @change="resetForm('sendForm')" v-model="express.is_express" label="2"><?= \Yii::t('components/order', '其它方式');?></el-radio>
                            </el-form-item>
                            <el-form-item label="<?= \Yii::t('components/order', '快递公司');?>" prop="express" v-if="express.is_express == 1">
                                <el-autocomplete
                                        size="small"
                                        v-model="express.express"
                                        :fetch-suggestions="querySearch"
                                        placeholder="<?= \Yii::t('components/order', '请选择快递公司');?>"
                                ></el-autocomplete>
                            </el-form-item>
                            <!--                    <el-form-item label="--><?//= \Yii::t('components/order', '收件人邮编');?><!--" v-if="express.is_express == 1">-->
                            <!--                        <el-input type="number" placeholder="--><?//= \Yii::t('components/order', '请输入收件人邮编');?><!--" size="small" v-model="express.code"-->
                            <!--                                  autocomplete="off"></el-input>-->
                            <!--                    </el-form-item>-->
                            <el-form-item label="<?= \Yii::t('components/order', '快递单号');?>" prop="express_no" class="express-no" v-if="express.is_express == 1">
                                <el-input placeholder="<?= \Yii::t('components/order', '请输入快递单号');?>" size="small" v-model.trim="express.express_no"
                                          autocomplete="off">
                                    <template v-if="isShowPrint" slot="append">
                                        <div flex="main:center" style="width: 100px">
                                            <el-button :loading="submitLoading" size="small" type="text" class="get-print"
                                                       @click="getPrint(express)"><?= \Yii::t('components/order', '获取面单');?>
                                            </el-button>
                                        </div>
                                    </template>
                                </el-input>
                                <el-button v-if="adminPrintTemplate" type="info" size="small" @click="adminPrint('preview')" plain><?= \Yii::t('components/order', '打印预览');?></el-button>
                                <el-button v-if="adminPrintTemplate" type="info" size="small" @click="adminPrint('design')" plain><?= \Yii::t('components/order', '打印设计');?></el-button>
                            </el-form-item>
                            <!--售后发货应该用插槽的方式-->
                            <el-form-item v-if="express.is_express == 1 && isRefund" prop="merchant_remark" label="<?= \Yii::t('components/order', '商家备注');?>">
                                <el-input type="textarea" size="small" v-model="express.merchant_remark"
                                          autocomplete="off"></el-input>
                            </el-form-item>
                            <el-form-item v-if="express.is_express == 1 && !isRefund" label="<?= \Yii::t('components/order', '商家备注');?>">
                                <el-input type="textarea" size="small" v-model="express.merchant_remark"
                                          autocomplete="off">
                                </el-input>
                            </el-form-item>
                            <el-form-item v-if="express.is_express == 2" prop="express_content" label="<?= \Yii::t('components/order', '物流内容');?>">
                                <el-input type="textarea" size="small" v-model="express.express_content"
                                          autocomplete="off"></el-input>
                            </el-form-item>
                        </template>
                        <template v-else>
                            <el-form-item label="<?= \Yii::t('components/order', '配送方式');?>">
                                <el-radio @change="resetForm('sendForm')" v-model="city.is_express" :label="2"><?= \Yii::t('components/order', '商家配送');?></el-radio>
                                <el-radio @change="resetForm('sendForm')" v-model="city.is_express" :label="1"><?= \Yii::t('components/order', '第三方配送');?></el-radio>
                            </el-form-item>
                            <!-- 商家配送 -->
                            <el-form-item v-if="city.is_express== 2" label="<?= \Yii::t('components/order', '配送员');?>" prop="city_name">
                                <template v-if="city.list.length > 0">
                                    <el-autocomplete @keyup.enter.native="prevD"
                                                     size="small"
                                                     v-model="city.man"
                                                     :fetch-suggestions="searchCity"
                                                     placeholder="<?= \Yii::t('components/order', '请选择配送员');?>">
                                    </el-autocomplete>
                                </template>
                                <template v-else>
                                    <div><?= \Yii::t('components/order', '未设置配送员');?>，
                                        <el-button type="text" @click="$navigate({r:'mall/delivery/index'}, true)"><?= \Yii::t('components/order', '请前往同城配送设置');?>
                                        </el-button>
                                    </div>
                                </template>
                            </el-form-item>
                            <!-- 第三方配送 -->
                            <template v-else>
                                <el-form-item label="<?= \Yii::t('components/order', '选择配送');?>" prop="delivery_name">
                                    <template v-if="city.city_service_list.length > 0">
                                        <el-autocomplete @keyup.enter.native="prevD"
                                                         ...
                                                         placeholder="<?= \Yii::t('components/order', '请选择配送名称');?>">
                                        </el-autocomplete>
                                    </template>
                                    <template v-else>
                                        <div><?= \Yii::t('components/order', '未设置配送商家');?>，
                                            <el-button type="text" @click="$navigate({r:'mall/city-service/edit'}, true)"><?= \Yii::t('components/order', '请前往配送商家设置');?>
                                            </el-button>
                                        </div>
                                    </template>
                                </el-form-item>
                                <template v-if="city.is_preview">
                                    <div style="margin: 5px 0 5px 60px;color: #62BC0F">{{city.name}} <?= \Yii::t('components/order', '可接单');?></div>
                                    <div style="margin: 5px 0 5px 60px;"><?= \Yii::t('components/order', '预计订单费用');?> ￥{{city.fee}}</div>
                                </template>
                            </template>
                        </template>
                        <el-form-item style="text-align: right">
                            <el-button size="small" @click="dialogVisible=false"><?= \Yii::t('components/order', '取消');?></el-button>
                            <!-- 第三方配送 -->
                ...
                <el-tab-pane label="<?= \Yii::t('mall/index', '聚合配送');?>" name="second" v-if="order.send_type == 2">
                    <app-send-second :active="activeName" :order="order" @close="closeDialog"></app-send-second>
                </el-tab-pane>
            </el-tabs>
        </el-dialog>
    </div>
    ...
    Vue.component('app-send', {
        ...
        data() {
            return {
                ...
                rules: {
                    express: [
                        {required: true, message: "<?= \Yii::t('components/order', '快递公司不能为空');?>", trigger: 'change'},
                    ],
                    express_no: [
                        {required: true, message: "<?= \Yii::t('components/order', '快递单号不能为空');?>", trigger: 'change'},
                        {pattern: /^[0-9a-zA-Z]+$/, message: "<?= \Yii::t('components/order', '仅支持数字与英文字母');?>"}
                    ],
                    merchant_remark: [
                        {required: true, message: "<?= \Yii::t('components/order', '商家留言不能为空');?>", trigger: 'change'},
                    ],
                    customer_name: [
                        {required: true, message: "<?= \Yii::t('components/order', '商家编码不能为空');?>", trigger: 'change'},
                    ],
                    express_content: [
                        {required: true, message: "<?= \Yii::t('components/order', '物流内容不能为空');?>", trigger: 'change'},
                    ],
                    // city_name: [
        ...
        methods: {
            ...
            send_order(e, formName) {
                ...
                if (res.length <= 0) {
                    this.$message.error("<?= \Yii::t('components/order', '请选择发货商品');?>");
                    // this.closeDialog()
                    return false;
                }
            ...
            adminPrint(type = 'design') {
                ...
                try {
                    ...
                    self.$msgbox({
                        title: "<?= \Yii::t('components/order', '提示');?>",
                        confirmButtonText: "<?= \Yii::t('components/order', '我知道了');?>",
                        message: res.message,
                        type: 'warning',
            ...
            getPrint(e) {
                ...
                request({
                    ...
                    if (e.data.code == 0) {
                        this.$message({
                            message: "<?= \Yii::t('components/order', '获取成功');?>",
                            type: 'success'
                        });
