...
<template id="app-attachment-edit">
    <div class="app-attachment-edit">
        <el-dialog :visible.sync="dialogVisible" title="<?= \Yii::t('components/other', '存储位置编辑');?>" :close-on-click-modal="false">
            <el-form ref="form" :model="form" :rules="rules" label-position="left">
                <el-form-item label="<?= \Yii::t('components/other', '存储位置');?>" prop="type">
                ...
                <template v-if="form.type == 1">
                    <el-form-item>
                        <div><?= \Yii::t('components/other', '本地服务器存储无需额外配置');?></div>
                    </el-form-item>
                </template>
                <template v-if="form.type == 2">
                    <el-form-item label="<?= \Yii::t('components/other', '存储空间名称');?>（Bucket）" prop="config.bucket" :rules="requiredRules">
                        <el-input v-model.trim="form.config.bucket"></el-input>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('components/other', '使用自定义域名');?>" prop="config.is_cname">
                        <el-switch v-model.trim="form.config.is_cname" active-value="1" inactive-value="0"></el-switch>
                    </el-form-item>
                    <el-form-item label="Endpoint <?= \Yii::t('components/other', '或自定义域名');?>" prop="config.domain" :rules="requiredRules">
                        <el-input v-model.trim="form.config.domain"></el-input>
                        <div style="font-size: 12px;color: #909399;line-height: normal;">
                            <span><?= \Yii::t('components/other', '示例');?>: </span>
                            <span style="color: #ff8576">http://oss-xx-xxx-1.aliyuncs.com</span>
                            <span><?= \Yii::t('components/other', '或');?></span>
                    ...
                    <el-form-item label="<?= \Yii::t('components/other', '图片样式接口');?>">
                ...
                <template v-if="form.type == 3">
                    <el-form-item label="<?= \Yii::t('components/other', '空间名称');?>（Bucket）" prop="config.bucket" :rules="requiredRules">
                        <el-input v-model.trim="form.config.bucket"></el-input>
                        <div style="font-size: 12px;color: #909399;line-height: normal;">
                            <span><?= \Yii::t('components/other', '示例');?>: </span>
                    ...
                    <el-form-item label="<?= \Yii::t('components/other', '所属地域');?>" prop="config.region" :rules="requiredRules">
                    ...
                    <el-form-item label="<?= \Yii::t('components/other', '自定义域名');?>" prop="config.domain">
                        <el-input v-model.trim="form.config.domain"></el-input>
                        <div style="font-size: 12px;color: #909399;line-height: normal;">
                            <span><?= \Yii::t('components/other', '示例');?>: </span>
                            <span style="color: #ff8576">http://mydomain.com</span>
                ...
                <template v-if="form.type == 4">
                    ...
                    <el-form-item label="<?= \Yii::t('components/other', '绑定域名');?>" prop="config.domain" :rules="requiredRules">
                        <el-input v-model.trim="form.config.domain"></el-input>
                        <div style="font-size: 12px;color: #909399;line-height: normal;">
                            <span><?= \Yii::t('components/other', '示例');?>: </span>
                            <span style="color: #ff8576">http://mydomain.com</span>
                ...
                <el-form-item>
                    <el-button type="primary" size="small" @click="handleSaveStorage('form')"><?= \Yii::t('components/other', '保存');?></el-button>
                </el-form-item>
            </el-form>
        </el-dialog>
...
<script>
    Vue.component('app-attachment-edit', {
        ...
        data() {
            return {
                requiredRules: [{required: true, message: "<?= \Yii::t('components/other', '不能为空');?>"}],
                dialogVisible: false,
                form: {
                    id: null,
                    type: null,
                ...
                rules: {
                    type: [
                        {required: true, message: "<?= \Yii::t('components/other', '请选择存储位置');?>",},
                    ]
                }
            };
        },
                ...
                this.$refs[formName].validate(valid => {
                    if (valid) {
                        this.$request({
                            ...
                            if (e.data.code === 0) {
                                this.$message.success("<?= \Yii::t('components/other', '保存成功');?>");
                        ...
                        this.$message.error("<?= \Yii::t('components/other', '提交内容有误');?>");
                    }
                });
            },
            handleEdit() {
