        ...
        <div class="app-setting">
            <el-form size="mini" :data="form" :label-width="label_width + 'px'">
                <el-card style="margin-bottom: 10px">
                    <div slot="header"><?= \Yii::t('components/other', '购买设置');?></div>

                    <!--是否开启分销-->
                    <el-form-item label="<?= \Yii::t('components/other', '是否开启分销');?>" prop="is_share" v-if="is_share && form.is_share != -1">
                        <el-switch
                                v-model="form.is_share"
                                :active-value="1"
                                :inactive-value="0">
                        </el-switch>
                        <span class="red"><?= \Yii::t('components/other', '注');?>“
                            <el-button type="text" @click="$navigate({r:'mall/share/basic'}, true)"><?= \Yii::t('components/other', '分销中心');?></el-button>
                            ”<?= \Yii::t('components/other', '中开启');?>
                        </span>
                    </el-form-item>

                    <!--是否开启区域允许购买-->
                    <el-form-item class="switch" label="<?= \Yii::t('components/other', '是否开启区域允许购买');?>" v-if="is_territorial_limitation">
                        <el-switch v-model="form.is_territorial_limitation" :active-value="1"
                                   :inactive-value="0"></el-switch>
                        <span class="ml-24 red"><?= \Yii::t('components/other', '注');?>“
                            <el-button type="text" @click="$navigate({r:'mall/territorial-limitation/index'}, true)">
                                <?= \Yii::t('components/other', '系统管理');?>
                            </el-button>
                            ”<?= \Yii::t('components/other', '中开启');?>
                        </span>
                    </el-form-item>
                    ...
                    <!--是否开启起送规则-->
                    <el-form-item class="switch" label="<?= \Yii::t('components/other', '是否开启起送规则');?>" v-if="is_offer_price">
                    ...
                    <!--支付方式-->
                    <el-form-item label="<?= \Yii::t('components/other', '支付方式');?>" prop="payment_type" v-if="is_payment">
                        <label slot="label"><?= \Yii::t('components/other', '支付方式');?>
                            <el-tooltip class="item" effect="dark" :content="is_surpport_huodao ? '<?= \Yii::t('components/other', '默认支持线上支付');?>' : ''" placement="top">
                                <i class="el-icon-info"></i>
                            </el-tooltip>
                        </label>
                        <el-checkbox-group v-model="form.payment_type" size="mini" :min="1" :max="3">
                            <el-checkbox label="online_pay" size="mini"><?= \Yii::t('components/other', '线上支付');?></el-checkbox>
                            <el-checkbox label="huodao" size="mini" v-if="is_surpport_huodao"><?= \Yii::t('components/other', '货到付款');?></el-checkbox>
                            <el-checkbox label="balance" size="mini"><?= \Yii::t('components/other', '余额支付');?></el-checkbox>
                        </el-checkbox-group>
                    </el-form-item>

                    <!--发货方式-->
                    <el-form-item label="<?= \Yii::t('components/other', '发货方式');?>" prop="send_type" v-if="is_send_type">
                        <label slot="label"><?= \Yii::t('components/other', '发货方式');?>
                            <el-tooltip
                                    class="item"
                                    effect="dark"
                                    content="<?= \Yii::t('components/other', '自提需要设置门店');?>"
                                    placement="top">
                                <i class="el-icon-info"></i>
                            </el-tooltip>
                        </label>
                    ...
                    <el-form-item label="<?= \Yii::t('components/other', '会员等级购买权限');?>" prop="is_vip_show" v-if="is_vip_show">
                        <el-switch
                    ...
                    <el-form-item label="<?= \Yii::t('components/other', '会员等级');?>" prop="vip_show_limit" v-if="is_vip_show && form.is_vip_show">
                        <div class="member-list">
                            <el-tag @close="handleClose(index)" class="member-item" v-for="(item,index) in form.vip_show_limit" :key="item.id" closable>
                                {{item.name}}
                            </el-tag>
                            <el-button @click="openMemberDialog"><?= \Yii::t('components/other', '选择会员等级');?></el-button>
                        </div>
                    </el-form-item>
                </el-card>
                <el-card style="margin-bottom: 10px" v-if="is_discount">
                    <div slot="header"><?= \Yii::t('components/other', '优惠叠加设置');?></div>
                    <el-form-item label="<?= \Yii::t('components/other', '优惠券');?>" v-if="is_coupon">
                        <el-switch v-model="form.is_coupon" :active-value="1"
                                   :inactive-value="0"></el-switch>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('components/other', '超级会员卡');?>" v-if="form.svip_status != -1 && form.svip_status != null && form.svip_status != undefined">
                        ...
                        <span class="ml-24 red"><?= \Yii::t('components/other', '注');?>“
                                <el-button type="text" @click="$navigate({r:'plugin/vip_card/mall/setting/index'}, true)">
                                    <?= \Yii::t('components/other', '插件中心');?>
                                </el-button>
                                ”<?= \Yii::t('components/other', '中开启');?>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('components/other', '会员价');?>" v-if="is_member_price">
                        <el-switch v-model="form.is_member_price" :active-value="1"
                                   :inactive-value="0"></el-switch>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('components/other', '积分抵扣');?>" v-if="is_integral">
                        <el-switch v-model="form.is_integral" :active-value="1"
                                   :inactive-value="0"></el-switch>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('components/other', '满减优惠');?>" v-if="is_full_reduce">
            ...
            <el-dialog class="app-setting-dialog" title="<?= \Yii::t('components/other', '选择会员等级');?>" :visible.sync="dialogVisible" width="30%">
                <el-form @submit.native.prevent size="mini">
                    <div class="input-item">
                        <el-input @keyup.enter.native="toSearch" size="small" placeholder="<?= \Yii::t('components/other', '请输入会员等级名称');?>"
                ...
                <div class="no-list-tip" v-if="list.length == 0 && !listLoading"><?= \Yii::t('components/other', '暂无会员等级');?></div>
                ...
                <el-button size="small" @click="dialogVisible = false"><?= \Yii::t('components/other', '取消');?></el-button>
                <el-button size="small" type="primary" @click="submitMember"><?= \Yii::t('components/other', '确定');?></el-button>
            </span>
            </el-dialog>
        </div>
</template>
            ...
            send_type_list() {
                ...
                for (let i in this.form.send_type) {
                    if (this.form.send_type[i] == 'express') {
                        list.push('<?= \Yii::t('components/other', '快递配送');?>');
                    }
                    if (this.form.send_type[i] == 'offline') {
                        list.push('<?= \Yii::t('components/other', '到店自提');?>');
                    }
                    if (this.sign !== 'mch' && this.form.send_type[i] == 'city') {
                        list.push('<?= \Yii::t('components/other', '同城配送');?>');
                    }
                }
                return list;
            }
