...
<template id="app-batch">
    <div class="app-batch" flex="dir:left cross:center">
        <el-button type="primary" size="small" @click="batchSetting" style="padding: 9px 15px !important;" ><?= \Yii::t('components/share', '批量设置');?></el-button>
        <el-dialog
                :visible.sync="dialogVisible"
                width="50%">
            <div slot="title">
                <div flex="dir:left">
                    <div class="batch-title"><?= \Yii::t('components/share', '批量修改');?></div>
                    <div flex="cross:center" class="select-count">{{dialogTitle}}</div>
                </div>
                <div class="batch-remark"><?= \Yii::t('components/share', '每次只能修改一项');?></div>
            </div>
                ...
                <div class="batch-box-right">
                    <el-form>
                        ...
                        <template v-if="currentBatch === 'level'">
                            <el-form-item label="<?= \Yii::t('components/share', '分销商等级');?>">
            ...
            <div slot="footer">
                <el-button size="small" @click="dialogVisible = false"><?= \Yii::t('components/share', '取消');?></el-button>
                <el-button size="small" :loading="btnLoading" type="primary" @click="dialogSubmit"><?= \Yii::t('components/share', '确定');?>
                </el-button>
            </div>
        </el-dialog>
    </div>
    ...
    Vue.component('app-batch', {
        ...
        data() {
            return {
                ...
                baseBatchList: [
                    {
                        name: "<?= \Yii::t('components/share', '分销商等级');?>",
                        key: 'level',// 唯一
                    },
                ],
                level: 0
        ...
        methods: {
            ...
            checkChooseList() {
                if (this.isAllChecked) {
                    this.dialogTitle = "<?= \Yii::t('components/share', '已选所有分销商');?>";
                    return true;
                }
                if (this.chooseList.length > 0) {
                    this.dialogTitle = "<?= \Yii::t('components/share', '已选分销商');?>" + this.chooseList.length + "<?= \Yii::t('components/share', '个');?>";
                    return true;
                }
                this.$message.warning("<?= \Yii::t('components/share', '请先勾选要设置的分销商');?>");
                return false;
            },
            batchAction(data) {
                let self = this;
                self.$confirm(data.content, "<?= \Yii::t('components/share', '提示');?>", {
                    confirmButtonText: "<?= \Yii::t('components/share', '确定');?>",
                    cancelButtonText: "<?= \Yii::t('components/share', '取消');?>",
                    type: 'warning'
                }).then(() => {
            ...
            dialogSubmit() {
                ...
                switch (this.currentBatch) {
                    case 'level':
                        if (this.level === '') {
                            self.$message.warning("<?= \Yii::t('components/share', '请选择分销商等级');?>");
                            return false;
                        }
                        params.level = this.level;
                        this.batchAction({
                            url: self.batchLevelUrl,
                            content: "<?= \Yii::t('components/share', '批量设置分销商等级');?>",
                            params: params
                        });
                        break;
                    default:
