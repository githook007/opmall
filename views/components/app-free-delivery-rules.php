        ...
        <div class="table-body">
            <div style="justify-content:space-between;display: flex">
                <div class="input-item">
                    <el-input @keyup.enter.native="search" size="small" placeholder="<?= \Yii::t('components/other', '输入规则名称搜索');?>" v-model="keyword" clearable @clear="search">
                        <el-button slot="append" icon="el-icon-search" @click="search"></el-button>
                    </el-input>
                </div>
                <el-button style="float: right; margin: -5px 0; width: 80px; height: 32px;" type="primary" size="small"
                           @click="$navigate({r:'mall/free-delivery-rules/edit'})"><?= \Yii::t('components/other', '添加规则');?>
                </el-button>
            </div>
            ...
            <el-table v-loading="loading" border :data="list" style="width: 100%;margin-bottom: 15px">
                ...
                <el-table-column prop="name" label="<?= \Yii::t('components/other', '规则名称');?>" width='250px'></el-table-column>

                <el-table-column label="<?= \Yii::t('components/other', '包邮类型');?>" prop="type_text" width='470px'></el-table-column>

                <el-table-column
                    prop="status"
                    label="<?= \Yii::t('components/other', '是否默认');?>"
                    width="380">
                    <template slot-scope="scope">
                        <el-radio v-model="scope.row.status" :label="1" @change="setStatus(scope.row)"><?= \Yii::t('components/other', '默认');?></el-radio>
                    </template>
                </el-table-column>

                <el-table-column label="<?= \Yii::t('components/other', '操作');?>" width="280px">
                    <template slot-scope="scope">
...
<script>
    Vue.component('app-free-delivery-rules', {
        ...
        methods: {
            ...
            destroy(id){
                this.$confirm("<?= \Yii::t('components/other', '此操作将删除该包邮规则, 是否继续');?>", "<?= \Yii::t('components/other', '提示');?>", {
                    ...
                    this.$message({
                        type: 'info',
                        message: "<?= \Yii::t('components/other', '已取消删除');?>"
                    });
                });
            },
