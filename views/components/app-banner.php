...
<template id="app-banner">
    <div class="app-banner">
        <el-dialog title="<?= \Yii::t('components/other', '选择轮播图');?>" :visible.sync="dialogVisible" @opened="dialogOpened" :close-on-click-modal="false"
                   width="718px" append-to-body style="z-index: 999;">
            <!-- 轮播图编辑 -->
            <el-dialog title="<?= \Yii::t('components/other', '轮播图编辑');?>" :visible.sync="bannerVisible" append-to-body>
                <el-form :model="bannerForm" label-width="100px" :rules="bannerFormRules" ref="bannerForm">
                    <el-form-item label="<?= \Yii::t('components/other', '标题');?>" prop="title">
                        <el-input size="small" v-model="bannerForm.title" autocomplete="off"></el-input>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('components/other', '跳转链接');?>" prop="page_url">
                        <el-input size="small" v-model="bannerForm.page_url" class="input-with-select" autocomplete="off">
                            <app-pick-link slot="append" @selected="selectPageUrl">
                                <el-button size="mini"><?= \Yii::t('components/other', '选择链接');?></el-button>
                            </app-pick-link>
                        </el-input>
                    </el-form-item>

                    <el-form-item label="<?= \Yii::t('components/other', '图片');?>" prop="pic_url">
                        <app-attachment title="<?= \Yii::t('components/other', '选择图片');?>" @selected="singlePicUrl">
                            <el-tooltip v-if="url == 'plugin/step/mall/banner/index'" class="item"
                                        effect="dark"
                                        content="<?= \Yii::t('components/other', '建议尺寸');?>:750 * 190"
                                        placement="top">
                            ...
                            <el-tooltip v-else-if="url == 'plugin/pintuan/mall/banner/index'" class="item"
                                        effect="dark"
                                        content="<?= \Yii::t('components/other', '建议尺寸');?>:750 * 230"
                                        placement="top">
                                <el-button size="mini"><?= \Yii::t('components/other', '选择图片');?></el-button>
                            </el-tooltip>
                            <el-tooltip v-else-if="url == 'plugin/bargain/mall/index/banner-store'" class="item"
                                        effect="dark"
                                        content="<?= \Yii::t('components/other', '建议尺寸');?>:750 * 280"
                                        placement="top">
                                <el-button size="mini"><?= \Yii::t('components/other', '选择图片');?></el-button>
                            </el-tooltip>
                            ...
                            <el-tooltip v-else class="item"
                                        effect="dark"
                                        content="<?= \Yii::t('components/other', '建议尺寸');?>:750 * 350"
                                        placement="top">
                                <el-button size="mini"><?= \Yii::t('components/other', '选择图片');?></el-button>
                            </el-tooltip>
                        </app-attachment>
                    ...
                    <el-form-item style="margin-bottom: 0">
                        <el-button style="float: right;width: 80px;margin-left: 20px" size="small" type="primary" @click="bannerSubmit" :loading="bannerLoading"><?= \Yii::t('components/other', '提交');?></el-button>
                        <el-button style="float: right;width: 80px;margin-left: 20px" size="small" @click="bannerVisible = false"><?= \Yii::t('components/other', '取消');?></el-button>
                    </el-form-item>
                </el-form>
            </el-dialog>
            <div class="app-banner-header">
                <div style="float:right">
                    <template v-if="showEditBlock">
                        <el-button @click="showEditBlock=false" size="mini"><?= \Yii::t('components/other', '退出编辑');?></el-button>
                        <el-button @click="bannerDestroy" size="mini" type="danger"><?= \Yii::t('components/other', '删除');?></el-button>
                    </template>
                    <el-button v-else @click="showEditBlock=true" size="mini"><?= \Yii::t('components/other', '开启编辑');?></el-button>
                </div>
            </div>
            <el-card shadow="never">
                <el-scrollbar class="app-banner-scrollbar" v-loading="loading">
                    <div v-if="showEditBlock==true" class="app-banner-item-image" style="height: 217px;text-align: center;" @click="bannerEdit">
                        ...
                        <div style="color: #C0C4CC;font-size: 16px"><?= \Yii::t('components/other', '添加轮播图');?></div>
                    </div>
                    <div v-for="(item, index) in banners" :key="index"
                        ...
                        <div style="padding:15px;font-size:12px;height: 63px">
                            <div class="app-banner-text"><?= \Yii::t('components/other', '标题');?>：{{item.title}}</div>
                            <div class="app-banner-text"><?= \Yii::t('components/other', '路径');?>：{{item.page_url}}</div>
                        </div>
                        <el-button v-if="showEditBlock" style="right:75px;" class="app-banner-dialog-btn" size="small" circle type="text" @click.stop="selectItem(item)">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('components/other', '编辑');?>" placement="top">
                ...
                <div style="padding: 5px;text-align: center">
                    <el-button v-if="noMore" type="text" :disabled="true"><?= \Yii::t('components/other', '已无更多');?></el-button>
                    <el-button v-else type="text" @click="handleLoadMore" :loading="loadMore"><?= \Yii::t('components/other', '加载更多');?></el-button>
                </div>
            </el-card>
            <div class="app-banner-end">
                <el-button @click="confirm" :disabled="checkedAttachments.length == 0" style="width: 80px;" size="small" type="primary"><?= \Yii::t('components/other', '选定');?></el-button>
            </div>
        </el-dialog>
        <el-card shadow="never" style="border:0" :body-style="title ? `background-color: #f3f3f3;padding: 10px 0 0;`:`background-color: #f3f3f3;padding: 0 0;`" v-loading="listLoading">
            <div v-if="title" slot="header" class="clearfix">
                <span><?= \Yii::t('components/other', '轮播图管理');?></span>
                <div style="float: right;margin-top: -5px">
                    <el-button type="primary" @click="editItem" size="small"><?= \Yii::t('components/other', '编辑');?></el-button>
                </div>
            </div>
            <div class="text item">
                <el-form label-width="20px" size="small" class="demo-ruleForm">
                    ...
                    <el-form-item class="app-banner-form-button">
                        <el-button class="button-item" type="primary" @click="onSubmit" size="small"><?= \Yii::t('components/other', '保存');?></el-button>
                    </el-form-item>
                </el-form>
            </div>
        </el-card>
    ...
    Vue.component('app-banner', {
        ...
        data() {
            return {
                ...
                bannerFormRules: {
                    title: [
                        {min: 1, max: 30, message: "<?= \Yii::t('components/other', '标题长度在1');?>"},
                    ],
                    sort: [
                        {required: false, pattern: /^[1-9]\d{0,8}$/, message: "<?= \Yii::t('components/other', '排序必须在9位正整数内');?>"}
                    ],
                    pic_url: [
                        {required: true, message: "<?= \Yii::t('components/other', '图片不能为空');?>", trigger: ['blur', 'change']},
                    ]
                },
                page: 1,
                dialogVisible: false,
        ...
        methods: {
            listDestroy(index) {
                this.$confirm("<?= \Yii::t('components/other', '确认删除该轮播图吗');?>", "<?= \Yii::t('components/other', '提示');?>", {
                    type: 'warning'
                }).then(() => {
            ...
            bannerDestroy(row) {
                ...
                if(row.id > -1) {
                    ...
                    if (!para || Object.keys(para).length == 0) {
                        this.$message.error("<?= \Yii::t('components/other', '请至少选择一项');?>");
                        return;
                    }
                }
                this.$confirm("<?= \Yii::t('components/other', '确认删除该选记录吗');?>", "<?= \Yii::t('components/other', '提示');?>", {
                    type: 'warning'
                }).then(() => {
                    request({
                        params: {
            ...
            onSubmit(number) {
                ...
                request({
                    ...
                    if (e.data.code === 0) {
                        if(number == 1) {
                            this.$message.success("<?= \Yii::t('components/other', '删除成功');?>");
                        }else {
