...
<template id="app-user-batch">
    <div class="app-user-batch">
        <el-button @click="batchSetting" size="small" type="primary"><?= \Yii::t('components/other', '批量设置');?></el-button>
        ...
        <el-dialog
            ...
            <div slot="title">
                <div flex="dir:left">
                    <div class="batch-title"><?= \Yii::t('components/other', '批量修改');?></div>
                    <div flex="cross:center" class="select-count">{{dialogTitle}}</div>
                </div>
                <div class="batch-remark"><?= \Yii::t('components/other', '每次只能修改一项');?></div>
            </div>
                ...
                <div class="batch-box-right">
                    <el-form>
                        <div v-if="currentBatch === 'member-level'">
                            <el-form-item label-width="120px" label="<?= \Yii::t('components/other', '选择会员等级');?>">
                                <el-select size="small" v-model="member.level" placeholder="<?= \Yii::t('components/other', '请选择会员等级');?>">
                                    <el-option key="<?= \Yii::t('components/other', '普通用户');?>" label="<?= \Yii::t('components/other', '普通用户');?>" :value="0"></el-option>
                                    <el-option
                        ...
                        <div v-if="currentBatch === 'integral'">
                            <el-form-item label="<?= \Yii::t('components/other', '操作');?>" prop="type" label-width="80px">
                                <el-radio v-model="integral.type" label="1"><?= \Yii::t('components/other', '充值');?></el-radio>
                                <el-radio v-model="integral.type" label="2"><?= \Yii::t('components/other', '扣除');?></el-radio>
                            </el-form-item>
                            <el-form-item label="<?= \Yii::t('components/other', '积分数');?>" prop="num" size="small" label-width="80px">
                                <el-input oninput="this.value = this.value.replace(/[^0-9]/g, '');" v-model="integral.num" :max="999999999"></el-input>
                            </el-form-item>
                            <el-form-item label="<?= \Yii::t('components/other', '充值图片');?>" prop="pic_url" label-width="80px">
                                <app-attachment :multiple="false" :max="1" @selected="integralPicUrl">
                                    <el-button size="mini"><?= \Yii::t('components/other', '选择文件');?></el-button>
                                </app-attachment>
                                <app-image width="80px" height="80px" mode="aspectFill" :src="integral.pic_url"></app-image>
                            </el-form-item>
                            <el-form-item label="<?= \Yii::t('components/other', '备注');?>" prop="remark" size="small" label-width="80px">
                                <el-input v-model="integral.remark"></el-input>
                            </el-form-item>
                        </div>
                        <div v-if="currentBatch === 'balance'">
                            <el-form-item label="<?= \Yii::t('components/other', '操作');?>" prop="type" label-width="80px">
                                <el-radio v-model="balance.type" label="1"><?= \Yii::t('components/other', '充值');?></el-radio>
                                <el-radio v-model="balance.type" label="2"><?= \Yii::t('components/other', '扣除');?></el-radio>
                            </el-form-item>
                            <el-form-item label="<?= \Yii::t('components/other', '金额');?>" prop="num" size="small" label-width="80px">
            ...
            <div slot="footer">
                ...
                <el-button size="small" :loading="btnLoading" type="primary" @click="dialogSubmit"><?= \Yii::t('components/other', '确定');?>
                </el-button>
            </div>
        </el-dialog>
    ...
    Vue.component('app-user-batch', {
        ...
        data() {
            return {
                ...
                baseBatchList: [
                    {
                        name: '<?= \Yii::t('components/other', '会员等级');?>',
                        key: 'member-level',// 唯一
                    },
                    {
                        name: '<?= \Yii::t('components/other', '积分');?>',
                        key: 'integral',
                    },
                    {
                        name: '<?= \Yii::t('components/other', '余额');?>',
                        key: 'balance',
                    }
                ]
            }
        ...
        methods: {
            checkChooseList() {
                if (this.isAllChecked) {
                    this.dialogTitle = '<?= \Yii::t('components/other', '已选所有用户');?>';
                    return true;
                }
                if (this.chooseList.length > 0) {
                    this.dialogTitle = '<?= \Yii::t('components/other', '已选用户');?>' + this.chooseList.length + '<?= \Yii::t('components/other', '人');?>';
                    return true;
                }
                this.$message.warning('<?= \Yii::t('components/other', '请先勾选要设置的用户');?>');
                return false;
            },
            batchAction(data) {
                let self = this;
                ...
                self.$confirm(data.content, '<?= \Yii::t('components/other', '提示');?>', {
                ...
                switch (this.currentBatch) {
                    case 'member-level':
                        if (self.member.level === '') {
                            self.$message.warning('<?= \Yii::t('components/other', '请选择会员等级');?>');
                            return false;
                        }
                        params.member_level = this.member.level;
                        var text = this.isAllChecked ? '<?= \Yii::t('components/other', '批量设置所有用户会员等级');?>' : '<?= \Yii::t('components/other', '批量设置会员等级');?>';
                        this.batchAction({
                        ...
                        break;
                    case 'integral':
                        var text = this.isAllChecked ? '<?= \Yii::t('components/other', '批量设置所有用户积分');?>' : '<?= \Yii::t('components/other', '是否继续');?>';
                        ...
                        break;
                    case 'balance':
                        var text = this.isAllChecked ? '<?= \Yii::t('components/other', '批量设置所有用户余额');?>' : '<?= \Yii::t('components/other', '批量设置余额');?>';
