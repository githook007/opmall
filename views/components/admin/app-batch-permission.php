...
<template id="app-batch-permission">
    <div class="app-batch-permission">
        <el-button size="small" type="primary" @click="batchSetting"><?= \Yii::t('components/admin', '批量权限设置');?></el-button>
        ...
        <el-dialog
            ...
            <div slot="title">
                <div flex="dir:left">
                    <div class="batch-title"><?= \Yii::t('components/admin', '批量权限设置');?></div>
                </div>
                ...
                <div class="batch-box-right" v-loading="baseLoading">
                    <div v-if="currentBatch === 'base-permission'">
                        <el-checkbox :indeterminate="isBaseIndeterminate" v-model="baseCheckAll"
                                     @change="handleBaseCheckAllChange"><?= \Yii::t('components/admin', '全选');?>
                        </el-checkbox>
                    ...
                    <div v-if="currentBatch === 'upload-permission'">
                        <el-checkbox :indeterminate="isUploadIndeterminate" v-model="uploadCheckAll"
                                     @change="handleUploadCheckAllChange"><?= \Yii::t('components/admin', '全选');?>
                        </el-checkbox>
                        <div class="permissions-list">
                    ...
                    <div v-if="currentBatch === 'template-permission'">
                        <template v-if="templateShow">
                            <el-form ref="form" label-position="left" label-width="80px" size="small">
                                <el-form-item label="<?= \Yii::t('components/admin', '显示权限');?>">
                                    <div flex>
                                        <el-button size="mini" type="text" style="margin-right: 10px;"
                                                   @click="templatePermission.show = true"
                                                   v-if="templatePermission.is_all == 0"><?= \Yii::t('components/admin', '添加模板');?>
                                        </el-button>
                                        <el-checkbox v-model="templatePermission.is_all"
                                                     true-label="1" false-label="0"><?= \Yii::t('components/admin', '全部选择');?>
                                        </el-checkbox>
                                    </div>
                                ...
                                <el-form-item label="<?= \Yii::t('components/admin', '使用权限');?>">
                                    <div flex>
                                        <el-button size="mini" type="text" style="margin-right: 10px;"
                                                   @click="templatePermission.show_use = true"
                                                   v-if="templatePermission.use_all == 0"><?= \Yii::t('components/admin', '添加模板');?>
                                        </el-button>
                        ...
                        <template v-else>
                            <?= \Yii::t('components/admin', '需设置此权限');?>
                        </template>
                    </div>
                </div>
            </div>
            <div slot="footer">
                <el-button size="small" @click="dialogVisible = false"><?= \Yii::t('components/admin', '取消');?></el-button>
                <el-button size="small" :loading="btnLoading" type="primary" @click="dialogSubmit"><?= \Yii::t('components/admin', '确定');?>
                </el-button>
            </div>
        </el-dialog>
        ...
        <el-dialog title="<?= \Yii::t('components/admin', '全局权限设置');?>" :visible.sync="globalDialogVisible">
            <el-form v-loading="globalLoading" label-position="left" label-width="180px" size="small">
                <el-form-item label="<?= \Yii::t('components/admin', '是否开启默认全局权限');?>">
                    <el-switch
                ...
                <el-form-item v-if="globalPermission.is_open" label="<?= \Yii::t('components/admin', '默认全局权限设定');?>">
                    <el-radio v-model="globalPermission.permission_type" :label="1"><?= \Yii::t('components/admin', '有所有权限');?></el-radio>
                    <el-radio v-model="globalPermission.permission_type" :label="2"><?= \Yii::t('components/admin', '无权限');?></el-radio>
                </el-form-item>
            </el-form>
            <div slot="footer">
                <el-button size="small" @click="globalDialogVisible = false"><?= \Yii::t('components/admin', '取消');?></el-button></el-button>
                <el-button size="small" :loading="btnLoading" type="primary" @click="saveGlobalPermission"><?= \Yii::t('components/admin', '确定');?>
                </el-button>
            </div>
        </el-dialog>
    </div>
    ...
    Vue.component('app-batch-permission', {
        ...
        data() {
            return {
                ...
                baseBatchList: [
                    {
                        name: "<?= \Yii::t('components/admin', '基础权限');?>",
                        key: 'base-permission',// 唯一
                    },
                    {
                        name: "<?= \Yii::t('components/admin', '上传权限');?>",
                        key: 'upload-permission',
                    },
                    {
                        name: "<?= \Yii::t('components/admin', '模板权限');?>",
        ...
        methods: {
            checkChooseList() {
                ...
                this.$message.warning("<?= \Yii::t('components/admin', '请先勾选要设置的账户');?>");
                return false;
            },
            // 打开批量设置框
            batchSetting() {
