    ...
    <div class="app-notice">
        <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="850px" top="20vh">
            ...
            <span slot="footer" class="dialog-footer">
                <el-button size="small" @click="dialogVisible = false"><?= \Yii::t('admin/notice', '取消');?></el-button>
                <el-button :loading="btnLoading" size="small" type="primary" @click="submit('add')"><?= \Yii::t('admin/notice', '发布');?></el-button>
            </span>
        </el-dialog>
        <!-- 查看公告 -->
        <el-dialog :visible.sync="dialogPreviewVisible" width="1240px" top="10vh">
            ...
            <span slot="footer" class="dialog-footer">
                <el-button size="small" type="primary" @click="dialogPreviewVisible = false"><?= \Yii::t('admin/notice', '知道了');?></el-button>
            </span>
        </el-dialog>
        <!-- 编辑公告 -->
        <el-dialog title="<?= \Yii::t('admin/notice', '编辑公告');?>" :visible.sync="dialogEditVisible" width="1358px" top="10vh" v-if="dialogEditVisible">
            <span slot="title">
                <div style="font-size: 16px;">
            ...
            <el-form ref="editForm" :model="editForm" label-width="100px" :rules="rules">
                <el-form-item label="<?= \Yii::t('admin/notice', '公告类型');?>" prop="type">
                ...
                <el-form-item label="<?= \Yii::t('admin/notice', '公告内容');?>" prop="content">
                    <app-rich-text v-model="editForm.content" label-icon="<?= \Yii::t('admin/notice', '插入图片');?>"></app-rich-text>
                </el-form-item>
            </el-form>
            <span slot="footer" class="dialog-footer">
                <el-button size="small" @click="dialogEditVisible = false"><?= \Yii::t('admin/notice', '取消');?></el-button>
                <el-button :loading="btnLoading" size="small" type="primary" @click="editSave"><?= \Yii::t('admin/notice', '发布');?></el-button>
            </span>
        </el-dialog>
                    ...
                    <div>
                        ...
                        <div v-loading="hLoading">
                            <div v-for="(history, key) in historyList" :key="key" flex="dir:left cross:center"
                                ...
                                <el-button type="text" style="margin-left: 12px" @click="openDialog(history)"><?= \Yii::t('admin/notice', '查看详情');?>
                                </el-button>
                                <el-button style="margin-left: auto" type="text" @click="editDialog(history)"><?= \Yii::t('admin/notice', '编辑');?>
                                </el-button>
                                <el-button style="margin-left: 20px" type="text" @click="delData(history,key)"><?= \Yii::t('admin/notice', '删除');?>
                                </el-button>
    ...
    Vue.component('app-notice', {
        ...
        data() {
            return {
                ...
                rules: {
                    type: [
                        {required: true, message: "<?= \Yii::t('admin/notice', '公告类型不能为空');?>", trigger: 'blur'},
                    ],
                    content: [
                        {required: true, message: "<?= \Yii::t('admin/notice', '公告内容不能为空');?>", trigger: 'change'},
                    ],
                },
                options: [
                    {
                        'label': "<?= \Yii::t('admin/notice', '更新公告');?>",
                        'value': 'update',
                    }, {
                        'label': "<?= \Yii::t('admin/notice', '紧急维护');?>",
                        'value': 'urgent',
                    }, {
                        'label': "<?= \Yii::t('admin/notice', '重要通知');?>",
                        'value': 'important'
                    },
                ],
                btnLoading: false,
        ...
        methods: {
            ...
            delData(column, index) {
                this.$confirm("<?= \Yii::t('admin/notice', '此操作将删除公告');?>", "<?= \Yii::t('admin/notice', '提示');?>", {
                    confirmButtonText: "<?= \Yii::t('admin/notice', '确定');?>",
                    cancelButtonText: "<?= \Yii::t('admin/notice', '取消');?>",
                    type: 'warning'
                }).then(() => {
                    request({
                        params: {
                    ...
                    this.$message({
                        type: 'info',
                        message: "<?= \Yii::t('admin/notice', '已取消删除');?>"
                    });
                });
            },
            pageChange(page) {
            ...
            clerkSubmit() {
                this.$refs.form.validate((valid) => {
                    if (valid) {
                        this.$confirm("<?= \Yii::t('admin/notice', '是否确认发布公告');?>", "<?= \Yii::t('admin/notice', '提示');?>", {
                            confirmButtonText: "<?= \Yii::t('admin/notice', '确定');?>",
                            cancelButtonText: "<?= \Yii::t('admin/notice', '取消');?>",
                            type: 'warning'
                        }).then(() => {
                            ...
                            this.$message({
                                type: 'info',
                                message: "<?= \Yii::t('admin/notice', '已取消发布');?>"
                            });
                        });
                    }
                });
