...
<template id="app-comment-reply">
    <el-card class="app-comment-reply" v-loading="listLoading" shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header" class="clearfix">
            <span><?= \Yii::t('components/other', '评价回复');?></span>
        </div>
        <div class="form-body">
            <el-form :model="form" label-width="150px" :rules="FormRules" ref="form">
                <el-form-item label="<?= \Yii::t('components/other', '用户');?>">
                    <div v-text="form.nickname"></div>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('components/other', '商品名称');?>">
                    <div v-text="form.goods_name"></div>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('components/other', '评分');?>">
                    <el-tag type="success" v-if="form.score==3"><?= \Yii::t('components/other', '好评');?></el-tag>
                    <el-tag type="warning" v-if="form.score==2"><?= \Yii::t('components/other', '中评');?></el-tag>
                    <el-tag type="danger" v-if="form.score==1"><?= \Yii::t('components/other', '差评');?></el-tag>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('components/other', '详情');?>">
                ...
                <el-form-item label="<?= \Yii::t('components/other', '回复评价');?>" prop="reply_content">
    ...
    Vue.component('app-comment-reply', {
        ...
        data() {
            return {
                ...
                FormRules: {
                    reply_content: [
                        { required: true, message: "<?= \Yii::t('components/other', '回复评价不能为空');?>", trigger: 'blur' },
                    ],
                },
            };
        },
