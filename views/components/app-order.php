    ...
    <div class="app-order-list" style="margin-bottom: 20px;">
        <div class="header-box" v-if="isShowHeader">
            ...
            <el-button
                    ...
                    type="primary"
                    size="small"
                    @click="toRecycleAll"><?= \Yii::t('components/order', '清空回收站');?>
            </el-button>
        </div>
            ...
            <div class="app-order-title">
                <div v-if="isShowCheckBox" style="width: 5%;">
                    <el-checkbox :indeterminate="checkBoxSelect.isIndeterminate" v-model="checkBoxSelect.checkAll"
                        ...
                        <span style="position: absolute;top: 11px;"><?= \Yii::t('components/order', '全选');?></span>
                    </el-checkbox>
                </div>
                ...
                <el-card
                    ...
                    <div slot="header" class="app-order-head" flex="cross:center">
                        ...
                        <div class="app-order-user">
                            <span class="app-order-time"><?= \Yii::t('components/order', '订单号');?>：</span>{{ item.order_no }}
                        </div>
                        ...
                        <div flex="cross:center" class="remark-box" v-if="item.remark || (item.merchant_remark_list && item.merchant_remark_list.length > 0)">
                            <el-tooltip effect="dark" placement="bottom">
                                <div slot="content">
                                    <span v-if="item.remark"><?= \Yii::t('components/order', '买家留言');?>:{{item.remark}}</span>
                                    <br v-if="item.remark"/>
                                    <span v-for="(merchantRemark, deIndex) in item.merchant_remark_list" :key="deIndex">
                                            <?= \Yii::t('components/order', '买家留言');?>{{item.merchant_remark_list.length > 1 ? deIndex + 1 : ''}}: {{merchantRemark}}
                        ...
                        <div class="app-order-offline" flex="dir:left wrap:wrap">
                            <template v-if="item.cancel_status == 2 && item.status == 1 && isShowCancel">
                                ...
                                <div class="cancel" flex="wrap:wrap">
                                    <span style="margin-right: 5px;"><?= \Yii::t('components/order', '用户正在申请取消该订单');?></span>
                                    <el-button type="success" size="mini" @click="agreeCancel(item,1)"><?= \Yii::t('components/order', '同意');?></el-button>
                                    <el-button type="danger" size="mini" @click="agreeCancel(item,0)"><?= \Yii::t('components/order', '拒绝');?></el-button>
                                </div>
                            </template>
                            <template v-else-if="isShowOrderStatus">
                                <el-tag size="small" type="success" v-if="item.replace_user_id > 0">代付单</el-tag>
                                <div v-if="item.send_type == 0" class="express-send-box">
                                    <el-tag size="small"><?= \Yii::t('components/order', '快递发送');?></el-tag>
                                    <div v-if="item.store_id > 0" class="triangle">
                                        <el-tooltip class="item" effect="dark" content="<?= \Yii::t('components/order', '该订单由门店自提改成快递发送');?>" placement="top">
                                            <span class="text"><?= \Yii::t('components/order', '改');?></span>
                                        </el-tooltip>
                                    </div>
                                </div>
                                <el-tag size="small" v-if="item.send_type == 1"><?= \Yii::t('components/order', '到店自提');?></el-tag>
                                <el-tag size="small" v-if="item.send_type == 2"><?= \Yii::t('components/order', '同城配送');?></el-tag>
                                <el-tag size="small" v-if="item.send_type == 3"><?= \Yii::t('components/order', '自动发货');?></el-tag>
                                <el-tag size="small" type="warning" v-if="item.is_pay == 0"><?= \Yii::t('components/order', '未付款');?></el-tag>
                                <el-tag size="small" type="warning" v-if="item.is_pay == 1 && item.is_send == 0"><?= \Yii::t('components/order', '已付款');?>
                                </el-tag>
                                <el-tag size="small" type="success"
                                        v-if="item.is_send == 0 && item.is_pay == 1 && item.detailExpress && item.detailExpress.length > 0">
                                    <?= \Yii::t('components/order', '部分发货');?>
                                </el-tag>
                                <el-tag size="small" type="success"
                                        v-if="item.is_send == 0 && item.is_pay == 1 && item.detailExpress && item.detailExpress.length == 0">
                                    <?= \Yii::t('components/order', '未发货');?>
                                </el-tag>
                                <el-tag size="small" type="success" v-if="item.is_send == 1 && item.is_confirm == 0">
                                    <?= \Yii::t('components/order', '已发货');?>
                                </el-tag>
                                <el-tag size="small" type="success" v-if="item.is_confirm == 0 && item.is_send == 1">
                                    <?= \Yii::t('components/order', '未收货');?>
                                </el-tag>
                                <el-tag size="small" type="success" v-if="item.is_sale == 1"><?= \Yii::t('components/order', '已完成');?></el-tag>
                                <el-tag size="small" type="danger" v-if="item.cancel_status == 1"><?= \Yii::t('components/order', '已取消');?></el-tag>
                                <el-tag size="small" type="danger" v-else-if="item.cancel_status == 2"><?= \Yii::t('components/order', '申请取消');?></el-tag>
                            </template>
                        ...
                        <el-button
                                ...
                                type="text">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('components/order', '强制取消');?>" placement="top">
                        ...
                        <el-button v-if="isShowRecycle && item.is_recycle == 0"
                                   ...
                                   type="text">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('components/order', '放入回收站');?>" placement="top">
                    ...
                    <div class="app-order-body">
                                ...
                                <div flex="dir:left">
                                    <div class="goods-info">
                                        ...
                                        <div style="margin-bottom: 24px;">
                                            <span style="margin-right: 10px;max-width: 300px;overflow: hidden;">
                                                <slot name="attr" :item="item">
                                                    <?= \Yii::t('components/order', '规格');?>：
                                                    <el-tag size="mini"
                                                            style="margin-right: 5px;white-space: nowrap;text-overflow: ellipsis"
                                        ...
                                        <div class="app-order-goods-price">
                                            <span v-if="goods.goods_info && goods.goods_info.goods_attr &&
                                             goods.goods_info.goods_attr.no">
                                                <?= \Yii::t('components/order', '货号');?>：{{goods.goods_info.goods_attr.no}}
                                            </span>
                                        </div>
                                    </div>
                                    <div style="width: 250px" flex="dir:left box:mean">
                                        ...
                                        <div flex="cross:center main:center"><?= \Yii::t('components/order', '数量');?>：x {{goods.num}}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div flex="cross:center" class="app-order-info" :style="{width:orderTitle[1].width}">
                            <div flex="dir:top">
                                <div>
                                    ...
                                    <el-popover
                                        ...
                                        <el-form class="app-order-price-item" label-width="100px" :model="item">
                                            <el-form-item label="<?= \Yii::t('components/order', '商品小计');?>">
                                                <span>{{ item.total_goods_original_price }}<?= \Yii::t('components/order', '元');?></span>
                                            </el-form-item>
                                            <el-form-item label="<?= \Yii::t('components/order', '会员优惠');?>" v-if="item.member_discount_price != 0.00">
                                                <span class="important" v-if="item.member_discount_price > 0">
                                                    -{{ item.member_discount_price }}<?= \Yii::t('components/order', '元');?>
                                                </span>
                                                <span class="addPrice" v-if="item.member_discount_price < 0">
                                                    +{{ -item.member_discount_price }}<?= \Yii::t('components/order', '元');?>
                                                </span>
                                            </el-form-item>
                                            <el-form-item label="<?= \Yii::t('components/order', '积分抵扣');?>" v-if="item.integral_deduction_price != 0.00">
                                                <span class="important">-{{ item.integral_deduction_price }}<?= \Yii::t('components/order', '元');?></span>
                                            </el-form-item>
                                            <el-form-item label="<?= \Yii::t('components/order', '优惠券抵扣');?>" v-if="item.coupon_discount_price != 0.00">
                                                <span class="important">-{{ item.coupon_discount_price }}<?= \Yii::t('components/order', '元');?></span>
                                            </el-form-item>
                                            <el-form-item label="<?= \Yii::t('components/order', '满减抵扣');?>" v-if="item.full_reduce_price != 0.00">
                                                <span class="important">-{{ item.full_reduce_price }}<?= \Yii::t('components/order', '元');?></span>
                                            </el-form-item>
                                            <el-form-item label="<?= \Yii::t('components/order', '订单改价');?>" v-if="item.back_price != 0.00">
                                                <span class="important" v-if="item.back_price > 0">-{{ item.back_price }}<?= \Yii::t('components/order', '元');?></span>
                                                <span class="addPrice" v-else>+{{ -item.back_price }}<?= \Yii::t('components/order', '元');?></span>
                                            </el-form-item>
                                            <el-form-item label="<?= \Yii::t('components/order', '运费改价');?>" v-if="item.express_price != 0">
                                                <span class="important"
                                                      v-if="+item.express_price > +item.express_original_price">
                                                    +{{(item.express_price - item.express_original_price).toFixed(2)}}<?= \Yii::t('components/order', '元');?>
                                                </span>
                                            ...
                                            <template v-if="item.plugin_data && item.plugin_data.discount_list">
                                                <el-form-item v-for="pluginData in item.plugin_data.discount_list"
                                                    ...
                                                    <span class="important" v-if="pluginData.value > 0">-{{ pluginData.value }}<?= \Yii::t('components/order', '元');?></span>
                                                    <span class="addPrice" v-else>+{{ -pluginData.value }}<?= \Yii::t('components/order', '元');?></span>
                                                </el-form-item>
                                            </template>
                                        </el-form>
                                ...
                                <div>
                                    <span>
                                        <span style="color: #909399" v-if="item.is_show_express === 1">(<?= \Yii::t('components/order', '含运费');?>￥{{item.express_price}})</span>
                                        <el-button type="text"
                                ...
                                <div>
                                    ...
                                    ><?= \Yii::t('components/order', '在线支付');?>
                                    </el-tag>
                                    <el-tag
                                            v-else-if="item.pay_type == 3"
                                            size="mini" color="#E6A23C"
                                            style="color:#fff;border:0"
                                    ><?= \Yii::t('components/order', '余额支付');?>
                                    </el-tag>
                                    <el-tag
                                            v-else-if="item.pay_type == 2"
                                            size="mini" color="#E6A23C"
                                            style="color:#fff;border:0"
                                    ><?= \Yii::t('components/order', '货到付款');?>
                                    </el-tag>
                                </div>
                                <span v-if="item.sign == 'community' && isShowProfit" style="color: #ff4544;margin-top: 5px;">(<?= \Yii::t('components/order', '团长利润');?>￥{{item.plugin_data.extra &&item.plugin_data.extra.profit_price ? item.plugin_data.extra.profit_price : '0.00'}})</span>
                            </div>
                        </div>
                        ...
                        <div v-if="isShowAction" class="app-order-info" :style="{width: showMoreInfo ? orderTitle[3].width : orderTitle[2].width}"
                            ...
                            <div flex="wrap:wrap cross:center">
                                <!-- 结束 -->
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('components/order', '结束订单');?>" placement="top">
                                ...
                                <!-- 确认收货 -->
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('components/order', '确认收货');?>" placement="top">
                                ...
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('components/order', '电子面单');?>" placement="top">
                                ...
                                <!-- 核销 -->
                                <el-tooltip class="item" effect="dark" :content="item.is_pay == 1 ? '<?= \Yii::t('components/order', '核销订单');?>' : '<?= \Yii::t('components/order', '确认收款');?>'"
                                            placement="top">
                                ...
                                <template>
                                    ...
                                    <!-- 发货 -->
                                    <el-tooltip class="item" effect="dark" content="<?= \Yii::t('components/order', '发货');?>" placement="top">
                                ...
                                <!-- 打印小票 -->
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('components/order', '打印小票');?>" placement="top">
                                ...
                                <el-tooltip class="item"
                                            ...
                                            effect="dark" content="<?= \Yii::t('components/order', '打印发货单');?>" placement="top">
                                    <img class="app-order-icon"
                                ...
                                <!-- 恢复订单 -->
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('components/order', '恢复订单');?>" placement="top">
                                    <img class="app-order-icon" v-if="item.is_recycle == 1"
                                         @click="toRecycle(item)"
                                         src="statics/img/mall/order/renew.png" alt="">
                                </el-tooltip>
                                <!-- 删除订单 -->
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('components/order', '删除订单');?>" placement="top">
                                    <img class="app-order-icon" v-if="item.is_recycle == 1"
                                         @click="toDelete(item)"
                                ...
                                <el-tooltip class="item" effect="dark"
                                            :content="item.seller_remark != '' || item.bonus_remark ? '<?= \Yii::t('components/order', '修改备注');?>' : '<?= \Yii::t('components/order', '添加备注');?>'"
                                            placement="top">
                                ...
                                <!-- 修改快递单号 -->
                                <template
                                        v-if="item.detailExpress && item.detailExpress.length == 1 && item.is_send == 1 && item.cancel_status != 1 && item.is_confirm == 0 && item.is_recycle == 0 && isShowSend && item.status != 0 && item.is_update_send == 1">
                                    <el-tooltip class="item" effect="dark" :content="item.send_type == 2 ? '<?= \Yii::t('components/order', '修改配送员');?>' : '<?= \Yii::t('components/order', '修改快递单号');?>'" placement="top">
                                        <img class="app-order-icon"
                                ...
                                <!-- 订单详情 -->
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('components/order', '查看订单详情');?>" placement="top">
                    ...
                    <div class="card-footer">
                        <template v-if="item.goods_type == 'ecard'">
                            <div style="margin: 10px 0;"><?= \Yii::t('components/order', '收货人');?>: {{item.name}} <?= \Yii::t('components/order', '电话');?>：{{item.mobile}}</div>
                        </template>
                        <template v-if="item.sign == 'scan_code_pay'">
                            <div style="margin: 10px 0;">
                                <template v-if="item.clerk">
                                    <span><?= \Yii::t('components/order', '核销员');?>:</span>
                                    <span>{{item.clerk.nickname}}</span>
                                </template>
                                <template v-if="item.store">
                                    <span style="margin-left: 10px;"><?= \Yii::t('components/order', '核销门店');?>:</span>
                        ...
                        <template v-if="item.send_type == 1">
                            <div flex="cross:center">
                                <el-tag style="margin-right: 10px;" size="small" hit type="warning"><?= \Yii::t('components/order', '到店自提');?></el-tag>
                                <span class="address-box" v-if="item.store"><?= \Yii::t('components/order', '门店名称');?>：{{item.store.name}} <?= \Yii::t('components/order', '电话');?>：{{item.store.mobile}} <?= \Yii::t('components/order', '地址');?>：{{item.store.address}}</span>
                            </div>
                            <div style="margin: 10px 0;"><?= \Yii::t('components/order', '收货人');?>: {{item.name}} <?= \Yii::t('components/order', '电话');?>：{{item.mobile}}</div>
                            <div v-if="item.orderClerk && item.orderClerk.clerk_remark" style="margin: 10px 0;"><?= \Yii::t('components/order', '核销备注');?>: {{item.orderClerk.clerk_remark}}</div>
                        </template>
                        <div v-else-if="(item.send_type == 0 || item.send_type == 2) && item.address">
                            <div flex="dir:left" style="word-break: break-word;">
                                <div class="address-box"><?= \Yii::t('components/order', '收货人');?>: {{item.name}} <?= \Yii::t('components/order', '电话');?>：{{item.mobile}} <?= \Yii::t('components/order', '地址');?>：{{item.address}}</div>
                                <el-button
                        ...
                        <div class="seller-remark" v-if="item.seller_remark"><?= \Yii::t('components/order', '商家备注');?>：{{item.seller_remark ? item.seller_remark : item.words}}</div>
        ...
        <el-dialog
                title="<?= \Yii::t('components/order', '电子面单');?>"
            ...
            <div v-for="(expressSingle, index) in newExpressSingle"
                ...
                <div>
                    <div class="label" style="background: #fffaef;color: #e6a23c;padding: 3px 0;">
                        <?= \Yii::t('components/order', '收货信息');?>:{{index + 1}}
                    </div>
                </div>
                <div flex="dir:top">
                    <div flex="cross:center">
                        <template v-if="expressSingle.send_type == 1">
                            ...
                            <a :href="'https://www.baidu.com/s?wd='+ expressSingle.express + expressSingle.express_no"
                               target="_blank" title='<?= \Yii::t('components/order', '点击搜索运单号');?>'>{{ expressSingle.express_no }}</a>
                        </template>
                        <template v-else>
                        ...
                        <el-button @click="printTeplate(expressSingle.print_teplate)" v-if="expressSingle.print_teplate"
                                   style="margin-left: 10px;" size="mini" type="default"><?= \Yii::t('components/order', '打印此面单');?>
                        </el-button>
                    </div>
                    <div flex="dir:left" style="margin-top: 10px;flex-wrap: wrap">
                        ...
                        <img v-for="(goods, index) in expressSingle.goods_list"
                             ...
                             class="goods-pic"
                             :src="goods.cover_pic">
                        <span style="margin-top: auto"><?= \Yii::t('components/order', '等共');?>{{ expressSingle.goods_list.length}}<?= \Yii::t('components/order', '个商品');?></span>
                    </div>
                </div>
            </div>
        </el-dialog>

        <el-dialog width="25%" title="<?= \Yii::t('components/order', '修改商品价格');?>" :visible.sync="editGoodsPriceVisible">
            <el-form :model="editGoodsForm" ref="goodsValidateForm" label-width="80px" size="small">
                <el-form-item
                        label="<?= \Yii::t('components/order', '商品价格');?>"
                        prop="total_price"
                        :rules="[{ required: true, message: '<?= \Yii::t('components/order', '价格不能为空');?>'}]">
                    <el-input type="number" v-model="editGoodsForm.total_price" auto-complete="off"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button size="small" @click="editGoodsPriceVisible = false"><?= \Yii::t('components/order', '取消');?></el-button>
                <el-button size="small" :loading="submitLoading" type="primary"
                           @click="changePrice('goodsValidateForm')"><?= \Yii::t('components/order', '确定');?>
                </el-button>
            </div>
        </el-dialog>
        <div id="orderprint" v-show="false">
            <div v-for="(item) in printData"
                ...
                <div :style="{width: mmConversionPx(printPar.stencil_width) + 'px',minHeight:mmConversionPx(printPar.stencil_high) + 'px', cursor: 'pointer', border: `${mmConversionPx(printPar.border_width)}px solid #000000`, boxSizing: 'content-box', margin: 0}">
                    ...
                    <div v-if="printPar.order.date || printPar.order.time || printPar.order.orderNumber"
                    ...
                    <div :style="{display: 'flex', boxSizing: 'border-box'}"
                        ...
                        >
                            <div style="font-size:10px;line-height:1.5;" v-if="printPar.personalInf.name">
                                <?= \Yii::t('components/order', '收货人信息');?>：{{item.name}}
                            </div>
                            <div style="font-size:10px;line-height:1.5;" v-if="printPar.personalInf.nickname">
                                <?= \Yii::t('components/order', '收货人昵称');?>：{{item.nickname}}
                            </div>
                            <div style="font-size:10px;line-height:1.5;" v-if="printPar.personalInf.phone">
                                <?= \Yii::t('components/order', '联系方式');?>：{{item.mobile}}
                            </div>
                            <div style="font-size:10px;line-height:1.5;" v-if="printPar.personalInf.payMethod">
                                <?= \Yii::t('components/order', '支付方式');?>：{{item.pay_type == 1 ? "<?= \Yii::t('components/order', '在线支付');?>" : item.pay_type == 2 ? "<?= \Yii::t('components/order', '货到付款');?>" : item.pay_type == 3 ?
                                "<?= \Yii::t('components/order', '余额支付');?>" : ''}}
                            </div>
                            <div style="font-size:10px;line-height:1.5;"
                                 v-if="printPar.personalInf.shipMethod && item.send_type != 1"><?= \Yii::t('components/order', '发货方式');?>：{{item.send_type ==
                                0 ? "<?= \Yii::t('components/order', '快递配送');?>" : item.send_type == 1 ? "<?= \Yii::t('components/order', '到店自提');?>" : item.send_type == 2 ? "<?= \Yii::t('components/order', '同城配送');?>" : ''}}
                            </div>
                            <div style="font-size:10px;line-height:1.5;"
                                 v-if="printPar.personalInf.address && item.send_type != 1"><?= \Yii::t('components/order', '收货地址');?>：{{item.address}}
                            </div>
                            <div style="font-size:10px;line-height:1.5;"
                                 v-if="printPar.personalInf.address && item.send_type == 1">
                                <?= \Yii::t('components/order', '自提门店地址');?>：{{item.store_address}}
                            </div>
                        </div>
                    ...
                    <div>
                        <div style="box-sizing:border-box;width: 100%;display: flex;border-bottom:1px solid #000000;position: relative;left: -0.5px;"
                            ...
                            <div style="box-sizing:border-box;width: 6%;border-left: 1px solid #000000;height: 30px;line-height: 30px;padding-left: 0.5%;font-size:10px;"
                                 v-if="printPar.goodsInf.serial"><?= \Yii::t('components/order', '序号');?>
                            </div>
                            ...
                            <div style="box-sizing:border-box;width: 8%;border-left: 1px solid #000000;height: 30px;line-height: 30px;padding-left: 0.5%;font-size:10px;"
                                 v-if="printPar.goodsInf.number"><?= \Yii::t('components/order', '数量');?>
                            </div>
                            <div style="box-sizing:border-box;width: 12%;border-left: 1px solid #000000;height: 30px;line-height: 30px;padding-left: 0.5%;font-size:10px;"
                                 v-if="printPar.goodsInf.univalent"><?= \Yii::t('components/order', '小计');?>
                            </div>
                            <div style="box-sizing:border-box;width: 18%;border-left: 1px solid #000000;height: 30px;line-height: 30px;padding-left: 0.5%;font-size:10px;"
                                 v-if="printPar.goodsInf.article_number"><?= \Yii::t('components/order', '货号');?>
                            </div>
                            ...
                            <div style="box-sizing:border-box;width: 8%;height: 30px;border-left: 1px solid #000000;line-height: 30px;padding-left: 0.5%;font-size:10px;"
                                 v-if="printPar.goodsInf.unit"><?= \Yii::t('components/order', '单位');?>
                            </div>
                        </div>
                        ...
                        <div style="box-sizing:border-box;display: flex;height: 30px;padding-left: .5%;border-bottom:1px solid #000000;font-size: 10px;"
                            ...
                            <div style="width: 24%;height: 30px;line-height:30px;" v-if="printPar.goodsInf.amount">
                                <?= \Yii::t('components/order', '订单金额');?>：￥{{item.total_goods_original_price}}
                            </div>
                            ...
                            <div style="width: 21%;height: 30px;line-height:30px;" v-if="printPar.goodsInf.fare">
                                <?= \Yii::t('components/order', '运费');?>：￥{{item.express_price}}
                            </div>
                            <div style="width: 20%;height: 30px;line-height:30px;" v-if="printPar.goodsInf.discount">
                                <?= \Yii::t('components/order', '优惠');?>：￥{{item.send_template_discount_price}}
                            </div>
                            <div style="width: 19%;height: 30px;line-height:30px;"
                                 v-if="printPar.goodsInf.actually_paid"><?= \Yii::t('components/order', '实付');?>：￥{{item.total_pay_price}}
                            </div>
                        </div>
                    </div>
                    <div :style="{boxSizing: 'border-box',display:'flex',borderBottom:'1px solid #000000'}"
                        ...
                        <div :style="{width:`${!printPar.sellerInf.remark ? '100%': '62%'}`,padding: ' 10px 10px 10px .5%', fontSize: '10px',borderRight: `${!printPar.sellerInf.remark ? 'none' : '1px solid #000000'}`}"
                            ...
                            <div v-if="printPar.sellerInf.address">
                                <?= \Yii::t('components/order', '网点地址');?>：{{address_list[0].province}}{{address_list[0].city}}{{address_list[0].district}}{{address_list[0].address}}
                            </div>
                        </div>
    ...
    Vue.component('app-order', {
        ...
        props: {
            orderTitle: {
                ...
                default: function () {
                    return [
                        {width: '55%', name: "<?= \Yii::t('components/order', '订单信息');?>"},
                        {width: '20%', name: "<?= \Yii::t('components/order', '实付金额');?>"},
                        {width: '20%', name: "<?= \Yii::t('components/order', '操作');?>"}
                    ]
                }
            },
            selectList: {
                ...
                default: function () {
                    return [
                        ...
                        {value: '3', name: "<?= \Yii::t('components/order', '收货人');?>"},
                        {value: '6', name: "<?= \Yii::t('components/order', '收货人电话');?>"},
                        {value: '7', name: "<?= \Yii::t('components/order', '门店名称');?>"},
                        {value: 'goods_no', name: "<?= \Yii::t('components/order', '商品货号');?>"},
                        {value: 'address', name: "<?= \Yii::t('components/order', '收货地址');?>"},
                    ]
                }
            },
            tabs: {
                ...
                default: function () {
                    return [
                        ...
                        {value: '9', name: "<?= \Yii::t('components/order', '已收货');?>"},
            ...
            dateTypeList: {
                ...
                default: function() {
                    return [
                        {
                            label: "<?= \Yii::t('components/order', '付款时间');?>",
                            value: 'pay_time'
                        },
                        {
                            label: "<?= \Yii::t('components/order', '发货时间');?>",
                            value: 'send_time'
                        },
                        {
                            label: "<?= \Yii::t('components/order', '收货时间');?>",
                            value: 'confirm_time'
                        },
                        {
                            label: "<?= \Yii::t('components/order', '结束时间');?>",
                            value: 'finish_time'
                        },
                    ]
                }
        ...
        data() {
            return {
                ...
                printTime: "2020<?= \Yii::t('components/order', '年');?>2<?= \Yii::t('components/order', '月');?>10<?= \Yii::t('components/order', '日');?>",
                printData: [],
                address_list: [],
        ...
        methods: {
            getNumberList(params) {
                ...
                para.r = 'mall/order/order-num'
                request({
                    params: para,
                }).then(e => {
                    if (e.data.code === 0) {
                        this.numberList = e.data.data.numList;
                    }
                })
            },
            ...
            // 回收站
            toRecycle(e) {
                let text = "<?= \Yii::t('components/order', '是否放入回收站(可在回收站中恢复)');?>?";
                let para = {
                    order_id: e.id,
                    is_recycle: 1
                };
                if (e.is_recycle == 1) {
                    para.is_recycle = 0;
                    text = "<?= \Yii::t('components/order', '是否移出回收站');?>?"
                }
                this.$confirm(text, '<?= \Yii::t('components/order', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('components/order', '确定');?>',
                    cancelButtonText: '<?= \Yii::t('components/order', '取消');?>',
                    type: 'warning',
                    center: true
            ...
            // 删除订单
            toDelete(e) {
                this.$confirm('<?= \Yii::t('components/order', '是否删除订单');?>', '<?= \Yii::t('components/order', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('components/order', '确定');?>',
                    cancelButtonText: '<?= \Yii::t('components/order', '取消');?>',
                    type: 'warning'
                }).then(() => {
            ...
            getList() {
                ...
                if (this.orderUrl == 'mall/order/index') {
                    this.getNumberList(params);
                }
                request({
                    params: params,
                }).then(e => {
            ...
            // 清空回收站
            toRecycleAll(e) {
                this.$confirm('<?= \Yii::t('components/order', '此操作将清空回收站, 是否继续');?>', '<?= \Yii::t('components/order', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('components/order', '确定');?>',
                    cancelButtonText: '<?= \Yii::t('components/order', '取消');?>',
                    type: 'warning',
                    center: true
            ...
            openExpressHint() {
                this.$alert('<?= \Yii::t('components/order', '该订单有多个物流,请到订单详情修改物流信息');?>', '<?= \Yii::t('components/order', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('components/order', '确定');?>',
                    callback: action => {
                    }
                });
            ...
            setPrintInvoice() {
                if (this.checkBoxSelect.chooseList.length === 0) {
                    this.$message({
                        message: '<?= \Yii::t('components/order', '请先勾选订单');?>',
                        type: 'warning'
                    });
                    return true;
                }
