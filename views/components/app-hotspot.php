...
<template id="app-hotspot">
    <div class="app-hotspot">
        ...
        <el-dialog append-to-body title="<?= \Yii::t('components/other', '热区划分');?>" :visible.sync="dialogVisible" :close-on-click-modal="false"
            ...
            <div flex="dir:left box:first">
                ...
                <el-form label-width="120px">
                    <div class="right" v-if="index > -1">
                        <el-form-item label="<?= \Yii::t('components/other', '热区尺寸');?>">
                            <el-row type="flex">
                                <el-col :span="12">
                                    <div flex="dir:left">
                                        <label>W</label>
                        ...
                        <el-form-item label="<?= \Yii::t('components/other', '热区位置');?>">
                            <el-row type="flex">
                                <el-col :span="12">
                                    <div flex="dir:left">
                                        <label>X</label>
                        ...
                        <el-form-item label="<?= \Yii::t('components/other', '热区链接');?>" v-if="isLink">
                            <app-pick-link style="margin-left: 10px" title="<?= \Yii::t('components/other', '选择链接');?>" @selected="selectLink">
                                <el-input size="small" style="width: 100%;" :disabled="true" v-model="hotspotList[index].link.name">
                                    <template slot="append">
                                        <el-button><?= \Yii::t('components/other', '选择链接');?></el-button>
                                    </template>
                                </el-input>
                            </app-pick-link>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('components/other', '热区属性');?>" v-if="mode == 'auth'">
                            <el-radio-group v-model="hotspotList[index].open_type">
                                <el-radio label="login" :disabled="radioDisabled('login')"><?= \Yii::t('components/other', '登录按钮');?></el-radio>
                                <el-radio label="cancel" :disabled="radioDisabled('cancel')"><?= \Yii::t('components/other', '不登录按钮');?></el-radio>
                                <el-radio label="link" :disabled="radioDisabled('link')"><?= \Yii::t('components/other', '自定义链接');?></el-radio>
                            </el-radio-group>
                        </el-form-item>

                        <el-form-item prop="params" label="<?= \Yii::t('components/other', '自定义按钮');?>" v-if="mode == 'auth'">
                            <el-input size="small" v-model="hotspotList[index]['link']['url']"
                                      class="input-with-select"
                                      autocomplete="off" disabled>
                                <app-pick-link slot="append" @selected="selectPageUrl">
                                    <el-button size="mini" @click="searchIndex(index)"><?= \Yii::t('components/other', '选择链接');?></el-button>
                                </app-pick-link>
                            </el-input>
                        </el-form-item>
                    </div>
                    <div v-if="index == -1 && hotspotList.length == 0" style="padding: 20px 40px;"><?= \Yii::t('components/other', '请先在左侧蓝框内用鼠标划出热区范围');?>
                    </div>
                    <el-form-item v-else label="">
                        <el-button style="margin-left: 10px" type="primary" size="small" @click="confirm"><?= \Yii::t('components/other', '保存');?></el-button>
                        <el-button size="small" @click="clearAll"><?= \Yii::t('components/other', '重置');?></el-button>
                    </el-form-item>
                </el-form>
            </div>
        </el-dialog>
