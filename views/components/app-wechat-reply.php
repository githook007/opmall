...
<template id="app-wechat-reply">
    <el-form @submit.native.prevent :model="form" label-width="140px" :rules="rules" ref="form" size="small">
        <el-form-item label="<?= \Yii::t('mall/wechat', '回复内容');?>" prop="type">
            <el-radio-group v-model="form.type">
                <el-radio :label="0"><?= \Yii::t('mall/wechat', '文字');?></el-radio>
                <el-radio :label="1"><?= \Yii::t('mall/wechat', '图片');?></el-radio>
                <el-radio :label="2"><?= \Yii::t('mall/wechat', '语音');?></el-radio>
                <el-radio :label="3"><?= \Yii::t('mall/wechat', '视频');?></el-radio>
                <el-radio :label="4"><?= \Yii::t('mall/wechat', '图文');?></el-radio>
            </el-radio-group>
        </el-form-item>
        ...
        <el-form-item class="required-icon" v-if="form.type == 2 || form.type == 3" label="<?= \Yii::t('mall/wechat', '语音');?>" prop="voice_url">
            <template slot="label">
                <span>{{form.type == 2 ? '<?= \Yii::t('mall/wechat', '语音');?>':'<?= \Yii::t('mall/wechat', '视频');?>'}}</span>
            </template>
            <div v-if="form.type == 2">
                <app-attachment display="block" :multiple="false" :max="1" @selected="voiceUrl" type="voice">
                    <el-input @blur="changeValue" style="width: 400px;" :disabled="voice_url ? true : false" v-model="voice_url" placeholder="<?= \Yii::t('mall/wechat', '请选择文件');?>">
                        <template slot="append">
                                <el-tooltip class="item"
                                            effect="dark"
                                            content="<?= \Yii::t('mall/wechat', '支持格式mp3;音频大小不能超过2 MB');?>"
                                            placement="top">
                                    <el-button size="mini"><?= \Yii::t('mall/wechat', '选择文件');?></el-button>
                                </el-tooltip>
                        </template>
                    </el-input>
                </app-attachment>
            </div>
            <el-input @blur="changeValue" v-else style="width: 400px;" v-model="video_url" placeholder="<?= \Yii::t('mall/wechat', '请选择文件或输入链接');?>">
                <template slot="append">
                    <app-attachment :multiple="false" :max="1" @selected="videoUrl"
                                    type="video">
                        <el-tooltip class="item"
                                    effect="dark"
                                    content="<?= \Yii::t('mall/wechat', '支持格式mp4;支持编码H.264;视频大小不能超过10 MB');?>"
                                    placement="top">
                            <el-button size="mini"><?= \Yii::t('mall/wechat', '选择文件');?></el-button>
                        </el-tooltip>
                    </app-attachment>
        ...
        <el-form-item class="required-icon" v-if="form.type == 4" label="<?= \Yii::t('mall/wechat', '标题');?>" prop="title">
            <el-input @blur="changeValue" style="width: 400px;" v-model="form.title"></el-input>
        </el-form-item>
        <el-form-item class="required-icon content-input" v-if="form.type == 4" label="<?= \Yii::t('mall/wechat', '文字');?>" prop="content">
        ...
        <el-form-item class="required-icon" v-if="form.type == 1 || form.type == 4" prop="picurl">
            <template slot="label">
                <span><?= \Yii::t('mall/wechat', '图片');?></span>
                <el-tooltip v-if="form.type == 4" effect="dark" placement="top"
                            content="<?= \Yii::t('mall/wechat', '图片支持JPG、PNG格式，较好的效果为大图360*200,小图200*200');?>">
        ...
        <el-form-item class="required-icon" v-if="form.type == 4" label="<?= \Yii::t('mall/wechat', '跳转链接');?>" prop="url">
            <el-input @blur="changeValue" @input="showValue" style="width: 400px;" placeholder="<?= \Yii::t('mall/wechat', '请填写');?>https<?= \Yii::t('mall/wechat', '开头的有效链接');?>" v-model="form.url"></el-input>
        </el-form-item>
    </el-form>
</template>
<script>
    ...
    data() {
        var checkUrl = (rule, value, callback) => {
            if(!this.form.url || this.form.url.indexOf('https://') == -1) {
                callback(new Error('<?= \Yii::t('mall/wechat', '请填写');?>https://<?= \Yii::t('mall/wechat', '开头的有效链接');?>'));
            } else {
                callback();
            }
        };
