...
<template id="app-number-input">
    <div class="app-number-input-area" :class="pad ? 'pad': ''" flex="dir:top main:center">
        ...
        <div v-if="mode == 'password'" style="text-align: center;color: #999999">{{ name == 'verify' ? "<?= \Yii::t('components/other', '请再次输入6位支付密码');?>" : "<?= \Yii::t('components/other', '请引导顾客输入6位数支付密码');?>"}}</div>
        ...
        <div class="app-member-number" flex="dir:left cross:center" :style="{'width': inputWidth + 'px'}">
            ...
            <div @click="confirmNumber" class="app-member-confirm" :style="{'width': buttonWidth + 'px','height': confirmHeight + 'px', 'line-height': confirmHeight + 'px'}"><?= \Yii::t('components/other', '确认');?></div>
        </div>
    </div>
</template>
<script>
    Vue.component('app-number-input', {
        ...
        watch: {
            mode: {
                handler(v) {
                    if(v == 'price') {
                        this.list = ['1','2','3','','4','5','6',"<?= \Yii::t('components/other', '确认');?>",'7','8','9',"<?= \Yii::t('components/other', '确认');?>",'0','.','00',"<?= \Yii::t('components/other', '确认');?>"]
                    }else {
                        this.list = ['1','2','3','','4','5','6',"<?= \Yii::t('components/other', '确认');?>",'7','8','9',"<?= \Yii::t('components/other', '确认');?>",'0',"<?= \Yii::t('components/other', '确认');?>"]
                    }
                },
                immediate: true,
            },
        ...
        methods: {
            ...
            clickNumber(str) {
                this.focus = false;
                if(str) {
                    if(str != "<?= \Yii::t('components/other', 'a277');?>") {
