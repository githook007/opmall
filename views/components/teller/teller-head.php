...
<template id="teller-head">
    <div class="teller-title" :class="pad ? 'pad':''" flex="main:justify cross:center">
        ...
        <div class="teller-cash-info" flex="dir:right cross:center">
            <div @click="loginout"><?= \Yii::t('components/teller', '退出登录');?></div>
            <div v-if="setting && setting.is_shifts == 1" @click="transition"  :style="{'color': cashier_info ? '#353535' : '#3399ff'}"><?= \Yii::t('components/teller', '交班');?></div>
            <div @click="showInfo" :style="{'color': is_cashier ? '#353535' : '#3399ff'}"><?= \Yii::t('components/teller', '个人信息');?></div>
            <div v-if="nickname"><?= \Yii::t('components/teller', '收银员');?>：{{nickname}}</div>
        </div>
    </div>
</template>
<script>
