    ...
    <div class="teller-order" v-infinite-scroll="load" :infinite-scroll-disabled="stop">
        <div v-for="(count,idx) in list" class="hung-list" :key="idx">
            <div v-if="count.order_no" class="order-info" flex="dir:left cross:center">
                <div><?= \Yii::t('components/teller', '订单号');?>：<span>{{count.order_no}}</span></div>
                <div v-if="count.sales_name">x<?= \Yii::t('components/teller', '导购员');?>：<span>{{count.sales_name}}</span></div>
                <div><?= \Yii::t('components/teller', '收银员');?>：<span>{{count.cashier_name}}</span></div>
            </div>
            ...
            <div style="margin-bottom: 4px;"><?= \Yii::t('components/teller', '订单总价');?> <span style="color: #ff4544;">￥{{type == 'order' ? count.total_pay_price : count.total}}</span><span style="margin-left: 16px;color: #999999"><?= \Yii::t('components/teller', '等');?>{{type == 'order' ? count.goods_count: count.list.length}}件商品</span></div>
            <div style="width: 60%;">
            ...
            <div class="handle-hung" v-if="type == 'order'">
                <el-button size="small" @click="addRemark(idx, 'order')" round>{{count.seller_remark ? "<?= \Yii::t('components/teller', '修改备注');?>":"<?= \Yii::t('components/teller', '添加备注');?>"}}</el-button>
                <el-button size="small" @click="toOrderDetail(count.order_id)" round><?= \Yii::t('components/teller', '详情');?></el-button>
            </div>
            <div class="handle-hung" v-if="type == 'hung'">
                <el-button size="small" @click="addCount(idx)" round><?= \Yii::t('components/teller', '取单');?></el-button>
                <el-button size="small" @click="addRemark(idx)" round>{{count.remark ? "<?= \Yii::t('components/teller', '修改备注');?>":"<?= \Yii::t('components/teller', '添加备注');?>"}}</el-button>
                <el-button size="small" @click="delHung(idx)" round><?= \Yii::t('components/teller', '删除');?></el-button>
            </div>
        </div>
    </div>
</template>
