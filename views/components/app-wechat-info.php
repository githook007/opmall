...
<template id="app-wechat-info">
    <div>
        <div class="app-wechat-info" :class="{position : float}" :style="{top: top + 'px'}" flex="dir:left cross:center">
            <div class="text">{{setting ? '<?= \Yii::t('mall/wechat', '点击查看公众号配置信息');?>': '<?= \Yii::t('mall/wechat', '点此快速配置公众号');?>'}}</div>
            <div flex="cross:center">
                <el-button @click="toWechat" type="primary" size="small">{{setting ? '<?= \Yii::t('mall/wechat', '立即查看');?>' :'<?= \Yii::t('mall/wechat', '立即配置');?>'}}</el-button>
            </div>
        </div>
