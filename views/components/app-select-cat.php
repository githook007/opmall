...
<template id="app-select-cat">
    <el-dialog title="<?= \Yii::t('components/other', '选择分类');?>" width="1100px" :visible.sync="dialogVisible">
        <el-row v-loading="dialogLoading" :gutter="20" style="margin-top: -30px;">
            <template v-if="options.length > 0">
                <el-col :span="8">
                    <h3><?= \Yii::t('components/other', '一级分类');?></h3>
                ...
                <el-col :span="8" v-if="children.length > 0">
                    <h3><?= \Yii::t('components/other', '二级分类');?></h3>
                    <div class="app-goods-cat-list">
                        <el-checkbox-group v-model="cat_list"
                                           size="mini" flex="dir:left" style="flex-wrap: wrap">
                ...
                <el-col :span="8" v-if="third.length > 0">
                    <h3><?= \Yii::t('components/other', '三级分类');?></h3>
                    <div class="app-goods-cat-list">
                        <el-checkbox-group v-model="cat_list"
                                           size="mini" flex="dir:left" style="flex-wrap: wrap">
            ...
            <template v-else>
                <div flex="main:center" style="align-items: center;margin-top: 20px;">
                    <span><?= \Yii::t('components/other', '无系统分类');?></span>
                    <el-button style="display: inline-block;margin-left: 10px" flex="main:center" type="primary"
                               size="small"
                               @click="$navigate({r: 'mall/cat/edit'})">
                        <?= \Yii::t('components/other', '请先添加商品分类');?>
                    </el-button>
                </div>
            </template>
        </el-row>
        <span slot="footer" class="dialog-footer">
                <el-button size='small' @click="catDialogCancel"><?= \Yii::t('components/other', '取消');?></el-button>
                <el-button size='small' type="primary" @click="beSelectCats"><?= \Yii::t('components/other', '确定');?></el-button>
            </span>
    </el-dialog>
</template>
<script>
