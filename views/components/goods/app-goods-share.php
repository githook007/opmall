        ...
        <el-form-item :label="pintuan_sign ? '' : '<?= \Yii::t("components/goods", "分销佣金");?>'">
            ...
            <template v-else-if="loading === false && shareLevel.length> 0">
                <div class="box">
                    ...
                    <label style="margin-bottom:0;padding:18px 10px;"><?= \Yii::t('components/goods', '批量设置');?></label>
                    <el-select v-model="selectLevel" slot="prepend" placeholder="<?= \Yii::t('components/goods', '请选择等级');?>"
                    ...
                    <el-select v-model="selectData" slot="prepend" placeholder="<?= \Yii::t('components/goods', '请选择层级');?>">
                    ...
                    <el-input @keyup.enter.native="enter" type="number" style="width: 150px;"
                        ...
                        <span slot="append">{{share_type == 1 ? '%' : "<?= \Yii::t('components/goods', '元');?>"}}</span>
                    </el-input>
                    <el-button type="primary" size="small" @click="batchAttr"><?= \Yii::t('components/goods', '设置');?></el-button>
                </div>
                <!--普通分销佣金设置 -->
                <template v-if="attr_setting_type == 0 || use_attr == 0">
                    <el-table ref="normal" :data="ruleForm.shareLevelList" border stripe style="width: 90%;"
                        ...
                        <el-table-column width="100" label="<?= \Yii::t('components/goods', '等级名称');?>" prop="name"></el-table-column>
                ...
                <template v-else>
                    ...
                    <!-- 默认规格 分销佣金-->
                    <template v-else>
                        <el-tag style="margin-top: 10px;" type="danger"><?= \Yii::t('components/goods', '如需设置多规格分销价, 请先添加商品规格');?></el-tag>
                    </template>
                </template>
            </template>
        </el-form-item>
            ...
            batchAttr() {
                if (this.attr_setting_type == 0) {
                    if (!this.selectList || this.selectList.length === 0) {
                        this.$message.warning("<?= \Yii::t('components/goods', '请勾选分销商等级');?>");
                        return;
                    }
                    if (this.selectData === '') {
                        this.$message.warning("<?= \Yii::t('components/goods', '请选择分销层级');?>");
                        return;
                    }
                    ...
                    if (!this.selectList || this.selectList.length === 0) {
                        this.$message.warning("<?= \Yii::t('components/goods', '请勾选商品规格');?>");
                        return;
                    }
                    if (this.selectLevel === '') {
                        this.$message.warning("<?= \Yii::t('components/goods', '请选择分销商等级');?>");
                        return;
                    }
                    if (this.selectData === '') {
                        this.$message.warning("<?= \Yii::t('components/goods', '请选择分销层级');?>");
