    ...
    <div class="app-attr-group">
        <div v-for="(group, i) in value" :key="i" style="margin-bottom: 16px">
            <div flex="dir:left cross:center" class="bg">
                <span><?= \Yii::t('components/goods', '规格名');?>:</span>
                ...
                <el-checkbox v-if="i === 0" v-model="attrPic"><?= \Yii::t('components/goods', '规格图片');?></el-checkbox>
            ...
            <div flex="dir:left" style="padding:0 10px;margin-top: 16px">
                <div class="box-grow-0"><?= \Yii::t('components/goods', '规格值');?>：</div>
                    ...
                    <div class="attr-list" v-for="(attr, j) in group.attr_list" :key="j">
                        ...
                        <div flex="cross:center main:center" class="img-box" v-if="i===0 && attrPic">
                            ...
                            <app-attachment v-else v-model="attr.pic_url" :multiple="false" :max="1">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('components/goods', '建议尺寸:750 * 750');?>" placement="top">
                                    <el-button type="text">+<?= \Yii::t('components/goods', '添加图片');?></el-button>
                                </el-tooltip>
                            </app-attachment>
                            ...
                            <div v-if="attrPicStatus === j && attr.pic_url" style="position: absolute">
                                <app-attachment v-model="attr.pic_url" :multiple="false" :max="1">
                                    <el-button type="primary"><?= \Yii::t('components/goods', '替换');?></el-button>
                                </app-attachment>
                            </div>
                        </div>
                    </div>
                    <div slot="footer">
                        <el-button type="text" @click="addAttr(i)"><?= \Yii::t('components/goods', '添加规格值');?></el-button>
                    </div>
                </draggable>
            </div>
            <div v-if="i === 0" style="margin-left: 65px;color:#c9c9c9">
                <?= \Yii::t('components/goods', '仅支持为第一组规格设置规格图片，买家选择不同规格会看到对应规格图片，建议尺寸：80 * 80像素');?>
            </div>
        </div>
        <div v-if="isAddAttrGroups" flex="dir:left cross:center" class="bg">
            <app-attr-template v-model="value" :attr-group-max-count="attrGroupMaxCount" @submit="makeAttrGroup">
                <el-button><?= \Yii::t('components/goods', '选择规格模板');?></el-button>
            </app-attr-template>
            <el-button @click="addAttrGroup" style="margin-left: 12px"><?= \Yii::t('components/goods', '添加规格项目');?></el-button>
            <span style="padding-left: 14px;color:#c9c9c9"><?= \Yii::t('components/goods', '注：规格名最多添加5个');?></span>
        </div>
    </div>
</template>
<script>
