    ...
    <div class="app-params-template">
        <el-dialog custom-class="custom-dialog" :visible.sync="templateDialog">
            <div slot="title" style="margin-bottom: -30px">
                <span style="font-size: 18px;color:#7a7a7a"><?= \Yii::t('components/goods', '选择参数模板');?></span>
                <span style="font-size:14px;color:#c9c9c9;margin-left: 10px"><?= \Yii::t('components/goods', '参数模板最多支持添加');?>{{lastMaxCount}}<?= \Yii::t('components/goods', '组');?></span>
            </div>
            <div flex="dir:left">
                <div class="tpl-box">
                    <div class="tpl-head"><?= \Yii::t('components/goods', '参数名选择');?></div>
                ...
                <div class="tpl-box" style="margin-left: 20px">
                    <div class="tpl-head"><?= \Yii::t('components/goods', '参数内容选择');?></div>
                    <el-scrollbar class="tpl-scrollbar">
                            ...
                            <div v-if="group.only == tplActiveBg[0]"
                                ...
                                <el-checkbox-group v-model="childrenValue">
                                    <el-checkbox class="tpl-item"
                                                 ...
                                                 name="type">
                                        <div><?= \Yii::t('components/goods', '参数名');?>：{{item.label}}</div>
                                        <div><?= \Yii::t('components/goods', '参数值');?>：{{item.value}}</div>
                                    </el-checkbox>
                                </el-checkbox-group>
                            </div>
                        </span>
            ...
            <div slot="footer" class="dialog-footer">
                <el-button size="small" type="primary" @click="templateSubmit"><?= \Yii::t('components/goods', '确定选择');?></el-button>
            </div>
        </el-dialog>
        ...
        methods: {
            ...
            templateSubmit() {
                ...
                if (arrLength + self.value.length > this.lastMaxCount) {
                    self.$message.error("<?= \Yii::t('components/goods', '参数内容最多添加');?>" + this.lastMaxCount + "个");
                    return;
                }
            ...
            emptyModel() {
                ...
                self.$msgbox({
                    title: "<?= \Yii::t('components/goods', '提示');?>",
                    confirmButtonText: "<?= \Yii::t('components/goods', '我知道了');?>",
                    message: h('p', {style: 'color:#666666;font-size:14px'}, [
                        h('p', null, "<?= \Yii::t('components/goods', '暂无参数模板数据');?>"),
                        h('span', null, "<?= \Yii::t('components/goods', '请先至');?>"),
                        h('span', {on: {click: navTemplate}, style: 'color:#3399ff;cursor:pointer'}, "<?= \Yii::t('components/goods', '商品管理-参数模板');?>"),
                        h('span', null, "<?= \Yii::t('components/goods', '中添加');?>"),
                    ]),
                    type: 'warning'
                });
            },
