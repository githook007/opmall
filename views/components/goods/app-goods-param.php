...
<template id="app-goods-param">
    <div class="app-goods-param">
        <el-form-item prop="param_name" label="<?= \Yii::t('components/goods', '参数标题');?>">
            <el-input placeholder="<?= \Yii::t('components/goods', '请输入参数标题');?>" v-model="value.param_name" maxlength="20" show-word-limit></el-input>
        </el-form-item>
        <el-form-item prop="param_content" label="<?= \Yii::t('components/goods', '参数内容');?>">
            <div class="box">
                <draggable :options="{draggable:'.d-select',filter:'.d-filter',preventOnFilter:false}"
                    ...
                    <div class="d-select" v-for="(group, i) of value.param_content">
                        <div flex="dir:left cross:center" class="bg">
                            <span style="user-select:none;" class="x"><?= \Yii::t('components/goods', '参数名');?>:</span>
                        ...
                        <div flex="dir:left" style="padding:0 10px;margin:16px 0">
                            <span style="user-select:none;" class="x"><?= \Yii::t('components/goods', '参数值');?>:</span>
                ...
                <div v-if="isAddAttrGroups" flex="dir:left cross:center" class="bg">
                    <app-params-template v-model="value.param_content" :last-max-count="maxNum" @submit="makeGroup">
                        <el-button><?= \Yii::t('components/goods', '选择参数模板');?></el-button>
                    </app-params-template>
                    <el-button @click="add" style="margin-left: 12px"><?= \Yii::t('components/goods', '添加参数项目');?></el-button>
                    <span style="padding-left: 14px;color:#c9c9c9"><?= \Yii::t('components/goods', '注：参数名最多添加');?>{{maxNum}}<?= \Yii::t('components/goods', '个');?></span>
                </div>
            ...
            add() {
                try {
                    if (this.value.param_content.length > this.maxNum) {
                        throw new Error("<?= \Yii::t('components/goods', '数量超限');?>");
                    }
