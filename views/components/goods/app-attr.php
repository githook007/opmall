    ...
    <div class="app-attr">
        <div class="box">
            <el-checkbox v-model="attrBatch" @change="selectClick" :disabled="!cData || cData.length == 0"
                         style="position:absolute"><?= \Yii::t('components/goods', '全选');?>
            </el-checkbox>
            <el-form-item label="<?= \Yii::t('components/goods', '批量设置');?>" style="margin-bottom:0;padding:18px 0">
    ...
    Vue.component('app-attr', {
        ...
        data() {
            return {
                ...
                attrBatch: false,
                data: {
                    price: "<?= \Yii::t('components/goods', '价格');?>",
                    stock: "<?= \Yii::t('components/goods', '库存');?>",
                    weight: "<?= \Yii::t('components/goods', '重量(克)');?>",
                    no: "<?= \Yii::t('components/goods', '货号');?>",
                    bar_code: "<?= \Yii::t('components/goods', '条形码');?>",
                },
                selectData: '',
                batch: 0,
                selectList: [],
        ...
        created() {
            ...
            if(this.extra && this.extra.supply_price == "<?= \Yii::t('components/goods', '团长供货价(元)');?>") {
                this.data.price = "<?= \Yii::t('components/goods', '买家购买价(元)');?>"
            }
        },
        watch: {
            'selectList': function() {
        ...
        methods: {
            ...
            batchAttr(param) {
                if (!param) {
                    this.$message.warning("<?= \Yii::t('components/goods', '请选择批量设置');?>");
                    return;
                }
