...
<template id="app-table">
    <div class="app-table">
        <el-card shadow="never" v-if="showStatus === 'store' || showStatus === 'mch' || showStatus === 'sub-account' || showStatus === 'super-account'">
            <div slot="header">
                <span v-if="isShowType"><?= \Yii::t('components/statistics', '趋势概况');?></span>
                <span v-else><?= \Yii::t('components/statistics', '支付数据');?></span>
            </div>
            <div v-if="isShowType" class="trend">
                <el-tabs v-model="tableSearch.type" @tab-click="changeTableTabs">
                    <el-tab-pane label="<?= \Yii::t('components/statistics', '支付数据');?>" name="1"></el-tab-pane>
                    <el-tab-pane label="<?= \Yii::t('components/statistics', '收益数据');?>" name="2"></el-tab-pane>
                </el-tabs>
            </div>
            <div>
    ...
    Vue.component('app-table', {
        ...
        data() {
            return {
                ...
                payForm: [
                    {
                        label: "<?= \Yii::t('components/statistics', '支付订单数（笔）');?>",
                        value: '0',
                        sign: 'order',
                        is_show: true,
                        remark: "<?= \Yii::t('components/statistics', '不包含取消订单的支付订单数');?>",
                        color: '#3399ff',
                    },
                    {
                        label: "<?= \Yii::t('components/statistics', '支付人数（人）');?>",
                        value: '0',
                        sign: 'people',
                        is_show: true,
                        remark: "<?= \Yii::t('components/statistics', '不包含取消订单的支付人数');?>",
                        color: '#FFA360',
                    },
                    {
                        label: "<?= \Yii::t('components/statistics', '支付金额（元）');?>",
                        value: '0',
                        sign: 'price',
                        is_show: true,
                        remark: "<?= \Yii::t('components/statistics', '不包含取消订单的支付金额');?>",
                        color: '#4BC282',
                    },
                    {
                        label: "<?= \Yii::t('components/statistics', '支付商品数（件）');?>",
                        value: '0',
                        sign: 'num',
                        is_show: true,
                        remark: "<?= \Yii::t('components/statistics', '不包含取消订单的支付商品数');?>",
                        color: '#FF8585',
                    },
                ],
                dataForm: [
                    {
                        label: "<?= \Yii::t('components/statistics', '余额收益（元）');?>",
                        value: '0',
                        sign: 'balance',
                        is_show: true,
                        remark: "<?= \Yii::t('components/statistics', '实际收益：扣除所有退款订单的支付金额');?>",
                        color: '#3399ff',
                    },
                    {
                        label: "<?= \Yii::t('components/statistics', '微信收益（元）');?>",
                        value: '0',
                        sign: 'wx',
                        is_show: true,
                        remark: "<?= \Yii::t('components/statistics', '实际收益：扣除所有退款订单的支付金额');?>",
                    ...
                    {
                        label: "<?= \Yii::t('components/statistics', '支付宝收益（元）');?>",
                        value: '0',
                        sign: 'ali',
                        is_show: true,
                        remark: "<?= \Yii::t('components/statistics', '实际收益：扣除所有退款订单的支付金额');?>",
                    ...
                    {
                        label: "<?= \Yii::t('components/statistics', '货到付款收益（元）');?>",
                        value: '0',
                        sign: 'huohao',
                        is_show: true,
                        remark: "<?= \Yii::t('components/statistics', '实际收益：扣除所有退款订单的支付金额');?>",
        ...
        methods: {
            ...
            form() {
                ...
                myChart.showLoading({text: "<?= \Yii::t('components/statistics', 'Loading_data');?>"});
            },
            //初始化
            getTable() {
                const self = this;
