    ...
    <div class="app-order-info">
        <div class="table-body" flex="dir:left" v-loading="loading">
            <div class="info-data">
                ...
                <div class="label">
                    <span><?= \Yii::t('components/statistics', '待发货订单（笔）');?></span>
                </div>
            </div>
            <div class="info-data">
                <div class="value" @click="nav('pro_order')">{{all_data.pro_order}}</div>
                <div class="label">
                    <span><?= \Yii::t('components/statistics', '售后订单（笔）');?></span>
                </div>
            </div>
            <div class="info-data">
                <div class="value" @click="nav('wait_pay')">{{all_data.wait_pay_num}}</div>
                <div class="label">
                    <span><?= \Yii::t('components/statistics', '待付款订单（笔）');?></span>
                </div>
            </div>
            <div class="info-data" v-if="isUser">
                <div class="value" @click="nav('user_count')">{{all_data.user_count}}</div>
                <div class="label">
                    <span><?= \Yii::t('components/statistics', '总用户数（人）');?></span>
                    <el-tooltip class="item" effect="dark" content="<?= \Yii::t('components/statistics', '全平台授权用户总数');?>" placement="top">
            ...
            <div class="info-data" v-if="isGoods">
                ...
                <div class="label">
                    <span><?= \Yii::t('components/statistics', '商品数（款）');?></span>
                    <el-tooltip class="item" effect="dark" content="<?= \Yii::t('components/statistics', '计算销售中，下架中和售罄的商品款数');?>" placement="top">
                        <i class="el-icon-question"></i>
                    </el-tooltip>
                </div>
            </div>
