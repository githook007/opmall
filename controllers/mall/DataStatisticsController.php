...
class DataStatisticsController extends MallController
{
    ...
    //插件菜单
    public function actionCommonMenus()
    {
        $form = new DataForm();
        $form->attributes = \Yii::$app->request->get();
        $form->attributes = \Yii::$app->request->post();
        return $this->asJson($form->common_menus());
    }
    ...
    // 弹出公告
    public function actionNoticeEject(){
        $from = new NoticeForm();
        $from->attributes = \Yii::$app->request->post();
        return $this->asJson($from->noticeEject());
    }

    // 弹出公告-不再提示
    public function actionNoticeIDo(){
        $from = new NoticeForm();
        $from->attributes = \Yii::$app->request->post();
        return $this->asJson($from->noticeIDo());
    }
}
