...
class PayNotifyController extends Controller
{
     ...
     * app 微信支付
     */
    public function actionWechatAppPay()
    {
        return $this->notifyWechat('app');
    }
    ...
    private function notifyWechat($type = 'wxapp')
    {
        ...
        try {
            \Yii::$app->trigger(PaymentOrderUnion::EVENT_TRADE_COMPLETE, new PaymentOrderEvent(['paymentOrderUnion' => $paymentOrderUnion]));
        }catch (\Exception $e){
            \Yii::error($e);
        }
        if($paymentOrderUnion->user->username == $res['openid']){
            $payType = \app\bootstrap\payment\PaymentOrder::PAY_TYPE_WECHAT;
        }else{
            $payType = \app\bootstrap\payment\PaymentOrder::PAY_TYPE_WECHAT_PAY_FOR_ANOTHER;
        }
            ...
            foreach ($paymentOrders as $paymentOrder) {
                ...
                try {
                    $po = new \app\bootstrap\payment\PaymentOrder([
                        ...
                        'title' => $paymentOrder->title,
                        'notifyClass' => $paymentOrder->notify_class,
                        'payType' => \app\bootstrap\payment\PaymentOrder::PAY_TYPE_ALIPAY
                    ]);
                    $notify->notify($po);
                } catch (\Exception $e) {
                    \Yii::error($e);
        ...
        foreach ($paymentOrders as $paymentOrder) {
            ...
            try {
                $po = new \app\bootstrap\payment\PaymentOrder([
                    ...
                    'title' => $paymentOrder->title,
                    'notifyClass' => $paymentOrder->notify_class,
                    'payType' => \app\bootstrap\payment\PaymentOrder::PAY_TYPE_BAIDU
                ]);
                $notify->notify($po);
            } catch (\Exception $e) {
                \Yii::error($e);
            ...
            foreach ($paymentOrders as $paymentOrder) {
                ...
                try {
                    $po = new \app\bootstrap\payment\PaymentOrder([
                        ...
                        'title' => $paymentOrder->title,
                        'notifyClass' => $paymentOrder->notify_class,
                        'payType' => \app\bootstrap\payment\PaymentOrder::PAY_TYPE_TOUTIAO
                    ]);
                    $notify->notify($po);
                } catch (\Exception $e) {
                    \Yii::error($e);
    ...
    {
        $data_infos = \Yii::$app->request->post();

        $data_info = [
            'out_trade_no' => $data_infos['cusorderid'],
            'txamt' => $data_infos['trxamt'],
            'status' => '1',
        ];

        //判断订单信息
        if (empty($data_info['out_trade_no'])
            || empty($data_info['txamt'])
            || empty($data_info['status'])
        ) {
            throw new \Exception('请求数据错误: ' . print_r($data_info));
        }

        if ($data_info['status'] !== '1') {
            throw new \Exception('订单尚未支付: ' . $data_info);
        }

        //查询订单信息
        $paymentOrderUnion = PaymentOrderUnion::findOne([
            'order_no' => $data_info['out_trade_no'],
        ]);
        if (!$paymentOrderUnion) {
            throw new \Exception('订单不存在: ' . $data_info['out_trade_no']);
        }
        if ($paymentOrderUnion->app_version) {
            \Yii::$app->setAppVersion($paymentOrderUnion->app_version);
        }
        ...
        $paymentOrderUnionAmount = (doubleval($paymentOrderUnion->amount) * 100) . '';
        if (intval($data_info['txamt']) !== intval($paymentOrderUnionAmount)) {
            throw new \Exception('支付金额与订单金额不一致。');
        }

        $paymentOrders = PaymentOrder::findAll(['payment_order_union_id' => $paymentOrderUnion->id]);
        $paymentOrderUnion->is_pay = 1;
        $paymentOrderUnion->pay_type = $this->wechatPayType()["wxapp"];
        if (!$paymentOrderUnion->save()) {
            throw new \Exception($paymentOrderUnion->getFirstErrors());
        }
        foreach ($paymentOrders as $paymentOrder) {
