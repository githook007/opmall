...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        ...
        <div slot="header">
            <div>
                <span><?= \Yii::t('plugins/teller', '基础设置');?></span>
            </div>
        </div>
        <el-form v-loading="listLoading" label-width="172px" size="small" :model="ruleForm" :rules="ruleFormRules"
            ...
            <el-card shadow="never" class="y-card" body-style="background-color: #ffffff;padding: 10px 0 0;">
                ...
                <div class="form-body">
                    <div style="margin:0 20px 20px">
                        <el-alert type="info" title="<?= \Yii::t('plugins/teller', '入口');?>" :closable="false">
                            <template>
                                <span>{{ruleForm.login_url}}</span>
                                <el-button @click="copyInput(ruleForm.login_url)" size="mini"><?= \Yii::t('plugins/teller', '复制链接');?></el-button>
                            </template>
                        </el-alert>
                    </div>
                    <el-form-item label="<?= \Yii::t('plugins/teller', '是否开启交班');?>" prop="is_shifts">
                        ...
                        <el-select v-if="ruleForm.is_shifts == 1"
                                   ...
                                   placeholder="<?= \Yii::t('plugins/teller', '请选择小票打印机');?>">
                    ...
                    <el-form-item label="<?= \Yii::t('plugins/teller', '是否开启会员充值');?>" prop="is_member_topup">
                        <el-switch v-model="ruleForm.is_member_topup" :active-value="1" :inactive-value="0"></el-switch>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('plugins/teller', '加钱开关');?>" prop="is_add_money">
            ...
            <el-card shadow="never" class="y-card" body-style="background-color: #ffffff;padding: 10px 0 0;">
                <div slot="header">
                    <div style="he">
                        <span><?= \Yii::t('plugins/teller', '购买设置');?></span>
                    </div>
                </div>
                <div class="form-body">
                    <el-form-item v-if="ruleForm.svip_status >= 0" label="<?= \Yii::t('plugins/teller', '是否开启分销');?>" prop="is_share">
                        <el-switch v-model="ruleForm.is_share" :active-value="1" :inactive-value="0"></el-switch>
                        <span class="rig" style="font-size: 14px">
                            <span style="color:#ff4544"><?= \Yii::t('plugins/teller', '注');?></span>
                            <el-button style="font-size: inherit"
                                       @click="$navigate({r:'mall/share/basic'},true)"
                                       type="text"><?= \Yii::t('plugins/teller', '分销中心');?></el-button>
                            <span style="color:#ff4544"><?= \Yii::t('plugins/teller', '中开启');?></span>
                        </span>
                    </el-form-item>
                </div>
            </el-card>
            ...
            <el-card shadow="never" class="y-card" body-style="background-color: #ffffff;padding: 10px 0 0;">
                <div slot="header">
                    <div style="he">
                        <span><?= \Yii::t('plugins/teller', '支付设置');?></span>
                    </div>
                </div>
                <div class="form-body">
                    <el-form-item label="<?= \Yii::t('plugins/teller', '微信支付');?>" prop="is_wechat_pay">
                        ...
                        <el-select v-if="ruleForm.is_wechat_pay == 1" class="rig" size="small" filterable
                                   ...
                                   placeholder="<?= \Yii::t('plugins/teller', '请选择');?>">
                            <el-option
                    ...
                    <el-form-item label="<?= \Yii::t('plugins/teller', '支付宝支付');?>" prop="is_ali_pay">
                    ...
                    <el-form-item label="<?= \Yii::t('plugins/teller', '现金支付');?>" prop="is_cash">
                    ...
                    <el-form-item v-if="ruleForm.is_balance != -1" label="<?= \Yii::t('plugins/teller', '余额支付');?>" prop="is_balance">
                    ...
                    <el-form-item v-if="ruleForm.is_balance_pay_password != -1 && ruleForm.is_balance == 1" label="<?= \Yii::t('plugins/teller', '余额支付密码开关');?>" prop="is_balance_pay_password">
                        <el-switch v-model="ruleForm.is_balance_pay_password" :active-value="1" :inactive-value="0"></el-switch>
                        <span class="rig" style="font-size: 14px">
                            <span style="color:#ff4544"><?= \Yii::t('plugins/teller', '必须在');?></span>
                            <el-button style="font-size: inherit"
                                       @click="$navigate({r:'mall/index/setting'},true)"
                                       type="text"><?= \Yii::t('plugins/teller', '设置基础设置');?></el-button>
                            <span style="color:#ff4544"><?= \Yii::t('plugins/teller', '设置支付密码');?></span>
                        </span>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('plugins/teller', 'POS支付');?>" prop="is_pos">
            ...
            <el-card shadow="never" class="y-card" body-style="background-color: #ffffff;padding: 10px 0 0;">
                <div slot="header">
                    <div style="he">
                        <span><?= \Yii::t('plugins/teller', 'Tab页签设置');?></span>
                    </div>
                </div>
                <div class="form-body">
                    <el-form-item label="<?= \Yii::t('plugins/teller', '开关');?>" prop="is_tab">
                        <el-switch v-model="ruleForm.is_tab" :active-value="1" :inactive-value="0"></el-switch>
                        <span class="rig color"><?= \Yii::t('plugins/teller', '设置收银台右边tab页签');?></span>
                    </el-form-item>
                    <el-form-item v-if="ruleForm.is_tab == 1" label="<?= \Yii::t('plugins/teller', '分类');?>" prop="tab_list">
                        <draggable v-model="ruleForm.new_tab_list"
                            ...
                            <el-button class="item-drag" @click="$refs.tab_list.openDialog()"><?= \Yii::t('plugins/teller', '选择分类');?></el-button>
                        </draggable>
            ...
            <el-card shadow="never" class="y-card" body-style="background-color: #ffffff;padding: 10px 0 0;">
                <div slot="header">
                    <div style="he">
                        <span><?= \Yii::t('plugins/teller', '优惠叠加设置');?></span>
                    </div>
                </div>
                <div class="form-body">
                    <el-form-item label="<?= \Yii::t('plugins/teller', '优惠券');?>" prop="is_coupon">
                        <el-switch v-model="ruleForm.is_coupon" :active-value="1" :inactive-value="0"></el-switch>
                    </el-form-item>
                    <el-form-item v-if="ruleForm.svip_status >= 0" label="<?= \Yii::t('plugins/teller', '超级会员卡');?>" prop="svip_status">
                        ...
                        <span class="rig" style="font-size: 14px">
                            ...
                            <el-button style="font-size: inherit"
                                       ...
                                       type="text"><?= \Yii::t('plugins/teller', '插件中心');?></el-button>
                            <span style="color:#ff4544"><?= \Yii::t('plugins/teller', '中开启');?></span>
                        </span>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('plugins/teller', '会员价');?>" prop="is_member_price">
                        <el-switch v-model="ruleForm.is_member_price" :active-value="1" :inactive-value="0"></el-switch>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('plugins/teller', '积分抵扣');?>" prop="is_integral">
                        <el-switch v-model="ruleForm.is_integral" :active-value="1" :inactive-value="0"></el-switch>
                    </el-form-item>
                    <el-form-item v-if="ruleForm.is_full_reduce != -1" label="<?= \Yii::t('plugins/teller', '满减优惠');?>" prop="is_full_reduce">
            ...
            <el-card shadow="never" class="y-card" body-style="background-color: #ffffff;padding: 10px 0 0;">
                <div slot="header">
                    <div style="he">
                        <span><?= \Yii::t('plugins/teller', '抹零设置');?></span>
                    </div>
                </div>
                <div class="form-body">
                    <el-form-item label="<?= \Yii::t('plugins/teller', '抹零设置开关');?>" prop="is_price">
                        ...
                        <div v-if="ruleForm.is_price == 1" style="color: #606266">
                            <div style="margin-top: 10px">
                                <el-radio style="width: 3vw" v-model="ruleForm.price_type" :label="1"><?= \Yii::t('plugins/teller', '抹分');?></el-radio>
                                <span class="rig color"><?= \Yii::t('plugins/teller', '向下抹分');?></span>
                            </div>
                            <div style="margin-top: 10px">
                                <el-radio style="width: 3vw" v-model="ruleForm.price_type" :label="2"><?= \Yii::t('plugins/teller', '抹角');?></el-radio>
                                <span class="rig color"><?= \Yii::t('plugins/teller', '向下抹角');?></span>
                            </div>
                            <div style="margin-top: 10px">
                                <el-radio style="width: 3vw" v-model="ruleForm.price_type" :label="3"><?= \Yii::t('plugins/teller', '四舍分');?></el-radio>
                                <span class="rig color"><?= \Yii::t('plugins/teller', '如9');?></span>
                            </div>
                            <div style="margin-top: 10px">
                                <el-radio style="width: 3vw" v-model="ruleForm.price_type" :label="4"><?= \Yii::t('plugins/teller', '五入到角');?></el-radio>
                                <span class="rig color"><?= \Yii::t('plugins/teller', '如9');?></span>
                            </div>
                        </div>
                    </el-form-item>
            ...
            <el-card shadow="never" class="y-card" body-style="background-color: #ffffff;padding: 10px 0 0;">
                <div slot="header">
                    <div style="he">
                        <span><?= \Yii::t('plugins/teller', '提成设置');?></span>
                    </div>
                </div>
                <div class="form-body">
                    <el-form-item label="<?= \Yii::t('plugins/teller', '收银员提成');?>" prop="is_cashier_push">
                        ...
                        <span class="rig" style="font-size: 14px">
                            ...
                            <el-button style="font-size: inherit"
                                       ...
                                       type="text"><?= \Yii::t('plugins/teller', '收银台');?></el-button>
                            <span style="color:#ff4544"><?= \Yii::t('plugins/teller', '”');?></span>
                        </span>
                    </el-form-item>
                    <el-form-item v-if="ruleForm.is_cashier_push == 1" label="<?= \Yii::t('plugins/teller', '提成类型');?>" prop="cashier_push_type">
                        <el-radio v-model="ruleForm.cashier_push_type" :label="1"><?= \Yii::t('plugins/teller', '按订单');?></el-radio>
                        <el-radio v-model="ruleForm.cashier_push_type" :label="2"><?= \Yii::t('plugins/teller', '按金额');?></el-radio>
                        <br>
                        <el-input v-if="ruleForm.cashier_push_type == 1"
                            ...
                            <template slot="append"><?= \Yii::t('plugins/teller', '元');?></template>
                        </el-input>
                    ...
                    <el-form-item label="<?= \Yii::t('plugins/teller', '导购员提成');?>" prop="is_sales_push">
                        ...
                        <span class="rig" style="font-size: 14px">
                            ...
                            <el-button style="font-size: inherit"
                                       ...
                                       type="text"><?= \Yii::t('plugins/teller', '收银台');?></el-button>
                            <span style="color:#ff4544"><?= \Yii::t('plugins/teller', '设置导购员');?></span>
                        </span>
                    </el-form-item>
                    <el-form-item v-if="ruleForm.is_sales_push == 1" label="<?= \Yii::t('plugins/teller', '提成类型');?>" prop="sales_push_type">
                        <el-radio v-model="ruleForm.sales_push_type" :label="1"><?= \Yii::t('plugins/teller', '按订单');?></el-radio>
                        <el-radio v-model="ruleForm.sales_push_type" :label="2"><?= \Yii::t('plugins/teller', '按金额');?></el-radio>
                        <br>
            ...
            <el-card shadow="never" class="y-card" body-style="background-color: #ffffff;padding: 10px 0 0;">
                <div slot="header">
                    <div style="he">
                        <span><?= \Yii::t('plugins/teller', '改价设置');?></span>
                    </div>
                </div>
                <div class="form-body">
                    <el-form-item label="<?= \Yii::t('plugins/teller', '商品改价开关');?>" prop="is_goods_change_price">
                        <el-switch v-model="ruleForm.is_goods_change_price" :active-value="1"
                                   :inactive-value="0"></el-switch>
                    </el-form-item>
                    <template v-if="ruleForm.is_goods_change_price == 1">
                        <el-form-item label="<?= \Yii::t('plugins/teller', '改价类型');?>" prop="is_goods_change_price_type">
                            <el-radio v-model="ruleForm.is_goods_change_price_type" :label="1"><?= \Yii::t('plugins/teller', '固定金额');?></el-radio>
                            <el-radio v-model="ruleForm.is_goods_change_price_type" :label="2"><?= \Yii::t('plugins/teller', '百分比');?></el-radio>
                        </el-form-item>

                        <template v-if="ruleForm.is_goods_change_price_type == 1">
                            <el-form-item label="<?= \Yii::t('plugins/teller', '最多可加');?>" prop="most_plus">
                            ...
                            <el-form-item label="<?= \Yii::t('plugins/teller', '最多可减');?>" prop="most_subtract">
                                <el-input type="number" min="0"
                                          oninput="this.value = this.value.match(/^\d+(\.)?\d{0,2}/g)"
                        ...
                        <template v-if="ruleForm.is_goods_change_price_type == 2">
                            <el-form-item label="<?= \Yii::t('plugins/teller', '最多可加');?>" prop="most_plus_percent">
                                <el-input type="number" min="0"
                                          oninput="this.value = this.value.match(/^\d+(\.)?\d{0,2}/g)"
                            ...
                            <el-form-item label="<?= \Yii::t('plugins/teller', '最多可减');?>" prop="most_subtract_percent">
                                <el-input type="number" min="0"
                                          oninput="this.value = this.value.match(/^\d+(\.)?\d{0,2}/g)"
            ...
            <el-card shadow="never" class="y-card" body-style="background-color: #ffffff;padding: 10px 0 0;">
                <div slot="header">
                    <div style="he">
                        <span><?= \Yii::t('plugins/teller', '版权设置');?></span>
                    </div>
                </div>
                <div class="form-body">
                    <el-form-item label="<?= \Yii::t('plugins/teller', '收银台登录页logo');?>" prop="logo_url">
                        <app-attachment style="margin-bottom:10px" :multiple="false" :max="1"
                                        @selected="tellerLogoPic">
                            <el-tooltip effect="dark"
                                        content="<?= \Yii::t('plugins/teller', '建议尺寸');?>:120 * 120"
                                        placement="top">
                                <el-button size="mini"><?= \Yii::t('plugins/teller', '选择图标');?></el-button>
                            </el-tooltip>
                        </app-attachment>
                        ...
                        <el-button size="mini" @click="resetImg('logo_url')" class="reset" type="primary"><?= \Yii::t('plugins/teller', '恢复默认');?>
                        </el-button>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('plugins/teller', '收银台登录页背景图');?>" prop="background_image_url">
                        <app-attachment style="margin-bottom:10px" :multiple="false" :max="1"
                                        @selected="tellerBgPic">
                            <el-tooltip effect="dark"
                                        content="<?= \Yii::t('plugins/teller', '建议尺寸');?>:1980 * 1080"
                                        placement="top">
                                <el-button size="mini"><?= \Yii::t('plugins/teller', '选择图片');?></el-button>
                            </el-tooltip>
                        </app-attachment>
                        <div style="margin-right: 20px;display:inline-block;position: relative;cursor: move;">
                            <app-attachment :multiple="false" :max="1"
                        ...
                        <el-button size="mini" @click="resetImg('background_image_url')" class="reset" type="primary">
                            <?= \Yii::t('plugins/teller', '恢复默认');?>
                        </el-button>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('plugins/teller', '底部版权信息');?>" prop="copyright">
                        <el-input class="y-input" size="small" v-model="ruleForm.copyright"></el-input>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('plugins/teller', '底部版权链接');?>" prop="copyright_url">
                        <el-input class="y-input" size="small" placeholder="<?= \Yii::t('plugins/teller', '例如');?> https://www.baidu.com"
                                  v-model="ruleForm.copyright_url"></el-input>
                    </el-form-item>
                </div>
            </el-card>
        </el-form>
        <el-button size="small" type="primary" :loading="btnLoading" @click="submit"><?= \Yii::t('plugins/teller', '保存');?></el-button>
    </el-card>
</div>
<script>
    const app = new Vue({
        ...
        methods: {
            getOption() {
                request({
                    ...
                    if (e.data.code === 0) {
                        let wxOption = [{label: '<?= \Yii::t('plugins/teller', '不启用');?>', value: 0}];
                        let aliOption = [{label: '<?= \Yii::t('plugins/teller', '不启用');?>', value: 0}];
            ...
            copyInput(text) {
                if (this.copyText(text)) {
                    this.$message.success('<?= \Yii::t('plugins/teller', '复制成功');?>');
                } else {
                    this.$message.error('<?= \Yii::t('plugins/teller', '复制失败');?>');
                }
            },
            selectCat(tab_list) {
