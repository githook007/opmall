...
<div id="app" v-cloak>
        ...
        <el-breadcrumb separator="/">
            <el-breadcrumb-item>
                 <span style="color: #409EFF;cursor: pointer" @click="$navigate({r:'plugin/teller/mall/sales/index'})">
                    <?= \Yii::t('plugins/teller', '导购员');?>
                 </span>
            </el-breadcrumb-item>
            <el-breadcrumb-item v-if="id"><?= \Yii::t('plugins/teller', '编辑导购员');?></el-breadcrumb-item>
            <el-breadcrumb-item v-else><?= \Yii::t('plugins/teller', '添加导购员');?></el-breadcrumb-item>
        </el-breadcrumb>
    </div>
    <el-card v-loading="listLoading" shadow="never" body-style="background-color: #ffffff;">
        <el-form :model="editForm" ref="editForm" :rules="editFormRules" label-width="150px" position-label="right">
            <el-form-item prop="number" label="<?= \Yii::t('plugins/teller', '导购员编号');?>">
            ...
            <el-form-item label="<?= \Yii::t('plugins/teller', '头像');?>" prop="head_url">
                <app-attachment style="margin-bottom:10px" :multiple="false" :max="1"
                                @selected="selectedHeadUrl">
                    <el-tooltip effect="dark"
                                content="<?= \Yii::t('plugins/teller', '建议尺寸');?>:40 * 40"
                                placement="top">
                ...
                <el-button size="mini" @click="resetImg('head')" class="reset" type="primary"><?= \Yii::t('plugins/teller', '恢复默认');?></el-button>
            </el-form-item>
    ...
    <el-button v-if="false" size="small" style="margin-top: 20px" :loading="btnLoading" @click="reset"><?= \Yii::t('plugins/teller', '重置');?></el-button>
</div>
<script>
    const app = new Vue({
        el: '#app',
        data() {
            return {
                ...
                editFormRules: {
                    number: [
                        {required: true, message: '<?= \Yii::t('plugins/teller', '导购员编号不能为空');?>', trigger: 'blur'},
                    ],
                    name: [
