...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header" flex="dir:left" style="justify-content:space-between;">
            <span style="margin: auto 0"><?= \Yii::t('plugins/teller', '交班记录');?></span>
        ...
        <div class="table-body">
            ...
            <el-col :span="24" class="toolbar" style="padding-bottom: 0px">
                <div class="col-li">
                    <span style="margin-right: 5px"><?= \Yii::t('plugins/teller', '交班时间');?></span>
                    <el-date-picker
                ...
                <div class="col-li">
                    <span style="margin-right: 5px"><?= \Yii::t('plugins/teller', '收银员');?></span>
                    <el-select size="small" @change="searchList" v-model="search.cashier_id" placeholder="<?= \Yii::t('plugins/teller', '请选择收银员');?>">
                        <el-option value="" label="<?= \Yii::t('plugins/teller', '全部收银员');?>"></el-option>
                        <el-option
                ...
                <div class="col-li">
                    <span style="margin-right: 5px"><?= \Yii::t('plugins/teller', '门店名称');?></span>
            ...
            <el-table v-loading="listLoading" :data="list" border>
                <el-table-column prop="name" label="<?= \Yii::t('plugins/teller', '收银员');?>">
                ...
                <el-table-column prop="start_time" label="<?= \Yii::t('plugins/teller', '上班时间');?>"></el-table-column>
                <el-table-column prop="end_time" label="<?= \Yii::t('plugins/teller', '交班时间');?>"></el-table-column>
                <el-table-column prop="total_pay_money" label="<?= \Yii::t('plugins/teller', '订单收款总额');?>">
                    <template slot-scope="scope">
                        ￥{{scope.row.total_pay_money}}
                    </template>
                </el-table-column>
                <el-table-column prop="total_recharge_money" label="<?= \Yii::t('plugins/teller', '会员充值总额');?>">
                    <template slot-scope="scope">
                        ￥{{scope.row.total_recharge_money}}
                    </template>
                </el-table-column>
                <el-table-column prop="refund_money" label="<?= \Yii::t('plugins/teller', '退款总额');?>">
                    <template slot-scope="scope">
                        ￥{{scope.row.refund_money}}
                    </template>
                </el-table-column>
                <el-table-column label="<?= \Yii::t('plugins/teller', '操作');?>" width="180" fixed="right">
                    <template slot-scope="scope">
                        <el-button type="text" @click="showPrintDialog(scope.row)"
                                   size="small" circle>
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/teller', '打印小票');?>" placement="top">
                        ...
                        <el-button size="mini" type="text"
                                   ...
                                   circle>
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/teller', '详情');?>" placement="top">
