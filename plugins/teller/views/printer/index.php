...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header" flex="dir:left" style="justify-content:space-between;">
            <span><?= \Yii::t('plugins/teller', '打印设置');?></span>
            <el-button @click="handleAdd"
                       style="margin: -5px 0"
                       type="primary"
                       size="small">
                <?= \Yii::t('plugins/teller', '添加');?>
            </el-button>
        </div>
        <div class="table-body">
            ...
            <el-table v-loading="listLoading" :data="list" border>
                ...
                <el-table-column prop="printer_name" label="<?= \Yii::t('plugins/teller', '打印机名称');?>"></el-table-column>
                <el-table-column prop="store_name" label="<?= \Yii::t('plugins/teller', '所属门店');?>"></el-table-column>
                <el-table-column prop="status" label="<?= \Yii::t('plugins/teller', '是否启用');?>">
        ...
        <!--编辑界面-->
        <el-dialog title="<?= \Yii::t('plugins/teller', '编辑');?>" :visible.sync="editFormVisible" :close-on-click-modal="false" width="35%">
            <el-form :model="editForm" label-width="120px" :rules="editFormRules" ref="editForm">
                <el-form-item label="<?= \Yii::t('plugins/teller', '选择打印机');?>" prop="printer_id">
                ...
                <el-form-item label="<?= \Yii::t('plugins/teller', '打印参数');?>" prop="show_type">
                    <el-checkbox-group v-model="show_type">
                        <el-checkbox label="attr"><?= \Yii::t('plugins/teller', '规格显示');?></el-checkbox>
                        <el-checkbox label="goods_no"><?= \Yii::t('plugins/teller', '货号显示');?></el-checkbox>
                    </el-checkbox-group>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/teller', '选择门店');?>" prop="store_id">
                    <!--<template slot="label">-->
                    <!--    <span>选择门店</span>-->
                ...
                <el-form-item label="<?= \Yii::t('plugins/teller', '显示大小');?>" prop="big">
                    <!--<template slot="label">-->
                    <!--    <span>显示大小</span>-->
                    ...
                    <el-radio-group v-model="editForm.big">
                        <el-radio :label="0"><?= \Yii::t('plugins/teller', '一倍');?></el-radio>
                        <el-radio :label="1"><?= \Yii::t('plugins/teller', '两倍');?></el-radio>
                        <el-radio :label="2"><?= \Yii::t('plugins/teller', '三倍');?></el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/teller', '是否启用');?>" prop="status">
                    <el-switch
            ...
            <div slot="footer" class="dialog-footer">
                <el-button @click.native="editFormVisible = false"><?= \Yii::t('plugins/teller', '取消');?></el-button>
                <el-button type="primary" @click.native="editSubmit" :loading="editBtnLoading"><?= \Yii::t('plugins/teller', '提交');?></el-button>
            </div>
        </el-dialog>
        <!--新增界面-->
        <el-dialog title="<?= \Yii::t('plugins/teller', '新增');?>" :visible.sync="addFormVisible" :close-on-click-modal="false" width="35%">
            <el-form :model="addForm" label-width="120px" :rules="addFormRules" ref="addForm">
                ...
                <el-form-item label="<?= \Yii::t('plugins/teller', '打印参数');?>" prop="show_type">
                    <el-checkbox-group v-model="show_type">
                        <el-checkbox label="attr"><?= \Yii::t('plugins/teller', '规格');?></el-checkbox>
                        <el-checkbox label="goods_no"><?= \Yii::t('plugins/teller', '货号');?></el-checkbox>
                    </el-checkbox-group>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/teller', '选择门店');?>" prop="store_id">
                    <!--<template slot="label">-->
            ...
            <div slot="footer" class="dialog-footer">
                <el-button @click.native="addFormVisible = false"><?= \Yii::t('plugins/teller', '取消');?></el-button>
                <el-button type="primary" @click.native="addSubmit" :loading="addBtnLoading"><?= \Yii::t('plugins/teller', '提交');?></el-button>
            </div>
        </el-dialog>
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                editFormRules: {
                    printer_id: [
                        {required: true, message: '<?= \Yii::t('plugins/teller', '打印机不能为空');?>', trigger: 'change'}
                    ],
                    store_id: [
                        {required: true, message: '<?= \Yii::t('plugins/teller', '门店不能为空');?>', trigger: 'change'}
                    ],
                },
                //编辑界面数据
                editForm: {
        ...
        methods: {
            ...
            destroy(params, index) {
                this.$confirm('<?= \Yii::t('plugins/teller', '是否删除该打印设置');?>', '<?= \Yii::t('plugins/teller', '提示');?>', {
