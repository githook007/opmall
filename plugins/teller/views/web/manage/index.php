...
<div id="app" v-cloak>
        ...
        <div v-loading="loading" flex-box="1" class="main-center" :class="activeTab == 1 || activeTab == 2 || addMoney || showHung || addCredit || changePrice || loginMember || step > 0 || goods || successPay ? 'padding':''">
            ...
            <div class="tab" flex="dir:left">
                <div @click="chooseTab(0)" class="tab-item" :class="activeTab == 0 && !stepTab && step != 2 && !loginMember && !addCredit ? 'active right' : activeTab == 1 ? 'last-active': ''">
                    <div flex="main:center cross:center">
                        ...
                        <div :style="{'color': activeTab == 0 && step == 0 && !loginMember && !addCredit ? '#3399ff' : '#adb2b8'}"><?= \Yii::t('plugins/teller', '商品');?></div>
                    </div>
                </div>
                <div @click="chooseTab(1)" class="tab-item" :class="(activeTab == 1 && !stepTab && step == 0) || (step == 2 && !stepTab) || loginMember || addCredit || stepTab == 1 ? 'active left right' : activeTab == 2 ? 'last-active': ''">
                    <div flex="main:center cross:center">
                        ...
                        <div :style="{'color': (activeTab == 1 && step == 0) || (step == 2 && !stepTab) || loginMember || addCredit ? '#3399ff' : '#adb2b8'}"><?= \Yii::t('plugins/teller', '会员');?></div>
                    </div>
                </div>
                <div @click="chooseTab(2)" class="tab-item" :class="(activeTab == 2 && !stepTab && step != 2) || stepTab == 2 ? 'active left' : ''">
                    <div flex="main:center cross:center">
                        ...
                        <div :style="{'color': activeTab == 2 && step == 0 ? '#3399ff' : '#adb2b8'}"><?= \Yii::t('plugins/teller', '订单');?></div>
                    </div>
                </div>
                <div @click="chooseTab(3)" v-if="fullReduce || activeTab == 3" class="tab-item" :class="activeTab == 3 && !stepTab && step != 2 && !loginMember && !addCredit ? 'active left' : ''">
                    <div flex="main:center cross:center">
                        ...
                        <div :style="{'color': activeTab == 3 && step == 0 ? '#3399ff' : '#adb2b8'}"><?= \Yii::t('plugins/teller', '满减');?></div>
                    </div>
                </div>
            </div>
            <div class="other-title" v-if="step > 1 && !stepTab && is_pad">
                <div flex="main:justify cross:center" v-if="sales.length > 0 && step != 1">
                    <div v-if="sale" flex="dir:left cross:centere">
                        <div><?= \Yii::t('plugins/teller', '导购员');?><span style="margin: 0 22px 0 7px">{{sale.name}}</span></div>
                        <div><?= \Yii::t('plugins/teller', '编号');?><span style="margin: 0 22px 0 7px">{{sale.number}}</span></div>
                    </div>
                    <div v-else><?= \Yii::t('plugins/teller', '未选择导购员');?></div>
                    <el-button :loading="memberLoading || couponLoading" size="small" type="primary" @click="nextSubmit" round>{{sale ? '<?= \Yii::t('plugins/teller', '切换导购员');?>' : '<?= \Yii::t('plugins/teller', '选择导购员');?>'}}</el-button>
                </div>
                <div flex="main:justify cross:center" v-if="step > 2 || (addCredit && !loginMember)">
                    <div v-if="member" flex="dir:left cross:center">
                        ...
                        <div><?= \Yii::t('plugins/teller', '余额');?><span style="margin: 0 22px 0 7px">{{member.balance}}</span></div>
                    </div>
                    <div v-else><?= \Yii::t('plugins/teller', '未选择会员');?></div>
                    <div>
                        <el-button :loading="memberLoading || couponLoading" v-if="setting.is_coupon == 1 || setting.is_integral == 1" size="small" v-if="member" type="primary" @click="useDiscount" round><?= \Yii::t('plugins/teller', '选择优惠抵扣');?></el-button>
                        <el-button :loading="memberLoading || couponLoading" size="small" type="primary" @click="member=null;mobile='';step=2" round>{{member ? '<?= \Yii::t('plugins/teller', '切换会员');?>' : '<?= \Yii::t('plugins/teller', '选择会员');?>'}}</el-button>
                    </div>
                </div>
            </div>
            <!-- 付款成功 -->
            <div v-if="successPay" class="choose-goods" :style="{'padding-top': is_pad ? '50px':'90px','padding-bottom': view_bottom + 'px'}">
                <div class="choose-goods-title" flex="dir:left cross:center">
                    <div @click="clearOrder" flex="main:center cross:center" class="return-btn">
                        ...
                        <div><?= \Yii::t('plugins/teller', '返回');?></div>
                    </div>
                    <div><?= \Yii::t('plugins/teller', '收款结果');?></div>
                </div>
                <div style="height: 100%;background-color: #fff" flex="dir:top main:center cross:center">
                    ...
                    <div style="font-size: 16px;margin-top: 24px;color: #999999;"><?= \Yii::t('plugins/teller', '收款成功');?></div>
                    <el-button @click="clearOrder" type="primary" style="width: 234px;margin-top: 100px;" plain round><?= \Yii::t('plugins/teller', '返回商品页');?></el-button>
                </div>
            </div>
            <!-- 会员充值 -->
            <div class="choose-goods" v-else-if="addCredit && !loginMember && step < 3 && step != 1" :style="{'padding-top': is_pad ? '50px':'90px','padding-bottom': view_other_bottom + 'px'}">
                <div class="choose-goods-title" flex="dir:left cross:center">
                    ...
                    <div>{{chooseRecharge ? '<?= \Yii::t('plugins/teller', '选择充值方案');?>' : '<?= \Yii::t('plugins/teller', '会员充值');?>'}}</div>
                </div>
                <div class="choose-goods-info member" :class="chooseRecharge? 'choose-recharge':''" :style="{'padding-left': is_pad || !member || !chooseRecharge ? '0': '28%'}" :flex="chooseRecharge ? 'dir:top':'main:center dir:top'">
                    <div v-if="!member" flex="main:center dir:top cross:center" :style="{'margin-top': is_pad ?'15%':0}">
                        ...
                        <div style="text-align: center;color: #999999;"><?= \Yii::t('plugins/teller', '请先登录会员');?></div>
                        <div class="member-login" @click="toLoginMember"><?= \Yii::t('plugins/teller', '去登录');?></div>
                    </div>
                    <div v-else-if="member && chooseRecharge" flex="dir:left">
                        <div @click="rechargeId = item.id;payPrice=item.pay_price" class="recharge-item" flex="dir:top corss:center main:center" v-for="item in rechargeList" :class="rechargeId == item.id ? 'active': ''" :key="item.id">
                            <div style="color: #353535;font-size: 15px"><?= \Yii::t('plugins/teller', '充');?>{{item.pay_price}}<?= \Yii::t('plugins/teller', '元');?></div>
                            <div v-if="item.send_price"><?= \Yii::t('plugins/teller', '送');?>{{item.send_price}}<?= \Yii::t('plugins/teller', '元');?></div>
                            <div v-if="item.send_integral"><?= Yii::t('plugins/teller', '送');?>{{item.send_integral}}<?= \Yii::t('plugins/teller', '积分');?></div>
                            <div v-if="item.member && item.member.id > 0"><?= \Yii::t('plugins/teller', '送');?>{{item.member.name}}</div>
                        </div>
                        <div @click="nextStep" v-if="rechargeList.length > 0" class="next-step absolute"><?= \Yii::t('plugins/teller', '下一步');?></div>
                    </div>
                    <div v-else-if="member && inputRecharge" flex="main:center" :style="{'padding-left': is_pad ? '0': '28%'}">
                        <app-number-input @status="getInputSataus" :pad="is_pad" label="<?= \Yii::t('plugins/teller', '自定义输入');?>" ref="recharge" :name="member && inputRecharge ? 'recharge' : ''" :width="numberWidth" placeholder="<?= \Yii::t('plugins/teller', '请输入金额');?>" @change="submitRecharge"></app-number-input>
                    </div>
                    <div flex="main:center dir:top" v-else>
                        ...
                        <div style="text-align: center;color: #999999;"><?= \Yii::t('plugins/teller', '当前会员可用余额');?><span style="color: #ff4544;font-size: 15px;">￥{{member.balance}}</span></div>
                        <div style="margin-top: 22px;" flex="dir:top cross:center main:center">
                            <div @click="creditType = 1" class="add-credit-type" :class="creditType == 1 ? 'active' : ''" flex="main:center cross:center">
                                ...
                                <div><?= \Yii::t('plugins/teller', '选择充值方式');?></div>
                            </div>
                            <div @click="creditType = 2" class="add-credit-type" :class="creditType == 2 ? 'active' : ''" flex="main:center cross:center">
                                ...
                                <div><?= \Yii::t('plugins/teller', '手动输入金额');?></div>
                            </div>
                        </div>
                        <div @click="submitCreditType" class="next-step" style="margin: 40px auto;"><?= \Yii::t('plugins/teller', '下一步');?></div>
                    </div>
                </div>
            </div>
            <!-- 挂单列表 -->
            <div v-else-if="showHung" class="choose-goods" :style="{'padding-top': is_pad ? '50px':'90px','padding-bottom': view_bottom + 'px'}">
                <div class="choose-goods-title" flex="dir:left cross:center">
                    ...
                    <div><?= \Yii::t('plugins/teller', '取单');?></div>
                </div>
            ...
            <div class="choose-goods" v-else-if="addMoney" :style="{'padding-top': is_pad ? '50px':'90px','padding-bottom': view_bottom + 'px'}">
                <div class="choose-goods-title" flex="dir:left cross:center">
                    ...
                    <div><?= \Yii::t('plugins/teller', '加钱');?></div>
                </div>
                <div class="choose-goods-info member" flex="main:center dir:top">
                    ...
                    <div style="text-align: center;color: #999999;"><?= \Yii::t('plugins/teller', '请添加指定金额加入结算清单');?></div>
                    <app-number-input @status="getInputSataus" :pad="is_pad" ref="add" :name="addMoney ? 'add':''" :width="numberWidth" placeholder="<?= \Yii::t('plugins/teller', '积分');?>" @change="addMoneyGoods"></app-number-input>
                </div>
            </div>
            <!-- 选择改价 -->
            <div class="choose-goods" v-else-if="changePrice" :style="{'padding-top': is_pad ? '50px':'90px','padding-bottom': view_bottom + 'px'}">
                <div class="choose-goods-title" flex="dir:left cross:center">
                    ...
                    <div><?= \Yii::t('plugins/teller', '改价');?></div>
                </div>
                <div class="choose-goods-info member" flex="main:center dir:top">
                    <div class="money-show no-margin">
                        <div><?= \Yii::t('plugins/teller', '应收金额');?></div>
                        <div class="total-money-show">￥<span>{{totalPrice}}</span></div>
                    </div>
                    <div class="change-type" flex="main:center cross:center">
                        <div v-if="(setting.is_goods_change_price_type == 1 && setting.most_plus) || (setting.is_goods_change_price_type == 2 && setting.most_plus_percent)" @click="changeType = 1" :class="changeType == 1 ? 'active' : ''"><?= \Yii::t('plugins/teller', '加价');?></div>
                        <div v-if="(setting.is_goods_change_price_type == 1 && setting.most_subtract) || (setting.is_goods_change_price_type == 2 && setting.most_subtract_percent)" @click="changeType = 2" :class="changeType == 2 ? 'active' : ''"><?= \Yii::t('plugins/teller', '减价');?></div>
                    </div>
                    <app-number-input @status="getInputSataus" :pad="is_pad" :price="changeMoney" ref="change" :name="changePrice? 'change':''" :width="numberWidth" :margin="5" placeholder="请输入金额" @change="changeMoneyInput">
                        <template>
                            <div style="text-align: center;margin-bottom: 5px;color: #cdcdcd">
                                <span v-if="setting.most_plus || setting.most_plus_percent"><?= \Yii::t('plugins/teller', '加价范围在');?>{{orderInfo.change_price_data.most_plus_start}}~{{orderInfo.change_price_data.most_plus_end}}<?= \Yii::t('plugins/teller', '内');?></span>
                                <span v-if="(setting.most_plus && setting.most_subtract) || (setting.most_plus_percent && setting.most_subtract_percent)">，</span>
                                <span v-if="setting.most_subtract || setting.most_subtract_percent"><?= \Yii::t('plugins/teller', '减价范围在');?>{{orderInfo.change_price_data.most_subtract_start}}~{{orderInfo.change_price_data.most_subtract_end}}<?= \Yii::t('plugins/teller', '内');?></span>
                            </div>
                        </template>
                    </app-number-input>
                </div>
            ...
            <div class="choose-goods" v-else-if="(activeTab == 2 && !stepTab && step == 0) || stepTab == 2" :style="{'padding-top': is_pad ? '50px':'90px','padding-bottom': view_bottom + 'px'}">
                <div class="choose-goods-title" flex="dir:left cross:center">
                    ...
                    <div>{{refundDetail ? refundDetail.refund ? '<?= \Yii::t('plugins/teller', '售后详情');?>' : '<?= \Yii::t('plugins/teller', '售后');?>' : orderDetail ? '<?= \Yii::t('plugins/teller', '订单详情');?>' : '<?= \Yii::t('plugins/teller', '订单');?>'}}</div>
                </div>
                <div v-if="!orderDetail && !refundDetail" class="input-item other" :style="{'top': is_pad? '90px' : '140px', 'width': '97%'}">
                    <div class="input-area">
                        <el-input @focus="inputStatus=true" @blur="inputStatus=false"@keyup.enter.native="searchOrderList" size="small" placeholder="<?= \Yii::t('plugins/teller', '请输入订单号搜索');?>" v-model="orderKeyword" clearable @clear="searchOrderList">
                ...
                <div v-if="order.length == 0" style="height: 100%;background-color: #fff;border-top-left-radius: 16px;" flex="dir:top main:center cross:center">
                    ...
                    <div style="font-size: 16px;margin-top: 24px;color: #999999;"><?= \Yii::t('plugins/teller', '暂无订单');?></div>
                </div>
                <div v-else-if="refundDetail" class="choose-goods-info">
                    ...
                    <div class="refund-info" v-if="refundDetail.refund">
                        <div class="refund-label">
                            <span><?= \Yii::t('plugins/teller', '售后类型');?></span>
                            <span style="font-size:15px;margin-left: 15px;color: #353535">{{refundDetail.refund.refund_type}}</span>
                        </div>
                        <div class="refund-label">
                            <span><?= \Yii::t('plugins/teller', '退款途径');?></span>
                            <span style="font-size:15px;margin-left: 15px;color: #353535">{{orderDetail.pay_type}}</span>
                        </div>
                        <div class="refund-label">
                            <span><?= \Yii::t('plugins/teller', '退款说明');?></span>
                            <span style="font-size:15px;margin-left: 15px;color: #353535">{{refundDetail.refund.remark ? refundDetail.refund.remark : '<?= \Yii::t('plugins/teller', '无');?>'}}</span>
                        </div>
                        <div class="refund-label">
                            <span><?= \Yii::t('plugins/teller', '退款金额');?></span>
                            <span style="font-size:15px;margin-left: 15px;color: #ff4544">￥{{refundDetail.refund.refund_price}}</span>
                        </div>
                        <div class="refund-label">
                            <span><?= \Yii::t('plugins/teller', '退款状态');?></span>
                            <span style="font-size:15px;margin-left: 15px;color: #353535">{{refundDetail.refund.status_text}}</span>
                        </div>
                    </div>
                    <div class="refund-info" v-else>
                        <div class="refund-label"><?= \Yii::t('plugins/teller', '售后类型');?></div>
                        <div>
                            <el-button @click="refundType = 3" size="small" :type="refundType == 3 ? 'primary' : ''" round><?= \Yii::t('plugins/teller', '仅退款');?></el-button>
                            <el-button @click="refundType = 1" size="small" :type="refundType == 1 ? 'primary' : ''" round><?= \Yii::t('plugins/teller', '退货退款');?></el-button>
                        </div>
                        <div class="refund-label"><?= \Yii::t('plugins/teller', '退款途径');?></div>
                        <div>
                            <el-button size="small" type="primary" round>{{orderDetail.pay_type}}</el-button>
                            <span style="margin-left: 12px;color: #999999"><?= \Yii::t('plugins/teller', '默认跟订单下单支付方式一致');?></span>
                        </div>
                        <div class="refund-label"><?= \Yii::t('plugins/teller', '退款说明');?></div>
                        <div style="width: 40%;">
                            <el-input @focus="inputStatus=true" @blur="inputStatus=false"type="textarea" :rows="5" maxlength="200" placeholder="<?= \Yii::t('plugins/teller', '请输入文字');?>" v-model="refundRemark"></el-input>
                        </div>
                        <div class="refund-label"><?= \Yii::t('plugins/teller', '退款金额');?></div>
                        <div>
                            <el-input @focus="inputStatus=true" @blur="inputStatus=false"class="refund-price-input" type="number" min="0" oninput="this.value = this.value.replace(/[^0-9\.]/, '');" v-model="refundPrice" :placeholder="'<?= \Yii::t('plugins/teller', '请输入0');?>' + refundDetail.total_price + '<?= \Yii::t('plugins/teller', '之间的金额');?>'"></el-input>
                            <el-button size="small" :loading="refundLoading" @click="toRefund" type="primary" round><?= \Yii::t('plugins/teller', '确认退款');?></el-button>
                        </div>
                    </div>
                </div>
                <div v-else-if="orderDetail" class="choose-goods-info">
                    <div class="hung-list">
                        <div class="order-info" flex="dir:left cross:center">
                            <div><?= \Yii::t('plugins/teller', '订单号');?><span>{{orderDetail.order_no}}</span></div>
                            <div><?= \Yii::t('plugins/teller', '支付方式');?><span>{{orderDetail.pay_type}}</span></div>
                            <div v-if="orderDetail.sales_name"><?= \Yii::t('plugins/teller', '导购员');?><span>{{orderDetail.sales_name}}</span></div>
                            <div><?= \Yii::t('plugins/teller', '收银员');?><span>{{orderDetail.cashier_name}}</span></div>
                        </div>
                        <div class="order-detail" flex="dir:left cross:center" v-for="(item,index) in orderDetail.detail" :key="item.id">
                            ...
                            <div><?= \Yii::t('plugins/teller', '数量');?>{{item.num}}</div>
                            <div><?= \Yii::t('plugins/teller', '小计');?>{{item.total_price}}</div>
                            <div flex="main:center">
                                <el-button v-if="item.refund" size="small" @click="refundDetail = item;" round><?= \Yii::t('plugins/teller', '售后详情');?></el-button>
                                <el-button :disabled="item.is_show_sale == 0" v-else size="small" @click="refundDetail = item;refundType=3;refundRemark='';refundPrice=item.total_price" round><?= \Yii::t('plugins/teller', '售后');?></el-button>
                            </div>
                        </div>
                        <div style="margin-bottom: 4px;"><?= \Yii::t('plugins/teller', '订单总价');?> <span style="color: #ff4544;">￥{{orderDetail.total_pay_price}}</span><span style="margin-left: 16px;color: #999999"><?= \Yii::t('plugins/teller', '等');?>{{orderDetail.goods_count}}<?= \Yii::t('plugins/teller', '件商品');?></span></div>
                        <div style="width: 60%;">
            ...
            <div v-loading="loading || memberLoading || couponLoading" class="choose-goods" v-else-if="(activeTab == 1 && !stepTab && step == 0) || step == 2 || loginMember || stepTab == 1" :style="{'padding-top': is_pad ? step > 1 && !stepTab ? '8px' : '50px' :'90px','padding-bottom': view_other_bottom + 'px'}">
                <div class="choose-goods-title" flex="dir:left cross:center" :class="step > 1 && !stepTab && is_pad && sales.length > 0 ? 'have-sale':''">
                    ...
                    <div><?= \Yii::t('plugins/teller', '选择会员');?></div>
                </div>
                <div class="choose-goods-info member" :class="sales.length > 0 ? '' :'attr-view'" v-if="!member" flex="main:center dir:top">
                    ...
                    <div style="text-align: center;color: #999999;"><?= \Yii::t('plugins/teller', '请扫描付款码或者输入会员手机号以便查询会员');?></div>
                    <app-number-input @status="getInputSataus" :pad="is_pad" ref="member" :name="!member ? 'member': ''" mode="number" :width="numberWidth" placeholder="<?= \Yii::t('plugins/teller', '请扫描付款码或会员手机号');?>" @change="submitMember"></app-number-input>
                </div>
                <div class="choose-goods-info member" v-else>
                    <div class="member-info" v-loading="memberLoading">
                        ...
                        <div v-if="member.vip_discount || member.vip_discount == '0.0'" class="vip-price" flex="dir:left">
                            ...
                            <div class="vip-right">{{member.vip_discount == 0 ? '<?= \Yii::t('plugins/teller', '免费');?>' :  member.vip_discount +'<?= \Yii::t('plugins/teller', '折');?>' }}</div>
                        </div>
                        <div style="margin-bottom: 10px;"><span class="member-label"><?= \Yii::t('plugins/teller', '会员类型');?></span>{{member.member_name}}</div>
                        <div style="margin-bottom: 10px;"><span class="member-label"><?= \Yii::t('plugins/teller', '可用积分');?></span>{{addCredit ? member.integral : orderInfo ? orderInfo.integral.use_num : member.integral}}</div>
                        <div style="margin-bottom: 10px;"><span class="member-label"><?= \Yii::t('plugins/teller', '可用余额');?></span>￥{{member.balance}}</div>
                        <div style="margin-bottom: 10px;"><span class="member-label"><?= \Yii::t('plugins/teller', '可用优惠券');?></span>{{addCredit ? member.coupon_count : couponList.length}}<?= \Yii::t('plugins/teller', '张');?></div>
                        <div v-if="setting.is_coupon == 1 || setting.is_integral == 1">
                            <el-button type="primary" @click="useDiscount" plain round>
                                <div flex="main:center cross:center">
                                    ...
                                    <span style="margin-left: 4px"><?= \Yii::t('plugins/teller', '使用优惠抵扣');?></span>
                                </div>
                            </el-button>
                        </div>
                        <div>
                            <el-button @click="reSearch" round><?= \Yii::t('plugins/teller', '重新搜索');?></el-button>
                        </div>
                        <div>
                            <el-button type="primary" @click="checkInMember" round><?= \Yii::t('plugins/teller', '确定');?></el-button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div v-else-if="goods" class="choose-goods" :style="{'padding-top': is_pad ? '50px':'90px','padding-bottom': view_bottom + 'px'}">
                <div class="choose-goods-title" flex="dir:left cross:center">
                    ...
                    <div><?= \Yii::t('plugins/teller', '选择商品');?></div>
                </div>
                <div class="choose-goods-info attr-view">
                    <div class="detail-goods" flex="dir:left cross:center">
                        ...
                        <div class="detail-goods-info">
                            ...
                            <div v-if="!selectAttr" style="color: #ff4544;font-size: 16px;">￥{{goods.price_min}}<span v-if="goods.price_max > goods.price_min">~￥{{goods.price_max}}</span><span style="margin-left: 16px;font-size: 15px;color: #999999"><?= \Yii::t('plugins/teller', '库存');?>{{goods.goods_stock}}</span></div>
                            <div v-else style="color: #ff4544;font-size: 16px;">￥{{selectAttr.price}}</span><span style="margin-left: 16px;font-size: 15px;color: #999999"><?= \Yii::t('plugins/teller', '库存');?>{{selectAttr.stock}}</span></div>
                        </div>
                    </div>
                    <div>
                    ...
                    <div class="detail-goods-attr detail-goods-number" v-if="selectAttr.stock > 0" :style="{'bottom': view_bottom + 'px'}">
                        <div class="detail-attr-name"><?= \Yii::t('plugins/teller', '数量');?></div>
                        <div flex="dir:left cross:center">
                            ...
                            <el-button size="small" @click="submitGoods" type="primary" round><?= \Yii::t('plugins/teller', '确定');?></el-button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="choose-goods" v-else-if="step == 1 && sales.length > 0" :style="{'padding-top': is_pad ? '50px':'90px','padding-bottom': view_bottom + 'px'}">
                <div class="choose-goods-title" flex="dir:left cross:center">
                    ...
                    <div><?= \Yii::t('plugins/teller', 'teller_z411');?></div>
                </div>
            ...
            <div class="choose-goods" v-else-if="step == 4 && payment_type == 'cash'" :style="{'padding-top': is_pad ? '50px':'90px','padding-bottom': view_other_bottom + 'px'}">
                <div class="choose-goods-title" flex="dir:left cross:center"  :class="is_pad && (sales.length == 0 && step > 2) ? 'have-sale':''">
                    ...
                    <div><?= \Yii::t('plugins/teller', '现金收款');?></div>
                </div>
                <div class="choose-goods-info member" flex="dir:top main:center">
                    ...
                    <app-number-input @status="getInputSataus" :pad="is_pad" :price="getMoney" ref="cash" :name="step == 4 && payment_type == 'cash' ? 'cash':''" :width="numberWidth" :margin="5" placeholder="<?= \Yii::t('plugins/teller', '实收金额');?>" @change="submitPay" @input="getMoneyInput">
                        <template>
                            <div flex="main:justify" :style="{'width': numberWidth / 3.5 > 480 ? '480px':numberWidth / 3.5 +'px', 'margin': is_pad ? '6px auto 10px' : '20px auto','font-size': is_pad? '14':'16px','color': '666666'}">
                                <div flex="dir:left cross:center">
                                    <div><?= \Yii::t('plugins/teller', '快捷输入');?></div>
                                ...
                                <div v-if="addCredit"><?= \Yii::t('plugins/teller', '找零');?><span style="color: #ff4544;">￥{{+getMoney > +payPrice ? (getMoney - +payPrice).toFixed(2): '0.00'}}</span></div>
                                <div v-else><?= \Yii::t('plugins/teller', '找零');?><span style="color: #ff4544;">￥{{+getMoney > +totalPrice ? (getMoney - +totalPrice).toFixed(2): '0.00'}}</span></div>
                            </div>
                        </template>
                    </app-number-input>
                </div>
            ...
            <div class="choose-goods" v-else-if="step > 3 && payment_type == 'balance'" :style="{'padding-top': is_pad ? '50px':'90px','padding-bottom': view_other_bottom + 'px'}">
                <div class="choose-goods-title" flex="dir:left cross:center"  :class="is_pad && (sales.length == 0 && step > 2) ? 'have-sale':''" flex="dir:left cross:center">
                    ...
                    <div>{{verifyPassword ?  '<?= \Yii::t('plugins/teller', '确认支付密码');?>' : setPassword || (step == 5 && balanceType == 2) ? '<?= \Yii::t('plugins/teller', '输入支付密码');?>': '<?= \Yii::t('plugins/teller', '会员余额收款');?>'}}</div>
                </div>
                <div class="choose-goods-info member" flex="dir:top main:center">
                    ...
                    <div flex="main:center dir:top" v-if="step == 5 && balanceType == 1">
                        <div style="text-align: center;color: #999999;"><?= \Yii::t('plugins/teller', '请使用扫码枪扫描客户付款码');?></div>
                        <el-input @focus="inputStatus=true" @blur="inputStatus=false" @keyup.enter.native="submitPay" ref="barcode" autofocus size="small" class="barcode-input" v-model="memberBarcode" placeholder="<?= \Yii::t('plugins/teller', '请输入付款码');?>"></el-input>
                        <div :style="{'margin-top': is_pad?'26px':'40px'}" flex="main:center" v-loading="countLoading">
                            <div  @click="submitPay" class="next-step"><?= \Yii::t('plugins/teller', '确认收款');?></div>
                        </div>
                    </div>
                    ...
                    <div v-loading="memberLoading" class="balance-pay-type" flex="main:center dir:top" v-if="step == 4 && !setPassword">
                        ...
                        <div :style="{'margin-top': is_pad ? '12px': '22px'}" flex="dir:top cross:center main:center">
                            <div @click="balanceType = 1" class="add-credit-type" :class="balanceType == 1 ? 'active' : ''" flex="main:center cross:center">
                                ...
                                <div><?= \Yii::t('plugins/teller', '动态付款码');?></div>
                            </div>
                            <div v-if="setting.is_balance_pay_password == 1" @click="balanceType = 2" class="add-credit-type" :class="balanceType == 2 ? 'active' : ''" flex="main:center cross:center">
                                ...
                                <div><?= \Yii::t('plugins/teller', '余额支付密码');?></div>
                            </div>
                        </div>
                        <div @click="submitBalanceType" class="next-step absolute"><?= \Yii::t('plugins/teller', '下一步');?></div>
                    </div>
                </div>
            </div>
            <!-- 其他收款 -->
            <div class="choose-goods" v-else-if="step == 4 && payment_type != 'cash' && payment_type != 'balance'" :style="{'padding-top': is_pad ? '50px':'90px','padding-bottom': view_other_bottom + 'px'}">
                <div class="choose-goods-title" flex="dir:left cross:center"  :class="is_pad && (sales.length == 0 && step > 2) ? 'have-sale':''">
                    ...
                    <div>{{payment_type == 'pos' ? '<?= \Yii::t('plugins/teller', 'POS机收款');?>':payment_type == 'wechat_scan' ? '<?= \Yii::t('plugins/teller', '微信收款');?>':'<?= \Yii::t('plugins/teller', '支付宝收款');?>'}}</div>
                </div>
                <div class="choose-goods-info member" flex="dir:top main:center">
                    <div class="money-show">
                        <div><?= \Yii::t('plugins/teller', '应收金额');?></div>
                    ...
                    <div v-if="payment_type == 'pos'" style="text-align: center;color: #999999;"><?= \Yii::t('plugins/teller', '请引导顾客刷卡支付');?></div>
                    <div v-else style="text-align: center;color: #999999;"><?= \Yii::t('plugins/teller', '请使用扫码枪扫描客户付款码');?></div>
                    <el-input @focus="inputStatus=true" @blur="inputStatus=false"@keyup.enter.native="submitPay" ref="barcode" v-if="payment_type != 'pos'" autofocus size="small" class="barcode-input" v-model="barcode" placeholder="<?= \Yii::t('plugins/teller', '请输入付款码');?>"></el-input>
                    <div :style="{'margin-top': is_pad?'26px':'40px'}" flex="main:center" v-loading="countLoading">
                        <div  @click="submitPay" class="next-step"><?= \Yii::t('plugins/teller', '确认收款');?></div>
                    </div>
                </div>
            ...
            <div class="choose-goods" v-else-if="activeTab == 1 || step == 3" :style="{'padding-top': is_pad ? '50px':'90px','padding-bottom': view_other_bottom + 'px'}">
                <div class="choose-goods-title" flex="dir:left cross:center"  :class="is_pad && (sales.length == 0 && step > 2) ? 'have-sale':''">
                    ...
                    <div><?= \Yii::t('plugins/teller', '选择支付方式');?></div>
                </div>
                <div class="choose-goods-info" flex="main:center dir:top">
                    <div class="pay-list" :style="{'justify-content': setting.payment_type.length > 3 ? 'flex-start' : 'center'}">
                        <div @click="payment_type = item" :class="payment_type == item ? 'active':''" class="pay-type" v-for="(item,index) in setting.payment_type" :key="index" flex="dir:top main:center cross:center" v-if="(addCredit && item != 'balance') || (!addCredit && ((member && item == 'balance') || item != 'balance'))">
                            ...
                            <div>
                                {{item == 'wechat_scan' ? '<?= \Yii::t('plugins/teller', '微信');?>' : item == 'alipay_scan' ? '<?= \Yii::t('plugins/teller', '支付宝');?>' : item == 'balance' ? '<?= \Yii::t('plugins/teller', '余额');?>' : item == 'cash' ? '<?= \Yii::t('plugins/teller', '现金');?>' : 'POS'}}
                            </div>
                        </div>
                    </div>

                    <div class="pay-list" v-loading="countLoading" style="justify-content: center;margin-top: 0;">
                        <div @click="nextStep" class="next-step"><?= \Yii::t('plugins/teller', '确定');?></div>
                    </div>
                </div>
            </div>
            <!-- 选择满减商品 -->
            <div class="goods-view" :style="{'padding-bottom': (screenHeight - 70) * 0.0859 + 'px'}" v-else-if="(activeTab == 3 && step == 0) || stepTab == 3">
                <div class="input-item">
                    <div class="full-reduce"><?= \Yii::t('plugins/teller', '满减优惠');?>
                        <template v-if="fullReduce.rule_type === 1">
                                <span v-for="(item, index) in fullReduce.rule"
                                      :key="index">
                                    <?= \Yii::t('plugins/teller', '满');?>{{item.min_money}}{{item.discount_type === '1' ? '<?= \Yii::t('plugins/teller', '减');?>' + item.cut : '<?= \Yii::t('plugins/teller', '打');?>' + item.discount + '<?= \Yii::t('plugins/teller', '折');?>'}}{{index !== fullReduce.rule.length - 1 ? ', ' : ''}}
                                </span>
                        </template>
                        <template v-else-if="fullReduce.rule_type === 2">
                            <?= \Yii::t('plugins/teller', '每满');?>{{fullReduce.rule.min_money}}<?= \Yii::t('plugins/teller', '减');?>{{fullReduce.rule.cut}}
                        </template>
                    </div>
                    <div class="input-area">
                        <el-input @focus="inputStatus=true" @blur="inputStatus=false"@keyup.enter.native="toSearch" size="small" placeholder="<?= \Yii::t('plugins/teller', '请输入商品名称');?>" v-model="fullKeyword" clearable @clear="toSearch">
            ...
            <div class="goods-view" :style="{'padding-bottom': (screenHeight - 70) * 0.0859 + 'px'}" v-else-if="activeTab == 0 && step == 0">
                <div class="input-item">
                    <div class="input-area">
                        <el-input @focus="inputStatus=true" @blur="inputStatus=false"@keyup.enter.native="toSearch" size="small" placeholder="<?= \Yii::t('plugins/teller', '请输入商品名称');?>" v-model="keyword" clearable @clear="toSearch">
                            <el-button slot="append" icon="el-icon-search" @click="toSearch"></el-button>
                        </el-input>
                    </div>
                </div>
            ...
            <div class="other-info" v-if="!is_pad && !stepTab && ((activeTab != 0 && activeTab != 3) || step > 0)">
                <div class="other-item" flex="dir:top cross:center" v-if="(step > 1 || chooseRecharge || inputRecharge) && sales.length > 0 && !successPay">
                    <div class="other-item-line" v-if="sale" flex="main:justify">
                        <div class="other-item-label"><?= \Yii::t('plugins/teller', '导购员');?></div>
                        <div>{{sale.name}}</div>
                    </div>
                    <div class="other-item-line" v-if="sale" flex="main:justify">
                        <div class="other-item-label"><?= \Yii::t('plugins/teller', '编号');?></div>
                        <div>{{sale.number}}</div>
                    </div>
                    <div class="other-item-line" :style="{'margin-bottom': !sale ? '0': '5px'}" flex="main:justify cross:center">
                        <div :style="{'visibility': !sale ? 'visible' : 'hidden'}"><?= \Yii::t('plugins/teller', '未选择导购员');?></div>
                        <el-button size="small" type="primary" @click="nextSubmit" round>{{sale ? '<?= \Yii::t('plugins/teller', '切换导购员');?>' : '<?= \Yii::t('plugins/teller', '选择导购员');?>'}}</el-button>
                    </div>
                </div>
                <div class="other-item" flex="dir:top cross:center" v-if="(step > 2 && !successPay) || (addCredit && member && !loginMember && step != 1)">
                    ...
                    <div class="other-item-line" v-if="member" flex="main:justify">
                        <div class="other-item-label"><?= \Yii::t('plugins/teller', '余额');?></div>
                        <div>{{member.balance}}</div>
                    </div>
                    <div class="other-item-line" :style="{'margin-bottom': !member ? '0': '5px'}" flex="main:justify cross:center">
                        <el-button size="small" v-if="member && (setting.is_coupon == 1 || setting.is_integral == 1)" type="primary" @click="useDiscount" round><?= \Yii::t('plugins/teller', '选择优惠抵扣');?></el-button>
                        <div v-if="!member"><?= \Yii::t('plugins/teller', '未选择会员');?></div>
                        <el-button size="small" type="primary" @click="member=null;mobile='';step=2" round>{{member ? '<?= \Yii::t('plugins/teller', '切换会员');?>' : '<?= \Yii::t('plugins/teller', '选择会员');?>'}}</el-button>
                    </div>
                </div>
            </div>
            <div v-if="(step == 1 || step == 2) && !(addCredit && step != 1) && !stepTab && !changePrice && !addMoney && !(((activeTab == 1 && step == 0) || step == 2 || loginMember) && member)" @click="nextStep" class="next-step absolute"><?= \Yii::t('plugins/teller', '下一步');?></div>
        </div>
        <div flex-box="0" class="menu">
            <div class="menu-list">
                <div class="menu-title">
                    <div>
                        <?= \Yii::t('plugins/teller', '结算清单');?>{{totalCountNumber}}<?= \Yii::t('plugins/teller', '件');?>
                        <el-tag style="margin-left: 18px;" effect="plain" size="mini"><?= \Yii::t('plugins/teller', '收银员');?>{{cashier.number}}</el-tag>
                    </div>
                </div>
                <div class="menu-goods-list">
                    <div style="margin-top: 30%;" flex="dir:top main:center cross:center" v-if="count.length == 0">
                        ...
                        <div style="margin-top: 25px;color: #999"><?= \Yii::t('plugins/teller', '还没有选中任何商品');?></div>
                    </div>
            ...
            <div class="menu-bottom" :class="is_pad?'pad-menu-bottom':''">
                ...
                <div class="menu-button" flex="main:justify">
                    <el-button @click="cancel" round><?= \Yii::t('plugins/teller', '整单取消');?></el-button>
                    <el-button @click="nextSubmit('count')" class="money" :loading="countLoading" round><?= \Yii::t('plugins/teller', '收款');?></el-button>
                </div>
            </div>
        </div>
    </div>
    <div class="main-view other" :style="{'padding-top': is_pad ? screenHeight * 0.0573 + 'px' : '90px'}" :class="is_pad ? 'is_pad': ''" v-if="is_cashier">
        <div class="tab-nav" flex="dir:left">
            <div @click="is_cashier = false" class="tab-nav-item tab-cashier" flex="main:center cross:center">
                ...
                <div><?= \Yii::t('plugins/teller', '收银台');?></div>
            </div>
            <div class="tab-nav-item" flex="main:center cross:center"><?= \Yii::t('plugins/teller', '个人信息');?></div>
        </div>
        <div class="cashier-info">
            ...
            <el-form class="cashier-about" :model="cashier" label-position="left" label-width="100px">
                <el-form-item label="<?= \Yii::t('plugins/teller', '收银员编号');?>" prop="number">
                    <div>{{cashier.number}}</div>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/teller', '姓名');?>" prop="nickname">
                    <div>{{cashier.nickname}}</div>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/teller', '电话');?>" prop="mobile">
                    <div>{{cashier.mobile}}</div>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/teller', '帐号');?>" prop="username">
                    <div>{{cashier.username}}</div>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/teller', '密码');?>" prop="password">
                    <el-button size="small" @click="togglePassword" round><?= \Yii::t('plugins/teller', '修改密码');?></el-button>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/teller', '门店');?>" prop="store_name">
    ...
    <div class="main-view other" :class="is_pad ? 'is_pad': ''" :style="{'padding-top': is_pad ? screenHeight * 0.0573 + 'px' : '90px'}" v-if="cashierInfo">
        <div class="tab-nav" flex="dir:left">
            ...
            <div class="tab-nav-item" flex="main:center cross:center"><?= \Yii::t('plugins/teller', '交班');?></div>
        </div>
        <div class="cashier-info get-off" :style="{'padding-top': is_pad? '35px':'70px'}">
            <div class="cashier-about">
                <div class="cashier-title" flex="dir:left cross:center">
                    ...
                    <div><?= \Yii::t('plugins/teller', '收银员');?></div>
                </div>
                <div class="cashier-form">
                    <div><?= \Yii::t('plugins/teller', '收银员');?><span>{{cashierInfo.name}}</span></div>
                    <div><?= \Yii::t('plugins/teller', '收银编号');?><span>{{cashierInfo.number}}</span></div>
                    <div><?= \Yii::t('plugins/teller', '上班时间');?><span>{{cashierInfo.start_time}}</span></div>
                    <div><?= \Yii::t('plugins/teller', '交班时间');?><span>{{cashierInfo.end_time}}</span></div>
                    <div><?= \Yii::t('plugins/teller', '时长');?><span>{{cashierInfo.hour}}</span></div>
                </div>
            </div>
            <div class="cashier-content">
                <div class="cashier-content-item">
                    <div class="cashier-title" flex="dir:left cross:center">
                        ...
                        <div><?= \Yii::t('plugins/teller', '收款总额');?></div>
                    </div>
                    <div flex="dir:left cross:center">
                        <div class="cashier-info-item" flex="dir:top main:center cross:center">
                            <div class="cashier-info-label" :style="{'margin-bottom': is_pad?'20px':'40px'}"><?= \Yii::t('plugins/teller', '收款总额');?></div>
                        ...
                        <div class="cashier-info-item" flex="dir:top main:center cross:center">
                            ...
                            <div class="cashier-info-label"><?= \Yii::t('plugins/teller', '微信');?></div>
                        ...
                        <div class="cashier-info-item" flex="dir:top main:center cross:center">
                            ...
                            <div class="cashier-info-label"><?= \Yii::t('plugins/teller', '支付宝');?></div>
                        ...
                        <div class="cashier-info-item" flex="dir:top main:center cross:center">
                            ...
                            <div class="cashier-info-label"><?= \Yii::t('plugins/teller', '现金');?></div>
                        ...
                        <div class="cashier-info-item" flex="dir:top main:center cross:center">
                            ...
                            <div class="cashier-info-label"><?= \Yii::t('plugins/teller', '会员余额');?></div>
                        ...
                        <div class="cashier-info-item" flex="dir:top main:center cross:center">
                            ...
                            <div class="cashier-info-label"><?= \Yii::t('plugins/teller', 'POS机');?></div>
                ...
                <div class="cashier-content-item">
                    <div class="cashier-title" flex="dir:left cross:center">
                        ...
                        <div><?= \Yii::t('plugins/teller', '充值总额');?></div>
                    </div>
                    <div flex="dir:left cross:center">
                        <div class="cashier-info-item" flex="dir:top main:center cross:center">
                            <div class="cashier-info-label" :style="{'margin-bottom': is_pad?'20px':'40px'}"><?= \Yii::t('plugins/teller', '充值总额');?></div>
                ...
                <div class="cashier-content-item">
                    <div class="cashier-title" flex="dir:left cross:center">
                        ...
                        <div><?= \Yii::t('plugins/teller', '总计');?></div>
                    </div>
                    <div flex="dir:left cross:center">
                        <div class="cashier-info-item" flex="dir:top main:center cross:center">
                            <div class="cashier-info-label" :style="{'margin-bottom': is_pad?'20px':'40px'}"><?= \Yii::t('plugins/teller', '收款总额');?></div>
                    ...
                    <div class="menu-button" v-if="is_pad" flex="dir:right">
                        <el-button :loading="loading" @click="getOff" class="money" round><?= \Yii::t('plugins/teller', '确认交班');?></el-button>
                        <el-button :loading="loading" @click="print" round><?= \Yii::t('plugins/teller', '打印小票');?></el-button>
                    </div>
                </div>
                <div class="menu-button" v-if="!is_pad" flex="dir:right">
                    <el-button :loading="loading" @click="getOff" class="money" round><?= \Yii::t('plugins/teller', '确认交班');?></el-button>
        ...
        <div>
            <div style="margin-bottom: 20px;" class="el-message-box__content">
                ...
                <div class="el-message-box__message">
                    <p><?= \Yii::t('plugins/teller', '确认选择该导购员');?></p>
                </div>
            </div>
            <div v-if="sale" class="sale-info">
                <div><?= \Yii::t('plugins/teller', '导购员编号');?><span>{{sale.number}}</span></div>
                <div><?= \Yii::t('plugins/teller', '姓名');?><span>{{sale.name}}</span></div>
                <div><?= \Yii::t('plugins/teller', '电话');?><span>{{sale.mobile}}</span></div>
                <div flex="cross:top"><?= \Yii::t('plugins/teller', '头像');?>
                    <img :src="sale.head_url? sale.head_url : 'statics/img/app/user-default-avatar.png'" alt="">
                </div>
                <div><?= \Yii::t('plugins/teller', '门店');?><span>{{sale.store_name}}</span></div>
            </div>
        </div>
        <span slot="footer" class="dialog-footer">
            <el-button size="small" @click="chooseSaleVisible = false"><?= \Yii::t('plugins/teller', '取消');?></el-button>
            <el-button size="small" type="primary" @click="chooseSaleVisible = false;saleIndex = openSaleIndex"><?= \Yii::t('plugins/teller', '确定');?></el-button>
        </span>
    </el-dialog>
    <!-- 修改密码 -->
    <el-dialog class="change-password" title="<?= \Yii::t('plugins/teller', '修改密码');?>" :visible.sync="passwordVisible" width="32%">
        <el-form style="width: 85%;margin: 0 auto" ref="passwordForm" :model="passwordForm" :rules="rules" label-width="80px">
            <el-form-item label="<?= \Yii::t('plugins/teller', '旧密码');?>" prop="password_old">
                <el-input @focus="inputStatus=true" @blur="inputStatus=false"type="password" size="small" placeholder="<?= \Yii::t('plugins/teller', '请输入旧密码');?>" maxlength="16" v-model="passwordForm.password_old" autocomplete="off"></el-input>
            </el-form-item>
            <el-form-item label="<?= \Yii::t('plugins/teller', '新密码');?>" prop="password">
                <el-input @focus="inputStatus=true" @blur="inputStatus=false"type="password" size="small" placeholder="<?= \Yii::t('plugins/teller', '请输入新密码');?>" maxlength="16" v-model="passwordForm.password" autocomplete="off"></el-input>
            </el-form-item>
            <el-form-item label="<?= \Yii::t('plugins/teller', '确认密码');?>" prop="password_verify">
                <el-input @focus="inputStatus=true" @blur="inputStatus=false"type="password" size="small" placeholder="<?= \Yii::t('plugins/teller', '请再次输入密码');?>" maxlength="16" v-model="passwordForm.password_verify" autocomplete="off"></el-input>
            </el-form-item>
        </el-form>
        <span slot="footer" class="dialog-footer">
            <el-button size="small" @click="togglePassword"><?= \Yii::t('plugins/teller', '取消');?></el-button>
            <el-button size="small" type="primary" :loading="passwordLoading" @click="submitChangePassword('passwordForm')"><?= \Yii::t('plugins/teller', '确定');?></el-button>
        </span>
    </el-dialog>
    <!-- 成功提示 -->
    ...
    <el-dialog :title="remarkTitle" :visible.sync="remarkVisible" width="32%">
        <el-input @focus="inputStatus=true" @blur="inputStatus=false"type="textarea" :rows="5" placeholder="<?= \Yii::t('plugins/teller', '请填写备注内容');?>" v-model="remark" autocomplete="off"></el-input>
        <span slot="footer" class="dialog-footer">
            <el-button size="small" @click="remarkVisible = false"><?= \Yii::t('plugins/teller', '取消');?></el-button>
            <el-button size="small" :loading="remarkLoading" type="primary" @click="submitRemark"><?= \Yii::t('plugins/teller', '确定');?></el-button>
        </span>
    </el-dialog>
    <!-- 选择用户 -->
    <el-dialog title="<?= \Yii::t('plugins/teller', '选择用户');?>" :visible.sync="memberVisible" width="610">
        <div>
        ...
        <span slot="footer" class="dialog-footer">
            <el-button size="small" @click="memberVisible = false"><?= \Yii::t('plugins/teller', '取消');?></el-button>
            <el-button size="small" type="primary" @click="chooseMember"><?= \Yii::t('plugins/teller', '确定');?></el-button>
        </span>
    </el-dialog>
    <!-- 选择优惠 -->
    <el-dialog title="<?= \Yii::t('plugins/teller', '选择优惠');?>" :visible.sync="couponVisible" width="460px">
        <div v-if="member">
            <div class="member-coupon">
                <div v-if="couponList.length > 0 && setting.is_coupon == 1">
                    <div><?= \Yii::t('plugins/teller', '优惠券');?></div>
                    <div class="coupon-input">
                        <div>
                            <el-input @focus="inputStatus=true" @blur="inputStatus=false"@keyup.enter.native="CouponKeywordSearch" size="small" placeholder="<?= \Yii::t('plugins/teller', '请输入优惠券名称');?>" v-model="couponKeyword" clearable @clear="CouponKeywordSearch">
                        ...
                        <div v-if="couponListLength > 0">
                            <div @click="chooseCoupon(item)" class="coupon-list-item" :class="item.id == temp_use_coupon.id ? 'active' :''" v-for="item in couponList" :key="item.id" v-if="item.show">
                                ...
                                <div class="item-left" flex="dir:top main:center">
                                    ...
                                    <div class="coupon-price t-omit" v-else>{{item.coupon_data.discount}}<?= \Yii::t('plugins/teller', '折');?></div>
                                    <div style="font-size: 10px" class="t-omit" v-if="item.coupon_min_price > 0"><?= \Yii::t('plugins/teller', '满');?>{{item.coupon_data.min_price}}<?= \Yii::t('plugins/teller', '可用');?></div>
                                    <div style="font-size: 10px" v-else><?= \Yii::t('plugins/teller', '无门槛使用');?></div>
                                    <div style="font-size: 10px" v-if="item.coupon_data.discount_limit"><?= \Yii::t('plugins/teller', '优惠上限');?>{{item.coupon_data.discount_limit}}</div>
                                </div>
                                <div class="item-right" flex="dir:top main:center">
                                    ...
                                    <div v-if="item.coupon_data.appoint_type == 3"><?= \Yii::t('plugins/teller', '全场通用');?></div>
                                    <div v-else-if="item.coupon_data.appoint_type == 4"><?= \Yii::t('plugins/teller', '仅限当面付活动使用');?></div>
                                    <div v-else-if="item.coupon_data.appoint_type == 5"><?= \Yii::t('plugins/teller', '仅限礼品卡使用');?></div>
                                    <div v-else><?= \Yii::t('plugins/teller', '限品类');?></div>
                                </div>
                            </div>
                        </div>
                        <div v-else flex="dir:top main:center cross:center">
                            <img width="150" height="112" src="./../plugins/teller/assets/img/no-coupon.png" alt="">
                            <div style="font-size: 16px;margin-top: 24px;color: #999999;"><?= \Yii::t('plugins/teller', '暂无可用优惠券');?></div>
                        </div>
                    </div>
                </div>
                <div v-if="setting.is_integral == 1">
                    <el-switch v-model="tempUseIntegral" :disabled="orderInfo.integral.use_num == 0" :active-text="'(<?= \Yii::t('plugins/teller', '可用积分');?>:'+orderInfo.integral.use_num+')'" inactive-text="<?= \Yii::t('plugins/teller', '使用积分');?>"></el-switch>
                </div>
            </div>
        </div>
        <span slot="footer" class="dialog-footer">
            <el-button size="small" @click="couponVisible = false"><?= \Yii::t('plugins/teller', '取消');?></el-button>
            <el-button size="small" type="primary" @click="checkInMember(1)"><?= \Yii::t('plugins/teller', '确定');?></el-button>
        </span>
    </el-dialog>
    <!-- 优惠明细 -->
    <el-dialog title="<?= \Yii::t('plugins/teller', '优惠明细');?>" :visible.sync="discountVisible" width="320px">
        <div style="margin-top: -20px;font-size: 16px;">
            <div style="margin-bottom: 5px;" flex="main:justify" v-if="orderInfo.integral && orderInfo.integral.use">
                <div style="color: #6c6c6c;"><?= \Yii::t('plugins/teller', '积分抵扣');?></div>
                ...
                <div style="color: #6c6c6c;"><?= \Yii::t('plugins/teller', '优惠券优惠');?></div>
            ...
            <div style="margin-bottom: 5px;" v-if="+orderInfo.full_reduce_discount > 0" flex="main:justify">
                <div style="color: #6c6c6c;"><?= \Yii::t('plugins/teller', '满减优惠');?></div>
            ...
            <div style="margin-bottom: 5px;" v-if="+orderInfo.member_discount > 0" flex="main:justify">
                <div style="color: #6c6c6c;"><?= \Yii::t('plugins/teller', '会员折扣');?></div>
            ...
            <div style="margin-bottom: 5px;" v-if="+orderInfo.vip_discount > 0" flex="main:justify">
                <div style="color: #6c6c6c;"><?= \Yii::t('plugins/teller', '超级会员卡');?></div>
            ...
            <div style="margin-bottom: 5px;" v-if="+orderInfo.total_change_price > 0" flex="main:justify">
                <div style="color: #6c6c6c;"><?= \Yii::t('plugins/teller', '订单改价');?></div>
            ...
            <div style="margin-bottom: 5px;" v-if="+orderInfo.erase_price > 0 || +orderInfo.erase_price < 0" flex="main:justify">
                <div style="color: #6c6c6c;"><?= \Yii::t('plugins/teller', '抹零');?></div>
...
<script>
    const app = new Vue({
        ...
        data() {
            let checkPassword = (rule, value, callback) => {
                if (!this.passwordForm.password_verify) {
                    callback(new Error('<?= \Yii::t('plugins/teller', '请再次输入密码');?>'))
                }else if (this.passwordForm.password != this.passwordForm.password_verify) {
                    callback(new Error('<?= \Yii::t('plugins/teller', '两次密码不一致');?>'))
                } else {
                    callback();
                }
            };
            return {
                ...
                rules: {  // 改密表单校验规则
                    password: [
                        {required: true, message: '<?= \Yii::t('plugins/teller', '请输入新密码');?>'},
                        {min: 6, max: 16, message: "<?= \Yii::t('plugins/teller', '密码长度在6');?>"},
                    ],
                    password_verify: [
                        {validator: checkPassword, trigger: 'blur'},
                    ],
                    password_old: [
                        {required: true, message: '<?= \Yii::t('plugins/teller', '请输入旧密码');?>'},
                        {min: 6, max: 16, message: "<?= \Yii::t('plugins/teller', '密码长度在6');?>"},
                    ],
                },
                showHung: false, // 取单
        ...
        mounted() {
            window.addEventListener("keydown", this.inputCode);
            window.onresize = () => {
                return (() => {
        ...
        methods: {
            ...
            inputCode(e) {
                if(!this.inputStatus) {
                    if(this.getGoodsCodeMode) {
                        if(e.key == 'Enter') {
                            this.searchBarCode();
                        }else if(e.key.length == 1) {
                            this.goods_bar_code += e.key
                        }
                    }
                    if(e.key == 'CapsLock') {
                        this.getGoodsCodeMode = !this.getGoodsCodeMode;
                        this.goods_bar_code = '';
                    }
                    setTimeout(()=>{
                        this.getGoodsCodeMode = false;
                    },1000)
                }
            },
            searchBarCode() {
            ...
            // 清空订单状态
            clearOrder() {
                if(this.successMsg != '<?= \Yii::t('plugins/teller', '充值成功');?>') {
                    this.saleIndex = -1;
            ...
            toggleView(name) {
                if(!this.orderInfo.change_price_data && name == 'changePrice') {
                    this.$message.error('<?= \Yii::t('plugins/teller', '暂不能改价');?>');
                    return false;
                }
                this.addMoney = false;
                this.showHung = false;
            ...
            closeSuccess() {
                ...
                if(this.successMsg == '<?= \Yii::t('plugins/teller', '退款成功');?>') {
                ...
                if(this.successMsg == '<?= \Yii::t('plugins/teller', '设置成功');?>') {
                    this.setPassword = false;
                    this.verifyPassword = false;
                    this.pay_password = '';
                    this.verify_pay_password = '';
                }
                if(this.successMsg == '<?= \Yii::t('plugins/teller', '交班成功');?>') {
                    this.$navigate({
                        r: this.url,
            ...
            submitBalanceType() {
                if(this.balanceType == 1) {
                    ...
                    if(+this.member.balance < +this.totalPrice) {
                        this.$confirm('<?= \Yii::t('plugins/teller', '余额不足是否前往充值');?>', {
                        ...
                        this.$confirm('<?= \Yii::t('plugins/teller', '该会员暂未设置余额支付密码');?>', {
                            confirmButtonText: '<?= \Yii::t('plugins/teller', '设置密码');?>',
                            cancelButtonText: '<?= \Yii::t('plugins/teller', '暂不设置');?>',
                            type: 'success'
                        }).then(() => {
                    ...
                    request({
                        ...
                        if (e.data.code === 0) {
                            ...
                            if(this.rechargeList.length == 0) {
                                this.$message.error('<?= \Yii::t('plugins/teller', '暂无充值方案');?>');
                            }
                        } else {
                            this.$message.error(e.data.msg);
                        }
            ...
            addMoneyGoods(e) {
                if(e > 0) {
                    ...
                    this.$message.error('<?= \Yii::t('plugins/teller', '请输入加钱金额');?>');
                }
            },
            // 确认交班
            getOff() {
            ...
            checkInMember(type) {
                ...
                if(this.loginMember) {
                    ...
                    if(this.setting.payment_type.length == 0) {
                        this.$message.error('<?= \Yii::t('plugins/teller', '请先在后台进行支付设置');?>');
                        return false;
                    }
                    this.step = 3;
                }else {
            ...
            submitMember(res,user_id) {
                if(!res) {
                    ...
                    this.$message.error('<?= \Yii::t('plugins/teller', '请输入完整手机号');?>');
                    return false;
                }
                ...
                request({
                    ...
                    if (e.data.code === 0) {
                        if(e.data.data.list.length == 0) {
                            ...
                            this.mobile = '';
                            this.$message.error('<?= \Yii::t('plugins/teller', '未找到相关会员');?>');
            ...
            useDiscount() {
                if(this.count.length == 0) {
                    this.$message.error('<?= \Yii::t('plugins/teller', '请先将商品添加到结算清单');?>');
                    return false;
                }
            ...
            addCount(idx) {
                if(this.hungOrder[idx].member && this.member && this.hungOrder[idx].member.user_id != this.member.user_id) {
                    ...
                    this.successMsg = '<?= \Yii::t('plugins/teller', '会员信息不一致');?>'
                }else {
                    if(this.count.length == 0) {
                        ...
                        this.$confirm('<?= \Yii::t('plugins/teller', '当前有进行中的收银');?>', '<?= \Yii::t('plugins/teller', '提示');?>', {
                            confirmButtonText: '<?= \Yii::t('plugins/teller', '确认');?>',
                        }).then(() => {
            ...
            clearhungOrder(idx) {
                ...
                this.successMsg = '<?= \Yii::t('plugins/teller', '取单成功');?>';
                setTimeout(()=>{
            ...
            addRemark(idx, mode) {
                ...
                if(mode == 'order') {
                    ...
                    this.remarkTitle = this.order[idx].seller_remark ? '<?= \Yii::t('plugins/teller', '修改备注');?>':'<?= \Yii::t('plugins/teller', '添加备注');?>';
                }else {
                    this.is_order = false;
                    this.remark = this.hungOrder[idx].remark ? this.hungOrder[idx].remark : '';
                    this.remarkTitle = this.hungOrder[idx].remark ? '<?= \Yii::t('plugins/teller', '修改备注');?>':'<?= \Yii::t('plugins/teller', '添加备注');?>';
                }
                this.orderIndex = idx;
            },
            // 删除挂单
            delHung(idx) {
                this.$confirm('<?= \Yii::t('plugins/teller', '确认删除当前订单数据');?>', {
                    confirmButtonText: '<?= \Yii::t('plugins/teller', '确认');?>',
                    cancelButtonText: '<?= \Yii::t('plugins/teller', '取消');?>',
                    type: 'warning'
                }).then(() => {
                    ...
                    this.$message({
                        type: 'info',
                        message: '<?= \Yii::t('plugins/teller', '已取消删除');?>'
                    });
                });
            },
            // 挂单
            toHung() {
                if(this.count.length == 0) {
                    this.$message.error('<?= \Yii::t('plugins/teller', '暂无可挂订单');?>');
                    return false;
                }
            ...
            getOrder() {
                if(this.hungLength == 0) {
                    this.$message.error('<?= \Yii::t('plugins/teller', '暂无可取订单');?>');
                    return false;
                }
            ...
            // 退出
            loginout() {
                this.$confirm('<?= \Yii::t('plugins/teller', '确认退出');?>', '<?= \Yii::t('plugins/teller', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('plugins/teller', '确认');?>',
                    cancelButtonText: '<?= \Yii::t('plugins/teller', '取消');?>',
                    type: 'warning'
                }).then(() => {
                    ...
                    if (valid) {
                        ...
                        request({
                            ...
                            if (e.data.code === 0) {
                                ...
                                this.successVisible = true;
                                this.failVisible = false;
                                this.successMsg = '<?= \Yii::t('plugins/teller', '修改成功');?>'
                            } else {
                                this.$message.error(e.data.msg);
                                this.$nextTick(() => {
                                    if(!this.is_pad) {
            ...
            submitMemberOrder() {
                if(!this.barcode && (this.payment_type == 'alipay_scan' || this.payment_type == 'wechat_scan')) {
                    this.$message.error('<?= \Yii::t('plugins/teller', '请使用扫码枪扫描客户付款码');?>');
                    return false;
                }
                this.payloading = this.$loading({
                    lock: true,
                    text: '<?= \Yii::t('plugins/teller', '支付中');?>',
                        ...
                        if(this.payment_type == 'alipay_scan' || this.payment_type == 'wechat_scan') {
                            ...
                            request({
                                ...
                                if (e.data.code === 0) {
                                    this.successVisible = true;
                                    this.failVisible = false;
                                    this.successMsg = '<?= \Yii::t('plugins/teller', '充值成功');?>'
                                    setTimeout(()=>{
                                        if(this.successVisible) {
                                            this.closeSuccess();
                                        }
            ...
            getCodePay(id, type) {
                ...
                request({
                    ...
                    if (e.data.code === 0) {
                        ...
                        if(e.data.msg.indexOf('<?= \Yii::t('plugins/teller', '需要用户输入支付密码');?>') > 0 || e.data.msg.indexOf('<?= \Yii::t('plugins/teller', '支付中请稍后再查询支付结果');?>') > 0 || e.data.msg.indexOf('order success pay inprocess') > 0) {
                            if(!type) {
                                this.msgbox = true;
                                msg = this.$confirm('<?= \Yii::t('plugins/teller', '收款中');?>', '<?= \Yii::t('plugins/teller', '提示');?>', {
                                    showClose: false,
                                    closeOnPressEscape: false,
                                    closeOnClickModal: false,
                                    confirmButtonText: '<?= \Yii::t('plugins/teller', '确认支付');?>',
                                    cancelButtonText: '<?= \Yii::t('plugins/teller', '取消支付');?>',
                                }).then(() => {
                            ...
                            this.payloading.close();
                            this.$confirm(e.data.msg, '<?= \Yii::t('plugins/teller', '提示');?>', {
                ...
                if(this.setPassword) {
                    if(!this.verifyPassword) {
                        ...
                        if(this.verify_pay_password != this.pay_password) {
                            this.verifyPassword = false;
                            this.$message.error('<?= \Yii::t('plugins/teller', '密码输入不一致');?>');
                            this.verify_pay_password = '';
                            this.pay_password = '';
                        }
                        request({
                            ...
                            if (e.data.code === 0) {
                                this.successVisible = true;
                                this.successMsg = '<?= \Yii::t('plugins/teller', '设置成功');?>';
            ...
            submitPay() {
                if(this.payment_type == "cash" && !this.getMoney) {
                    this.$message.error('<?= \Yii::t('plugins/teller', '请输入实付金额');?>');
                    return false;
                }
                if(this.addCredit) {
                    if(this.payment_type == "cash" && +this.getMoney < +this.payPrice) {
                        this.$message.error('<?= \Yii::t('plugins/teller', '实付金额小于应收金额');?>');
                        return false;
                    }
            ...
            nextStep() {
                if(this.chooseRecharge && this.step != 3) {
                    ...
                    if(this.rechargeId) {
                        ...
                        this.$message.error('<?= \Yii::t('plugins/teller', '请选择充值方案');?>');
                    }
                    return false
                }
                ...
                if(this.step == 2 && !this.member && this.payment_type == 'balance') {
                    if(this.setting.payment_type.length == 1) {
                        this.$message.error('<?= \Yii::t('plugins/teller', '请先登录会员');?>');
                        return false;
                    }else {
            ...
            submitGoods() {
                ...
                if(this.number > stock) {
                    this.$message.error('<?= \Yii::t('plugins/teller', '超出库存数量');?>'+this.selectAttr.stock+'<?= \Yii::t('plugins/teller', '件');?>');
                    return false
                }
                ...
                if(this.number > 0) {
                    ...
                    this.$message.error('<?= \Yii::t('plugins/teller', '请输入数量');?>');
                }
            },
            // 计算折扣并缓存
            countPrice() {
            ...
            toRefund() {
                if(+this.refundPrice < 0 || +this.refundPrice > +this.refundDetail.total_price) {
                    this.$message.error('<?= \Yii::t('plugins/teller', '输入的退款金额有误');?>');
                    return false;
                }
                if(!this.refundRemark) {
                    this.$message.error('<?= \Yii::t('plugins/teller', '请输入退款说明');?>');
                    return false;
                }
                ...
                request({
                    ...
                    if (e.data.code === 0) {
                        this.successMsg = '<?= \Yii::t('plugins/teller', '退款成功');?>';
                        this.failVisible = false;
                        this.successVisible = true;
                        setTimeout(()=>{
                            if(this.successVisible) {
