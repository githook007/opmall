        ...
        <div class="opacity" flex="cross:center main:center">
            <el-card class="box-card" shadow="always">
                ...
                <el-form :model="ruleForm" class="login-form" :rules="rules2" ref="ruleForm" label-width="0"
                         size="small">
                    <div class="form-title">{{setting.name}}<?= \Yii::t('plugins/teller', '收银系统');?></div>
                    <el-form-item prop="username">
                        <el-input @keyup.enter.native="login('ruleForm')" placeholder="<?= \Yii::t('plugins/teller', '请输入用户名');?>"
                                  v-model="ruleForm.username"></el-input>
                    </el-form-item>
                    <el-form-item prop="password">
                        <el-input @keyup.enter.native="login('ruleForm')" type="password" placeholder="<?= \Yii::t('plugins/teller', '请输入密码');?>"
                                  v-model="ruleForm.password"></el-input>
                    </el-form-item>
                    <el-form-item prop="pic_captcha">
                        <el-input @keyup.enter.native="login('ruleForm')" placeholder="<?= \Yii::t('plugins/teller', '验证码');?>"
                                  style="width: 165px"
                    ...
                    <el-form-item>
                        <el-button class="login-btn" :loading="btnLoading" round type="primary"
                                   @click="login('ruleForm')"><?= \Yii::t('plugins/teller', '登录');?>
                        </el-button>
                    </el-form-item>
                </el-form>
                <div class="foot">
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                rules2: {
                    username: [
                        {required: true, message: '<?= \Yii::t('plugins/teller', '请输入用户名');?>', trigger: 'blur'},
                    ],
                    password: [
                        {required: true, message: '<?= \Yii::t('plugins/teller', '请输入密码');?>', trigger: 'blur'},
                    ],
                    pic_captcha: [
                        {required: true, message: '<?= \Yii::t('plugins/teller', '请输入右侧图片上的文字');?>', trigger: 'blur'},
                    ],
                },
                btnLoading: false,
                setting: null,
        ...
        created() {
            this.getSetting();
            this.loadPicCaptcha();
            document.title = '<?= \Yii::t('plugins/teller', '收银系统');?>';
        },
        methods: {
            loadPicCaptcha() {
                this.$request({
            ...
            getSetting() {
                ...
                request({
                    ...
                    if(e.data.code == 0) {
                        ...
                        document.title = self.setting.name + `<?= \Yii::t('plugins/teller', 'teller_z606');?>`;
                    }else {
                        if(!getQuery('mall_id')) {
                            setTimeout(()=>{
