...
<style>
    .app-invoice-detail .app-order-count-price {
        float: right;
        margin-right: 55px;
        font-size: 12px;
        text-align: right;
    ...
    .app-invoice-detail .el-step__icon-inner {
        font-size: 30px;
    }

    .app-invoice-detail .app-order-status {
        padding: 50px 120px;
        margin-bottom: 30px;
    }

    .app-invoice-detail .app-order-status .el-step__icon.is-text {
        border: 0px;
        width: 40px;
    }

    .app-invoice-detail .app-order-count-price .el-form-item {
        margin-bottom: 5px;
    }

    .app-invoice-detail .el-collapse-item__header {
        overflow: hidden;
        white-space: nowrap;
    ...
    .app-invoice-detail .el-collapse-item:last-child {
        margin-bottom: 0;
    }

    .app-invoice-detail .el-collapse {
        border: none;
    }

    .app-invoice-detail .order-status {
        display: flex;
        flex-wrap: wrap;
    }

    .app-invoice-detail .table-body {
        padding: 20px;
        background-color: #fff;
    }

    .app-invoice-detail .order-status .el-form-item {
        width: 50%;
        min-width: 250px;
    }

    .app-invoice-detail .el-step.is-center .el-step__description {
        padding: 0 10%;
    }

    /*新的*/
    .app-invoice-detail .card-box {
    ...
    .app-invoice-detail .card-box .label {
        color: #999999;
        margin-right: 10px;
    }

    .app-invoice-detail .share-price {
        color: #EF8933;
    }

    .app-invoice-detail .share-price .orange-label {
        color: #EF8933;
    }

    .app-invoice-detail .share-title {
        font-size: 15px;
        margin: 10px 0 5px;
    }

    .app-invoice-detail .action-box {
        padding: 10px 20px;
    }

    .app-invoice-detail .item-box {
        margin-bottom: 10px;
    }

    .app-invoice-detail .store-address {
        margin-left: 65px;
        margin-top: 5px;
    }

    .app-invoice-detail .express-address {
        width: 80%;
    }

    .app-invoice-detail .goods-pic {
        width: 35px;
        height: 35px;
        margin: 0 4px;
    }
...
<template id="app-invoice-detail">
    <div class="app-invoice-detail">
        <app-edit-address
                @close="dialogClose"
                @submit="dialogSubmit"
        ...
        <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
            ...
            <div slot="header">
                <el-breadcrumb separator="/">
                    ...
                    <el-breadcrumb-item><?= \Yii::t('plugins/invoice', '订单详情');?></el-breadcrumb-item>
                </el-breadcrumb>
            </div>
            <!-- 订单进度 -->
            <div class="table-body" v-loading="loading">
                ...
                <el-card class="app-order-status" shadow="never">
                    <el-steps v-if="isShowSteps" :active="active" align-center>
                        <el-step title="<?= \Yii::t('plugins/invoice', '已下单');?>" :description="order.created_at">
                            <template slot="icon">
                                <img src="statics/img/mall/order/status/status_1_active.png">
                            </template>
                        </el-step>
                        <el-step :title="active > 1 && order.is_pay == 1 ? '<?= \Yii::t('plugins/invoice', '已付款');?>':'<?= \Yii::t('plugins/invoice', '未付款');?>'"
                            ...
                            <template slot="description">
                                ...
                                <div v-if="order.is_pay == 0 && order.pay_type != 2 && order.auto_cancel_time"
                                     style="color: #ff4544">预计 {{order.auto_cancel_time}} <?= \Yii::t('plugins/invoice', '自动取消订单');?>
                                </div>
                            </template>
                        </el-step>
                        <el-step :title="active > 2 ? '<?= \Yii::t('plugins/invoice', '已发货');?>':'<?= \Yii::t('plugins/invoice', '未发货');?>'" v-show="order.cancel_status != 1"
                        ...
                        <el-step :title="active > 3 ? '<?= \Yii::t('plugins/invoice', '已收货');?>':'<?= \Yii::t('plugins/invoice', '未收货');?>'"
                            ...
                            <template slot="description">
                                ...
                                <div v-if="order.is_send == 1 && order.is_confirm == '0' && order.auto_confirm_time"
                                     style="color: #ff4544"><?= \Yii::t('plugins/invoice', '预计');?> {{order.auto_confirm_time}} <?= \Yii::t('plugins/invoice', '自动确认收货');?>
                                </div>
                            </template>
                        </el-step>
                        <el-step :title="active > 4 ? '<?= \Yii::t('plugins/invoice', '已结束');?>':'<?= \Yii::t('plugins/invoice', '未结束');?>'"
                            ...
                            <template slot="description">
                                ...
                                <div v-if="order.is_confirm == 1 && order.is_sale == 0 && order.auto_sales_time"
                                     style="color: #ff4544"><?= \Yii::t('plugins/invoice', '预计');?> {{order.auto_sales_time}} <?= \Yii::t('plugins/invoice', '自动结束订单');?>
                                </div>
                            </template>
                        </el-step>
                        <el-step title="<?= \Yii::t('plugins/invoice', '已取消');?>"
                ...
                <el-row :gutter="12">
                    <el-col :span="8">
                        <div class="card-box" style="padding: 0;">
                            <h3 style="padding: 10px 10px 0 10px"><?= \Yii::t('plugins/invoice', '订单信息');?></h3>
                            <div class="item-box item-margin" flex="dir:left cross:center">
                                <span class="label"><?= \Yii::t('plugins/invoice', '订单号');?>:</span>
                                <div>{{ order.order_no}}</div>
                            </div>
                            <div v-if="order.paymentOrder" class="item-box item-margin" flex="dir:left cross:center">
                                <span class="label"><?= \Yii::t('plugins/invoice', '商户单号');?>:</span>
                                <div>{{ order.paymentOrder.paymentOrderUnion.order_no }}</div>
                            </div>
                            <div class="item-box item-margin" flex="dir:left cross:center">
                                <span class="label"><?= \Yii::t('plugins/invoice', '支付方式');?>:</span>
                                <el-tag size="small" hit type="success" v-if="order.pay_type == 1"><?= \Yii::t('plugins/invoice', '线上支付');?></el-tag>
                                <el-tag size="small" hit type="success" v-else-if="order.pay_type == 3"><?= \Yii::t('plugins/invoice', '余额支付');?></el-tag>
                                <el-tag size="small" hit type="success" v-else-if="order.pay_type == 2"><?= \Yii::t('plugins/invoice', '货到付款');?></el-tag>
                                <el-tag size="small" hit type="success" v-else-if="order.pay_type == 4"><?= \Yii::t('plugins/invoice', '现金支付');?></el-tag>
                                <el-tag size="small" hit type="success" v-else-if="order.pay_type == 5"><?= \Yii::t('plugins/invoice', 'POS机支付');?></el-tag>
                            </div>
                            <div class="item-box item-margin" flex="dir:left cross:center" v-if="order.goods_type == 'goods' || order.goods_type == 'exchange'">
                                <span class="label"><?= \Yii::t('plugins/invoice', '配送方式');?>:</span>
                                <el-tag size="small" hit type="primary" v-if="order.send_type == 0"><?= \Yii::t('plugins/invoice', '快递发送');?></el-tag>
                                <el-tag size="small" hit type="primary" v-else-if="order.send_type == 1"><?= \Yii::t('plugins/invoice', '到店自提');?></el-tag>
                                <el-tag size="small" hit type="primary" v-else-if="order.send_type == 2"><?= \Yii::t('plugins/invoice', '同城配送');?></el-tag>
                                <el-tag size="small" hit type="primary" v-else-if="order.send_type == 3"><?= \Yii::t('plugins/invoice', '自动发货');?></el-tag>
                            </div>
                            <div class="item-box item-margin" flex="dir:left cross:center">
                                <span class="label"><?= \Yii::t('plugins/invoice', '用户');?>:</span>
                                <div>{{ order.user.nickname }}</div>
                            </div>
                            <div class="item-box item-margin" v-if="order.name" flex="dir:left cross:center">
                                <span class="label">{{order.send_type == 0 ? '<?= \Yii::t('plugins/invoice', '收货人');?>' : '<?= \Yii::t('plugins/invoice', '联系人');?>'}}:</span>
                                <div>{{ order.name }}</div>
                            </div>
                            <div class="item-box item-margin" v-if="order.mobile" flex="dir:left cross:center">
                                <span class="label"><?= \Yii::t('plugins/invoice', '电话');?>:</span>
                                <div>{{ order.mobile }}</div>
                            </div>
                            <div class="item-box item-margin" flex="dir:left cross:center" v-if="order.goods_type == 'ecard' && order.paymentOrder">
                                <span class="label"><?= \Yii::t('plugins/invoice', '卡密名称');?>:</span>
                            ...
                            <div v-if="order.sign == 'exchange' && order.exchange_code">
                                <div class="item-box item-margin" flex="dir:left cross:center">
                                    <span class="label"><?= \Yii::t('plugins/invoice', '兑换码库');?>:</span>
                                    <div>{{ order.exchange_code.library_name }}</div>
                                </div>
                                <div class="item-box item-margin" flex="dir:left cross:center">
                                    <span class="label"><?= \Yii::t('plugins/invoice', '兑换码');?>:</span>
                            ...
                            <div class="ecard-expand" @click="ecardExpand" v-if="order.type_data && order.type_data.ecard && order.type_data.ecard.length > 1">
                                {{ecard.length > 1 ? '<?= \Yii::t('plugins/invoice', '收起全部');?>' : '<?= \Yii::t('plugins/invoice', '展开全部');?>'}}
                                <image src="statics/img/mall/customize_jp.png" :class="ecard.length > 1 ? 'big' : 'small'"></image>
                            </div>
                            <div class="item-box item-margin" flex="dir:left cross:center">
                                <template v-if="order.send_type == 1">
                                    <span class="label"><?= \Yii::t('plugins/invoice', '收货地址');?>:</span>
                                    <el-tag size="small" hit type="warning"><?= \Yii::t('plugins/invoice', '到店自提');?></el-tag>
                                </template>
                            ...
                            <template>
                                ...
                                <div v-if="order.is_send == 1 && order.detailExpress.length == 0 && order.express && order.express_no"
                                    ...
                                    <span class="label">{{order.send_type == 2 ? '<?= \Yii::t('plugins/invoice', '配送员');?>' : '<?= \Yii::t('plugins/invoice', '物流信息');?>'}}:</span>
                                    ...
                                    <template v-else>
                                        ...
                                        <a :href="'https://www.baidu.com/s?wd='+ order.express + order.express_no"
                                           target="_blank" title='<?= \Yii::t('plugins/invoice', '点击搜索运单号');?>'>{{ order.express_no }}</a>
                                    </template>
                                    ...
                                    <el-button v-if="order.expressSingle" size="mini" @click="printTeplate(order.expressSingle.print_teplate)"><?= \Yii::t('plugins/invoice', '打印此面单');?></el-button>
                                </div>
                                <div v-else-if="order.is_send == 1 && order.detailExpress.length == 1" class="item-box item-margin"
                                    ...
                                    <div flex="dir:left cross:center">
                                        ...
                                        <template v-else>
                                            ...
                                            <template v-if="order.detailExpress[0].send_type == 1">
                                                ...
                                                <a :href="'https://www.baidu.com/s?wd='+ order.detailExpress[0].express + order.detailExpress[0].express_no"
                                                   target="_blank" title='<?= \Yii::t('plugins/invoice', '点击搜索运单号');?>'>{{ order.detailExpress[0].express_no }}</a>
                                            </template>
                                            <!-- 无需物流发货 -->
                                            <template v-else>
                                        ...
                                        <el-button v-if="order.detailExpress[0].expressSingle" size="mini" @click="printTeplate(order.detailExpress[0].expressSingle.print_teplate)"><?= \Yii::t('plugins/invoice', '打印此面单');?></el-button>
                                    </div>
                                    <div flex="dir:left" v-if="order.detailExpress[0].send_type == 1 && order.detailExpress[0].merchant_remark">
                                        <span class="label"><?= \Yii::t('plugins/invoice', '商家留言');?>:</span>
                                ...
                                <div v-else-if="order.detailExpress.length >= 1"
                                    ...
                                    <div>
                                        <div class="label" style="background: #fffaef;color: #e6a23c;padding: 3px 0;">
                                            {{order.send_type == 2 ? '<?= \Yii::t('plugins/invoice', '点击搜索运单号');?>' : '<?= \Yii::t('plugins/invoice', '收货信息');?>'}}:{{expressIndex + 1}}
                                        </div>
                                    </div>
                                    <div flex="dir:top">
                                        <div flex="cross:center">
                                            ...
                                            <template v-else>
                                                ...
                                                <template v-if="expressItem.send_type == 1">
                                                    ...
                                                    <a :href="'https://www.baidu.com/s?wd='+ expressItem.express + expressItem.express_no"
                                                       target="_blank" title='<?= \Yii::t('plugins/invoice', '点击搜索运单号');?>'>{{ expressItem.express_no }}</a>
                                                </template>
                                                <!-- 无需物流发货 -->
                                                <template v-else>
                                            ...
                                            <el-button v-if="expressItem.expressSingle" size="mini" @click="printTeplate(expressItem.expressSingle.print_teplate)"><?= \Yii::t('plugins/invoice', '打印此面单');?></el-button>
                                        </div>
                                        ...
                                        <div flex="dir:left" style="margin-top: 10px;">
                                            <span class="label"><?= \Yii::t('plugins/invoice', '配送商品');?>:</span>
                            ...
                            <div v-if="order.send_type == 1" class="item-box item-margin" flex="dir:top">
                                <div flex="dir:left cross:center">
                                    <span class="label"><?= \Yii::t('plugins/invoice', '自提门店');?>:</span>
                            ...
                            <div v-if="order.clerk != null" class="item-box item-margin" flex="dir:left cross:center">
                                <span class="label"><?= \Yii::t('plugins/invoice', '核销人');?>:</span>
                            ...
                            <div v-if="order.orderClerk != null && order.orderClerk.clerk_remark" class="item-box item-margin"
                                 flex="dir:left cross:center">
                                <span class="label"><?= \Yii::t('plugins/invoice', '核销备注');?>:</span>
                    ...
                    <el-col :span="8">
                        <div flex="dir:top" class="card-box">
                            <h3><?= \Yii::t('plugins/invoice', '表单信息');?></h3>
                            <!-- 排除身份证 -->
                            ...
                            <div class="order-detail-form-list">
                                <div class="order-detail-form-item"
                                    ...
                                    <h3><?= \Yii::t('plugins/invoice', '商品标题');?></h3>
                        ...
                        <div v-if="isShowShare" flex="dir:top" class="card-box">
                            ...
                            <div v-if="order.shareOrder" v-for="(shareItem, index) in order.shareOrder" :key="index">
                                ...
                                <template v-if="shareItem.third_parent">
                                    <div class="share-title">
                                        {{shareItem.is_zigou ? '<?= \Yii::t('plugins/invoice', '二级分销商');?>' : '<?= \Yii::t('plugins/invoice', '三级分销商');?>'}}
                                    </div>
                                    <div class="item-box" flex="dir:left cross:center">
                                        <span class="label"><?= \Yii::t('plugins/invoice', '姓名');?>:</span>
                ...
                <el-card shadow="never" style="margin-top: 15px;">
                    <el-table stripe border :span-method="objectSpanMethod" :data="goods_list" style="width: 100%;margin-bottom: 15px;">
                        <el-table-column v-if="order.sign == 'composition'" align="center" prop="composition_type" label="套餐类型" width="200">
                            <template slot-scope="scope">
                                {{scope.row.composition_type == 1 ? '<?= \Yii::t('plugins/invoice', '固定套餐');?>' : '<?= \Yii::t('plugins/invoice', '搭配套餐');?>'}}
                            </template>
                        </el-table-column>
                        <el-table-column prop="goods" label="<?= \Yii::t('plugins/invoice', '商品标题');?>">
                            <template slot-scope="scope">
                                <div flex="dir:left cross:center">
                        ...
                        <el-table-column align="center" label="<?= \Yii::t('plugins/invoice', '规格');?>" width="220">
                            <template slot-scope="scope">
                        ...
                        <el-table-column align="center" label="<?= \Yii::t('plugins/invoice', '售价');?>" width="120">
                            <template slot-scope="scope">
                                ￥{{scope.row.unit_price}}
                            </template>
                        </el-table-column>
                        <el-table-column align="center" prop="num" label="<?= \Yii::t('plugins/invoice', '数量');?>" width="80"></el-table-column>
                        <el-table-column align="center" label="<?= \Yii::t('plugins/invoice', '商品小计');?>" width="120">
                            <template slot-scope="scope">
                                ￥{{scope.row.total_original_price}}
                            </template>
                        </el-table-column>
                        <el-table-column align="center" label="<?= \Yii::t('plugins/invoice', '优惠后');?>" width="120">
                            <template slot-scope="scope">
                    ...
                    <el-form label-width="200px" :model="order" class="app-order-count-price">
                        <el-form-item label="<?= \Yii::t('plugins/invoice', '订单售价');?>">
                        ...
                        <el-form-item label="<?= \Yii::t('plugins/invoice', '运费');?>" v-if="order.is_show_express === 1">
                            <span>￥{{ order.express_original_price }}</span>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('plugins/invoice', '会员折扣');?>" v-if="order.member_discount_price != 0.00">
                            <span style="color:#ff4544;">-￥{{ order.member_discount_price }}</span>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('plugins/invoice', '积分抵扣');?>" v-if="order.integral_deduction_price != 0.00">
                            <span style="color:#ff4544;">-￥{{ order.integral_deduction_price }}</span>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('plugins/invoice', '优惠券抵扣');?>" v-if="order.coupon_discount_price != 0.00">
                        ...
                        <el-form-item label="<?= \Yii::t('plugins/invoice', '满减优惠');?>" v-if="order.full_reduce_price != 0.00">
                            <span style="color:#ff4544;">-￥{{ order.full_reduce_price }}</span>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('plugins/invoice', '商品加价');?>"
                        ...
                        <el-form-item label="<?= \Yii::t('plugins/invoice', '运费减免');?>" v-if="(order.express_original_price - order.express_price) > 0">
                            <span style="color:#ff4544;">-￥{{ (order.express_original_price - order.express_price).toFixed(2) }}</span>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('plugins/invoice', '运费增加');?>" v-if="(order.express_original_price - order.express_price) < 0">
                            <span style="color:#ff4544;">+￥{{ (order.express_price - order.express_original_price).toFixed(2) }}</span>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('plugins/invoice', '订单改价');?>" v-if="order.back_price != 0.00">
                            <span v-if="order.back_price > 0.00" style="color:#ff4544;">-￥{{ order.back_price }}</span>
                            <span v-if="order.back_price < 0.00" style="color:#ff4544;">+￥{{ -order.back_price }}</span>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('plugins/invoice', '实付款');?>">
                ...
                <div class="card-box" style="padding: 0;">
                    <h3 style="padding: 10px 10px 0 10px"><?= \Yii::t('plugins/invoice', '发票信息');?></h3>
                    <div class="item-box item-margin" flex="dir:left cross:center">
                        <span class="label"><?= \Yii::t('plugins/invoice', '申请时间');?>:</span>
                        <div>{{ invoice.add_time}}</div>
                    </div>
                    <div class="item-box item-margin" flex="dir:left cross:center">
                        <span class="label"><?= \Yii::t('plugins/invoice', '申请人');?>:</span>
                        <div>{{ invoice.nickname}}</div>
                    </div>
                    <div class="item-box item-margin" flex="dir:left cross:center">
                        <span class="label"><?= \Yii::t('plugins/invoice', '购方名称');?>:</span>
                        <div>{{ invoice.buyer_title}}</div>
                    </div>
                    <div class="item-box item-margin" flex="dir:left cross:center">
                        <span class="label"><?= \Yii::t('plugins/invoice', '购方纳税人识别号');?>:</span>
                        <div>{{ invoice.buyer_taxpayer_num}}</div>
                    </div>
                    <div class="item-box item-margin" flex="dir:left cross:center">
                        <span class="label"><?= \Yii::t('plugins/invoice', '购方地址');?>:</span>
                        <div>{{ invoice.buyer_address}}</div>
                    </div>
                    <div class="item-box item-margin" flex="dir:left cross:center">
                        <span class="label"><?= \Yii::t('plugins/invoice', '购方电话');?>:</span>
                        <div>{{ invoice.buyer_phone}}</div>
                    </div>
                    <div class="item-box item-margin" flex="dir:left cross:center">
                        <span class="label"><?= \Yii::t('plugins/invoice', '购方银行名称');?>:</span>
                        <div>{{ invoice.buyer_bank_name}}</div>
                    </div>
                    <div class="item-box item-margin" flex="dir:left cross:center">
                        <span class="label"><?= \Yii::t('plugins/invoice', '购方银行账号');?>:</span>
                        <div>{{ invoice.buyer_bank_account}}</div>
                    </div><div class="item-box item-margin" flex="dir:left cross:center">
                        <span class="label"><?= \Yii::t('plugins/invoice', '收票人邮箱');?>:</span>
                        <div>{{ invoice.buyer_email}}</div>
                    </div><div class="item-box item-margin" flex="dir:left cross:center">
                        <span class="label"><?= \Yii::t('plugins/invoice', '收款人姓名');?>:</span>
                        <div>{{ invoice.payee}}</div>
                    </div><div class="item-box item-margin" flex="dir:left cross:center">
                        <span class="label"><?= \Yii::t('plugins/invoice', '开具发票类型');?>:</span>
                        <div>{{ invoice.invoice_type_code}}</div>
                    </div>
                    <div class="item-box item-margin" flex="dir:left cross:center">
                        <span class="label"><?= \Yii::t('plugins/invoice', '发票介质');?>:</span>
                        <div>{{ invoice.medium}}</div>
                    </div>
                    <div class="item-box item-margin" flex="dir:left cross:center">
                        <span class="label"><?= \Yii::t('plugins/invoice', '发票金额');?>:</span>
                        <div>{{ invoice.total_pay_price}}</div>
                    </div>
                    <div class="item-box item-margin" flex="dir:left cross:center">
                        <span class="label"><?= \Yii::t('plugins/invoice', '拒绝原因');?>:</span>
                        <div>{{ invoice.refusal}}</div>
                    </div>
                    <div class="item-box item-margin" flex="dir:left cross:center">
                        <span class="label"><?= \Yii::t('plugins/invoice', '状态');?>:</span>
                        <el-tag size="small" hit type="success" v-if="invoice.status == 0"><?= \Yii::t('plugins/invoice', '审核中');?>=</el-tag>
                        <el-tag size="small" hit type="success" v-else-if="invoice.status == 1"><?= \Yii::t('plugins/invoice', '审核成功');?></el-tag>
                        <el-tag size="small" hit type="success" v-else-if="invoice.status == 2"><?= \Yii::t('plugins/invoice', '开票成功');?></el-tag>
                        <el-tag size="small" hit type="error" v-else-if="invoice.status == 3"><?= \Yii::t('plugins/invoice', '发票撤销');?></el-tag>
                        <el-tag size="small" hit type="error" v-else-if="invoice.status == 4"><?= \Yii::t('plugins/invoice', '发票红冲');?></el-tag>
                        <el-tag size="small" hit type="error" v-else-if="invoice.status == 5"><?= \Yii::t('plugins/invoice', '审核失败');?></el-tag>
                        <el-tag size="small" hit type="success" v-else-if="invoice.status == 6"><?= \Yii::t('plugins/invoice', '再次提交');?></el-tag>
                    </div>
                ...
                <div class="action-box" flex="dir:right">
                    <div style="margin-left:10px;">
                        <el-button size="small" type="primary" @click="goBack"><?= \Yii::t('plugins/invoice', '返回列表');?></el-button>
                    </div>
                    <div style="margin-left:10px;" v-if="invoice.status != 2 && invoice.status != 5">
                        <el-button size="small" type="warning" @click="refuse(invoice, refuseRemark = true)"><?= \Yii::t('plugins/invoice', '拒绝开票');?></el-button>
                    </div>
                    <div v-if="invoice.status != 2" style="margin-left:10px;">
                        <el-button size="small" type="success" @click="apply(invoice)"><?= \Yii::t('plugins/invoice', '开票');?></el-button>
                    </div>
                </div>
        </el-card>
        <div id="print" v-show="false">
            <div v-for="(item) in printData" :style="{padding: `0 ${mmConversionPx(printPar.left_right_margins) + 'px'}`, width: `${mmConversionPx(Number(printPar.left_right_margins) + Number(printPar.left_right_margins) + Number(printPar.stencil_width) + Number(printPar.border_width)+ Number(printPar.border_width)) + 'px'}`, marginBottom: '20px'}">
                <div id="invoice" :style="{width: mmConversionPx(printPar.stencil_width) + 'px',minHeight:mmConversionPx(printPar.stencil_high) + 'px', marginLeft: printPar.offset.left + 'px', marginRight: printPar.offset.right + 'px', cursor: 'pointer', border: `${mmConversionPx(printPar.border_width)}px solid #000000`, boxSizing: 'content-box', margin: 0}">
                    ...
                    >
                        <div style="width: 50%;font-size:10px;line-height:1;margin-bottom: 6px" v-if="printPar.order.date"><?= \Yii::t('plugins/invoice', '打印日期');?>{{printTime}}</div>
                        <div style="width: 50%;font-size:10px;line-height:1;" v-if="printPar.order.time"><?= \Yii::t('plugins/invoice', '订单时间');?>{{item.pay_time}}</div>
                        <div style="width: 50%;font-size:10px;line-height:1;" v-if="printPar.order.orderNumber"><?= \Yii::t('plugins/invoice', '订单号');?>{{item.order_no}}</div>
                    </div>
                    <div :style="{display: 'flex', boxSizing: 'border-box'}"
                        ...
                        >
                            <div style="font-size:10px;line-height:1.5;" v-if="printPar.personalInf.name"><?= \Yii::t('plugins/invoice', '收货人信息');?>{{item.name}}</div>
                            <div style="font-size:10px;line-height:1.5;" v-if="printPar.personalInf.nickname"><?= \Yii::t('plugins/invoice', '收货人昵称');?>{{item.nickname}}</div>
                            <div style="font-size:10px;line-height:1.5;" v-if="printPar.personalInf.phone"><?= \Yii::t('plugins/invoice', '联系方式');?>{{item.mobile}}</div>
                            <div style="font-size:10px;line-height:1.5;" v-if="printPar.personalInf.payMethod"><?= \Yii::t('plugins/invoice', '支付方式');?>{{item.pay_type == 1 ? '<?= \Yii::t('plugins/invoice', '在线支付');?>' : item.pay_type == 2 ? '<?= \Yii::t('plugins/invoice', '货到付款');?>' : item.pay_type == 3 ? '<?= \Yii::t('plugins/invoice', '余额支付');?>' : ''}}</div>
                            <div style="font-size:10px;line-height:1.5;" v-if="printPar.personalInf.shipMethod && item.send_type != 1"><?= \Yii::t('plugins/invoice', '发货方式');?>{{item.send_type == 0 ? '<?= \Yii::t('plugins/invoice', '快递配送');?>' : item.send_type == 1 ? '<?= \Yii::t('plugins/invoice', '到店自提');?>' :  item.send_type == 2 ? '<?= \Yii::t('plugins/invoice', '同城配送');?>' : ''}}</div>
                            <div style="font-size:10px;line-height:1.5;" v-if="printPar.personalInf.address && item.send_type != 1"><?= \Yii::t('plugins/invoice', '收货地址');?>{{item.address}}</div>
                            <div style="font-size:10px;line-height:1.5;" v-if="printPar.personalInf.mention_address && item.send_type == 1"><?= \Yii::t('plugins/invoice', '自提门店地址');?>{{item.store_address}}</div>
                        </div>
                        <div :style="{boxSizing: 'border-box', width: `${printPar.personalInf.name || printPar.personalInf.nickname || printPar.personalInf.phone || printPar.personalInf.address  || printPar.personalInf.payMethod || printPar.personalInf.shipMethod ? '38%' : '100%'}`,borderBottom:'1px solid #000000',padding: '10px 10px 10px 0.5%', fontSize:'10px', lineHeight:'1.2'}"
                            ...
                            <?= \Yii::t('plugins/invoice', '买家留言');?>{{item.remark}}
                        </div>
                    </div>
                    <div>
                        <div  style="box-sizing:border-box;width:100%;display: flex;border-bottom:1px solid #000000;position: relative;left: -0.5px;" v-if="printPar.goodsInf.serial || printPar.goodsInf.name || printPar.goodsInf.attr || printPar.goodsInf.number || printPar.goodsInf.univalent || printPar.goodsInf.article_number || printPar.goodsInf.unit || printPar.goodsInf.unitPrice">
                            ...
                            <div :style="tableWidth" style="display: flex;" v-if="printPar.goodsInf.name || printPar.goodsInf.attr || printPar.goodsInf.unitPrice">
                                ...
                                ><?= \Yii::t('plugins/invoice', '商品名称');?></div>
                                <div :style="
                                     {
                                     ...
                                     style="min-width: 0;flex-grow: 1;flex-shrink: 1;-webkit-box-flex: 1;box-sizing:border-box;border-left: 1px solid #000000;height: 30px;line-height: 30px;padding-left: 0.5%;font-size:10px;" v-if="printPar.goodsInf.attr"><?= \Yii::t('plugins/invoice', '规格');?></div>
                            </div>
                            ...
                            ><?= \Yii::t('plugins/invoice', '单价');?>
                            </div>
                            <div style="box-sizing:border-box;width: 8%;border-left: 1px solid #000000;height: 30px;line-height: 30px;padding-left: 0.5%;font-size:10px;" v-if="printPar.goodsInf.number"><?= \Yii::t('plugins/invoice', '数量');?></div>
                            ...
                            <div style="box-sizing:border-box;width: 18%;border-left: 1px solid #000000;height: 30px;line-height: 30px;padding-left: 0.5%;font-size:10px;"  v-if="printPar.goodsInf.article_number"><?= \Yii::t('plugins/invoice', '货号');?></div>

                            <div style="box-sizing:border-box;width: 8%;border-left: 1px solid #000000;height: 30px;line-height: 30px;padding-left: 0.5%;font-size:10px;" v-if="printPar.goodsInf.unit"><?= \Yii::t('plugins/invoice', '单位');?></div>
                        </div>
                        ...
                        <div style="box-sizing:border-box;display: flex;height: 30px;padding-left: 0.5%;border-bottom:1px solid #000000;font-size: 10px" v-if="printPar.goodsInf.amount || printPar.goodsInf.totalAmount || printPar.goodsInf.fare || printPar.goodsInf.discount || printPar.goodsInf.actually_paid">
                            <div style="width: 24%;height: 30px;line-height:30px;" v-if="printPar.goodsInf.amount"><?= \Yii::t('plugins/invoice', '订单金额');?>{{item.total_goods_original_price}}</div>
                            <div style="width: 16%;height: 30px;line-height:30px;" v-if="printPar.goodsInf.totalAmount">
                                <?= \Yii::t('plugins/invoice', '总数量');?>{{item.goods_num}}
                            </div>
                            <div style="width: 21%;height: 30px;line-height:30px;" v-if="printPar.goodsInf.fare"><?= \Yii::t('plugins/invoice', '运费');?>{{item.express_price}}</div>
                            <div style="width: 20%;height: 30px;line-height:30px;" v-if="printPar.goodsInf.discount"><?= \Yii::t('plugins/invoice', '优惠');?>{{item.send_template_discount_price}}</div>
                            <div style="width: 19%;height: 30px;line-height:30px;" v-if="printPar.goodsInf.actually_paid"><?= \Yii::t('plugins/invoice', '实付');?>{{item.total_pay_price}}</div>
                        </div>
                    </div>
                    <div :style="{boxSizing: 'border-box', display:'flex',borderBottom:'1px solid #000000'}" v-if="printPar.sellerInf.branch || printPar.sellerInf.name || printPar.sellerInf.phone || printPar.sellerInf.postcode || printPar.sellerInf.address || printPar.sellerInf.remark">
                        <div :style="{boxSizing: 'border-box',width:`${!printPar.sellerInf.remark ? '100%': '62%'}`,padding: '10px 10px 10px 0.5%', fontSize: '10px',borderRight: `${!printPar.sellerInf.remark ? 'none' : '1px solid #000000'}`}" v-if="printPar.sellerInf.branch || printPar.sellerInf.name || printPar.sellerInf.phone || printPar.sellerInf.postcode || printPar.sellerInf.address">
                            <div v-if="printPar.sellerInf.branch"><?= \Yii::t('plugins/invoice', '网点名称');?>{{address_list[0].name}}</div>
                            <div v-if="printPar.sellerInf.name"><?= \Yii::t('plugins/invoice', '联系人');?>{{address_list[0].username}}</div>
                            <div v-if="printPar.sellerInf.phone"><?= \Yii::t('plugins/invoice', '联系方式');?>{{address_list[0].mobile}}</div>
                            <div v-if="printPar.sellerInf.postcode"><?= \Yii::t('plugins/invoice', '网点邮编');?>{{address_list[0].code}}</div>
                            <div v-if="printPar.sellerInf.address"><?= \Yii::t('plugins/invoice', '网点地址');?>{{address_list[0].province}}{{address_list[0].city}}{{address_list[0].district}}{{address_list[0].address}}</div>
                        </div>
                        <div :style="{boxSizing: 'border-box',width: `${!printPar.sellerInf.branch && !printPar.sellerInf.name && !printPar.sellerInf.phone && !printPar.sellerInf.postcode && !printPar.sellerInf.address ? '100%' : '38%'}`,padding: '10px 10px 10px .5%', fontSize: '10px'}"  v-if="printPar.sellerInf.remark">
                            <?= \Yii::t('plugins/invoice', '商家备注');?>{{item.seller_remark ? item.seller_remark : item.words}}
                        </div>
                    </div>
...
<script>
    Vue.component('app-invoice-detail', {
        template: '#app-invoice-detail',
        props: {
            getDetailUrl: {
                type: String,
                default: 'plugin/invoice/mall/applyOrder/detail'
            },
            getOrderListUrl: {
                type: String,
                default: 'plugin/invoice/mall/applyOrder/index'
            },
            // 控制按钮是否显示
            // 编辑收货地址
            isShowEditAddress: {
            ...
            titleLabel: {
                type: String,
                default: '<?= \Yii::t('plugins/invoice', '订单列表');?>'
            },
            // 订单数据可从父组件传入 end
            // 订单状态
            newActive: {
        ...
        data() {
            return {
                ...
                printPar: {
                    ...
                    headline: {
                        name: '<?= \Yii::t('plugins/invoice', '发货单');?>',
                        fimaly: "<?= \Yii::t('plugins/invoice', '微软雅黑');?>",
                        font: 16,
                        align: 0,
                        line: 48,
                        space: -100,
        ...
        methods: {
            ...
            getDetail() {
                ...
                request({
                    ...
                    this.loading = false;
                    if (e.data.code === 0) {
                        this.order = e.data.data.list[0]['order'];
                        this.invoice = e.data.data.list[0];
                        // // 获取电子卡密数据
            ...
            apply(row) {
                request({
                    params: {
                        r: 'plugin/invoice/mall/applyOrder/agree'
                    },
                    data: {
                        id: row.id,
                    },
            ...
            // 确认收货
            confirm(id) {
                this.$confirm('<?= \Yii::t('plugins/invoice', '是否确认收货');?>', '<?= \Yii::t('plugins/invoice', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('plugins/invoice', '确定');?>',
                    cancelButtonText: '<?= \Yii::t('plugins/invoice', '取消');?>',
                }).then(() => {
            ...
            // 结束订单
            saleOrder(id) {
                this.$confirm('<?= \Yii::t('plugins/invoice', '是否结束该订单');?>', '<?= \Yii::t('plugins/invoice', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('plugins/invoice', '确定');?>',
                    cancelButtonText: '<?= \Yii::t('plugins/invoice', '取消');?>',
                }).then(() => {
                    this.btnLoading = true;
            ...
            // 打印小票
            print(id) {
                this.$confirm('<?= \Yii::t('plugins/invoice', '是否打印小票');?>', '<?= \Yii::t('plugins/invoice', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('plugins/invoice', '确定');?>',
                    cancelButtonText: '<?= \Yii::t('plugins/invoice', '取消');?>',
                }).then(() => {
                    this.btnLoading = true;
            ...
            storeOrderSend(order) {
                this.$alert('<?= \Yii::t('plugins/invoice', '是否将配送方式改为快递配送');?>', '<?= \Yii::t('plugins/invoice', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('plugins/invoice', '确定');?>',
            ...
            select_template(e, select_order, order) {
                request({
                    ...
                    if (!res.data.data.detail) {
                        this.$message({
                            message: '<?= \Yii::t('plugins/invoice', '请先添加发货地址');?>',
                            type: 'warning'
                        });
                        return;
                    }
                    ...
                    let date = new Date();
                    let Y = date.getFullYear() + '<?= \Yii::t('plugins/invoice', '年');?>';
                    let M = (date.getMonth()+1 < 10 ? '0'+(date.getMonth()+1) : date.getMonth()+1) + '<?= \Yii::t('plugins/invoice', '月');?>';
                    let D = date.getDate() + '<?= \Yii::t('plugins/invoice', '日');?>';
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout(() => {
                            let newWindow= window.open("<?= \Yii::t('plugins/invoice', '打印窗口');?>","_blank");//打印窗口要换成页面的url
