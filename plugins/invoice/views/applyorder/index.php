...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <span><?= \Yii::t('plugins/invoice', '开票申请');?></span>
        </div>
        <div class="table-body">
            ...
            <el-tabs v-model="activeName" @tab-click="tables">
                <el-tab-pane label="<?= \Yii::t('plugins/invoice', '申请中');?>" name="0"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('plugins/invoice', '已拒绝');?>" name="5"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('plugins/invoice', '撤销开票');?>" name="3"></el-tab-pane>
            </el-tabs>
            <el-form size="small" :inline="true" :model="search">
                <el-form-item>
                    <span><?= \Yii::t('plugins/invoice', '开具发票类型');?></span>
            ...
            <el-table v-loading="loading" border :data="list" style="width: 100%;margin-bottom: 15px;">
                ...
                <el-table-column
                        label="<?= \Yii::t('plugins/invoice', '操作');?>"
                        fixed="right"
                        width="220">
                    <template slot-scope="scope">
                        <slot name="action" :item="scope.row"></slot>
                        <template>
                            <el-button  v-if="activeName == 0" @click="apply(scope.row)" size="mini" type="primary"><?= \Yii::t('plugins/invoice', '开票');?></el-button>
                            <el-button @click="detail(scope.row, detailInfo = true)" size="mini" type="info"><?= \Yii::t('plugins/invoice', '详情');?></el-button>
                            <el-button v-if="activeName == 0" @click="refuse(scope.row, refuseRemark = true)" type="danger" size="mini"><?= \Yii::t('plugins/invoice', '拒绝');?></el-button>
                        </template>
                    </template>
                </el-table-column>
            </el-table>
...
<script>
    const app = new Vue({
        ...
        methods: {
            ...
            detail(row) {
                this.$navigate({
                    r: 'plugin/invoice/mall/applyOrder/detail',
                    order_id: row.id,
                })
            },
