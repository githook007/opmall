...
class Plugin extends \app\plugins\Plugin
{
    public function getMenus()
    {
        return [
            [
                'name' => \Yii::t('plugins/step', '基本配置'),
                'route' => 'plugin/step/mall/setting',
                'icon' => 'el-icon-star-on',
            ],
            [
                'name' => \Yii::t('plugins/step', '用户列表'),
            ...
            [
                'name' => \Yii::t('plugins/step', '流量主'),
                'route' => 'plugin/step/mall/ad',
                'icon' => 'el-icon-star-on',
            ],
            [
                'name' => \Yii::t('plugins/step', '步数挑战'),
                'route' => 'plugin/step/mall/activity',
                'icon' => 'el-icon-star-on',
            ],
            [
                'name' => \Yii::t('plugins/step', '商品列表'),
            ...
            [
                'name' => \Yii::t('plugins/step', '轮播图'),
                'route' => 'plugin/step/mall/banner',
                'icon' => 'el-icon-star-on',
            ],
            [
                'name' => \Yii::t('plugins/step', '订单管理'),
...
//            ],
    ...
    public function getPickLink()
    {
        ...
        return [
            ...
            [
                'key' => 'step',
                'name' => \Yii::t('plugins/step', '步数宝商品详情'),
                'open_type' => '',
                'icon' => $iconBaseUrl . '/icon-step-detail.png',
                'value' => '/plugins/step/goods/goods',
                'params' => [
                    [
                        'key' => 'goods_id',
                        'value' => '',
                        'desc' => \Yii::t('plugins/step', 'a315'),
                        'is_required' => true,
                        'data_type' => 'number',
                        'page_url' => 'plugin/step/mall/goods',
                        'page_url_text' => \Yii::t('plugins/step', '商品列表'),
                    ],
                ],
        ...
        if ($order['sign'] == $this->getName()) {
            ...
            if ($stepOrder) {
                foreach ($order['detail'] as $key => $value) {
                    $data['exchange_list'][] = [
                        'order_id' => $value['id'],
                        'label' => \Yii::t('plugins/step', '兑换活力币'),
                        'value' => $stepOrder->currency,
                    ];
                    $data['price_list'][] = [
                        'order_id' => $value['id'],
                        'label' => \Yii::t('plugins/step', '兑换金额'),
                        'value' => $value['total_price'],
                    ];
                }
                $data['price_name'] = \Yii::t('plugins/step', '活力币');
    ...
    {
        return [
            StepNoticeInfo::TPL_NAME => StepNoticeInfo::class,
        ];
    }
    ...
    public function getGoodsExtra($goods)
    {
        ...
        $setting = CommonStep::getSetting();
        $content = $goods->stepGoods->currency . $setting['currency_name'];
        if ($goods->price > 0) {
            $content .= '+￥' . $goods->price;
        }
        return [
