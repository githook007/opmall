...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <span><?= \Yii::t('plugins/step', '用户列表');?></span>
            <el-button type="primary" style="float: right;margin-top: -5px" @click="destroyCurrency" size="small" :loading="deLoading"><?= \Yii::t('plugins/step', '一键清空活力币');?></el-button>
        </div>
        <div class="table-body">
            <el-form @submit.native.prevent inline>
                <el-form-item>
                    <div class="input-item">
                        <el-input @keyup.enter.native="search" size="small" placeholder="<?= \Yii::t('plugins/step', '请输入搜索内容');?>" v-model="keyword">
                            <el-button slot="append" icon="el-icon-search" @click="search"></el-button>
                        </el-input>
                    </div>
                </el-form-item>
                <el-form-item v-if="delList.length != 0">
                    <el-button type="primary" @click="batchCurrency" size="small"><?= \Yii::t('plugins/step', '批量编辑');?></el-button>
                </el-form-item>
            </el-form>
            <el-table v-loading="loading" border :data="list" @selection-change="handleSelectionChange" style="width: 100%;margin-bottom: 15px">
                ...
                <el-table-column align="center" width='100' prop="user_id" label="<?= \Yii::t('plugins/step', '用户ID');?>"></el-table-column>
                <el-table-column prop="user" label="<?= \Yii::t('plugins/step', '基本信息');?>">
                ...
                <el-table-column align="center" prop="child_num" label="<?= \Yii::t('plugins/step', '邀请人数');?>">
                    <template slot-scope="scope">
                        <div style="color: #3399ff;cursor: pointer" @click="getPeople(scope.row)">{{scope.row.child_num}}</div>
                    </template>
                </el-table-column>
                <el-table-column align="center" prop="ratio" label="<?= \Yii::t('plugins/step', '步数加成');?>"></el-table-column>
                <el-table-column align="center" width='200' prop="step_currency" label="<?= \Yii::t('plugins/step', '活力币');?>">
                    <template slot-scope="scope">
                ...
                <el-table-column align="center" width='300' prop="created_at" label="<?= \Yii::t('plugins/step', '创建时间');?>">
                </el-table-column>
                <el-table-column align="center" width='150' label="<?= \Yii::t('plugins/step', '详情');?>" fixed="right">
                    <template slot-scope="scope">
                        <el-button circle size="mini" type="text" @click="toDetail(scope.row)">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/step', '兑换记录');?>" placement="top">
    ...
    <el-dialog title="<?= \Yii::t('plugins/step', '邀请名单');?>" :visible.sync="people_list">
        <el-table :data="invite_list" v-loading="invite_loading">
            <el-table-column property="user" label="<?= \Yii::t('plugins/step', '基础信息');?>">
                <template slot-scope="scope">
                    <!-- <app-image mode="" width="40px" height="40px" :src="scope.row.user.avatar" style="float: left;margin-right: 10px"></app-image> -->
                    <div style="line-height: 40px;height: 40px;">{{scope.row.user.nickname}}</div>
                </template>
            </el-table-column>
            <el-table-column property="invite_ratio" label="<?= \Yii::t('plugins/step', '邀请加成');?>" width="200"></el-table-column>
            <el-table-column property="created_at" label="<?= \Yii::t('plugins/step', '邀请时间');?>" width="300"></el-table-column>
        </el-table>
    </el-dialog>

    <!--默认发件人信息-->
    <el-dialog title="<?= \Yii::t('plugins/step', '步数币批量修改');?>" :visible.sync="batchFormVisible" width="50%" :close-on-click-modal="false">
        <el-form :model="batchForm" label-width="100px" :rules="batchFormRules" ref="batchForm">
            <el-form-item label="<?= \Yii::t('plugins/step', '步数币');?>" prop="currency">
                <el-input v-model="batchForm.currency" type="number" style="width:50%" auto-complete="off"></el-input>
            </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
            <el-button @click.native="batchFormVisible = false"><?= \Yii::t('plugins/step', '取消');?></el-button>
            <el-button type="primary" @click.native="batchSubmit" :loading="batchLoading"><?= \Yii::t('plugins/step', '提交');?></el-button>
        </div>
    </el-dialog>
</div>
...
<script>
    const app = new Vue({
        ...
        methods: {
            ...
            destroyCurrency(row) {
                this.$confirm('<?= \Yii::t('plugins/step', '是否清空用户活力币');?>', '<?= \Yii::t('plugins/step', '提示');?>', {
                    type: 'warning'
                }).then(() => {
                    this.deLoading = true;
            ...
            batchCurrency() {
                if(this.delList.length == 0){
                    this.$message.info('<?= \Yii::t('plugins/step', '请勾选');?>');
                    return;
                }
            ...
            changeExpressPrice(row) {
                ...
                if(currency < 0) {
                    this.$message.error('<?= \Yii::t('plugins/step', '数量不能为负');?>');
                    return;
                }
