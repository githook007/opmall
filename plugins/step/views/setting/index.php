    ...
    <el-card v-loading="loading" style="border:0" shadow="never" body-style="background-color: #f3f3f3;padding: 0 0;">
        <el-form :model="form" label-width="10rem" ref="form" :rules="rules">
            <el-tabs v-model="activeName">
                <el-tab-pane label="<?= \Yii::t('plugins/step', '基础配置');?>" name="first">
                    <app-setting v-model="form" :is_discount="false"></app-setting>
                </el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('plugins/step', '步数宝设置');?>" name="three">
                    <div class="form-body">
                        <el-form-item v-if="false" label="<?= \Yii::t('plugins/step', '小程序标题');?>" prop="title">
                            <el-input size="small" v-model="form.title" autocomplete="off"></el-input>
                        </el-form-item>

                        <el-form-item prop="convert_max">
                            <template slot='label'>
                                <span><?= \Yii::t('plugins/step', '每日最高兑换数');?></span>
                                <el-tooltip effect="dark" content="<?= \Yii::t('plugins/step', '请输入最高兑换数');?>" placement="top">
                            ...
                            <div flex="dir:left main:center cross:center">
                                <div style="width:100%">
                                    <el-input size="small"
                                              ...
                                              placeholder="<?= \Yii::t('plugins/step', '请输入最高兑换数');?>"
                                              v-model.number="form.convert_max"
                                              autocomplete="off">
                                        <template slot="append"><?= \Yii::t('plugins/step', '步');?></template>
                                    </el-input>
                                </div>
                                <el-checkbox style="margin-left: 5px;" @change="itemChecked"
                                             v-model="form.convert_max === 0"
                                ><?= \Yii::t('plugins/step', '无限制');?>
                                </el-checkbox>
                            </div>
                        </el-form-item>
                        <el-form-item prop="invite_ratio">
                            <template slot='label'>
                                <span><?= \Yii::t('plugins/step', '邀请用户加成比率');?></span>
                                <el-tooltip effect="dark" content="<?= \Yii::t('plugins/step', '邀请概率为无限制永久增加');?>" placement="top">
                                    <i class="el-icon-info"></i>
                                </el-tooltip>
                            </template>
                            <el-input size="small" type="number" v-model="form.invite_ratio" autocomplete="off">
                                <template slot="prepend"><?= \Yii::t('plugins/step', '千分之');?></template>
                            </el-input>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('plugins/step', '步数兑换比率');?>" prop="convert_ratio">
                            <el-input size="small" type="number" v-model="form.convert_ratio" autocomplete="off">
                                <template slot="append"><?= \Yii::t('plugins/step', '兑换1活力币');?></template>
                            </el-input>
                        </el-form-item>
                        <el-form-item prop="remind_at">
                            <template slot='label'>
                                <span><?= \Yii::t('plugins/step', '兑换提醒时间');?></span>
                                <el-tooltip effect="dark" content="<?= \Yii::t('plugins/step', '消息一天只发送一次');?>" placement="top">
                                    <i class="el-icon-info"></i>
                                </el-tooltip>
                            </template>
                            <el-time-select size="small" v-model="form.remind_at" placeholder="<?= \Yii::t('plugins/step', '选择时间');?>"
                                            :picker-options="{
                              start: '00:00',
                              step: '00:15',
                              end: '23:45'
                        ...
                        <el-form-item prop="qrcode_title">
                            <template slot='label'>
                                <span><?= \Yii::t('plugins/step', '海报文字');?></span>
                                <el-tooltip effect="dark" content="<?= \Yii::t('plugins/step', '最多显示十二个字');?>" placement="top">
                                    <i class="el-icon-info"></i>
                                </el-tooltip>
                            </template>
                            <el-input size="small" v-model="form.qrcode_title" autocomplete="off"></el-input>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('plugins/step', '步数兑换规则说明');?>" prop="rule">
                            <div style="width: 458px; min-height: 458px;">
                                <app-rich-text v-model="form.rule"></app-rich-text>
                            </div>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('plugins/step', '步数挑战规则说明');?>" prop="activity_rule">
                            <div style="width: 458px; min-height: 458px;">
                ...
                <el-tab-pane label="<?= \Yii::t('plugins/step', '自定义设置');?>" name="second">
                    <div class="form-body">
                        ...
                        <el-form-item label="<?= \Yii::t('plugins/step', '活力币别名');?>" prop="currency_name">
                            <el-input size="small" v-model="form.currency_name" autocomplete="off"></el-input>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('plugins/step', '全国纪录最多显示');?>" prop="ranking_num">
                            <el-input size="small" v-model="form.ranking_num" autocomplete="off"></el-input>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('plugins/step', '排行榜背景');?>" prop="ranking_pic">
                            <el-input size="small" v-model="form.ranking_pic" autocomplete="off">
                                <template slot="append">
                                    <app-attachment :multiple="true" :max="100" @selected="chooseTop"><?= \Yii::t('plugins/step', '选择文件');?></app-attachment>
                                </template>
                            </el-input>
                        ...
                        <el-form-item label="<?= \Yii::t('plugins/step', '步数挑战背景');?>" prop="activity_pic">
                            <el-input size="small" v-model="form.activity_pic" autocomplete="off">
                                <template slot="append">
                                    <app-attachment :multiple="true" :max="100" @selected="chooseDare"><?= \Yii::t('plugins/step', '选择文件');?></app-attachment>
                                </template>
                            </el-input>
                ...
                <el-tab-pane label="<?= \Yii::t('plugins/step', '商品海报自定义');?>" name="four" v-if="false" >
                    <app-poster
                            :rule_form="form.goods_poster"
                            :goods_component="goodsComponentGoods">
                    </app-poster>
                </el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('plugins/step', '分享海报自定义');?>" name="fives">
                    <app-poster
                            :rule_form="form.step_poster"
                            :goods_component="goodsComponentStep"
                    >
                        <template v-slot:step_poster>
                            <el-form-item label="<?= \Yii::t('plugins/step', '海报模板');?>" prop="qrcode_pic">
                                <app-attachment :multiple="true" :max="100" @selected="choosePic" style="padding-bottom:10px">
                                    <el-button size="small"><?= \Yii::t('plugins/step', '选择文件');?></el-button>
                                </app-attachment>
                ...
                <el-tab-pane label="<?= \Yii::t('plugins/step', '分享自定义');?>" name="six">
                    <div class="form-body">
                        <el-form-item prop="share_title">
                            <label slot="label">
                                <span><?= \Yii::t('plugins/step', '自定义分享标题');?></span>
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/step', '分享给好友时');?>"
                                            placement="top">
                                    <i class="el-icon-info"></i>
                                </el-tooltip>
                            </label>
                            <el-input size="small" placeholder="<?= \Yii::t('plugins/step', '请输入分享标题');?>"
                                      v-model="form.share_title"></el-input>
                            <el-button @click="app_share.dialog = true;app_share.type = 'name_bg'"
                                       type="text"><?= \Yii::t('plugins/step', '查看图例');?>
                            </el-button>
                        </el-form-item>
                        <el-form-item prop="share_pic">
                            <label slot="label">
                                <span><?= \Yii::t('plugins/step', '自定义分享图片');?></span>
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/step', '作为微信分享卡片主图');?>"
                                            placement="top">
                                    <i class="el-icon-info"></i>
                                </el-tooltip>
                            </label>
                            <app-attachment v-model="form.share_pic" :multiple="false" :max="1">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/step', '建议尺寸');?>:420 * 336"
                                            placement="top">
                ...
                <el-button class="button-item" type="primary" size="small" :loading=submitLoading @click="submit"><?= \Yii::t('plugins/step', '保存');?></el-button>
        </el-form>
    </el-card>
    <el-dialog :title="app_share.type == 'name_bg' ? '<?= \Yii::t('plugins/step', '查看自定义分享标题图例');?>':'<?= \Yii::t('plugins/step', '查看自定义分享图片图例');?>'" :visible.sync="app_share.dialog" width="30%">
        ...
        <span slot="footer" class="dialog-footer">
            <el-button size="small" type="primary" @click="app_share.dialog = false"><?= \Yii::t('plugins/step', '我知道了');?></el-button>
        </span>
    </el-dialog>
</div>
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                rules: {
                    title: [
                        {min: 0, max: 30, message: "<?= \Yii::t('plugins/step', '标题长度在0');?>"},
                    ],
                    convert_max: [
                        {required: true, pattern: /^[0-9]\d{0,8}$/, message: '<?= \Yii::t('plugins/step', '每日最高兑换数不能为空');?>'},
                    ],
                    convert_ratio: [
                        {required: true, pattern: /^[0-9]\d{0,8}$/, message: '<?= \Yii::t('plugins/step', '步数兑换比率不能为空');?>'},
                    ],
                    remind_at: [
                        {required: true, message: '<?= \Yii::t('plugins/step', '提醒时间不能为空');?>'},
                    ],
                    rule: [
                        {required: true, message: '<?= \Yii::t('plugins/step', '步数兑换规则不能为空');?>'},
                    ],
                    activity_rule: [
                        {required: true, message: '<?= \Yii::t('plugins/step', '步数挑战规则不能为空');?>'},
                    ]
                },
                goodsComponentGoods: [
                    {
                ...
                goodsComponentStep: [
                    ...
                    },
                    {
                        ...
                        title: '<?= \Yii::t('plugins/step', '步数');?>',
                        is_active: true
                    },
                    {
                        key: 'pic',
                        icon_url: 'statics/img/mall/poster/icon_pic.png',
                        title: '<?= \Yii::t('plugins/step', '模板图片');?>',
                        is_active: true
                    },
                    {
                        key: 'desc',
