...
<div id="app" v-cloak>
    <app-goods-list
        ...
        <template slot="column-col">
            <el-table-column width="100" label="<?= \Yii::t('plugins/step', '所需活力币');?>">
...
<script>
    const app = new Vue({
        ...
        methods: {
            ...
            destroy(row, index) {
                let self = this;
                self.$confirm('<?= \Yii::t('plugins/step', '删除该条数据');?>', '<?= \Yii::t('plugins/step', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('plugins/step', '确定');?>',
                    cancelButtonText: '<?= \Yii::t('plugins/step', '取消');?>',
                    type: 'warning'
                }).then(() => {
                    self.listLoading = true;
                    request({
                    ...
                    self.$message.info('<?= \Yii::t('plugins/step', '已取消删除');?>')
                });
            },
            handleClick(row) {
                console.log(row);
