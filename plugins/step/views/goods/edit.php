        ...
        <div slot="header" style="justify-content:space-between;display: flex">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><span style="color: #409EFF;cursor: pointer"
                                          @click="$navigate({r:'plugin/step/mall/goods/index'})"><?= \Yii::t('plugins/step', '商品管理');?></span>
                </el-breadcrumb-item>
                <el-breadcrumb-item v-if="form.goods_id > 0"><?= \Yii::t('plugins/step', '详情');?></el-breadcrumb-item>
                <el-breadcrumb-item v-else><?= \Yii::t('plugins/step', '添加商品');?></el-breadcrumb-item>
            </el-breadcrumb>
        </div>
            ...
            <template slot="before_price">
                ...
                <el-form-item label="售价" prop="price">
                    <el-input type="number"
                              ...
                              v-model="form.price">
                        <template slot="append"><?= \Yii::t('plugins/step', '元');?></template>
                    </el-input>
                </el-form-item>

                <!-- 活力币 -->
                <el-form-item label="<?= \Yii::t('plugins/step', '活力币');?>" prop="step_currency"  >
                    <el-input type="number"
                              oninput="this.value = this.value.replace(/[^0-9\.]/, '');" :min="0"
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                rule: {
                    price: [
                        {required: true, message: '<?= \Yii::t('plugins/step', '请输入售价');?>', trigger: 'change'},
                    ],
                    step_currency: [
                        {required: true, message: '<?= \Yii::t('plugins/step', '请输入活力币');?>', trigger: 'change'},
                    ],
                },
                form: {
                    price: '0',
                    step_currency: 0,
                    extra: {
                        step_currency: '<?= \Yii::t('plugins/step', '活力币');?>'
                    }
                },
                user_attr: 0
            };
