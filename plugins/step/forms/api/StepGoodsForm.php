...
class StepGoodsForm extends Model
{
    ...
    public function getList()
    {
        ...
        foreach ($list as $goods) {
            ...
            $newList[] = [
                ...
                'count_stock' => $stock,
                'goods_stock' => $stock,
                'buy_goods_auth' => GoodsAuth::create($goods->sign)->checkBuyAuth($goods)
            ];
        }
        return [
    ...
    public function getDetail()
    {
        ...
        try {
            ...
            if (!GoodsAuth::create($form->goods->sign)->checkShowAuth($form->goods)) {
            ...
            $commonGoods->setIsLimitBuy(true);
            $detail = CommonStepGoods::getDetail($commonGoods->getAll());
            $setting = (new CommonStep())->getSetting();

            // 视频号
            $commonGoodsDetail = new CommonGoodsDetail();
            $commonGoodsDetail->user = \Yii::$app->user->identity;
            $isVideoNumber = $commonGoodsDetail->isVideoNumber();
            ...
            $newList = [
                ...
                'is_video_number' => $isVideoNumber,
                'buy_goods_auth' => GoodsAuth::create($form->goods->sign)->checkBuyAuth($form->goods),
                'min_number' => $detail['min_number'],
                'limit_buy' => $detail['limit_buy']
            ];
