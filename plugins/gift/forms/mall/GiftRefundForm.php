        ...
        try {
            ...
            if ($gift_log['open_type'] == 0) {
                $order = GiftSendOrder::find()
                    ->andWhere([
                        'gift_id' => $user_order[0]['gift_id'], 'is_pay' => 1, 'is_delete' => 0, 'is_refund' => 0
                    ])
                    ->with('detail')
                    ->asArray()->all();
                foreach ($order as $value) {
                    ...
                    if ($refund_price > 0) {
                        ...
                        if ($re) {
                            ...
                            CommonGift::sendTemplate(
                                ['order_no' => $value['order_no'], 'name' => '礼物', 'user' => $user],
                                $refund_price,
                                '礼物退款'
                            );
                        }
                ...
                foreach ($user_order as $v) {
                    ...
                    if ($refund_price > 0) {
                        ...
                        if ($re) {
                            ...
                            CommonGift::sendTemplate(
                                ['order_no' => $order['order_no'], 'name' => '礼物', 'user' => $user],
                                $refund_price,
                                '礼物退款'
                            );
                        }
