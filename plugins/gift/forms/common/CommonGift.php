...
<p>
    ...
    private static function sendMsg($type, $data)
    {
        ...
        try {
            TemplateList::getInstance()->getTemplateClass(GiftConVeryInfo::TPL_NAME)->send([
                'title' => $data['title'],
                'name' => $data['name'],
                'remark' => $data['remark'],
                'user' => $data['user'],
        ...
        foreach ($order as $value) {
            ...
            if ($refund_price > 0) {
                ...
                if ($re) {
                    ...
                    self::sendTemplate(['order_no' => $value['order_no'], 'name' => '礼物', 'user' => $user], $refund_price, '礼物到期自动退款');
                    CommonGift::sendSms($user['mobile'], 'gift_refund');
...
//        $orderDetail->form_id = 0;
        ...
        $orderDetail->form_id = 0;
        $orderDetail->erase_price = 0;
        $orderDetail->goods_type = 'goods';
        return $orderDetail->attributes;
    }


    public static function sendTemplate($orderRefund, $refund_price, $remark)
    {
        try {
            TemplateList::getInstance()->getTemplateClass(OrderRefundInfo::TPL_NAME)->send([
                'order_no' => $orderRefund['order_no'],
                'name' => $orderRefund['name'],
                'refundPrice' => price_format($refund_price),
                'remark' => $remark,
                'user' => $orderRefund['user'],
                'page' => 'pages/index/index'
            ]);
        } catch (\Exception $exception) {
            \Yii::error('模板消息发送: ' . $exception->getMessage());
