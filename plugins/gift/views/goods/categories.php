...
<div id="app">
    <el-card style="border:0" shadow="never" body-style="background-color: #f3f3f3;padding: 20px 0 0 0;"
        ...
        <div slot="header" >
            <span>
                <?= \Yii::t('plugins/gift', '分类列表');?>
            </span>
            <div style="float: right;margin-top: -5px">
                <el-button  size="small" style="display: inline-block;" type="primary" @click="$refs.mchCats.openDialog()"><?= \Yii::t('plugins/gift', '添加分类');?></el-button>
            </div>
        </div>
        <div class="table-body">
            <div style="justify-content:space-between;display: flex">
                <div class="input-item">
                    <el-input @keyup.enter.native="search" size="small" placeholder="<?= \Yii::t('plugins/gift', '请输入搜索内容');?>" v-model="keyword" clearable @clear="search">
            ...
            <el-table v-loading="loading" border :data="list" style="width: 100%;margin-bottom: 15px">
                ...
                <el-table-column prop="name" label="<?= \Yii::t('plugins/gift', '分类名称');?>"></el-table-column>
                <el-table-column label="<?= \Yii::t('plugins/gift', '图标');?>">
                    <template slot-scope="scope">
                        <app-image mode="aspectFill" :src="scope.row.cats.pic_url"></app-image>
                    </template>
                </el-table-column>
                <el-table-column label="<?= \Yii::t('plugins/gift', '排序');?>">
                    <template slot-scope="scope">
                        <div v-if="id != scope.row.id">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/gift', '排序');?>" placement="top">
                ...
                <el-table-column prop="name" label="<?= \Yii::t('plugins/gift', '添加时间');?>"></el-table-column>
                <el-table-column label="<?= \Yii::t('plugins/gift', '操作');?>" width="200" fixed="right">
                    <template slot-scope="scope">
                        <el-button size="small" type="text" @click="destroy(scope.row.id)" circle>
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/gift', '删除');?>" placement="top">
