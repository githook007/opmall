        ...
        <div slot="header" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item @click.native="returnBack">
                    <a><?= \Yii::t('plugins/gift', '商品管理');?></a></el-breadcrumb-item>
                <el-breadcrumb-item>{{edit ? '<?= \Yii::t('plugins/gift', '修改商品');?>' : '<?= \Yii::t('plugins/gift', '添加商品');?>'}}</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <app-goods
                sign="gift"
            ...
            <template slot="before_price">
                <!-- 售价 -->
                <el-form-item label="<?= \Yii::t('plugins/gift', '售价');?>" prop="price" >
                    <el-input type="number"
                              oninput="this.value = this.value.replace(/[^0-9\.]/, '');" :min="0"
                              v-model="form.price">
                        <template slot="append"><?= \Yii::t('plugins/gift', '元');?></template>
                    </el-input>
                </el-form-item>
            </template>

            <template slot="member_route_setting">
                 <span class="red"><?= \Yii::t('plugins/gift', '必须在');?>
                    <el-button type="text" @click="$navigate({r: 'plugin/gift/mall/setting/index'}, true)"><?= \Yii::t('plugins/gift', '优惠叠加设置');?></el-button>
                    <?= \Yii::t('plugins/gift', '中开启');?>
                </span>
            </template>
        </app-goods>
    </el-card>
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                rule: {
                    price: [
                        {required: true, message: '<?= \Yii::t('plugins/gift', '请输入售价');?>', trigger: 'change'},
                    ],
                },
                form: {
                    price: ''
