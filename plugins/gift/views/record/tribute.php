...
<div id="app">
    <el-card style="border:0" shadow="never" body-style="background-color: #f3f3f3;padding: 0 0;"
        ...
        <div slot="header">
            <?= \Yii::t('plugins/gift', '礼物记录');?>
        </div>
        <div class="card-body">
            <el-form size="small" :model="form" ref="form" label-width="100px" :inline="true">
                <div class="search-bar">
                    <el-form-item label="<?= \Yii::t('plugins/gift', '礼物生成时间');?>">
                        <label slot="label">
                            <?= \Yii::t('plugins/gift', '礼物生成时间');?>:
                        </label>
                        <el-date-picker
                                @change="searchOrder"
                                v-model="search.time"
                                ...
                                range-separator="<?= \Yii::t('plugins/gift', '至');?>"
                                start-placeholder="<?= \Yii::t('plugins/gift', '开始日期');?>"
                                end-placeholder="<?= \Yii::t('plugins/gift', '结束日期');?>">
                        </el-date-picker>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('plugins/gift', '选择玩法');?>">
                        <label slot="label">
                            <?= \Yii::t('plugins/gift', '选择玩法');?>:
                        </label>
                        <el-select style="width: 120px;" size="small" v-model="search.type" @change='searchOrder'>
                            <el-option key="all" label="<?= \Yii::t('plugins/gift', '全部');?>" value=""></el-option>
                            <el-option key="direct_open" label="<?= \Yii::t('plugins/gift', '直接送礼');?>" value="direct_open"></el-option>
                            <el-option key="time_open" label="<?= \Yii::t('plugins/gift', '定时开奖');?>" value="time_open"></el-option>
                            <el-option key="num_open" label="<?= \Yii::t('plugins/gift', '满人开奖');?>" value="num_open"></el-option>
                        </el-select>
                    </el-form-item>

                    <el-form-item label="<?= \Yii::t('plugins/gift', '所属平台');?>">
                        <label slot="label">
                            <?= \Yii::t('plugins/gift', '所属平台');?>:
                        </label>
                        <el-select style="width: 120px;" size="small" v-model="search.platform" @change='searchOrder'>
                            <el-option key="all" label="<?= \Yii::t('plugins/gift', '全部平台');?>" value=""></el-option>
                            <el-option key="wxapp" label="<?= \Yii::t('plugins/gift', '微信');?>" value="wxapp"></el-option>
                            <el-option key="aliapp" label="<?= \Yii::t('plugins/gift', '支付宝');?>" value="aliapp"></el-option>
                            <el-option key="ttapp" label="<?= \Yii::t('plugins/gift', '抖音');?>" value="ttapp"></el-option>
                            <el-option key="bdapp" label="<?= \Yii::t('plugins/gift', '百度');?>" value="bdapp"></el-option>
                        </el-select>
                    </el-form-item>

                    <el-form-item>
                        <div class="input-item">
                            <el-input v-model="search.keyword_1" placeholder="<?= \Yii::t('plugins/gift', '请输入搜索内容');?>" clearable @clear="searchOrder"
                                      @keyup.enter.native="searchOrder">
                                <el-select style="width: 120px" slot="prepend" v-model="search.keyword">
                                    <el-option key="name" label="<?= \Yii::t('plugins/gift', '商品名称');?>" value="name"></el-option>
                                    <el-option key="order_no" label="<?= \Yii::t('plugins/gift', '订单号');?>" value="order_no"></el-option>
                                    <el-option key="nickname" label="<?= \Yii::t('plugins/gift', '用户名');?>" value="nickname"></el-option>
                                    <el-option key="user_id" label="<?= \Yii::t('plugins/gift', '用户ID');?>" value="user_id"></el-option>
                                </el-select>
                            </el-input>
                        </div>
                    </el-form-item>
                ...
                <div class="taber">
                    <el-table
                                ...
                                <div class="good-detail">
                                    <div flex="" style="padding: 10px 10px 10px 10px;"
                                        ...
                                        <div style="width: 170px;min-height: 50px">
                                            ...
                                            <p style="margin: 0;">
                                                <?= \Yii::t('plugins/gift', '数量');?>
                                                <span style="color: #ff4544">{{item.num}}</span>
                                                <?= \Yii::t('plugins/gift', '件');?>
                                            </p>
                                        </div>
                                    </div>
                        ...
                        <el-table-column
                                ...
                                label="<?= \Yii::t('plugins/gift', '订单号');?>"
                                width="228">
                        </el-table-column>
                        <el-table-column
                                prop="user"
                                width="230"
                                label="<?= \Yii::t('plugins/gift', '用户名');?>">
                            <template slot-scope="scope">
                                <div flex="">
                        ...
                        <el-table-column
                                prop="type"
                                label="<?= \Yii::t('plugins/gift', '玩法');?>">
                            <template slot-scope="scope">
                                <span>{{scope.row.type === 'direct_open'? '<?= \Yii::t('plugins/gift', '直接送礼');?>' : scope.row.type === 'time_open' ? '<?= \Yii::t('plugins/gift', '定时开奖');?>' : '<?= \Yii::t('plugins/gift', '满人开奖');?>'}}</span>
                            </template>
                        </el-table-column>
                        <el-table-column
                                prop="type"
                                label="<?= \Yii::t('plugins/gift', '领取方式');?>">
                            <template slot-scope="scope">
                                <span>{{scope.row.open_type == 1 ? '<?= \Yii::t('plugins/gift', '多人领取');?>' : '<?= \Yii::t('plugins/gift', '一人领取');?>'}}</span>
                            </template>
                        </el-table-column>
                        <el-table-column
                                prop="join_num"
                                label="<?= \Yii::t('plugins/gift', '参与人数');?>">
                        </el-table-column>
                        <el-table-column
                                prop="address_status"
                                width="110"
                                label="<?= \Yii::t('plugins/gift', '地址填写状态');?>">
                            <template slot-scope="scope">
                                <el-tag :type="scope.row.address_status === '<?= \Yii::t('plugins/gift', '未填写');?>' ? 'danger' : scope.row.address_status === '<?= \Yii::t('plugins/gift', '部分填写');?>' ? 'warning' : 'success'">
                        ...
                        <el-table-column
                                ...
                                label="<?= \Yii::t('plugins/gift', '实付金额');?>">
                        </el-table-column>
                        <el-table-column
                                prop="sendOrder[0].created_at"
                                width="200"
                                label="<?= \Yii::t('plugins/gift', '生成时间');?>">
                        </el-table-column>
                        <el-table-column
                                width="100"
                                fixed="right"
                                label="<?= \Yii::t('plugins/gift', '操作');?>">
                            <template slot-scope="scope">
                                <el-button type="text" circle @click="detail(scope.row.id)" size="mini">
                                    <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/gift', '领取详情');?>" placement="top">
