...
<div id="app" v-cloak>
    <el-card v-loading="listLoading"  shadow="never"  class="activity-list" style="border:0"  body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        ...
        <el-dialog :visible.sync="goodsVisible" width="60%" class="activity-visible" :before-close="handleClose">
            <div slot="title">
                <span class="goods-visible-title"><?= \Yii::t('plugins/pick', '选择商品');?></span>
            </div>
            <div style="margin-bottom: 27px;">
                <el-input v-model="keyword"  @change="selectGood" @clear="selectGood" placeholder="<?= \Yii::t('plugins/pick', '根据名称搜索');?>" clearable autocomplete="off">
                    <template slot="append">
                        <el-button @click="selectGood"><?= \Yii::t('plugins/pick', '搜索');?></el-button>
                    </template>
                </el-input>
            </div>
            <el-table
            ...
            <div slot="footer" class="dialog-footer" flex="main:justify">
                ...
                <el-button type="primary" size="small" @click="addGoods()"><?= \Yii::t('plugins/pick', '确定');?></el-button>
            </div>
        </el-dialog>
        <!--库存弹框-->
        <el-dialog title="<?= \Yii::t('plugins/pick', '编辑库存');?>" :visible.sync="stockVisible" :before-close="stockClose">
            <div class="set-inventory">
                <div class="input" flex="main:center cross:center">
                    <div style="margin-right: 20px;">
                        <span><?= \Yii::t('plugins/pick', '批量设置');?></span>
                    </div>
                    <div>
                        <el-input size="small" v-model="stock" @change="setStock">
                            <template slot="prepend">
                                <?= \Yii::t('plugins/pick', '库存');?>
                            </template>
                            <template slot="append">
                                <el-button @click="setStock">
                                    <?= \Yii::t('plugins/pick', '确定');?>
                                </el-button>
                            </template>
                        </el-input>
                    </div>
            ...
            <div slot="footer" class="dialog-footer">
                <el-button  @click="stockVisible = false"><?= \Yii::t('plugins/pick', '取消');?></el-button>
                <el-button type="primary" @click="sureStock"><?= \Yii::t('plugins/pick', '确定');?></el-button>
            </div>
        </el-dialog>

        <div slot="header">
            <span style="color: #3399ff;cursor: pointer;" @click="routeGo('plugin/pick/mall/activity/index')"><?= \Yii::t('plugins/pick', 'N元任选活动');?>/</span>
            <span v-if="!activity_id"><?= \Yii::t('plugins/pick', '新建活动');?></span>
            <span v-else><?= \Yii::t('plugins/pick', '编辑活动');?></span>
        </div>
        <div class="activity-body">
            <el-form label-width="116px">
                <el-tabs v-model="activeName">
                    <el-tab-pane label="<?= \Yii::t('plugins/pick', '活动信息');?>" name="second">
                        <div class="item">
                            <div class="item-title"><?= \Yii::t('plugins/pick', '活动设置');?></div>
                            <div class="item-body">
                                <el-form-item  label="<?= \Yii::t('plugins/pick', '活动名称');?>" required>
                                    <div style="width: 350px">
                                        <el-input v-model="title"></el-input>
                                    </div>
                                </el-form-item>
                                <el-form-item label="<?= \Yii::t('plugins/pick', '活动时间');?>" required>
                                    <el-date-picker
                                            size="small"
                                            type="datetimerange"
                                ...
                                <el-form-item required label="<?= \Yii::t('plugins/pick', '组合方案');?>">
                                    <div flex="" class="item-input">
                                        <el-input :min="0" style="width:148px" type="number" v-model="rule_price"></el-input>
                                        <span style="margin: 0 8px;"><?= \Yii::t('plugins/pick', '元');?></span>
                                        <el-input :min="0" style="width:148px" type="number" v-model="rule_num"></el-input>
                                        <span style="margin-left: 10px;"><?= \Yii::t('plugins/pick', '件');?></span>
                                    </div>
                                    <p class="add-goods"><?= \Yii::t('plugins/pick', '组合方案将显示在活动主页');?></p>
                                </el-form-item>
                            </div>
                        </div>
                    </el-tab-pane>
                    <el-tab-pane label="<?= \Yii::t('plugins/pick', '商品信息');?>" name="first">
                        <div class="item">
                            <div class="item-title"><?= \Yii::t('plugins/pick', '商品设置');?></div>
                            <div class="item-body">
                                <el-form-item label="<?= \Yii::t('plugins/pick', '选择商品');?>" required>
                                    <div style="margin-bottom: 15px;">
                                        <el-button @click="getGood"><?= \Yii::t('plugins/pick', '选择商品');?></el-button>
                                        <p class="add-goods" v-if="activityGoods.length === 0" @click="routeGo('mall/goods/edit')"><?= \Yii::t('plugins/pick', '商城还未添加商品');?></p>
                                    </div>
                                                ...
                                                <template slot-scope="scope">
                                                    <div flex="dir:left cross:center">
                                                        <el-button class="edit-sort" type="text" @click="editGoods(scope.row)">
                                                            <el-tooltip  effect="dark" content="<?= \Yii::t('plugins/pick', '编辑商品');?>" placement="top">
                                                                <img src="statics/img/mall/order/edit.png" alt="">
                                                            </el-tooltip>
                                                        </el-button>
                                                        <el-button @click="destroy(scope.row, scope.$index)" type="text" circle size="mini">
                                                            <el-tooltip  effect="dark" content="<?= \Yii::t('plugins/pick', '删除');?>" placement="top">
        ...
        <el-button :loading="saveLoading" type="primary" class="save"  @click="saveEdit"><?= \Yii::t('plugins/pick', '保存');?></el-button>
    </el-card>
</div>
    ...
    const app = new Vue({
        ...
        methods: {
            ...
            changeSortSubmit(row) {
                if (row.sort < 0) {
                    this.$message.warning('<?= \Yii::t('plugins/pick', '排序值不能小于0');?>');
                } else {
                    this.sort_goods_id = 0;
            ...
            destroy(row, index) {
                this.$confirm('<?= \Yii::t('plugins/pick', '将删除该活动商品');?>', '<?= \Yii::t('plugins/pick', '提示');?>', {
            ...
            setStock() {
                if (this.setStockList.length === 0) {
                    this.$message({
                        type: 'warning',
                        message: '<?= \Yii::t('plugins/pick', '请选择规格');?>'
                    });
                }
        ...
        watch: {
            activeName(data) {
                if (data === 'first') {
                    if (!this.activity_id) {
                        this.$confirm('<?= \Yii::t('plugins/pick', '请先保存活动信息');?>', '<?= \Yii::t('plugins/pick', '提示');?>', {
                            confirmButtonText: '<?= \Yii::t('plugins/pick', '确定');?>',
                            cancelButtonText: '<?= \Yii::t('plugins/pick', '取消');?>',
                            type: 'warning'
                        }).then(() => {
