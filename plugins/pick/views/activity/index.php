...
<div id="app" v-cloak>
    <el-card  shadow="never"  class="activity-list" style="border:0"  body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        ...
        <el-dialog :visible.sync="activityVisible" width="60%" class="activity-visible">
            <div slot="title">
                <span class="activity-visible-title"><?= \Yii::t('plugins/pick', '活动商品');?></span>
                <span><?= \Yii::t('plugins/pick', '共计');?>{{activityNumber}}<?= \Yii::t('plugins/pick', '款商品');?></span>
            </div>
            <div style="margin-bottom: 25px;">
                <el-input @change="searchGetGoods" @clear="searchGetGoods" v-model="activityGoods.keyword" placeholder="<?= \Yii::t('plugins/pick', '根据名称搜索');?>" autocomplete="off" clearable>
                    <template slot="append">
                        <el-button @click="searchGetGoods"><?= \Yii::t('plugins/pick', '搜索');?></el-button>
                    </template>
                </el-input>
            </div>
            <template>
            ...
            <div slot="footer" class="dialog-footer" flex="main:justify">
                ...
                <el-button type="primary" size="small" @click="saveItem()"><?= \Yii::t('plugins/pick', '确定');?></el-button>
            </div>
        </el-dialog>
        <div slot="header">
            <span><?= \Yii::t('plugins/pick', 'N元任选活动');?></span>
            <div style="float: right; margin: -5px 0">
                <el-button type="primary" size="small" @click="edit"><?= \Yii::t('plugins/pick', '新建活动');?></el-button>
            </div>
        </div>
        <div class="activity-body">
            <div class="search">
                ...
                <div class="search-time" flex="dir:left cross-center">
                    <div class="search-box" flex="dir:left">
                        <div flex="cross:center" style="height: 32px;"><?= \Yii::t('plugins/pick', '活动时间');?></div>
                        <el-date-picker
                                size="small"
                                @change="toSearch"
                                v-model="search.date"
                                ...
                                range-separator="<?= \Yii::t('plugins/pick', '至');?>"
                                start-placeholder="<?= \Yii::t('plugins/pick', '开始日期');?>"
                                end-placeholder="<?= \Yii::t('plugins/pick', '结束日期');?>">
                        </el-date-picker>
                    </div>
                    <div class="search-input search-box" flex="cross-center">
                        <div>
                            <el-input @keyup.enter.native="toSearch" size="small" placeholder="<?= \Yii::t('plugins/pick', '请输入活动名称搜索');?>"
            ...
            <div class="batch" flex="dir:left cross:center">
                <el-button @click="batchDeletion" size="mini"><?= \Yii::t('plugins/pick', '批量删除');?></el-button>
            </div>
            <el-table
                    border
                    :data="data_list"
            ...
            >
                ...
                <el-table-column
                        label="<?= \Yii::t('plugins/pick', '活动名称');?>"
                        prop="title">
                </el-table-column>
                <el-table-column
                        label="<?= \Yii::t('plugins/pick', '组合方案');?>"
                        width="300">
                    <template slot-scope="scope">
                        {{scope.row.rule_price}}<?= \Yii::t('plugins/pick', '元');?>/{{scope.row.rule_num}}<?= \Yii::t('plugins/pick', '件');?>
                    </template>
                </el-table-column>
                <el-table-column
                        label="<?= \Yii::t('plugins/pick', '商品件数');?>"
                        prop="kind"
                        width="220">
                    <template slot-scope="scope">
                ...
                <el-table-column
                        label="<?= \Yii::t('plugins/pick', '活动时间');?>"
                        width="240">
                    <template slot-scope="scope">
                        <p>{{scope.row.start_at}}<?= \Yii::t('plugins/pick', '至');?></p>
                        <p>{{scope.row.end_at}}</p>
                    </template>
                </el-table-column>
                <el-table-column
                        label="<?= \Yii::t('plugins/pick', '活动状态');?>"
                        width="150">
                    <template slot-scope="scope">
                ...
                <el-table-column
                        label="<?= \Yii::t('plugins/pick', '操作');?>"
                        fixed="right"
                        width="250">
                    <template slot-scope="scope">
                        <el-button v-if="scope.row.time_status != 3" @click="edit(scope.row, '0')" type="text" circle size="mini">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/pick', '编辑活动');?>" placement="top">
                                <img src="statics/img/mall/edit.png" alt="">
                            </el-tooltip>
                        </el-button>
                        <el-button v-if="scope.row.time_status != 3" @click="edit(scope.row, '1')" type="text" circle size="mini">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/pick', '编辑商品');?>" placement="top">
                        ...
                        <el-button v-if="scope.row.time_status != 3" @click="takeOff(scope.row)" type="text" circle size="mini">
                            <el-tooltip  class="item" effect="dark" :content="scope.row.time_status == 0 ? '<?= \Yii::t('plugins/pick', '上架');?>' : '<?= \Yii::t('plugins/pick', '下架');?>'" placement="top">
                        ...
                        <el-button @click="destroy(scope.row, scope.$index)" type="text" circle size="mini">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/pick', '删除');?>" placement="top">
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                tabs: [
                    {
                        name: '<?= \Yii::t('plugins/pick', '全部');?>',
                        value: '-1'
                    },
                    {
                        name: '<?= \Yii::t('plugins/pick', '未开始');?>',
                        value: '0'
                    },
                    {
                        name: '<?= \Yii::t('plugins/pick', '进行中');?>',
                        value: '1'
                    },
                    {
                        name: '<?= \Yii::t('plugins/pick', '已结束');?>',
                        value: '2'
                    },
                    {
                        name: '<?= \Yii::t('plugins/pick', '下架中');?>',
                        value: '3'
                    },
                ],
                activeName: '-1',
        ...
        methods: {
            ...
            batchDeletion() {
                if (this.selectionList.length === 0) {
                    this.$message({
                        type: 'warning',
                        message: '<?= \Yii::t('plugins/pick', '请选中');?>'
                    });
                    return;
                }
                this.$confirm('<?= \Yii::t('plugins/pick', '选中');?>' + this.selectionList.length +'<?= \Yii::t('plugins/pick', '个活动');?>', '<?= \Yii::t('plugins/pick', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('plugins/pick', '确定');?>',
                    cancelButtonText: '<?= \Yii::t('plugins/pick', '取消');?>',
                    type: 'warning'
                }).then(() => {
                    ...
                    this.$message({
                        type: 'info',
                        message: '<?= \Yii::t('plugins/pick', '已取消批量删除');?>'
                    });
                });
            },
            ...
            getStatusText(status) {
                ...
                switch (status) {
                    case '1':
                        data = '<?= \Yii::t('plugins/pick', '未开始');?>';
                        break;
                    case '2':
                        data = '<?= \Yii::t('plugins/pick', '进行中');?>';
                        break;
                    case '3':
                        data = '<?= \Yii::t('plugins/pick', '已结束');?>';
                        break;
                    case '0':
                        data = '<?= \Yii::t('plugins/pick', '下架中');?>';
                        break;
                }
                return data;
            },
            ...
            // 删除
            destroy(e, index) {
                this.$confirm('<?= \Yii::t('plugins/pick', '删除该条数据');?>', '<?= \Yii::t('plugins/pick', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('plugins/pick', '确定');?>',
                    cancelButtonText: '<?= \Yii::t('plugins/pick', '取消');?>',
                    type: 'warning'
                }).then(() => {
                    ...
                    this.$message({
                        type: 'info',
                        message: '<?= \Yii::t('plugins/pick', '已取消删除');?>'
                    });
                });
            },
            ...
            takeOff(e) {
                ...
                let str = `<?= \Yii::t('plugins/pick', '确认');?>${data === 'up' ? '<?= \Yii::t('plugins/pick', '上架');?>' : '<?= \Yii::t('plugins/pick', '下架');?>'}<?= \Yii::t('plugins/pick', '此活动');?>`;
                this.$confirm(str, '<?= \Yii::t('plugins/pick', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('plugins/pick', '确定');?>',
                    cancelButtonText: '<?= \Yii::t('plugins/pick', '取消');?>',
                    type: 'warning'
                }).then(() => {
                    ...
                    this.$message({
                        type: 'info',
                        message: '<?= \Yii::t('plugins/pick', '已取消操作');?>'
                    });
                });
            },
            ...
            async setStatus(ids, type) {
                ...
                if (e.data.code === 0) {
                    this.$message({
                        type: 'success',
                        message: type === 'up' ? '<?= \Yii::t('plugins/pick', '上架成功');?>' : type === 'down' ? '<?= \Yii::t('plugins/pick', '下架成功');?>' : '<?= \Yii::t('plugins/pick', '删除成功');?>'
                    });
                } else {
                    this.$message({
                        type: 'warning',
