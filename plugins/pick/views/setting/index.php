    ...
    <el-card v-loading="loading" style="border:0" shadow="never" body-style="background-color: #f3f3f3;padding: 0 0;">
        <el-form :model="form" :rules="rules" label-width="150px" ref="form">
            <el-tabs v-model="activeName">
                <el-tab-pane label="<?= \Yii::t('plugins/pick', '基本设置');?>" name="first">
                    <app-setting
                            v-model="form"
                            :is_mail="false"
                            :is_print="false"
                    ...
                    <div class="form-body " style="padding-right: 0">
                        <div class="poster-form-title" style="margin-bottom: 24px;"><?= \Yii::t('plugins/pick', '活动规则设置');?></div>
                        <div style="width: 50%">
                            <el-form-item label="<?= \Yii::t('plugins/pick', '规则标题');?>">
                                <el-input v-model="form.title"></el-input>
                            </el-form-item>
                            <el-form-item label="<?= \Yii::t('plugins/pick', '活动规则');?>">
                ...
                <el-tab-pane label="<?= \Yii::t('plugins/pick', '自定义海报');?>" name="second" v-if="false">
                    <app-poster :rule_form="form.goods_poster"
                                :goods_component="goodsComponent"
                    ></app-poster>
                </el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('plugins/pick', '自定义背景图');?>" name="three">
                    <div style="display: flex;" class="zidingyi">
                        <div class="poster-mobile-box" flex="dir:top">
                            <div class="poster-bg-box">
                                ...
                                <div class="poster-bg-title" :style="{backgroundColor: form.form.bg.color, color: form.form.text.color}"><?= \Yii::t('plugins/pick', '100元');?></div>
                            </div>
                        </div>
                        <div flex="dir:top" style="width: calc(100% - 420px);">
                            <div class="poster-form">
                                <div class="poster-form-title" style="border-bottom: 1px solid #ebeef5;">
                                    <?= \Yii::t('plugins/pick', '活动背景图设置');?>
                                </div>
                                <div class="poster-form-baby"  style="margin-bottom: 15px;position: relative;">
                                    <el-form-item label="<?= \Yii::t('plugins/pick', '头部banner图片');?>" required>
                                        <div flex="">
                                            <app-attachment
                                                    :multiple="false"
                                                    :max="1"
                                                ...
                                                <el-tooltip
                                                        class="item"
                                                        effect="dark"
                                                        content="<?= \Yii::t('plugins/pick', '建议尺寸');?>:750 * 1334"
                                                        placement="top">
                                                    <el-button size="mini">
                                                        <?= \Yii::t('plugins/pick', '选择文件');?>
                                                    </el-button>
                                                </el-tooltip>
                                            </app-attachment>
                                            <div style="margin-left: 15px;">
                                                <el-button size="mini" @click="form.bg_url = form.default_bg_url" type="primary">
                                                    <?= \Yii::t('plugins/pick', '恢复默认');?>
                                                </el-button>
                                            </div>
                                        </div>
                                    </el-form-item>
                            ...
                            <div  class="poster-form">
                                <div class="poster-form-title" style="margin-bottom: 30px;">
                                    <?= \Yii::t('plugins/pick', '组合方案标识设置');?>
                                </div>
                                <el-form-item label="<?= \Yii::t('plugins/pick', '文字颜色');?>" >
                                ...
                                <el-form-item label="<?= \Yii::t('plugins/pick', '背景颜色');?>" >
                                    <div flex="cross:center">
                ...
                <el-button class="button-item" type="primary" size="small" :loading="submitLoading" @click="submit('form')"><?= \Yii::t('plugins/pick', '保存');?></el-button>
        </el-form>
    </el-card>
</div>
    ...
    const app = new Vue({
        ...
        methods: {
            ...
            async submit() {
                ...
                if (e.data.code === 0) {
                    this.$message({
                        message: '<?= \Yii::t('plugins/pick', '保存成功');?>',
                        type: 'success'
                    });
                }
            }
