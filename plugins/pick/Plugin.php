...
class Plugin extends \app\plugins\Plugin
{
    public function getMenus()
    {
        return [
            [
                'name' => \Yii::t('plugins/pick', '设置'),
                'icon' => 'el-icon-star-on',
                'route' => 'plugin/pick/mall/setting/index',
            ],
            [
                'name' => \Yii::t('plugins/pick', 'N元任选活动'),
                'icon' => 'el-icon-star-on',
    ...
    {
        return \Yii::t('plugins/pick', 'N元任选');
    }
    ...
    public function getStatisticsMenus($bool = true)
    {
        return [
            ...
            'name' => $bool ? $this->getDisplayName() : \Yii::t('plugins/pick', '插件统计'),
            'key' => $this->getName(),
    ...
    public function getPickLink()
    {
        ...
        return [
            [
                'key' => 'pick',
                'name' => \Yii::t('plugins/pick', 'N元任选首页'),
                'open_type' => '',
...
//        $order = Order::findOne(['id' => $orderId, 'sign' => 'pick']);
        if ($order && $order['sign'] == $this->getName()) {
            $data = [
                'discount_list' => [
                    'pick_ladder_discount' => [
                        'label' => \Yii::t('plugins/pick', '活动优惠'),
