...
//            ],
            [
                'name' => \Yii::t('plugins/bdapp', '小程序发布'),
    ...
    {
        return \Yii::t('plugins/bdapp', '百度小程序');
    }
    ...
    public function templateInfoList()
    {
        return [
            'order_pay_tpl' => [
                ...
                'title' => \Yii::t('plugins/bdapp', '下单成功通知')
            ],
            'order_cancel_tpl' => [
                'id' => 'BD0021',
                'keyword_id_list' => [24, 5, 4, 17],
                'title' => \Yii::t('plugins/bdapp', '订单取消通知')
            ],
            'order_send_tpl' => [
                'id' => 'BD0003',
                'keyword_id_list' => [5, 2, 23, 55],
                'title' => \Yii::t('plugins/bdapp', '订单发货提醒')
            ],
            'order_refund_tpl' => [
                'id' => 'BD0022',
                'keyword_id_list' => [33, 13, 3, 4],
                'title' => \Yii::t('plugins/bdapp', '退款通知')
            ],
            'enroll_success_tpl' => [
                'id' => 'BD0261',
                'keyword_id_list' => [8, 9, 10],
                'title' => \Yii::t('plugins/bdapp', '信息提交成功通知')
            ],
            'enroll_error_tpl' => [
                'id' => 'BD0031',
                'keyword_id_list' => [6, 1, 7],
                'title' => \Yii::t('plugins/bdapp', '报名失败通知')
            ],
            'account_change_tpl' => [
                'id' => 'BD0643',
                'keyword_id_list' => [1, 3],
                'title' => \Yii::t('plugins/bdapp', '账户变动提醒')
            ],
            'audit_result_tpl' => [
                'id' => 'BD0141',
                'keyword_id_list' => [28, 1, 55, 5],
                'title' => \Yii::t('plugins/bdapp', '审核结果通知')
            ],
            'withdraw_success_tpl' => [
                'id' => 'BD0781',
                'keyword_id_list' => [5, 3, 6, 7],
                'title' => \Yii::t('plugins/bdapp', '提现到账通知')
            ],
            'withdraw_error_tpl' => [
                'id' => 'BD1161',
                'keyword_id_list' => [2, 3],
                'title' => \Yii::t('plugins/bdapp', '提现失败通知')
            ],
            'share_audit_tpl' => [
                'id' => 'BD0641',
                'keyword_id_list' => [1, 2, 6, 4],
                'title' => \Yii::t('plugins/bdapp', '审核状态通知')
            ],
            'remove_identity_tpl' => [
                'id' => 'BD0643',
                'keyword_id_list' => [5, 3],
    ...
    public function addTemplateList($attributes)
    {
        foreach ($attributes as $item) {
            if (!isset($item['tpl_name'])) {
                throw new \Exception(\Yii::t('plugins/bdapp', '缺少必要的参数')."tpl_name");
            }
            if (!isset($item[$item['tpl_name']])) {
                throw new \Exception(\Yii::t('plugins/bdapp', '缺少必要的参数')."{$item['tpl_name']}");
            }
    ...
    public function getPaymentForm()
    {
        ...
        if (!$config || !$config->pay_app_key || !$config->pay_private_key || !$config->pay_public_key || !$config->pay_dealid) {
            throw new \Exception(\Yii::t('plugins/bdapp', '百度小程序支付信息尚未配置'));
        }
    ...
    public function getBdConfig()
    {
        ...
        if (!$config || !$config->app_key || !$config->app_secret) {
            throw new \Exception(\Yii::t('plugins/bdapp', '小程序信息尚未配置.'));
        }
        return $config;
    }
    ...
    public function getPlatformIconUrl()
    {
        return [
            [
                ...
                'icon' => PluginHelper::getPluginBaseAssetsUrl($this->getName()) . '/img/bdapp.png'
            ]
        ];
    }
}
