...
<div id="app" v-cloak>
    <el-card v-loading="loading" shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <div>
                <span><?= \Yii::t('plugins/exchange', '兑换中心');?></span>
            </div>
        </div>
        <div class="table-body">
            <div flex="wrap:wrap cross:center" style="margin-bottom: 15px;">
                <div><?= \Yii::t('plugins/exchange', '兑换');?></div>
                <div class="input-item">
                    <el-input maxlength="12" @keyup.enter.native="toSearch" size="small" placeholder="<?= \Yii::t('plugins/exchange', '请输入兑换码');?>" v-model="keyword" clearable @clear="toSearch">
                        <el-button slot="append" icon="el-icon-search" @click="toSearch"></el-button>
                    </el-input>
                </div>
            </div>
            <el-form v-show="form.rewards.length > 0" @submit.native.prevent size="small" ref="form" :model="form" label-width="80px">
                <el-form-item label="<?= \Yii::t('plugins/exchange', '兑换用户');?>" prop="user">
                    <div flex="dir:left cross:center">
                        ...
                        <el-button size="small" @click="dialogVisible=true">{{form.user && form.user.id > 0 ? '<?= \Yii::t('plugins/exchange', '修改用户');?>': '<?= \Yii::t('plugins/exchange', '选择用户');?>'}}</el-button>
                    </div>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/exchange', '姓名');?>" prop="name">
                    <div style="width: 370px">
                        <el-input v-model="form.name"></el-input>
                    </div>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/exchange', '联系电话');?>" prop="mobile">
                    <div style="width: 370px">
                        <el-input v-model="form.mobile"></el-input>
                    </div>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/exchange', '兑换奖励');?>" prop="rewards">
        ...
        <el-button v-if="form.rewards.length > 0" style="margin-top: 20px;" :loading="btnLoading" class="button-item" type="primary" @click="submit" size="small"><?= \Yii::t('plugins/exchange', '确认兑换');?></el-button>
    </el-card>
    <el-dialog :title="form.user && form.user.id > 0 ? '<?= \Yii::t('plugins/exchange', '修改用户');?>': '<?= \Yii::t('plugins/exchange', '选择用户');?>'" :visible.sync="dialogVisible" width="980px">
        <el-form @submit.native.prevent size="small" ref="user" label-width="90px">
            <el-form-item prop="list">
                <template slot='label'>
                    <span><?= \Yii::t('plugins/exchange', '兑换用户');?></span>
                </template>
                    <el-input @keyup.enter.native="search" size="small" v-model="userKeyword" autocomplete="off" placeholder="<?= \Yii::t('plugins/exchange', '昵称/ID/手机号');?>" style="width: 20%"></el-input>
                <el-button size="small" :loading=foundLoading @click="search"><?= \Yii::t('plugins/exchange', '查找用户');?></el-button>
                ...
                <div v-if="isSearch && user.list.length == 0" flex="main:center cross:bottom" style="height: 80px;color: #999"><?= \Yii::t('plugins/exchange', '暂无用户');?></div>
            </el-form-item>
        </el-form>
        <span slot="footer" class="dialog-footer">
            <el-button size="small" @click="dialogVisible = false;id=[]"><?= \Yii::t('plugins/exchange', '取消');?></el-button>
            <el-button size="small" type="primary" @click="submitUser"><?= \Yii::t('plugins/exchange', '确定');?></el-button>
        </span>
    </el-dialog>
</div>
<script>
        ...
        methods: {
            ...
            submit() {
                if(this.form.mode == 1 && !this.token) {
                    this.$message.error('<?= \Yii::t('plugins/exchange', '请选择兑换奖励');?>');
                    return false
                }
                for(let item of this.form.rewards) {
                    if(this.form.mode == 0 && item.type != 'goods' && !this.form.user.id) {
                        this.$message.error('<?= \Yii::t('plugins/exchange', '请选择用户');?>');
                        return false
                    }
            ...
            toSearch() {
                ...
                request({
                    ...
                    if (e.data.code == 0) {
                        ...
                        that.msg = e.data.msg;
                        that.other = '';
                        if(e.data.msg == '<?= \Yii::t('plugins/exchange', '该兑换码未到使用时间');?>') {
                            that.other = e.data.data.valid_start_time + ' - ' + e.data.data.valid_end_time + ' <?= \Yii::t('plugins/exchange', '可用');?>'
                        }
                    }
                })
            },
