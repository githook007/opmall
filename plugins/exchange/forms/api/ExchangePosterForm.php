...
class ExchangePosterForm extends GrafikaOption
{
    ...
    private function get()
    {
        ...
        if (isset($option['qr_code'])) {
            if (in_array(\Yii::$app->appPlatform, [APP_PLATFORM_MOBILE, APP_PLATFORM_WECHAT])) {
                $option['qr_code']['type'] = 2;
                if (isset($option['desc'])) {
                    $option['desc']['text'] = str_replace("长按识别小程序二维码", "长按识别二维码领取", $option['desc']['text']);
                    $option['desc']['left'] = 455;
                };
