...
use app\bootstrap\express\exception\Exception;
use app\bootstrap\response\ApiCode;
use app\plugins\exchange\forms\api\GetCode;
...
class ExchangeFactory
{
        ...
        try {
            ...
            if ($this->origin === ExchangeCode::ORIGIN_ADMIN) {
                ...
                $model->hasExchangeSetting([
                    ...
                    'limit_user_num' => $model->libraryModel->limit_user_num,
                    'limit_user_success_num' => $model->libraryModel->limit_user_success_num,
                    'limit_user_type' => $model->libraryModel->limit_user_type,
                ]);
            }
            $mode = intval($model->libraryModel->mode);
            $rewards = CommonModel::getFormatRewards(CommonModel::destoryCoupon($model->libraryModel->rewards));
            /**
             * 查询购买用户id
             */
    ...
    public function unite()
    {
        try {
            ...
            $rewards = CommonModel::destoryCoupon($model->libraryModel->rewards);
            $f->imitateUser($rewards, $model->libraryModel->mode, $this->token, $has_imitate);//后台
            $model->libraryModel->mode > 0 && $f->token($rewards, $this->token);   //简单检测
            ...
            if ($codeModel) {
                //清除自动缓存
                GetCode::destroyCache(\Yii::$app->mall->id, \Yii::$app->user->id);
                return [
