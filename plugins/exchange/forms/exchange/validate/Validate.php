...
class Validate extends BasicModel
{
    ...
    //判断商品库存
    public function hasGoodsStock($rewards, $token)
    {
        if (is_string($rewards)) {
            $rewards = \yii\helpers\BaseJson::decode($rewards);
        }
        foreach ($rewards as $goods) {
            if ($goods['type'] === 'goods' && $goods['token'] === $token) {
                ...
                if ($goods_num > $attr['stock']) {
                    throw new \Exception('商品库存不足无法兑换');
                }
            }
            if ($goods['type'] === 'coupon' && $goods['token'] === $token) {
                ...
                $coupon_num = $goods['coupon_num'];
                if ($coupon_num > $coupon['total_count']) {
                    throw new \Exception('优惠券库存不足无法兑换');
                }
            }
            if ($goods['type'] === 'card' && $goods['token'] === $token) {
                $goodsCards = GoodsCards::find()->where([
                    'id' => $goods['card_id'],
                    'is_delete' => 0,
                ])->one();
                if (!$goodsCards) {
                    throw new \Exception('卡劵不存在或已删除');
                }
                if ($goodsCards['total_count'] == -1) {
                    continue;
                }
                $card_num = $goods['card_num'];
                if ($card_num > $goodsCards['total_count']) {
                    throw new \Exception('卡券库存不足无法兑换');
                }
            }
        }
    }
    ...
    public function hasExchangeSetting($setting)
    {
        ...
        if ($setting['is_limit']) {
            ...
            if ($setting['limit_user_type'] === 'all' && intval($logQuery->count()) >= $setting['limit_user_success_num']) {
                throw new ExchangeException('您的兑换次数已达上限');
            }
            $logQuery->andWhere([
                'AND',
