...
class FacadeAdmin
{
    ...
    public function library($mall_id, $library_id)
    {
        $this->validate->setMallId($mall_id);
        $this->validate->setLibraryModel($library_id);
        $this->validate->hasLibrary();//是否存在库
        $this->validate->hasDisableLibrary();
        $this->validate->hasExpireLibrary();
    }
    //用户
    ...
    public function token($rewards, $token, $type = '')
    {
        ...
        $this->validate->hasGoodsStock($rewards, $token);
    }
