...
use app\plugins\lottery\forms\common\LotteryInfo;
...
class Plugin extends \app\plugins\Plugin
{
    ...
    public function getMenus()
    {
        return [
            [
                'name' => \Yii::t('plugins/lottery', '基本配置'),
                'route' => 'plugin/lottery/mall/setting',
                'icon' => 'el-icon-star-on',
            ],
            [
                'name' => \Yii::t('plugins/lottery', '奖品列表'),
            ...
            [
                'name' => \Yii::t('plugins/lottery', '轮播图'),
                'route' => 'plugin/lottery/mall/banner',
                'icon' => 'el-icon-star-on',
            ],
            [
                'name' => \Yii::t('plugins/lottery', '赠品订单'),
...
//            ],
    ...
    {
        return \Yii::t('plugins/lottery', '幸运抽奖');
    }
    ...
    public function getAppConfig()
    {
        ...
        return [
            'app_image' => [
                ...
                'bg_pic' => $setting['bg_pic'],
            ],
            'config' => [
                'bg_color' => $setting['bg_color'],
                'bg_color_type' =>  $setting['bg_color_type'],
                'bg_gradient_color' =>  $setting['bg_gradient_color'],
            ],
        ];
    }
    ...
    public function getPickLink()
    {
        ...
        return [
            [
                'key' => 'lottery',
                'name' => \Yii::t('plugins/lottery', '幸运抽奖'),
                'open_type' => '',
            ...
            [
                'key' => 'lottery',
                'name' => \Yii::t('plugins/lottery', '抽奖商品详情'),
                'open_type' => '',
                'icon' => $iconBaseUrl . '/icon-lottery-detail.png',
                'value' => '/plugins/lottery/goods/goods',
                'params' => [
                    [
                        'key' => 'lottery_id',
                        'value' => '',
                        'desc' => \Yii::t('plugins/lottery', 'a166'),
                        'is_required' => true,
                        'data_type' => 'number',
                        'page_url' => 'plugin/lottery/mall/lottery',
                        'page_url_text' => \Yii::t('plugins/lottery', '奖品列表')
                    ]
                ],
                'ignore' => [PickLinkForm::IGNORE_TITLE, PickLinkForm::IGNORE_NAVIGATE],
            ],
...
//        $setting = CommonLottery::getSetting();
    ...
    public function getStatisticsMenus($bool = true)
    {
        return [
            ...
            'name' => $bool ? $this->getDisplayName() : \Yii::t('plugins/lottery', '插件统计'),
            'key' => $this->getName(),
    ...
    public function install()
    {
        $sql = <<<EOF
        -- v1.0.4
ALTER TABLE `op_lottery` ADD COLUMN `buy_goods_id` int(11) NOT NULL COMMENT '购买商品id' AFTER `code_num`;
EOF;
        sql_execute($sql);
    ...
    {
        return [
            LotteryInfo::TPL_NAME => LotteryInfo::class,
        ];
    }
