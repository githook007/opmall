...
class LotteryDetailForm extends Model
{
    ...
    public function detail()
    {
        ...
        try {
            ...
            if ($lottery->log) {
                ...
                $deplete_integral_num = (int)$lottery->deplete_integral_num;
                if($deplete_integral_num > 0){
                    $desc = '幸运抽奖消耗' . $deplete_integral_num . '积分;ID:'. $lottery->id;
                    \Yii::$app->currency->setUser(\Yii::$app->user->identity)->integral->sub((int)$lottery->deplete_integral_num, $desc);
                }
                if ($model->save()) {
                    ...
                    if ($parent) {
                        $parent->status = 1;
                        if (!$parent->save()) {
                            throw new \Exception($this->getErrorMsg($parent));
                        }
                        $lottery->code_num += 1;
                    };
                    if (!$lottery->save()) {
    ...
    public function goods()
    {
        ...
        try {
            ...
            $isVideoNumber = $form->isVideoNumber();
            ...
            $newGoods = [
                ...
                'cover_pic' =>  $lottery['goods']['goodsWarehouse']['cover_pic'],
                'price' => $lottery['goods']['price'],
            ];
