...
class LotteryForm extends Model
{
    ...
    public function getList()
    {
        ...
        $form = $this->is_progress ? new class extends CommonGoodsList {
            protected function setWith()
            {
                $relations = $this->relations;
                if (count($relations) > 0) {
                    if (false !== ($v = array_search('lotteryGoods', $relations))) {
                        unset($relations[$v]);
                        sort($relations);
                        $this->query->innerJoinWith(['lotteryGoods lg' => function ($query) {
                            $query->where([
                                'AND',
                                ['lg.mall_id' => \Yii::$app->mall->id],
                                ['lg.is_delete' => 0],
                                ['lg.status' => 1],
                                ['lg.type' => 0],
                                ['>=', 'lg.end_at', date('Y-m-d H:i:s')]
                            ]);
                        }]);
                    }
            ...
            protected function keywordWhere()
            {
                if ($this->keyword !== '') {
                    $goodsIds = GoodsWarehouse::find()->andWhere(['is_delete' => 0])
                        ->andWhere(['or', ['like', 'name', $this->keyword], ['like', 'keyword', $this->keyword]])
                        ->select('id');
                    $this->query->andWhere([
                        'or',
