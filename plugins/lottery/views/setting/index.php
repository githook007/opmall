    ...
    <el-card style="border:0" shadow="never" body-style="background-color: #f3f3f3;padding: 0 0;"
        ...
        <el-form :model="form" label-width="150px" :rules="FormRules" ref="form">
            <el-tabs v-model="activeName">
                <el-tab-pane label="<?= \Yii::t('plugins/lottery', '基础设置');?>" name="first">
                    <app-setting v-model="form" :is_discount="false" :is_share="false"
                                 :is_territorial_limitation="false"></app-setting>
                    <div class="form-body">
                        <el-form-item label="<?= \Yii::t('plugins/lottery', '小程序标题');?>" prop="title">
                            <el-input size="small" style="width: 30%" v-model="form.title"
                                      autocomplete="off"></el-input>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('plugins/lottery', '中奖码获取规则');?>" prop="type">
                            <el-radio-group v-model="form.type">
                                <el-radio :label="0"><?= \Yii::t('plugins/lottery', '分享点击即送');?></el-radio>
                                <el-radio :label="1"><?= \Yii::t('plugins/lottery', '参加抽奖即送');?></el-radio>
                            </el-radio-group>
                        </el-form-item>

                        <el-form-item label="<?= \Yii::t('plugins/lottery', '规则说明');?>" prop="rule">
                ...
                <el-tab-pane label="<?= \Yii::t('plugins/lottery', '自定义海报');?>" v-if="false"  class="form-body" style="padding: 0;background:none" name="second">
                    <app-poster :rule_form="form.goods_poster" :goods_component="goodsComponent"></app-poster>
                </el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('plugins/lottery', '客服设置');?>" class="form-body" name="three">
                    <el-form-item class="switch" label="<?= \Yii::t('plugins/lottery', '是否开启客服提示');?>" prop="cs_status">
                        <el-switch v-model="form.cs_status" :active-value="1" :inactive-value="0"></el-switch>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('plugins/lottery', '客服提示图片');?>" prop="cs_prompt_pic">
                        <div style="margin-bottom:10px;">
                            <app-attachment style="display:inline-block;margin-right: 10px" :multiple="false" :max="1"
                                            @selected="wechatPrompt">
                                <el-tooltip effect="dark" content="<?= \Yii::t('plugins/lottery', '建议尺寸');?>:750 * 150" placement="top">
                                    <el-button size="mini"><?= \Yii::t('plugins/lottery', '选择文件');?></el-button>
                                </el-tooltip>
                            </app-attachment>
                            <el-button type="primary" @click="wechatPromptDefault" size="mini"><?= \Yii::t('plugins/lottery', '恢复默认');?></el-button>
                        </div>
                    ...
                    <el-form-item label="<?= \Yii::t('plugins/lottery', '客服微信');?>" prop="cs_wechat">
                        <el-button size="mini" @click="addWechat"><?= \Yii::t('plugins/lottery', '选择');?></el-button>
                        <div flex="dir:left" style="flex-wrap:wrap">
                            <div v-for="(value,index) in form.cs_wechat" style="margin-right: 24px;margin-top: 12px">
                                <div class="wechat-image" flex="dir:top"
                                    ...
                                    <el-tooltip class="v" effect="dark" :content="'<?= \Yii::t('plugins/lottery', '微信号');?>'+ value.name" placement="top">
                                        <div class="wechat-end-box"><?= \Yii::t('plugins/lottery', '微信号');?>{{value.name}}</div>
                                    </el-tooltip>
                                    <el-button v-if="form.cs_prompt_pic" class="del-btn" size="mini" type="danger"
                        ...
                        <div style="color:#909399"><?= \Yii::t('plugins/lottery', '最多允许上传10张');?></div>
                    </el-form-item>

                    <el-form-item label="<?= \Yii::t('plugins/lottery', '微信群二维码');?>" prop="cs_wechat_flock_qrcode_pic">
                        <app-attachment style="margin-bottom:10px" :multiple="true" :max="50" @selected="wechatFlock">
                            <el-tooltip effect="dark" content="<?= \Yii::t('plugins/lottery', '建议尺寸');?>:360 * 360" placement="top">
                                <el-button size="mini"><?= \Yii::t('plugins/lottery', '选择文件');?></el-button>
                            </el-tooltip>
                        </app-attachment>
                ...
                <el-tab-pane label="<?= \Yii::t('plugins/lottery', '背景图设置');?>" name="bg">
                    <div style="display: flex;">
                        <div class="mobile-box">
                            <div class="head-bar" flex="main:center cross:center">
                                <div><?= \Yii::t('plugins/lottery', '抽奖');?></div>
                            </div>
                        ...
                        <div style="width: 100%">
                            <div style="background: #FFFFFF;padding: 30px 0px">
                                <el-form-item label="<?= \Yii::t('plugins/lottery', '背景图');?>" prop="bg_pic">
                                    <app-attachment style="margin-bottom:10px" :multiple="false" :max="1"
                                                    @selected="selectBgPic">
                                        <el-tooltip effect="dark"
                                                    content="<?= \Yii::t('plugins/lottery', '建议尺寸');?>:750 * 242"
                                                    placement="top">
                                            <el-button size="mini"><?= \Yii::t('plugins/lottery', '选择图标');?></el-button>
                                        </el-tooltip>
                                    </app-attachment>
                                    <div style="margin-right: 20px;display:inline-block;position: relative;cursor: move;">
                                    ...
                                    <el-button size="mini" @click="resetImg('bg_pic')"
                                               style="position: absolute;top: 7px;left: 90px" type="primary"><?= \Yii::t('plugins/lottery', '恢复默认');?>
                                    </el-button>
                                </el-form-item>
                                <el-form-item label="<?= \Yii::t('plugins/lottery', '下半部颜色配置');?>" prop="bg_color_type">
                                    <el-radio v-model="form.bg_color_type" label="pure"><?= \Yii::t('plugins/lottery', '纯色');?></el-radio>
                                    <el-radio v-model="form.bg_color_type" label="gradient"><?= \Yii::t('plugins/lottery', '渐变');?></el-radio>
                                </el-form-item>
                                <el-form-item label="<?= \Yii::t('plugins/lottery', '下半部背景颜色');?>" prop="bg_color">
                                ...
                                <el-form-item v-if="form.bg_color_type === 'gradient'" label="<?= \Yii::t('plugins/lottery', '下半部渐变颜色配置');?>"
                            ...
                            <el-button class="button-item" type="primary" :loading="btnLoading" @click="onSubmit">
                                <?= \Yii::t('plugins/lottery', '提交');?>
                            </el-button>
                            <el-button class="button-item" @click="reDefault"><?= \Yii::t('plugins/lottery', '恢复默认');?></el-button>
                        </div>
                    </div>
                </el-tab-pane>
            </el-tabs>
            <el-button class="button-item" type="primary" :loading="btnLoading" @click="onSubmit"><?= \Yii::t('plugins/lottery', '提交');?></el-button>
        </el-form>

        <!--客服微信-->
        <el-dialog title="<?= \Yii::t('plugins/lottery', '客服微信');?>" :visible.sync="wechatVisible" width="30%" :close-on-click-modal="false">
            <el-form :model="wechatForm" label-width="150px" :rules="wechatRules" ref="wechatForm"
                     @submit.native.prevent>
                <el-form-item label="<?= \Yii::t('plugins/lottery', '客服微信二维码');?>" prop="qrcode_url">
                    <div style="margin-bottom:10px;">
                        <app-attachment style="display:inline-block;margin-right: 10px" :multiple="false" :max="1"
                ...
                <el-form-item label="<?= \Yii::t('plugins/lottery', '客服微信号');?>" prop="name">
                    <el-input size="small" v-model="wechatForm.name" auto-complete="off"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button size="small" @click="wechatVisible = false"><?= \Yii::t('plugins/lottery', '取消');?></el-button>
                <el-button size="small" type="primary" @click.native="wechatSubmit"><?= \Yii::t('plugins/lottery', '提交');?></el-button>
            </div>
        </el-dialog>
    </el-card>
</section>
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                wechatRules: {
                    qrcode_url: [
                        {required: true, message: '<?= \Yii::t('plugins/lottery', '图片不能为空');?>', trigger: 'blur'},
                    ]
                },
                ...
                FormRules: {
                    type: [
                        {required: true, message: '<?= \Yii::t('plugins/lottery', '规则不能为空');?>', trigger: 'blur'},
                    ]
                },
