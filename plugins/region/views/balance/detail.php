    ...
    <el-card class="box-card" shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header" class="clearfix">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item><?= \Yii::t('plugins/region', '结算详情');?></el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="table-body">
            <div flex="dir:left cross:bottom" class="choose-area">
                ...
                <el-button size="small" @click="openChoose"><?= \Yii::t('plugins/region', '筛选地区');?></el-button>
                <div v-if="chooseArea[0].id > 0" class="clean" @click="clean"><?= \Yii::t('plugins/region', '清空筛选');?></div>
            </div>
            <el-table :data="detail" border v-loading="loading" style="margin-bottom: 15px;width: 900px">
                <el-table-column label="<?= \Yii::t('plugins/region', '结算周期');?>" width="300">
                    <template slot-scope="scope">
                        <div flex="dir:left cross:center">
                            <el-tag size="small" v-if="scope.row.bonus_type == 1"><?= \Yii::t('plugins/region', '按周');?></el-tag>
                            <el-tag size="small" v-else><?= \Yii::t('plugins/region', '按月');?></el-tag>
                            <div>{{scope.row.start_time}}~{{scope.row.end_time}}</div>
                        </div>
                    </template>
                </el-table-column>
                <el-table-column label="<?= \Yii::t('plugins/region', '订单数');?>" prop="order_num"></el-table-column>
                <el-table-column label="<?= \Yii::t('plugins/region', '分红金额');?>" prop="price" width="250">
                    <template slot-scope="scope">
                        <div>￥{{scope.row.bonus_price}}({{scope.row.bonus_rate}}%<?= \Yii::t('plugins/region', '订单分红比例');?>)</div>
                    </template>
                </el-table-column>
                <el-table-column label="<?= \Yii::t('plugins/region', '代理数');?>" prop="region_num"></el-table-column>
            </el-table>
            <div flex="dir:left cross:center" style="margin-bottom: 20px;">
                <div flex="dir:left cross:center">
                    <div class="input-item">
                        <el-input @keyup.enter.native="toSearch" size="small" placeholder="<?= \Yii::t('plugins/region', '请输入搜索内容');?>" v-model="keyword_1" clearable @clear="toSearch">
            ...
            <el-table :data="list" border v-loading="loading" size="small" style="margin-bottom: 15px;">
                <el-table-column label="<?= \Yii::t('plugins/region', '基本信息');?>" width="350">
                    <template slot-scope="scope">
                ...
                <el-table-column label="<?= \Yii::t('plugins/region', '姓名');?>" width="200" prop="name">
                ...
                <el-table-column label="<?= \Yii::t('plugins/region', '代理级别');?>" width="200" prop="level">
                </el-table-column>
                <el-table-column label="<?= \Yii::t('plugins/region', '分红区域');?>" width="400" prop="level_name">
                </el-table-column>
                <el-table-column label="<?= \Yii::t('plugins/region', '分红比例');?>" prop="price">
                    <template slot-scope="scope">
                        <div>{{scope.row.bonus_rate}}%</div>
                    </template>
                </el-table-column>
                <el-table-column label="<?= \Yii::t('plugins/region', '分红金额');?>" prop="price">
                    <template slot-scope="scope">
        ...
        <el-dialog title="<?= \Yii::t('plugins/region', '筛选地区');?>" :visible.sync="dialogVisible" width="50%">
...
<script>
    new Vue({
        ...
        data() {
            return {
                type: '1',
                level: '<?= \Yii::t('plugins/region', '全部代理');?>',
                level_name: '',
                keyword: 1,
                keyword_1: '',
                bonus_id: 0,
                ...
                other_list: {
                    id: 0,
                    name: '<?= \Yii::t('plugins/region', '全国');?>',
                    checked: true
                },
                list: [],
                area: [{id:0,name:'<?= \Yii::t('plugins/region', '全国');?>'}],
                chooseArea: [{id:0,name:'<?= \Yii::t('plugins/region', '全国');?>'}],
                dialogVisible: false,
                detail: [],
                pagination: {}
        ...
        methods: {
            clean() {
                this.all = true;
                this.area = [{id:0,name:'<?= \Yii::t('plugins/region', '全国');?>',checked: true}]
                this.chooseArea = [{id:0,name:'<?= \Yii::t('plugins/region', '全国');?>',checked: true}]
            ...
            changeLevel(e) {
                this.level_name = e != '<?= \Yii::t('plugins/region', '全部代理');?>' ? e : '';
                this.loadData();
            },
            loadData(page) {
                this.loading = true;
            ...
            districtConfirm() {
                if(this.area.length > 0) {
                    ...
                    this.$message.error('<?= \Yii::t('plugins/region', '请选择要筛选地区');?>');
                }
            },
            pageChange(page) {
                this.loading = true;
