    ...
    <app-order-detail
        ...
        <template slot="steps">
            <el-steps :active="active" align-center>
                <el-step :title="active > 0 ? '<?= \Yii::t('plugins/region', '已付款');?>':'<?= \Yii::t('plugins/region', '未付款');?>'" v-if="order.cancel_status != 1 || order.is_pay == 1"
                    ...
                    <template slot="description">
                        ...
                        <div v-if="order.is_pay == 0 && order.pay_type != 2 && order.auto_cancel_time"
                             style="color: #ff4544"><?= \Yii::t('plugins/region', '预计');?> {{order.auto_cancel_time}} <?= \Yii::t('plugins/region', '自动取消订单');?>
                        </div>
                    </template>
                </el-step>

                <el-step :title="active > 1 ? '<?= \Yii::t('plugins/region', '已完成');?>':'<?= \Yii::t('plugins/region', '未完成');?>'" v-if="order.cancel_status != 1 || order.is_sale == 1"
                    ...
                    <template slot="description">
                        ...
                        <div v-if="order.is_confirm == 1 && order.is_sale == 0 && order.auto_sales_time"
                             style="color: #ff4544"><?= \Yii::t('plugins/region', '预计');?> {{order.auto_sales_time}} <?= \Yii::t('plugins/region', '自动完成订单');?>
                        </div>
                    </template>
                </el-step>
            </el-steps>
