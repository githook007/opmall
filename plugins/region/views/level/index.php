...
<div id="app" v-cloak>
    <el-card v-loading="loading" shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <span><?= \Yii::t('plugins/region', '代理级别');?></span>
            <el-form v-if="provinces.length < 34" size="small" :inline="true" style="float: right;margin-top: -5px;">
                <el-form-item>
                    <el-button type="primary" @click="addRegion" size="small"><?= \Yii::t('plugins/region', '新增区域组');?></el-button>
                </el-form-item>
            </el-form>
        </div>
        <div class="table-body">
            <div class="tip-area"><span><?= \Yii::t('plugins/region', '温馨提示');?></span><?= \Yii::t('plugins/region', '最多可添加34个区域组');?></div>
            <el-table :data="list" border v-loading="tableLoading" style="margin: 15px 0;">
                <el-table-column label="<?= \Yii::t('plugins/region', '区域组名称');?>" prop="name"></el-table-column>
                <el-table-column label="<?= \Yii::t('plugins/region', '代理区域');?>" prop="list">
                ...
                <el-table-column label="<?= \Yii::t('plugins/region', '省代理分红比例');?>" prop="province_rate">
                    <template slot-scope="scope">
                        {{scope.row.province_rate}}%
                    </template>
                </el-table-column>
                <el-table-column label="<?= \Yii::t('plugins/region', '市代理分红比例');?>" prop="city_rate">
                    <template slot-scope="scope">
                        {{scope.row.city_rate}}%
                    </template>
                </el-table-column>
                <el-table-column label="<?= \Yii::t('plugins/region', '县代理分红比例');?>" prop="district_rate">
                    <template slot-scope="scope">
                        {{scope.row.district_rate}}%
                    </template>
                </el-table-column>
                <el-table-column label="<?= \Yii::t('plugins/region', '操作');?>" width="180" fixed="right">
                    <template slot-scope="scope">
                        ...
                        <el-button v-if="scope.row.is_default != 1" circle size="mini" type="text" @click="destroy(scope.row,scope.$index)">
                            <el-tooltip v-if="!tableLoading" class="item" effect="dark" content="<?= \Yii::t('plugins/region', '删除');?>" placement="top">
    ...
    <el-dialog :title="addList.id > 0 ? '<?= \Yii::t('plugins/region', '编辑区域组');?>': '<?= \Yii::t('plugins/region', '新增区域组');?>'" :visible.sync="toChange" width="45%" :before-close="closeRegion">
        <el-dialog width="20%" title="<?= \Yii::t('plugins/region', '选择区域');?>" :visible.sync="innerVisible" append-to-body>
            ...
            <div slot="footer" class="dialog-footer" flex="main:justify cross:center">
                <el-checkbox v-model="checkAll" @change="handleCheckAllChange">
                    <?= \Yii::t('plugins/region', '全选');?>
                </el-checkbox>
                <el-button type="primary" @click="changeProvince"><?= \Yii::t('plugins/region', '确认选择');?></el-button>
            </div>
        </el-dialog>
        <el-form :model="addList" :rules="addRules" size="small" ref="addForm" label-width="150px">
            <el-form-item label="<?= \Yii::t('plugins/region', '区域组名称');?>" prop="name">
                <el-input maxlength="6" type="text" placeholder="<?= \Yii::t('plugins/region', '请输入区域组名称');?>" style="width: 85%;" size="small" v-model="addList.name" autocomplete="off"></el-input>
            </el-form-item>
            <el-form-item label="<?= \Yii::t('plugins/region', '选择区域');?>" class="required-icon" prop="detail">
                <el-tag type="info" style="margin:5px;border:0" v-for="(value, key) in addList.detail" :key="key.id">
                    {{value.name}}
                </el-tag>
                <el-button @click="innerVisible=true"><?= \Yii::t('plugins/region', '选择');?></el-button>
            </el-form-item>
            <el-form-item style="margin-bottom: 8px" class="required-icon" label="<?= \Yii::t('plugins/region', '分红比例');?>" prop="bonus_rate">
                <el-input type="text" style="width: 85%;margin-bottom: 5px" size="small" v-model="addList.province_rate" autocomplete="off">
                    <template slot="prepend"><?= \Yii::t('plugins/region', '省代理');?></template>
                    <template slot="append">%</template>
                </el-input>
                <el-input type="text" style="width: 85%;margin-bottom: 5px" size="small" v-model="addList.city_rate" autocomplete="off">
                    <template slot="prepend"><?= \Yii::t('plugins/region', '市代理');?></template>
                    <template slot="append">%</template>
                </el-input>
                <el-input type="text" style="width: 85%;" size="small" v-model="addList.district_rate" autocomplete="off">
                    <template slot="prepend"><?= \Yii::t('plugins/region', '区');?></template>
                    <template slot="append">%</template>
                </el-input>
            </el-form-item>
            <el-form-item label="<?= \Yii::t('plugins/region', '成为代理条件');?>" prop="become_type">
                <el-radio-group @change="chooseUpType" v-model="addList.become_type">
                    <el-radio style="height: 32px;line-height: 32px;" :label="1"><?= \Yii::t('plugins/region', '下线总人数');?>
                        <el-tooltip effect="dark" content="<?= \Yii::t('plugins/region', '下线总人数');?>"
                                    placement="top">
                            <i class="el-icon-info"></i>
                        </el-tooltip>
                    </el-radio>
                    <el-radio style="height: 32px;line-height: 32px;" :label="4"><?= \Yii::t('plugins/region', '分销订单总数');?></el-radio>
                    <el-radio style="height: 32px;line-height: 32px;" :label="5"><?= \Yii::t('plugins/region', '分销订单总金额');?></el-radio>
                    <el-radio style="height: 32px;line-height: 32px;" :label="2"><?= \Yii::t('plugins/region', '累计佣金总额');?>
                        <el-tooltip effect="dark" content="<?= \Yii::t('plugins/region', '累计佣金总额');?>"
                                    placement="top">
                            <i class="el-icon-info"></i>
                        </el-tooltip>
                    </el-radio>
                    <el-radio style="height: 32px;line-height: 32px;" :label="3"><?= \Yii::t('plugins/region', '已提现佣金总额');?></el-radio>
                    <el-radio style="height: 32px;line-height: 32px;" :label="6"><?= \Yii::t('plugins/region', '消费金额');?></el-radio>
                </el-radio-group>
            </el-form-item>
            <el-form-item style="margin-bottom: 8px" class="required-icon" :label="label" prop="condition" >
                <el-input v-if="addList.become_type == 1 || addList.become_type == 4" oninput="this.value = this.value.replace(/[^0-9]/g, '')" style="width: 85%;margin-bottom: 5px" size="small" v-model="addList.province_condition" autocomplete="off">
                    <template slot="prepend"><?= \Yii::t('plugins/region', '省代理满');?></template>
                    <template slot="append" v-if="addList.become_type == 1">人</template>
                    <template slot="append" v-else-if="addList.become_type == 4"><?= \Yii::t('plugins/region', '单');?></template>
                </el-input>
                ...
                <el-input v-if="addList.become_type == 1 || addList.become_type == 4" oninput="this.value = this.value.replace(/[^0-9]/g, '')" style="width: 85%;margin-bottom: 5px" size="small" v-model="addList.city_condition" autocomplete="off">
                    <template slot="prepend"><?= \Yii::t('plugins/region', '市代理满');?></template>
                    <template slot="append" v-if="addList.become_type == 1">人</template>
                    <template slot="append" v-else-if="addList.become_type == 4"><?= \Yii::t('plugins/region', '县代理满');?></template>
                </el-input>
                ...
                <el-input v-if="addList.become_type == 1 || addList.become_type == 4" oninput="this.value = this.value.replace(/[^0-9]/g, '')" style="width: 85%;margin-bottom: 5px" size="small" v-model="addList.district_condition" autocomplete="off">
                    <template slot="prepend"><?= \Yii::t('plugins/region', '县代理满');?></template>
                    <template slot="append" v-if="addList.become_type == 1">人</template>
                    <template slot="append" v-else-if="addList.become_type == 4"><?= \Yii::t('plugins/region', 'region_z176');?></template>
                </el-input>
        ...
        <div slot="footer" class="dialog-footer">
            <el-button size="small" @click="closeRegion"><?= \Yii::t('plugins/region', '取消');?></el-button>
            <el-button size="small" type="primary" @click="changeSubmit('addForm')" :loading="contentBtnLoading"><?= \Yii::t('plugins/region', '保存');?></el-button>
        </div>
    </el-dialog>
</div>
<script>
    const app = new Vue({
        ...
        data() {
            var validateRate = (rule, value, callback) => {
                if(this.addList.province_rate != null && this.addList.city_rate != null && this.addList.district_rate != null) {
                    if (!this.addList.province_rate && this.addList.province_rate != 0) {
                        callback(new Error('<?= \Yii::t('plugins/region', '请输入省代理分红比例');?>'));
                    } else if(!this.addList.city_rate && this.addList.city_rate != 0){
                        callback(new Error('<?= \Yii::t('plugins/region', '请输入市代理分红比例');?>'));
                    } else if(!this.addList.district_rate && this.addList.district_rate != 0){
                        callback(new Error('<?= \Yii::t('plugins/region', '请输入区');?>'));
                    } else {
                        callback();
                    }
                } else {
            ...
            var validateCondition = (rule, value, callback) => {
                if(this.addList.province_condition != null && this.addList.city_condition != null && this.addList.district_condition != null) {
                    if (!this.addList.province_condition && this.addList.province_condition != 0) {
                        callback(new Error('<?= \Yii::t('plugins/region', '请输入成为省代理的条件');?>'));
                    } else if(!this.addList.city_condition && this.addList.city_condition != 0){
                        callback(new Error('<?= \Yii::t('plugins/region', '请输入成为市代理的条件');?>'));
                    } else if(!this.addList.district_condition && this.addList.district_condition != 0){
                        callback(new Error('<?= \Yii::t('plugins/region', '请输入成为区');?>'));
                    } else {
                        callback();
                    }
                } else {
            ...
            return {
                ...
                provinces: [],
                pagination: {},
                label: '<?= \Yii::t('plugins/region', '下线总人数');?>',
                addRules: {
                    name: [
                        { required: true, message: '<?= \Yii::t('plugins/region', '请输入区域组名称');?>', trigger: 'change' }
                    ],
                    bonus_rate: [
                        { validator: validateRate, trigger: 'blur'}
                    ],
                    become_type: [
                        { required: true, validator: '<?= \Yii::t('plugins/region', '请选择升级条件');?>',type: 'number', trigger: 'change' }
                    ],
                    condition: [
        ...
        methods: {
            ...
            chooseUpType(e) {
                switch(e) {
                    case 1:
                        this.label = '<?= \Yii::t('plugins/region', '下线总人数');?>'
                        ...
                        break;
                    case 4:
                        this.label = '<?= \Yii::t('plugins/region', '分销订单总数');?>'
                        this.addList.province_condition = this.addList.province_condition.toString();
                        this.addList.city_condition = this.addList.city_condition.toString();
                        this.addList.district_condition = this.addList.district_condition.toString();
                        if(this.addList.province_condition && this.addList.province_condition.indexOf('.') > -1) {
                        ...
                        break;
                    case 5:
                        this.label = '<?= \Yii::t('plugins/region', '分销订单总金额');?>'
                        break;
                    case 2:
                        this.label = '<?= \Yii::t('plugins/region', '累计佣金总额');?>'
                        break;
                    case 3:
                        this.label = '<?= \Yii::t('plugins/region', '已提现佣金总额');?>'
                        break;
                    case 6:
                        this.label = '<?= \Yii::t('plugins/region', '消费金额');?>'
                        break;
                }
            },
            closeRegion() {
            ...
            destroy(row,index) {
                let self = this;
                self.$confirm('<?= \Yii::t('plugins/region', '删除该条数据');?>', '<?= \Yii::t('plugins/region', '提示');?>', {
                    ...
                    request({
                        ...
                        if (e.data.code === 0) {
                            ...
                            if(e.data.msg.indexOf('<?= \Yii::t('plugins/region', '有代理');?>') > -1) {
                                self.$confirm(e.data.msg, '<?= \Yii::t('plugins/region', '提示');?>', {
                                    confirmButtonText: '<?= \Yii::t('plugins/region', '确定');?>',
                                    cancelButtonText: '<?= \Yii::t('plugins/region', '取消');?>',
                                    type: 'warning'
                                }).then(() => {
                    ...
                    self.$message.info('<?= \Yii::t('plugins/region', '已取消删除');?>')
                });
            },
            getList(page) {
                let that = this;
