...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <span><?= \Yii::t('plugins/region', '代理列表');?></span>
            <el-form size="small" :inline="true" style="float: right;margin-top: -5px;">
                <el-form-item>
                    <app-new-export-dialog-2
                            ...
                            action_url="plugin/region/mall/region/index">
                    </app-new-export-dialog-2>
                </el-form-item>
                <el-form-item>
                    <el-button type="primary" @click="addregion" size="small"><?= \Yii::t('plugins/region', '新增代理');?></el-button>
                </el-form-item>
            </el-form>
        </div>
        <div class="table-body">
            <div flex="dir:left cross:center" style="margin-bottom: 20px;">
                <div flex="dir:left cross:center">
                    <div style="margin-right: 10px;"><?= \Yii::t('plugins/region', '申请时间');?></div>
                    <el-date-picker
                            size="small"
                            @change="changeTime"
                            v-model="search.time"
                            ...
                            range-separator="<?= \Yii::t('plugins/region', '至');?>"
                            start-placeholder="<?= \Yii::t('plugins/region', '开始日期');?>"
                            end-placeholder="<?= \Yii::t('plugins/region', '结束日期');?>">
                    </el-date-picker>
                </div>
                <div style="margin-left: 20px;" flex="dir:left cross:center">
                    <div style="margin-right: 10px;"><?= \Yii::t('plugins/region', '代理级别');?></div>
                    <el-select style="width: 120px" size="small" v-model="level" @change="loadData" class="select">
                        <el-option key="0" label="<?= \Yii::t('plugins/region', '全部');?>" value="0"></el-option>
                        <el-option key="1" label="<?= \Yii::t('plugins/region', '区');?>" value="3"></el-option>
                        <el-option key="2" label="<?= \Yii::t('plugins/region', '市代理');?>" value="2"></el-option>
                        <el-option key="3" label="<?= \Yii::t('plugins/region', '省代理');?>" value="1"></el-option>
                    </el-select>
                </div>
                <div style="margin-left: 20px;" flex="dir:left cross:center">
                    <div style="margin-right: 10px;"><?= \Yii::t('plugins/region', '代理区域');?></div>
                ...
                <div class="input-item">
                    <el-input @keyup.enter.native="toSearch" size="small" placeholder="<?= \Yii::t('plugins/region', '请输入搜索内容');?>" v-model="search.keyword" clearable @clear="toSearch">
                        <el-select size="small" v-model="search.type" slot="prepend" class="select">
                            <el-option key="4" label="<?= \Yii::t('plugins/region', '用户ID');?>" value="4"></el-option>
                            <el-option key="1" label="<?= \Yii::t('plugins/region', '昵称');?>" value="1"></el-option>
                            <el-option key="2" label="<?= \Yii::t('plugins/region', '姓名');?>" value="2"></el-option>
                            <el-option key="3" label="<?= \Yii::t('plugins/region', '手机号');?>" value="3"></el-option>
                        </el-select>
            ...
            <el-tabs v-model="activeName" @tab-click="handleClick">
                <el-tab-pane label="<?= \Yii::t('plugins/region', '全部');?>" name="-1"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('plugins/region', '未审核');?>" name="0"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('plugins/region', '已通过');?>" name="1"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('plugins/region', '已拒绝');?>" name="2"></el-tab-pane>
                <el-table :data="list" border v-loading="loading" size="small" style="margin-bottom: 15px;">
                    ...
                    <el-table-column label="<?= \Yii::t('plugins/region', '基本信息');?>" width="300">
                    ...
                    <el-table-column label="<?= \Yii::t('plugins/region', '归属省市');?>" width="150" prop="attr">
                        <template slot-scope="scope">
                            <div>
                    ...
                    <el-table-column label="<?= \Yii::t('plugins/region', '代理级别');?>" width="150" prop="level">
                        <template slot-scope="scope">
                            <div>
                                ...
                                <el-tooltip v-if="scope.row.level > 1" class="item" effect="dark" :content="scope.row.relationList" placement="top">
                                    <el-button type="text">
                                        [<?= \Yii::t('plugins/region', '详情');?>]
                                    </el-button>
                                </el-tooltip>
                            </div>
                        </template>
                    </el-table-column>
                    <el-table-column label="<?= \Yii::t('plugins/region', '姓名');?>" prop="name">
                        <el-table-column label="<?= \Yii::t('plugins/region', '手机号');?>" prop="phone">
                            <template slot-scope="scope">
                    ...
                    <el-table-column label="<?= \Yii::t('plugins/region', '累计分红');?>" prop="name">
                        <el-table-column label="<?= \Yii::t('plugins/region', '已提现分红');?>" prop="phone">
                            <template slot-scope="scope">
                    ...
                    <el-table-column label="<?= \Yii::t('plugins/region', '时间');?>" width="250">
                        <template slot-scope="scope">
                            <div v-if="scope.row.status >= 0 && scope.row.applyed_at != '0000-00-00 00:00:00'"><?= \Yii::t('plugins/region', '申请时间');?>{{scope.row.applyed_at}}</div>
                            <div v-if="scope.row.status == 1 || scope.row.status == 2"><?= \Yii::t('plugins/region', '审核时间');?>{{scope.row.agreed_at}}</div>
                        </template>
                    </el-table-column>
                    <el-table-column label="<?= \Yii::t('plugins/region', '状态');?>" width="80" prop="status">
                        <template slot-scope="scope">
                            <el-tag size="small" type="info" v-if="scope.row.status == 0"><?= \Yii::t('plugins/region', '待审核');?></el-tag>
                            <el-tag size="small" v-if="scope.row.status == 1"><?= \Yii::t('plugins/region', '已通过');?></el-tag>
                            <el-tag size="small" type="danger" v-if="scope.row.status == 2"><?= \Yii::t('plugins/region', '拒绝');?></el-tag>
                            <el-tag size="small" type="warning" v-if="scope.row.status == 3"><?= \Yii::t('plugins/region', '处理中');?></el-tag>
                        </template>
                    </el-table-column>
                    <el-table-column label="<?= \Yii::t('plugins/region', '操作');?>" width="250px" fixed="right">
                        <template slot-scope="scope">
                            <el-button v-if="scope.row.level_up" type="text" size="mini" circle style="margin-top: 10px" @click.native="update(scope.row,1)">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/region', '通过升级');?>" placement="top">
                            ...
                            <el-button v-if="scope.row.level_up" type="text" size="mini" circle style="margin-top: 10px" @click.native="update(scope.row,2)">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/region', '拒绝升级');?>" placement="top">
                            ...
                            <el-button v-if="scope.row.status == 1 && !scope.row.level_up" type="text" size="mini" circle style="margin-top: 10px" @click.native="agree(scope.row)">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/region', '编辑');?>" placement="top">
                                    <img src="statics/img/mall/edit.png" alt="">
                                </el-tooltip>
                            </el-button>
                            <el-button v-if="scope.row.status == 0" type="text" size="mini" circle style="margin-top: 10px" @click.native="agree(scope.row)">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/region', '通过申请');?>" placement="top">
                            ...
                            <el-button v-if="scope.row.status == 0" type="text" size="mini" circle style="margin-left: 10px;margin-top: 10px" @click.native="toRelease(scope.row,false)">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/region', '拒绝申请');?>" placement="top">
                            ...
                            <el-button v-if="scope.row.status == 1" type="text" size="mini" circle style="margin-top: 10px" @click.native="toRelease(scope.row,true)">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/region', '解除代理');?>" placement="top">
                            ...
                            <el-button v-if="scope.row.status == 2" type="text" size="mini" circle style="margin-left: 10px;margin-top: 10px" @click.native="deleteShare(scope.row.user_id)">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/region', '删除');?>" placement="top">
                                    <img src="statics/img/mall/del.png" alt="">
                                </el-tooltip>
                            </el-button>
                            <el-button type="text" size="mini" circle style="margin-left: 10px;margin-top: 10px" @click.native="openContent(scope.row)">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/region', '备注');?>" placement="top">
    ...
    <el-dialog class="look-info" :title="is_remove? '<?= \Yii::t('plugins/region', '解除代理');?>':'<?= \Yii::t('plugins/region', '申请成为代理');?>'" :visible.sync="dialogContent" width="30%">
        <el-form :model="detail" :rules="removeRulues" size="small" ref="remove" label-width="120px">
            <el-form-item label="<?= \Yii::t('plugins/region', '昵称');?>">
                {{detail.user ? detail.user.nickname : ''}}
            </el-form-item>
            <el-form-item label="<?= \Yii::t('plugins/region', '代理级别');?>">
                {{detail.level_desc}}
            </el-form-item>
            <el-form-item label="<?= \Yii::t('plugins/region', '归属省');?>">
                {{detail.province}}
            </el-form-item>
            ...
            <el-form-item label="<?= \Yii::t('plugins/region', '代理区域');?>" v-if="detail.level > 2">
                <el-tag type="info" style="margin:5px;margin-top:0;border:0" v-for="(value, key) in detail.relation" :key="key.id">
                    {{value.name}}
                </el-tag>
            </el-form-item>
            <el-form-item :label="is_remove? '<?= \Yii::t('plugins/region', '解除理由');?>':'<?= \Yii::t('plugins/region', '拒绝理由');?>'" prop="content">
                <el-input type="textarea" :rows="5" v-model="content" :placeholder="is_remove? '<?= \Yii::t('plugins/region', '请填写解除代理理由');?>':'<?= \Yii::t('plugins/region', '请填写拒绝理由');?>'" autocomplete="off"></el-input>
            </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
            <el-button size="small" @click="dialogContent = false"><?= \Yii::t('plugins/region', '取消');?></el-button>
            <el-button size="small" type="primary" v-if="is_remove" @click="beRelease('remove')" :loading="contentBtnLoading"><?= \Yii::t('plugins/region', '确认解除');?></el-button>
            <el-button size="small" type="primary" v-else @click="beApply" :loading="contentBtnLoading"><?= \Yii::t('plugins/region', '拒绝申请');?></el-button>
        </div>
    </el-dialog>
    <el-dialog class="look-info" :title="detail.regionInfo.remark.length > 0 ? '<?= \Yii::t('plugins/region', '修改备注');?>': '<?= \Yii::t('plugins/region', '添加备注');?>'" :visible.sync="dialogRemark" width="30%">
        <el-form>
            <el-form-item>
                <el-input type="textarea" :rows="5" v-model="remark" placeholder="<?= \Yii::t('plugins/region', '请填写备注内容');?>" autocomplete="off"></el-input>
            </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
            <el-button size="small" @click="dialogRemark = false"><?= \Yii::t('plugins/region', '取消');?></el-button>
            <el-button size="small" type="primary" @click="beRemark" :loading="contentBtnLoading"><?= \Yii::t('plugins/region', '确认');?></el-button>
        </div>
    </el-dialog>
    <el-dialog :title="lookApply ? addList.status == 1 ?'编辑代理信息' : '申请成为代理':'添加新代理'" :visible.sync="toChange" width="30%" :before-close="handleClose">
        <el-dialog width="20%" title="<?= \Yii::t('plugins/region', '选择代理区域');?>" :visible.sync="innerVisible" append-to-body>
            ...
            <div slot="footer" class="dialog-footer">
                <el-button type="primary" @click="changePlace"><?= \Yii::t('plugins/region', '确认选择');?></el-button>
            </div>
        </el-dialog>
        <el-form :model="addList" :rules="rules" size="small" ref="addForm" label-width="120px">
            <el-form-item v-if="!lookApply" label="<?= \Yii::t('plugins/region', '分销商选择');?>" prop="nickname">
                <el-autocomplete size="small" style="width: 70%;" v-model="addList.nickname" value-key="nickname" :fetch-suggestions="querySearchAsync" placeholder="<?= \Yii::t('plugins/region', '请输入分销商昵称');?>" @select="shareClick"></el-autocomplete>
            </el-form-item>
            <el-form-item v-else label="<?= \Yii::t('plugins/region', '昵称');?>">
                {{addList.user ? addList.user.nickname : ''}}
            </el-form-item>
            <el-form-item label="<?= \Yii::t('plugins/region', '代理级别');?>" prop="level">
                <el-select size="small" @change="chooseLevel" style="width: 70%;" v-model="addList.level" placeholder="<?= \Yii::t('plugins/region', '请选择');?>">
                    <el-option label="<?= \Yii::t('plugins/region', '省代理');?>" :value="1"></el-option>
                    <el-option label="<?= \Yii::t('plugins/region', '市代理');?>" :value="2"></el-option>
                    <el-option label="<?= \Yii::t('plugins/region', '区');?>" :value="3"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="<?= \Yii::t('plugins/region', '归属省');?>" prop="province">
                <el-select size="small" @change="chooseProvince" style="width: 70%;" v-model="addList.province" placeholder="<?= \Yii::t('plugins/region', '请选择');?>">
                    <el-option v-for="item in province" :label="item.name" :value="item.id"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item :label="addList.level == 2 ? '<?= \Yii::t('plugins/region', '代理区域');?>' : '<?= \Yii::t('plugins/region', '归属市');?>'" class="required-icon" v-if="addList.level > 1" prop="city">
                <el-select v-if="addList.level == 3" size="small" @change="chooseCity" style="width: 70%;" v-model="addList.city" placeholder="<?= \Yii::t('plugins/region', '请选择');?>">
                ...
                <div v-else>
                    ...
                    <el-button @click="choosePlace"><?= \Yii::t('plugins/region', '选择');?></el-button>
                </div>
            </el-form-item>
            <el-form-item label="<?= \Yii::t('plugins/region', '代理区域');?>" class="required-icon" v-if="addList.level == 3" prop="district">
        ...
        <div slot="footer" class="dialog-footer">
            <el-button size="small" @click="handleClose"><?= \Yii::t('plugins/region', '取消');?></el-button>
            <el-button size="small" type="primary" @click="changeSubmit('addForm')" :loading="contentBtnLoading">{{lookApply ? addList.status == 1 ?'<?= \Yii::t('plugins/region', '确认修改');?>':'<?= \Yii::t('plugins/region', '通过申请');?>' : '<?= \Yii::t('plugins/region', '确定');?>'}}</el-button>
        </div>
    </el-dialog>
    <el-dialog title="<?= \Yii::t('plugins/region', '升级代理');?>" :visible.sync="toUpdate" width="30%" :before-close="handleClose">
        <el-form :rules="removeRulues" :model="updateList" size="small" ref="updateFrom" label-width="150px">
            <el-form-item label="<?= \Yii::t('plugins/region', '昵称');?>">
                {{updateList.user.nickname}}
            </el-form-item>
            <el-form-item label="<?= \Yii::t('plugins/region', '申请升级代理级别');?>" prop="level">
                <div v-if="showLevel == 1"><?= \Yii::t('plugins/region', '省代理');?></div>
                <div v-if="showLevel == 2"><?= \Yii::t('plugins/region', '市代理');?></div>
            </el-form-item>
            <el-form-item label="<?= \Yii::t('plugins/region', '申请升级代理区域');?>" prop="province" v-if="showLevel == 1">
                <el-select disabled size="small" @change="chooseProvince" style="width: 70%;" v-model="updateList.province_id" placeholder="<?= \Yii::t('plugins/region', '请选择');?>">
                    <el-option v-for="item in province" :label="item.name" :value="item.id"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="<?= \Yii::t('plugins/region', '申请升级代理区域');?>" v-if="showLevel == 2" prop="city">
                <el-tag type="info" style="margin:0 5px;border:0" v-for="(value, key) in updateList.level_up.relation" :key="key.id">
                    {{value.name}}
                </el-tag>
            </el-form-item>
            <el-form-item v-if="upStatus == 2" label="<?= \Yii::t('plugins/region', '拒绝理由');?>" prop="content">
                <el-input type="textarea" :rows="5" v-model="content" placeholder="<?= \Yii::t('plugins/region', '请填写拒绝理由');?>" autocomplete="off"></el-input>
            </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
            <el-button size="small" @click="handleClose"><?= \Yii::t('plugins/region', '取消');?></el-button>
            <el-button size="small" type="primary" @click="updateSubmit('updateFrom')" :loading="contentBtnLoading">{{upStatus == 1 ?'<?= \Yii::t('plugins/region', '通过申请');?>':'<?= \Yii::t('plugins/region', '拒绝申请');?>'}}</el-button>
        </div>
    </el-dialog>
</div>
<script>
        ...
        data() {
            var validateRate = (rule, value, callback) => {
                if (this.value === '' || this.value === undefined) {
                    callback(new Error('<?= \Yii::t('plugins/region', '请选择代理等级');?>'));
                } else {
                    callback();
                }
            };
            var validateRate2 = (rule, value, callback) => {
                if (this.content === '' || this.content === undefined) {
                    callback(new Error('<?= \Yii::t('plugins/region', '请填写理由');?>'));
                } else {
                    callback();
                }
            };
            ...
            return {
                ...
                rules: {
                    nickname: [
                        { required: true, message: '<?= \Yii::t('plugins/region', '请选择分销商');?>', trigger: 'change' }
                    ],
                    province: [
                        { required: true, message: '<?= \Yii::t('plugins/region', '请选择归属省');?>', trigger: 'change' }
                    ],
                    level: [
        ...
        methods: {
                    ...
                    if (valid) {
                        ...
                        if(that.updateList.level_up.level == '2') {
                            if(that.updateList.level_up.relation.length > 0) {
                                ...
                                that.$message.error('<?= \Yii::t('plugins/region', '请选择申请升级代理区域');?>');
                                return false;
                            }
                        }
            ...
            choosePlace() {
                if(this.addList.level == 2) {
                    if(this.addList.province) {
                        ...
                        this.$message.error('<?= \Yii::t('plugins/region', '请选择归属省');?>');
                    }
                }
                if(this.addList.level == 3) {
                    if(this.addList.city) {
                        ...
                        this.$message.error('<?= \Yii::t('plugins/region', '请选择归属市');?>');
                    }
                }
                if(this.showLevel == 2) {
                    this.innerVisible=true
            ...
            addregion() {
                if(this.province.length == 0) {
                    this.$message.error('<?= \Yii::t('plugins/region', '请先添加代理级别');?>');
                    return false
                }
                this.toChange = true;
                ...
                request({
                    ...
                    if (e.data.code == 0) {
                        if(that.status == 1) {
                            ...
                            that.$message.success('<?= \Yii::t('plugins/region', '操作成功');?>');
                            that.loadData();
                            that.contentBtnLoading = false;
                ...
                that.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        request({
                            ...
                            that.contentBtnLoading = false;
                            if (e.data.code == 0) {
                                that.$message.success('<?= \Yii::t('plugins/region', '解除成功');?>');
                                that.loadData();
                                that.contentBtnLoading = false;
                                that.dialogContent = false;
                                // let queue_id = e.data.data.queue_id;
            ...
            // 删除记录
            deleteShare(id) {
                this.$confirm('<?= \Yii::t('plugins/region', '是否删除该条记录');?>', '<?= \Yii::t('plugins/region', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('plugins/region', '确定');?>',
                    cancelButtonText: '<?= \Yii::t('plugins/region', '取消');?>',
                    type: 'warning',
                    center: true,
                    beforeClose: (action, instance, done) => {
                        if (action === 'confirm') {
                            instance.confirmButtonLoading = true;
                            instance.confirmButtonText = '<?= \Yii::t('plugins/region', '执行中');?>...';
                            request({
                                params: {
                    ...
                    this.$message({
                        type: 'info',
                        message: '<?= \Yii::t('plugins/region', '取消了操作');?>'
                    });
                });
            },
            // 申请添加备注
