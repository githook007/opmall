...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;" v-loading="loading">
        <!-- 标题栏 -->
        <div slot="header">
            <span><?= \Yii::t('plugins/miaosha', '售后订单');?></span>
        ...
        <div class="form-body">
                    ...
                    <div class="input-item">
                        <el-input size="small" placeholder="请输入搜索内容" v-model="search.keyword">
                            <el-select style="width: 120px" slot="prepend" v-model="search.keyword_1">
                                <el-option label="<?= \Yii::t('plugins/miaosha', '订单号');?>" value="1"></el-option>
                                <el-option label="<?= \Yii::t('plugins/miaosha', '用户名');?>" value="2"></el-option>
                                <el-option label="<?= \Yii::t('plugins/miaosha', '用户ID');?>" value="4"></el-option>
                                <el-option label="<?= \Yii::t('plugins/miaosha', '商品名称');?>" value="5"></el-option>
                                <el-option label="<?= \Yii::t('plugins/miaosha', '收件人');?>" value="3"></el-option>
                                <el-option label="<?= \Yii::t('plugins/miaosha', '收件人电话');?>" value="6"></el-option>
                            </el-select>
            ...
            <el-tabs v-model="activeName" @tab-click="handleClick">
                <el-tab-pane label="<?= \Yii::t('plugins/miaosha', '全部');?>" name="0"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('plugins/miaosha', '待处理');?>" name="1"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('plugins/miaosha', '已处理');?>" name="2"></el-tab-pane>
            </el-tabs>
            <!-- 订单内容 -->
            <app-order :select_list="selectList" @get="getList" :list="list" refund :other_list="address">
                <template slot-scope="item">
                    ...
                    <div class="app-order-info" style="width: 8%">
                        <div>
                            <div v-if="item.item.type == 1" type="danger"><?= \Yii::t('plugins/miaosha', '退货退款');?></div>
                            <div v-if="item.item.type == 2" type="warning"><?= \Yii::t('plugins/miaosha', '换货');?></div>
                        </div>
                    ...
                    <div class="app-order-info" style="width: 7.5%">
                        <div>
                            <div size="medium" hit v-if="item.item.status == 1" type="info">
                                <span><?= \Yii::t('plugins/miaosha', '待处理');?></span>
                                <el-tooltip v-if="item.item.pay_type == 2" effect="dark" content="<?= \Yii::t('plugins/miaosha', '货到付款方式的退款需要线下与客户自行协商');?>"
                                            placement="top">
                                    <i class="header-icon el-icon-info"></i>
                                </el-tooltip>
                            </div>
                            <div v-if="item.item.status == 2"><?= \Yii::t('plugins/miaosha', '已同意');?>
                            </div>
                            <div v-if="item.item.status == 3"><?= \Yii::t('plugins/miaosha', '已拒绝');?>
                            </div>
                            <div style="margin-top: 10px;" v-if="item.item.status == 2 && item.item.is_send == 0"
                                 type="warning"><?= \Yii::t('plugins/miaosha', '等待买家发货');?>
                            </div>
                            <div style="margin-top: 10px;" v-if="item.item.status == 2 && item.item.is_send == 1"
                                 type="success"><?= \Yii::t('plugins/miaosha', '买家已发货');?>
                            </div>
                        </div>
                    </div>
                    <!-- 退款金额 -->
    ...
    new Vue({
        ...
        data() {
            return {
                ...
                list: [],
                price: '9%',
                selectList: [{value:'1',name:'<?= \Yii::t('plugins/miaosha', '订单号');?>'},{value:'2',name:'<?= \Yii::t('plugins/miaosha', '用户名');?>'},{value:'4',name:'<?= \Yii::t('plugins/miaosha', '用户ID');?>'},{value:'5',name:'<?= \Yii::t('plugins/miaosha', '商品名称');?>'},{value:'3',name:'<?= \Yii::t('plugins/miaosha', '收件人');?>'},{value:'6',name:'<?= \Yii::t('plugins/miaosha', '收件人电话');?>'}],
                address: [],
                export_list: [],
            };
        },
        ...
        methods: {
            ...
            getList() {
                ...
                loadList('plugin/miaosha/mall/order/refund').then(e => {
                    this.$message({
                        message: '<?= \Yii::t('plugins/miaosha', '请求成功');?>',
                        type: 'success'
                    });
                    this.loading = false;
                    this.list = e.list;
