    ...
    <el-card v-loading="listLoading" class="box-card" shadow="never" style="border:0"
        ...
        <div slot="header">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><span style="color: #409EFF;cursor: pointer"
                                          @click="$navigate({r:'plugin/miaosha/mall/goods'})"><?= \Yii::t('plugins/miaosha', '秒杀商品');?></span>
                </el-breadcrumb-item>
                <el-breadcrumb-item><?= \Yii::t('plugins/miaosha', '秒杀场次');?></el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="table-body">
            ...
            <el-table @selection-change="handleSelectionChange" :data="list" border
                ...
                <el-table-column prop="goods_id" width="80" label="<?= \Yii::t('plugins/miaosha', '商品ID');?>"></el-table-column>
                <el-table-column prop="open_date" width="120" label="<?= \Yii::t('plugins/miaosha', '秒杀日期');?>"></el-table-column>
                <el-table-column prop="open_time" label="<?= \Yii::t('plugins/miaosha', '秒杀时间');?>"></el-table-column>
                <el-table-column label="<?= \Yii::t('plugins/miaosha', '限购数量');?>">
                    <template slot-scope="scope">
                        <span>{{scope.row.goods.confine_count == '-1' ? '<?= \Yii::t('plugins/miaosha', '无限制');?>' : scope.row.goods.confine_count}}</span>
                    </template>
                </el-table-column>
                <el-table-column label="<?= \Yii::t('plugins/miaosha', '限单');?>">
                    <template slot-scope="scope">
                        <span>{{scope.row.goods.confine_order_count == '-1' ? '<?= \Yii::t('plugins/miaosha', '无限制');?>' : scope.row.goods.confine_order_count}}</span>
                    </template>
                </el-table-column>
                <el-table-column prop="status" width="100" label="<?= \Yii::t('plugins/miaosha', '上架状态');?>">
                    <template slot-scope="scope">
                        <el-switch
                                active-value="1"
                                inactive-value="0"
                ...
                <el-table-column
                    ...
                    <template slot-scope="scope">
                        ...
                        <el-button @click="destroy(scope.row, scope.$index)" type="text" circle size="mini">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/miaosha', '删除');?>" placement="top">
