    ...
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 0 0;">
        <div slot="header">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item v-if="goods_id"><?= \Yii::t('plugins/miaosha', '编辑活动');?></el-breadcrumb-item>
                <el-breadcrumb-item v-else><?= \Yii::t('plugins/miaosha', '新建活动');?></el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <app-goods ref="appGoods"
            ...
            <template slot="before_info">
                <el-card shadow="never" style="margin-bottom: 24px">
                    <div slot="header"><?= \Yii::t('plugins/miaosha', '活动设置');?></div>
                    <el-col :span="24">
                        <el-form-item v-if="is_add == 1" label="<?= \Yii::t('plugins/miaosha', '开放日期');?>">
                            <el-date-picker
                    ...
                    <el-col v-if="is_add == 1" :span="24">
                        <el-form-item label="<?= \Yii::t('plugins/miaosha', '开放时间');?>">
                            <template v-if="!form.id > 0">
                                <el-checkbox  :indeterminate="isIndeterminate" v-model="checkAll"
                                              @change="handleCheckAllChange">
                                    <?= \Yii::t('plugins/miaosha', '全选');?>
                                </el-checkbox>
            ...
            <template slot="before_price">
                <el-form-item label="<?= \Yii::t('plugins/miaosha', '秒杀价');?>" prop="price">
                    <el-input type="number"
                              placeholder="<?= \Yii::t('plugins/miaosha', '请输入商品秒杀价');?>"
                              :disabled="use_attr == 1"
                              oninput="this.value = this.value.replace(/^(\-)*(\d+)\.(\d\d).*$/,'$1$2.$3');"
                              min="0"
                              v-model="form.price">
                        <template slot="append"><?= \Yii::t('plugins/miaosha', '元');?></template>
                    </el-input>
                </el-form-item>
            </template>
            ...
            <template slot="before_services">
                <el-form-item prop="virtual_sales">
                    <template slot='label'>
                        <span><?= \Yii::t('plugins/miaosha', '已秒杀量');?></span>
                        <el-tooltip effect="dark" content="<?= \Yii::t('plugins/miaosha', '前端展示的销量');?>" placement="top">
            ...
            <template slot="member_route_setting">
                 <span class="red"><?= \Yii::t('plugins/miaosha', '必须在');?>
                    <el-button type="text" @click="$navigate({r: 'plugin/miaosha/mall/index'}, true)"><?= \Yii::t('plugins/miaosha', '优惠叠加设置');?></el-button>
                    <?= \Yii::t('plugins/miaosha', '才能使用');?>
                </span>
            </template>
                        ...
                        <div flex="dir:left" style="font-size:14px">
                            <div flex="dir:top" style="font-size: 10px">
                                ...
                                <div flex="dir:left">
                                    ...
                                    <div style="color: #999999;margin-left: 6px"><?= \Yii::t('plugins/miaosha', '销量');?>{{previewData.virtual_sales}}{{previewData.unit}}</div>
                                </div>
                            </div>
                            <div class="share" flex="dir:top main:center cross:center">
                                <el-image src="statics/img/mall/goods/icon-share.png"></el-image>
                                <div><?= \Yii::t('plugins/miaosha', '分享');?></div>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        data() {
            return {
                ...
                form: {
                    ...
                    price: '',
                    form: {},
                    unit: '<?= \Yii::t('plugins/miaosha', '件');?>',
                    virtual_sales: 0,
                    extra: {
                        price: '<?= \Yii::t('plugins/miaosha', '秒杀价');?>'
                    }
                },
                use_attr: 0,
                rule: {
                    is_alone_buy: [
                        {required: true, message: '<?= \Yii::t('plugins/miaosha', '请选择是否允许单独购买');?>', trigger: 'change'},
                    ],
                    price: [
                        {required: true, message: '<?= \Yii::t('plugins/miaosha', '请输入商品价格');?>', trigger: 'change'}
                    ]
                },
                ...
                pickerOptions: {
                    ...
                    shortcuts: [{
                        text: '<?= \Yii::t('plugins/miaosha', '未来一周');?>',
                        onClick(picker) {
                            const end = new Date();
                        ...
                        text: '<?= \Yii::t('plugins/miaosha', '未来半月');?>',
                        onClick(picker) {
                            const end = new Date();
                            const start = new Date();
                        ...
                        text: '<?= \Yii::t('plugins/miaosha', '未来一个月');?>',
                        onClick(picker) {
                            const end = new Date();
                            const start = new Date();
        ...
        methods: {
            ...
            handlePreview(e) {
                ...
                if (max > min && min >= 0) {
                    ...
                    actualPrice = '<?= \Yii::t('plugins/miaosha', '免费');?>';
                    type = '';
                }
            ...
            childrenGoods(e) {
                ...
                if (getQuery('id')) {
                    ...
                    this.form.open_date = [];
                    this.form.open_date.push(e.miaoshaGoods.open_date, e.miaoshaGoods.open_date);
