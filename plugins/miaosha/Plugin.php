...
use app\plugins\miaosha\forms\api\v2\StatisticsForm;
...
class Plugin extends \app\plugins\Plugin
{
    ...
    public function getMenus()
    {
        return [
            [
                'name' => \Yii::t('plugins/miaosha', '秒杀设置'),
                'route' => 'plugin/miaosha/mall/index',
                'icon' => 'el-icon-star-on',
            ],
            [
                'name' => \Yii::t('plugins/miaosha', '秒杀活动'),
            ...
            [
                'name' => \Yii::t('plugins/miaosha', '活动数据'),
    ...
    {
        return \Yii::t('plugins/miaosha', '整点秒杀');
    }
    ...
    public function getPickLink()
    {
        ...
        return [
            [
                'key' => 'miaosha',
                'name' => \Yii::t('plugins/miaosha', '秒杀首页'),
                'open_type' => '',
            ...
            [
                'key' => 'miaosha',
                'name' => \Yii::t('plugins/miaosha', '秒杀商品详情'),
                'open_type' => '',
                'icon' => $iconBaseUrl . '/icon-miaosha-goods.png',
                'value' => '/plugins/miaosha/goods/goods',
                'params' => [
                    [
                        'key' => 'id',
                        'value' => '',
                        'desc' => \Yii::t('plugins/miaosha', '请填写秒杀场次ID'),
                        'is_required' => true,
                        'data_type' => 'number',
                        'page_url' => 'plugin/miaosha/mall/activity/index',
                        'page_url_text' => \Yii::t('plugins/miaosha', '商品列表'),
                    ],
                    [
                        'key' => 'is_activity',
                        'value' => 1,
                        'desc' => \Yii::t('plugins/miaosha', '判断是秒杀活动ID还是场次ID'),
                        'is_required' => true,
                        'data_type' => 'number',
                        'is_show' => false,
                    ],
    ...
    {
        // 修改diy首页小程序不显示商品 @czs
        return \app\plugins\miaosha\forms\common\v2\CommonGoods::getCommon()->getHomePage($type, $limit);
    }
    ...
    public function getStatisticsMenus($bool = true)
    {
        return [
            ...
            'name' => $bool ? $this->getDisplayName() : \Yii::t('plugins/miaosha', '插件统计'),
            'key' => $this->getName(),
    ...
    public function checkGoods($goods, $item)
    {
        ...
        if (!$miaoshaGoods) {
            throw new OrderException(\Yii::t('plugins/miaosha', '秒杀商品不存在'));
        }

        if ($miaoshaGoods->open_date != date('Y-m-d') || $miaoshaGoods->open_time != date('H')) {
            throw new OrderException(\Yii::t('plugins/miaosha', '秒杀活动未开始或已结束'));
        }
        ...
        if (!$activity) {
            throw new OrderException(\Yii::t('plugins/miaosha', '当前秒杀活动已结束'));
        }
        ...
        if ($goods->confine_order_count != -1 && $buyCount >= $goods->confine_order_count) {
            throw new OrderException(\Yii::t('plugins/miaosha', '超出购买限单').'(' . $goods->confine_order_count . ')'.\Yii::t('plugins/miaosha', '次数'));
        }
    }
