...
class Plugin extends \app\plugins\Plugin
{
    public function getMenus()
    {
        return [
            [
                'name' => \Yii::t('plugins/integral_mall', '积分商城设置'),
                'route' => 'plugin/integral_mall/mall/setting/index',
                'icon' => 'el-icon-star-on',
            ],
            [
                'name' => \Yii::t('plugins/integral_mall', '商品管理'),
            ...
            [
                'name' => \Yii::t('plugins/integral_mall', '优惠券管理'),
                'route' => 'plugin/integral_mall/mall/coupon/index',
                'icon' => 'el-icon-star-on',
            ],
            [
                'name' => \Yii::t('plugins/integral_mall', '用户兑换券'),
    ...
    {
        return \Yii::t('plugins/integral_mall', '积分商城');
    }
    ...
    public function getPickLink()
    {
        ...
        return [
            [
                'key' => 'integral_mall',
                'name' => \Yii::t('plugins/integral_mall', '积分商城'),
                'open_type' => '',
            ...
            [
                'key' => 'integral_mall',
                'name' => \Yii::t('plugins/integral_mall', '积分商品详情'),
                'open_type' => '',
                'icon' => $iconBaseUrl . '/icon-integral-detail.png',
                'value' => '/plugins/integral_mall/goods/goods',
                'params' => [
                    [
                        'key' => 'goods_id',
                        'value' => '',
                        'desc' => \Yii::t('plugins/integral_mall', '请填写积分商品ID'),
                        'is_required' => true,
                        'data_type' => 'number',
                        'page_url' => 'plugin/integral_mall/mall/goods/index',
                        'page_url_text' => \Yii::t('plugins/integral_mall', '商品管理'),
                    ],
                ],
    ...
    public function getStatisticsMenus($bool = true)
    {
        return [
            ...
            'name' => $bool ? $this->getDisplayName() : \Yii::t('plugins/integral_mall', '插件统计'),
            'key' => $this->getName(),
    ...
    public function getGoodsExtra($goods)
    {
        ...
        /* @var Goods $goods */
        $content = $goods->integralMallGoods->integral_num . \Yii::t('plugins/integral_mall', '积分');
        if ($goods->price > 0) {
            $content .= '+￥' . $goods->price;
        }
        return [
        ...
        if ($order['sign'] == $this->getName()) {
            ...
            if ($integralOrder) {
                foreach ($order['detail'] as $key => $value) {
                    $data['price_list'][] = [
                        'order_id' => $value['id'],
                        'label' => \Yii::t('plugins/integral_mall', '兑换金额'),
                        'value' => $value['total_price'],
                    ];
                    $data['exchange_list'][] = [
                        'order_id' => $value['id'],
                        'label' => \Yii::t('plugins/integral_mall', '兑换积分'),
                        'value' => $integralOrder->integral_num,
                    ];
                }
                $data['price_name'] = \Yii::t('plugins/integral_mall', '积分');
