    ...
    <div class="header-box">
        <el-breadcrumb separator="/">
            <el-breadcrumb-item><span style="color: #409EFF;cursor: pointer"
                                      @click="$navigate({r:'plugin/integral_mall/mall/goods/index'})"><?= \Yii::t('plugins/integral_mall', '商品管理');?></span></el-breadcrumb-item>
            <el-breadcrumb-item v-if="form.id>0"><?= \Yii::t('plugins/integral_mall', '编辑商品');?></el-breadcrumb-item>
            <el-breadcrumb-item v-else><?= \Yii::t('plugins/integral_mall', '添加商品');?></el-breadcrumb-item>
        </el-breadcrumb>
    </div>
    <app-goods
            ref="appGoods"
        ...
        <template slot="before_attr">
            <el-card shadow="never" class="mt-24">
                <div slot="header">
                    <span><?= \Yii::t('plugins/integral_mall', '显示设置');?></span>
                </div>
                <el-row>
                    <el-col :xl="12" :lg="16">
                        <el-form-item label="<?= \Yii::t('plugins/integral_mall', '放置首页');?>">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/integral_mall', '商品会在积分商城首页展示');?>"
                                        placement="top">
                                <el-switch
                                        :active-value="1"
                                        :inactive-value="0"
        ...
        <template slot="before_price">
            <!-- 兑换价格 -->
            <el-form-item label="<?= \Yii::t('plugins/integral_mall', '兑换价格');?>" prop="price" >
                <el-input type="number"
                          oninput="this.value = this.value.replace(/[^0-9\.]/, '');" :min="0"
                          v-model="form.price">
                    <template slot="append"><?= \Yii::t('plugins/integral_mall', '元');?></template>
                </el-input>
            </el-form-item>
            ...
            <el-form-item label="<?= \Yii::t('plugins/integral_mall', '兑换积分');?>" prop="integral_num" >
                <el-input type="number"
                    ...
                    <template slot="append"><?= \Yii::t('plugins/integral_mall', '分');?></template>
                </el-input>
            </el-form-item>
        </template>
                ...
                <div class="goods">
                    ...
                    <div flex="dir:left" style="font-size:14px">
                        <div flex="dir:left" style="font-size: 18px;height:22px;color:#ff4544;margin-top:15px">
                            <div flex="dir:top">
                                <div style="font-size:15px;margin-bottom:3px">
                                    {{previewData.integral_num}}<?= \Yii::t('plugins/integral_mall', '积分');?>+{{previewData.price}}<?= \Yii::t('plugins/integral_mall', '元');?>
                                </div>
                        ...
                        <div class="share" flex="dir:top main:center cross:center">
                            ...
                            <div><?= \Yii::t('plugins/integral_mall', '分享');?></div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                rule: {
                    integral_num: [
                        {required: true, message: '<?= \Yii::t('plugins/integral_mall', '请输入兑换积分');?>', trigger: 'change'},
                    ],
                    price: [
                        {required: true, message: '<?= \Yii::t('plugins/integral_mall', '请输入兑换价格');?>', trigger: 'change'},
                    ],
                },
                ...
                form: {
                    ...
                    price: '',
                    extra: {
                        integral_num: '<?= \Yii::t('plugins/integral_mall', '积分');?>',
                    }
                },
            };
        },
