...
class Plugin extends \app\plugins\Plugin
{
    ...
    public function getMenus()
    {
        return [
            [
                'name' => \Yii::t('plugins/mobile', '基础配置'),
                'route' => 'plugin/mobile/mall/config/index',
                'icon' => 'el-icon-setting',
            ],
            [
                'name' => \Yii::t('plugins/mobile', '其他设置'),
    ...
    {
        return \Yii::t('plugins/mobile', 'h5商城');
    }
    ...
    public function getAliConfig($platform)
    {
        ...
        if (!$payType) {
            throw new \Exception(\Yii::t('plugins/mobile', '支付宝支付尚未配置'));
        }
        return $payType;
    }
    ...
    public function checkSign($platform)
    {
        ...
        if (!$config || !$config->alipay_public_key || !$config->app_private_key || !$config->alipay_appid) {
            throw new \Exception(\Yii::t('plugins/mobile', 'H5商城支付宝支付尚未配置'));
        }
        try {
    ...
    public function getWechatPay($platform)
    {
        ...
        if (!$newPayType) {
            throw new \Exception(\Yii::t('plugins/mobile', '支付方式不存在'));
        }
    ...
    public function getSmsSetting()
    {
        return [
            'password' => [
                'title' => \Yii::t('plugins/mobile', '注册成功短信'),
                'content' => \Yii::t('plugins/mobile', '恭喜您注册成功'),
                'support_mch' => false,
                'loading' => false,
                'variable' => [
                    [
                        'key' => 'password',
                        'value' => \Yii::t('plugins/mobile', '模板变量'),
                        'desc' => \Yii::t('plugins/mobile', '您的默认密码为').'${password}'.\Yii::t('plugins/mobile', '请及时登录并修改密码')
                    ]
                ],
                'key' => 'user'
            ],
