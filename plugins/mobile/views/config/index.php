...
<div id="app" v-cloak>
    <el-card shadow="never" v-loading="cardLoading" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <span><?= \Yii::t('plugins/mobile', '基础配置');?></span>
        </div>
        <div class="build-qr" v-if="!qrcode">
            <img src="<?= $iconBaseUrl ?>bg.png" alt="">
            <div class="build-qr-btn">
                <div style="margin-bottom: 38px;"><?= \Yii::t('plugins/mobile', '配置H5');?></div>
                <el-button size="small" type="primary" :loading="btnLoading" @click="issue"><?= \Yii::t('plugins/mobile', '去配置');?></el-button>
            </div>
        </div>
        <div class="build-qr-item" flex="dir:top cross:center" v-else>
            ...
            <div flex="cross:center main:center" class="build-qr-item-info">
                <div><?= \Yii::t('plugins/mobile', '链接');?></div>
                <div id="path" style="max-width: 40%;margin: 0 20px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap">{{path}}</div>
                <el-tooltip style="cursor:pointer" effect="dark" content="<?= \Yii::t('plugins/mobile', '复制链接');?>" placement="top">
            ...
            <div class="build-update-btn" flex="dir:top cross:center">
                <div style="margin-bottom: 5px;"><?= \Yii::t('plugins/mobile', '当前版本');?> {{version}}</div>
                <div style="margin-bottom: 50px;"><?= \Yii::t('plugins/mobile', '最新版本');?> {{last_version}}</div>
                <el-button size="small" type="primary" :loading="btnLoading" @click="issue"><?= \Yii::t('plugins/mobile', '发布新版本');?></el-button>
            </div>
        </div>
    </el-card>
</div>
...
<script>
    ...
    clipboard.on('success', function (e) {
        self.ELEMENT.Message.success('<?= \Yii::t('plugins/mobile', '复制成功');?>');
        e.clearSelection();
    });
    clipboard.on('error', function (e) {
        self.ELEMENT.Message.success('<?= \Yii::t('plugins/mobile', '复制失败');?>');
    });
    const app = new Vue({
        el: '#app',
        data() {
        ...
        methods: {
            issue() {
                ...
                request({
                    ...
                    if (e.data.code == 0) {
                        this.$message({
                            message: '<?= \Yii::t('plugins/mobile', '发布成功');?>',
                            type: 'success'
                        });
