...
class OrderSubmitForm extends Model
{
    ...
    private function getPreferential()
    {
        ...
        $data['send_balance_type'] = (int)$data['send_balance_type'];
        ...
        foreach ($group->rules as $rule) {
            ...
            if ($activity->send_type == 1) {
                ...
                if ($data['pay_price'] >= $rule->consume_money) {
                    ...
                    if ($rule->send_money_type == 1) {
                        $sendData['send_balance'] += (float)$rule->send_money;
                    } else {
                        $sendData['send_balance'] += $data['pay_price'] * ($rule->send_money / 100);
                    }
    ...
    private function getSupportPayTypes($scanCodePaySetting)
    {
        ...
        foreach ($scanCodePaySetting['payment_type'] as $item) {
            if ($item == 'online_pay') {
                $arr[] = \app\bootstrap\payment\Payment::PAY_TYPE_WECHAT;
                $arr[] = \app\bootstrap\payment\Payment::PAY_TYPE_ALIPAY;
                $arr[] = \app\bootstrap\payment\Payment::PAY_TYPE_BAIDU;
                $arr[] = \app\bootstrap\payment\Payment::PAY_TYPE_TOUTIAO;
                $arr[] = \app\bootstrap\payment\Payment::PAY_TYPE_WECHAT_H5;
                $arr[] = \app\bootstrap\payment\Payment::PAY_TYPE_ALIPAY_H5;
            }
            if ($item == 'balance') {
                $arr[] = \app\bootstrap\payment\Payment::PAY_TYPE_BALANCE;
            }
        }
