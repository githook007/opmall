...
class OrderExport extends BaseExport
{
    ...
    {
        $query = $this->query;
        $query = $query->with(['user.userInfo', 'detail.goods.goodsWarehouse'])->orderBy('o.created_at DESC');
        ...
        foreach ($list as $item) {
            ...
            if ($sign) {
                foreach ($item->detail as $detailItem) {
                    ...
                    // 规格详情
                    $goodsInfo = json_decode($detailItem->goods_info, true);
                    $attr = '';
                    ...
                    $newArr['goods_name'] = $goodsInfo['goods_attr']['name'];
                    $newArr['attr'] = $attr;
