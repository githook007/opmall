    ...
    <el-card v-loading="listLoading" style="border:0" shadow="never"
            ...
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <span style="color: #409EFF;cursor: pointer"
                        ...
                        <?= \Yii::t('plugins/scan_code_pay', '买单设置');?>
                    </span>
                </el-breadcrumb-item>
                <el-breadcrumb-item v-if="form.id > 0"><?= \Yii::t('plugins/scan_code_pay', '编辑活动');?></el-breadcrumb-item>
                <el-breadcrumb-item v-else><?= \Yii::t('plugins/scan_code_pay', '添加活动');?></el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        ...
        <el-form :model="form" label-width="10rem" ref="form" :rules="rules">
            <el-tabs v-model="activeName">
                <!-- 基础设置 -->
                <el-tab-pane label="<?= \Yii::t('plugins/scan_code_pay', '基础设置');?>" class="form-body" name="first">
                    <div class="form-body">
                        <el-form-item label="<?= \Yii::t('plugins/scan_code_pay', '活动名称');?>" prop="name">
                            <el-input size="small" v-model="form.name" autocomplete="off"></el-input>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('plugins/scan_code_pay', '活动日期');?>" prop="time">
                            <el-date-picker v-model="form.time" unlink-panels type="datetimerange" size="small"
                                            value-format="yyyy-MM-dd HH:mm:ss" range-separator="<?= \Yii::t('plugins/scan_code_pay', '至');?>"
                                            start-placeholder="<?= \Yii::t('plugins/scan_code_pay', '开始日期');?>" end-placeholder="<?= \Yii::t('plugins/scan_code_pay', '结束日期');?>"></el-date-picker>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('plugins/scan_code_pay', '赠送规则');?>" prop="send_type">
                            <label slot="label"><?= \Yii::t('plugins/scan_code_pay', '赠送规则');?>
                                <el-tooltip class="item" effect="dark" slot="content" placement="top">
                                    <div slot="content"><?= \Yii::t('plugins/scan_code_pay', '设置了100');?><br/>
                                        <?= \Yii::t('plugins/scan_code_pay', '若选择赠送满足的所有规则');?></br>
                                        <?= \Yii::t('plugins/scan_code_pay', '若选择只赠送满足的最高级规则');?>
                                    </div>
                                    <i class="el-icon-info"></i>
                                </el-tooltip>
                            </label>
                            <el-radio-group v-model="form.send_type">
                                <el-radio :label="1"><?= \Yii::t('plugins/scan_code_pay', '赠送满足的所有规则');?></el-radio>
                                <el-radio :label="2"><?= \Yii::t('plugins/scan_code_pay', '只赠送满足的最高级规则');?></el-radio>
                            </el-radio-group>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('plugins/scan_code_pay', '买单规则');?>" prop="rules">
                            <el-input :rows="10" type="textarea" v-model="form.rules" autocomplete="off"></el-input>
                        </el-form-item>
                        <el-form-item class="switch" label="<?= \Yii::t('plugins/scan_code_pay', '是否启用');?>" prop="status">
                            <el-switch v-model="form.status" :active-value="1" :inactive-value="0"></el-switch>
                        </el-form-item>
                    </div>
                </el-tab-pane>
                <!-- 规则设置 -->
                <el-tab-pane label="<?= \Yii::t('plugins/scan_code_pay', '规则设置');?>" class="form-body" style="padding-right:0%" name="second">
                    <div class="title"><?= \Yii::t('plugins/scan_code_pay', '若设置了单独会员');?></div>
                    <div flex="dir:left">
                        <span class="key"><?= \Yii::t('plugins/scan_code_pay', '用户组');?></span>
                        <div style="width:100%">
                            <el-table style="margin-bottom:20px"
                                      v-if="form.groups && form.groups.length"
                                      :data="form.groups" border>
                                <el-table-column prop="name" label="<?= \Yii::t('plugins/scan_code_pay', '用户组名称');?>"></el-table-column>
                                <el-table-column prop="member" label="<?= \Yii::t('plugins/scan_code_pay', '用户组成员');?>">
                                ...
                                <el-table-column label="<?= \Yii::t('plugins/scan_code_pay', '操作');?>">
                                    <template slot-scope="scope">
                                        <el-button circle size="mini" type="text"
                                                   @click="editUserGroup(scope.$index,scope.row)">
                                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/scan_code_pay', '编辑');?>" placement="top">
                                        ...
                                        <el-button type="text" size="mini"
                                            ...
                                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/scan_code_pay', '删除');?>" placement="top">
                            ...
                            <el-link v-if="userGroupBtnStatus" @click="addUserGroup" type="primary" :underline="false">
                                +<?= \Yii::t('plugins/scan_code_pay', '新增用户组');?>
                            </el-link>
                        </div>
                    </div>
                    ...
                    <div v-if="form.groups && form.groups.length" flex="dir:left" style="margin-top: 40px">
                        <span class="key"><?= \Yii::t('plugins/scan_code_pay', '规则设定');?></span>
                        <div style="width:100%;">
                            <div v-for="(group,index) in form.groups" :key="index" class="scan-box">
                                ...
                                <div style="background:#FFFFFF;width:100%">
                                    <div flex="dir:left cross:center" style="height:48px;">
                                        <div class="scan-rules"><?= \Yii::t('plugins/scan_code_pay', '赠送规则');?></div>
                                        <div @click="sendBatchSet(index)"
                                             v-if="group.send_rules && group.send_rules.length"
                                             class="scan-sendBatch"><?= \Yii::t('plugins/scan_code_pay', '批量设置');?>
                                        </div>
                                        <el-link @click="sendRules(index)" type="primary" :underline="false">
                                            +<?= \Yii::t('plugins/scan_code_pay', '新增赠送规则');?>
                                        </el-link>
                                    </div>
                                    <el-table v-if="group.send_rules && group.send_rules.length"
                                              :data="group.send_rules" border>
                                        <el-table-column prop="consume_money" label="<?= \Yii::t('plugins/scan_code_pay', '实际付款金额');?>"
                                                         width="180"></el-table-column>
                                        <el-table-column prop="send_integral_num" label="<?= \Yii::t('plugins/scan_code_pay', '赠送积分');?>" width="180">
                                            <template slot-scope="scope">
                                                <div v-if="scope.row.send_integral_type == 1">
                                                    {{scope.row.send_integral_num}}<?= \Yii::t('plugins/scan_code_pay', '积分');?>       </div>
                                                <div v-if="scope.row.send_integral_type == 2">
                                                    {{scope.row.send_integral_num}}%
                                                </div>
                                            </template>
                                        </el-table-column>
                                        <el-table-column prop="send_money" label="<?= \Yii::t('plugins/scan_code_pay', '赠送余额');?>"
                                                         width="180">
                                            <template slot-scope="scope">
                                                <div v-if="scope.row.send_money_type == 1">
                                                    {{scope.row.send_money}}<?= \Yii::t('plugins/scan_code_pay', '元');?>
                                                </div>
                                                <div v-if="scope.row.send_money_type == 2">
                                                    {{scope.row.send_money}}%
                                                </div>
                                            </template>
                                        </el-table-column>
                                        <el-table-column label="<?= \Yii::t('plugins/scan_code_pay', '赠送优惠券');?>">
                                            <template slot-scope="scope">
                                                <el-tag style="margin:5px;" v-for="(tag,i) in scope.row.coupons"
                                                        :key="tag.name" closable
                                                        @close="couponClose(scope.row,i)">
                                                    {{tag.send_num}}<?= \Yii::t('plugins/scan_code_pay', '张');?> | {{tag.name}}
                                                </el-tag>
                                            </template>
                                        </el-table-column>
                                        <el-table-column label="<?= \Yii::t('plugins/scan_code_pay', '赠送卡券');?>">
                                            <template slot-scope="scope">
                                        ...
                                        <el-table-column label="<?= \Yii::t('plugins/scan_code_pay', '操作');?>" width="250">
                                            <template slot-scope="scope">
                                                <el-button circle size="mini" type="text"
                                ...
                                <div style="background:#FFFFFF;width:100%;margin-top:20px">
                                    <div flex="dir:left cross:center" style="height:48px;">
                                        <div class="scan-rules"><?= \Yii::t('plugins/scan_code_pay', '优惠规则');?></div>
                                        ...
                                        <el-link @click="preferentialRules(index)" type="primary" :underline="false">
                                            +<?= \Yii::t('plugins/scan_code_pay', '新增优惠规则');?>
                                        </el-link>
                                        <div style="margin-left: 20px;font-size:13px;color:#f56c6c"><?= \Yii::t('plugins/scan_code_pay', '优惠规则无法叠加');?></div>
                                    </div>
                                    <el-table :data="group.preferential_rules"
                                              v-if="group.preferential_rules && group.preferential_rules.length"
                                              border>
                                        <el-table-column prop="consume_money" label="<?= \Yii::t('plugins/scan_code_pay', '单次消费金额');?>"
                                                         width="180"></el-table-column>
                                        <el-table-column prop="preferential_money" label="<?= \Yii::t('plugins/scan_code_pay', '优惠金额');?>"></el-table-column>
                                        <el-table-column prop="integral_deduction" label="<?= \Yii::t('plugins/scan_code_pay', '积分最多抵扣');?>">
                                            <template slot-scope="scope">
                                        ...
                                        <el-table-column label="<?= \Yii::t('plugins/scan_code_pay', '是否可以使用优惠券');?>">
                                            <template slot-scope="scope">
            ...
            <el-button class="button-item" type="primary" :loading="btnLoading" @click="onSubmit"><?= \Yii::t('plugins/scan_code_pay', '提交');?></el-button>
        </el-form>

        <!--用户组-->
        <el-dialog title="<?= \Yii::t('plugins/scan_code_pay', '新增用户组');?>" :visible.sync="userGroupVisible" width="50%" :close-on-click-modal="false">
            <el-form :model="userGroupForm" label-width="120px" :rules="userGroupRules" ref="userGroupForm"
                     @submit.native.prevent>
                <el-form-item label="<?= \Yii::t('plugins/scan_code_pay', '用户组名称');?>" prop="name">
                    <el-input size="small" v-model="userGroupForm.name" auto-complete="off"></el-input>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/scan_code_pay', '用户组成员');?>" prop="members">
            ...
            <div slot="footer" class="dialog-footer">
                <el-button size="small" @click="userGroupVisible = false"><?= \Yii::t('plugins/scan_code_pay', '取消');?></el-button>
                <el-button size="small" type="primary" @click="userGroupSubmit"><?= \Yii::t('plugins/scan_code_pay', '提交');?></el-button>
            </div>
        </el-dialog>

        <!-- 赠送规则 -->
        <el-dialog title="<?= \Yii::t('plugins/scan_code_pay', '新增赠送规则');?>" :visible.sync="sendRulesVisible" width="50%" :close-on-click-modal="false">

            <el-dialog title="<?= \Yii::t('plugins/scan_code_pay', '新增优惠券');?>" :visible.sync="couponVisible" width="30%" append-to-body>
                <div class="input-item">
                    <el-input @keyup.enter.native="couponSearch" size="small" placeholder="<?= \Yii::t('plugins/scan_code_pay', '请输入优惠券名称搜索');?>"
                ...
                <div slot="footer" class="dialog-footer">
                    <el-button size="small" @click="couponVisible = false"><?= \Yii::t('plugins/scan_code_pay', '取消');?></el-button>
                    <el-button size="small" type="primary" @click="senderCouponSubmit"><?= \Yii::t('plugins/scan_code_pay', '添加');?></el-button>
                </div>

            </el-dialog>

            <el-dialog title="<?= \Yii::t('plugins/scan_code_pay', '新增卡券');?>" :visible.sync="cardsVisible" width="30%" append-to-body>
                <div class="input-item">
                    <el-input @keyup.enter.native="cardsSearch" size="small" placeholder="<?= \Yii::t('plugins/scan_code_pay', '请输入卡券名称搜索');?>"
                ...
                <div slot="footer" class="dialog-footer">
                    <el-button size="small" @click="cardsVisible = false"><?= \Yii::t('plugins/scan_code_pay', '取消');?></el-button>
                    <el-button size="small" type="primary" @click="senderCardSubmit"><?= \Yii::t('plugins/scan_code_pay', '提交');?>
                    </el-button>
                </div>
            </el-dialog>
            ...
            <el-form :model="senderRulesForm" label-width="120px" :rules="senderRulesFormRules" ref="senderRulesForm"
                     @submit.native.prevent>
                <el-form-item label="<?= \Yii::t('plugins/scan_code_pay', '实际付款');?>" prop="consume_money">
                    <el-input size="small" type="number" v-model="senderRulesForm.consume_money">
                        <template slot="append"><?= \Yii::t('plugins/scan_code_pay', '元');?></template>
                    </el-input>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/scan_code_pay', '赠送积分');?>" prop="send_integral_num">
                    <el-input size="small" type="number" v-model="senderRulesForm.send_integral_num">
                        <template slot="append">
                            <el-radio-group v-model="senderRulesForm.send_integral_type">
                                <el-radio :label="1"><?= \Yii::t('plugins/scan_code_pay', '固定值');?></el-radio>
                                <el-radio :label="2"><?= \Yii::t('plugins/scan_code_pay', '百分比');?></el-radio>
                            </el-radio-group>
                        </template>
                    </el-input>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/scan_code_pay', '赠送余额');?>" prop="send_money">
                    <el-input size="small"  type="number" v-model="senderRulesForm.send_money">
                        <template slot="append">
                            <el-radio-group v-model="senderRulesForm.send_money_type">
                                <el-radio :label="1"><?= \Yii::t('plugins/scan_code_pay', '固定值');?></el-radio>
                                <el-radio :label="2"><?= \Yii::t('plugins/scan_code_pay', '百分比');?></el-radio>
                            </el-radio-group>
                        </template>
                ...
                <el-form-item label="<?= \Yii::t('plugins/scan_code_pay', '赠送优惠券');?>" prop="coupons">
                    ...
                    <el-button class="button-new-tag" size="small" @click="handleCoupon"><?= \Yii::t('plugins/scan_code_pay', '新增优惠券');?></el-button>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/scan_code_pay', '赠送卡券');?>" prop="cards">
                    <el-tag style="margin:5px"
                    ...
                    <el-button class="button-new-tag" size="small" @click="handleCard"><?= \Yii::t('plugins/scan_code_pay', '新增卡券');?></el-button>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button size="small" @click="sendRulesVisible = false"><?= \Yii::t('plugins/scan_code_pay', '取消');?></el-button>
                <el-button size="small" type="primary" @click="senderSubmit"><?= \Yii::t('plugins/scan_code_pay', '提交');?></el-button>
            </div>
        </el-dialog>

        <!-- 优惠规则 -->
        <el-dialog title="<?= \Yii::t('plugins/scan_code_pay', '新增优惠规则');?>" :visible.sync="preferentialVisible" width="50%" :close-on-click-modal="false">
            <el-form :model="preferentialForm" label-width="120px" :rules="preferentialFormRules"
                     ref="preferentialForm" @submit.native.prevent>
                <el-form-item label="<?= \Yii::t('plugins/scan_code_pay', '单次消费');?>" prop="consume_money">
                    <el-input size="small" type="number" v-model="preferentialForm.consume_money">
                        <template slot="append"><?= \Yii::t('plugins/scan_code_pay', '元');?></template>
                    </el-input>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/scan_code_pay', '优惠金额');?>" prop="preferential_money">
                    <el-input size="small" type="number" min="0" v-model="preferentialForm.preferential_money">
                        <template slot="append"><?= \Yii::t('plugins/scan_code_pay', '元');?></template>
                    </el-input>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/scan_code_pay', '积分抵扣');?>" prop="integral_deduction">
                    <el-input size="small" type="number" min="0" v-model="preferentialForm.integral_deduction">
                        <template slot="prepend"><?= \Yii::t('plugins/scan_code_pay', '最多抵扣');?></template>
                        <template slot="append">
                ...
                <el-form-item class="switch" label="<?= \Yii::t('plugins/scan_code_pay', '使用优惠券');?>" prop="is_coupon">
                    <el-switch v-model="preferentialForm.is_coupon" :active-value="1" :inactive-value="0"></el-switch>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button size="small" @click="preferentialVisible = false"><?= \Yii::t('plugins/scan_code_pay', '取消');?></el-button>
                <el-button size="small" type="primary" @click="preferentialSubmit"><?= \Yii::t('plugins/scan_code_pay', '提交');?></el-button>
            </div>
        </el-dialog>

        <!-- 赠送批量设置 -->
        <el-dialog title="<?= \Yii::t('plugins/scan_code_pay', '批量设置');?>" :visible.sync="sendBatchVisible" width="50%" :close-on-click-modal="false">
            <el-dialog title="<?= \Yii::t('plugins/scan_code_pay', '新增优惠券');?>" :visible.sync="batchCouponVisible" width="30%" append-to-body>
                <div class="input-item">
                    <el-input @keyup.enter.native="couponSearch" size="small" placeholder="<?= \Yii::t('plugins/scan_code_pay', '请输入优惠券名称搜索');?>"
                              v-model="couponKeyword"
                              clearable @clear='couponSearch'>
                ...
                <div slot="footer" class="dialog-footer">
                    <el-button size="small" @click="batchCouponVisible = false"><?= \Yii::t('plugins/scan_code_pay', '取消');?></el-button>
                    <el-button size="small" type="primary" @click="senderBatchCouponSubmit"><?= \Yii::t('plugins/scan_code_pay', '添加');?></el-button>
                </div>

            </el-dialog>

            <el-dialog title="<?= \Yii::t('plugins/scan_code_pay', '新增卡券');?>" :visible.sync="batchCardsVisible" width="30%" append-to-body>
                <div class="input-item">
                    <el-input @keyup.enter.native="cardsSearch" size="small" placeholder="<?= \Yii::t('plugins/scan_code_pay', '请输入卡券名称搜索');?>"
                              v-model="cardsKeyword"
                              clearable @clear='cardsSearch'>
                ...
                <div slot="footer" class="dialog-footer">
                    <el-button size="small" @click="batchCardsVisible = false"><?= \Yii::t('plugins/scan_code_pay', '取消');?></el-button>
                    <el-button size="small" type="primary" @click="senderBatchCardSubmit"><?= \Yii::t('plugins/scan_code_pay', '提交');?>
                    </el-button>
                </div>
            </el-dialog>
            ...
            <el-form :model="sendBatchForm" label-width="120px" :rules="sendBatchFormRules" ref="sendBatchForm"
                ...
                <el-form-item label="规则选择" prop="send_type">
                    <el-radio-group v-model="sendBatchForm.send_type">
                        <el-radio :label="1"><?= \Yii::t('plugins/scan_code_pay', '赠送积分');?></el-radio>
                        <el-radio :label="2"><?= \Yii::t('plugins/scan_code_pay', '赠送余额');?></el-radio>
                        <el-radio :label="3"><?= \Yii::t('plugins/scan_code_pay', '赠送优惠券');?></el-radio>
                        <el-radio :label="4"><?= \Yii::t('plugins/scan_code_pay', '赠送卡券');?></el-radio>
                    </el-radio-group>
                </el-form-item>
                ...
                <el-form-item v-if="sendBatchForm.send_type == 2" label="<?= \Yii::t('plugins/scan_code_pay', '赠送余额');?>" prop="send_money">
                    <el-input size="small" type="number" v-model="sendBatchForm.send_money">
                        <template slot="append">
                            <el-radio-group v-model="sendBatchForm.send_money_type">
                                <el-radio :label="1"><?= \Yii::t('plugins/scan_code_pay', '固定值');?></el-radio>
                                <el-radio :label="2"><?= \Yii::t('plugins/scan_code_pay', '百分比');?></el-radio>
                            </el-radio-group>
                        </template>
                ...
                <el-form-item v-if="sendBatchForm.send_type == 3" label="<?= \Yii::t('plugins/scan_code_pay', '赠送优惠券');?>" prop="coupons">
                    <el-tag style="margin:5px"
                    ...
                    <el-button class="button-new-tag" size="small" @click="batchCouponVisible = true"><?= \Yii::t('plugins/scan_code_pay', '新增优惠券');?></el-button>
                </el-form-item>
                <el-form-item v-if="sendBatchForm.send_type == 4" label="<?= \Yii::t('plugins/scan_code_pay', '赠送卡券');?>" prop="cards">
                    <el-tag style="margin:5px"
                    ...
                    <el-button class="button-new-tag" size="small" @click="batchCardsVisible = true"><?= \Yii::t('plugins/scan_code_pay', '新增卡券');?></el-button>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button size="small" @click="sendBatchVisible = false"><?= \Yii::t('plugins/scan_code_pay', '取消');?></el-button>
                <el-button size="small" type="primary" @click="sendBatchSubmit"><?= \Yii::t('plugins/scan_code_pay', '提交');?></el-button>
            </div>
        </el-dialog>
        <!-- 优惠批量设置 -->
        <el-dialog title="<?= \Yii::t('plugins/scan_code_pay', '批量设置');?>" :visible.sync="preferentialBatchVisible" width="50%" :close-on-click-modal="false">
            <el-form :model="preferentialBatchForm" label-width="120px" :rules="preferentialBatchFormRules"
                ...
                <el-form-item label="<?= \Yii::t('plugins/scan_code_pay', '规则选择');?>" prop="company">
                    <el-radio-group v-model="preferentialBatchForm.send_type">
                        <el-radio :label="1"><?= \Yii::t('plugins/scan_code_pay', '优惠金额');?></el-radio>
                        <el-radio :label="2"><?= \Yii::t('plugins/scan_code_pay', '积分抵扣');?></el-radio>
                        <el-radio :label="3"><?= \Yii::t('plugins/scan_code_pay', '使用优惠券');?></el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item v-if="preferentialBatchForm.send_type == 1" label="<?= \Yii::t('plugins/scan_code_pay', '优惠金额');?>" prop="preferential_money">
                    <el-input size="small" type="number" v-model="preferentialBatchForm.preferential_money">
                        <template slot="append"><?= \Yii::t('plugins/scan_code_pay', 'scan_code_pay_z46');?></template>
                    </el-input>
                </el-form-item>
                <el-form-item v-if="preferentialBatchForm.send_type == 2" label="<?= \Yii::t('plugins/scan_code_pay', '积分抵扣');?>" prop="integral_deduction">
                ...
                <el-form-item v-if="preferentialBatchForm.send_type == 3" class="switch" label="<?= \Yii::t('plugins/scan_code_pay', '使用优惠券');?>" prop="is_coupon">
                    <el-switch v-model="preferentialBatchForm.is_coupon" :active-value="1"
                               :inactive-value="0"></el-switch>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button size="small" @click="preferentialBatchVisible = false"><?= \Yii::t('plugins/scan_code_pay', '取消');?></el-button>
                <el-button size="small" type="primary" @click="preferentialBatchSubmit"><?= \Yii::t('plugins/scan_code_pay', '提交');?></el-button>
            </div>
        </el-dialog>
    </el-card>
</div>
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                rules: {
                    name: [
                        {required: true, message: '<?= \Yii::t('plugins/scan_code_pay', '活动名称不能为空');?>', trigger: 'blur'},
                    ],
                    time: [
                        {required: true, message: '<?= \Yii::t('plugins/scan_code_pay', '日期不能为空');?>', trigger: 'blur'},
                    ],
                    type: [
                        {required: true, message: '<?= \Yii::t('plugins/scan_code_pay', '买单规则不能为空');?>', trigger: 'blur'},
                    ],
                    send_type: [
                        {required: true, message: '<?= \Yii::t('plugins/scan_code_pay', '赠送规则不能为空');?>', trigger: 'blur'},
                    ],
                    status: [
                        {required: true, message: '<?= \Yii::t('plugins/scan_code_pay', '是否启用不能为空');?>', trigger: 'blur'},
                    ]
                },
                tags: [],
                otherIndex: '',
                ...
                userGroupRules: {
                    name: [
                        {required: true, message: '<?= \Yii::t('plugins/scan_code_pay', '用户组名称不能为空');?>', trigger: 'blur'},
                    ],
                    members: [
                        {required: true, message: '<?= \Yii::t('plugins/scan_code_pay', '用户组成员不能为空');?>', trigger: 'blur'},
                        {
                            validator(rule, value, callback, source, options) {
                                ...
                                if (sentry) {
                                    ...
                                    callback('<?= \Yii::t('plugins/scan_code_pay', '用户组成员不能为空');?>');
                                }
                            }
                        }
                    ],
                ...
                senderRulesFormRules: {
                    consume_money: [
                        {required: true, message: '<?= \Yii::t('plugins/scan_code_pay', '实际付款不能为空');?>', trigger: 'blur'},
                        {required: true, pattern: /^(0|[1-9]\d*)(\.\d{1,2})?$/, message: '<?= \Yii::t('plugins/scan_code_pay', '实际付款不合规范');?>'}
                    ],
                    send_integral_num: [
                        {required: true, message: '<?= \Yii::t('plugins/scan_code_pay', '赠送积分不能为空');?>', trigger: 'blur'},
                        {required: true, pattern: /^(0|[1-9][0-9]*)$/, message: '<?= \Yii::t('plugins/scan_code_pay', '赠送积分不合规范');?>'}
                    ],
                    send_money: [
                        {required: true, message: '<?= \Yii::t('plugins/scan_code_pay', '赠送余额不能为空');?>', trigger: 'blur'},
                        {required: true, pattern: /^(0|[1-9]\d*)(\.\d{1,2})?$/, message: '<?= \Yii::t('plugins/scan_code_pay', '赠送余额不合规范');?>'}
                    ],
                },
                /* 优惠规则 */
                ...
                preferentialFormRules: {
                    consume_money: [
                        {required: true, message: '<?= \Yii::t('plugins/scan_code_pay', '单次消费不能为空');?>', trigger: 'blur'},
                        {required: true, pattern: /^(0|[1-9]\d*)(\.\d{1,2})?$/, message: '<?= \Yii::t('plugins/scan_code_pay', '单次消费不合规范');?>'}
                    ],
                    preferential_money: [
                        {required: true, message: '<?= \Yii::t('plugins/scan_code_pay', '优惠金额不能为空');?>', trigger: 'blur'},
                        {required: true, pattern: /^(0|[1-9]\d*)(\.\d{1,2})?$/, message: '<?= \Yii::t('plugins/scan_code_pay', '优惠金额不合规范');?>'}
                    ],
                    integral_deduction: [
                        {required: true, message: '<?= \Yii::t('plugins/scan_code_pay', '积分抵扣不能为空');?>', trigger: 'blur'},
                        {required: true, pattern: /^(0|[1-9][0-9]*)$/, message: '<?= \Yii::t('plugins/scan_code_pay', '积分抵扣不合规范');?>'}
                    ],
                    is_coupon: [
                        {required: true, message: '<?= \Yii::t('plugins/scan_code_pay', '使用优惠券不能为空');?>', trigger: 'change'},
                    ]
                },
                /* 卡券 */
                cardsList: [],
        ...
        methods: {
            ...
            sendRules(index) {
                this.senderRulesForm = {
                    ...
                    consume_money: '',
                    send_integral_num: 0,
                    send_integral_type: 1,//1固定 2.百分比
                    send_money: 0,
                    send_money_type: 1,//1.固定 2.百分比
                        ...
                        send.forEach(v => {
                            switch (sendBatchForm.send_type) {
                                ...
                                case 2:
                                    ...
                                    v.send_money_type = sendBatchForm.send_money_type;
                                    break;
                                case 3:
