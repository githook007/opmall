...
<div id="app" v-cloak>
    <el-card v-loading="loading" style="border:0" shadow="never" body-style="background-color: #f3f3f3;padding: 0 0;">
        <el-form :model="form" label-width="10rem" ref="form" :rules="rule">
            <el-tabs v-model="activeName">
                <el-tab-pane label="<?= \Yii::t('plugins/scan_code_pay', '基础配置');?>" name="first">
                    <div class="form-body">
                        <el-form-item label="<?= \Yii::t('plugins/scan_code_pay', '是否开启当面付');?>" prop="is_scan_code_pay">
                            <el-switch v-model="form.is_scan_code_pay" :active-value="1"
                                       :inactive-value="0"></el-switch>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('plugins/scan_code_pay', '是否支持核销员');?>" prop="is_clerk">
                ...
                <el-tab-pane v-if="is_show_share && form.is_share" label="<?= \Yii::t('plugins/scan_code_pay', '分销设置');?>" name="second">
                    <div class="form-body">
                        <el-row>
                            <el-col :span="24">
                                <template v-if="form.is_share">
                                    <el-form-item label="<?= \Yii::t('plugins/scan_code_pay', '分销类型');?>" prop="share_type">
                                        <el-radio v-model="goods.share_type" :label="0"><?= \Yii::t('plugins/scan_code_pay', '固定金额');?></el-radio>
                                        <el-radio v-model="goods.share_type" :label="1"><?= \Yii::t('plugins/scan_code_pay', '百分比');?></el-radio>
                                    </el-form-item>
                ...
                <el-tab-pane label="<?= \Yii::t('plugins/scan_code_pay', '自定义海报');?>" name="three">
                    <div class="form-body" style="background:none;padding:0" flex="dir:left">
                        ...
                        <div class="poster-form-body">
                            <div flex="dir:left" style="margin-bottom: 15px">
                                <app-attachment :multiple="false" :max="1"
                                    ...
                                    <el-tooltip class="item"
                                                effect="dark"
                                                content="<?= \Yii::t('plugins/scan_code_pay', '建议尺寸');?>:750 * 1334"
                                                placement="top">
                                        <el-button size="mini">
                                            {{form.poster.bg_pic.url ? '<?= \Yii::t('plugins/scan_code_pay', '更换背景图');?>' : '<?= \Yii::t('plugins/scan_code_pay', '添加背景图');?>'}}
                                        </el-button>
                                    </el-tooltip>
                                </app-attachment>
                                <el-button v-if="form.poster.bg_pic.url" @click="removeBgPic()"
                                           ...
                                           type="danger"
                                           size="mini">
                                    <?= \Yii::t('plugins/scan_code_pay', '删除背景');?>
                                </el-button>
                            </div>
                            <div flex="wrap:wrap">
                            ...
                            <el-card shadow="never" class="box-card" style="width: 100%;">
                                <div slot="header">
                                    <span>{{title_desc}}<?= \Yii::t('plugins/scan_code_pay', '设置');?></span>
                                </div>
                                <div>
                                    <template if="component_key == 'qr_code'">
                                        <el-form-item label="<?= \Yii::t('plugins/scan_code_pay', '样式');?>">
                                            <el-radio v-model="form.poster.qr_code.type" :label="1"><?= \Yii::t('plugins/scan_code_pay', '圆形');?></el-radio>
                                            <el-radio v-model="form.poster.qr_code.type" :label="2"><?= \Yii::t('plugins/scan_code_pay', '方形');?></el-radio>
                                        </el-form-item>
                                        <el-form-item label="<?= \Yii::t('plugins/scan_code_pay', '大小');?>">
                                            <el-slider
                                                    :min=35
                                                    :max=300
                                        ...
                                        <el-form-item label="<?= \Yii::t('plugins/scan_code_pay', '上间距');?>">
                                            <el-slider
                                                    :min=0
                                        ...
                                        <el-form-item label="<?= \Yii::t('plugins/scan_code_pay', '左间距');?>">
                                            <el-slider
                                                    :min=0
                            ...
                            <el-card shadow="never" class="box-card" style="width: 100%;">
                                <div slot="header">
                                    <span><?= \Yii::t('plugins/scan_code_pay', '选择生成海报平台');?></span>
                                </div>
                            ...
                            <el-button :loading="downloading" style="margin-top: 20px;" size="small"
                                       @click="downloadPoster"><?= \Yii::t('plugins/scan_code_pay', '下载海报');?>
                            </el-button>
                            <span style='color: #ff4544;'><?= \Yii::t('plugins/scan_code_pay', '请先保存设置');?></span>
                        </div>
                    </div>
                </el-tab-pane>
                <el-button :loading="btnLoading" class="button-item" type="primary" @click="store('form')" size="small">
                    <?= \Yii::t('plugins/scan_code_pay', '保存');?>
                </el-button>
            </el-tabs>
        </el-form>
        <el-dialog center title="<?= \Yii::t('plugins/scan_code_pay', '海报');?>" :visible.sync="dialogVisible">
    ...
    <el-button v-if="posterImgUrl" type="primary" @click="down"><?= \Yii::t('plugins/scan_code_pay', '保存');?></el-button>
  </span>
        </el-dialog>
    </el-card>
</div>
    ...
    const app = new Vue({
                ...
                components: [
                    {
                        ...
                        title: '<?= \Yii::t('plugins/scan_code_pay', '二维码');?>',
                        is_active: true
                    },
                ],
                components_key: null,
        ...
        methods: {
            ...
            down() {
                ...
                alink.download = '<?= \Yii::t('plugins/scan_code_pay', '当面付海报');?>';
                alink.click();
            }
        },
        created() {
