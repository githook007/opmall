        ...
        <div slot="header" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item @click.native="returnBack">
                    <a style="color: #409EFF"><?= \Yii::t('plugins/advance', '商品管理');?></a></el-breadcrumb-item>
                <el-breadcrumb-item>{{edit ? '<?= \Yii::t('plugins/advance', '修改商品');?>' : '<?= \Yii::t('plugins/advance', '添加商品');?>'}}</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <app-goods
                sign="advance"
            ...
            <template slot="before_price">
                <el-form-item label="<?= \Yii::t('plugins/advance', '售价');?>" prop="price" >
                    <el-input type="number"
                              oninput="this.value = this.value.replace(/[^0-9\.]/, '');" :min="0"
                              v-model="form.price">
                        <template slot="append"><?= \Yii::t('plugins/advance', '元');?></template>
                    </el-input>
                </el-form-item>
            </template>

            <template slot="member_route_setting">
                 <span class="red"><?= \Yii::t('plugins/advance', '注：必须在');?>“
                    <el-button type="text" @click="$navigate({r: 'plugin/advance/mall/setting/index'}, true)"><?= \Yii::t('plugins/advance', '预售设置');?>=><?= \Yii::t('plugins/advance', '基本设置');?>=><?= \Yii::t('plugins/advance', '优惠叠加设置');?></el-button>
                    ”<?= \Yii::t('plugins/advance', '中开启，才能使用');?>
                </span>
            </template>

            <template slot="before_detail">
                <el-card shadow="never" style="margin-bottom: 24px">
                    <div slot="header"><?= \Yii::t('plugins/advance', '预售设置');?></div>
                    <el-col :span="24">
                        <el-form-item label="<?= \Yii::t('plugins/advance', '定金金额');?>">
                            <div flex="dir:left">
                                <el-row span="12">
                                    <el-input type="number" placeholder="<?= \Yii::t('plugins/advance', '请输入定金金额');?>"
                            ...
                            <div style="font-size: 12px;color: #909399;">
                                <?= \Yii::t('plugins/advance', '注：定金金额＜任何设置的售价或者会员价，该价格为不开启规格时的定金金额');?>
                            </div>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('plugins/advance', '定金膨胀金');?>">
                            <div flex="dir:left">
                                <el-row span="12">
                                    <el-input type="number" placeholder="<?= \Yii::t('plugins/advance', '请输入定金膨胀金');?>"
                                              :disabled="ruleForm.use_attr == 1"
                            ...
                            <div style="font-size: 12px;color: #909399;">
                                <?= \Yii::t('plugins/advance', '注：定金金额≤定金膨胀金＜任何设置的售价或者会员价，该价格为不开启规格时的定金膨胀金');?>
                            </div>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('plugins/advance', '定金与定金膨胀金');?>" v-if="ruleForm.use_attr == 1" :rules="[
                                          { type: 'number', message: '<?= \Yii::t('plugins/advance', '定金与定金膨胀金必须为数字值');?>'}
                                        ]">
                            <div flex="dir:left">
                                <div style="width: 70px;margin-right: 20px;color: #606266;">
                                    <?= \Yii::t('plugins/advance', '批量设置');?>
                                </div>
                                <el-row style="display: flex">
                                    <el-input placeholder="<?= \Yii::t('plugins/advance', '请输入内容');?>" v-model="input3" class="input-with-select">
                                        <el-select v-model="select" slot="prepend" style="width: 120px;"
                                                   placeholder="<?= \Yii::t('plugins/advance', '请选择');?>">
                                            <el-option label="<?= \Yii::t('plugins/advance', '定金');?>" value="1"></el-option>
                                            <el-option label="<?= \Yii::t('plugins/advance', '定金膨胀金');?>" value="2"></el-option>
                                        </el-select>
                                        <template slot="append">
                                            <el-button @click="batchSetting"><?= \Yii::t('plugins/advance', '确定');?></el-button>
                                        </template>
                                    </el-input>
                                </el-row>
                            </div>
                            <div>
                                <el-table
                                    ...
                                    <el-table-column
                                            label="<?= \Yii::t('plugins/advance', '定金');?>"
                                            prop="deposit"
                                            width="150">
                                        <template slot-scope="scope">
                                    ...
                                    <el-table-column
                                            prop="swell_deposit"
                                            width="150"
                                            label="<?= \Yii::t('plugins/advance', '定金膨胀金');?>">
                                        <template slot-scope="scope">
                        ...
                        <el-form-item label="<?= \Yii::t('plugins/advance', '定金支付时间');?>">
                            <el-date-picker
                                    ...
                                    range-separator="<?= \Yii::t('plugins/advance', '至');?>"
                                    start-placeholder="<?= \Yii::t('plugins/advance', '开始日期');?>"
                                    end-placeholder="<?= \Yii::t('plugins/advance', '结束日期');?>"
                                    :picker-options="pickerOptions">
                            </el-date-picker>
                            <div style="font-size: 12px;color: #909399;color: #ff4544;">
                                <?= \Yii::t('plugins/advance', '注：定金支付时间一旦保存无法修改，请认真填写');?>
                            </div>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('plugins/advance', '尾款支付时间');?>"
                                      :rules="[
                                          { required: true, message: '<?= \Yii::t('plugins/advance', '尾款支付时间不能为空');?>'},
                                          { type: 'number', message: '<?= \Yii::t('plugins/advance', '尾款支付时间必须为数字值');?>'}
                                        ]">
                            <div flex="dir:left">
                                <el-row span="24">
                                    <el-input :disabled="isBuyLimit" type="number" placeholder="<?= \Yii::t('plugins/advance', '请输入天数');?>"
                                              v-model.number="form.pay_limit">
                                        <template slot="prepend"><?= \Yii::t('plugins/advance', '尾款支付时间结束');?></template>
                                        <template slot="append"><?= \Yii::t('plugins/advance', '天内');?></template>
                                    </el-input>
                                </el-row>
                                <el-checkbox style="margin-left: 5px;" @change="itemChecked(1)" v-model="isBuyLimit"><?= \Yii::t('plugins/advance', '无限制');?>
                                </el-checkbox>
                            </div>
                        </el-form-item>
                    </el-col>
            ...
            <template slot="after_detail">
                <el-card shadow="never" style="margin-bottom: 24px">
                    <div slot="header"><?= \Yii::t('plugins/advance', '阶梯设置');?></div>
                    <el-row :span="24">
                        ...
                        <el-table
                                ...
                                style="width: 100%">
                            <el-table-column
                                    label="<?= \Yii::t('plugins/advance', '达标件数');?>"
                                    sortable
                                    width="546">
                                <template slot-scope="scope">
                                    <el-input type="age" v-model.number="scope.row.num" placeholder="<?= \Yii::t('plugins/advance', '请输入件数');?>"
                                              @change="change_ladder"></el-input>
                                </template>
                            </el-table-column>
                            <el-table-column
                                    label="<?= \Yii::t('plugins/advance', '折扣');?>（0.1~10）"
                                    width="520">
                                <template slot-scope="scope">
                                    <el-input type="number" v-model.number="scope.row.discount" min="0.1" max="10"
                                              placeholder="<?= \Yii::t('plugins/advance', '请输入折扣');?>" @input="e => scope.row.discount = inputMe(e)"
                                              @change="(e) => {change_ladder_dis(e)}"></el-input>
                                </template>
                            </el-table-column>
                            <el-table-column
                                    label="<?= \Yii::t('plugins/advance', '操作');?>"
                                    width="520">
                                <template slot-scope="scope">
                                    <el-button size="small" @click="destroyRules(scope.$index)" type="text" circle>
                                        <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/advance', '删除');?>" placement="top ">
                        ...
                        <el-button type="text" @click="addRules">
                            ...
                            <span style="color: #353535;font-size: 14px"><?= \Yii::t('plugins/advance', '添加新区间');?></span>
                        </el-button>
                    </el-row>
                </el-card>
            </template>
                ...
                <div v-if="previewData" flex="dir:top">
                    ...
                    <div class="goods" style="padding-top:12px">
                        ...
                        <div flex="dir:left" style="font-size:14px">
                            ...
                            <div class="share" flex="dir:top main:center cross:center">
                                ...
                                <div><?= \Yii::t('plugins/advance', '分享');?></div>
                            </div>
                        </div>
                    </div>
                    <div v-if="form.ladder_rules && form.ladder_rules.length"
                        ...
                        <div flex="dir:left cross:center">
                            <div flex="dir:top">
                                <div style="font-size:14px;"><?= \Yii::t('plugins/advance', '再抢购0件');?></div>
                                <div flex="dir:left" style="margin-top:5px">
                                    <div><?= \Yii::t('plugins/advance', '可享');?></div>
                                    <div style="font-size:20px;line-height:1;margin-top:-2px">
                                        {{form.ladder_rules[0].discount}}
                                    </div>
                                    <div><?= \Yii::t('plugins/advance', '折优惠');?></div>
                                </div>
                            </div>
                            <div style="margin-left:auto">
                                <div style="padding:5px 12px;border-radius:28px;border:1px solid #ffffff"><?= \Yii::t('plugins/advance', '邀请好友购买');?></div>
                            </div>
                        </div>
                        ...
                        <el-scrollbar style="width:100%;margin-top:16px;height:25px">
                            ...
                            <div flex="dir:left" v-if="form.ladder_rules.length === 1">
                                <div>0</div>
                                <div style="margin-left:auto">
                                    <?= \Yii::t('plugins/advance', '满');?>{{form.ladder_rules[0].num}}<?= \Yii::t('plugins/advance', '件，享');?>{{form.ladder_rules[0].discount}}<?= \Yii::t('plugins/advance', '折');?>
                                </div>
                            </div>
                            <div v-else flex="dir:left">
                                ...
                                <div id="plimit" flex="dir:left">
                                    <div style="position:relative;margin-left:120px"
                                         ...
                                         :key="index">
                                        <?= \Yii::t('plugins/advance', '满');?>{{item.num}}<?= \Yii::t('plugins/advance', '件，享');?>{{item.discount * 10}}<?= \Yii::t('plugins/advance', '折');?>
        ...
        data() {
            return {
                ...
                pickerOptions: {
                    ...
                    shortcuts: [{
                        text: '<?= \Yii::t('plugins/advance', '未来一周');?>',
                        onClick(picker) {
                            const end = new Date();
                        ...
                        text: '<?= \Yii::t('plugins/advance', '未来半月');?>',
                        onClick(picker) {
                            const end = new Date();
                            const start = new Date();
                        ...
                        text: '<?= \Yii::t('plugins/advance', '未来一个月');?>',
                        onClick(picker) {
                            const end = new Date();
                            const start = new Date();
                ...
                rules: {
                    open_date: [
                        {required: true, message: '<?= \Yii::t('plugins/advance', '请输入预付时间');?>', trigger: 'change',},
                    ],
                    pay_limit: [
                        {required: true, message: '<?= \Yii::t('plugins/advance', '请输入尾款支付天数');?>', trigger: 'change'},
                    ],
                    price: [
                        {required: true, message: '<?= \Yii::t('plugins/advance', '请输入售价');?>', trigger: 'change'},
                    ],
                },
                activeName: 'one',
                ruleForm: {
        ...
        methods: {
            handlePreview(e) {
                ...
                if (max > min && min >= 0) {
                    ...
                    actualPrice = "<?= \Yii::t('plugins/advance', '免费');?>";
                    type = '';
                }
                ...
                let title_b = '';
                {
                    ...
                    title_a = "<?= \Yii::t('plugins/advance', '定金');?>" + func(deposit) + "<?= \Yii::t('plugins/advance', '抵扣');?>" + func(swell_deposit);
                }
                {
                    let date = "<?= \Yii::t('plugins/advance', '无限制');?>";
                    ...
                    title_b = "<?= \Yii::t('plugins/advance', '支付尾款时间');?>：" + start + '~' + date;
                }
            ...
            change_ladder() {
                ...
                this.form.ladder_rules = this.form.ladder_rules.reduce((item, next) => {
                    if (!obj[next.num]) {
                        ...
                        this.$message({
                            message: "<?= \Yii::t('plugins/advance', '达标件数不可重复设置相同件数');?>",
                            type: 'warning'
                        });
                    }
                    return item;
