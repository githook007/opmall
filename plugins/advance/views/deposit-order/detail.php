    ...
    <el-card shadow="never" v-loading="loading" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item @click.native="returnBack">
                    <a><?= \Yii::t('plugins/advance', '预售订单');?></a></el-breadcrumb-item>
                <el-breadcrumb-item><?= \Yii::t('plugins/advance', '订单详情');?></el-breadcrumb-item>
            </el-breadcrumb>
        </div>
            ...
            <div style="border:1px solid #e2e2e2; border-radius: 4px;height: 220px;">
               <div style="width:80%; height: 220px;padding-top: 65px;margin-left: 10%; margin-right: 10%;">
                   <el-steps :active="active" align-center>
                       <el-step title="<?= \Yii::t('plugins/advance', '已下单');?>" icon="el-icon-s-order" :description="order.created_at"></el-step>
                       <el-step :title="order.is_pay === '0' ? '<?= \Yii::t('plugins/advance', '未支付定金');?>' : '<?= \Yii::t('plugins/advance', '已支付定金');?>'" :description="order.is_pay === '1' ? order.pay_time : ''" icon="el-icon-upload"></el-step>
                       <el-step :title="order.is_pay === '0' ? '<?= \Yii::t('plugins/advance', '未结束');?>' : '<?= \Yii::t('plugins/advance', '已结束');?>'" :description="order.is_pay === '1' ? order.pay_time : ''" icon="el-icon-picture"></el-step>
                   </el-steps>
               </div>
            </div>
        </div>
        <div class="table-center">
            <div class="order-status">
                <h3><?= \Yii::t('plugins/advance', '订单状态');?></h3>
                <div class="text" style="margin: 26px 0;">
                    <span style="width: 56px; display: inline-block;margin-right: 10px;"><?= \Yii::t('plugins/advance', '订单号');?></span>
                    <span>{{order.advance_no}}</span>
                </div>
                <template v-if="order.order_no != '0'">
                    <div class="text" style="margin: 26px 0;">
                        <span style="width: 70px; display: inline-block;margin-right: 10px;"><?= \Yii::t('plugins/advance', '尾款订单号');?></span>
                        <span>{{order.order_no}}</span>
                    </div>
                </template>
                <div class="text" v-if="order.is_pay === '1'">
                    <span style="margin-right: 10px;"><?= \Yii::t('plugins/advance', '支付方式');?></span>
                    <span class="is_pay">{{order.pay_type === '1' ? '<?= \Yii::t('plugins/advance', '在线支付');?>' : order.pay_type === '2' ? '<?= \Yii::t('plugins/advance', '货到付款');?>' : order.pay_type === '3' ? '<?= \Yii::t('plugins/advance', '余额支付');?>' : ''}}</span>
                </div>
            </div>
            <div class="form-name">
                <h3><?= \Yii::t('plugins/advance', '表单名称');?></h3>
                <div>
                    <span style="margin-right: 10px;"><?= \Yii::t('plugins/advance', '商家备注');?>{{order.remark}}</span>
        ...
        <div class="table-footer">
            <div class="content">
                <el-table
                    ...
                    <el-table-column
                            prop="namePic"
                            label="<?= \Yii::t('plugins/advance', '商品标题');?>"
                            width="650">
                    ...
                    <el-table-column
                            prop="attr"
                            label="<?= \Yii::t('plugins/advance', '规格');?>"
                            align="center"
                            width="220">
                        <template slot-scope="scope">
                    ...
                    <el-table-column
                            ...
                            prop="price"
                            width="180"
                            label="<?= \Yii::t('plugins/advance', '单价');?>">
                        <template slot-scope="scope">
                    ...
                    <el-table-column
                            ...
                            align="center"
                            width="180"
                            label="<?= \Yii::t('plugins/advance', '数量');?>">
                    </el-table-column>
                    <el-table-column
                            prop="original_price"
                            align="center"
                            width="180"
                            label="<?= \Yii::t('plugins/advance', '原价');?>">
                        <template slot-scope="scope">
                ...
                <div style="float: right;margin-top: 26px;font-size: 15px;color: #606266;margin-right: 10px;">
                    <?= \Yii::t('plugins/advance', '商品定金');?>：￥{{order.deposit_num}}
                </div>
            </div>
        </div>
    </el-card>
...
<script>
    const app = new Vue({
        ...
        methods: {
            ...
            open() {
                this.$prompt('<?= \Yii::t('plugins/advance', '添加备注信息');?>：', '<?= \Yii::t('plugins/advance', '备注');?>', {
