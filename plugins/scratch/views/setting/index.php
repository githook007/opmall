...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <el-tabs v-model="activeName">
            <el-tab-pane label="<?= \Yii::t('plugins/scratch', '刮刮卡抽奖设置');?>" name="setting"></el-tab-pane>
            <el-tab-pane label="<?= \Yii::t('plugins/scratch', '背景图设置');?>" name="bg"></el-tab-pane>
        </el-tabs>
        <el-form v-loading="loading" :model="form" label-width="120px" ref="form" size="small"
            ...
            <template v-if="activeName === 'setting'">
                ...
                <div class="form-body">
                    <el-form-item label="<?= \Yii::t('plugins/scratch', '中奖概率');?>" prop="probability">
                        <el-input size="small" type="number" v-model="form.probability" autocomplete="off">
                            <template slot="prepend"><?= \Yii::t('plugins/scratch', '万分之');?></template>
                        </el-input>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('plugins/scratch', '抽奖时间');?>" prop="time">
                        <el-date-picker v-model="form.time" unlink-panels type="datetimerange" size="small"
                                        value-format="yyyy-MM-dd HH:mm:ss" range-separator="<?= \Yii::t('plugins/scratch', '至');?>" start-placeholder="<?= \Yii::t('plugins/scratch', '开始日期');?>"
                                        end-placeholder="<?= \Yii::t('plugins/scratch', '结束日期');?>"></el-date-picker>
                        </el-date-picker>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('plugins/scratch', '抽奖规则');?>" prop="type">
                        <el-radio v-model="form.type" :label="1"><?= \Yii::t('plugins/scratch', '一天');?>{{ form.oppty }}<?= \Yii::t('plugins/scratch', '次');?></el-radio>
                        <el-radio v-model="form.type" :label="2"><?= \Yii::t('plugins/scratch', '一人');?>{{ form.oppty }}<?= \Yii::t('plugins/scratch', '次');?></el-radio>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('plugins/scratch', '抽奖次数');?>" prop="oppty">
                        <el-input size="small" type="number" v-model="form.oppty" autocomplete="off"></el-input>
                    </el-form-item>
                    <el-form-item v-if="false" label="<?= \Yii::t('plugins/scratch', '小程序标题');?>" prop="title">
                        <el-input size="small" v-model="form.title" autocomplete="off"></el-input>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('plugins/scratch', '消耗积分');?>" prop="deplete_integral_num">
                        <el-input size="small" v-model="form.deplete_integral_num" autocomplete="off"></el-input>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('plugins/scratch', '规则说明');?>" prop="rule">
                        <el-input rows="15" type="textarea" v-model="form.rule" autocomplete="off"></el-input>
                    </el-form-item>
                </div>
                <el-button type="primary" class="button-item" size="small" :loading=btnLoading @click="onSubmit"><?= \Yii::t('plugins/scratch', '保存');?>
                </el-button>
            </template>
                ...
                <div style="display: flex;">
                    <div class="mobile-box">
                        <div class="head-bar" flex="main:center cross:center">
                            <div><?= \Yii::t('plugins/scratch', '刮刮卡');?></div>
                        </div>
                        <div class="show-box scratch-bg" :style="{backgroundImage: 'url(' + form.bg_pic + ')'}">
                            <div style="position: relative">
                                <div class="bg-rule"><?= \Yii::t('plugins/scratch', '规则');?></div>
                                <div class="bg-rule"><?= \Yii::t('plugins/scratch', '分享');?></div>
                            </div>
                            <div style="line-height: 28px;font-size: 14px;margin-top: 190px;text-align: center;">
                                <div style="color: #FFFFFF;padding: 0 15px;display: inline-block;background: rgba(0,0,0,0.3);border-radius: 14px">
                                    <?= \Yii::t('plugins/scratch', '您还有');?><span style="color: #ffb92a">3</span><?= \Yii::t('plugins/scratch', '次机会');?>
                                </div>
                            </div>
                            <el-image
                            ...
                            <div flex="dir:left cross:center main:center"
                                ...
                                <div flex="dir:left cross:center">
                                    ...
                                    <span style="margin-left: 10px"><?= \Yii::t('plugins/scratch', '回到首页');?></span>
                                </div>
                                <div flex="dir:left cross:center" style="margin-left: 90px">
                                    <span style="margin-right: 10px"><?= \Yii::t('plugins/scratch', '我的中奖记录');?></span>
                    ...
                    <div style="width: 100%;">
                        <div style="background: #FFFFFF;padding: 24px">
                            <el-form-item label="<?= \Yii::t('plugins/scratch', '背景图');?>" prop="bg_pic">
                                <app-attachment style="margin-bottom:10px" :multiple="false" :max="1"
                                                @selected="selectBgPic">
                                    <el-tooltip effect="dark"
                                                content="<?= \Yii::t('plugins/scratch', '建议尺寸');?>:750 * 1334"
                                                placement="top">
                                        <el-button size="mini"><?= \Yii::t('plugins/scratch', '选择图标');?></el-button>
                                    </el-tooltip>
                                </app-attachment>
                                ...
                                <el-button size="mini" @click="resetImg('bg_pic')"
                                           style="position: absolute;top: 3px;left: 90px" type="primary"><?= \Yii::t('plugins/scratch', '恢复默认');?>
                                </el-button>
                            </el-form-item>
                        </div>
                        <el-button style="margin-top: 20px"
                                   ...
                                   size="small"
                                   :loading=btnLoading
                                   @click="onSubmit"><?= \Yii::t('plugins/scratch', '保存');?>
                        </el-button>
                    </div>
                </div>
            </template>
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                FormRules: {
                    probability: [
                        {required: true, message: '<?= \Yii::t('plugins/scratch', '概率不能为空');?>', trigger: 'blur'},
                    ],
                    oppty: [
                        {required: true, message: '<?= \Yii::t('plugins/scratch', '抽奖次数不能为空');?>', trigger: 'blur'},
                    ],
                    type: [
                        {required: true},
                    ],
                    time: [
                        {required: true, message: '<?= \Yii::t('plugins/scratch', '抽奖时间不能为空');?>', trigger: 'blur'},
                        {
                            validator(rule, value, callback, source, options) {
                                ...
                                status ? callback("<?= \Yii::t('plugins/scratch', '时间溢出');?>") : callback();
                            }
                        }
                    ],
                },
