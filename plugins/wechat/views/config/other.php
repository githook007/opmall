...
<div id="app" v-cloak>
    <el-card shadow="never" v-loading="cardLoading" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <span><?= \Yii::t('plugins/wechat', '其他设置');?></span>
        </div>
        <el-form @submit.native.prevent :model="form" label-width="150px" ref="form">
            <div class="title">
                <span><?= \Yii::t('plugins/wechat', '强制关注公众号');?></span>
            </div>
            <div class="form-body">
                <el-form-item class="switch" label="<?= \Yii::t('plugins/wechat', '选择页面');?>" prop="page">
        ...
        <el-button class='button-item' :loading="btnLoading" type="primary" @click="store" size="small"><?= \Yii::t('plugins/wechat', '保存');?></el-button>
    </el-card>
</div>
<script>
    new Vue({
