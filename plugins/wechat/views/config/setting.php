...
<div id="app" v-cloak>
    <el-card shadow="never" v-loading="cardLoading" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <span><?= \Yii::t('plugins/wechat', '基础设置');?></span>
        </div>
        ...
        <div class="form_box">
            <el-row>
                <el-form :model="ruleForm" :rules="rules" size="small" ref="ruleForm" label-width="150px">
                    <el-col :span="12">
                        <el-form-item v-if="has_third_permission" label="<?= \Yii::t('plugins/wechat', '是否使用第三方授权');?>" prop="is_third">
                            <el-radio-group @change="qrLoading=false" :disabled="third && third.id > 0" v-model="is_third">
                                <el-radio :label="0"><?= \Yii::t('plugins/wechat', '否');?></el-radio>
                                <el-radio :label="1"><?= \Yii::t('plugins/wechat', '是');?></el-radio>
                            </el-radio-group>
                        </el-form-item>
                        <el-form-item v-if="is_third == 0" label="<?= \Yii::t('plugins/wechat', '公众号名称');?>" prop="name">
                            <el-input v-model="ruleForm.name"></el-input>
                        </el-form-item>
                        <el-form-item v-if="is_third == 0" label="<?= \Yii::t('plugins/wechat', '公众号头像');?>" prop="logo" class="required-icon">
                            <app-attachment :multiple="false" :max="1" @selected="logoUrl">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/wechat', '建议尺寸');?>:100*100" placement="top">
                                    <el-button size="mini"><?= \Yii::t('plugins/wechat', '选择文件');?></el-button>
                                </el-tooltip>
                            </app-attachment>
                        ...
                        <el-form-item v-if="is_third == 0" label="<?= \Yii::t('plugins/wechat', '关注公众号二维码');?>" prop="qrcode" class="required-icon">
                            <app-attachment :multiple="false" :max="1" @selected="qrcodeUrl">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/wechat', '建议尺寸');?>:280*280" placement="top">
                                    <el-button size="mini"><?= \Yii::t('plugins/wechat', '选择文件');?></el-button>
                                </el-tooltip>
                            </app-attachment>
                            <div style="margin: 10px 0;position: relative;width: 80px;">
                        ...
                        <div v-if="is_third == 0">
                            <div class="build-qr-item" v-if="qrcode">
                                ...
                                <div flex="cross:center main:center" class="build-qr-item-info">
                                    <div><?= \Yii::t('plugins/wechat', '链接');?></div>
                                    <div id="path" style="max-width: 40%;margin: 0 20px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap">{{path}}</div>
                                    <el-tooltip style="cursor:pointer" effect="dark" content="<?= \Yii::t('plugins/wechat', '复制链接');?>" placement="top">
                                ...
                                <div class="build-update-version">
                                    <div style="margin-bottom: 5px;"><?= \Yii::t('plugins/wechat', '当前版本');?> {{version}}</div>
                                    <div><?= \Yii::t('plugins/wechat', '最新版本');?> {{last_version}}</div>
                                </div>
                            </div>
                        </div>
                        <div v-if="is_third == 1 && !third" class="choose" flex="dir:left cross:center">
                            ...
                            <div><?= \Yii::t('plugins/wechat', '选择已有公众号');?></div>
                            <el-button class='button-item' type="primary" @click="auth" size="small"><?= \Yii::t('plugins/wechat', '去绑定');?></el-button>
                        </div>
                        <div v-if="is_third == 1 && third">
                            <div v-if="!qrcode" class="choose" flex="dir:left cross:center">
                                ...
                                <div><?= \Yii::t('plugins/wechat', '公众号绑定成功');?></div>
                                <el-button class='button-item issue' :loading="qrLoading" type="primary" @click="issue" size="small"><?= \Yii::t('plugins/wechat', '生成公众号版本');?></el-button>
                            </div>
                            <!-- 公众号信息 -->
                            <div class="build-qr-item" v-if="qrcode">
                                <img class="build-qr-item-bg" src="<?= $iconBaseUrl ?>qr-bg.png" alt="">
        ...
        <el-button v-if="activeName != 'first' || is_third == 0" class='button-item' :loading="btnLoading" type="primary" @click="store('ruleForm')" size="small"><?= \Yii::t('plugins/wechat', '发布新版本');?></el-button>
        <el-button v-else class='button-item' :loading="qrLoading" type="primary" @click="getbuildQr" size="small"><?= \Yii::t('plugins/wechat', '发布新版本');?></el-button>
    </el-card>
</div>
...
<script>
    ...
    clipboard.on('success', function (e) {
        self.ELEMENT.Message.success('<?= \Yii::t('plugins/wechat', '复制成功');?>');
        e.clearSelection();
    });
    clipboard.on('error', function (e) {
        self.ELEMENT.Message.success('<?= \Yii::t('plugins/wechat', '复制失败');?>');
    });
    const app = new Vue({
        el: '#app',
        data() {
            return {
                ...
                rules: {
                    name: [
                        {required: true, message: '<?= \Yii::t('plugins/wechat', '请输入公众号名称');?>', trigger: 'change'},
                    ]
                },
                btnLoading: false,
                cardLoading: false,
        ...
        methods: {
            ...
            auth() {
                ...
                this.$confirm('<?= \Yii::t('plugins/wechat', '请在新窗口中完成微信公众号授权');?>', '<?= \Yii::t('plugins/wechat', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('plugins/wechat', '已成功授权');?>',
                    cancelButtonText: '<?= \Yii::t('plugins/wechat', '授权失败');?>',
                    type: 'warning'
                }).then(() => {
                    this.$message({
                        type: 'success',
                        message: '<?= \Yii::t('plugins/wechat', '已完成授权');?>'
                    });
                    this.getDetail();
                }).catch(() => {
                });
            ...
            store(formName) {
                ...
                if(!self.ruleForm.logo) {
                    this.$message.error('<?= \Yii::t('plugins/wechat', '请选择公众号头像');?>');
                    return false;
                }
                if(!self.ruleForm.qrcode) {
                    this.$message.error('<?= \Yii::t('plugins/wechat', '请选择关注公众号二维码');?>');
                    return false;
                }
