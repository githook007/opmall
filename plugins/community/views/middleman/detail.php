...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <span style="color: #3399ff;cursor: pointer;" @click="routeGo('plugin/community/mall/middleman/index')"><?= \Yii::t('plugins/community', '团长管理');?></span>
            <span style="margin: 0 5px;">/</span>
            <span><?= \Yii::t('plugins/community', '团长详情');?></span>
        </div>
        <div class="table-body">
                    ...
                    <el-table-column prop="name">
                        <template slot="header" slot-scope="scope">
                            <div style="height: 28px;">
                                ...
                                <div style="height: 20px;line-height: 20px;color: #6fb1ff"><?= \Yii::t('plugins/community', '团长');?></div>
                            </div>
                        </template>
                    ...
                    <el-table-column prop="children_count">
                        <template slot="header" slot-scope="scope">
                            <div style="height: 28px;">
                                ...
                                <div style="height: 20px;line-height: 20px;color: #6fb1ff"><?= \Yii::t('plugins/community', '团员人数');?></div>
                            </div>
                        </template>
                        <template slot-scope="prop">
                ...
                <div class="input-item">
                    <el-input @keyup.enter.native="toSearch" size="small" placeholder="<?= \Yii::t('plugins/community', '请输入搜索内容');?>" v-model="search.keyword" clearable @clear="toSearch">
                        <el-select size="small" v-model="search.type" slot="prepend" class="select">
                            <el-option key="1" label="<?= \Yii::t('plugins/community', '团员昵称');?>" value="name"></el-option>
                            <el-option key="4" label="<?= \Yii::t('plugins/community', '团员ID');?>" value="id"></el-option>
                        </el-select>
            ...
            <div style="width: 780px">
                <el-table :header-cell-style="{backgroundColor: '#F5F7FA'}" :data="list" border v-loading="loading" style="margin-bottom: 25px;">
                    ...
                    <el-table-column label="<?= \Yii::t('plugins/community', '基本信息');?>" prop="nickname" width="450">
                        <template slot-scope="scope">
                    ...
                    <el-table-column label="<?= \Yii::t('plugins/community', '操作');?>">
                        <template slot-scope="scope">
                            <el-button @click="release(scope.row.id)" type="text" circle size="mini">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/community', '解除关系');?>" placement="top">
...
<script>
    const app = new Vue({
        ...
        methods: {
            ...
            release(id) {
                let that = this;
                that.$confirm("<?= \Yii::t('plugins/community', '确认解除关系');?>", "<?= \Yii::t('plugins/community', '提示');?>", {
