    ...
    <el-card style="border:0" shadow="never" body-style="background-color: #f3f3f3;padding: 0 0;" v-loading="loading">
        <div class="text item" style="width:100%">
                            ...
                            <el-form-item class="url" label="<?= \Yii::t('plugins/community', '小程序路径');?>">
                                ...
                                <el-popover placement="bottom" width="210" trigger="click">
                                    <div flex="dir:top cross:center main:center">
                                        ...
                                        <el-button type="text" @click="downloadImg"><?= \Yii::t('plugins/community', '下载小程序码');?></el-button>
                                    </div>
                                    <el-button slot="reference" type="text"><?= \Yii::t('plugins/community', '在小程序上推广');?></el-button>
                                </el-popover>
                            </el-form-item>
                            <el-form-item label="<?= \Yii::t('plugins/community', '页面标题');?>">
            ...
            <div class="bottom-div" flex="cross:center">
                <el-button class="button-item" type="primary" :loading="btnLoading" @click="onSubmit"><?= \Yii::t('plugins/community', '提交');?></el-button>
                </el-button>
            </div>
        </div>
    </el-card>
...
<script>
    ...
    clipboard.on('success', function (e) {
        self.ELEMENT.Message.success("<?= \Yii::t('plugins/community', '复制成功');?>");
        e.clearSelection();
    });
    clipboard.on('error', function (e) {
        self.ELEMENT.Message.success("<?= \Yii::t('plugins/community', '复制失败，请手动复制');?>");
    });
    const app = new Vue({
        el: '#app',
        ...
        data() {
            return {
                ...
                form: {
                    recruit_title: "<?= \Yii::t('plugins/community', '招募令');?>",
                    recruit_content:''
                },
            };
        },
        ...
        methods: {
            downloadImg() {
                ...
                alink.download = this.form.recruit_title ? this.form.recruit_title : "<?= \Yii::t('plugins/community', '招募令');?>";
                alink.click();
            },
            qrcode() {
                let para = {
        	...
        	onSubmit() {
                if(!this.form.recruit_title) {
                    this.$message.error("<?= \Yii::t('plugins/community', '页面标题不能为空');?>");
                    return false
                }
