...
<div id="app" v-cloak>
    <el-card v-loading="listLoading"  shadow="never"  class="activity-list" style="border:0"  body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <el-dialog title="<?= \Yii::t('plugins/community', '选择地区');?>" :visible.sync="dialogVisible" width="50%">
        ...
        <el-dialog :visible.sync="goodsVisible" width="60%" class="activity-visible" :before-close="handleClose">
            <div slot="title">
                <span class="goods-visible-title"><?= \Yii::t('plugins/community', '选择商品');?></span>
            </div>
            <div style="margin-bottom: 27px;">
                <el-input v-model="keyword"  @change="searchGood" @clear="searchGood" placeholder="<?= \Yii::t('plugins/community', '根据名称搜索');?>" clearable autocomplete="off">
                    <template slot="append">
                        <el-button @click="searchGood"><?= \Yii::t('plugins/community', '搜索');?></el-button>
                    </template>
                </el-input>
            </div>
            <el-table
                ...
                <el-table-column
                        prop="name"
                        label="<?= \Yii::t('plugins/community', '名称');?>">
            ...
            <div slot="footer" class="dialog-footer" flex="main:justify">
                ...
                <el-button type="primary" size="small" @click="addGoods()"><?= \Yii::t('plugins/community', '确定');?></el-button>
            </div>
        </el-dialog>
        <!--库存弹框-->
        <el-dialog title="<?= \Yii::t('plugins/community', '编辑库存');?>" :visible.sync="stockVisible" :before-close="stockClose">
            <div class="set-inventory">
                <div class="input" flex="main:center cross:center">
                    <div style="margin-right: 20px;">
                        <span><?= \Yii::t('plugins/community', '批量设置');?></span>
                    </div>
                    <div>
                        <el-input size="small" v-model="stock" @change="setStock">
                            <template slot="prepend">
                                <?= \Yii::t('plugins/community', '库存');?>
                            </template>
                            <template slot="append">
                                <el-button @click="setStock">
                                    <?= \Yii::t('plugins/community', '确定');?>
                                </el-button>
                            </template>
                        </el-input>
                    </div>
            ...
            <el-table
                ...
                <el-table-column
                        ...
                        label="<?= \Yii::t('plugins/community', '库存');?>">
                    <template slot-scope="scope">
            ...
            <div slot="footer" class="dialog-footer">
                <el-button  @click="stockVisible = false"><?= \Yii::t('plugins/community', '取消');?></el-button>
                <el-button type="primary" @click="sureStock"><?= \Yii::t('plugins/community', '确定');?></el-button>
            </div>
        </el-dialog>

        <div slot="header">
            <span style="color: #3399ff;cursor: pointer;" @click="routeGo('plugin/community/mall/activity/index')"><?= \Yii::t('plugins/community', '社区团购');?></span>
            <span style="margin: 0 5px;">/</span>
            <span v-if="!activity_id"><?= \Yii::t('plugins/community', '新建活动');?></span>
            <span v-else><?= \Yii::t('plugins/community', '编辑活动');?></span>
        </div>
        <div class="activity-body">
            <el-form :model="form" size="small" label-width="116px">
                <el-tabs v-model="activeName">
                    <el-tab-pane label="<?= \Yii::t('plugins/community', '活动信息');?>" name="second">
                        <div class="item">
                            <div class="item-title"><?= \Yii::t('plugins/community', '活动设置');?></div>
                            <div class="item-body">
                                <el-form-item  label="<?= \Yii::t('plugins/community', '团购名称');?>" required>
                                    <div style="width: 350px">
                                        <el-input v-model="form.title" show-word-limit maxlength="10"></el-input>
                                    </div>
                                </el-form-item>
                                <el-form-item label="<?= \Yii::t('plugins/community', '活动时间');?>" required>
                                    <el-date-picker
                                ...
                                <el-form-item label="<?= \Yii::t('plugins/community', '活动地区');?>" required>
                                    <el-radio :disabled="form.activity_status == 1" v-model="form.is_area_limit" :label="0"><?= \Yii::t('plugins/community', '全部地区');?></el-radio>
                                    <el-radio :disabled="form.activity_status == 1" v-model="form.is_area_limit" :label="1"><?= \Yii::t('plugins/community', '指定地区');?></el-radio>
                                    <div v-if="form.is_area_limit == 1">
                                        <el-button type="text" size="mini" :disabled="form.activity_status == 1" circle @click.native="openDistrict"><?= \Yii::t('plugins/community', '选择地区');?></el-button>
                                    </div>
                                ...
                                <el-form-item label="<?= \Yii::t('plugins/community', '最低成团');?>" required>
                                    <el-radio v-model="form.condition" v-if="form.condition == 1 || form.condition == 0" :label="1"><?= \Yii::t('plugins/community', '开启');?></el-radio>
                                    <el-radio v-model="form.condition" v-if="form.condition == 2" :label="2"><?= \Yii::t('plugins/community', '开启');?></el-radio>
                                    <el-radio v-model="form.condition" :label="0"><?= \Yii::t('plugins/community', '关闭');?></el-radio>
                                    <div style="color: #999;font-size: 12px;margin-top: -5px;"><?= \Yii::t('plugins/community', '开启后，未达到成团条件的活动订单，将自动退款');?></div>
                                    <div v-if="form.condition > 0" flex="dir:left cross:center" style="margin-bottom: 15px">
                                        <el-radio v-model="form.condition" :label="1"><?= \Yii::t('plugins/community', '参团人数，至少满');?></el-radio>
                                        <div>
                                            <el-input oninput="this.value = this.value.replace(/[^0-9]/g, '');" type="number" min="1" style="width: 200px;" v-model.number="people_num">
                                                <template slot="append"><?= \Yii::t('plugins/community', '人');?></template>
                                            </el-input>
                                        </div>
                                    </div>
                                    <div v-if="form.condition > 0" flex="dir:left cross:center">
                                        <el-radio v-model="form.condition" :label="2"><?= \Yii::t('plugins/community', '商品件数，至少满');?></el-radio>
                                        <div>
                                            <el-input oninput="this.value = this.value.replace(/[^0-9]/g, '');" type="number" min="1" style="width: 200px;" v-model.number="goods_num">
                                                <template slot="append"><?= \Yii::t('plugins/community', '件');?></template>
                                            </el-input>
                                        </div>
                                    </div>
                                </el-form-item>
                                <el-form-item v-if="form.activity_status != 1 || (form.activity_status == 1 && form.full_price.length > 0)" label="<?= \Yii::t('plugins/community', '优惠方式');?>">
                                    <div class="rlue-list" v-if="form.full_price.length > 0">
                                        <div v-for="(item,index) in form.full_price" flex="dir:left cross:center">
                                            <div class="item-input" flex="dir:left cross:center">
                                                <span style="margin: 0 8px;"><?= \Yii::t('plugins/community', '满');?></span>
                                                <el-input :disabled="form.activity_status == 1" :min="0" style="width:148px" type="number" v-model="item.full_price"></el-input>
                                                <span style="margin: 0 8px;"><?= \Yii::t('plugins/community', '元，减');?></span>
                                                ...
                                                <span style="margin: 0 8px;"><?= \Yii::t('plugins/community', '元');?></span>
                                            </div>
                                            <div>
                                                <el-button v-if="form.activity_status != 1" type="text" size="mini" circle style="margin: 0 40px;" @click.native="deleteRlue(index)">
                                                    <el-tooltip effect="dark" content="<?= \Yii::t('plugins/community', '删除');?>" placement="top">
                                    ...
                                    <div v-if="form.full_price.length < 3">
                                        <el-button v-if="form.activity_status != 1" type="primary" class="save" style="margin-top: -10px" @click="addRlue"><?= \Yii::t('plugins/community', '新增一条优惠');?></el-button>
                                    </div>
                                </el-form-item>
                            </div>
                        </div>
                    </el-tab-pane>
                    <el-tab-pane label="<?= \Yii::t('plugins/community', '商品信息');?>" name="first">
                        <div class="item">
                            <div class="item-title"><?= \Yii::t('plugins/community', '商品信息');?></div>
                            <div class="item-body">
                                <el-form-item label="<?= \Yii::t('plugins/community', '选择商品');?>" required>
                                    <div style="margin-bottom: 15px;">
                                        <el-button @click="getGood"><?= \Yii::t('plugins/community', '选择商品');?></el-button>
                                        <el-button v-if="activityGoods.length == 0" @click="findOther"><?= \Yii::t('plugins/community', '导入其他活动商品');?></el-button>
                                        <p class="add-goods" v-if="activityGoods.length === 0" @click="routeGo('mall/goods/edit')"><?= \Yii::t('plugins/community', '商城还未添加商品？点击前往');?></p>
                                    </div>
                                    <div style="width: 960px">
                                        <el-table
                                            ...
                                            <el-table-column
                                                    prop="name"
                                                    label="<?= \Yii::t('plugins/community', '商品');?>">
                                                <template slot-scope="scope">
                                                    <div flex="first">
                                                        <div style="padding-right: 10px;">
                                                            <app-image mode="aspectFill" :src="scope.row.goodsWarehouse.cover_pic"></app-image>
                                                        ...
                                                        <div flex="dir:top">
                                                            ...
                                                            <div v-if="scope.row.use_attr == 1" style="color: #999;font-size: 12px"><?= \Yii::t('plugins/community', '已选');?>{{scope.row.attr.length}}<?= \Yii::t('plugins/community', '个规格');?></div>
                                                        </div>
                                                    </div>
                                                </template>
                                            </el-table-column>
                                            <el-table-column
                                                    prop="name"
                                                    width="80"
                                                    label="<?= \Yii::t('plugins/community', '活动库存');?>">
                                                <template slot-scope="scope">
                                            ...
                                            <el-table-column
                                                    width="130"
                                                    label="<?= \Yii::t('plugins/community', '排序');?>">
                                                <template slot-scope="scope">
                                            ...
                                            <el-table-column
                                                    prop="name"
                                                    width="150"
                                                    label="<?= \Yii::t('plugins/community', '买家购买价格');?>">
                                                <template slot-scope="scope">
                                                    <div flex="dir:left cross:center">
                                            ...
                                            <el-table-column
                                                    prop="name"
                                                    width="150"
                                                    label="<?= \Yii::t('plugins/community', '团长供货价格');?>">
                                                <template slot-scope="scope">
                                                    <div flex="dir:left cross:center">
                                                        <div flex="dir:left cross:center" v-if="scope.row.use_attr == 1">
                                            ...
                                            <el-table-column
                                                    prop="name"
                                                    width="80"
                                                    label="<?= \Yii::t('plugins/community', '是否上架');?>">
                                                <template slot-scope="scope">
        ...
        <el-button :loading="saveLoading" type="primary" size="small" class="save"  @click="saveEdit"><?= \Yii::t('plugins/community', '保存');?></el-button>
    </el-card>
    <el-dialog class="dialog-choose" title="<?= \Yii::t('plugins/community', '选择其他团购活动');?>" :visible.sync="listDialog" width="30%">
        <div class="dialog-goods-list">
            <div style="margin-bottom: 27px;">
                <el-input size="small" v-model="activityKeyword"  @change="searchActivity" @clear="searchActivity" placeholder="<?= \Yii::t('plugins/community', '根据名称搜索');?>" clearable autocomplete="off">
                    <template slot="append">
                        <el-button @click="searchActivity"><?= \Yii::t('plugins/community', '搜索');?></el-button>
                    </template>
                </el-input>
            </div>
            <el-table v-loading="listLoading" :data="list" style="width: 100%">
                ...
                <el-table-column label="<?= \Yii::t('plugins/community', '名称');?>">
                    <template slot-scope="props">
                        <div flex="dir:left cross:center">
        ...
        <span slot="footer" class="dialog-footer">
            <el-button size="small" @click="listDialog = false"><?= \Yii::t('plugins/community', '取消');?></el-button>
            <el-button size="small" type="primary" :loading="saveLoading" @click="submitActivity"><?= \Yii::t('plugins/community', '确定');?></el-button>
        </span>
    </el-dialog>
</div>
