    ...
    public function getActivityList()
    {
        ...
        try {
            ...
            if ($this->type == 1) {
                ...
                $relations = CommunityRelations::find()
                    ->andWhere(['user_id' => \Yii::$app->user->id, 'is_delete' => 0])
                    ->andWhere(['!=', 'middleman_id', 0])->one();
                if (empty($relations)) {
                    //未绑定，非分享进入
                    if (!$this->middleman_id) {
                        $middlemanInfo = $this->getMiddleman();
                    }
                } else {
            ...
            $removeStorage = false;
            //团长信息判断
            $middleman_info = CommunityMiddleman::find()->with('address')
                ->where(['user_id' => $this->middleman_id, 'status' => 1, 'is_delete' => 0])
                ->one();
            if (empty($middleman_info)) {
                if ($this->type == 1) {
                    $removeStorage = true;
                    $middleman_info = $this->getMiddleman();
                    if (empty($middleman_info)) {
...
//                    $model->andWhere(['status' => 1]);
                    ...
                    default:
                        $model->orderBy([
                            new Expression("FIELD(`status_name`,'进行中','未开始','已结束')"), 'start_at' => SORT_ASC
                        ]);
                        break;
                }
            }
            ...
            foreach ($list as $key => &$item) {
                ...
                $virtual_sales = (CommunityGoods::find()->alias('cg')
                        ->where(['cg.activity_id' => $item['id'], 'cg.is_delete' => 0])
                        ->leftJoin(['g' => Goods::tableName()], 'g.id = cg.goods_id')->andWhere(['g.status' => 1])
                        ->keyword(
                            $this->type == 1,
                            ['not in', 'cg.goods_id', CommunitySwitch::find()->select('goods_id')
                                ->where([
                                    'activity_id' => $item['id'],'middleman_id' => $this->middleman_id, 'is_delete' => 0
                                ])]
                        )
                ...
                switch ($item['condition']) {
                    case 1:
                        $item['condition_count'] = CommunityOrder::find()->alias('co')
                                ...
                                ->andWhere(['o.is_delete' => 0, 'o.is_recycle' => 0])
                                ->andWhere([
                                    'co.middleman_id' => $this->middleman_id, 'co.activity_id' => $item['id'],
                                    'co.is_delete' => 0
                                ])
                ...
                $goods_model = CommunityGoods::find()->alias('cg')
                    ->where(['cg.activity_id' => $item['id'], 'cg.is_delete' => 0])
                    ->leftJoin(['g' => Goods::tableName()], 'g.id = cg.goods_id')
                    ->andWhere(['g.status' => 1]);
                if ($this->type == 1) {
                    $goods_model->andWhere(['not in', 'cg.goods_id',
                        CommunitySwitch::find()
                            ->where([
                                'middleman_id' => $this->middleman_id, 'activity_id' => $item['id'], 'is_delete' => 0
                            ])
                            ->select('goods_id')])
...
//                }
    ...
    {
        $middlemanInfo = CommunityMiddleman::find()->with(['address', 'userInfo'])
            ->where(['mall_id' => \Yii::$app->mall->id, 'status' => 1, 'is_delete' => 0])
            ->all();
        ...
        $tempDistance = 0;
        $info = [];
        /** @var CommunityMiddleman[] $middlemanInfo */
        foreach ($middlemanInfo as $item) {
            $distance = get_distance($this->longitude, $this->latitude, $item->address->longitude, $item->address->latitude);
            //取最近距离的
