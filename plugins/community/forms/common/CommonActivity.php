...
//                $user_arr[$k]['nickname'] = $value['user']['nickname'];
    ...
    public static function getActivity($id)
    {
        ...
        $activity = CommunityActivity::find()
            ->with(['communityGoods.goods.attr', 'communityGoods.goods.goodsWarehouse'])
    ...
    public static function getNewActivityDetail($activity_id, $middleman_id, $type = 1)
    {
        ...
        $data['address'] = $address;
        $data['full_price'] = Json::decode($data['full_price'], true);
        ...
        switch ($data['condition']) {
            case 1:
                $data['condition_count'] = CommunityOrder::find()->alias('co')
                        ...
                        ->andWhere(['o.is_delete' => 0, 'o.is_recycle' => 0, 'co.is_delete' => 0])
                        ->andWhere(['co.middleman_id' => $middleman_id, 'co.activity_id' => $data['id']])
                        ->groupBy('co.user_id')->count() ?? 0;
                break;
            case 2:
                $data['condition_count'] = CommunityOrder::find()->alias('co')
        ...
        //锁定成团
        if (
        !empty(CommunityActivityLocking::findOne([
            'middleman_id' => $middleman_id, 'activity_id' => $data['id'], 'is_delete' => 0]))
        ) {
            $data['is_success'] = 1;
        }
        if (strtotime($data['start_at']) <= time()) {
        ...
        $allVirtualSales = $query->select('sum(`g`.`virtual_sales`)')->scalar();
        ...
        $data['all_sales'] = intval(CommunityOrder::find()->alias('co')
                ...
                ->sum('co.num')) ?? 0;
        if ($type == 1) {
            $data['all_sales'] += $allVirtualSales;
        }
        ...
        $robot_count = ceil(bcmul($allVirtualSales, 0.95, 2));//虚拟销量95%转换成人数
        $data['user_num'] = $count;
        if ($type == 1) {
            $data['user_num'] += $robot_count;
        }
        //补充机器人
        $robots_info = CommunityActivityRobots::findOne([
            'activity_id' => $activity_id, 'middleman_id' => $middleman_id, 'is_delete' => 0
        ]);
        if (empty($robots_info)) {
            $ids_arr = [];
            //随机7个机器人ID
        ...
        if ($robot_count > 0) {
            ...
            for ($i = 0; $i < (($robot_count <= 7 ? $robot_count : 7) - $count); $i++) {
                ...
                array_push(
                    $user_arr,
                    [
                        'avatar' => $host_url . '/web/statics/img/plugins/community/user_avatar/user_' .
                            $robots_ids_arr[$i] . '.png',
                        'time' => time() - strtotime($data['start_at'])
                    ]
                );
            }
        }
        ...
        if ($type == 1) {
            ...
            if ($data['condition'] == 2) {
                $data['condition_count'] += $allVirtualSales;
                $data['num'] += $allVirtualSales;
            }
        }
        $data['rate'] = $data['num'] == 0 ? 0 : bcdiv($data['condition_count'], $data['num'], 4) * 100;//比例
        $data['rate'] = $data['rate'] <= 100 ? $data['rate'] : 100;
        ...
        return [
            'activity' => ArrayHelper::filter($data, [
                'id', 'activity_status', 'all_sales', 'condition', 'condition_count', 'full_price', 'num',
                'rate', 'start_at', 'end_at', 'time', 'title', 'user_num', 'is_success'
            ]),
            'user_list' => array_reverse($user_arr),//倒序输出
        ];
