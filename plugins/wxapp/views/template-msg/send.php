...
<div id="app" v-cloak>
    <el-card v-loading="cardLoading" style="border:0" shadow="never"
        ...
        <div slot="header">
            <div>
                <span><?= \Yii::t('plugins/wxapp', '群发订阅消息');?></span>
            </div>
        </div>
        <el-form :model="ruleForm" ref="ruleForm" :rules="rules" size="small" label-width="120px">
            <div class="form-body">
                    ...
                    <el-form-item>
                        <el-col :span="14">
                            <el-tag style='border:0;width: 49%;min-width: 380px;margin-right: 10px;margin-bottom: 10px'
                                    type="danger"><?= \Yii::t('plugins/wxapp', '群发订阅消息有被封号的风险');?>
                            </el-tag>
                            <el-tag style='border:0;width: 49%;min-width: 380px;' type="danger">
                                <?= \Yii::t('plugins/wxapp', '订阅消息只发送给7天内在小程序内点击过的活跃用户');?>
                            </el-tag>
                        </el-col>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('plugins/wxapp', '发送对象');?>">
                        <el-col :span="14">
                            <el-button :disabled="ruleForm.is_all" type="primary" @click="getUsers"><?= \Yii::t('plugins/wxapp', '添加用户');?></el-button>
                            <el-checkbox style="margin-left: 15px;" @change="selectAllUsers"
                                         v-model="ruleForm.is_all"><?= \Yii::t('plugins/wxapp', '所有用户');?>
                            </el-checkbox>
                        </el-col>
                    </el-form-item>
                </el-row>
                ...
                <el-row>
                    <el-form-item label="<?= \Yii::t('plugins/wxapp', '选择模板');?>">
                        <el-col :span="22">
                            <div v-if="templateList.length > 0"
                                                ...
                                                <div>
                                                    <div style="display: inline-block;width: 90px;margin-bottom: 10px;padding-right: 10px;text-align: right">
                                                        <el-tag style="border:0;font-size: 14px" type="warning">
                                                            {{item.style == 1 ? '<?= \Yii::t('plugins/wxapp', '小标题');?>' : '<?= \Yii::t('plugins/wxapp', '大标题');?>'}}
                                                        </el-tag>
                                                    </div>
                                                    <el-tag style="font-size: 14px" type="info">{{item.name}}</el-tag>
                                                </div>
                                                <div>
                                                    <span class="label"><?= \Yii::t('plugins/wxapp', '模板ID');?></span>
                                        ...
                                        <div>
                                            <div v-for="(item2,index2) in item.fields"
                                                ...
                                                <el-input class="fields-input" v-model="item2.field_value"
                                                          :placeholder="'<?= \Yii::t('plugins/wxapp', '请输入');?>' + item2.field_name">
                                                </el-input>
                                            </div>
                                            <div flex="dir:left">
                                                <span class="label"><?= \Yii::t('plugins/wxapp', '模板链接Url');?></span>
                                                <el-input class="fields-input" v-model="item.link_url"
                                                          placeholder="<?= \Yii::t('plugins/wxapp', '例如');?>: pages/index/index">
                                                </el-input>
                                            </div>
                                        </div>
                                    </el-card>
                                ...
                                <div style="margin-left: -60px;margin-top: 20px">
                                    <el-button @click="destroyTemplate(index)" type="text" size="small" circle>
                                        <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/wxapp', '删除');?>" placement="top">
                            ...
                            <template v-if="templateList.length < 3">
                                <el-button style="margin-left: 20px;font-size: 14px;" @click="openDialog"
                                           type="text"><i class="el-icon-plus"><?= \Yii::t('plugins/wxapp', '新增模板');?></i></el-button>
                            </template>
                        </el-col>
                    </el-form-item>
                </el-row>
            </div>
            <el-button class="button-item" :loading="btnLoading" type="primary" @click="send" size="small"><?= \Yii::t('plugins/wxapp', '发送');?>
            </el-button>
        </el-form>

        <el-dialog title="<?= \Yii::t('plugins/wxapp', '订阅消息格式');?>" :visible.sync="dialogVisible">
            <el-form :model="dialogRuleForm" ref="dialogRuleForm" :rules="dialogRules" size="small" label-width="120px">
                <el-row>
                    <el-col :span="18">
                        <el-form-item label='<?= \Yii::t('plugins/wxapp', '模板名称');?>' prop="name">
                            <el-input v-model="dialogRuleForm.name" placeholder="<?= \Yii::t('plugins/wxapp', '请输入模板名称');?>"></el-input>
                        </el-form-item>
                        <el-form-item label='<?= \Yii::t('plugins/wxapp', '模板ID');?>' prop="tpl_id">
                            <el-input v-model="dialogRuleForm.tpl_id" placeholder="<?= \Yii::t('plugins/wxapp', '请输入模板ID');?>"></el-input>
                        </el-form-item>
                        <el-form-item label='<?= \Yii::t('plugins/wxapp', '首行样式');?>' prop="style">
                            <el-radio v-model="dialogRuleForm.style" label="1"><?= \Yii::t('plugins/wxapp', '小标题');?></el-radio>
                            <el-radio v-model="dialogRuleForm.style" label="2"><?= \Yii::t('plugins/wxapp', '大标题');?></el-radio>
                        </el-form-item>
                        <el-form-item label='<?= \Yii::t('plugins/wxapp', '模版字段');?>'>
                            <div v-for="(item,index) in dialogRuleForm.fields"
                                 :key="index"
                                 flex="dir:left"
                                 style="margin-bottom: 15px">
                                <el-input :placeholder="'<?= \Yii::t('plugins/wxapp', '请输入第');?>' +(index + 1)+ '<?= \Yii::t('plugins/wxapp', '行模板名称');?>'"
                            ...
                            <el-button type="text" @click="addField"><i class="el-icon-plus"></i><?= \Yii::t('plugins/wxapp', '添加字段');?></el-button>
                        </el-form-item>
                    </el-col>
                </el-row>
                <el-row>
                    <el-form-item>
                        <el-button :loading="btnLoading"
                                   ...
                                   size="small"><?= \Yii::t('plugins/wxapp', '保存');?>
                        </el-button>
                    </el-form-item>
                </el-row>
            </el-form>
        ...
        <el-dialog title="<?= \Yii::t('plugins/wxapp', '用户列表');?>" :visible.sync="dialogUsersVisible">
            <template>
                <el-input style="width: 260px;margin-bottom: 20px;" size="small" :disabled="ruleForm.is_all"
                          v-model="ruleForm.keyword"
                          placeholder="<?= \Yii::t('plugins/wxapp', '输入用户ID');?>">
                    <template slot="append">
                        <el-button :disabled="ruleForm.is_all" @click="getUsers"><?= \Yii::t('plugins/wxapp', '搜索');?></el-button>
                    </template>
                </el-input>
                <el-table
            ...
            <div style="text-align: right;margin-top: 20px;">
                <el-button type="primary" @click="selectUserSubmit" size="small"><?= \Yii::t('plugins/wxapp', '确定');?></el-button>
            </div>
        </el-dialog>
        <el-dialog title="<?= \Yii::t('plugins/wxapp', '发送订阅消息');?>" :visible.sync="progressVisible">
            <div style="margin: 10px 0;">
                <?= \Yii::t('plugins/wxapp', '总数');?>{{selectUsers.length}}<?= \Yii::t('plugins/wxapp', '条失败');?>{{progressErrorCount}}<?= \Yii::t('plugins/wxapp', '条');?>
            </div>
            <el-progress :text-inside="true" :stroke-width="18" :percentage="progressCount"></el-progress>
            <div style="text-align: right;margin-top: 20px;">
                <el-button type="success" :loading="btnLoading" @click="sendConfirm" size="small"><?= \Yii::t('plugins/wxapp', '确定');?></el-button>
            </div>
            <div style="margin-top: 20px;" v-for="(item,index) in progressErrors" :key="index">
                <?= \Yii::t('plugins/wxapp', '用户');?>: {{item.nickname}}(ID:{{item.id}}):{{item.value}}
            </div>
    ...
    const app = new Vue({
            ...
            data() {
                return {
                    ...
                    dialogRules: {
                        name: [
                            {required: true, message: '<?= \Yii::t('plugins/wxapp', '请输入模板名称');?>', trigger: 'change'},
                        ],
                        tpl_id: [
                            {required: true, message: '<?= \Yii::t('plugins/wxapp', '请输入模板ID');?>', trigger: 'change'},
                        ],
                        style: [
                            {required: true, message: '<?= \Yii::t('plugins/wxapp', '请选择样式');?>', trigger: 'change'},
                        ],
                    },
            ...
            methods: {
                send() {
                    ...
                    if (self.selectUsers.length == 0) {
                        self.$message.warning('<?= \Yii::t('plugins/wxapp', '请选择用户');?>');
                        return;
                    }
                    if (self.ruleForm.current_index === '') {
                        self.$message.warning('<?= \Yii::t('plugins/wxapp', '请选择模板');?>');
                        return;
                    }
                ...
                destroyTemplate(index) {
                    let self = this;
                    self.$confirm('<?= \Yii::t('plugins/wxapp', '删除该条模板');?>', '<?= \Yii::t('plugins/wxapp', '提示');?>', {
                        ...
                        self.$message.info('<?= \Yii::t('plugins/wxapp', '已取消删除');?>')
                    });
                },
                openDialog() {
