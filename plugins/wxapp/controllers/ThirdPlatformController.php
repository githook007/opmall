...
class ThirdPlatformController extends Controller
{
    public function actionAuthorizer()
    {
        ...
        try {
            ...
            \Yii::warning($res);
            $mall = Mall::findOne(\Yii::$app->request->get("_mall_id"));
            if($mall){
            ...
            WxappWxminiprograms::updateAll(['is_delete' => 1, "deleted_at" => mysql_timestamp()], [
                'AND',
                [
            ...
            // @czs 记录错误消息并显示到后台页面
            \Yii::$app->cache->set(WxappWxminiprograms::ERR_MSG_KEY.\Yii::$app->mall->id, $exception->getMessage());
            \Yii::error($exception);
            return \Yii::$app->response->redirect(\Yii::$app->urlManager->createUrl(['plugin/wxapp/wx-app-config/setting']));
        }
    }
    ...
    public function actionUpload()
    {
        try {
            ...
            $user_version = \Yii::$app->request->post('user_version');
            $is_plugin = \Yii::$app->request->post('is_plugin', 0);
            $ext = ExtAppForm::instance();
            $res = $ext->uploadCode($template_id, $is_plugin, $user_version ?: app_version(), \Yii::$app->mall->name);
            return [
                'code' => ApiCode::CODE_SUCCESS,
                'msg' => '请求成功',
                'data' => $res
