...
class RegisterService extends BaseService
{
     ...
     * 上传类目、品牌、商品时，需使用图片上传接口换取临时链接用于上传，上传后微信侧会转为永久链接
     * @czs
     */
    public function imgUpload($imgUrl){
        $api = "https://api.weixin.qq.com/shop/img/upload?access_token={$this->accessToken}";
        $res = CurlHelper::getInstance()->httpPost($api, [], [
            ...
            'img_url' => $imgUrl, // upload_type=1时必传
        ]);
        return $this->getResult($res);
    }
    ...
    /**
     * @param $args ['audit_id' => '审核单id']
     * @return mixed
     * @throws \Exception
     * 获取审核结果
     */
    public function auditResult($args)
    {
        $api = "https://api.weixin.qq.com/shop/audit/result?access_token={$this->accessToken}";
        $res = $this->getClient()->httpPost($api, [], [
    ...
    {
        $api = "https://api.weixin.qq.com/shop/account/get_info?access_token={$this->accessToken}";
        $res = $this->getClient()->httpPost($api, [], []);
        return $this->getResult($res);
    }
     ...
     * 更新商家信息
     */
    public function updateInfo($args)
    {
        $api = "https://api.weixin.qq.com/shop/account/update_info?access_token={$this->accessToken}";
