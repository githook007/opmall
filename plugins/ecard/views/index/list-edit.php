    ...
    <el-card class="box-card" shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;" v-loading="loading">
        <div slot="header">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item class="route" @click.native="route('plugin/ecard/mall/index/index')"><?= \Yii::t('plugins/edit', '卡密列表');?></el-breadcrumb-item>
                <el-breadcrumb-item class="route"  @click.native="route('plugin/ecard/mall/index/list', {id: ecard_id})"><?= \Yii::t('plugins/edit', '卡密管理');?></el-breadcrumb-item>
                <el-breadcrumb-item><?= \Yii::t('plugins/edit', '添加卡密数据');?></el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <el-form label-width="120px" :model="form" :rules="rules" ref="form">
            <div class="table-body">
                <div class="poster-form-title" style="margin-bottom: 24px;">
                    <?= \Yii::t('plugins/edit', '向');?>
                    <span class="name">{{form.ecard.name}}</span>
                    <?= \Yii::t('plugins/edit', '添加卡密数据');?>
                    <span style="color: #999999;"><?= \Yii::t('plugins/edit', '限制50个字');?></span>
                </div>
                <div style="padding: 20px;">
                    ...
                    >
                        ...
                        >
                            <template slot-scope="scope">
                                ...
                                <span v-if="scope.row.is_repeat" style="color: #ff4544;font-size: 10px;"><?= \Yii::t('plugins/edit', '重复数据');?></span>
                            </template>
                        </el-table-column>
                        <el-table-column
                                prop="address"
                                fixed="right"
                                width="240"
                                label="<?= \Yii::t('plugins/edit', '操作');?>">
                            <template slot-scope="scope">
                                <el-button type="text" circle size="mini" @click="deleteItem(scope.$index)">
                                    <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/edit', '删除');?>" placement="top">
                    ...
                    <div flex="main:justify">
                        <el-button type="primary" size="small" @click="addField" style="margin-top: 20px;" >
                            ...
                            <span style="font-size: 14px"><?= \Yii::t('plugins/edit', '新增一条数据');?></span>
                        </el-button>
                    </div>
                </div>
            </div>
            <el-button @click="save" type="primary" size="small" ><?= \Yii::t('plugins/edit', '保存');?></el-button>
        </el-form>
    </el-card>
</section>
