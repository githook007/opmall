...
<div id="app" v-cloak>
    ...
    >
        ...
        <span slot="footer" class="dialog-footer">
            <el-button @click="dialogVisible = false"><?= \Yii::t('plugins/ecard', '取消');?></el-button>
            <el-button type="primary" @click="saveItem"><?= \Yii::t('plugins/ecard', '确定');?></el-button>
        </span>
    </el-dialog>
    <el-dialog
            title="<?= \Yii::t('plugins/ecard', '批量导入卡密数据');?>"
            :visible.sync="updateDialogVisible"
            width="20%"
    >
        <div>
            <div style="margin-bottom: 15px;"><?= \Yii::t('plugins/ecard', '文件大小');?>{{sizeM}}</div>
            <div flex="">
                <div style="display: inline-block">
                    <?= \Yii::t('plugins/ecard', '导入进度');?>
                </div>
                <div style="width: 80%">
            ...
            <div v-if="updateData">
                <div flex="cross:center main:center" style="margin-top: 15px;">
                    ...
                    <span><?= \Yii::t('plugins/ecard', '成功导入');?>{{updateData.success}}<?= \Yii::t('plugins/ecard', '条数据');?></span>
                </div>
                <div flex="cross:center main:center" style="margin-top: 15px;" >
                    <span style="margin-right: 5px;">
                        <?= \Yii::t('plugins/ecard', '未导入');?>
                        <span style="color: #ff5d5d">{{updateData.fail}}</span>
                        <?= \Yii::t('plugins/ecard', '条数据');?>
                    </span>
                    <div>
                        <el-button @click="updateNo" style="color: #3398ff;cursor: pointer;border: none;padding: 0;margin-left: 5px;">
                            <?= \Yii::t('plugins/ecard', '下载未导入数据');?>
                        </el-button>
                    </div>
                </div>
            </div>
        ...
        <div slot="footer" class="dialog-footer" flex="main:center">
            <el-button type="primary" @click="updateDialogVisible = false"><?= \Yii::t('plugins/ecard', '我知道了');?></el-button>
        </div>
    </el-dialog>
    <el-dialog
            flex="cross:center"
            class="export-dialog"
            title="<?= \Yii::t('plugins/ecard', '提示');?>"
            ...
            <div>
                <div class="el-message-box__content">
                    ...
                    <div class="el-message-box__message">
                        <p>{{exportParams.export == 'export' ? '<?= \Yii::t('plugins/ecard', '确认导出所有卡密数据');?>' : '<?= \Yii::t('plugins/ecard', '确认下载卡密库模板');?>'}}</p>
                    </div>
                </div>
            </div>
            <span slot="footer" class="dialog-footer">
                <form target="_blank" :action="exportParams.action_url" method="post">
                    ...
                    <div flex="dir:right" style="margin-top: 20px;">
                        <button type="submit"
                                class="el-button el-button--primary el-button--small"><?= \Yii::t('plugins/ecard', '点击下载');?></button>
                    </div>
                </form>
            </span>
        </template>
    ...
    <el-dialog
        ...
        <p flex="cross:center">
            ...
            <span><?= \Yii::t('plugins/ecard', '请导入不大于2M的Excel文件');?></span>
        </p>
        <span slot="footer" class="dialog-footer">
             <el-upload
                ...
                <el-button type="primary" ><?= \Yii::t('plugins/ecard', '点击导入');?></el-button>
            </el-upload>
        </span>
    </el-dialog>
    <el-card class="box-card" shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;" v-loading="loading">
        <div slot="header">
            <div>
                <span @click="route" style="cursor: pointer;color: #409eff;"><?= \Yii::t('plugins/ecard', '卡密列表');?></span>
                <span>/<?= \Yii::t('plugins/ecard', '卡密管理');?></span>
                <div style="float: right;margin-top: -5px" flex="">
                    <el-button style="margin-right: 10px;" type="primary" @click="newEdit" size="small"><?= \Yii::t('plugins/ecard', '添加卡密数据');?></el-button>
                    <el-button type="primary" @click="uploadVisible = true" size="small"><?= \Yii::t('plugins/ecard', '批量导入卡密数据');?></el-button>
                    <el-button type="primary" @click="exportEmpty" size="small"><?= \Yii::t('plugins/ecard', '下载模板');?></el-button>
                    <el-button type="primary" @click="exportData" size="small"><?= \Yii::t('plugins/ecard', '导出卡密数据');?></el-button>
                </div>
            </div>
        </div>
        <div style="background-color: #ffffff;">
            ...
            <el-form>
                <div style="margin-left: 50px;padding: 20px;">
                    <el-form-item label="<?= \Yii::t('plugins/ecard', '卡密名称');?>" >
                        <el-input disabled class="card-name" v-model="form.ecard.name" placeholder="<?= \Yii::t('plugins/ecard', '最多输入10个字');?>" maxlength="10"></el-input>
                    </el-form-item>
                </div>
            </el-form>
            <div class="table-body">
                ...
                <div flex="wrap:nowrap cross:center" style="margin-bottom: 15px;">
                    <div class="item-box" style="margin-right: 15px;" flex="dir:left cross:center">
                        ...
                        <div>
                            <el-input style="width: 350px" size="small"
                                      ...
                                      :placeholder="'<?= \Yii::t('plugins/ecard', '请输入');?>'+ form.ecard.key"
                                      clearable
                                      @clear="toSearch"
                    ...
                    <div class="label"><?= \Yii::t('plugins/ecard', '添加时间');?></div>
                    <el-date-picker
                            class="item-box"
                            size="small"
                            @change="toSearch"
                            ...
                            range-separator="<?= \Yii::t('plugins/ecard', '至');?>"
                            start-placeholder="<?= \Yii::t('plugins/ecard', '开始日期');?>"
                            end-placeholder="<?= \Yii::t('plugins/ecard', '结束日期');?>">
                    </el-date-picker>
                    <el-button class="item-box" style="margin-left: 20px;" @click="deleteList" size="small"><?= \Yii::t('plugins/ecard', '批量删除');?></el-button>
                </div>
                <el-tabs v-model="search.status" @tab-click="toSearch">
                    <el-tab-pane label="<?= \Yii::t('plugins/ecard', '全部');?>" name="-1"></el-tab-pane>
                    <el-tab-pane label="<?= \Yii::t('plugins/ecard', '已售出');?>" name="1"></el-tab-pane>
                    <el-tab-pane label="<?= \Yii::t('plugins/ecard', '未售出');?>" name="0"></el-tab-pane>
                </el-tabs>
                <el-table v-loading="listLoading" :data="form.list" border
                    ...
                    <el-table-column
                        ...
                        <template slot-scope="scope">
                            <el-tag type="success" v-if="scope.row.is_sales === 0"><?= \Yii::t('plugins/ecard', '未售出');?></el-tag>
                            <el-tag type="warning" v-else-if="scope.row.is_sales === 1"><?= \Yii::t('plugins/ecard', '已售出');?></el-tag>
                        </template>
                    </el-table-column>
                    <el-table-column
                            prop="created_at"
                            fixed="right"
                            width="265"
                            label="<?= \Yii::t('plugins/ecard', '添加时间');?>">
                    </el-table-column>
                    <el-table-column
                            prop="address"
                            fixed="right"
                        ...
                        <template slot-scope="scope">
                            <el-button type="text" v-if="scope.row.is_sales === 0" circle size="mini" @click="edit(scope.row)">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/ecard', '编辑');?>" placement="top">
    ...
    const app = new Vue({
        ...
        methods: {
            ...
            updateNo() {
                ...
                elt.setAttribute('download', '<?= \Yii::t('plugins/ecard', '未导入数据');?>.csv');
            ...
            deleteList() {
                ...
                if(that.selectList.length === 0) {
                    that.$message({
                        type: 'warning',
                        message: '<?= \Yii::t('plugins/ecard', '请先选择数据');?>'
                    });
                    return;
                }
                this.$confirm('<?= \Yii::t('plugins/ecard', '是否确认删除选中的数据');?>', '<?= \Yii::t('plugins/ecard', '提示');?>', {
                    ...
                    request({
                        ...
                        if (e.data.code === 0) {
                            ...
                            that.$message({
                                type: 'success',
                                message: '<?= \Yii::t('plugins/ecard', '批量删除成功');?>!'
                            });
                            this.getInform();
                        } else {
                            that.$message({
                    ...
                    this.$message({
                        type: 'info',
                        message: '<?= \Yii::t('plugins/ecard', '已取消删除');?>'
                    });
                });
            },
            edit(row) {
            ...
            deleteItem(data) {
                let that = this;
                this.$confirm('<?= \Yii::t('plugins/ecard', '删除该条数据');?>', '<?= \Yii::t('plugins/ecard', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('plugins/ecard', '确定');?>',
                    cancelButtonText: '<?= \Yii::t('plugins/ecard', '取消');?>',
                    type: 'warning'
                }).then(() => {
                    request({
                        ...
                        if (e.data.code === 0) {
                            that.$message({
                                type: 'success',
                                message: '<?= \Yii::t('plugins/ecard', '删除成功');?>!'
                            });
                            this.getInform();
                        } else {
                            that.$message({
            ...
            async saveItem() {
                ...
                if (e.data.code === 0) {
                    this.$message({
                        type: 'success',
                        message: '<?= \Yii::t('plugins/ecard', '编辑成功');?>!'
                    });
            ...
            newEdit() {
                ...
                if (this.form.ecard.list.length > 0) {
                    ...
                    this.$confirm('<?= \Yii::t('plugins/ecard', '请先添加字段');?>', '<?= \Yii::t('plugins/ecard', '提示');?>', {
                        confirmButtonText: '<?= \Yii::t('plugins/ecard', '确定');?>',
                        cancelButtonText: '<?= \Yii::t('plugins/ecard', '取消');?>',
                        type: 'warning'
                    }).then(() => {
