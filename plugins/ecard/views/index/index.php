...
<div id="app" v-cloak>
    <el-card class="box-card" shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <div>
                <span><?= \Yii::t('plugins/ecard', '卡密列表');?></span>
                <div style="float: right;margin-top: -5px">
                    <el-button type="primary" @click="edit" size="small"><?= \Yii::t('plugins/ecard', '新增卡密');?></el-button>
                </div>
            </div>
        </div>
        <div class="table-body">
            <div class="input-item">
                <el-input @keyup.enter.native="search" size="small" placeholder="<?= \Yii::t('plugins/ecard', '请输入卡密名称搜索');?>" v-model="form.keyword" clearable @clear="search">
            ...
            <el-table v-loading="listLoading" :data="list" border style="width: 100%">
                <el-table-column
                        prop="name"
                        label="<?= \Yii::t('plugins/ecard', '卡密名称');?>">
                </el-table-column>
                <el-table-column
                        prop="sales"
                        label="<?= \Yii::t('plugins/ecard', '已售');?>">
                </el-table-column>
                <el-table-column
                        prop="stock"
                        sortable
                        label="<?= \Yii::t('plugins/ecard', '库存');?>">
                ...
                <el-table-column
                        prop="created_at"
                        label="<?= \Yii::t('plugins/ecard', '创建时间');?>">
                </el-table-column>
                <el-table-column
                        prop="address"
                        fixed="right"
                    ...
                    <template slot-scope="scope">
                        ...
                        <el-button  type="text" circle size="mini" @click="management(scope.row.id)">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/ecard', '卡密管理');?>" placement="top">
    ...
    const app = new Vue({
        ...
        methods: {
            ...
            async deleteItem(row) {
                console.log(row);
                if (!row.can_delete) {
                    this.$confirm('<?= \Yii::t('plugins/ecard', '卡密数据正在售卖中');?>', '<?= \Yii::t('plugins/ecard', '提示');?>', {
                        confirmButtonText: '<?= \Yii::t('plugins/ecard', '我知道了');?>',
                ...
                let that = this;
                that.$confirm('<?= \Yii::t('plugins/ecard', '删除该条卡密');?>', '<?= \Yii::t('plugins/ecard', '提示');?>', {
                    ...
                    request({
                        ...
                        if (e.data.code === 0) {
                            ...
                            this.$confirm(e.data.msg, '<?= \Yii::t('plugins/ecard', '提示');?>', {
                                confirmButtonText: '<?= \Yii::t('plugins/ecard', '我知道了');?>',
                                showCancelButton: false,
                                type: 'warning'
                            }).then(() => {
