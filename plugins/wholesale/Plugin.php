...
class Plugin extends \app\plugins\Plugin
{
    public function getMenus()
    {
        return [
            [
                'name' => \Yii::t('plugins/wholesale', '批发设置'),
                'route' => 'plugin/wholesale/mall/setting/index',
                'icon' => 'el-icon-star-on',
            ],
            [
                'name' => \Yii::t('plugins/wholesale', '商品管理'),
    ...
    {
        return \Yii::t('plugins/wholesale', '商品批发');
    }
    ...
    public function getPickLink()
    {
        ...
        return [
            [
                'key' => 'wholesale',
                'name' => \Yii::t('plugins/wholesale', '商品批发'),
                'open_type' => '',
            ...
            [
                'key' => 'wholesale',
                'name' => \Yii::t('plugins/wholesale', '批发商品详情'),
                'open_type' => '',
                'icon' => $iconBaseUrl . '/icon-wholesale-goods.png',
                'value' => '/plugins/wholesale/goods/goods',
                'params' => [
                    [
                        'key' => 'id',
                        'value' => '',
                        'desc' => \Yii::t('plugins/wholesale', '请填写批发商品ID'),
                        'is_required' => true,
                        'data_type' => 'number',
                        'page_url' => '/plugin/wholesale/mall/goods/index',
                        'page_url_text' => \Yii::t('plugins/wholesale', '商品列表'),
                    ],
                ],
    ...
    public function getStatisticsMenus($bool = true)
    {
        return [
            ...
            'name' => $bool ? $this->getDisplayName() : \Yii::t('plugins/wholesale', '插件统计'),
    ...
    public function getOrderInfo($orderId, $order)
    {
        ...
        if ($wholesaleOrder) {
            $data = [
                'discount_list' => [
                    'wholesale_discount' => [
                        'label' => \Yii::t('plugins/wholesale', '批发优惠'),
...
//        }
    ...
    public function pluginDiscount($goodsItem, $mchItem, $goods_num_info)
    {
        ...
        if (is_numeric($wholesaleUnitPrice) && $wholesaleUnitPrice >= 0) {
            ...
            if ($wholesaleSubPrice != 0) {
                ...
                $goodsItem['discounts'][] = [
                    'name' => \Yii::t('plugins/wholesale', '批发优惠'),
    ...
    public function pluginDiscountData($mchItem)
    {
        if ($mchItem['wholesale_discount'] > 0) {
            $mchItem['discounts'][] = [
                'title' => \Yii::t('plugins/wholesale', '批发优惠'),
