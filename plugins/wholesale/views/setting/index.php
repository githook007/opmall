    ...
    <el-card v-loading="loading" style="border:0" shadow="never" body-style="background-color: #f3f3f3;padding: 0 0;">
        <el-form :model="form" label-width="180px" ref="form">
            <el-tabs v-model="activeName">
                <el-tab-pane label="<?= \Yii::t('plugins/wholesale', '基本设置');?>" name="first">
                    <app-setting v-model="form" :is_payment="false" :is_vip_show="true" :is_send_type="false"></app-setting>
                </el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('plugins/wholesale', '自定义背景图');?>" name="second">
                    <div class="bg" flex="dir:left box:first">
                        ...
                        <div class="right form-body">
                            <el-form :model="form" label-width="120px" ref="form">
                                <el-form-item label="<?= \Yii::t('plugins/wholesale', '头部banner图');?>" prop="banner" class="required-icon">
                                    <div flex style="margin-bottom: 8px;">
                                        <app-attachment :multiple="false" :max="1" @selected="topPicUrl">
                                            <el-tooltip effect="dark"
                                                        content="<?= \Yii::t('plugins/wholesale', '建议尺寸');?>:750 * 280"
                                                        placement="top">
                                                <el-button size="mini"><?= \Yii::t('plugins/wholesale', '选择图标');?></el-button>
                                            </el-tooltip>
                                        </app-attachment>
                                        <div style="margin-left: 10px;">
                                            <el-button type="primary" size="mini" @click="resetImg"><?= \Yii::t('plugins/wholesale', '恢复默认');?></el-button>
                                        </div>
                                    </div>
                            ...
                            <div class="right-button" v-if="activeName == 'second'">
                                <el-button class="button-item" :loading="btnLoading" type="primary" @click="submit('form')" size="small"><?= \Yii::t('plugins/wholesale', '保存');?></el-button>
                            </div>
                        </div>
                    </div>
                </el-tab-pane>
            </el-tabs>
            <el-button v-if="activeName == 'first'" style="margin-bottom: 150px;" :loading="btnLoading" class="button-item" type="primary" @click="submit('form')" size="small"><?= \Yii::t('plugins/wholesale', '保存');?></el-button>
            </el-tabs>
        </el-form>
    </el-card>
</div>
    ...
    const app = new Vue({
        ...
        methods: {
            ...
            async submit(formName) {
                if(!this.form.banner) {
                    this.$message.error('<?= \Yii::t('plugins/wholesale', '请选择头部banner图');?>');
                    return false;
                }
                this.$refs[formName].validate(valid => {
                    if (valid) {
                        ...
                        request({
                            ...
                            if (e.data.code === 0) {
                                this.$message.success('<?= \Yii::t('plugins/wholesale', '保存成功');?>');
                            } else {
