    ...
    <div class="header-box">
        <el-breadcrumb separator="/">
            <el-breadcrumb-item><span style="color: #409EFF;cursor: pointer"
                                      @click="$navigate({r:'plugin/wholesale/mall/goods/index'})"><?= \Yii::t('plugins/wholesale', '商品管理');?></span></el-breadcrumb-item>
            <el-breadcrumb-item v-if="form.id>0"><?= \Yii::t('plugins/wholesale', '编辑商品');?></el-breadcrumb-item>
            <el-breadcrumb-item v-else><?= \Yii::t('plugins/wholesale', '添加商品');?></el-breadcrumb-item>
        </el-breadcrumb>
    </div>
    <app-goods
            ref="appGoods"
        ...
        <template slot="before_basic_tab_pane">
            <el-tab-pane label="<?= \Yii::t('plugins/wholesale', '批发价设置');?>" name="four">
                <el-card shadow="never" style="margin-bottom: 10px">
                    <div slot="header"><?= \Yii::t('plugins/wholesale', '起批设置');?></div>
                    <el-form-item label="<?= \Yii::t('plugins/wholesale', '起批数量');?>" prop="num">
                        <template slot='label'>
                            <span class="required-icon"><?= \Yii::t('plugins/wholesale', '起批数量');?></span>
                        </template>
                        <el-input @input="change" style="width: 40%;" type="number" :min="0" v-model="form.rise_num">
                            <template slot="append"><?= \Yii::t('plugins/wholesale', '件');?></template>
                        </el-input>
                        <div style="color: #ff4544;margin-top: -5rpx"><?= \Yii::t('plugins/wholesale', '起批数量必须为大于0的整数');?></div>
                    </el-form-item>
                </el-card>
                <el-card shadow="never" style="margin-bottom: 10px">
                    <div slot="header"><?= \Yii::t('plugins/wholesale', '阶梯优惠设置');?></div>
                    <el-form-item label="<?= \Yii::t('plugins/wholesale', '开启阶梯优惠');?>">
                        <el-switch @change="statusChange" v-model="form.rules_status" :active-value="1"
                                   :inactive-value="0"></el-switch>
                    </el-form-item>
                    <el-form-item v-if="form.rules_status == 1" label="<?= \Yii::t('plugins/wholesale', '优惠方式');?>">
                        <el-radio @change="typeChange(0)" v-model="form.wholesale_type" :label="0"><?= \Yii::t('plugins/wholesale', '打折');?></el-radio>
                        <el-radio @change="typeChange(1)" v-model="form.wholesale_type" :label="1"><?= \Yii::t('plugins/wholesale', '减钱');?></el-radio>
                    </el-form-item>
                    <div v-if="form.rules_status == 1">
                        <div class="step" flex="dir:left cross:center">
                            <div class="step-label required-icon"><?= \Yii::t('plugins/wholesale', '一阶梯');?></div>
                            <div class="step-item" flex="main:justify">
                                <div flex="dir:left cross:center">
                                    ...
                                    <div style="margin-left: 10px"><?= \Yii::t('plugins/wholesale', '及以上');?></div>
                                </div>
                                <div>
                                    <div v-if="form.wholesale_type == 0" flex="dir:left cross:center">
                                        <div style="margin-right: 10px"><?= \Yii::t('plugins/wholesale', '打');?></div>
                                        <div>
                                            <el-input oninput="this.value = this.value.replace(/^(\-)*(\d+)\.(\d).*$/,'$1$2.$3');" @blur="changeDiscount(0)" size="small" style="width: 220px;height: 30px;" type="number" :min="0" v-model="form.wholesale_rules[0].discount">
                                                <template slot="append"><?= \Yii::t('plugins/wholesale', '折');?></template>
                                            </el-input>
                                        </div>
                                    </div>
                                    <div v-else flex="dir:left cross:center">
                                        <div style="margin-right: 10px"><?= \Yii::t('plugins/wholesale', '减');?></div>
                                        <div>
                                    ...
                                    <div v-if="form.wholesale_type == 1" class="wholesale-tip"><?= \Yii::t('plugins/wholesale', '减钱金额不大于最低价');?>{{showPrice}}<?= \Yii::t('plugins/wholesale', '元');?></div>
                                </div>
                            </div>
                        </div>
                        <div class="step" flex="dir:left cross:center">
                            <div class="step-label"><?= \Yii::t('plugins/wholesale', '二阶梯');?></div>
                            <div class="step-item" flex="main:justify">
                                <div flex="dir:left cross:center">
                                    <div>
                        ...
                        <div class="step" flex="dir:left cross:center">
                            <div class="step-label"><?= \Yii::t('plugins/wholesale', '三阶梯');?></div>
                            <div class="step-item" flex="main:justify">
                                <div flex="dir:left cross:center">
                                    <div>
                ...
                handler: function(data) {
                    ...
                    if (this.new_attr_groups.length === 0 ) {
                        this.new_attr_groups = [
                            {
                                attr_group_id: 1,
                                attr_group_name: "<?= \Yii::t('plugins/wholesale', '规格');?>",
                                attr_list: {
                                    attr_id: 0,
                                    attr_name: '<?= \Yii::t('plugins/wholesale', '默认');?>',
                                    pic_url: '',
                                }
                            }
                        ]
