...
class CommonGoods
{
     ...
     * @return array
     */
    public static function getList($keyword = '', $goods_id = '', $type = 1, $limit = null)
    {
        ...
        $pagination = null;
        $list = $query->orderBy('sort ASC')->page($pagination, 10)->limit($limit)->all();
        ...
        foreach ($list as $item) {
            ...
            $newItem['buy_goods_auth'] = GoodsAuth::create($item->sign)->checkBuyAuth($item);
            $newItem['unit'] = $item['goodsWarehouse']['unit'];
            $limitBuy = LimitBuy::create($item->sign);
            $limitBuy->goods = $item;
            $newItem['limit_buy'] = $limitBuy->getLimitBuy();
            $newList[] = $newItem;
        }

        return [
            'list' => $newList,
            'activity' => !empty($newList) ? $activity : null,
