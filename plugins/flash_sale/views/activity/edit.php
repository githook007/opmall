...
<div id="app" v-cloak>
    <el-card style="border:0"  shadow="never" body-style="background-color: #f3f3f3;padding: 10px 0 0;"
             v-loading="loading">
        <div slot="header">
            <span style="color: #3399ff;cursor: pointer;" @click="routeGo('plugin/flash_sale/mall/activity/index')"><?= \Yii::t('plugins/flash-sale', '限时抢购');?>/</span>
            <span>{{header_name}}</span>
        </div>
        <!--商品弹框-->
        <el-dialog :visible.sync="goodsVisible" width="60%" class="activity-visible">
            <div slot="title">
                <span class="goods-visible-title" style="font-size: 23px;"><?= \Yii::t('plugins/flash-sale', '选择商品');?></span>
            </div>
            <div style="margin-bottom: 27px;">
                <el-input v-model="goodsSearch.keyword"  @change="selectGood" @clear="selectGood" placeholder="<?= \Yii::t('plugins/flash-sale', '根据名称搜索');?>" clearable autocomplete="off">
                    <template slot="append">
                        <el-button @click="selectGood"><?= \Yii::t('plugins/flash-sale', '搜索');?></el-button>
                    </template>
                </el-input>
            </div>
            <el-table
                ...
                <el-table-column
                        prop="name"
                        label="<?= \Yii::t('plugins/flash-sale', '名称');?>">
            ...
            <div slot="footer" class="dialog-footer" flex="main:justify">
                ...
                <el-button type="primary" size="small" @click="addGoods()"><?= \Yii::t('plugins/flash-sale', '确定');?></el-button>
            </div>
        </el-dialog>

        <el-dialog :visible.sync="exampleVisible" width="30%" class="example-visible">
            <div slot="title">
                <span class="goods-visible-title"><?= \Yii::t('plugins/flash-sale', '查看活动预告图例');?></span>
            </div>
            ...
            <div slot="footer" class="dialog-footer" flex="main:right">
                <el-button type="primary" size="small" @click="exampleVisible = false"><?= \Yii::t('plugins/flash-sale', '我知道了');?></el-button>
            </div>
        </el-dialog>

        <el-form :model="form" label-width="100px" class="activity-body" :rules="rule" ref="form" style="background-color: #ffffff;">
            <el-tabs v-model="activeName" @tab-click="handleClick">
                <el-tab-pane label="<?= \Yii::t('plugins/flash-sale', '活动信息');?>" name="first">
                    <div class="item-cont">
                        <div class="item-title"><?= \Yii::t('plugins/flash-sale', '活动设置');?></div>
                        <div class="item-body">
                            <el-form-item  label="<?= \Yii::t('plugins/flash-sale', '活动名称');?>" prop="title">
                            ...
                            <el-form-item label="<?= \Yii::t('plugins/flash-sale', '开始时间');?>" prop="start_at">
                                <el-date-picker
                                        type="datetime"
                                        ...
                                        placeholder="<?= \Yii::t('plugins/flash-sale', '选择日期');?>">
                                </el-date-picker>
                            </el-form-item>
                            <el-form-item label="<?= \Yii::t('plugins/flash-sale', '结束时间');?>" prop="end_at">
                                <el-date-picker
                                        type="datetime"
                                        v-model="form.end_at"
                            ...
                            <el-form-item label="<?= \Yii::t('plugins/flash-sale', '活动预告');?>" style="margin-bottom: 0">
                                <div flex="dir:top" style="padding-top: 15px;">
                                    <el-radio
                                            :label="0"
                                            :disabled="notice_disabled"
                                            v-model="form.notice"
                                    ><?= \Yii::t('plugins/flash-sale', '不进行预告');?>
                                    </el-radio>
                                    <el-radio
                                            ...
                                            :disabled="notice_disabled"
                                            v-model="form.notice"
                                    ><?= \Yii::t('plugins/flash-sale', '创建后就进行活动预告');?>
                                    </el-radio>
                                    <el-radio
                                            ...
                                            v-model="form.notice"
                                    >
                                        <span><?= \Yii::t('plugins/flash-sale', '活动开始前');?></span>
                                        <el-input
                                                v-model="form.notice_hours"
                                                :disabled="notice_disabled"
                                                placeholder="<?= \Yii::t('plugins/flash-sale', '请输入大于0的整数');?>"
                                                @input.native="inputhandle"
                                                style="width: 180px"
                                        ></el-input>
                                        <span><?= \Yii::t('plugins/flash-sale', '小时进行预告');?></span>
                                    </el-radio>
                                    <div v-if="notice_hours" style="font-size: 11px;color: #ff7b7b;line-height: 1.5;padding-left: 100px;"><?= \Yii::t('plugins/flash-sale', '请输入大于0的整数');?></div>
                                </div>
                                <div style="color: #999999;font-size: 14px;">
                                    <?= \Yii::t('plugins/flash-sale', '活动预告期间');?>
                                    <span style="color: #409eff;cursor: pointer;"  @click="exampleVisible = true"><?= \Yii::t('plugins/flash-sale', '查看图例');?></span>
                                </div>
                            </el-form-item>
                        </div>
                    </div>
                </el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('plugins/flash-sale', '商品信息');?>" name="second">
                    <div class="item-cont">
                        <div class="item-title"><?= \Yii::t('plugins/flash-sale', '商品设置');?></div>
                        <div class="item-body">
                            <el-form-item label="<?= \Yii::t('plugins/flash-sale', '选择商品');?>" required>
                                <div style="margin-bottom: 15px;">
                                    <el-button  @click="getGood"><?= \Yii::t('plugins/flash-sale', '选择商品');?></el-button>
                                    <p class="add-goods" v-if="list.length === 0" @click="routeGo('mall/goods/edit')"><?= \Yii::t('plugins/flash-sale', '商城还未添加商品');?></p>
                                </div>
                            </el-form-item>
                            <el-table
                                    v-if="list.length > 0"
                            ...
                            >
                                        ...
                                        <div flex="first" >
                                            ...
                                            <div style="position: relative;width:200px;">
                                                <div style="position:absolute;">
                                                    ...
                                                    <div><?= \Yii::t('plugins/flash-sale', '库存');?>{{scope.row.goods_stock}}</div>
                                                    <div v-if="scope.row.use_attr == 1"><?= \Yii::t('plugins/flash-sale', '已选');?>{{scope.row.attr.length}}<?= \Yii::t('plugins/flash-sale', '个规格');?></div>
                                                </div>
                                            </div>
                                        </div>
                                    </template>
                                ...
                                <el-table-column
                                        label="<?= \Yii::t('plugins/flash-sale', '售价');?>">
                                    <template slot-scope="scope">
                                        {{scope.row | getPrice}}
                                    </template>
                                </el-table-column>
                                <el-table-column
                                        width="185"
                                        label="<?= \Yii::t('plugins/flash-sale', '折扣');?>">
                                    <template slot-scope="scope">
                                        <div style="height: 60px;">
                                            <div flex="main:center cross: center" style="margin-top: 8px;">
                                                ...
                                                ></el-input>
                                                <div flex="cross:center"><?= \Yii::t('plugins/flash-sale', '折');?></div>
                                            </div>
                                            <div v-if="scope.row.is_discount" style="font-size: 11px;color: #ff7b7b;"><?= \Yii::t('plugins/flash-sale', '请输入0');?></div>
                                        </div>
                                    </template>
                                </el-table-column>
                                <el-table-column
                                        width="185"
                                        label="<?= \Yii::t('plugins/flash-sale', '减钱');?>">
                                    <template slot-scope="scope">
                                        <div style="height: 60px;">
                                            <div flex="main:center cross: center" style="margin-top: 8px;">
                                                <div flex="cross:center"><?= \Yii::t('plugins/flash-sale', '减');?></div>
                                            ...
                                            ><?= \Yii::t('plugins/flash-sale', '减钱金额须小于售价');?></div>
                                        </div>
                                    </template>
                                </el-table-column>
                                <el-table-column
                                        ...
                                        width="125"
                                        label="<?= \Yii::t('plugins/flash-sale', '优惠方式');?>">
                                    <template slot-scope="scope">
                                        {{scope.row.flashSaleGoods.type == 1 ? '<?= \Yii::t('plugins/flash-sale', '折扣');?>' : scope.row.flashSaleGoods.type == 2 ? '<?= \Yii::t('plugins/flash-sale', '减钱');?>' : scope.row.flashSaleGoods.type == 3 ? '<?= \Yii::t('plugins/flash-sale', '促销价');?>' : ''}}
                                    </template>
                                </el-table-column>
                                <el-table-column
                                        width="130"
                                        label="<?= \Yii::t('plugins/flash-sale', '排序');?>">
                                    <template slot-scope="scope">
                                ...
                                <el-table-column
                                        ...
                                        fixed="right"
                                        label="<?= \Yii::t('plugins/flash-sale', '操作');?>">
                                    <template slot-scope="scope">
                                        <el-button type="text" circle size="mini" @click="edit(scope.row)" >
                                            <el-tooltip class="" effect="dark" content="<?= \Yii::t('plugins/flash-sale', '编辑');?>" placement="top">
                                                <img src="statics/img/mall/edit.png">
                                            </el-tooltip>
                                        </el-button>
                                        <el-button type="text" circle size="mini" @click="deleteGoods(scope.row)" >
                                            <el-tooltip class="" effect="dark" content="<?= \Yii::t('plugins/flash-sale', '删除');?>" placement="top">
                            ...
                            <div flex="main:justify cross:center"  class="goods_page" v-if="list.length > 0">
                                <div style="position: relative;">
                                    <el-checkbox v-model="all" @change="toggleSelection" :disabled="goods_disabled" style="margin-right: 32px;">
                                        <?= \Yii::t('plugins/flash-sale', '全选当前页');?>
                                    </el-checkbox>
                                    <el-button plain size="small" :disabled="goods_disabled" @click="batchSetting(0)"><?= \Yii::t('plugins/flash-sale', '设置折扣');?></el-button>
                                    <el-button plain size="small" :disabled="goods_disabled" @click="batchSetting(1)"><?= \Yii::t('plugins/flash-sale', '设置减钱');?></el-button>
                                </div>
                                <div>
                                    <el-pagination
                            ...
                            <div style="margin-left: 10px;">
                                <div>
                                    <el-input style="width: 132px" size="small" @change="batchInput" v-model="batch">
                                        <span slot="append">{{batchN == 0 ? '<?= \Yii::t('plugins/flash-sale', '折');?>' : '<?= \Yii::t('plugins/flash-sale', '元');?>'}}</span>
                                    </el-input>
                                </div>
                            </div>
                            <div style="margin-left: 21px;">
                                <el-button size="small" @click="is_batch = false"><?= \Yii::t('plugins/flash-sale', '取消');?></el-button>
                            </div>
                            <div style="margin-left: 7px;">
                                <el-button size="small" @click="sureBatch"  type="primary"><?= \Yii::t('plugins/flash-sale', '确定');?></el-button>
                            </div>
                        </div>
                    </div>
                </el-tab-pane>
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                rule: {
                    title: [
                        {required: true, message: '<?= \Yii::t('plugins/flash-sale', '请输入活动名称');?>', trigger: 'change'}
                    ],
                    start_at: [
                        {required: true, message: '<?= \Yii::t('plugins/flash-sale', '请输入开始时间');?>', trigger: 'change'}
                    ],
                    end_at: [
                        {required: true, message: '<?= \Yii::t('plugins/flash-sale', '请输入结束时间');?>', trigger: 'change'}
                    ]
                },
                activeName: 'first',
                list: [],
                ...
                header_name: '<?= \Yii::t('plugins/flash-sale', '新建活动');?>',
                notice_hours: false,
                sort_goods_id: null
            };
        },
        methods: {
                    ...
                    this.$refs[formName].validate(valid => {
                        if (valid) {
                            ...
                            request({
                                ...
                                this.btnLoading = false;
                                if (e.data.code === 0) {
                                    this.$message.success('<?= \Yii::t('plugins/flash-sale', '保存成功');?>');
                                } else {
                                    this.$message.error(e.data.msg);
                                }
                            });
            ...
            deleteGoods(row) {
                this.$confirm('<?= \Yii::t('plugins/flash-sale', '删除商品');?>', '<?= \Yii::t('plugins/flash-sale', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('plugins/flash-sale', '确定');?>',
                    cancelButtonText: '<?= \Yii::t('plugins/flash-sale', '取消');?>',
                    type: 'warning'
                }).then(() => {
                    this.tabLoading = true;
                    let del = [{
                    ...
                    this.$message({
                        type: 'info',
                        message: '<?= \Yii::t('plugins/flash-sale', '已取消');?>'
                    });
                });
            },
            ...
            changeSortSubmit(row) {
                if (row.sort < 0) {
                    this.$message.warning('<?= \Yii::t('plugins/flash-sale', '排序值不能小于0');?>');
                } else {
                    this.sort_goods_id = 0;
                    for (let i = 0; i < this.editList.length; i++) {
                        if (this.editList[i].id == row.id) {
            ...
            activeName: {
                handler(data) {
                    if (data === 'second' && !this.form.id) {
                        this.$confirm('<?= \Yii::t('plugins/flash-sale', '请先保存活动信息');?>', '<?= \Yii::t('plugins/flash-sale', '提示');?>', {
                            confirmButtonText: '<?= \Yii::t('plugins/flash-sale', '确定');?>',
                            cancelButtonText: '<?= \Yii::t('plugins/flash-sale', '取消');?>',
                            type: 'warning'
                        }).then(() => {
        ...
        created() {
            ...
            this.form.id = id;
            if (id) {
                this.header_name = '<?= \Yii::t('plugins/flash-sale', '编辑活动');?>';
            }
            ...
            if (status === '3') {
                ...
                this.header_name = '<?= \Yii::t('plugins/flash-sale', '查看活动');?>';
            } else if (status === '2') {
                this.goods_disabled = true;
                this.start_at_disabled = true;
            }
