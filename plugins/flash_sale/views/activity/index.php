...
<div id="app" v-cloak>
    <el-card  class="box-card" shadow="never"
        ...
        <el-dialog :visible.sync="activityVisible" width="50%" class="activity-visible">
            <div slot="title">
                <span class="activity-visible-title"><?= \Yii::t('plugins/flash-sale', '活动商品');?></span>
                <span><?= \Yii::t('plugins/flash-sale', '共计');?>{{activityNumber}}<?= \Yii::t('plugins/flash-sale', '款商品');?></span>
            </div>
            <div style="margin-bottom: 25px;">
                <el-input @change="searchGetGoods" @clear="searchGetGoods" v-model="activityGoods.keyword" placeholder="<?= \Yii::t('plugins/flash-sale', '根据名称搜索');?>" autocomplete="off" clearable>
                    <template slot="append">
                        <el-button @click="searchGetGoods"><?= \Yii::t('plugins/flash-sale', '搜索');?></el-button>
                    </template>
                </el-input>
            </div>
            <template>
                <el-table
                    ...
                    <el-table-column
                            ...
                            width="180"
                            props="goods_stock"
                            label="<?= \Yii::t('plugins/flash-sale', '剩余库存');?>">
                        <template slot-scope="scope">
            ...
            <div slot="footer" class="dialog-footer" flex="main:justify">
                ...
                <el-button type="primary" size="small" @click="saveItem()"><?= \Yii::t('plugins/flash-sale', '确定');?></el-button>
            </div>
        </el-dialog>
        <div slot="header">
            <span><?= \Yii::t('plugins/flash-sale', '限时抢购');?></span>
            <el-button style="float: right; margin: -5px 0" type="primary" size="small" @click="edit"><?= \Yii::t('plugins/flash-sale', '新建活动');?></el-button>
        </div>
        <div class="table-body">
            <app-search :is-show-cat="false" :tabs="tabs" date-label="<?= \Yii::t('plugins/flash-sale', '活动时间');?>" :new-search="search" place-holder="<?= \Yii::t('plugins/flash-sale', '请输入活动名称搜索');?>" @to-search="toSearch" date_label="<?= \Yii::t('plugins/flash-sale', '活动时间');?>" :new-active-name="activeName"></app-search>
            <div class="shelves">
                <!--                <el-button size="mini" @click="operatingActivity(0)">上架</el-button>-->
                <!--                <el-button size="mini" @click="operatingActivity(1)">下架</el-button>-->
                <el-button size="mini" @click="operatingActivity(2)"><?= \Yii::t('plugins/flash-sale', '批量删除');?></el-button>
            </div>
            <el-table
                    ref="multipleTable"
                    :data="data_list"
                ...
                <el-table-column
                        prop="title"
                        label="<?= \Yii::t('plugins/flash-sale', '活动名称');?>"
                        width="250">
                </el-table-column>
                <el-table-column
                        prop="flashSaleGoods"
                        width="180"
                        label="<?= \Yii::t('plugins/flash-sale', '商品款数');?>">
                    <template slot-scope="scope">
                ...
                <el-table-column
                        ...
                        width="180"
                        label="<?= \Yii::t('plugins/flash-sale', '订单实付金额');?>">
                </el-table-column>
                <el-table-column
                        prop="statics.order_num"
                        width="180"
                        label="<?= \Yii::t('plugins/flash-sale', '支付订单数');?>">
                </el-table-column>
                <el-table-column
                        prop="statics.user_num"
                        width="180"
                        label="<?= \Yii::t('plugins/flash-sale', '参与人数');?>">
                </el-table-column>
                <el-table-column
                        label="<?= \Yii::t('plugins/flash-sale', '活动时间');?>"
                        width="180"
                        show-overflow-tooltip>
                    <template slot-scope="scope">
                        <template>
                            <p>{{scope.row.start_at}} <?= \Yii::t('plugins/flash-sale', '至');?></p>
                ...
                <el-table-column
                        prop="time_status"
                        label="<?= \Yii::t('plugins/flash-sale', '活动状态');?>"
                        width="120">
                    <template slot-scope="scope">
                        <el-tag  :type="scope.row.time_status == 1 ? 'info' : scope.row.time_status == 2 ? 'success' : scope.row.time_status == 3 ? 'info' : scope.row.time_status == 0 ? 'warning' : ''">
                            {{scope.row.time_status == 1 ? '<?= \Yii::t('plugins/flash-sale', '未开始');?>' : scope.row.time_status == 2 ? '<?= \Yii::t('plugins/flash-sale', '进行中');?>' : scope.row.time_status == 3 ? '<?= \Yii::t('plugins/flash-sale', '已结束');?>' : scope.row.time_status == 0 ? '<?= \Yii::t('plugins/flash-sale', '下架中');?>' : ''}}
                        </el-tag>
                    </template>
                </el-table-column>
                    ...
                    <template slot-scope="scope">
                        <template v-if="scope.row.time_status != 3">
                            <el-button type="text" circle size="mini" @click="edit(scope.row, 1)">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/flash-sale', '编辑活动');?>" placement="top">
                                    <img src="statics/img/plugins/edit.png" alt="">
                                </el-tooltip>
                            </el-button>
                            <el-button type="text" circle size="mini" @click="edit(scope.row, 2)" >
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/flash-sale', '编辑商品');?>" placement="top">
                        ...
                        <template v-else>
                            <el-button type="text" circle size="mini" @click="lookOver(scope.row)">
                                <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/flash-sale', '查看活动');?>" placement="top">
                        ...
                        <el-button @click="destroy(scope.row, scope.$index)" type="text" circle size="mini">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/flash-sale', '删除');?>" placement="top">
            ...
            <div flex="box:last cross:center" style="margin-top: 20px;">
                <div style="visibility: hidden;">
                    <el-button plain type="primary" size="small"><?= \Yii::t('plugins/flash-sale', '批量操作1');?></el-button>
                    <el-button plain type="primary" size="small"><?= \Yii::t('plugins/flash-sale', '批量操作2');?></el-button>
                </div>
                <div>
                    <el-pagination
                            v-if="page_count > 0"
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                tabs: [
                    {
                        name: '<?= \Yii::t('plugins/flash-sale', '全部');?>',
                        value: '-1'
                    },
                    {
                        name: '<?= \Yii::t('plugins/flash-sale', '未开始');?>',
                        value: '0'
                    },
                    {
                        name: '<?= \Yii::t('plugins/flash-sale', '进行中');?>',
                        value: '1'
                    },
                    {
                        name: '<?= \Yii::t('plugins/flash-sale', '已结束');?>',
                        value: '2'
                    },
                    {
                        name: '<?= \Yii::t('plugins/flash-sale', '下架中');?>',
                        value: '3'
                    },
                ],
                activeName: '-1',
        ...
        methods: {
            ...
            destroy(data) {
                this.$confirm('<?= \Yii::t('plugins/flash-sale', '删除该条数据');?>', '<?= \Yii::t('plugins/flash-sale', '提示');?>', {
            ...
            takeOff(e) {
                ...
                let str = `<?= \Yii::t('plugins/flash-sale', '确定');?>${data === 'up' ? '<?= \Yii::t('plugins/flash-sale', '上架');?>' : '<?= \Yii::t('plugins/flash-sale', 'z115');?>'}<?= \Yii::t('plugins/flash-sale', '此活动');?>`;
                this.$confirm(str, '<?= \Yii::t('plugins/flash-sale', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('plugins/flash-sale', '确定');?>',
                    cancelButtonText: '<?= \Yii::t('plugins/flash-sale', '取消');?>',
                    type: 'warning'
                }).then(() => {
                    ...
                    this.$message({
                        type: 'info',
                        message: '<?= \Yii::t('plugins/flash-sale', '已取消操作');?>'
                    });
                });
            },
            operatingActivity() {
                if (this.selectionList.length !== 0) {
                    let type = 'del';
                    let str = `<?= \Yii::t('plugins/flash-sale', '选中');?>${this.selectionList.length}<?= \Yii::t('plugins/flash-sale', '个活动');?>`;
                    this.$confirm(str, '<?= \Yii::t('plugins/flash-sale', '提示');?>', {
                        confirmButtonText: '<?= \Yii::t('plugins/flash-sale', '确定');?>',
                        cancelButtonText: '<?= \Yii::t('plugins/flash-sale', '取消');?>',
                        type: 'warning'
                    ...
                    this.$message.warning('<?= \Yii::t('plugins/flash-sale', '请先选择活动');?>');
                }
            },
