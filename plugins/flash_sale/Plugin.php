...
class Plugin extends \app\plugins\Plugin
{
    ...
    public function getMenus()
    {
        return [
            [
                'name' => \Yii::t('plugins/flash-sale', '设置'),
                'icon' => 'el-icon-star-on',
                'route' => 'plugin/flash_sale/mall/setting',
            ],
            [
                'name' => \Yii::t('plugins/flash-sale', '限时抢购'),
                'icon' => 'el-icon-star-on',
    ...
    {
        return \Yii::t('plugins/flash-sale', '限时抢购');
    }
    ...
    public function getPickLink()
    {
        ...
        return [
            ...
            [
                'key' => 'flash_sale',
                'name' => \Yii::t('plugins/flash-sale', '限时抢购详情'),
                'open_type' => '',
                'icon' => $iconBaseUrl . '/icon-flash-sale-detail.png',
                'value' => '/plugins/flash_sale/goods/goods',
                'params' => [
                    [
                        'key' => 'id',
                        'value' => '',
                        'desc' => \Yii::t('plugins/flash-sale', 'a104'),
                        'is_required' => true,
                        'data_type' => 'number',
                        'page_url' => 'plugin/flash_sale/mall/activity/index',
                        'page_url_text' => \Yii::t('plugins/flash-sale', '活动商品列表'),
                    ],
                ],
    ...
    public function getHomePage($type, $limit = null)
    {
        if ($type == 'mall') {
            ...
            $data = CommonGoods::getList('', '', 4, $limit);
            if (!empty($data)) {
                if (is_array($data['list'])) {
                    ...
                    foreach ($data['list'] as $item) {
                        ...
                        $item = ArrayHelper::filter($item, [
                            ...
                            'is_level', 'level_price', 'vip_card_appoint', 'price_content', 'page_url', 'price', 'sales', 'sign'
                            , 'percentage'
                        ]);
    ...
    public function getStatisticsMenus($bool = true)
    {
        return [
            ...
            'name' => $bool ? $this->getDisplayName() : \Yii::t('plugins/flash-sale', '插件统计'),
            'key' => $this->getName(),
    ...
    public function checkGoods($goods, $item)
    {
        ...
        if (!$flashGoods) {
            throw new OrderException(\Yii::t('plugins/flash-sale', '商品不存在'));
        }
        ...
        if (!$activity) {
            throw new OrderException(\Yii::t('plugins/flash-sale', '当前活动已结束'));
        }

        if ($activity->start_at > mysql_timestamp()) {
            throw new OrderException(\Yii::t('plugins/flash-sale', '限时抢购活动尚未开始'));
        }
        ...
        if ($goods->confine_order_count != -1 && $buyCount >= $goods->confine_order_count) {
            throw new OrderException(\Yii::t('plugins/flash-sale', '超出购买限单').'(' . $goods->confine_order_count . ')'.\Yii::t('plugins/flash-sale', '次数'));
        }
    }
    ...
    public function pluginDiscount($goodsItem)
    {
        ...
        if (empty($flashSaleGoodsAttr) || !isset($flashSaleGoodsAttr->type)) {
            throw new OrderException(\Yii::t('plugins/flash-sale', '商品异常'));
        }
        ...
        if ($flashSaleGoodsAttr->type == 1) {
            ...
            throw new OrderException(\Yii::t('plugins/flash-sale', '优惠类型错误'));
        }
        if ($flashSubPrice != 0) {
            // 减去限时抢购优惠金额
    ...
    public function pluginDiscountData($mchItem)
    {
        ...
        $mchItem['insert_rows'][] = [
            'title' => \Yii::t('plugins/flash-sale', '限时抢购优惠'),
            'value' => '-￥' . $totalSubPrice,
        ...
        if (!$activity) {
            throw new OrderException(\Yii::t('plugins/flash-sale', '活动不存在'));
        }
    ...
    public function getOrderInfo($orderId, $order)
    {
        ...
        if ($flash) {
            return [
                'discount_list' => [
                    'flash_sale_discount' => [
                        'label' => \Yii::t('plugins/flash-sale', '限时抢购优惠'),
    ...
    public function getDiscount($goods)
    {
        ...
        if (!$flashGoods) {
            throw new \Exception(\Yii::t('plugins/flash-sale', '商品不存在'));
        }
        if ($flashGoods->status != 1) {
            throw new \Exception(\Yii::t('plugins/flash-sale', '商品未上架'));
        }
