    ...
    <el-card v-loading="loading" style="border:0" shadow="never" body-style="background-color: #f3f3f3;padding: 0 0;">
        <el-form v-model="loading" :model="form" label-width="150px" ref="form">
            <el-tabs v-model="activeName">
                <el-tab-pane label="<?= \Yii::t('plugins/bonus', '基本');?>" name="first">
                    <div class="form-body">
                        <el-form-item class="switch" label="<?= \Yii::t('plugins/bonus', '团队分红');?>" prop="bonusSwitch">
                            <el-switch v-model="bonusSwitch" :active-value="1" :inactive-value="0"></el-switch>
                        </el-form-item>
                        <el-form-item v-if="bonusSwitch == 1" prop="bonus_rate">
                            <template slot='label'>
                                <span><?= \Yii::t('plugins/bonus', '分红比例');?></span>
                                <el-tooltip effect="dark" content="<?= \Yii::t('plugins/bonus', '分红');?>=<?= \Yii::t('plugins/bonus', '商品实付金额');?>*<?= \Yii::t('plugins/bonus', '分红比例');?>"
                                        placement="top">
                        ...
                        <el-form-item v-if="bonusSwitch == 1" class="switch" label="<?= \Yii::t('plugins/bonus', '成为队长条件');?>" prop="become_type">
                            <el-radio-group @change="tabType" v-model="form.become_type">
                                <el-radio :label="0"><?= \Yii::t('plugins/bonus', '申请');?></el-radio>
                                <el-radio :label="3"><?= \Yii::t('plugins/bonus', '下线人数');?></el-radio>
                                <el-radio :label="4"><?= \Yii::t('plugins/bonus', '下线分销商数');?></el-radio>
                                <el-radio :label="1"><?= \Yii::t('plugins/bonus', '累计佣金总额');?>
                                    <el-tooltip effect="dark" content="<?= \Yii::t('plugins/bonus', '累计佣金总额');?>=<?= \Yii::t('plugins/bonus', '可提现佣金金额');?>+<?= \Yii::t('plugins/bonus', '已提现佣金金额');?>"
                                            placement="top">
                                        <i class="el-icon-info"></i>
                                    </el-tooltip>
                                </el-radio>
                                <el-radio :label="2"><?= \Yii::t('plugins/bonus', '已提现佣金总额');?></el-radio>
                            </el-radio-group>
                        </el-form-item>
                        <el-form-item class="switch" v-if="bonusSwitch == 1 && form.become_type == 3" label="<?= \Yii::t('plugins/bonus', '下线人数');?>" prop="title">
                            <el-input type="number" size="small" style="width: 590px;" v-model="form.condition" autocomplete="off"></el-input>
                        </el-form-item>
                        <el-form-item class="switch" v-if="bonusSwitch == 1 && form.become_type == 4" label="<?= \Yii::t('plugins/bonus', '下线分销商数');?>" prop="title">
                            <el-input type="number" size="small" style="width: 590px;" v-model="form.condition" autocomplete="off"></el-input>
                        </el-form-item>
                        <el-form-item class="switch" v-if="bonusSwitch == 1 && form.become_type == 1" label="<?= \Yii::t('plugins/bonus', '累计佣金总额');?>" prop="title">
                            <el-input type="number" size="small" style="width: 590px;" v-model="form.condition" autocomplete="off"></el-input>
                        </el-form-item>
                        <el-form-item class="switch" v-if="bonusSwitch == 1 && form.become_type == 2" label="<?= \Yii::t('plugins/bonus', '已提现佣金总额');?>" prop="title">
                            <el-input type="number" size="small" style="width: 590px;" v-model="form.condition" autocomplete="off"></el-input>
                        </el-form-item>
                        <el-form-item v-if="bonusSwitch == 1" class="switch" label="<?= \Yii::t('plugins/bonus', '显示申请协议');?>" prop="showTreaty">
                            <el-switch v-model="showTreaty" :active-value="1" :inactive-value="0"></el-switch>
                        </el-form-item>
                        <el-form-item v-if="bonusSwitch == 1 && showTreaty == 1" class="switch" label="<?= \Yii::t('plugins/bonus', '协议名称');?>" prop="agreement_title">
                            <el-input size="small" style="width: 590px;" v-model="form.agreement_title" autocomplete="off"></el-input>
                        </el-form-item>
                        <el-form-item v-show="bonusSwitch == 1 && showTreaty == 1" class="switch" label="<?= \Yii::t('plugins/bonus', '协议内容');?>" prop="agreement_content">
                            <app-rich-text style="width: 590px;" v-model="form.agreement_content"></app-rich-text>
                        </el-form-item>
                    </div>
                </el-tab-pane>
                <el-tab-pane v-if="bonusSwitch == 1" label="<?= \Yii::t('plugins/bonus', '结算');?>" name="second">
                    <div class="form-body">
                        <el-form-item label-width="220px" class="switch" label="提现方式" prop="is_share">
                            <el-checkbox-group v-model="pay_type">
                                <el-checkbox label="auto"><?= \Yii::t('plugins/bonus', '自动打款');?></el-checkbox>
                                <el-checkbox label="wechat"><?= \Yii::t('plugins/bonus', '微信线下转账');?></el-checkbox>
                                <el-checkbox label="alipay"><?= \Yii::t('plugins/bonus', '支付宝线下转账');?></el-checkbox>
                                <el-checkbox label="bank"><?= \Yii::t('plugins/bonus', '银行卡线下转账');?></el-checkbox>
                                <el-checkbox label="balance"><?= \Yii::t('plugins/bonus', '余额提现');?></el-checkbox>
                            </el-checkbox-group>
                        </el-form-item>
                        <el-form-item label-width="220px" prop="min_money">
                            <template slot='label'>
                                <span><?= \Yii::t('plugins/bonus', '提现门槛金额');?></span>
                                <el-tooltip effect="dark" content="<?= \Yii::t('plugins/bonus', '可提现金额到达此金额时，方可申请提现');?>"
                                        placement="top">
                                    <i class="el-icon-info"></i>
                                </el-tooltip>
                            </template>
                            <el-input type="number" size="small" style="width: 590px;" v-model="form.min_money" autocomplete="off">
                                <template slot="append"><?= \Yii::t('plugins/bonus', '元');?></template>
                            </el-input>
                        </el-form-item>
                        <el-form-item label-width="220px" prop="cash_service_charge">
                            <template slot='label'>
                                <span><?= \Yii::t('plugins/bonus', '分红提现手续费');?></span>
                                <el-tooltip effect="dark" content="<?= \Yii::t('plugins/bonus', '申请提现金额');?>-<?= \Yii::t('plugins/bonus', '申请提现金额');?>x<?= \Yii::t('plugins/bonus', '手续费');?>=<?= \Yii::t('plugins/bonus', '实际到账金额');?>；<?= \Yii::t('plugins/bonus', '设置0，则不扣除手续费');?>"
                                        placement="top">
                                    <i class="el-icon-info"></i>
                                </el-tooltip>
                            </template>
                        ...
                        <el-form-item label-width="220px">
                            <template slot='label'>
                                <span><?= \Yii::t('plugins/bonus', '免手续费提现金额区间');?></span>
                                <el-tooltip effect="dark" placement="top">
                                    <div slot="content" style="text-align: center"><?= \Yii::t('plugins/bonus', '单笔申请提现金额在此区间时，不扣除手续费');?>；<br/><?= \Yii::t('plugins/bonus', '结束金额必须大于起始金额');?>；<br/><?= \Yii::t('plugins/bonus', '均不填，则不设置免手续提现金额');?></div>
                                    <i class="el-icon-info"></i>
                                </el-tooltip>
                            </template>
                            <div flex="dir:left">
                                <div style="margin-right: 10px"><?= \Yii::t('plugins/bonus', '起始金额');?></div>
                                <div>
                                ...
                                <div style="margin-right: 10px"><?= \Yii::t('plugins/bonus', '结束金额');?></div>
                                <div>
                ...
                <el-tab-pane v-if="bonusSwitch == 1" label="<?= \Yii::t('plugins/bonus', '页面');?>" name="three">
                    <div class="title">
                        <span><?= \Yii::t('plugins/bonus', '头部banner图片');?></span>
                    </div>
                    <div class="form-body">
                        <div flex="dir:left">
                            <div class="mobile">
                            ...
                            <div>
                                <el-form-item label="<?= \Yii::t('plugins/bonus', '申请页面背景图片');?>" prop="top_pic_url">
                                    <div style="position: relative">
                                        <app-attachment :multiple="false" :max="1" @selected="topPicUrl">
                                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/bonus', '建议尺寸');?>:750*360" placement="top">
                                                <el-button size="mini"><?= \Yii::t('plugins/bonus', '选择文件');?></el-button>
                                            </el-tooltip>
                                        </app-attachment>
                                        ...
                                        <el-button size="mini" @click="resetImg(2,'top')" class="reset" type="primary"><?= \Yii::t('plugins/bonus', '恢复默认');?></el-button>
                                    </div>
                                </el-form-item>
                                <el-form-item label="<?= \Yii::t('plugins/bonus', '申请页面底部图片');?>" prop="bottom_pic_url">
                                    <div style="position: relative">
                                        ...
                                        <el-button size="mini" @click="resetImg(2,'bottom')" class="reset" type="primary"><?= \Yii::t('plugins/bonus', '恢复默认');?></el-button>
                                    </div>
                                </el-form-item>
                            </div>
                        </div>
                    ...
                    <div class="title" style="margin-top: 10px">
                        <span><?= \Yii::t('plugins/bonus', '自定义文字');?></span>
                    </div>
                    <div class="form-body" style="padding: 40px;display: flex;">
                        <div class='left-setting-menu'>
                            <el-form-item label-width="140px" :class='active_setting == "1" ? "active":""' @click.native='chooseSetting("1")' label="<?= \Yii::t('plugins/bonus', '入口位置');?>" prop="is_purchase_frame">
                            </el-form-item>
                            <el-form-item v-if="form.become_type != 0" label-width="140px" v-if="" :class='active_setting == "7" ? "active":""' @click.native='chooseSetting("7")' label="<?= \Yii::t('plugins/bonus', '未达条件页');?>" prop="become_name">
                            </el-form-item>
                            <el-form-item label-width="140px" :class='active_setting == "2" ? "active":""' @click.native='chooseSetting("2")' label="<?= \Yii::t('plugins/bonus', '页面标题');?>" prop="is_comment">
                            </el-form-item>
                            <el-form-item label-width="140px" :class='active_setting == "3" ? "active":""' @click.native='chooseSetting("3")' label="<?= \Yii::t('plugins/bonus', '主页');?>" prop="is_sales">
                            </el-form-item>
                            <el-form-item label-width="140px" :class='active_setting == "4" ? "active":""' @click.native='chooseSetting("4")' label="<?= \Yii::t('plugins/bonus', '数据统计页');?>" prop="is_member_price">
                            </el-form-item>
                            <el-form-item label-width="140px" :class='active_setting == "5" ? "active":""' @click.native='chooseSetting("5")' label="<?= \Yii::t('plugins/bonus', '主页');?>-<?= \Yii::t('plugins/bonus', '订单页');?>" prop="is_share_price">
                            </el-form-item>
                            <el-form-item label-width="140px" :class='active_setting == "6" ? "active":""' @click.native='chooseSetting("6")' label="<?= \Yii::t('plugins/bonus', '主页');?>-<?= \Yii::t('plugins/bonus', '队员页');?>" prop="is_mobile_auth">
                            </el-form-item>
                        </div>
                        <div style='background-color: #F3F5F6;padding: 30px;border-radius: 10px;' :class='active_setting == "1" ? "no-radius":""' flex="dir:left">
                            <div v-if='active_setting == "1"' class="item-img">
                                ...
                                <div class="enter" flex="dir:left">
                                    <div class="enter-item">
                                        ...
                                        <div><?= \Yii::t('plugins/bonus', '分销佣金');?></div>
                                        <div><span>3500</span><?= \Yii::t('plugins/bonus', '元');?></div>
                                    </div>
                                    <div class="enter-item">
                                        <img src="statics/img/mall/share-custom/img-share-order.png" alt="">
                                        <div><?= \Yii::t('plugins/bonus', '分销佣金');?></div>
                                        <div><span>26</span><?= \Yii::t('plugins/bonus', '元');?></div>
                                    </div>
                                    <div class="enter-item">
                                        <img src="statics/img/mall/share-custom/img-share-cash.png" alt="">
                                        <div><?= \Yii::t('plugins/bonus', '提现明细');?></div>
                                        <div><span>10</span><?= \Yii::t('plugins/bonus', '元');?></div>
                                    </div>
                                    <div class="enter-item">
                                        <img src="statics/img/mall/share-custom/img-share-team.png" alt="">
                                        <div><?= \Yii::t('plugins/bonus', '我的团队');?></div>
                                    ...
                                    <div class="enter-item">
                                        ...
                                        <div>{{form.form.entry_bonus?form.form.entry_bonus:"<?= \Yii::t('plugins/bonus', '团队分红');?>"}}</div>
                                    </div>
                                </div>
                            </div>
                            <div v-if='active_setting == "2" || active_setting == "3"' class="item-img">
                                <div class="index">
                                    ...
                                    <div class="title-txt">{{form.form.title?form.form.title:"<?= \Yii::t('plugins/bonus', '团队分红');?>"}}</div>
                                    <div class="header-item">
                                        <div flex="main:justify">
                                            <div flex="cross:center">
                                                ...
                                                <div><?= \Yii::t('plugins/bonus', '潘先生');?></div>
                                            </div>
                                            <div flex="cross:center" class="rate">
                                                <span>{{form.form.rate?form.form.rate:"<?= \Yii::t('plugins/bonus', '提成比例');?>"}}5%</span>
                                        ...
                                        <div flex="dir:left" style="margin-top: 20px;">
                                            <div class="price-item">
                                                <div>
                                                    <span style="font-size: 20px">2500.20</span><?= \Yii::t('plugins/bonus', '元');?>
                                                </div>
                                                <div>
                                                    <span>{{form.form.total_bonus?form.form.total_bonus:"<?= \Yii::t('plugins/bonus', '累计分红金额');?>"}}</span>
                                                    <span>></span>
                                                </div>
                                            </div>
                                            <div>
                                                <div>
                                                    <span style="font-size: 20px">5136.20</span><?= \Yii::t('plugins/bonus', '元');?>
                                                </div>
                                                <div>
                                                    <span>{{form.form.expect_bonus?form.form.expect_bonus:"<?= \Yii::t('plugins/bonus', '预计分红金额');?>"}}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div flex="main:justify cross:center" class="bonus-item">
                                        <div style="color: #999999">{{form.form.cashd_bonus?form.form.cashd_bonus:"<?= \Yii::t('plugins/bonus', '已提现分红');?>"}}</div>
                                        <div flex="cross:center">
                                            <div>
                                                <span style="font-size: 16px">8652.36</span><?= \Yii::t('plugins/bonus', '元');?>
                                    ...
                                    <div flex="main:justify cross:center" class="bonus-item" style="height: 70px;">
                                        <div>
                                            ...
                                            <div style="color: #999999">{{form.form.can_cash_bonus?form.form.can_cash_bonus:"<?= \Yii::t('plugins/bonus', '可提现分红');?>"}}</div>
                                        </div>
                                        <div class="cash-btn"><?= \Yii::t('plugins/bonus', '提现');?></div>
                                    </div>
                                    <div flex="main:justify cross:center" class="bonus-item" style="height: 100px;">
                                        ...
                                        <div>
                                            <div style="height: 50px;width: 275px" flex="main:justify cross:center">
                                                <div>{{form.form.orders?form.form.orders:"<?= \Yii::t('plugins/bonus', '订单');?>"}}</div>
                                                <div style="color: #9999999;">></div>
                                            </div>
                                            <div style="height: 50px;border-top: 1px solid #e2e2e2;width: 275px" flex="main:justify cross:center">
                                                <div>{{form.form.members?form.form.members:"<?= \Yii::t('plugins/bonus', '队员');?>"}}</div>
                                                <div style="color: #9999999">></div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div v-if='active_setting == "4"' class="item-img">
                                ...
                                <div class="title-txt">{{form.form.statistic_bonus?form.form.statistic_bonus:"<?= \Yii::t('plugins/bonus', '分红统计');?>"}}</div>
                                <div class="status-txt" flex="main:justify">
                                    <div>昨日{{form.form.statics_text ? form.form.statics_text : "<?= \Yii::t('plugins/bonus', '分红');?>"}}</div>
                                    <div>7日{{form.form.statics_text ? form.form.statics_text : "<?= \Yii::t('plugins/bonus', '分红');?>"}}<?= \Yii::t('plugins/bonus', '总计');?></div>
                                    <div>月{{form.form.statics_text ? form.form.statics_text : "<?= \Yii::t('plugins/bonus', '分红');?>"}}<?= \Yii::t('plugins/bonus', '总计');?></div>
                                </div>
                                <div class="tab-txt" flex="main:center">
                                    <div class="tab-list" flex="main:center">
                                        <div>昨日{{form.form.statics_text ? form.form.statics_text : "<?= \Yii::t('plugins/bonus', '分红');?>"}}</div>
                                        <div style="background-color: #ff4544;color:#fff">7日{{form.form.statics_text ? form.form.statics_text : "<?= \Yii::t('plugins/bonus', '分红');?>"}}</div>
                                        <div>月{{form.form.statics_text ? form.form.statics_text : '<?= \Yii::t('plugins/bonus', '分红');?>'}}</div>
                                    </div>
                                </div>
                            </div>
                            <div v-if='active_setting == "5"' class="item-img">
                                ...
                                <div class="title-txt">{{form.form.orders?form.form.orders:"<?= \Yii::t('plugins/bonus', '分销订单');?>"}}</div>
                                <div class="price-text">
                                    <div v-for="item in [1,2,3,4,5]">{{form.form.price_text ? form.form.price_text : "<?= \Yii::t('plugins/bonus', '分红金额');?>"}}
                            ...
                            <div v-if='active_setting == "6"' class="item-img">
                                ...
                                <div class="title-txt">{{form.form.members?form.form.members:"<?= \Yii::t('plugins/bonus', '队员');?>"}}</div>
                                <div class="title-txt" style="top: 145px;font-weight: 400;font-size: 14px;color: #666;align-items: center" flex="main:center">
                                    <span>{{form.form.offer_text?form.form.offer_text:"<?= \Yii::t('plugins/bonus', '贡献分红金额');?>"}}</span>
                                ...
                                <div class="offer-text">
                                    <div v-for="item in [1,2,3,4,5,6,7]">
                                        <span style="display: inline-block;height: 18px;padding: 0 5px;text-align: center;line-height: 18px;background-color: #efeff4">{{form.form.offer_text ? form.form.offer_text : "<?= \Yii::t('plugins/bonus', '贡献分红金额');?>"}}</span>
                            ...
                            <div v-if='active_setting == "7"' class="item-img">
                                ...
                                <div v-if="form.become_type == 3" class="become-txt"><?= \Yii::t('plugins/bonus', '还差');?><span style="font-size: 19px;color: #ff8f17">XXX</span>个{{form.form.become_name?form.form.become_name:"<?= \Yii::t('plugins/bonus', '下线');?>"}}<?= \Yii::t('plugins/bonus', '成为队长');?></div>
                                <div v-if="form.become_type == 4" class="become-txt"><?= \Yii::t('plugins/bonus', '还差');?><span style="font-size: 19px;color: #ff8f17">XXX</span>个{{form.form.become_name?form.form.become_name:"<?= \Yii::t('plugins/bonus', '下线分销商');?>"}}<?= \Yii::t('plugins/bonus', '成为队长');?></div>
                                <div v-if="form.become_type == 1" class="become-txt"><?= \Yii::t('plugins/bonus', '还差');?><span style="font-size: 19px;color: #ff8f17">XXX</span>{{form.form.become_name?form.form.become_name:"<?= \Yii::t('plugins/bonus', '累计佣金');?>"}}<?= \Yii::t('plugins/bonus', '成为队长');?></div>
                                <div v-if="form.become_type == 2" class="become-txt"><?= \Yii::t('plugins/bonus', '还差');?><span style="font-size: 19px;color: #ff8f17">XXX</span>{{form.form.become_name?form.form.become_name:"<?= \Yii::t('plugins/bonus', '已提现佣金');?>"}}<?= \Yii::t('plugins/bonus', '成为队长');?></div>
                                <div v-if="form.become_type == 3" class="become-about">已有{{form.form.become_name?form.form.become_name:"<?= \Yii::t('plugins/bonus', '下线');?>"}}XXX人</div>
                                <div v-if="form.become_type == 4" class="become-about">已有{{form.form.become_name?form.form.become_name:"<?= \Yii::t('plugins/bonus', '下线分销商');?>"}}XXX人</div>
                                <div v-if="form.become_type == 1" class="become-about">已有{{form.form.become_name?form.form.become_name:"<?= \Yii::t('plugins/bonus', '累计佣金');?>"}}￥XXX</div>
                                <div v-if="form.become_type == 2" class="become-about">已有{{form.form.become_name?form.form.become_name:"<?= \Yii::t('plugins/bonus', '已提现佣金');?>"}}￥XXX</div>
                            </div>
                            <div class="item-img" style="border: 0;width: 570px;margin-left: 20px;">
                                <div style="height: 32px;margin-left: 40px;margin-bottom: 20px" flex="dir:left cross:center" v-if="active_setting == 1">
                                    <div class="default"><?= \Yii::t('plugins/bonus', '团队分红');?></div>
                                ...
                                <div style="height: 32px;margin-left: 40px;margin-bottom: 20px" flex="dir:left cross:center" v-if="active_setting == 7">
                                    <div v-if="form.become_type == 3" class="default"><?= \Yii::t('plugins/bonus', '下线');?></div>
                                    <div v-if="form.become_type == 4" class="default"><?= \Yii::t('plugins/bonus', '下线分销商');?></div>
                                    <div v-if="form.become_type == 1" class="default"><?= \Yii::t('plugins/bonus', '累计佣金');?></div>
                                    <div v-if="form.become_type == 2" class="default"><?= \Yii::t('plugins/bonus', '已提现佣金');?></div>
                                    <app-image width="12px" height="12px" mode="aspectFill" :src="customize_pic"></app-image>
            ...
            <el-button class="button-item" type="primary" size="small" :loading=submitLoading @click="submit"><?= \Yii::t('plugins/bonus', '保存');?></el-button>
        </el-form>
    </el-card>
</div>  
