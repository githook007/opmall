    ...
    <el-card shadow="never" v-loading="loading" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <span style="color: #409EFF;cursor: pointer" @click="$navigate({r:'plugin/bonus/mall/captain/index'})"><?= \Yii::t('plugins/bonus', '队长管理');?></span>
                </el-breadcrumb-item>
                <el-breadcrumb-item><?= \Yii::t('plugins/bonus', '队员列表');?></el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <el-card shadow="never">
            <div class="title"><?= \Yii::t('plugins/bonus', '队长信息');?></div>
            <div flex="dir:left cross:center">
                <div flex="dir:left cross:center" style="height: 70px;border-right: 1px dashed #e2e2e2;width: 500px">
                    ...
                    <div class="info-item">
                        <div><?= \Yii::t('plugins/bonus', '昵称');?>：{{captain.nickname}}</div>
                        <div><?= \Yii::t('plugins/bonus', '姓名');?>：{{captain.name}}</div>
                        <div><?= \Yii::t('plugins/bonus', '手机号');?>：{{captain.mobile}}</div>
                    </div>
                </div>
                <div style="margin-left: 60px">
                    ...
                    <div><?= \Yii::t('plugins/bonus', '队员');?>：{{captain.all_member}}人</div>
                </div>
            </div>
        </el-card>
        <div class="table-body">
            <div class="input-item">
                <el-input @keyup.enter.native="toSearch" size="small" placeholder="<?= \Yii::t('plugins/bonus', '请输入搜索内容');?>" v-model="keyword" clearable @clear="toSearch">
                    <el-select size="small" v-model="type" slot="prepend" class="select">
                        <el-option key="3" label="<?= \Yii::t('plugins/bonus', '用户ID');?>" value="3"></el-option>
                        <el-option key="1" label="<?= \Yii::t('plugins/bonus', '昵称');?>" value="1"></el-option>
                        <el-option key="2" label="<?= \Yii::t('plugins/bonus', '手机号');?>" value="2"></el-option>
                    </el-select>
            ...
            <el-table :data="list" border>
                ...
                <el-table-column label="<?= \Yii::t('plugins/bonus', '基本信息');?>">
                ...
                <el-table-column prop="mobile" label="<?= \Yii::t('plugins/bonus', '手机号');?>"></el-table-column>
                <el-table-column prop="bonus_price" label="<?= \Yii::t('plugins/bonus', '贡献分红金额');?>"></el-table-column>
                <el-table-column label="<?= \Yii::t('plugins/bonus', '操作');?>">
                    <template slot-scope="scope">
        ...
        <el-form>
            <el-form-item label="<?= \Yii::t('plugins/bonus', '队长等级');?>">
                <el-select size="small" style="width: 70%;" v-model="value" placeholder="<?= \Yii::t('plugins/bonus', '请选择');?>">
                    <el-option label="<?= \Yii::t('plugins/bonus', '无');?>" value="0"></el-option>
                    <el-option
            ...
            getList() {
                ...
                request({
                    ...
                    if (e.data.code == 0) {
                        ...
                        this.$alert(e.data.msg, "<?= \Yii::t('plugins/bonus', '提示');?>", {
