    ...
    public function getList()
    {
        ...
        if ($this->flag == "EXPORT") {
            $queueId = CommonExport::handle([
                'export_class' => 'app\\plugins\\bonus\\forms\\export\\CaptainExport',
                'params' => [
                    'fieldsKeyList' => $this->fields,
                ],
                'model_class' => 'app\\plugins\\bonus\\forms\\mall\\CaptainForm',
                'function_name' => 'where'
            ]);
        ...
        $userList = User::find()->where(['id' => array_column($list, 'user_id')])->with(['userInfo', 'userPlatform'])
            ->all();
        $userPlatformList = [];
        ...
        foreach ($list as &$v) {
            ...
            $v['platform_icon'] = $userPlatformList[$v['user_id']]['platform_icon'];
        }
        unset($v);
