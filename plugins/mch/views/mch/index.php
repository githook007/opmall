...
<div id="app" v-cloak>
    <el-card class="box-card" shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <div>
                <span><?= \Yii::t('plugins/mch', '商户列表');?></span>
                <div style="float: right;margin-top: -5px">
                    <el-button type="primary" @click="edit" size="small"><?= \Yii::t('plugins/mch', '添加商户');?></el-button>
                </div>
            </div>
        </div>
        ...
        <div class="table-body">
            <el-alert
                    ...
                    type="info"
                    title="<?= \Yii::t('plugins/mch', '入驻商户PC端登录网址');?>"
                    :closable="false">
                <template>
                    ...
                    <el-button v-if="loginRoute" id="copy_btn"
                               ...
                               size="mini"><?= \Yii::t('plugins/mch', '复制链接');?>
                    </el-button>
                </template>
            </el-alert>
            <el-form @submit.native.prevent="searchList" size="small" :inline="true" :model="search">
                <el-form-item>
                    <div class="input-item">
                        <el-input  @keyup.enter.native="searchList" size="small" v-model="search.keyword" clearable
                                   placeholder="<?= \Yii::t('plugins/mch', '请输入店铺名/用户名搜索');?>"
                                  @clear='searchList'>
            ...
            <el-table
                ...
                <el-table-column
                        ...
                        label="<?= \Yii::t('plugins/mch', '店铺信息');?>" width="200">
...
<!--                            <app-ellipsis style="margin-left: 10px;" :line="1">{{scope.row.store.name}}</app-ellipsis>-->
                    ...
                    <template slot-scope="scope">
                        <div flex="dir:left cross:center" v-if="scope.row.user">
                            <el-tooltip class="item" effect="dark"
                                        :content="scope.row.user.platform_name" placement="top">
                                <img height="25px" width="25px" style="margin-right: 10px" :src="scope.row.user.platform_icon" alt="">
                            </el-tooltip>
                ...
                <el-table-column
                        label="<?= \Yii::t('plugins/mch', '联系人');?>" width="200">
                    <template slot-scope="scope">
                        <div>
                            <app-ellipsis style="margin-left: 10px;" :line="1">{{scope.row.realname}}
                            </app-ellipsis>
                            <app-ellipsis style="margin-left: 10px;" :line="1"><?= \Yii::t('plugins/mch', '电话');?>:{{scope.row.mobile}}
                            </app-ellipsis>
                        </div>
                    </template>
                </el-table-column>
                <el-table-column
                        label="<?= \Yii::t('plugins/mch', '排序');?>"
                        prop="sort"
                        width="300">
                    <template slot-scope="scope">
                        <div v-if="id != scope.row.id">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/mch', '排序');?>" placement="top">
                ...
                <el-table-column
                        label="<?= \Yii::t('plugins/mch', '入驻时间');?>"
                        prop="review_time"
                        width="220">
                </el-table-column>
                <el-table-column
                        label="<?= \Yii::t('plugins/mch', '开业');?>"
                        width="80">
                    <template slot-scope="scope">
                        <el-switch
                ...
                <el-table-column
                    ...
                    <template slot-scope="scope">
                        <el-button @click="fastLogin(scope.row.id)" type="text" circle size="mini">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/mch', '跳转店铺后台');?>" placement="top">
                                <img src="statics/img/mall/fast.png" alt="">
                            </el-tooltip>
                        </el-button>
                        <el-button @click="edit(scope.row.id)" type="text" circle size="mini">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/mch', '编辑');?>" placement="top">
                                <img src="statics/img/mall/edit.png" alt="">
                            </el-tooltip>
                        </el-button>
                        <el-button @click="$navigate({r: 'plugin/mch/mall/mch/mall-setting', mch_id: scope.row.id})"
                                   type="text" circle size="mini">
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/mch', '设置');?>" placement="top">
                                <img src="statics/img/plugins/setting.png" alt="">
                            </el-tooltip>
                        </el-button>
                        <el-button @click="updatePasswordDialog(scope.row.id)" type="text" size="mini" circle>
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/mch', '修改密码');?>" placement="top">
        ...
        <el-dialog title="<?= \Yii::t('plugins/mch', '修改密码');?>" :visible.sync="dialogFormVisible" width="30%">
            <el-form size="small" @submit.native.prevent="" :model="form" :rules="passwordRules" ref="form">
                <el-form-item label="<?= \Yii::t('plugins/mch', '新密码');?>" prop="password">
                    <el-input type="password" v-model="form.password" autocomplete="off"></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button @click="dialogFormVisible = false"><?= \Yii::t('plugins/mch', '取消');?></el-button>
                <el-button :loading="btnLoading" type="primary" @click="updatePassword('form')"><?= \Yii::t('plugins/mch', '确定');?></el-button>
            </div>
        </el-dialog>
    </el-card>
</div>
...
<script>
    const app = new Vue({
        ...
        data() {
            return {
                ...
                passwordRules: {
                    password: [
                        {required: true, message: '<?= \Yii::t('plugins/mch', '请输入新密码');?>', trigger: 'change'},
                    ],
                },
                btnLoading: false,
                mch_id: 0,
        ...
        methods: {
            ...
            fastLogin(id) {
                navigateTo({
                    r: 'mall/mch/fast-login',
                    id: id,
                });
            },
            destroy(row, index) {
                let self = this;
                self.$confirm('<?= \Yii::t('plugins/mch', '删除该条数据, 是否继续?');?>?', '<?= \Yii::t('plugins/mch', '提示');?>', {
    ...
    clipboard.on('success', function (e) {
        self.ELEMENT.Message.success('<?= \Yii::t('plugins/mch', '复制成功');?>');
        e.clearSelection();
    });
    clipboard.on('error', function (e) {
        self.ELEMENT.Message.success('<?= \Yii::t('plugins/mch', '复制失败');?>');
    });
</script>
