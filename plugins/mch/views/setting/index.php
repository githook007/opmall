        ...
        <div class="form-body">
            <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="120px" size="small">
                <el-tabs v-if="is_show" v-model="activeName" @tab-click="handleClick">
                    <el-tab-pane label="<?= \Yii::t('plugins/mch', '基础设置');?>" name="first">
                        <el-row>
                            <el-col :span="12">
                                <el-form-item label="<?= \Yii::t('plugins/mch', '提现方式');?>" prop="cash_type">
                                    <el-checkbox :indeterminate="isIndeterminate"
                                                 v-model="checkAll"
                                                 @change="handleCheckAllChange">
                                        <?= \Yii::t('plugins/mch', '全选');?>
                                    </el-checkbox>
                                ...
                                <el-form-item label="<?= \Yii::t('plugins/mch', '入驻协议');?>" prop="desc">
                                    <el-input
                                            type="textarea"
                                            :rows="10"
                                            placeholder="<?= \Yii::t('plugins/mch', '请输入驻协议内容');?>"
                                            v-model="ruleForm.desc">
                                    </el-input>
                                </el-form-item>
                                <el-form-item label="<?= \Yii::t('plugins/mch', '客服图标');?>" prop="is_service">
                                    <el-switch v-model="ruleForm.is_service" :onactive-value="0"
                                               :active-value="1"></el-switch>
                                </el-form-item>
                                <el-form-item label="<?= \Yii::t('plugins/mch', '商户距离排序');?>" prop="is_distance">
                                    <el-switch v-model="ruleForm.is_distance" :onactive-value="0"
                                               :active-value="1"></el-switch>
                                </el-form-item>
                                <el-form-item label="<?= \Yii::t('plugins/mch', '商品上架审核');?>" prop="is_goods_audit">
                                    <el-switch v-model="ruleForm.is_goods_audit" :onactive-value="0"
                                               :active-value="1"></el-switch>
                                </el-form-item>
                                <el-form-item label="<?= \Yii::t('plugins/mch', '订单确认收货');?>" prop="is_confirm_order">
                                    <el-switch v-model="ruleForm.is_confirm_order" :onactive-value="0"
                                               :active-value="1"></el-switch>
                                </el-form-item>
                                <el-form-item label="<?= \Yii::t('plugins/mch', '商家地址显示');?>" prop="is_mch_address">
                                    <el-switch v-model="ruleForm.is_mch_address" :onactive-value="0"
                                               :active-value="1"></el-switch>
                                </el-form-item>
                            </el-col>
                        </el-row>
                    </el-tab-pane>
                    <el-tab-pane label="<?= \Yii::t('plugins/mch', '自定义资料审核');?>" name="second">
                        <el-row>
                            <el-col :span="24">
                                <el-form-item label="<?= \Yii::t('plugins/mch', '表单状态');?>" prop="status" style="width: 180px">
                                    <div>
                                        <el-switch
                                ...
                                <template v-if="ruleForm.status == 1">
                                    <el-form-item label="<?= \Yii::t('plugins/mch', '表单设置');?>" prop="selectedOptions">
                    ...
                    <el-tab-pane label="<?= \Yii::t('plugins/mch', '商户登录页设置');?>" name="third">
                        <el-row>
                            <el-col :span="12">
                                <el-form-item label="<?= \Yii::t('plugins/mch', 'LOGO图片URL');?>">
                                        ...
                                        <template slot="append">
                                            <app-attachment v-model="ruleForm.logo" :multiple="false" :max="1">
                                                <el-tooltip class="item"
                                                            effect="dark"
                                                            content="<?= \Yii::t('plugins/mch', '建议尺寸:325 * 325');?>"
                                                            placement="top">
                                                    <el-button size="mini"><?= \Yii::t('plugins/mch', '上传图片');?></el-button>
                                                </el-tooltip>
                                            </app-attachment>
                                        </template>
                                    </el-input>
                                    ...
                                    <div v-else class="preview"><?= \Yii::t('plugins/mch', '建议尺寸98*36');?></div>
                                </el-form-item>
                                <el-form-item label="<?= \Yii::t('plugins/mch', '底部版权信息');?>">
                                    <el-input class="currency-width" v-model="ruleForm.copyright"></el-input>
                                </el-form-item>
                                <el-form-item label="<?= \Yii::t('plugins/mch', '底部版权链接');?>">
                                    <el-input class="currency-width" v-model="ruleForm.copyright_url"
                                              placeholder="<?= \Yii::t('plugins/mch', '例如');?>:https://www.baidu.com"></el-input>
                                </el-form-item>
                            </el-col>
                        </el-row>
                    </el-tab-pane>
...
<script>
    const options = [
        {
            label: '<?= \Yii::t('plugins/mch', '自动打款');?>',
            value: 'auto'
        },
        {
            label: '<?= \Yii::t('plugins/mch', '微信线下转账');?>',
            value: 'wx'
        },
        {
            label: '<?= \Yii::t('plugins/mch', '支付宝线下转账');?>',
            value: 'alipay'
        },
        {
            label: '<?= \Yii::t('plugins/mch', '银行卡线下转账');?>',
            value: 'bank'
        },
        {
            label: '<?= \Yii::t('plugins/mch', '余额');?>',
            value: 'balance'
        },
    ];
    const app = new Vue({
                ...
                this.$refs[formName].validate((valid) => {
                    ...
                    if (valid) {
                        if (self.ruleForm.status == 1 && !this.ruleForm.form_data.length) {
                            this.$message.error('<?= \Yii::t('plugins/mch', '至少添加一项表单设置');?>');
                            return;
                        }
