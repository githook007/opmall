...
<div id="app" v-cloak>
    <el-card class="box-card" shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <div>
                <span><?= \Yii::t('plugins/mch', '所售类目');?></span>
                <div style="float: right;margin-top: -5px">
                    <el-button type="primary" @click="edit" size="small"><?= \Yii::t('plugins/mch', '添加类目');?></el-button>
                </div>
            </div>
        </div>
            ...
            <el-form @submit.native.prevent="searchList" size="small" :inline="true" :model="search">
                <el-form-item>
                    <div class="input-item">
                        <el-input size="small" placeholder="<?= \Yii::t('plugins/mch', '请输入搜索内容');?>" v-model="search.keyword" clearable
                                  @clear='searchList'>
        ...
        <el-dialog :title="title" :visible.sync="dialogVisible">
            <el-form :model="ruleForm" :rules="rules" size="small" ref="ruleForm" label-width="120px">
                <el-form-item label="<?= \Yii::t('plugins/mch', '名称');?>" prop="name">
                    <el-input v-model="ruleForm.name" placeholder="<?= \Yii::t('plugins/mch', '请输入名称');?>"></el-input>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/mch', '排序');?>" prop="sort">
                    <el-input type="number" v-model="ruleForm.sort" placeholder="<?= \Yii::t('plugins/mch', '请输入排序');?>"></el-input>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/mch', '是否显示');?>" prop="status">
                    <el-switch
            ...
            <span slot="footer" class="dialog-footer">
                <el-button size="small" type="primary" @click="store('ruleForm')"><?= \Yii::t('plugins/mch', '保存');?></el-button>
            </span>
        </el-dialog>
    </el-card>
</div>
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                rules: {
                    name: [
                        {required: true, message: '<?= \Yii::t('plugins/mch', '请输入类目名称');?>', trigger: 'blur'},
                    ],
                    sort: [
                        {required: true, message: '<?= \Yii::t('plugins/mch', '请输入类目排序');?>', trigger: 'change'},
                    ],
                    status: [
                        {required: true, message: '<?= \Yii::t('plugins/mch', '请选择是否显示');?>', trigger: 'change'},
                    ],
                },
            };
        },
        methods: {
            ...
            edit(id) {
                let self = this;
                if (id > 0) {
                    self.title = '<?= \Yii::t('plugins/mch', '编辑类目');?>';
                    request({
                        params: {
                            r: 'plugin/mch/mall/common-cat/edit',
                            id: id
                    ...
                    self.title = '<?= \Yii::t('plugins/mch', '添加类目');?>';
                    self.dialogVisible = true;
                    self.ruleForm = {
                        name: '',
                        sort: 100,
