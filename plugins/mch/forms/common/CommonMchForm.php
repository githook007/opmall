...
class CommonMchForm extends Model
{
    ...
    public function getList()
    {
        ...
        $list = $query->orderBy(['sort' => SORT_ASC])
            ...
            ->page($pagination)->asArray()
            ->all();
        $list = array_map(function ($item) {
            if ($item['user']['id']) {
                $user = User::findOne($item['user']['id']);
                $item['user']['platform_name'] = (PlatformConfig::getInstance())->getPlatformText($user);
                $item['user']['platform_icon'] = (PlatformConfig::getInstance())->getPlatformIcon($user);
            }
           return $item;
        }, $list);
        return [
    ...
    public function getDetail()
    {
        ...
        /** @var Mch $detail */
        $detail = $query->with('store', 'category')->one();
        if (!$detail) {
