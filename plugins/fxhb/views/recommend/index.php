...
<div id="app" v-cloak>
    <el-card v-loading="loading" v-if="is_show" style="border:0"
        ...
        <div slot="header">
            <?= \Yii::t('plugins/fxhb', '推荐设置');?>
        </div>
        <el-form ref="form" :model="form" label-width="150px" size="small">
            <el-card style="margin-top: 10px;" shadow="never">
                <el-row>
                    <el-col :span="12">
                        <el-form-item label="<?= \Yii::t('plugins/fxhb', '推荐商品状态');?>">
                            <el-switch :active-value="1" :inactive-value="0"
                                       v-model="form.fxhb.is_recommend_status"></el-switch>
                        </el-form-item>
                        <el-form-item v-if="form.fxhb.is_recommend_status" label="<?= \Yii::t('plugins/fxhb', '自定义推荐商品');?>">
                            <el-switch :active-value="1" :inactive-value="0" v-model="form.fxhb.is_custom"></el-switch>
                            <span>{{form.fxhb.is_custom ? "<?= \Yii::t('plugins/fxhb', '最多添加20件商品');?>" : "<?= \Yii::t('plugins/fxhb', '按商品列表排序显示前10件商品');?>"}}</span>
                            <div v-if="form.fxhb.is_custom">
                                <draggable v-model="form.fxhb.goods_list" flex class="goods-list">
                                    <div class="goods-item" v-for="(goods,goodsIndex) in form.fxhb.goods_list">
                                        <el-tooltip effect="dark" content="<?= \Yii::t('plugins/fxhb', '移除商品');?>" placement="top">
                                ...
                                <div v-if="form.fxhb.goods_list.length < goodsDialog.max" class="goods-add">
                                    <el-tooltip effect="dark" content="<?= \Yii::t('plugins/fxhb', '添加商品');?>" placement="top">
            ...
            <el-card style="margin-top: 10px;" shadow="never">
                <el-button :loading="btnLoading" type="primary" size="small" @click="store"><?= \Yii::t('plugins/fxhb', '保存');?></el-button>
            </el-card>
        </el-form>
    </el-card>

    <el-dialog @open="getGoods" title="<?= \Yii::t('plugins/fxhb', '选择商品');?>" :visible.sync="goodsDialog.visible" :close-on-click-modal="false">
            ...
            <el-form-item>
                <div class="input-item">
                    <el-input @clear="toSearch" clearable @keyup.enter.native="toSearch" size="small"
                              placeholder="<?= \Yii::t('plugins/fxhb', '请输入商品ID');?>"
        ...
        <el-table :data="goodsDialog.list" v-loading="goodsDialog.loading" @selection-change="goodsSelectionChange">
            <el-table-column label="<?= \Yii::t('plugins/fxhb', '选择');?>" type="selection"></el-table-column>
            <el-table-column label="ID" prop="id" width="100px"></el-table-column>
            <el-table-column label="<?= \Yii::t('plugins/fxhb', '名称');?>" prop="name"></el-table-column>
        </el-table>
        ...
        <div slot="footer">
            <el-button @click="goodsDialog.visible = false"><?= \Yii::t('plugins/fxhb', '取消');?></el-button>
            <el-button type="primary" @click="addGoods"><?= \Yii::t('plugins/fxhb', '确定');?></el-button>
        </div>
