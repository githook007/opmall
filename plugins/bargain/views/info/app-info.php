            ...
            <el-breadcrumb separator="/">
                <el-breadcrumb-item v-if="type == 'single'">
                    <span style="color: #409EFF;cursor: pointer"
                        ...
                        <?= \Yii::t('plugins/bargain', '砍价活动');?>
                    </span>
                </el-breadcrumb-item>
                <el-breadcrumb-item><?= \Yii::t('plugins/bargain', '活动数据');?></el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="table-body">
            <!-- 状态选择 -->
            <el-tabs v-model="search.status" @tab-click="handleClick">
                <el-tab-pane label="<?= \Yii::t('plugins/bargain', '全部');?>" name="-1"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('plugins/bargain', '砍价中');?>" name="0"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('plugins/bargain', '砍价成功');?>" name="1"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('plugins/bargain', '砍价失败');?>" name="2"></el-tab-pane>
            </el-tabs>
            <el-form size="small" :inline="true" :model="search">
                <el-form-item style="display: none">
                    <el-input></el-input>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/bargain', '发起时间');?>:">
                    <el-date-picker
                            class="item-box"
                            size="small"
                            @change="changeTime"
                            ...
                            range-separator="<?= \Yii::t('plugins/bargain', '至');?>"
                            start-placeholder="<?= \Yii::t('plugins/bargain', '开始日期');?>"
                            end-placeholder="<?= \Yii::t('plugins/bargain', '结束日期');?>">
                    </el-date-picker>
                </el-form-item>
                <el-form-item>
                    <div class="input-item">

                        <el-input v-model="search.prop_value" placeholder="<?= \Yii::t('plugins/bargain', '请输入搜索内容');?>" clearable @clear="commonSearch"
                                  @keyup.enter.native="commonSearch">
                            <el-select style="width: 130px" slot="prepend" v-model="search.prop">
                                <el-option key="nickname" label="<?= \Yii::t('plugins/bargain', '发起人昵称');?>" value="nickname"></el-option>
                                <el-option key="user_id" label="<?= \Yii::t('plugins/bargain', '发起人用户ID');?>" value="user_id"></el-option>
                                <el-option key="name" label="<?= \Yii::t('plugins/bargain', '商品名称');?>" value="name" v-if="type == 'all'"></el-option>
                            </el-select>
            ...
            <template v-if="type == 'single'">
                <div flex="cross:center box:first" style="margin-bottom: 15px;width: 480px;" v-if="goods">
                    <div><?= \Yii::t('plugins/bargain', '商品信息');?>:</div>
            ...
            <el-table :data="list" border>
                <el-table-column label="<?= \Yii::t('plugins/bargain', '砍价信息');?>">
                        ...
                        <div class="bargain-info" flex="dir:left box:first">
                            ...
                            <div class="goods-info">
                                ...
                                <div class="price-info">
                                    <div>
                                        <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/bargain', '售价');?>" placement="top">
                                            <img src="statics/img/plugins/price.png" alt="">
                                        </el-tooltip>
                                        <span>￥{{scope.row.price}}</span>
                                    </div>
                                    <div>
                                        <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/bargain', '最低价');?>" placement="top">
                                            <img src="statics/img/plugins/low.png" alt="">
                                        </el-tooltip>
                                        <span>￥{{scope.row.min_price}}</span>
                                    </div>
                                    <div>
                                        <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/bargain', '当前价');?>" placement="top">
                ...
                <el-table-column prop="created_at" label="<?= \Yii::t('plugins/bargain', '发起时间');?>" width="180"></el-table-column>
                <el-table-column prop="status" label="<?= \Yii::t('plugins/bargain', '状态');?>" width="130">
                    <template slot-scope="scope">
                        <el-tag v-if="scope.row.status == 0"><?= \Yii::t('plugins/bargain', '进行中');?></el-tag>
                        <el-tag v-if="scope.row.status == 1" type="success"><?= \Yii::t('plugins/bargain', '砍价成功');?></el-tag>
                        <el-tag v-if="scope.row.status == 2" type="danger"><?= \Yii::t('plugins/bargain', '砍价失败');?></el-tag>
                    </template>
                </el-table-column>
                <el-table-column prop="join_list" label="<?= \Yii::t('plugins/bargain', '参与详情');?>">
                    <template slot-scope="scope">
                        <div class="item-center" style="justify-content: flex-start">
                            ...
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/bargain', '参与详情');?>" v-if="scope.row.join_list.length != 0" placement="top">
            ...
            <el-dialog title="<?= \Yii::t('plugins/bargain', '参与详情');?>" :visible.sync="dialogVisible" width="30%">
                <div v-for="value in detail_list" class="detail-item">
                    ...
                    <div style="float: right"><?= \Yii::t('plugins/bargain', '砍了');?>￥{{value.price}}</div>
                </div>
                <div @click="more" v-if="detail.length > 5 && clickMore" class="load-more"><?= \Yii::t('plugins/bargain', '加载更多');?>...</div>
            </el-dialog>
        </div>
    </el-card>
</template>
