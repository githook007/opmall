...
<div id="app" v-cloak>
    <el-card class="box-card" shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <div>
                <span><?= \Yii::t('plugins/composition', '套餐组合');?></span>
            </div>
        </div>
        <div class="edit-content">
            <div style="display: block;margin-bottom: 20px">
                <div class="item">
                    <div><?= \Yii::t('plugins/composition', '固定套餐');?></div>
                    <div><?= \Yii::t('plugins/composition', '套餐内商品打包销售');?></div>
                    <div>
                        <el-button type="primary"
                                   @click="$navigate({r: 'plugin/composition/mall/index/fixed'})"
                                   size="mini"><?= \Yii::t('plugins/composition', '新建套餐');?>
                        </el-button>
                    </div>
                </div>
                <div class="item">
                    <div><?= \Yii::t('plugins/composition', '搭配套餐');?></div>
                    <div><?= \Yii::t('plugins/composition', '套餐内主商品必选');?></div>
                    <div>
                        <el-button type="primary"
            ...
            <el-tabs v-model="activeName" @tab-click="toSearch">
                <el-tab-pane label="<?= \Yii::t('plugins/composition', '全部');?>" name="-1"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('plugins/composition', '销售中');?>" name="1"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('plugins/composition', '下架中');?>" name="0"></el-tab-pane>
            </el-tabs>
            <div class="table-body">
                <div flex="dir:left cross:center" style='margin: 5px 0 20px'>
                    <div class="item-box" flex="dir:left cross:center">
                        <div class="label"><?= \Yii::t('plugins/composition', '套餐类型');?></div>
                        <el-select style="width: 120px;margin-left: 10px" size="small" v-model="search.type" @change='toSearch'>
                            <el-option key="all" label="<?= \Yii::t('plugins/composition', '全部套餐');?>" value=""></el-option>
                            <el-option key="1" label="<?= \Yii::t('plugins/composition', '固定套餐');?>" value="1"></el-option>
                            <el-option key="2" label="<?= \Yii::t('plugins/composition', '搭配套餐');?>" value="2"></el-option>
                        </el-select>
                    </div>
                    <div style='margin-left: 30px'><?= \Yii::t('plugins/composition', '添加时间');?></div>
                    <el-date-picker
                            class="item-box"
                            size="small"
                            @change="changeTime"
                            ...
                            range-separator="<?= \Yii::t('plugins/composition', '至');?>"
                            start-placeholder="<?= \Yii::t('plugins/composition', '开始日期');?>"
                            end-placeholder="<?= \Yii::t('plugins/composition', '结束日期');?>">
                    </el-date-picker>
                    <div class="input-item">
                        <el-input @keyup.enter.native="toSearch" size="small" placeholder="<?= \Yii::t('plugins/composition', '请输入套餐名称搜索');?>" v-model="search.keyword" clearable @clear='toSearch'>
                ...
                <el-table @sort-change="changeSort" :data="list" border v-loading="loading" @selection-change="handleSelectionChange">
                    ...
                    <el-table-column prop="sort" :width="sort_goods_id != id ? 150 : 100" label="<?= \Yii::t('plugins/composition', '排序');?>"
                                     sortable="custom">
                    ...
                    <el-table-column label="<?= \Yii::t('plugins/composition', '套餐名称');?>" width="300" prop="name"></el-table-column>
                    <el-table-column label="<?= \Yii::t('plugins/composition', '套餐类型');?>" prop="type">
                        <template slot-scope="scope">
                            <div>{{scope.row.type == 1 ? '固定套餐' : '搭配套餐'}}</div>
                        </template>
                    </el-table-column>
                    <el-table-column label="<?= \Yii::t('plugins/composition', '商品数量');?>" prop="goods_count"></el-table-column>
                    <el-table-column label="<?= \Yii::t('plugins/composition', '优惠金额');?>" prop="price">
                        <template slot-scope="scope">
                    ...
                    <el-table-column label="<?= \Yii::t('plugins/composition', '库存');?>" prop="stock" sortable="custom">
                        <template slot-scope="scope">
                            <div>
                    ...
                    <el-table-column label="<?= \Yii::t('plugins/composition', '状态');?>">
                        <template slot-scope="scope">
                            <div v-if="scope.row.status == 1">
                                <el-tag type="success"><?= \Yii::t('plugins/composition', '销售中');?></el-tag>
                            </div>
                            <div v-else-if="scope.row.status == 0">
                                <el-tag type="warning"><?= \Yii::t('plugins/composition', '下架中');?></el-tag>
                            </div>
                            <div v-else-if="scope.row.status == 2">
                                <el-tag type="danger"><?= \Yii::t('plugins/composition', '售罄');?></el-tag>
                            </div>
                            <div v-else-if="scope.row.status == 3">
                                <el-tooltip effect="dark" content="<?= \Yii::t('plugins/composition', '套餐异常');?>" placement="top">
                                    <el-tag type="warning"><?= \Yii::t('plugins/composition', '异常');?></el-tag>
                                </el-tooltip>
                            </div>
                        </template>
                    </el-table-column>
                    <el-table-column label="<?= \Yii::t('plugins/composition', '添加时间');?>" prop="created_at"></el-table-column>
                    <el-table-column fixed="right" width='200' label="<?= \Yii::t('plugins/composition', '操作');?>">
                        <template slot-scope="scope">
                            <el-button circle size="mini" type="text" @click="look(scope.row)">
                                <el-tooltip effect="dark" content="<?= \Yii::t('plugins/composition', '查看');?>" placement="top">
                                    <img src="statics/img/mall/look.png" alt="">
                                </el-tooltip>
                            </el-button>
                            <el-button circle size="mini" type="text" @click="edit(scope.row)">
                                <el-tooltip effect="dark" content="<?= \Yii::t('plugins/composition', '编辑');?>" placement="top">
                                    <img src="statics/img/mall/edit.png" alt="">
                                </el-tooltip>
                            </el-button>
                            <el-button circle size="mini" type="text" @click="destroy(scope.row.id,scope.$index)">
                                <el-tooltip effect="dark" content="<?= \Yii::t('plugins/composition', '删除');?>" placement="top">
    ...
    <el-dialog title="<?= \Yii::t('plugins/composition', '查看商品');?>" class="goods-dialog" :visible.sync="dialogVisible" width="30%">
        <div>
            <span v-if="detail.type == 1"><?= \Yii::t('plugins/composition', '固定套餐');?></span>
            <span v-else><?= \Yii::t('plugins/composition', '搭配套餐');?></span>
            <span><?= \Yii::t('plugins/composition', '共计');?>{{detail.goods_count}}款商品</span>
            <span v-if="detail.type == 1"><?= \Yii::t('plugins/composition', '合计金额');?>
                <span style="color: #ff4544;">¥{{min_price}}{{max_price != min_price &&  max_price != 0? ' ~ '+max_price : ''}} </span>
            </span>
            <span v-if="detail.type == 1" class="off-price" style="margin-left: 5px;"><?= \Yii::t('plugins/composition', '优惠金额');?>{{detail.price}}</span>
        </div>
        ...
        <div v-else>
            <div class="list-title"><?= \Yii::t('plugins/composition', '主商品');?></div>
                        ...
                        <div flex="dir:left">
                            ...
                            <div>
                                ...
                                <div style="color: #ff4544;">
                                    ...
                                    <span v-if="detail.type == 2" class="off-price">（<?= \Yii::t('plugins/composition', '优惠金额');?>{{scope.row.price}}）</span>
                                </div>
                            </div>
                        </div>
                    </template>
            ...
            <div class="list-title"><?= \Yii::t('plugins/composition', '搭配商品');?></div>
...
<script>
    const app = new Vue({
        ...
        methods: {
            ...
            destroy(id, index) {
                let self = this;
                self.$confirm("<?= \Yii::t('plugins/composition', '删除该条数据');?>", "<?= \Yii::t('plugins/composition', '提示');?>", {
                    confirmButtonText: "<?= \Yii::t('plugins/composition', '确定');?>",
                    cancelButtonText: "<?= \Yii::t('plugins/composition', '取消');?>",
                    type: 'warning'
                }).then(() => {
                    self.loading = true;
                    request({
                    ...
                    self.$message.info("<?= \Yii::t('plugins/composition', '已取消删除');?>")
                });
            },
            getAllChecked(e) {
