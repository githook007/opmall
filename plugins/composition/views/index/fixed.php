    ...
    <el-card class="box-card" shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;"
        ...
        <div slot="header">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item v-if="id > 0"><?= \Yii::t('plugins/composition', '编辑固定套餐');?></el-breadcrumb-item>
                <el-breadcrumb-item v-else><?= \Yii::t('plugins/composition', '新建固定套餐');?></el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div style="background: #ffffff;padding: 20px;">
            <el-form @submit.native.prevent :model="ruleForm" :rules="rules" size="small"
                     ref="ruleForm" label-width="120px">
                <el-card header="<?= \Yii::t('plugins/composition', '固定套餐设置');?>" style="margin-bottom: 10px" shadow="never">
                    <div class="form-body">
                        ...
                        <el-form-item label="<?= \Yii::t('plugins/composition', '选择商品');?>" prop="list">
                            <div>
                                <div flex="dir:left">
                                    <app-dialog-select :default="ruleForm.list" :id="true" title="<?= \Yii::t('plugins/composition', '选择商品');?>" :multiple="true" @selected="selectGoods" :max="max"
                                                       url="plugin/composition/mall/index/get-goods" :params="{type: 1}">
                                        <el-button size="mini"><?= \Yii::t('plugins/composition', '选择商品');?></el-button>
                                    </app-dialog-select>
                                    <div style="font-size: 10px;color: #b7b7b7;margin-left: 10px"><?= \Yii::t('plugins/composition', '最多选择5款商品');?></div>
                                </div>
                                <div class="list" v-if="ruleForm.list.length > 0">
                                    ...
                                    <div><?= \Yii::t('plugins/composition', '合计金额');?>
                        ...
                        <el-form-item label="<?= \Yii::t('plugins/composition', '优惠金额');?>" prop="price">
                            <div>
                                <el-input v-model="ruleForm.price" type="number" size="small">
                                    <span slot="append"><?= \Yii::t('plugins/composition', '元');?></span>
                                </el-input>
                            </div>
                            <div v-if="!error" style="font-size: 12px;color: #b7b7b7;margin-top: -8px"><?= \Yii::t('plugins/composition', '优惠金额不大于套餐金额');?></div>
                        </el-form-item>
                    </div>
                </el-card>
        ...
        data() {
            var validateRate = (rule, value, callback) => {
                if (this.ruleForm.price === '') {
                    this.error = false;
                    callback(new Error("<?= \Yii::t('plugins/composition', '优惠金额不能为空');?>"));
                    ...
                    this.error = true;
                    callback(new Error("<?= \Yii::t('plugins/composition', '优惠金额不大于套餐金额');?>"));
                } else {
                    this.error = false;
                    callback();
                }
            ...
            var validateRate2 = (rule, value, callback) => {
                if (this.ruleForm.list.length < 2) {
                    callback(new Error("<?= \Yii::t('plugins/composition', '请至少选择两个商品');?>"));
                } else {
                    callback();
                }
            };
            return {
                ...
                rules: {
                    ...
                    stock: [
                        {required: true, message: "<?= \Yii::t('plugins/composition', '库存不能为空');?>"}
                    ]
                }
            };
        },
            ...
            store(formName) {
                this.$refs[formName].validate((valid) => {
                    ...
                    if (valid) {
                        ...
                        if(+form.price > price) {
                            self.$message.error("<?= \Yii::t('plugins/composition', '优惠金额超过了套餐金额');?>");
                            return false;
                        }
