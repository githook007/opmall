        ...
        <div slot="header">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><span style="color: #409EFF;cursor: pointer"
                                          @click="$navigate({r:'plugin/composition/mall/index/list'})"><?= \Yii::t('plugins/composition', '套餐组合');?></span>
                </el-breadcrumb-item>
                <el-breadcrumb-item v-if="id > 0"><?= \Yii::t('plugins/composition', '编辑搭配套餐');?></el-breadcrumb-item>
                <el-breadcrumb-item v-else><?= \Yii::t('plugins/composition', '新建搭配套餐');?></el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div style="background: #ffffff;padding: 20px;">
            <el-form @submit.native.prevent :model="ruleForm" :rules="rules" size="small"
                     ref="ruleForm" label-width="120px">
                <el-card header="<?= \Yii::t('plugins/composition', '搭配套餐设置');?>" shadow="never" style="margin-bottom: 10px;">
                    <div class="form-body">
                        <el-form-item label="<?= \Yii::t('plugins/composition', '套餐名称');?>" prop="name">
                            <el-input v-model="ruleForm.name" size="small" placeholder="<?= \Yii::t('plugins/composition', '请输入套餐名称');?>"></el-input>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('plugins/composition', '选择主商品');?>" prop="host_list">
                            <div>
                                <div flex="dir:left">
                                    <app-dialog-select :default="ruleForm.list" title="<?= \Yii::t('plugins/composition', '选择主商品');?>" :multiple="false" @selected="selectHostGoods"
                                                       url="plugin/composition/mall/index/get-goods" :params="params">
                                        <el-button size="mini"><?= \Yii::t('plugins/composition', '选择商品');?></el-button>
                                    </app-dialog-select>
                                    <div style="font-size: 10px;color: #b7b7b7;margin-left: 10px"><?= \Yii::t('plugins/composition', '最多选择1款商品');?></div>
                                </div>
                                    ...
                                    <div class="item" v-for="(item, index) in ruleForm.host_list" :key="index"
                                        ...
                                        <div flex="dir:top main:justify" style="flex-grow: 1;margin-right: 15px;">
                                            ...
                                            <div flex="dir:left">
                                                ...
                                                <div style="color: #ffa525;" v-if="item.discounts_price">（<?= \Yii::t('plugins/composition', '优惠金额');?>￥{{item.discounts_price}}）</div>
                                            </div>
                                        </div>
                                        <div style="flex-grow: 0" v-if="item.is_delete != 0">
                                            <el-tag type="danger"><?= \Yii::t('plugins/composition', '商品不存在');?></el-tag>
                                        </div>
                                        <div style="flex-grow: 0" v-else-if="item.stock == 0">
                                            <el-tag type="danger"><?= \Yii::t('plugins/composition', '0库存');?></el-tag>
                                        </div>
                                        <div style="flex-grow: 0" v-else>
                                            <el-button size="mini" type="primary" plain @click="editPrice(item, -1)">
                                                <?= \Yii::t('plugins/composition', '优惠金额');?><i class="el-icon-edit el-icon--right"></i>
                                            </el-button>
                                        </div>
                                ...
                                <el-button v-else type="text" @click="$navigate({r:'mall/goods/edit'}, true)">
                                    <?= \Yii::t('plugins/composition', '商城还未添加商品');?>
                                </el-button>
                            </div>
                        </el-form-item>
                        <el-form-item label="<?= \Yii::t('plugins/composition', '选择搭配商品');?>" prop="list">
                            <div>
                                <div flex="dir:left">
                                    <app-dialog-select :default="ruleForm.list" title="<?= \Yii::t('plugins/composition', '选择搭配商品');?>" :multiple="true" @selected="selectGoods" :max="max"
                                                       url="plugin/composition/mall/index/get-goods" :params="params">
                                        <el-button size="mini"><?= \Yii::t('plugins/composition', '选择商品');?></el-button>
                                    </app-dialog-select>
                                    <div style="font-size: 10px;color: #b7b7b7;margin-left: 10px"><?= \Yii::t('plugins/composition', '最多选择4款商品');?></div>
                                </div>
            ...
            <el-dialog
                ...
                <div slot="footer" class="dialog-footer">
                    <el-button type="primary" @click="goodsConfirm('goods')"><?= \Yii::t('plugins/composition', '确定');?></el-button>
                </div>
            </el-dialog>
        </div>
    </el-card>
        ...
        data() {
            var validateRate = (rule, value, callback) => {
                ...
                if (this.goods.discounts_price === '') {
                    callback(new Error("<?= \Yii::t('plugins/composition', '请输入优惠金额');?>"));
                } else if(this.goods.discounts_price < 0) {
                    callback(new Error("<?= \Yii::t('plugins/composition', '优惠金额不能为负');?>"));
                } else if(this.goods.discounts_price > +this.goods.min_price) {
                    callback(new Error("<?= \Yii::t('plugins/composition', '优惠金额不能大于商品最低价');?>"));
                } else {
                    callback();
                }
            };
            return {
                ...
                rules: {
                    name: [
                        {required: true, message: "<?= \Yii::t('plugins/composition', '请输入套餐名');?>"}
                    ],
                    list: [
                        {required: true, message: "<?= \Yii::t('plugins/composition', '请至少选择一个商品');?>"}
                    ],
                    host_list: [
                        {required: true, message: "<?= \Yii::t('plugins/composition', '主商品必须选择');?>"}
                    ],
                },
                discounts: {
                    discounts: [
                ...
                index: -1,
                attrDefault: {
                    price: "<?= \Yii::t('plugins/composition', '套餐价格');?>",
                    stock: "<?= \Yii::t('plugins/composition', '套餐库存');?>"
                }
            };
        },
        created() {
                ...
                this.$refs[formName].validate((valid) => {
                    ...
                    if (valid) {
                        if(!self.ruleForm.host_list[0].discounts_price) {
                            self.$message.error("<?= \Yii::t('plugins/composition', '请设置主商品的优惠金额');?>");
                            return false;
                        }
                        for(let i in self.ruleForm.list) {
                            if(!self.ruleForm.list[i].discounts_price) {
                                self.$message.error("<?= \Yii::t('plugins/composition', '请设置搭配商品的优惠金额');?>");
                                return false;
                            }
                        }
