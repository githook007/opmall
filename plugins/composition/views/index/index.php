    ...
    <el-card style="border:0" shadow="never" body-style="background-color: #f3f3f3;padding: 0 0;"
        ...
        <div class="text item" style="width:100%">
            <el-tabs v-model="activeName">
                <el-tab-pane label="<?= \Yii::t('plugins/composition', '基础设置');?>" name="first">
                    ...
                    <app-setting v-model="form" :label_width="200" :is_full_reduce="true" :is_integral="false" :is_member_price="false"
                        ...
                        <template slot="more" slot-scope="scope">
                            <el-form-item label="<?= \Yii::t('plugins/composition', '是否允许使用优惠券');?>" prop="is_coupon">
                    ...
                    <div class="title">
                        <span><?= \Yii::t('plugins/composition', '活动规则设置');?></span>
                    </div>
                    <div class="form-body" v-if="!loading">
                        <el-form :model="form" label-width="200px" :rules="rules" ref="form">
                            <el-form-item label="<?= \Yii::t('plugins/composition', '规则标题');?>" prop="title">
                                <el-input v-model="form.title"></el-input>
                            </el-form-item>
                            <el-form-item label="<?= \Yii::t('plugins/composition', '活动规则');?>" prop="rule">
                ...
                <el-tab-pane label="<?= \Yii::t('plugins/composition', '自定义广告图');?>" name="second">
                    <div class="bg" flex="dir:left box:first">
                        <div class="left">
                            <div class="content">
                                <img class="mobile-bg" src="statics/img/plugins/composition.png" alt="">
                                <div class="mobile-rule"><?= \Yii::t('plugins/composition', '活动规则');?></div>
                                <img class="activity-bg" :src="form.activityBg" alt="">
                            </div>
                        </div>
                        <div class="right form-body">
                            <el-form :model="form" label-width="120px" :rules="rules" ref="form">
                                <el-form-item label="<?= \Yii::t('plugins/composition', '广告图');?>" prop="activityBg">
                                    <div flex style="margin-bottom: 8px;">
                                        <app-attachment :multiple="false" :max="1" @selected="sellOutOtherPic">
                                            <el-tooltip effect="dark"
                                                        content="<?= \Yii::t('plugins/composition', '建议尺寸');?>"
                                                        placement="top">
                                                <el-button size="mini"><?= \Yii::t('plugins/composition', '选择图标');?></el-button>
                                            </el-tooltip>
                                        </app-attachment>
                                        <div style="margin-left: 10px;">
                                            <el-button type="primary" size="mini" @click="resetBg"><?= \Yii::t('plugins/composition', '恢复默认');?></el-button>
                                        </div>
                                    </div>
                            ...
                            <div class="right-button" v-if="activeName == 'second'">
                                <el-button class="button-item" :loading="btnLoading" type="primary" @click="store('form')" size="small"><?= \Yii::t('plugins/composition', '保存');?></el-button>
                            </div>
                        </div>
                    </div>
                </el-tab-pane>
            ...
            <el-button class="button-item" v-if="activeName == 'first'" :loading="btnLoading" type="primary" @click="store('form')" size="small">
                <?= \Yii::t('plugins/composition', '保存');?>
            </el-button>
        </div>
    </el-card>
    <el-dialog title="<?= \Yii::t('plugins/composition', '确定选择');?>" :visible.sync="dialogVisible" width="50%">
        <div style="margin-bottom: 1rem;">
            ...
            <div style="text-align: right;margin-top: 1rem;">
                <el-button type="primary" @click="districtConfirm">
                    <?= \Yii::t('plugins/composition', '请选择广告图');?>
                </el-button>
            </div>
        </div>
    </el-dialog>
        ...
        data() {
            var validateRate = (rule, value, callback) => {
                if (this.form.activityBg === '' || this.form.activityBg === undefined) {
                    callback(new Error("<?= \Yii::t('plugins/composition', '请选择广告图');?>"));
                } else {
                    callback();
                }
            };
        ...
        methods: {
            ...
            store(formName) {
                this.$refs[formName].validate(valid => {
                    if (valid) {
                        if (this.form.activityBg == '' || this.form.activityBg == undefined) {
                            this.$message.error("<?= \Yii::t('plugins/composition', '请选择广告图');?>");
                            return false;
                        }
