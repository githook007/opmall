...
class OrderSubmitForm extends \app\forms\api\order\OrderSubmitForm
{
    ...
    public function getCompositionList($compositionList)
    {
        ...
        foreach ($compositionList as $compositionItem) {
            ...
            if ($composition->type == 1) {
                ...
                $compositionPrice = min($price, $compositionPrice);
                ...
                foreach ($goodsList as $index => &$goods) {
                    ...
                    $goods['goods_attr']['composition_price'] =
                        price_format($goodsAttr['price'] - $goodsPrice); // 商品优惠金额
                ...
                foreach ($goodsList as $index => &$goods) {
                    $goodsAttr = $goods['goods_attr'];
                    $goodsCompositionPrice = $model->getGoodsPrice(
                        $goodsAttr['goods_id'],
                        $goodsAttr['id'],
                        $goodsAttr['price']
                    ); // 商品优惠金额
                    $compositionPrice += $goodsCompositionPrice;
     ...
     * 套餐组合不支持单商品发货方式（由于商品性质问题，需要重写这个方法）
     */
