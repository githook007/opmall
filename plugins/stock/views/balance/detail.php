    ...
    <el-card class="box-card" shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header" class="clearfix">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item><?= \Yii::t('plugins/stock', '结算详情');?></el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="table-body">
            <el-table :data="detail" border v-loading="loading" style="margin-bottom: 15px;width: 900px">
                <el-table-column label="<?= \Yii::t('plugins/stock', '结算周期');?>" width="300">
                    <template slot-scope="scope">
                        <div flex="dir:left cross:center">
                            <el-tag size="small" v-if="scope.row.bonus_type == 1"><?= \Yii::t('plugins/stock', '按周');?></el-tag>
                            <el-tag size="small" v-else><?= \Yii::t('plugins/stock', '按月');?></el-tag>
                            <div>{{scope.row.start_time}}~{{scope.row.end_time}}</div>
                        </div>
                    </template>
                </el-table-column>
                <el-table-column label="<?= \Yii::t('plugins/stock', '订单数');?>" prop="order_num"></el-table-column>
                <el-table-column label="<?= \Yii::t('plugins/stock', '分红金额');?>" prop="price" width="250">
                    <template slot-scope="scope">
                        <div>￥{{scope.row.bonus_price}}({{scope.row.bonus_rate}}%<?= \Yii::t('plugins/stock', '分红比例');?>)</div>
                    </template>
                </el-table-column>
                <el-table-column label="<?= \Yii::t('plugins/stock', '股东数');?>" prop="stock_num"></el-table-column>
            </el-table>
            <div flex="dir:left cross:center" style="margin-bottom: 20px;">
                <div flex="dir:left cross:center">
                    ...
                    <div class="input-item">
                        <el-input @keyup.enter.native="toSearch" size="small" placeholder="<?= \Yii::t('plugins/stock', '请输入搜索内容');?>" v-model="keyword_1" clearable @clear="toSearch">
            ...
            <el-table :data="list" border v-loading="loading" size="small" style="margin-bottom: 15px;">
                ...
                <el-table-column label="<?= \Yii::t('plugins/stock', '等级分红比例');?>" prop="rate">
                    <template slot-scope="scope">
                        <div>{{scope.row.bonus_rate}}%</div>
                    </template>
                </el-table-column>
                <el-table-column label="<?= \Yii::t('plugins/stock', '分红金额');?>" prop="price">
                    <template slot-scope="scope">
...
<script>
    new Vue({
        ...
        data() {
            return {
                type: '1',
                level: '<?= \Yii::t('plugins/stock', '全部股东');?>',
                level_name: '',
                keyword: 1,
                keyword_1: '',
                bonus_id: 0,
        ...
        methods: {
            changeLevel(e) {
                this.level_name = e != '<?= \Yii::t('plugins/stock', '全部股东');?>' ? e : '';
                this.loadData();
            },
            loadData(page) {
                this.loading = true;
                request({
                    ...
                    if (e.data.code == 0) {
                        ...
                        let defult = {level_name:'<?= \Yii::t('plugins/stock', '全部股东');?>'}
