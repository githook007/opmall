    ...
    <el-card v-loading="loading" style="border:0" shadow="never" body-style="background-color: #f3f3f3;padding: 0 0;">
        <el-form :model="form" :rules="rules" label-width="200px" ref="form">
            <el-tabs v-model="activeName">
                <el-tab-pane label="<?= \Yii::t('plugins/stock', '基本设置');?>" name="first">
                    <div class="form-body">
                        <el-form-item class="switch" label="<?= \Yii::t('plugins/stock', '股东分红');?>" prop="is_stock">
                        ...
                        <el-form-item class="input-rate" v-if="is_stock == 1" prop="stock_rate">
                            <template slot='label'>
                                <span><?= \Yii::t('plugins/stock', '订单总分红比例');?></span>
                                <el-tooltip effect="dark" content="<?= \Yii::t('plugins/stock', '订单总分红比例订单实付金额');?>"
                                            placement="top">
                        ...
                        <el-form-item class="input-rate" v-if="is_stock == 1" prop="base_rate">
                            <template slot='label'>
                                <span><?= \Yii::t('plugins/stock', '基础等级分红比例');?></span>
                                <el-tooltip effect="dark" content="<?= \Yii::t('plugins/stock', '默认等级的分红比例');?>"
                                            placement="top">
                                    <i class="el-icon-info"></i>
                                </el-tooltip>
                            </template>
                        ...
                        <el-form-item v-if="is_stock == 1" class="switch" label="<?= \Yii::t('plugins/stock', '申请成为股东');?>" prop="apply_type">
                            <el-radio-group @change="tabType" v-model="form.apply_type">
                                <el-radio label="1"><?= \Yii::t('plugins/stock', '填信息需审核');?></el-radio>
                                <el-radio label="2"><?= \Yii::t('plugins/stock', '填信息无需审核');?></el-radio>
                                <el-radio label="3"><?= \Yii::t('plugins/stock', '不填信息需审核');?></el-radio>
                                <el-radio label="4"><?= \Yii::t('plugins/stock', '不填信息无需审核');?></el-radio>
                            </el-radio-group>
                        </el-form-item>
                        <el-form-item v-if="is_stock == 1" class="switch" label="<?= \Yii::t('plugins/stock', '成为股东条件');?>" prop="become_type">
                            <el-radio-group @change="tabType" v-model="form.become_type">
                                <el-radio :label="1"><?= \Yii::t('plugins/stock', '下线总人数');?>
                                    <el-tooltip effect="dark" content="<?= \Yii::t('plugins/stock', '下线总人数');?>"
                                                placement="top">
                                        <i class="el-icon-info"></i>
                                    </el-tooltip>
                                </el-radio>
                                <el-radio :label="4"><?= \Yii::t('plugins/stock', '分销订单总数');?></el-radio>
                                <el-radio :label="5"><?= \Yii::t('plugins/stock', '分销订单总金额');?></el-radio>
                                <el-radio :label="2"><?= \Yii::t('plugins/stock', '累计佣金总额');?>
                                    <el-tooltip effect="dark" content="<?= \Yii::t('plugins/stock', '累计佣金金额');?>"
                                                placement="top">
                                        <i class="el-icon-info"></i>
                                    </el-tooltip>
                                </el-radio>
                                <el-radio :label="3"><?= \Yii::t('plugins/stock', '已提现佣金总额');?></el-radio>
                            </el-radio-group>
                        </el-form-item>
                        <el-form-item class="switch" v-if="is_stock == 1 && form.become_type == 1" label="<?= \Yii::t('plugins/stock', '下线总人数');?>" prop="condition">
                            <el-input type="number" size="small" style="width: 590px;" v-model="form.condition" autocomplete="off"></el-input>
                        </el-form-item>
                        <el-form-item class="switch" v-if="is_stock == 1 && form.become_type == 4" label="<?= \Yii::t('plugins/stock', '分销订单总数');?>" prop="condition">
                            <el-input type="number" size="small" style="width: 590px;" v-model="form.condition" autocomplete="off"></el-input>
                        </el-form-item>
                        <el-form-item class="switch" v-if="is_stock == 1 && form.become_type == 5" label="<?= \Yii::t('plugins/stock', '分销订单总金额');?>" prop="condition">
                            <el-input type="number" size="small" style="width: 590px;" v-model="form.condition" autocomplete="off"></el-input>
                        </el-form-item>
                        <el-form-item class="switch" v-if="is_stock == 1 && form.become_type == 2" label="<?= \Yii::t('plugins/stock', '累计佣金总额');?>" prop="condition">
                            <el-input type="number" size="small" style="width: 590px;" v-model="form.condition" autocomplete="off"></el-input>
                        </el-form-item>
                        <el-form-item class="switch" v-if="is_stock == 1 && form.become_type == 3" label="<?= \Yii::t('plugins/stock', '已提现佣金总额');?>" prop="condition">
                            <el-input type="number" size="small" style="width: 590px;" v-model="form.condition" autocomplete="off"></el-input>
                        </el-form-item>
                        <el-form-item v-if="is_stock == 1" class="switch" label="<?= \Yii::t('plugins/stock', '显示申请协议');?>" prop="showTreaty">
                            <el-switch v-model="showTreaty" :active-value="1" :inactive-value="0"></el-switch>
                        </el-form-item>
                        <el-form-item v-if="is_stock == 1 && showTreaty == 1" class="switch" label="<?= \Yii::t('plugins/stock', '协议名称');?>" prop="agreement_title">
                            <el-input size="small" style="width: 590px;" v-model="form.agreement_title" autocomplete="off"></el-input>
                        </el-form-item>
                        <el-form-item v-show="is_stock == 1 && showTreaty == 1" class="switch" label="<?= \Yii::t('plugins/stock', '协议内容');?>" prop="agreement_content">
                            <app-rich-text style="width: 590px;" v-model="form.agreement_content"></app-rich-text>
                        </el-form-item>
                        <el-form-item v-show="is_stock == 1" class="switch" label="<?= \Yii::t('plugins/stock', '用户须知');?>" prop="user_instructions">
                            <template slot='label'>
                                <span><?= \Yii::t('plugins/stock', '用户须知');?></span>
                                <el-tooltip effect="dark" content="<?= \Yii::t('plugins/stock', '用户须知展示在股东分红页面');?>"
                                            placement="top">
                                    <i class="el-icon-info"></i>
                                </el-tooltip>
                            </template>
                ...
                <el-tab-pane v-if="is_stock == 1" label="<?= \Yii::t('plugins/stock', '结算设置');?>" name="second">
                    <div class="form-body">
                        <el-form-item label-width="220px" class="switch" label="<?= \Yii::t('plugins/stock', '提现方式');?>" prop="is_share">
                            <el-checkbox-group v-model="pay_type">
                                <el-checkbox label="auto"><?= \Yii::t('plugins/stock', '自动打款');?></el-checkbox>
                                <el-checkbox label="wechat"><?= \Yii::t('plugins/stock', '微信线下提现');?></el-checkbox>
                                <el-checkbox label="alipay"><?= \Yii::t('plugins/stock', '支付宝线下提现');?></el-checkbox>
                                <el-checkbox label="bank"><?= \Yii::t('plugins/stock', '银行卡线下提现');?></el-checkbox>
                                <el-checkbox label="balance"><?= \Yii::t('plugins/stock', '余额提现');?></el-checkbox>
                            </el-checkbox-group>
                        </el-form-item>
                        <el-form-item label-width="220px" prop="min_money">
                            <template slot='label'>
                                <span><?= \Yii::t('plugins/stock', '提现门槛金额');?></span>
                                <el-tooltip effect="dark" content="<?= \Yii::t('plugins/stock', '可提现金额到达此金额时');?>"
                                            placement="top">
                                    <i class="el-icon-info"></i>
                                </el-tooltip>
                            </template>
                            <el-input min="0" type="number" size="small" style="width: 590px;" v-model="form.min_money" autocomplete="off">
                                <template slot="append"><?= \Yii::t('plugins/stock', '元');?></template>
                            </el-input>
                        </el-form-item>
                        <el-form-item label-width="220px" prop="cash_service_charge">
                            <template slot='label'>
                                <span><?= \Yii::t('plugins/stock', '分红提现手续费');?></span>
                                <el-tooltip effect="dark" content="<?= \Yii::t('plugins/stock', '则不扣除手续费');?>"
                                            placement="top">
                                    <i class="el-icon-info"></i>
                                </el-tooltip>
                            </template>
                        ...
                        <el-form-item label-width="220px">
                            <template slot='label'>
                                <span><?= \Yii::t('plugins/stock', '免手续费提现金额区间');?></span>
                                <el-tooltip effect="dark" placement="top">
                                    <div slot="content" style="text-align: center"><?= \Yii::t('plugins/stock', '单笔申请提现金额在此区间时');?><br/><?= \Yii::t('plugins/stock', '结束金额必须大于起始金额');?><br/><?= \Yii::t('plugins/stock', '均不填');?></div>
                                    <i class="el-icon-info"></i>
                                </el-tooltip>
                            </template>
                            <div flex="dir:left">
                                <div style="margin-right: 10px;min-width: 60px"><?= \Yii::t('plugins/stock', '起始金额');?></div>
                                <div>
                                ...
                                <div style="margin-right: 10px;min-width: 60px"><?= \Yii::t('plugins/stock', '结束金额');?></div>
                                <div>
                ...
                <el-tab-pane v-if="is_stock == 1" label="<?= \Yii::t('plugins/stock', '页面自定义');?>" name="three">
                    <div class="title">
                        <span><?= \Yii::t('plugins/stock', '头部banner图片');?></span>
                    </div>
                    <div class="form-body">
                        <div flex="dir:left">
                            <div class="mobile">
                            ...
                            <div>
                                <el-form-item label="<?= \Yii::t('plugins/stock', '头部banner图片');?>" prop="bg_url">
                                    <div style="position: relative">
                                        <app-attachment :multiple="false" :max="1" @selected="topPicUrl">
                                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/stock', '建议尺寸');?>:750*360" placement="top">
                                                <el-button size="mini"><?= \Yii::t('plugins/stock', '选择文件');?></el-button>
                                            </el-tooltip>
                                        </app-attachment>
                                        ...
                                        <el-button size="mini" @click="resetImg(2,'top')" class="reset" type="primary"><?= \Yii::t('plugins/stock', '恢复默认');?></el-button>
                                    </div>
                                </el-form-item>
                                <el-form-item label="<?= \Yii::t('plugins/stock', '底部图片');?>" prop="bottom_bg_url">
                                    <div style="position: relative">
                                        ...
                                        <el-button size="mini" @click="resetImg(2,'bottom')" class="reset" type="primary"><?= \Yii::t('plugins/stock', '恢复默认');?></el-button>
                                    </div>
                                </el-form-item>
                            </div>
                        </div>
                    ...
                    <div class="title" style="margin-top: 10px">
                        <span><?= \Yii::t('plugins/stock', '自定义文字');?></span>
                    </div>
                    <div class="form-body" style="padding: 40px;display: flex;">
                        <div class='left-setting-menu'>
                            <el-form-item label-width="140px" :class='active_setting == "1" ? "active":""' @click.native='chooseSetting("1")' label="<?= \Yii::t('plugins/stock', '入口位置');?>">
                            </el-form-item>
                            <el-form-item label-width="140px" :class='active_setting == "2" ? "active":""' @click.native='chooseSetting("2")' label="<?= \Yii::t('plugins/stock', '页面标题');?>">
                            </el-form-item>
                            <el-form-item label-width="140px" :class='active_setting == "3" ? "active":""' @click.native='chooseSetting("3")' label="<?= \Yii::t('plugins/stock', '主页');?>">
                            </el-form-item>
                            <el-form-item label-width="140px" :class='active_setting == "4" ? "active":""' @click.native='chooseSetting("4")' label="<?= \Yii::t('plugins/stock', '股东升级');?>">
                            </el-form-item>
                            <el-form-item label-width="140px" :class='active_setting == "5" ? "active":""' @click.native='chooseSetting("5")' label="<?= \Yii::t('plugins/stock', '股东分红');?>">
                            </el-form-item>
                            <el-form-item label-width="140px" :class='active_setting == "6" ? "active":""' @click.native='chooseSetting("6")' label="<?= \Yii::t('plugins/stock', '结算明细');?>">
                            </el-form-item>
                        </div>
                        <div style='background-color: #F3F5F6;padding: 30px;border-radius: 10px;overflow-y: auto;' :class='active_setting == "1" ? "no-radius":""' flex="dir:left">
                            <div v-if='active_setting == "1"' class="item-img">
                                ...
                                <div class="enter" flex="dir:left">
                                    <div class="enter-item">
                                        ...
                                        <div><?= \Yii::t('plugins/stock', '分销佣金');?></div>
                                        <div><span>3500</span><?= \Yii::t('plugins/stock', '元');?></div>
                                    </div>
                                    <div class="enter-item">
                                        <img src="statics/img/mall/share-custom/img-share-order.png" alt="">
                                        <div><?= \Yii::t('plugins/stock', '分销订单');?></div>
                                        <div><span>26</span><?= \Yii::t('plugins/stock', '元');?></div>
                                    </div>
                                    <div class="enter-item">
                                        <img src="statics/img/mall/share-custom/img-share-cash.png" alt="">
                                        <div><?= \Yii::t('plugins/stock', '提现明细');?></div>
                                        <div><span>10</span><?= \Yii::t('plugins/stock', '元');?></div>
                                    </div>
                                    <div class="enter-item">
                                        <img src="statics/img/mall/share-custom/img-share-team.png" alt="">
                                        <div><?= \Yii::t('plugins/stock', '我的团队');?></div>
                                    ...
                                    <div class="enter-item">
                                        ...
                                        <div>{{form.form.entry_bonus?form.form.entry_bonus:'<?= \Yii::t('plugins/stock', '股东分红');?>'}}</div>
                                    </div>
                                </div>
                            </div>
                            <div v-if='active_setting == "2" || active_setting == "3"' class="item-img">
                                <div class="page_title">{{form.form.title?form.form.title:'<?= \Yii::t('plugins/stock', '股东分红');?>'}}</div>
                                ...
                                <div class="rate_text"><?= \Yii::t('plugins/stock', '当前');?>{{form.form.rate ? form.form.rate : '<?= \Yii::t('plugins/stock', '分红比例');?>'}}<?= \Yii::t('plugins/stock', '为10');?></div>
                                <div class="total_bonus_text">{{form.form.total_bonus ? form.form.total_bonus : '<?= \Yii::t('plugins/stock', '可提现分红');?>'}}(<?= \Yii::t('plugins/stock', '元');?>)</div>
                                <div class="cash_bonus">{{form.form.cash_bonus ? form.form.cash_bonus : '<?= \Yii::t('plugins/stock', '已提现分红');?>'}}(<?= \Yii::t('plugins/stock', '元');?>)</div>
                                <!-- <div class="cash_bonus all_bonus">{{form.form.all_bonus ? form.form.all_bonus : '累计提现分红'}}(<?= \Yii::t('plugins/stock', '元');?>)</div> -->
                                <div class="menu-list enter" flex="dir:left">
                                    <div class="enter-item">
                                        <img src="statics/img/plugins/stock/bonus.png" alt="">
                                        <div>{{form.form.stock?form.form.stock:'<?= \Yii::t('plugins/stock', '股东分红');?>'}}</div>
                                    </div>
                                    <div class="enter-item">
                                        <img src="statics/img/plugins/stock/cash.png" alt="">
                                        <div>{{form.form.cash_detail?form.form.cash_detail:'<?= \Yii::t('plugins/stock', '提现明细');?>'}}</div>
                                    </div>
                                    <div class="enter-item">
                                        <img src="statics/img/plugins/stock/detail.png" alt="">
                                        <div>{{form.form.balance_detail?form.form.balance_detail:'<?= \Yii::t('plugins/stock', '结算明细');?>'}}</div>
                                    </div>
                                </div>
                            </div>
                            <div v-if='active_setting == "4"' class="item-img">
                                ...
                                <div class="level_rate_text">{{form.form.level_rate ? form.form.level_rate : '<?= \Yii::t('plugins/stock', '分红比例');?>'}}</div>
                                <div class="level_rate__list_text">
                                    <div>{{form.form.level_rate ? form.form.level_rate : '<?= \Yii::t('plugins/stock', '分红比例');?>'}} <span style="color: #ff4544;font-size: 13px;">15%</span></div>
                                    <div>{{form.form.level_rate ? form.form.level_rate : '<?= \Yii::t('plugins/stock', '分红比例');?>'}} <span style="color: #ff4544;font-size: 13px;">20%</span></div>
                                    <div>{{form.form.level_rate ? form.form.level_rate : '<?= \Yii::t('plugins/stock', '分红比例');?>'}} <span style="color: #ff4544;font-size: 13px;">25%</span></div>
                                </div>
                            </div>
                            <div v-if='active_setting == "5"' class="item-img">
                                ...
                                <div class="bonus_title_text">{{form.form.bonus_title ? form.form.bonus_title : '<?= \Yii::t('plugins/stock', '股东分红');?>'}}(<?= \Yii::t('plugins/stock', '元');?>)</div>
                                <div class="bonus_total_text" flex="main:justify">
                                    <div>{{form.form.bonus_total ? form.form.bonus_total : '<?= \Yii::t('plugins/stock', '可提现分红');?>'}}</div>
                                    <div>2500<?= \Yii::t('plugins/stock', '元');?></div>
                                </div>
                                <div class="bonus_cash_text" flex="main:justify">
                                    <div>{{form.form.bonus_cash ? form.form.bonus_cash : '<?= \Yii::t('plugins/stock', '已提现分红');?>'}}</div>
                                    <div>0<?= \Yii::t('plugins/stock', '元');?></div>
                                </div>
                                <div class="bonus_loading_text" flex="main:justify">
                                    <div>{{form.form.bonus_loading ? form.form.bonus_loading : '<?= \Yii::t('plugins/stock', '待打款分红');?>'}}</div>
                                    <div>0<?= \Yii::t('plugins/stock', '元');?></div>
                                </div>
                            </div>
                                ...
                                <div class="balance-list">
                                    <div class="balance-item">
                                        <div flex="dir:left cross:center" style="margin-bottom: 10px">
                                            <div class="balance-name"><?= \Yii::t('plugins/stock', '青铜股东');?></div>
                                        </div>
                                        <div>
                                            <span><?= \Yii::t('plugins/stock', '订单数');?> 102</span>
                                            <span style="margin-left: 10px;">{{form.form.rate_text ? form.form.rate_text : '<?= \Yii::t('plugins/stock', '分红比例');?>'}}15%</span>
                                        </div>
                                        <div class="bonus-info" flex="dir:top main:center">
                                            <div>{{form.form.price_text ? form.form.price_text : '<?= \Yii::t('plugins/stock', '分红金额');?>'}}</div>
                                    ...
                                    <div class="balance-item">
                                        ...
                                        <div>
                                            <span><?= \Yii::t('plugins/stock', '订单数');?> 859626</span>
                                            <span style="margin-left: 10px;">{{form.form.rate_text ? form.form.rate_text : '<?= \Yii::t('plugins/stock', '分红比例');?>'}}15%</span>
                                        </div>
                                        <div class="bonus-info" flex="dir:top main:center">
                                            <div>{{form.form.price_text ? form.form.price_text : '<?= \Yii::t('plugins/stock', '分红金额');?>'}}</div>
                                    ...
                                    <div class="balance-item">
                                        <div flex="dir:left cross:center" style="margin-bottom: 10px">
                                            <div class="balance-name"><?= \Yii::t('plugins/stock', '默认等级');?></div>
                                        </div>
                                        <div>
                                            <span><?= \Yii::t('plugins/stock', '订单数');?> 8592</span>
                                            <span style="margin-left: 10px;">{{form.form.rate_text ? form.form.rate_text : '<?= \Yii::t('plugins/stock', '分红比例');?>'}}10%</span>
                                        </div>
                                        <div class="bonus-info" flex="dir:top main:center">
                                            <div>{{form.form.price_text ? form.form.price_text : '<?= \Yii::t('plugins/stock', '分红金额');?>'}}</div>
                            ...
                            <div class="item-img" style="border: 0;width: 570px;margin-left: 20px;">
                                <div style="height: 32px;margin-left: 40px;margin-bottom: 20px" flex="dir:left cross:center" v-if="active_setting == 1">
                                    <div class="default"><?= \Yii::t('plugins/stock', '股东分红');?></div>
                                ...
                                <div style="height: 32px;margin-left: 40px;margin-bottom: 20px" flex="dir:left cross:center" v-if="active_setting == 3">
                                    <div class="default"><?= \Yii::t('plugins/stock', '已提现分红');?></div>
                                    <app-image width="12px" height="12px" mode="aspectFill" :src="customize_pic"></app-image>
                                ...
                                <div style="height: 32px;margin-left: 40px;margin-bottom: 20px" flex="dir:left cross:center" v-if="active_setting == 3">
                                    <div class="default"><?= \Yii::t('plugins/stock', '提现明细');?></div>
                                    <app-image width="12px" height="12px" mode="aspectFill" :src="customize_pic"></app-image>
                                ...
                                <div style="height: 32px;margin-left: 40px;margin-bottom: 20px" flex="dir:left cross:center" v-if="active_setting == 3">
                                    <div class="default"><?= \Yii::t('plugins/stock', '结算明细');?></div>
                                    <app-image width="12px" height="12px" mode="aspectFill" :src="customize_pic"></app-image>
                                ...
                                <div style="height: 32px;margin-left: 40px;margin-bottom: 20px" flex="dir:left cross:center" v-if="active_setting == 5">
                                    <div class="default"><?= \Yii::t('plugins/stock', '待打款分红');?></div>
                                    <app-image width="12px" height="12px" mode="aspectFill" :src="customize_pic"></app-image>
                                ...
                                <div style="height: 32px;margin-left: 40px;margin-bottom: 20px" flex="dir:left cross:center" v-if="active_setting == 6">
                                    <div class="default"><?= \Yii::t('plugins/stock', '分红金额');?></div>
                                    <app-image width="12px" height="12px" mode="aspectFill" :src="customize_pic"></app-image>
                ...
                <el-button class="button-item" type="primary" size="small" :loading="submitLoading" @click="submit('form')"><?= \Yii::t('plugins/stock', '保存');?></el-button>
        </el-form>
    </el-card>
</div>
        ...
        data() {
            var validateRate = (rule, value, callback) => {
                if (value === '' || value === undefined) {
                    callback(new Error('<?= \Yii::t('plugins/stock', '请填写订单最大分红比例');?>'));
                } else if (value > 100) {
                    callback(new Error('<?= \Yii::t('plugins/stock', '分红比例不得大于100');?>'));
                } else {
                    callback();
                }
            };
            var validateRate2 = (rule, value, callback) => {
                if (this.bg_url === '' || this.bg_url === undefined) {
                    callback(new Error('<?= \Yii::t('plugins/stock', '请选择头部banner图');?>'));
                } else {
                    callback();
                }
            };
            var validateRate3 = (rule, value, callback) => {
                if (this.bottom_bg_url === '' || this.bottom_bg_url === undefined) {
                    callback(new Error('<?= \Yii::t('plugins/stock', '请选择底部图');?>'));
                } else {
                    callback();
                }
            };
            var validateRate4 = (rule, value, callback) => {
                if (this.form.cash_service_charge === '' || this.form.cash_service_charge === undefined) {
                    callback(new Error('<?= \Yii::t('plugins/stock', '请填写分红提现手续费');?>'));
                } else if (this.form.cash_service_charge > 100) {
                    callback(new Error('<?= \Yii::t('plugins/stock', '分红提现手续费不得大于100');?>'));
                } else {
                    callback();
                }
            };
            return {
                ...
                rules: {
                    is_stock: [
                        { required: true, message: '<?= \Yii::t('plugins/stock', '请选择是否开启股东分红');?>', trigger: 'change' }
                    ],
                    stock_rate: [
                        { required: true, validator: validateRate, trigger: 'blur' }
                    ],
                    base_rate: [
                        { required: true, message: '<?= \Yii::t('plugins/stock', '请填写基础等级分红比例');?>', trigger: 'blur' }
                    ],
                    apply_type: [
                        { required: true, message: '<?= \Yii::t('plugins/stock', '请选择申请成为股东的条件');?>', trigger: 'change' }
                    ],
                    become_type: [
                        { required: true, message: '<?= \Yii::t('plugins/stock', '请选择申请成为股东的条件');?>', trigger: 'change' }
                    ],
                    user_instructions: [
                        { required: true, message: '<?= \Yii::t('plugins/stock', '请填写用户须知');?>', trigger: 'blur' }
                    ],
                    condition: [
                        { required: true, message: '<?= \Yii::t('plugins/stock', '请填写升级条件');?>', trigger: 'blur' }
                    ],
                    bg_url: [
