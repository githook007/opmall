...
class Plugin extends \app\plugins\Plugin
{
    ...
    public function getMenus()
    {
        return [
            [
                'name' => \Yii::t('plugins/aliapp', '基础配置'),
...
//            ],
            [
                'name' => \Yii::t('plugins/aliapp', '小程序发布'),
    ...
    public function getAliConfig()
    {
        ...
        if (!$aliappConfig) {
            throw new \Exception(\Yii::t('plugins/aliapp', '支付宝小程序支付尚未配置'));
        }
        return $aliappConfig;
    }
    ...
    {
        return \Yii::t('plugins/aliapp', '支付宝小程序');
    }
    ...
    public function addTemplateList($attributes)
    {
        foreach ($attributes as $item) {
            if (!isset($item['tpl_name'])) {
                throw new \Exception(\Yii::t('plugins/aliapp', '缺少必要的参数'));
            }
            if (!isset($item[$item['tpl_name']])) {
                throw new \Exception(\Yii::t('plugins/aliapp', '缺少必要的参数')."{$item['tpl_name']}");
            }
    ...
    public function getPlatformIconUrl()
    {
        return [
            [
                ...
                'icon' => PluginHelper::getPluginBaseAssetsUrl($this->getName()) . '/img/aliapp.png'
            ]
        ];
    }
}
