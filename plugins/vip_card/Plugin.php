...
class Plugin extends \app\plugins\Plugin
{
    ...
    public function getMenus()
    {
        return [
            [
                'name' => \Yii::t('plugins/vip_card', '基础配置'),
...
//            ],
            [
                'name' => \Yii::t('plugins/vip_card', '会员卡管理'),
            ...
            [
                'name' => \Yii::t('plugins/vip_card', '订单管理'),
                'route' => 'plugin/vip_card/mall/order/index',
                'icon' => 'el-icon-star-on',
            ],
            [
                'name' => \Yii::t('plugins/vip_card', '会员管理'),
    ...
    {
        return \Yii::t('plugins/vip_card', '超级会员卡');
    }
    ...
    public function getPickLink()
    {
        ...
        return [
            [
                'key' => 'vip_card',
                'name' => \Yii::t('plugins/vip_card', '超级会员卡'),
                'open_type' => '',
    ...
    public function vipDiscount($mchItem, $useTempVip = false, $orderSubmitForm = null, $user = null)
    {
        ...
        foreach ($mchItem['goods_list'] as &$goodsItem) {
            ...
            if (is_array($typeInfo) && !empty($typeInfo)) {
                ...
                $isAppoint = !empty($appoint);
                $setting = $this->getRules();
                $isRule = in_array($goodsItem['sign'], $setting['rules']);

                if (($typeInfo['all'] || $isInGoods || $isInCats) && $isAppoint && $isRule) {

                    $discountName = ($main->name ?? $vip->image_name) . \Yii::t('plugins/vip_card', '优惠');
                    ...
                    if ($vipSubPrice != 0) {
                        // 减去超级会员卡优惠金额
                        $vipSubPrice = price_format(min($goodsItem['total_price'], $vipSubPrice));
                        ...
                        $goodsItem['vip_discount'] = $vipSubPrice;
                    }
                }
            }
        }
    ...
    {
        $p = 1;
        $errorMsg = \Yii::t('plugins/vip_card', '有会员卡权限');
        $permission = \Yii::$app->branch->childPermission(\Yii::$app->mall->user->adminInfo);
        if (!in_array('vip_card', $permission)) {
            $errorMsg = \Yii::t('plugins/vip_card', '无会员卡权限');
            $p = 0;
        }
        ...
        if ($setting['is_vip_card'] == 0) {
            $errorMsg = \Yii::t('plugins/vip_card', '会员卡插件已关闭');
            $p = 0;
        }

        $card = $this->getCard();
        if (!$card) {
            $errorMsg = \Yii::t('plugins/vip_card', '尚未添加会员卡');
            $p = 0;
        }
        ...
        try {
            ...
            if ($vip) {
                ...
                $data = [
                    'discount_list' => [
                        'vip_discount' => [
                            'label' => $name . \Yii::t('plugins/vip_card', '优惠'),
                            'value' => $vip->discount,
                        ],
                    ],
                    'print_list' => [
    ...
    public function getSmsSetting()
    {
        return [
            'vipCard' => [
                'title' => \Yii::t('plugins/vip_card', '超级会员卡续费短信通知'),
                'content' => \Yii::t('plugins/vip_card', '您的超级会员卡即将到期').'${name}',
                'support_mch' => false,
                'loading' => false,
                'variable' => [
                    [
                        'key' => 'name',
                        'value' => \Yii::t('plugins/vip_card', '模板变量name'),
                        'desc' => \Yii::t('plugins/vip_card', '请及时续费').'${name}'.\Yii::t('plugins/vip_card', '，则需填写name'),
                    ],
                ],
                'key' => 'user'
            ],
    ...
    {
        $sql = <<<EOF
ALTER TABLE `op_vip_card_user` MODIFY COLUMN `all_send` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL COMMENT '所有赠送信息' AFTER `image_name`;
EOF;
        sql_execute($sql);
