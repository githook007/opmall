...
class PintuanSuccessForm extends Model
{
    ...
    private function sendTemplateMsg($item)
    {
        try {
            ...
            TemplateList::getInstance()->getTemplateClass(PintuanSuccessInfo::TPL_NAME)->send([
                'goodsName' => $goodsName,
     ...
     * 通过公众号向商家发送公众号消息
     */
    private function sendMpTemplate(PintuanOrders $pintuanOrder)
    {
        try {
            ...
            $tplMsg->params = [
                ...
                'order_no' => '拼团订单号集合',
                'time' => $pintuanOrder->success_time,
                'is_pay' => $pintuanOrder->getStatusText($pintuanOrder),
                'remark' => '拼团订单预计失效时间 - ' . $pintuanOrder->expected_over_time
            ];
