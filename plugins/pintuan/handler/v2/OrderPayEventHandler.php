...
class OrderPayEventHandler extends OrderPayedHandlerClass
{
    ...
    public function handle()
    {
         ...
         * 去掉取消状态  @czs
         */
        $this->event->order = Order::findOne($this->event->order->id);
        if ($this->event->order->cancel_status == 1 && $this->event->order->pay_type == 1) {
            $this->event->order->cancel_status = 0;
            $this->event->order->cancel_time = "0000-00-00 00:00:00";
