    ...
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 0 0;">
        <div slot="header">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><span style="color: #409EFF;cursor: pointer" @click="$navigate({r:'plugin/pintuan/mall/goods/index'})"><?= \Yii::t('plugins/pintuan', '商品列表');?></span></el-breadcrumb-item>
                <el-breadcrumb-item v-if="form.goods_id > 0"><?= \Yii::t('plugins/pintuan', '详情');?></el-breadcrumb-item>
                <el-breadcrumb-item v-else><?= \Yii::t('plugins/pintuan', '添加商品');?></el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <app-goods
                sign="pintuan"
            ...
            <template slot="before_detail">
                <el-card shadow="never" style="margin-top: 24px">
                    <el-form-item label-width="120px" label="<?= \Yii::t('plugins/pintuan', '阶梯团设置');?>" prop="desc">
                        <el-table
                    ...
                    <div class="goods">
                        ...
                        <div flex="dir:left" style="font-size:14px">
                            <div flex="dir:top" style="font-size: 10px">
                                ...
                                <div flex="dir:left">
                                    <div style="padding:3px 6px;background: #feeeee;color:#ff4544;"><?= \Yii::t('plugins/pintuan', '2人拼团');?></div>
                                    <div style="margin-left: 3px;padding:3px 6px;background:#feeeee;color:#ff4544">
                                        <?= \Yii::t('plugins/pintuan', '拼团立省￥');?>{{previewData.original_price - previewData.group_min_price}}
                                    </div>
                                </div>
                            </div>
            ...
            <template slot="member_route_setting">
                 <span class="red"><?= \Yii::t('plugins/pintuan', '必须在');?>
                    <el-button type="text" @click="$navigate({r: 'plugin/pintuan/mall/index'}, true)"><?= \Yii::t('plugins/pintuan', '优惠叠加设置');?></el-button>
    ...
    const app = new Vue({
        ...
        data() {
            return {
                ...
                rule: {
                    is_alone_buy: [
                        {required: true, message: '<?= \Yii::t('plugins/pintuan', '请选择是否允许单独购买');?>', trigger: 'change'},
                    ],
                    end_time: [
                        {required: true, message: '<?= \Yii::t('plugins/pintuan', '请选择拼团结束时间');?>', trigger: 'change'},
                    ],
                    groups_restrictions: [
                        {required: true, message: '<?= \Yii::t('plugins/pintuan', '请输入拼团次数限制');?>', trigger: 'change'},
                    ],
                },
