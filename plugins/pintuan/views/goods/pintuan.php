...
<div id="app" v-cloak>
    <el-card class="app-goods" v-loading="cardLoading" shadow="never" style="border:0"
        ...
        <div slot="header">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><span style="color: #409EFF;cursor: pointer"
                                          @click="$navigate({r:'plugin/pintuan/mall/goods/index'})"><?= \Yii::t('plugins/pintuan', '商品列表');?></span>
                </el-breadcrumb-item>
                <el-breadcrumb-item><?= \Yii::t('plugins/pintuan', '阶梯团');?></el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="form-body">
            <el-form :model="ruleForm"
                ...
                <el-tabs v-model="activeName" @tab-click="handleClick">
                    <el-tab-pane label="<?= \Yii::t('plugins/pintuan', '拼团设置');?>" name="first">
                        <el-form-item label-width="120px" prop="desc">
                            <el-table
                                    ...
                                    style="width: 100%">
                                <el-table-column
                                        label="<?= \Yii::t('plugins/pintuan', '拼团人数');?>"
                                        width="200">
                                ...
                                <el-table-column
                                        label="<?= \Yii::t('plugins/pintuan', '拼团时间');?>"
                                        width="400">
                                    <template slot-scope="scope">
                                        <el-input v-model.number="scope.row.pintuan_time" placeholder="<?= \Yii::t('plugins/pintuan', '请输入拼团时间');?>">
                                            <template slot="append"><?= \Yii::t('plugins/pintuan', '小时');?></template>
                                        </el-input>
                                    </template>
                                </el-table-column>
                                <el-table-column
                                        label="<?= \Yii::t('plugins/pintuan', '团长数量');?>"
                                        :render-header="renderHeader"
                                        width="200">
                                    <template slot-scope="scope">
                                        <el-input v-model.number="scope.row.group_num" placeholder="<?= \Yii::t('plugins/pintuan', '请输入团长数量');?>">
                                            <template slot="append"><?= \Yii::t('plugins/pintuan', '人');?></template>
                                        </el-input>
                                    </template>
                                </el-table-column>
                                <el-table-column
                        ...
                        <el-card v-for="(item,index) in pintuan" style="margin-top: 24px;"
                            ...
                            <el-form-item v-if="attrGroups.length" label="<?= \Yii::t('plugins/pintuan', '价格与库存');?>">
                                <app-attr :attr-groups="attrGroups" v-model="item.attr"
                                          :list="{pintuan_price: '<?= \Yii::t('plugins/pintuan', '拼团价');?>', pintuan_stock: '<?= \Yii::t('plugins/pintuan', '拼团库存');?>'}"></app-attr>
                            </el-form-item>
                        </el-card>
                    </el-tab-pane>
                    <el-tab-pane label="<?= \Yii::t('plugins/pintuan', '分销价设置');?>" name="second">
                        <el-row>
                            <template v-if="ruleForm.individual_share == 1">
                                <template v-if="pintuan.length > 0">
                                    <el-card v-for="(item, index) in pintuan" :key="index">
                                        <el-tag type="danger" slot="header">
                                            {{item.people_num}}<?= \Yii::t('plugins/pintuan', '人团');?>
                                        </el-tag>
                                ...
                                <template v-else>
                                    <el-form-item>
                                        <el-tag type="danger"><?= \Yii::t('plugins/pintuan', '请先添加阶梯团');?></el-tag>
                                    </el-form-item>
                                </template>
                            </template>
                            <template v-else>
                                <el-tag style="margin: 15px 0;" type="danger"><?= \Yii::t('plugins/pintuan', '拼团商品未开启分销功能');?></el-tag>
                            </template>
                        </el-row>
                    </el-tab-pane>
                    <el-tab-pane label="<?= \Yii::t('plugins/pintuan', '会员价设置');?>" name="third">
                        <template v-if="ruleForm.is_level == 1">
                                ...
                                <template v-if="pintuan.length > 0">
                                    <el-card v-for="(item, index) in pintuan">
                                        ...
                                        <template v-if="ruleForm.use_attr == 1 && memberLevel.length > 0">
                                            <!--多规格会员价设置-->
                                            <el-form-item label="<?= \Yii::t('plugins/pintuan', '会员价设置');?>">
                                                <app-attr :attr-groups="attrGroups" v-model="item.attr"
                                        ...
                                        <template v-if="ruleForm.use_attr == 0 && memberLevel.length > 0">
                                            <el-form-item label="<?= \Yii::t('plugins/pintuan', '默认规格会员价设置');?>">
                                                <el-col :span="12">
                                                    <el-input v-for="mItem in item.defaultMemberPrice"
                                                        ...
                                                        <span slot="append"><?= \Yii::t('plugins/pintuan', '元');?></span>
                                                    </el-input>
                                                </el-col>
                                            </el-form-item>
                                            <el-form-item>
                                                <el-tag type="danger"><?= \Yii::t('plugins/pintuan', '如需设置多规格会员价');?></el-tag>
                                            </el-form-item>
                                        </template>

                                        <el-form-item v-if="memberLevel.length == 0" label="<?= \Yii::t('plugins/pintuan', '会员价设置');?>">
                                            <el-button type="danger"
                                                       @click="$navigate({r: 'mall/mall-member/edit'})">
                                                <?= \Yii::t('plugins/pintuan', '如需设置');?>
                                            </el-button>
                                        </el-form-item>
                                    </el-card>
                                </template>
                            ...
                            <template v-else>
                                <el-tag style="margin: 15px 0;" type="danger"><?= \Yii::t('plugins/pintuan', '拼团商品享受全局会员折扣');?></el-tag>
                            </template>
                        </template>
                        <template v-else>
                            <el-tag style="margin: 15px 0;" type="danger"><?= \Yii::t('plugins/pintuan', '拼团商品未开启会员功能');?></el-tag>
                        </template>
                    </el-tab-pane>
                </el-tabs>
            </el-form>
        ...
        <el-button class="button-item" :loading="btnLoading" type="primary" style="margin-top: 24px" size="small"
                   @click="store('ruleForm')"><?= \Yii::t('plugins/pintuan', '保存');?>
        </el-button>
    </el-card>
</div>
<script>
            ...
            store(formName) {
                ...
                self.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        self.$message.error('<?= \Yii::t('plugins/pintuan', '请填写必填参数');?>');
                        return false;
                    }
                });
            },
