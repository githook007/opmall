        ...
        <div class="text item" style="width:100%">
            <el-form :model="form" label-width="150px" :rules="rule" ref="form">
                <el-tabs v-model="activeName">
                    ...
                    <el-tab-pane label="<?= \Yii::t('plugins/pintuan', '基本设置');?>" class="form-body" name="first">
                        <app-setting :is_full_reduce="true" v-model="form.detail" :is_coupon="true" ></app-setting>

                        <el-card>
                            <div slot="header"><?= \Yii::t('plugins/pintuan', '拼团规则设置');?></div>
                            <el-form-item label="<?= \Yii::t('plugins/pintuan', '活动规则');?>">
                    ...
                    <el-tab-pane v-if="false" label="<?= \Yii::t('plugins/pintuan', '自定义海报');?>" class="form-body" style="background:none;padding:0" name="second">
                        <app-poster :rule_form="form.goods_poster"
                                    :goods_component="goodsComponent"
                        ></app-poster>
                    </el-tab-pane>

                    <el-tab-pane label="<?= \Yii::t('plugins/pintuan', '轮播图');?>" class="form-body" name="fitrst">
                        <app-plugins-banner
                                @concat_list="concat_banner_list"
                                @delete_list="delete_banner_list"
                                pic_size="<?= \Yii::t('plugins/pintuan', '建议尺寸');?>:750 * 230"
                                :list="form.banner_list"></app-plugins-banner>
                    </el-tab-pane>

                    <el-tab-pane label="<?= \Yii::t('plugins/pintuan', '拼团广告');?>" class="form-body" name="f">
                        <el-card style="border:0;background-color: #ffffff" shadow="never">
                            <el-form-item label="<?= \Yii::t('plugins/pintuan', '拼团广告状态');?>" prop="is_advertisement">
                                <el-switch v-model="form.detail.is_advertisement" :active-value="1"
                                           :inactive-value="0"></el-switch>
                            </el-form-item>
                            <el-form-item label="<?= \Yii::t('plugins/pintuan', '样式选择');?>">
                                <div @click="selectStyle(item)" v-for="item in style_1" class="block-box"
                                    ...
                                    <img :src="item.bg_url">
                                    <div class="open-css">
                                        <el-button type="primary"><?= \Yii::t('plugins/pintuan', '启用该样式');?></el-button>
                                    </div>
                                </div>
                            ...
                            <el-form-item label="<?= \Yii::t('plugins/pintuan', '板块设置');?>">
                                <template v-if="form.detail.advertisement.list.length > 0">
                                    <el-row v-for="(item,index) in form.detail.advertisement.list"
                                        ...
                                        <el-col :span="2" style="min-width: 100px">
                                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/pintuan', '建议尺寸');?>:300*600"
                                                        placement="top">
                                        ...
                                        <el-col :span="10">
                                            <div flex="box:last">
                                                ...
                                                <app-pick-link :params="{'index': index}" @selected="selectLinkUrl">
                                                    <el-button size="mini"><?= \Yii::t('plugins/pintuan', '选择链接');?></el-button>
                                                </app-pick-link>
                                            </div>
                                        </el-col>
                                    </el-row>
                                ...
                                <template v-else>
                                    <el-tag type="danger"><?= \Yii::t('plugins/pintuan', '请先选择样式');?></el-tag>
                                </template>
                            </el-form-item>
                        </el-card>
                    </el-tab-pane>
