...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <div>
                <span><?= \Yii::t('plugins/pintuan', '活动数据');?></span>
            </div>
        </div>
        <div class="table-body">
                ...
                <el-form size="small" :inline="true" @submit.native.prevent>
                    ...
                    <el-form-item >
                        <div class="input-item">
                            <el-input @keyup.enter.native="requestList" size="small" class="input-with-select" placeholder="<?= \Yii::t('plugins/pintuan', '请输入内容');?>"
            ...
            <el-table :data="list" border tooltip-effect="dark" v-loading="loading" style="width: 100%">
                <el-table-column
                        label="<?= \Yii::t('plugins/pintuan', '商品信息');?>"
                >
                ...
                >
                    <template slot-scope="scope">
                        {{scope.row.people_num}}人团{{scope.row.robot_num > 0 && scope.row.status === 2 ? '<?= \Yii::t('plugins/pintuan', '含机器人');?>' + scope.row.robot_num  +'）' : ''}}
                    </template>
                </el-table-column>
                <el-table-column
                        prop="date"
                    ...
                    <template slot-scope="scope">
                        <div >
                            ...
                            <img style="vertical-align:middle;" width="24px" height="24px" :src="scope.row.platform_icon" alt="">
                            {{scope.row.group_nickname}}
                        </div>
                        <div>
                            <?= \Yii::t('plugins/pintuan', '团长优惠');?>{{scope.row.preferential_price}}
                        </div>
                    </template>
                </el-table-column>
                <el-table-column
        ...
        data() {
            return {
                search: {
                    date_picker: [],
                    keyword: '',
                    keyword_name: '<?= \Yii::t('plugins/pintuan', '订单号');?>',
                    status: '-1',
                    date_start: '',
                    date_end: '',
                    page: 1
