        ...
        <el-card v-loading="listLoading" class="box-card" shadow="never" style="border:0"
            ...
            <div class="table-body">
                ...
                <el-table
                    ...
                    <el-table-column prop="id" label="<?= \Yii::t('plugins/pintuan', '商品ID');?>" sortable="false" width="120"></el-table-column>
                    <el-table-column label="<?= \Yii::t('plugins/pintuan', '商品名称');?>" width="300">
                    ...
                    <el-table-column prop="created_at" width="180" label="<?= \Yii::t('plugins/pintuan', '活动时间');?>">
                        <template slot-scope="scope">
                            <div>{{scope.row.begin_time}}<?= \Yii::t('plugins/pintuan', '至');?></div>
                            <div>{{scope.row.end_time}}</div>
                        </template>
                    </el-table-column>
                    <el-table-column prop="goods_stock" label="<?= \Yii::t('plugins/pintuan', '库存');?>" sortable="false">
                        <template slot-scope="scope">
                            <div v-if="scope.row.goods_stock > 0">{{scope.row.goods_stock}}</div>
                            <div v-else style="color: red;"><?= \Yii::t('plugins/pintuan', '售罄');?></div>
                        </template>
                    </el-table-column>
                    <el-table-column prop="min_price" width="160" label="<?= \Yii::t('plugins/pintuan', '最低价');?>"></el-table-column>
                    <el-table-column label="<?= \Yii::t('plugins/pintuan', '活动状态');?>" width="100" prop="status_text">
                        <template slot-scope="scope">
                            <el-tag v-if="scope.row.status_text == 1" type="info"><?= \Yii::t('plugins/pintuan', '未开始');?></el-tag>
                            <el-tag v-if="scope.row.status_text == 2"><?= \Yii::t('plugins/pintuan', '进行中');?></el-tag>
                            <el-tag v-if="scope.row.status_text == 3" type="danger"><?= \Yii::t('plugins/pintuan', '已结束');?></el-tag>
                            <el-tag v-if="scope.row.status_text == 4" type="warning"><?= \Yii::t('plugins/pintuan', '已下架');?></el-tag>
                        </template>
                    </el-table-column>
                    <el-table-column
                        ...
                        <template slot-scope="scope">
                            ...
                            <template v-if="is_action">
                                ...
                                <el-button @click="goto(scope.row)" type="text" circle size="mini">
                                    <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/pintuan', '活动数据');?>" placement="top">
...
<script>
    Vue.component('app-goods-list', {
        ...
        props: {
            ...
            add_goods_title: {
                type: String,
                default: '<?= \Yii::t('plugins/pintuan', '添加商品');?>',
            },
            header_title: {
                type: String,
                default: '<?= \Yii::t('plugins/pintuan', '商品列表');?>',
            },
            tabs: {
                type: Array,
                default: function () {
                    return [
                        ...
                        {
                            name: '<?= \Yii::t('plugins/pintuan', '已下架');?>',
                            value: '0'
                        },
                    ];
                }
        ...
        methods: {
            ...
            destroy(row, index) {
                let self = this;
                self.$confirm('<?= \Yii::t('plugins/pintuan', '删除该条数据');?>', '<?= \Yii::t('plugins/pintuan', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('plugins/pintuan', '确定');?>',
                    cancelButtonText: '<?= \Yii::t('plugins/pintuan', '取消');?>',
                    type: 'warning'
                }).then(() => {
                    self.listLoading = true;
                    request({
                    ...
                    self.$message.info('<?= \Yii::t('plugins/pintuan', '已取消删除');?>')
                });
            },
            applyStatus(id) {
                let self = this;
                self.$confirm('<?= \Yii::t('plugins/pintuan', '申请上架');?>', '<?= \Yii::t('plugins/pintuan', '提示');?>', {
                    confirmButtonText: '<?= \Yii::t('plugins/pintuan', '确定');?>',
                    cancelButtonText: '<?= \Yii::t('plugins/pintuan', '取消');?>',
                    type: 'warning'
                }).then(() => {
                    ...
                    self.$message.info('<?= \Yii::t('plugins/pintuan', '已取消申请');?>');
                });
            },
            // 商品上下架
            switchStatus(row) {
            ...
            changeSortSubmit(row) {
                ...
                if (!row.sort || row.sort < 0) {
                    self.$message.warning('<?= \Yii::t('plugins/pintuan', '排序值不能小于0');?>');
                    return;
                }
                if (this.is_mch) {
