    ...
    <el-card shadow="never" v-loading="detailLoading" style="border:0;background-color: #f3f3f3;" body-style="background-color: #f3f3f3;padding: 0 0 0 0;">
        <div slot="header">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item><?= \Yii::t('plugins/pintuan', '活动数据');?></el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <el-card style="margin-top: 10px">
            <el-tabs v-model="status" @tab-click="getList">
                <el-tab-pane label="<?= \Yii::t('plugins/pintuan', '全部');?>" name="-1"></el-tab-pane>
                <el-tab-pane label="<?= \Yii::t('plugins/pintuan', '进行中');?>" name="1"></el-tab-pane>
            ...
            <el-form size="small" :inline="true" :model="search">
                ...
                <el-form-item label="<?= \Yii::t('plugins/pintuan', '开团时间');?>:">
                    <el-date-picker
                            class="item-box"
                            size="small"
                            @change="toSearch"
                ...
                <el-form-item>
                    <div class="input-item">
                        ...
                        <el-input v-model="search.keyword" placeholder="<?= \Yii::t('plugins/pintuan', '请输入搜索内容');?>" clearable @clear="toSearch"
            ...
            <div flex="dir:left cross: center" style="margin-bottom: 15px;">
                <span style="line-height: 76px;margin-right: 16px"><?= \Yii::t('plugins/pintuan', '商品信息');?></span>
            ...
            >
                ...
                <el-table-column
                        label="<?= \Yii::t('plugins/pintuan', '开团时间');?>"
                ...
                <el-table-column
                        label="<?= \Yii::t('plugins/pintuan', '拼团信息');?>"
                        width="150"
                        show-overflow-tooltip
                >
                    <template slot-scope="scope">
                        {{scope.row.people_num}}<?= \Yii::t('plugins/pintuan', '人团');?>{{scope.row.robot_num > 0 ? '<?= \Yii::t('plugins/pintuan', '含机器人');?>' + scope.row.robot_num +')' : ''}}
                    </template>
                </el-table-column>

                <el-table-column
                        label="<?= \Yii::t('plugins/pintuan', '团长信息');?>"
                        show-overflow-tooltip>
                    <template slot-scope="scope">
                        <div flex="">
                            <span><?= \Yii::t('plugins/pintuan', '团长');?></span>
                        ...
                        <div><?= \Yii::t('plugins/pintuan', '团长优惠');?>{{scope.row.preferential_price}}</div>
                    </template>
                </el-table-column>

                <el-table-column
                        label="<?= \Yii::t('plugins/pintuan', '活动状态');?>"
                        width="100"
                        show-overflow-tooltip>
                    <template slot-scope="scope">
                        <el-tag v-if="scope.row.status === '未开始'" type="info"><?= \Yii::t('plugins/pintuan', '未开始');?></el-tag>
                        <el-tag v-else-if="scope.row.status === 1"><?= \Yii::t('plugins/pintuan', '进行中');?></el-tag>
                        <el-tag v-else-if="scope.row.status === 2" type="success"><?= \Yii::t('plugins/pintuan', '拼团成功');?></el-tag>
                        <el-tag v-else-if="scope.row.status === 3" type="danger"><?= \Yii::t('plugins/pintuan', '拼团失败');?></el-tag>
                        <el-tag v-else-if="scope.row.status === 4" type="danger"><?= \Yii::t('plugins/pintuan', '未退款');?></el-tag>
                    </template>
                </el-table-column>
