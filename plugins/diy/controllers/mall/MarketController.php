...
class MarketController extends Controller
{
    ...
    {
        if (!in_array($action->id, ['get-mall', 'get-template', 'buy', 'pay', 'install', 'issue', 'update-template', 'uninstall'])) {
    ...
    public function actionInstalls()
    {
        $form = new TemplateForm();
        $form->attributes = \Yii::$app->request->get();
        return $this->asJson($form->installAll());
    }

    public function actionInstallListen()
    {
        $form = new TemplateForm();
        $form->attributes = \Yii::$app->request->get();
        return $this->asJson($form->installList());
    }
