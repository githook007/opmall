...
<template id="diy-form">
    <div class="diy-form">
                ...
                <div :style="'background-color:'+ data.backgroundColor+';padding:'+data.padding+'px 0;background-image:url('+data.backgroundPicUrl+');background-size:'+data.backgroundWidth+'% '+data.backgroundHeight+'%;background-repeat:'+repeat+';background-position:'+position">
                    <div v-for="(item,index) in list" :flex="data.style==3?'wrap:wrap':'dir:left'" :style="'margin-top:'+(index == 0 ? '0':data.marginBottom)+'px;padding: 0px '+data.inputPadding+'px;'">
                        ...
                        <div v-if="item.key =='img_upload'" class="img-up">
                            ...
                            <div><?= \Yii::t('plugins/diy', '上传图片');?></div>
                        </div>
                        <div v-else-if="item.key =='textarea'" style="text-align: left;height: 200px;padding: 10px 24px" :class="data.inputBorder?'border':''" :style="'border-color:'+data.inputBorder+';background:'+data.inputBackground+';border-radius:'+data.radius+'px;width: '+(data.style==1?'80%;':'100%;')">
                            ...
                            <span :class="item.key == 'time'|| item.key == 'date'?'time':''" :style="'color:'+data.inputTip" v-else-if="item.key == 'time'|| item.key == 'date'"><?= \Yii::t('plugins/diy', '请选择');?></span>
        ...
        <div class="diy-component-edit">
            <div><?= \Yii::t('plugins/diy', '表单');?></div>
            <div flex="dir:left">
                <el-button style="margin: 20px 0;" @click="visible=true" size="small" type="primary"><?= \Yii::t('plugins/diy', '内容设置');?></el-button>
            </div>
            <el-form label-width="100px" @submit.native.prevent>
                <el-form-item label="<?= \Yii::t('plugins/diy', '样式');?>">
                    <div flex="dir:left">
                ...
                <el-form-item label="<?= \Yii::t('plugins/diy', '填充颜色');?>">
                    <div class="input-color" flex="dir:left cross:center">
                        ...
                        <div style="margin-right: 12px;color: #606266"><?= \Yii::t('plugins/diy', '边框颜色');?></div>
                        <el-color-picker :change="changeColor" size="small"
                                         v-model="data.inputBorder"></el-color-picker>
                        <el-input size="small" style="width: 80px;" v-model="data.inputBorder"></el-input>
                    </div>
                    <div style="color: #909399;margin-left: 212px"><?= \Yii::t('plugins/diy', '不填默认无边框');?></div>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/diy', '名称颜色');?>">
                    <div class="input-color" flex="dir:left cross:center">
                        <el-color-picker :change="changeColor" size="small" v-model="data.inputLabel"></el-color-picker>
                        <el-input size="small" style="width: 80px;margin-right: 25px;"
                                  v-model="data.inputLabel"></el-input>
                        <div style="margin-right: 12px;color: #606266"><?= \Yii::t('plugins/diy', '提示颜色');?></div>
                ...
                <hr>
                <div style="margin-bottom: 20px"><?= \Yii::t('plugins/diy', '提交按钮样式');?></div>
                <el-form-item label="<?= \Yii::t('plugins/diy', '按钮高度');?>">
                    <el-slider v-model="data.buttonHeight" style="float: left;width: 95%" :max="160"
                               show-input></el-slider>
                    <div style="float: right">px</div>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/diy', '按钮边距');?>">
                    <el-slider v-model="data.buttonPadding" style="float: left;width: 95%" :max="50"
                               show-input></el-slider>
                    <div style="float: right">px</div>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/diy', '按钮上间距');?>">
                    <el-slider v-model="data.buttonMargin" style="float: left;width: 95%" :max="50"
                               show-input></el-slider>
                    <div style="float: right">px</div>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/diy', '按钮圆角');?>">
                    <el-slider v-model="data.buttonRadius" style="float: left;width: 95%" :max="data.buttonHeight/2"
                               show-input></el-slider>
                    <div style="float: right">px</div>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/diy', '按钮文本');?>">
                    <el-input size="small" v-model="data.borderContent"></el-input>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/diy', '按钮提交地址');?>">
                    <el-input size="small" v-model="data.borderSubmitUrl"></el-input>
                    <div style="color: #909399;"><?= \Yii::t('plugins/diy', '不填用默认表单提交地址');?></div>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/diy', '返回上一页');?>" >
                    <el-switch
                            v-model="data.isSubmitBack"
                            :active-value="1"
                            :inactive-value="0">
                    </el-switch>
                    <div style="color: #909399;"><?= \Yii::t('plugins/diy', '提交表单后是否返回上一页');?></div>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/diy', '填充颜色');?>">
                    <div class="input-color" flex="dir:left cross:center">
                        <el-color-picker :change="changeColor" size="small"
        ...
        <el-dialog title="<?= \Yii::t('plugins/diy', '表单内容设置');?>" :visible.sync="visible">
            <app-form :value.sync="list"></app-form>
            <div slot="footer">
                <el-button size="small" type="primary" @click="addForm"><?= \Yii::t('plugins/diy', '确定');?></el-button>
            </div>
        </el-dialog>
    </div>
</template>
        ...
        data() {
            return {
                data: {
                    ...
                    padding: 40,
                    buttonRadius: 40,
                    borderContent: "<?= \Yii::t('plugins/diy', '提交');?>",
                    borderSubmitUrl: '', // @czs
                    isSubmitBack: 1, // @czs
