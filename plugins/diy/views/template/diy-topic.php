    ...
    <div class="diy-topic">
        ...
        <div class="diy-component-edit">
            <el-form label-width="100px" @submit.native.prevent>
                <el-form-item label="<?= \Yii::t('plugins/diy', '专题样式');?>">
                    <app-radio v-model="data.style" label="normal"><?= \Yii::t('plugins/diy', '简易模式');?></app-radio>
                    <app-radio v-model="data.style" label="list"><?= \Yii::t('plugins/diy', '列表模式');?></app-radio>
                </el-form-item>
                <template v-if="data.style == 'normal'">
                    <el-form-item label="<?= \Yii::t('plugins/diy', '显示行数');?>">
                        <el-select size="small" v-model="data.count">
                            <el-option :value="1"></el-option>
                            <el-option :value="2"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="logo(1<?= \Yii::t('plugins/diy', '行');?>)">
                        <app-attachment title="<?= \Yii::t('plugins/diy', '选择图片');?>" tag="topic" v-model="data.logo_1">
                            <el-tooltip class="item" effect="dark"
                                        content="<?= \Yii::t('plugins/diy', '建议尺寸');?>104*32"
                                        placement="top">
                    ...
                    <el-form-item label="logo(2<?= \Yii::t('plugins/diy', '行');?>)">
                        <app-attachment title="<?= \Yii::t('plugins/diy', '选择图片');?>" tag="topic" v-model="data.logo_2">
                            <el-tooltip class="item" effect="dark"
                                        content="<?= \Yii::t('plugins/diy', '建议尺寸');?>104*50"
                                        placement="top">
                                <el-button size="mini"><?= \Yii::t('plugins/diy', '选择图片');?></el-button>
                            </el-tooltip>
                        </app-attachment>
                    ...
                    <el-form-item label="<?= \Yii::t('plugins/diy', '专题标签');?>">
                        <app-attachment title="<?= \Yii::t('plugins/diy', '选择图片');?>" tag="topic" v-model="data.icon">
                            <el-tooltip class="item" effect="dark"
                                        content="<?= \Yii::t('plugins/diy', '建议尺寸');?>54*28"
                                        placement="top">
                                <el-button size="mini"><?= \Yii::t('plugins/diy', '选择图片');?></el-button>
                            </el-tooltip>
                        </app-attachment>
                ...
                <template v-else>
                    <el-form-item label="<?= \Yii::t('plugins/diy', '是否显示分类');?>">
                    ...
                    <template v-if="data.cat_show">
                        ...
                        <el-form-item label="<?= \Yii::t('plugins/diy', '选中文字颜色');?>">
                    ...
                    <el-form-item label="<?= \Yii::t('plugins/diy', '分类列表');?>" v-if="data.cat_show">
                        <el-card shadow="never" class="topic-edit-options" style="margin-bottom: 24px"
                            ...
                            <div @click="selectCat(catIndex)">
                                <div flex="box:first">
                                    <div class="diy-topic-label"><?= \Yii::t('plugins/diy', '专题分类');?></div>
                                    <div>{{cat.cat_name}}</div>
                                </div>
                                <div flex="box:first">
                                    <div class="diy-topic-label"><?= \Yii::t('plugins/diy', '菜单名称');?></div>
                                    <div>
                                        <el-input v-model="cat.name" size="small"></el-input>
                                    </div>
                                </div>
                                <div flex="box:first">
                                    <div class="diy-topic-label"><?= \Yii::t('plugins/diy', '自定义专题');?></div>
                                    <div>
                                        <el-switch v-model="cat.custom"></el-switch>
                                    </div>
                                </div>
                                <div flex="box:first" v-if="!cat.custom">
                                    <div class="diy-topic-label"><?= \Yii::t('plugins/diy', '专题数量');?></div>
                                    <div>
                                        <el-input v-model="cat.number" size="small"></el-input>
                                    </div>
                                </div>
                                <div flex="box:first" v-else>
                                    <div class="diy-topic-label"><?= \Yii::t('plugins/diy', '专题列表');?></div>
                                    <div>
                                        <app-dialog-select url="mall/topic/index" :multiple="true"
                                                           @selected="selectTopic" :extra-search="{type: cat.cat_id}"
                                                           title="<?= \Yii::t('plugins/diy', '选择专题');?>" list-key="title">
                                            <el-button size="mini"><?= \Yii::t('plugins/diy', '添加专题');?></el-button>
                                        </app-dialog-select>
                        ...
                        <app-dialog-select url="mall/topic-type/index" :multiple="true" @selected="selectCatList"
                                           title="<?= \Yii::t('plugins/diy', '选择分类');?>">
                            <el-button size="mini"><?= \Yii::t('plugins/diy', '选择分类');?></el-button>
                        </app-dialog-select>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('plugins/diy', '专题列表');?>" v-else>
        ...
        computed: {
            cList() {
                if (this.data.list && this.data.list.length > 0 && this.data.cat_show) {
                    ...
                    let catList = {
                        cat_id: 0,
                        cat_name: '<?= \Yii::t('plugins/diy', '分类名称');?>',
                        name: '<?= \Yii::t('plugins/diy', '分类名称');?>',
                        children: [],
                        custom: 0,
                        number: 0
                    };
            ...
            cTopic() {
                if (this.data.list && this.data.list.length > 0
                    ...
                    let topic = {
                        title: '<?= \Yii::t('plugins/diy', '这是一条专题示例');?>',
                        cover_pic: '',
                        read_count: '999',
                        layout: 0,
                        id: 0
