    ...
    <div class="diy-modal">
        <div class="diy-component-preview">
            <div @click="dialogVisible=true" style="padding: 20px 0;text-align: center;">
                <div><?= \Yii::t('plugins/diy', '弹窗广告设置');?></div>
                <div style="font-size: 22px;color: #909399"><?= \Yii::t('plugins/diy', '本条内容不占高度');?></div>
            </div>
        </div>
        <div class="diy-component-edit">
            <el-dialog title="<?= \Yii::t('plugins/diy', '弹窗广告设置');?>" :visible.sync="dialogVisible" :close-on-click-modal="false">
                <div flex="box:first">
                    <div>
                    ...
                    <div>
                        <el-form @submit.native.prevent>
                            ...
                            <el-form-item v-if="data.opened" label="<?= \Yii::t('plugins/diy', '广告图片');?>">
                                <span style="color: #909399;"><?= \Yii::t('plugins/diy', '图片最大高度700px，最大宽度650px');?></span>
                                <br>
                                <div style="border: 1px solid #e2e2e2;padding: 5px;margin-bottom: 5px;line-height: normal;max-width: 400px" v-for="(item,index) in data.list" v-if="data.list.length">
                                    ...
                                    <div flex="box:first">
                                        ...
                                        <div class="chooseLink">
                                            <div style="margin: 5px 0;"><?= \Yii::t('plugins/diy', '广告链接');?></div>
                                            <div @click="pickLinkClick(index)">
                                                <el-input v-model="item.link.url" placeholder="<?= \Yii::t('plugins/diy', '点击选择链接');?>" :disabled="true"
                                                              size="small">
                                ...
                                <el-button size="small" @click="addAd" v-if="data.list.length<3"><?= \Yii::t('plugins/diy', '添加广告');?></el-button>
                            </el-form-item>
                            <el-form-item v-if="data.opened" label="<?= \Yii::t('plugins/diy', '弹窗次数');?>">
                                <app-radio v-model="data.times" :label="0"><?= \Yii::t('plugins/diy', '每次');?></app-radio>
                                <app-radio v-model="data.times" :label="1"><?= \Yii::t('plugins/diy', '仅首次');?></app-radio>
                            </el-form-item>
                            <el-button style="padding: 9px 25px" v-if="data.opened" size="small" type="primary" @click="dialogVisible = false"><?= \Yii::t('plugins/diy', '保存');?></el-button>
                        </el-form>
                    </div>
                </div>
            </el-dialog>
