...
<template id="diy-module">
    <div class="diy-module">
        <div class="diy-component-preview">
            ...
            <div v-else>
                <div style="padding: 50px 0;text-align: center;background: #fff;"><?= \Yii::t('plugins/diy', '请选择自定义模块');?></div>
            </div>
        </div>
        <div class="diy-component-edit">
            <el-form label-width="120px">
                <template v-if="data.list && data.list.length > 1">
                    <el-form-item label="<?= \Yii::t('plugins/diy', '标签类型');?>">
                        <div flex="dir:left">
                            <div flex="dir:top cross:center main:center"
                                ...
                                <div :style="{color: `${data.modeType == 'line' ? '#409EFF':'#666666'}`}"><?= \Yii::t('plugins/diy', '线条标签');?></div>
                            </div>
                            <div flex="dir:top cross:center main:center"
                                ...
                                <div :style="{color: `${data.tabType == 'filling' ? '#409EFF':'#666666'}`}"><?= \Yii::t('plugins/diy', '填充标签');?></div>
                            </div>
                        </div>
                    </el-form-item>
                    <el-form-item label="<?= \Yii::t('plugins/diy', '标签颜色');?>">
                    ...
                    <el-form-item label="<?= \Yii::t('plugins/diy', '未选中文字颜色');?>">
                    ...
                    <el-form-item label="<?= \Yii::t('plugins/diy', '自定义模块');?>">
                                ...
                                <div flex="dir:left box:first cross:center">
                                    <div style="flex-grow: 1;max-width: 100%">
                                        <el-input class="item-drag"
                                                  v-model="nav.tabName"
                                                  placeholder="<?= \Yii::t('plugins/diy', '请输入名称，最多输入5个字');?>"
                                                  size="small"
                                        ...
                                        <div v-else>
                                            <el-input v-model="nav.url" placeholder="<?= \Yii::t('plugins/diy', '点击选择模块');?>" readonly size="small"
                                                      disabled>
                                                <div slot="append" @click="edit(index)">
                                                    <el-button size="small"><?= \Yii::t('plugins/diy', '选择模块');?></el-button>
                                                </div>
                                            </el-input>
                                        </div>
                                    </div>
                        ...
                        <el-button v-if="data.list.length < maxNum" class="add-btn" type="primary" @click="addTab"
                                   size="small" plain>+<?= \Yii::t('plugins/diy', '添加一个模块');?>
                        </el-button>
                        <div style="color: #666666;font-size: 12px;"><?= \Yii::t('plugins/diy', '最多添加');?>{{maxNum}}<?= \Yii::t('plugins/diy', '个,拖拽可改变顺序');?></div>
                    </el-form-item>
                </template>
                <template v-else>
                    <el-form-item label="<?= \Yii::t('plugins/diy', '自定义模块');?>">
                        <div v-for="(item,index) in data.list" :key="index">
                            <div flex="dir:left" style="margin-bottom: 12px">
                                <div class="box">
                                    <?= \Yii::t('plugins/diy', '模块');?>: {{item.name}}
                                </div>
                                <el-button type="text" @click="edit(index)"><?= \Yii::t('plugins/diy', '修改');?></el-button>
                            </div>
                        </div>
                        <el-button class="add-btn" type="primary" @click="addTab" size="small" plain>+<?= \Yii::t('plugins/diy', '添加一个模块');?></el-button>
                    </el-form-item>
                </template>
            </el-form>

            <el-dialog title="<?= \Yii::t('plugins/diy', '自定义模块');?>" :visible.sync="dialogVisible">
                <!--工具条 表单搜索-->
                <el-form size="small" :inline="true" :model="search" @submit.native.prevent>
                    <el-form-item style="margin-bottom: 20px;width: 100%">
                        <el-input @keyup.enter.native="searchModule" placeholder="<?= \Yii::t('plugins/diy', '根据名称搜索');?>" style="width: 100%"
                                  v-model="search.keyword">
                            <template slot="append">
                                <el-button @click="searchModule" :loading="listLoading"><?= \Yii::t('plugins/diy', '搜索');?></el-button>
                            </template>
                        </el-input>
                    </el-form-item>
                </el-form>
                ...
                <el-table :data="moduleList" style="width: 100%" v-loading="listLoading">
                    ...
                    <el-table-column prop="name" label="<?= \Yii::t('plugins/diy', '标题');?>"></el-table-column>
                    <el-table-column prop="created_at" label="<?= \Yii::t('plugins/diy', '创建时间');?>" width="250"></el-table-column>
                </el-table>
                ...
                <div slot="footer">
                    <el-button size="small" @click="submit" type="primary"><?= \Yii::t('plugins/diy', '确定');?></el-button>
                </div>
            </el-dialog>
        </div>
    </div>
...
<script>
    Vue.component('diy-module', {
        ...
        methods: {
            ...
            formatLabel(column) {
                return "<?= \Yii::t('plugins/diy', '模块');?>: " + column.name;
            },
            edit(index) {
            ...
            submit() {
                ...
                if (!sentinel) {
                    this.$message.error("<?= \Yii::t('plugins/diy', '请选择模块');?>");
                    return;
                }
