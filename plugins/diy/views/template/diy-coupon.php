    ...
    <div>
            ...
            <div class="diy-coupon" flex="dir:left">
                <div v-for="(coupon,index) in couponList" :style="[couponBox]" :class="{receive: index > 2}">
                    ...
                    <div v-if="couponList.length === 1" flex="dir:left" style="height: 100%">
                        ...
                        <div style="width: 71%;font-size: 20px;padding-left: 25px" flex="cross:center"><?= \Yii::t('plugins/diy', '满200元可用');?></div>
                        <div style="width: 20%" class="right" flex="main:center cross:center"><?= \Yii::t('plugins/diy', '立即领取');?></div>
                    </div>
                    <div v-if="couponList.length === 2 || couponList.length === 3" flex="dir:left" style="height: 100%">
                        <div style="text-align: center;width: 75%">
                            ...
                            <div style="height: 50%;font-size: 20px;padding-top: 15px"><?= \Yii::t('plugins/diy', '满200元可用');?></div>
                        </div>
                        <div style="flex-grow: 1" flex="main:center cross:center">
                            <div class="right" flex="main:center cross:center">{{index > 2 ? `<?= \Yii::t('plugins/diy', '已领取');?>` : `<?= \Yii::t('plugins/diy', '立即领取');?>`}}</div>
                        </div>
                    </div>
        ...
        <div class="diy-component-edit">
            <el-form label-width="100px" @submit.native.prevent>
                <el-form-item label="<?= \Yii::t('plugins/diy', '添加方式');?>">
                    <app-radio v-model="data.addType" label="manual"><?= \Yii::t('plugins/diy', '手动添加');?></app-radio>
                    <app-radio v-model="data.addType" label=""><?= \Yii::t('plugins/diy', '自动添加');?></app-radio>
                    <div style="width: 385px" v-if="data.addType === `manual`">
                        <div v-for="(coupon,index) in data.coupons" class="coupon-box">
                            <div>
                                <span><?= \Yii::t('plugins/diy', '优惠券');?> {{coupon.name}} (</span>
                                <span v-if="coupon.appoint_type == 1"><?= \Yii::t('plugins/diy', '指定商品类目');?>,</span>
                                <span v-if="coupon.appoint_type == 2"><?= \Yii::t('plugins/diy', '指定商品');?>,</span>
                                <span v-if="coupon.appoint_type == 3"><?= \Yii::t('plugins/diy', '全场通用');?></span>
                                <span v-if="coupon.appoint_type == 4"><?= \Yii::t('plugins/diy', '当面付');?></span>
                                <span v-if="coupon.type == 2 && coupon.min_price > 0"><?= \Yii::t('plugins/diy', '满');?>{{coupon.min_price}}<?= \Yii::t('plugins/diy', '减');?>{{coupon.sub_price}}）</span>
                                <span v-else-if="coupon.type == 2"><?= \Yii::t('plugins/diy', '减');?>{{coupon.sub_price}}）</span>
                                <span v-else-if="coupon.type == 1 && coupon.min_price > 0"><?= \Yii::t('plugins/diy', '满');?>{{coupon.min_price}}<?= \Yii::t('plugins/diy', '打');?>{{coupon.discount}}<?= \Yii::t('plugins/diy', '折');?>）</span>
                                <span v-else-if="coupon.type == 1">{{coupon.discount}}<?= \Yii::t('plugins/diy', '折');?>）</span>
                            </div>
                        ...
                        <app-select-coupon-two v-model="data.coupons"
                                               ...
                                               :is-join="1"
                                               :max-num="10">
                            <el-button style="width: 385px" size="mini" type="primary" plain class="add-btn">+ <?= \Yii::t('plugins/diy', '添加优惠券');?></el-button>
                        </app-select-coupon-two>
                        <span style="margin-top: 10px;color:#666666;font-size: 12px"><?= \Yii::t('plugins/diy', '最多添加10张优惠券');?></span>
                    </div>
                </el-form-item>
                <el-form-item v-if="data.addType === ``" label="<?= \Yii::t('plugins/diy', '优惠券数量');?>">
                    <app-radio v-model="data.has_limit" label=""><?= \Yii::t('plugins/diy', '全部');?></app-radio>
                    <app-radio v-model="data.has_limit" label="limit">
                        <el-input placeholder="<?= \Yii::t('plugins/diy', '请输入优惠券数量');?>" oninput="this.value = this.value.replace(/[^0-9]/g, '');"
                                  maxlength="10"
                ...
                <el-form-item label="<?= \Yii::t('plugins/diy', '背景颜色');?>">
                    ...
                    <el-input size="small" style="width: 80px;margin-right: 25px;" v-model="data.couponBg"></el-input>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/diy', '背景效果');?>">
                    <app-radio v-model="data.couponBgType" label="pure"><?= \Yii::t('plugins/diy', '纯色');?></app-radio>
                    <app-radio v-model="data.couponBgType" label="gradient"><?= \Yii::t('plugins/diy', '渐变');?></app-radio>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/diy', '隐藏券');?>">
                    <el-checkbox v-model="data.has_hide"><?= \Yii::t('plugins/diy', '隐藏已领完券');?></el-checkbox>
                </el-form-item>
                <el-form-item v-if="false" label="<?= \Yii::t('plugins/diy', '不可领取');?>">
                    <app-attachment title="<?= \Yii::t('plugins/diy', '选择图片');?>" :multiple="false" :max="1" type="image"
                                    v-model="data.receiveBg">
                        <el-tooltip class="item" effect="dark"
                                    content="<?= \Yii::t('plugins/diy', '建议尺寸');?>:256*130"
                                    placement="top">
                ...
                <el-form-item v-if="false" label="<?= \Yii::t('plugins/diy', '可领取');?>">
                    <app-attachment title="<?= \Yii::t('plugins/diy', '选择图片');?>" :multiple="false" :max="1" type="image"
