...
<div id="app" v-cloak>
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header" flex="dir:left" style="justify-content:space-between;">
            <span><?= \Yii::t('plugins/diy', '微页面');?></span>
            <app-market list-url="plugin/diy/mall/template/market-search"
                        edit-url="plugin/diy/mall/template/edit" type="page">
                <el-button style="margin: -5px 0" type="primary" size="small"><?= \Yii::t('plugins/diy', '新建微页面');?>
                </el-button>
            </app-market>
        </div>
        ...
        <div class="table-body">
            <el-table v-loading="listLoading" :data="list" border>
                ...
                <el-table-column prop="name" label="<?= \Yii::t('plugins/diy', '名称');?>">
                ...
                <el-table-column :formatter="formatterCount" label="<?= \Yii::t('plugins/diy', '浏览人数');?>"></el-table-column>
                <el-table-column prop="created_at" label="<?= \Yii::t('plugins/diy', '创建时间');?>"></el-table-column>
                <el-table-column fixed="right" width="250" label="<?= \Yii::t('plugins/diy', '操作');?>">
                    <template slot-scope="scope">
                        ...
                        <el-button type="text" @click="noticePoster(scope.row)"
                                   size="small" circle>
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/diy', '推广');?>" placement="top">
                                <img src="statics/img/mall/notice.png" alt="">
                            </el-tooltip>
                        </el-button>
                        <el-button type="text" @click="settingIndex(scope.row)" size="small" circle>
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/diy', '设为首页');?>" placement="top">
                                <img src="statics/img/mall/list_icon_home.png" alt="">
                            </el-tooltip>
                        </el-button>
                        <el-button type="text" @click="openBrowse(scope.row)" size="small" circle>
                            <el-tooltip class="item" effect="dark" content="<?= \Yii::t('plugins/diy', '设置浏览权限');?>" placement="top">
        ...
        <el-dialog title="<?= \Yii::t('plugins/diy', '设置浏览权限');?>" :visible.sync="browseVisible" width="30%">
            <!-- 选择会员等级 -->
            <el-dialog class="app-setting-dialog" title="<?= \Yii::t('plugins/diy', '选择会员等级');?>" :visible.sync="m_visible" width="30%"
                       @submit.native.prevent append-to-body>
                <el-form @submit.native.prevent size="small">
                    <div class="input-item">
                        <el-input @keyup.enter.native="m_toSearch" size="small" placeholder="<?= \Yii::t('plugins/diy', '请输入会员等级名称');?>"
                ...
                <div class="no-list-tip" v-if="m_list.length == 0 && !m_listLoading"><?= \Yii::t('plugins/diy', '暂无会员等级');?></div>
                <el-pagination
                        v-if="m_pagination"
                ...
                <span slot="footer" class="dialog-footer">
                    <el-button size="small" @click="m_visible = false"><?= \Yii::t('plugins/diy', '取消');?></el-button>
                    <el-button size="small" type="primary" @click="submitMember"><?= \Yii::t('plugins/diy', '确定');?></el-button>
                 </span>
            </el-dialog>

            <el-form ref="browseForm" :model="browseForm" label-width="130px" size="small">
                <el-form-item label="<?= \Yii::t('plugins/diy', '会员等级浏览权限');?>">
                    ...
                    <el-button v-if="browseForm.access_limit.is_all == 0" size="small" @click="openMemberDialog">
                        <?= \Yii::t('plugins/diy', '选择会员等级');?>
                    </el-button>
                    <el-checkbox style="margin-left: 10px" v-model="browseForm.access_limit.is_all" :true-label="1"
                                 @change="changeAccessLimit"
                                 :false-label="0">
                        <?= \Yii::t('plugins/diy', '所有用户');?>
                    </el-checkbox>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button size="small" @click="browseVisible = false"><?= \Yii::t('plugins/diy', '取消');?></el-button>
                <el-button size="small" type="primary" @click="browseSubmit" :loading="browseBtnLoading"><?= \Yii::t('plugins/diy', '确定');?></el-button>
            </div>
        </el-dialog>

        <!-- 推广 -->
        <el-dialog title="<?= \Yii::t('plugins/diy', '推广');?>" :visible.sync="bgVisible" width="956px">
            <div flex="dir:left" v-loading="bgLoading">
                ...
                <el-form label-position="top" label-width="80px" style="margin-left: 50px">
                    <el-form-item label="<?= \Yii::t('plugins/diy', '复制小程序路径');?>">
                        <el-input size="small" style="width: 500px" v-model="bgPosterForm.text" disabled>
                            <template slot="append">
                                <el-button type="primary" @click="copyInput"><?= \Yii::t('plugins/diy', '复制');?></el-button>
                            </template>
                        </el-input>
                        <div>
                            <a style="color:#409EFF;text-decoration:none"
                               :href="bgPosterForm.picUrl"
                               :download="downloadText"><?= \Yii::t('plugins/diy', '下载海报');?></a>
                        </div>
                    </el-form-item>
                </el-form>
            </div>
        ...
        methods: {
            ...
            copyInput() {
                if (this.copyText(this.bgPosterForm.text)) {
                    this.$message.success('<?= \Yii::t('plugins/diy', '复制成功');?>');
                } else {
                    this.$message.error('<?= \Yii::t('plugins/diy', '复制失败');?>');
                }
            },
            ...
            destroy(column, index) {
                this.$confirm('<?= \Yii::t('plugins/diy', '此操作将删除该微页面');?>', '<?= \Yii::t('plugins/diy', '提示');?>', {
                    ...
                    this.$message({
                        type: 'info',
                        message: '<?= \Yii::t('plugins/diy', '已取消删除');?>'
                    });
                });
            },
            pageChange(page) {
