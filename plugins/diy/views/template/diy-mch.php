    ...
    <div class="diy-mch">
                ...
                <div class="mch-item" :style="cItemStyle" v-for="(mch,mchIndex) in cMchList">
                    <div flex="box:justify cross:center">
                        ...
                        <div>
                            ...
                            <div class='mch-info'>
                                <span><?= \Yii::t('plugins/diy', '商品数');?>:{{mch.goodsNum}}</span>
                                <span style="margin-left: 10px"><?= \Yii::t('plugins/diy', '已售');?> {{mch.orderNum}}</span>
                            </div>
                        </div>
                        <div>
                            <div class="to-look"><?= \Yii::t('plugins/diy', '进店逛逛');?></div>
                        </div>
                    </div>
                    ...
                    <div v-else class="goods-list" flex="main:center cross:center"
                         style="height: 100px;color: #909399;"><?= \Yii::t('plugins/diy', '暂无商品');?>
                    </div>
                </div>
            </div>
        </div>
        <div class="diy-component-edit">
            <el-form @submit.native.prevent label-width="100px">
                <el-form-item label="<?= \Yii::t('plugins/diy', '显示商品');?>">
                    <el-switch v-model="data.showGoods"></el-switch>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/diy', '商户列表');?>">
                    <div class="edit-item" v-for="(mch,mchIndex) in data.list">
                        ...
                        <div flex="box:first">
                            <div style="width: 95px;"><?= \Yii::t('plugins/diy', '商户名称');?></div>
                            <div>{{mch.name}}</div>
                        </div>
                        <template v-if="data.showGoods">
                            <div flex="box:first" v-if="!mch.staticGoods">
                                <div style="width: 95px;"><?= \Yii::t('plugins/diy', '商品显示数量');?></div>
                                <div>
                                    <el-input size="small" v-model.number="mch.showGoodsNum" type="number"
                                              min="0" max="10"
                                              placeholder="<?= \Yii::t('plugins/diy', '最多10个');?>" @change="goodsNumChange(mchIndex)"></el-input>
                                </div>
                            </div>
                            <div flex="box:first">
                                <div style="width: 95px;"><?= \Yii::t('plugins/diy', '自定义商品');?></div>
                                <div>
                                    <el-switch v-model="mch.staticGoods"></el-switch>
                                </div>
                            </div>
                            <div v-if="mch.staticGoods">
                                <div><?= \Yii::t('plugins/diy', '商品列表');?></div>
                    ...
                    <el-button size="small" @click="showMchDialog"><?= \Yii::t('plugins/diy', '添加商户');?></el-button>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/diy', '卡片样式');?>">
                    <el-radio v-model="data.cardStyle" :label="1"><?= \Yii::t('plugins/diy', '白底无边框');?></el-radio>
                    <el-radio v-model="data.cardStyle" :label="2"><?= \Yii::t('plugins/diy', '白底有边框');?></el-radio>
                    <el-radio v-model="data.cardStyle" :label="3"><?= \Yii::t('plugins/diy', '无底无边框');?></el-radio>
                </el-form-item>
                <diy-bg :hr="hr" :data="data" @update="updateData" @toggle="toggleData" @change="changeData"></diy-bg>
            </el-form>
        </div>
        <el-dialog title="<?= \Yii::t('plugins/diy', '选择商户');?>" :visible.sync="mchDialog.visible" @open="mchDialogOpened">
            <el-input size="mini" v-model="mchDialog.keyword" placeholder="<?= \Yii::t('plugins/diy', '根据名称搜索');?>" :clearable="true"
                      @clear="loadMchList" @keyup.enter.native="loadMchList">
                <el-button slot="append" @click="loadMchList"><?= \Yii::t('plugins/diy', '搜索');?></el-button>
            </el-input>
            <el-table :data="mchDialog.list" v-loading="mchDialog.loading" @selection-change="mchSelectionChange">
                <el-table-column type="selection" width="50px"></el-table-column>
                <el-table-column label="ID" prop="id" width="100px"></el-table-column>
                <el-table-column label="<?= \Yii::t('plugins/diy', '名称');?>" prop="store.name"></el-table-column>
            </el-table>
            ...
            <div slot="footer">
                <el-button type="primary" @click="addMch"><?= \Yii::t('plugins/diy', '确定');?></el-button>
            </div>
        </el-dialog>
