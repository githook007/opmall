...
<div id="app" v-cloak>
    <el-card shadow="never" style="margin-bottom: 10px;"><?= \Yii::t('plugins/diy', '授权自定义');?></el-card>
        ...
        <div class="right">
            <el-form ref="ruleForm" :model="ruleForm" :rules="rules" label-width="100px" size="small">
                <el-card shadow="never" v-loading="loading">
                    <div slot="header"><?= \Yii::t('plugins/diy', '设置');?></div>
                    <el-form-item prop="is_open" label="<?= \Yii::t('plugins/diy', '是否开启');?>">
                        <el-switch
                    ...
                    <el-form-item prop="pic_url" label="<?= \Yii::t('plugins/diy', '上传图片');?>">
                        <app-attachment :multiple="false" :max="1" @selected="selectPic" tag="auth_page">
                            <el-tooltip effect="dark" content="<?= \Yii::t('plugins/diy', '推荐尺寸650');?>" placement="bottom">
                                <el-button><?= \Yii::t('plugins/diy', '选择图片');?></el-button>
                            </el-tooltip>
                        </app-attachment>
                    ...
                    <el-form-item prop="pic_url" label="<?= \Yii::t('plugins/diy', '热区');?>">
                        <div flex="dir:left">
                            <app-hotspot :multiple="true" width="650px" height="700px" @confirm="confirm"
                                ...
                                <el-button><?= \Yii::t('plugins/diy', '划分热区');?></el-button>
                            </app-hotspot>
                        </div>
                        <div><?= \Yii::t('plugins/diy', '需要划分两个热区');?></div>
                    </el-form-item>
                </el-card>
                <el-button class="button-item" type="primary" size="small" :loading="submitLoading"
                           @click="submit('ruleForm')">
                    <?= \Yii::t('plugins/diy', '保存');?>
                </el-button>
            </el-form>
        </div>
    </el-row>
...
<script>
    ...
    const app = new Vue({
        ...
        methods: {
            ...
            // 划分热区
            confirm(e) {
                this.ruleForm.hotspot = '';
                this.ruleForm.hotspot_cancel = '';
                this.ruleForm.hotspot_link = '';
                for (let i of e) {

                    if (i.open_type == 'login') {
                        this.ruleForm.hotspot = i;
                    }
                    if (i.open_type == 'cancel') {
                        this.ruleForm.hotspot_cancel = i;
                    }
                    if(i.open_type == 'link') {
                        this.ruleForm.hotspot_link = i;
                    }
                }
            },
            // 删除图片
            ...
            submit(formName) {
                if (!this.ruleForm.hotspot) {
                    this.$message.error("<?= \Yii::t('plugins/diy', '请先选择登录按钮热区');?>");
                    return ;
                }
                if (!this.ruleForm.hotspot_cancel) {
                    this.$message.error("<?= \Yii::t('plugins/diy', '请先选择不登录按钮热区');?>");
                    return ;
                }
            ...
            hotspot() {
                ...
                if (this.ruleForm.hotspot_link) {
                    res.push(this.ruleForm.hotspot_link);
                }
                return res;
            }
        }
