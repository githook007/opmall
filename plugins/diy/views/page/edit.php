    ...
    <el-card shadow="never" style="border:0" body-style="background-color: #f3f3f3;padding: 10px 0 0;">
        <div slot="header">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item><span style="color: #409EFF;cursor: pointer" @click="$navigate({r:'plugin/diy/mall/page/index'})"><?= \Yii::t('plugins/diy', '自定义页面');?></span></el-breadcrumb-item>
                <el-breadcrumb-item v-if="form.id > 0"><?= \Yii::t('plugins/diy', '编辑');?></el-breadcrumb-item>
                <el-breadcrumb-item v-else><?= \Yii::t('plugins/diy', '新增页面');?></el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class='table-body'>
            <el-form @submit.native.prevent :model="form" :rules="rules" ref="form" v-loading="loading" label-width="150px">
                <el-form-item label="<?= \Yii::t('plugins/diy', '标题');?>" prop="title">
                    <el-input size='small' v-model="form.title"></el-input>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/diy', '启用');?>">
                    <el-switch v-model="form.is_disable" inactive-value="1" active-value="0">
                    </el-switch>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/diy', '显示导航条');?>">
                    <el-switch v-model="form.show_navs" inactive-value="0" active-value="1">
                    </el-switch>
                </el-form-item>
                <el-form-item label="<?= \Yii::t('plugins/diy', '模板和导航');?>" prop="navs">
                    <div style="max-width: 480px">
                        <template v-for="(nav,index) in form.navs">
                            <el-card shadow="never" style="margin-bottom: 10px;">
                                <div flex="box:last">
                                    <el-form label-width="80px" ref="navForm">
                                        <el-form-item label="<?= \Yii::t('plugins/diy', '导航名称');?>" style="margin-bottom: 20px">
                                            <el-input size='small' v-model="nav.name"></el-input>
                                        </el-form-item>
                                        <el-form-item label="<?= \Yii::t('plugins/diy', '对应模板');?>">
                                            <el-input
                                                    size='small'
                                                    readonly
                                                    :value="nav.template_id?('#'+nav.template_id+':'+nav.template_name):''">
                                                <template slot="append">
                                                    <el-button size='small' @click="selectTemplate(index)"><?= \Yii::t('plugins/diy', '选择模板');?></el-button>
                                                </template>
                                            </el-input>
                                        </el-form-item>
                                    </el-form>
                                    <div style="padding-left: 15px">
                                        <el-tooltip effect="dark" content="<?= \Yii::t('plugins/diy', '删除导航');?>" placement="bottom">
                        ...
                        <el-button v-if="form.navs.length < 10" @click="addNav" type='text'>
                            ...
                            <span style='color: #353535'><?= \Yii::t('plugins/diy', '新增模版和导航');?></span>
                        </el-button>
                    </div>
                </el-form-item>
            </el-form>
        </div>
        <el-button class='button-item' @click="submit('form')" :loading="submitLoading" type="primary"><?= \Yii::t('plugins/diy', '保存');?></el-button>
    </el-card>

    <el-dialog title="<?= \Yii::t('plugins/diy', '选择模板');?>" :visible.sync="showTemplateDialog" @open="templateDialogOpen">
        <el-table v-loading="template.loading" :data="template.list" style="margin-bottom: 20px">
            <el-table-column label="ID" prop="id" width="100px"></el-table-column>
            <el-table-column label="<?= \Yii::t('plugins/diy', '模板名称');?>" prop="name"></el-table-column>
            <el-table-column label="<?= \Yii::t('plugins/diy', '操作');?>" width="100px">
                <template slot-scope="scope">
                    <el-button @click="selectTemplateItem(scope.row)" size="mini"><?= \Yii::t('plugins/diy', '选择');?></el-button>
                </template>
            </el-table-column>
        </el-table>
    ...
    new Vue({
        ...
        data() {
            const validateNavs = (rule, value, callback) => {
                for (let i in value) {
                    if (!value[i].name || !value[i].template_id) {
                        return callback(new Error("<?= \Yii::t('plugins/diy', '导航名称和对应模板不能为空');?>"));
                    }
                }
                return callback();
            };
            return {
                ...
                rules: {
                    title: [
                        {required: true, message: "<?= \Yii::t('plugins/diy', '请输入标题');?>"},
                    ],
                    navs: [
                        {required: true, message: "<?= \Yii::t('plugins/diy', '至少添加一个导航');?>"},
