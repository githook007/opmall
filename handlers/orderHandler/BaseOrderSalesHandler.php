...
abstract class BaseOrderSalesHandler extends BaseOrderHandler
{
     ...
     * @czs
     */
    protected function memberAutoLevel()
    {
        try {
            \Yii::warning('购买商品，会员升级');
            $goodsId = [];
            ...
            $memberGoods = MallMemberGoods::find()->where([
                ...
                'mall_id' => $this->event->order->mall_id
            ])->select("member_id");

            $nowLevel = $this->user->identity->member_level;
            $query = MallMembers::find()
                ->where(['mall_id' => $this->event->order->mall_id, 'is_delete' => 0, 'status' => 1, 'condition_type' => 2])
                ->andWhere(['>', 'level', $nowLevel])->andWhere(['id' => $memberGoods])
            ...
            \Yii::warning($sql->createCommand()->getRawSql());
            \Yii::warning('购买商品自动升级会员等级处理成功');
        } catch (\Exception $exception) {
            \Yii::warning('购买商品自动升级会员等级错误' . $exception->getMessage());
        }
        return $this;
    }
    ...
    protected function memberLevel($orderMoneyCount)
    {
        try {
            ...
            $nextLevel = MallMembers::find()
                ->where(['mall_id' => $this->order->mall_id, 'is_delete' => 0, 'auto_update' => 1, 'status' => 1, 'condition_type' => 1])
    ...
    protected function shareLevel($orderMoneyCount)
    {
        try {
            ...
            $commonShareLevel->user = $this->event->order->user;
     ...
     * @czs
     */
    protected function upShare()
    {
        try {
            $commonShareLevel = CommonShareLevel::getInstance();
            $commonShareLevel->user = $this->event->order->user;
            $commonShareLevel->levelShareByBuyGoods($this->event->order, 2);
            \Yii::warning('购买商品升级分销商等级处理成功');
        } catch (\Exception $exception) {
            \Yii::error('购买商品升级分销商等级: ' . $exception->getMessage());
        }
        return $this;
    }
