...
abstract class BaseOrderCanceledHandler extends BaseOrderHandler
{
    ...
    {
        $this->integralResume()->couponResume()->refund()->cardResume()->couponDestrory()
            ...
            $exists = Order::find()
                ->where([
                    ...
                    [
                        ...
                        'use_user_coupon_id' => $this->event->order->use_user_coupon_id,
                    ],
    ...
    protected function couponDestrory()
    {
        /** @var CouponMallRelation[] $userCoupons */
        $userCoupons = CouponMallRelation::find()->with('userCoupon')->where(['order_id' => $this->event->order->id, 'is_delete' => 0])->all();
        foreach ($userCoupons as $userCoupon) {
            if(!$userCoupon->userCoupon || $userCoupon->userCoupon->is_use){
                continue;
            }
            $userCoupon->userCoupon->is_delete = 1;
            $userCoupon->userCoupon->coupon->updateCount(1, 'add');
            $res = $userCoupon->userCoupon->save();
            if (!$res) {
                \Yii::error('优惠券销毁事件处理异常：'.var_export($userCoupon->userCoupon->getFirstErrors()));
            }
