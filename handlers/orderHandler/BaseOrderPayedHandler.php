...
abstract class BaseOrderPayedHandler extends BaseOrderHandler
{
    ...
    {
        if(\Yii::$app->mall->getMallSettingOne("goods_coupon_usage_scenario") != 1){
            return [];
        }
    ...
    protected function sendSms()
    {
        try {
            ...
            $sms = new Sms(['mch_id' => $this->event->order->mch_id, 'sms_config' => $smsConfig]);
    ...
    protected function upShare()
    {
        try {
            ...
            $commonShareLevel->levelShareByBuyGoods($this->event->order, 1);
    ...
    protected function sendTemplate()
    {
        try {
            ...
            foreach ($order->detail as $orderDetail) {
                $goodsInfo = $orderDetail->decodeGoodsInfo();
                $goodsName .= $goodsInfo['goods_attr']['name'] ?? $orderDetail->goods->name;
            }
    ...
    protected function sendMpTemplate()
    {
        ...
        try {
            ...
            $tplMsg->params = [
                ...
                'time' => $this->event->order->pay_time,
                'is_pay' => $this->event->order->is_pay,
            ];
