...
class ModuleSms extends Component
{
    ...
    {
        try {
            $message->setGateways($this->easySms->getConfig()['gateways']);
            $res = $this->easySms->send($mobile, $message);
            $res = array_column($res, "result");
            if(!empty($res[0]['ActionStatus']) && $res[0]['ActionStatus'] == 'FAIL'){
                throw new \Exception($res[0]['ErrorInfo'] ?? '');
            }
