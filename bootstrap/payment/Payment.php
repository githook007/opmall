...
class Payment extends Component
{
    ...
    public function createOrder($paymentOrders, $user = null)
    {
        ...
        $paymentOrderUnion->is_profit_sharing = \Yii::$app->mall->getMallSettingOne("profit_sharing");
        if ($appVersion) {
...
//                    $appendPayTypes[] = static::PAY_TYPE_UNIONPAY;
    ...
    // @czs
    public function getCheckPayData($id, $payType){
        ...
        foreach ($paymentOrderUnion->paymentOrder as $paymentOrder){
            if($paymentOrder->order->is_pay == 1){
                throw new PaymentException('订单已支付', 200);
            }
        }
    ...
    {
        $paymentOrderUnion = $this->getCheckPayData($id, $payType); // @czs
        ...
        switch ($payType) {
            ...
            case static::PAY_TYPE_WECHAT:
                if (\Yii::$app->appPlatform == 'app'){  // 判断app <@jayi>
                    $plugin = \Yii::$app->plugin->getPlugin('app');
                    ...
                    $payList = [
                        ...
                        'profit_sharing' => $paymentOrderUnion->is_profit_sharing ? 'Y' : 'N'
                    ];
                    $data = $wechatPay->unifiedOrder($payList);
                }else {
                    ...
                    try {
                        ...
                        $orderInfo = $plugin->getPayment($paymentOrderUnion);
                        if (empty($orderInfo)) {
                            throw new \Exception("");
                        }
                        $data = array_merge([
                        ...
                        if ($wechatPay->channel == 5) { // 通联支付
                            $transmitData = [
                                'body' => $paymentOrderUnion->title,
                                'out_trade_no' => $paymentOrderUnion->order_no,
                                'total_fee' => $paymentOrderUnion->amount,
                                'notify_url' => $this->getNotifyUrl($wechatPay::NOTIFY_URL),
                                'openid' => $user->username,
                            ];
                            $data = $wechatPay->unifiedOrder($transmitData);
                        }else{
    ...
    public function payBuyBalance($id, $pay_number = null, $params = [])
    {
        ...
        $user = $this->getUser();
        $paymentOrderUnion = $this->getCheckPayData($id, static::PAY_TYPE_BALANCE); // @czs
...
//        }
    ...
    {
        $paymentOrderUnion = $this->getCheckPayData($id, static::PAY_TYPE_CASH); // @czs
        $t = \Yii::$app->db->beginTransaction();
        try {
            /** @var \app\models\PaymentOrder[] $paymentOrders */
            $paymentOrders = $paymentOrderUnion->paymentOrder;
    ...
    {
        $paymentOrderUnion = $this->getCheckPayData($id, static::PAY_TYPE_POS); // @czs
        $t = \Yii::$app->db->beginTransaction();
        try {
            /** @var \app\models\PaymentOrder[] $paymentOrders */
            $paymentOrders = $paymentOrderUnion->paymentOrder;
    ...
    public function payBuyHuodao($id)
    {
        ...
        $paymentOrderUnion = $this->getCheckPayData($id, static::PAY_TYPE_HUODAO); // @czs
    ...
    private function refundClass($payType)
    {
        switch ($payType) {
            ...
            case 12:
                ...
                break;
            case 13: // czs 微信自定义交易组件退款
                $class = 'app\\plugins\\minishop\\forms\\RefundForm';
                break;
            case 14: // czs app退款
                $class = 'app\\plugins\\app\\forms\\common\\AppRefund';
                break;
            default:
    ...
    public function transfer($paymentTransfer)
    {
        ...
        $model = \app\models\PaymentTransfer::find()->where([
            'transfer_order_no' => $paymentTransfer->orderNo, 'user_id' => $paymentTransfer->user->id,
    ...
    private function transferClass($type)
    {
        switch ($type) {
            ...
            case PaymentTransfer::TRANSFER_TYPE_WECHAT_MP:
                ...
                break;
            case PaymentTransfer::TRANSFER_TYPE_APP:
                throw new PaymentException('app端暂不支持提现');
                break;
            default:
